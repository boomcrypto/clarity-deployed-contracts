...
(define-constant ERR_FATAL (err u1003))
(define-constant ERR_ROUTER_STATUS (err u1004))
(define-constant ERR_MINIMUM_OUTPUT (err u1005))
...
(define-public (route-a (amount uint) (min-output uint))
  (let (
    (a (unwrap! (bitflow-a amount) ERR_FATAL))
    (b (unwrap! (alex-a a) ERR_FATAL))
  )
    (begin
...
(define-public (route-b (amount uint) (min-output uint))
  (let (
    (a (unwrap! (alex-b amount) ERR_FATAL))
    (b (unwrap! (bitflow-b a) ERR_FATAL))
  )
    (begin
      (asserts! (> amount u0) ERR_INVALID_AMOUNT)
      (asserts! (is-eq (var-get router-status) true) ERR_ROUTER_STATUS)
