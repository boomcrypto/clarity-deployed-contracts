---
title: "Trait miami-beach-volleyball"
draft: true
---
```
;; miami-beach-volleyball

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token miami-beach-volleyball uint)
...
(define-data-var artist-address principal 'SP2GNDY6AVZBEZAQ5R2BY04FYXTC23CHVE1216PK8)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qma7vFzXNDoxGwmorDMWtbD1xkakYces1nn8Tcadc8jfNa/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? miami-beach-volleyball next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? miami-beach-volleyball token-id sender recipient)))

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? miami-beach-volleyball token-id)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u6700)
