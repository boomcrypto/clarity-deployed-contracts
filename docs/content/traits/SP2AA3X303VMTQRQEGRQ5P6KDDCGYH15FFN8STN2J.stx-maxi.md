...
(define-non-fungible-token stx-maxi uint)
...
(define-data-var artist-address principal 'SP2AA3X303VMTQRQEGRQ5P6KDDCGYH15FFN8STN2J)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQfoEQwMbjpLpvhLNg6xmk2kzsSspL6f1ekCUFvVTLeEK/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stx-maxi next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-maxi token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-maxi token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-maxi token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-maxi id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-maxi id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-maxi id) (err ERR-NOT-FOUND)))
