---
title: "Trait cross-peg-out-v2-01c-agg"
draft: true
---
```
...
(define-public (transfer-to-swap (amount-in-fixed uint) (token-in-trait <ft-trait>) (token-out principal) (min-amount-out (optional uint)) (dest-chain-id uint) (expiry uint) (success-settle { address: (buff 256), chain-id: (optional uint), token: principal }) (fail-settle { address: (buff 256), chain-id: (optional uint), token: principal }))
  (let (
      (sender tx-sender)
    ...
    (print { object: "cross-bridge-endpoint", action: "transfer-to-swap", chain: (get name chain-details), dest-chain-id: dest-chain-id, net-amount: net-amount, fee-amount: fee, success-settle: success-settle, fail-settle: fail-settle, token-in: token-in, token-out: token-out, min-amount-out: min-amount-out, expiry: expiry })
    (ok true)))
