...
(define-non-fungible-token spermofgoblin uint)
...
(define-data-var artist-address principal 'SPGR8BEZJG2ZDSHX0G0TM7GZV9TBC2QRQZDQFRT)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeK82XnDHJ4e7TwAgPLJVUG53ieUch1hjAr4LBdTEdToQ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? spermofgoblin next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? spermofgoblin token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? spermofgoblin token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? spermofgoblin token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? spermofgoblin id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? spermofgoblin id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? spermofgoblin id) (err ERR-NOT-FOUND)))
