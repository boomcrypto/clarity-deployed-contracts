---
title: "Trait aeusdc-susdt-lp-token-v-1-2"
draft: true
---
```
;; aeusdc-susdt-lp-token
...
(define-fungible-token aeusdc-susdt-lpt)
...
(define-data-var approved-supply-controller principal 'SPQC38PW542EQJ5M11CR25P7BS1CA6QT4TBXGB3M.stableswap-aeusdc-susdt-v-1-2)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance aeusdc-susdt-lpt account))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply aeusdc-susdt-lpt))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? aeusdc-susdt-lpt amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint (who principal) (amount uint))
  (begin
    ...
    (ft-mint? aeusdc-susdt-lpt amount who)
  )
)
...
(define-public (burn (burner principal) (amount uint))
  (begin
    ...
    (ft-burn? aeusdc-susdt-lpt amount burner)
  )
)
