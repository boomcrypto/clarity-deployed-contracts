---
title: "Trait leo-cat-collection"
draft: true
---
```
;; leo-cat-collection
...
(define-non-fungible-token leo-cat-collection uint)
...
(define-data-var artist-address principal 'SP5N7QWC3BGM58AV63VK1HDRN217DH8JK42M292G)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmU4XrwcG2FzBxpCy8XuuQfyXjy5BJzQEc924NUbZpuscg/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? leo-cat-collection next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? leo-cat-collection token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? leo-cat-collection token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? leo-cat-collection token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? leo-cat-collection id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? leo-cat-collection id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? leo-cat-collection id) (err ERR-NOT-FOUND)))
