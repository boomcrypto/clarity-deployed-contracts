---
title: "Trait meta-peg-in-v2-06e-swap"
draft: true
---
```
  ...
  (let (
    ...
    (and (> amt-net u0) (if (get no-burn token-details) 
      (let (
        ...
        (and (> peg-out-balance u0) (as-contract (try! (contract-call? token-trait transfer-fixed peg-out-balance tx-sender .meta-peg-out-endpoint-v2-04 none)))))
      (as-contract (try! (contract-call? token-trait mint-fixed amt-net tx-sender)))))
    (try! (update-peg-out-fee pair-tuple (var-get peg-out-fee) u0)) ;; % fee charged by relevant peg-out contract, but min/gas-fee charged upfront
