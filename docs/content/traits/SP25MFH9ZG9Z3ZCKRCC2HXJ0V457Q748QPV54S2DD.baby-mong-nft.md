---
title: "Trait baby-mong-nft"
draft: true
---
```
;; baby-mong-nft 1.0

;; (use-trait nft-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait) ;; For mainnet sip-009
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait) ;; For mainnet sip-009
;; (impl-trait .nft-trait.nft-trait) ;; sip-009
...
;; define a new NFT. Make sure to replace baby-mong-nft
(define-non-fungible-token baby-mong-nft uint)
...
(define-constant MINT-LIMIT u25)   
...
(define-data-var mint-price-wl  uint u1000000)  ;; 1.0 STX
(define-data-var mint-price-pub uint u1500000)  ;; 1.5 STX
(define-data-var WALLET-1 principal tx-sender)
(define-data-var WALLET-2 principal tx-sender)
(define-data-var WALLET-3 principal tx-sender)
(define-data-var base-uri (string-ascii 80) "ipfs://QmRCgoD2SkGmFJPLp9e7PGqPD1BboZbhCR5j59LC9VGveb/")
...
;; Vars for converting uint to string
...
(define-read-only (get-owner (token-id uint)) ;; SIP-009 
  (ok (nft-get-owner? baby-mong-nft token-id)))
...
(define-public (set-wallets (new-wallet-1 principal) (new-wallet-2 principal) (new-wallet-3 principal))
  (begin
    (asserts! (is-authorized) ERR-NOT-AUTHORIZED)    
    (var-set WALLET-1 new-wallet-1)
    (var-set WALLET-2 new-wallet-2)
    (var-set WALLET-3 new-wallet-3)
    (ok true)))
...
(define-public (mint-with-stx (target-wallet principal))
    (begin
        ...
        (let ((next-id (+ u1 (var-get last-id)))
            ...
            ;; (target-mint-id next-id)
            )              
            (match (nft-mint? baby-mong-nft target-mint-id target-wallet)
                success
...
(define-public (mint-vip (target-wallet principal))
    (begin 
        (asserts! (var-get is-minting-ready) ERR-FORBIDDEN)
        (asserts! (called-from-mint) ERR-NOT-AUTHORIZED)
        (asserts! (< (var-get last-id) MINT-LIMIT) ERR-SOLD-OUT)        
...
(define-public (transfer (id uint) (sender principal) (recipient principal)) ;; SIP-009 implementation
  (begin
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? baby-mong-nft id) ERR-NOT-FOUND))
...
(define-private (transferNFT (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? baby-mong-nft id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? baby-mong-nft id) false)))
