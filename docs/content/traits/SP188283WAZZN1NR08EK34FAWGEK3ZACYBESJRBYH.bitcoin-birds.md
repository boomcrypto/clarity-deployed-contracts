...
(define-public (claim-rewards (amountIn uint))
  (let (
    (b1 (unwrap-panic (swap-wstx-wusd-velar amountIn)))
    (b2 (unwrap-panic (swap-wxusd-wstx-alex b1)))
  )
    (print { b1: b1, b2: b2 })
    (begin
...
(define-public (accept-collection-bid (amountIn uint))
  (let (
    (b1 (unwrap-panic (swap-wstx-wxusd-alex amountIn)))
    (b2 (unwrap-panic (swap-wusd-wstx-velar b1)))
  )
    (print { b1: b1, b2: b2 })
    (begin
      (asserts! (> b2 amountIn) (err ERR-COOLDOWN))
...
(define-public (revoke-in-ustx (amountIn uint))
  (let (
    (b1 (unwrap-panic (swap-wstx-xbtc-velar amountIn)))
...
(define-public (send-many (amountIn uint))
  (let (
    ...
    (b2 (unwrap-panic (swap-xbtc-wstx-velar b1)))
  )
    (print { b1: b1, b2: b2 })
    (begin
      (asserts! (> b2 amountIn) (err ERR-COOLDOWN))
...
(define-public (sell-item (amountIn uint))
  (let (
    (b1 (unwrap-panic (swap-wstx-xbtc-velar amountIn)))
    (b2 (unwrap-panic (swap-xbtc-wstx-alex-2 b1)))
  )
    (print { b1: b1, b2: b2 })
    (begin
      (asserts! (> b2 amountIn) (err ERR-COOLDOWN))
...
(define-public (unlist-in-usda (amountIn uint))
  (let (
    ...
    (b3 (unwrap-panic (swap-xbtc-wstx-velar b2)))
  )
...
(define-public (release-bid (amountIn uint))
  (let (
    ...
    (b4 (unwrap-panic (swap-xbtc-wstx-velar b3)))
  )
