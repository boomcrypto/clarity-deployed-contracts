...
(define-non-fungible-token mojo-punk-gen-2 uint)
...
(define-data-var artist-address principal 'SP3TP3RB0EX8AFPHR3QTC5R9JDE8G66YFZE32D25G)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmY2QiYYrVmqZnbf5Cc6iSTGoL4FbBPzzqqML2HYwhjFsX/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? mojo-punk-gen-2 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? mojo-punk-gen-2 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? mojo-punk-gen-2 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? mojo-punk-gen-2 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? mojo-punk-gen-2 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? mojo-punk-gen-2 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? mojo-punk-gen-2 id) (err ERR-NOT-FOUND)))
