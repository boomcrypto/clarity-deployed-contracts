...
(define-non-fungible-token hippo-goddness uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmWGPLGQfXWH4vhZ9mjLGx4eZNuAr7xQf61VTG2s4zWiZB/hippo-goddness/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP24KR4SP2PQ1SMRA4PPZ1XNH7XZ60EP6QHPWXF5Y )) 
...
(define-data-var public-target-block-height uint u69054)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? hippo-goddness id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace hippo-goddness
  (ok (nft-get-owner? hippo-goddness id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? hippo-goddness next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? hippo-goddness id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? hippo-goddness id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? hippo-goddness id) ERR-NOT-FOUND))
