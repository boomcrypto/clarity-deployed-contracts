...
(define-non-fungible-token giftcats uint)
...
(define-data-var artist-address principal 'SP16NH8365EYGTQCA7N41GW960CKY0KSCK3V5K97K)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTNdfoGzE6mvWa1pmec1PUUest9eRCa3byS6L52QRSTqt/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? giftcats next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? giftcats token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? giftcats token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? giftcats token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? giftcats id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? giftcats id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? giftcats id) (err ERR-NOT-FOUND)))
