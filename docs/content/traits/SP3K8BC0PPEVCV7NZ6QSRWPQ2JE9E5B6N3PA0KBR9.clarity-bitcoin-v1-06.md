---
title: "Trait clarity-bitcoin-v1-06"
draft: true
---
```
...
(define-read-only (read-uint8 (ctx { txbuff: (buff 16384), index: uint}))
	(let (
			(data (get txbuff ctx))
			(base (get index ctx)))
...
(define-read-only (read-uint16 (ctx { txbuff: (buff 16384), index: uint}))
	(let (
			(data (get txbuff ctx))
			(base (get index ctx)))
...
(define-read-only (read-uint32 (ctx { txbuff: (buff 16384), index: uint}))
	(let (
			(data (get txbuff ctx))
			(base (get index ctx)))
...
(define-read-only (read-uint64 (ctx { txbuff: (buff 16384), index: uint}))
	(let (
			(data (get txbuff ctx))
			(base (get index ctx)))
...
(define-read-only (read-varint (ctx { txbuff: (buff 16384), index: uint}))
	(let (
			(ptr (get index ctx))
			(tx (get txbuff ctx))
...
(define-read-only (read-varslice (old-ctx { txbuff: (buff 16384), index: uint}))
	(let (
...
(define-read-only (reverse-buff32 (input (buff 32)))
	(fold inner-reverse
		...
		input))
(define-read-only (read-hashslice (old-ctx { txbuff: (buff 16384), index: uint}))
	(let (
...
(define-read-only (read-next-txin
		...
		(state-res
			(response {
					ctx: { txbuff: (buff 16384), index: uint },
					remaining: uint,
					txins: (list 62 {
						outpoint: {
							hash: (buff 32),
	...
	(match state-res
		...
		error
			(err error)))
(define-read-only (read-txins (ctx { txbuff: (buff 16384), index: uint}))
	(let (
			...
			(let (
				...
				(ok {
					...
					txouts: (unwrap!
						...
						ERR-TOO-MANY-TXOUTS)}))
		error (err error)))
(define-read-only (read-txouts (ctx { txbuff: (buff 16384), index: uint}))
	(let (
...
(define-read-only (read-next-element
		(ignored bool)
		(state-res (response
			{ctx: { txbuff: (buff 16384), index: uint }, elements: (list 62 (buff 256))}
			uint)))
	(match state-res
		state
			(let (
...
(define-read-only (read-next-witness
		(ignored bool)
		(state-res (response
			{ctx: {txbuff: (buff 16384), index: uint}, witnesses: (list 62 (list 62 (buff 256)))}
			uint)))
	(match state-res
		state
			(let (
				...
				(if (> varint u0)
					...
					(begin
						(ok {
							...
							ctx: (get ctx parsed-num-items)}))))
		error (err u1)))
(define-read-only (read-witnesses (ctx { txbuff: (buff 16384), index: uint }) (num-txins uint))
...
(define-read-only (is-segwit-tx (tx (buff 16384)))
	(let (
...
(define-read-only (parse-wtx (tx (buff 16384)))
	(let (
			(ctx { txbuff: tx, index: u0})
			(parsed-version (try! (read-uint32 ctx)))
			(parsed-segwit-marker (try! (read-uint8 (get ctx parsed-version))))
...
(define-read-only (parse-tx (tx (buff 16384)))
	(let (
			(ctx { txbuff: tx, index: u0})
			(parsed-version (try! (read-uint32 ctx)))
...
(define-read-only (verify-block-header (headerbuff (buff 80)) (expected-block-height uint))
	(match (get-bc-h-hash expected-block-height)
		...
		false))
(define-read-only (get-reversed-txid (tx (buff 16384)))
	(sha256 (sha256 tx)))
(define-read-only (get-txid (tx (buff 16384)))
	(reverse-buff32 (get-reversed-txid tx)))
(define-read-only (get-reversed-segwit-txid (tx (buff 16384)))
	(let (
			(ctx { txbuff: tx, index: u0})
...
(define-read-only (get-segwit-txid (tx (buff 16384)))
			...
			(get verified
				(fold inner-merkle-proof-verify
						(list u0 u1 u2 u3 u4 u5 u6 u7 u8 u9 u10 u11 u12 u13)
						{
							...
							verified: false})))))
(define-read-only (was-tx-mined? (block { header: (buff 80), height: uint }) (tx (buff 16384)) (proof { tx-index: uint, hashes: (list 14 (buff 32)), tree-depth: uint }))
	(let (
		...
		(if header-valid
			merkle-valid
			(ok false))))
(define-read-only (was-segwit-tx-mined? (block { header: (buff 80), height: uint }) (tx (buff 16384)) (proof { tx-index: uint, hashes: (list 14 (buff 32)), tree-depth: uint }))
	(let (
			(header-valid (verify-block-header (get header block) (get height block)))
