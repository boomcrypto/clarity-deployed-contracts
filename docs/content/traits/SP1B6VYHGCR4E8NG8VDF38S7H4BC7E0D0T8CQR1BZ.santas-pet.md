...
(define-non-fungible-token santas-pet uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmSGqf2y23yCacnzRSW6cZBTN3MPXEgXgdevNuEnMRE8JF/santas-pet/")
...
(define-data-var public-target-block-height uint u64223)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? santas-pet id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace santas-pet
  (ok (nft-get-owner? santas-pet id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? santas-pet next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? santas-pet id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? santas-pet id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? santas-pet id) ERR-NOT-FOUND))
...
(define-data-var mia-cost-per-mint uint u5000000)
