...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://peanut.com"))
(define-data-var token-name (string-ascii 32) "peanut")
(define-data-var token-symbol (string-ascii 32) "PEANUT")
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance PEANUT from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? PEANUT amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
;; Mint Tokens
(begin
  (let ((result (ft-mint? PEANUT MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
