...
(define-non-fungible-token cyber-stx uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmULvXJPA6mreVZ5FiQWSMqZUCFjMARwYtKQMzd1MNoPNM/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? cyber-stx next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? cyber-stx token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? cyber-stx token-id)))
