---
title: "Trait pixelated-stacks-monkeys"
draft: true
---
```
;; pixelated-stacks-monkeys
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token pixelated-stacks-monkeys uint)
...
(define-data-var artist-address principal 'SP32VK5AXW7BXGSKZ02AY6749D7BA7HP1A56EAG79)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmV8xRZnZMZEM2UzvaVFxfUv3hKyg7zzsZFFPUJ3wuTfUU/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? pixelated-stacks-monkeys next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? pixelated-stacks-monkeys token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? pixelated-stacks-monkeys token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? pixelated-stacks-monkeys token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pixelated-stacks-monkeys id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pixelated-stacks-monkeys id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pixelated-stacks-monkeys id) (err ERR-NOT-FOUND)))
