---
title: "Trait stxstone-ordinal-abandoned-v2"
draft: true
---
```
;; stxstone-ordinal-abandoned-v2
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token stxstone-ordinal-abandoned-v2 uint)
...
;; Internal variables
(define-data-var mint-limit uint u7700)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmU7ssHsS3mC4rv64g5ETpedHepQLc6YG4q513sec3NZgu/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? stxstone-ordinal-abandoned-v2 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stxstone-ordinal-abandoned-v2 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stxstone-ordinal-abandoned-v2 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stxstone-ordinal-abandoned-v2 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stxstone-ordinal-abandoned-v2 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stxstone-ordinal-abandoned-v2 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stxstone-ordinal-abandoned-v2 id) (err ERR-NOT-FOUND)))
