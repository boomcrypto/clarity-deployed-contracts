---
title: "Trait neoswap-sc-1682271453639"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/ZA1aF1SA3iiLqce5YuDQ
;; SwapId: rJD0nYKr1W92lZbZowqE

;; traders
(define-constant TRADER-1 'SP136AXDAQ41R31GJWJX8KX14E2T4K8PA08NCE6Q5)
(define-constant TRADER-2 'SP1N1SR72VPW79ZTXV57TSYEEKVTW2H0BSREZ7Y1H)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u300000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
		(unwrap! (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-stacks-dragons transfer u326 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-stacks-dragons transfer u326 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u300000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u300000 tx-sender TRADER-1)))
		(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-stacks-dragons transfer u326 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.stacks-mandala transfer u539 tx-sender TRADER-2)))
	    )
            true
        )
