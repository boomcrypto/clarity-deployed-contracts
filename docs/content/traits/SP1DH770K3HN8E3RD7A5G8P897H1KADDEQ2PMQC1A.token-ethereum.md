...
(define-data-var symbol (string-ascii 256) "")
...
(define-public (request-swap (to-chain (buff 256)) (to-addr (buff 20)) (amount uint))
    (let
        ...
        (print {type: "request-swap", to-chain: to-chain, to-addr: to-addr, amount: amount})
...
(define-read-only (is-valid-chain (chain (buff 256)))
    (contract-call? .multisig-eth is-valid-chain chain)
)

(define-read-only (is-validator (validator principal))
	(contract-call? .multisig-eth is-validator validator)
)

(define-read-only (get-required)
    (contract-call? .multisig-eth get-required)
)
...
;; ================ constructor ================
(var-set name "Orbit Bridge Stacks Sandbox Ethereum")
(var-set symbol "SBETH")
(var-set token-decimals u18)
(var-set governance-id 0x37bebde0a214f58a11481af735ee17c883764a24fbede2cc53f8ff7e31b21462)
