---
title: "Trait keeper-4-nej4s62ye-v-1-1"
draft: true
---
```
;; keeper-4-nej4s62ye-v-1-1
...
(define-data-var owner-address principal 'SP3D03X5BHMNSAAW71NN7BQRMV4DW2G4JB3MZAGJ8)
...
(define-public (execute-action-a
  ...
  (let (
    ...
    (keeper-check (asserts! (unwrap! (contract-call? .keeper-4-helper-v-1-1 get-keeper-status) ERR_INVALID_HELPER_DATA) ERR_KEEPER_STATUS))

    ;; Assert keeper action trait is approved by helper contract and owner
    (action-check-a (asserts! (unwrap! (contract-call? .keeper-4-helper-v-1-1 get-action-approved action-trait) ERR_INVALID_HELPER_DATA) ERR_ACTION_NOT_APPROVED))
