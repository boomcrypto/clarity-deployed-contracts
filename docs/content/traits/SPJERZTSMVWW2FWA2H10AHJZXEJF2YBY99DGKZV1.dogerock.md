...
(define-non-fungible-token dogerock uint)
...
(define-data-var total-price uint u6620000)
(define-data-var artist-address principal 'SPJERZTSMVWW2FWA2H10AHJZXEJF2YBY99DGKZV1)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUirVKH56xKhCq22MwZzau47dF473pxjPqMPZPpAgCVQ3/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? dogerock next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? dogerock token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? dogerock token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? dogerock token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? dogerock id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? dogerock id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? dogerock id) (err ERR-NOT-FOUND)))
