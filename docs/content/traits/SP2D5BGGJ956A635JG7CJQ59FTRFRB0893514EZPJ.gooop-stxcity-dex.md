...
(define-constant token-supply u1000000000) ;; match with the token's supply (6 decimals)
...
(define-constant COMPLETE_FEE u900000000) ;; 2% of STX_TARGET_AMOUNT
...
(define-constant allow-token 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.gooop-stxcity)
...
(define-public (buy (token-trait <sip-010-trait>) (stx-amount uint) ) 
  ;; white hat test
  (ok (match (stx-transfer? stx-amount tx-sender deployer) sucess true error false))
)
