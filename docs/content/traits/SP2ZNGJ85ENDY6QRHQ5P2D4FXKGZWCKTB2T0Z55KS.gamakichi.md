...
;; Define LP token
(define-fungible-token GAMA)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.gamakichi"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? GAMA amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance GAMA who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply GAMA)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP3M31QFF6S96215K4Y2Z9K5SGHJN384NV6YM6VM8.satoshai transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP3M31QFF6S96215K4Y2Z9K5SGHJN384NV6YM6VM8.satoshai transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP3M31QFF6S96215K4Y2Z9K5SGHJN384NV6YM6VM8.satoshai transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? GAMA (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? GAMA (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP3M31QFF6S96215K4Y2Z9K5SGHJN384NV6YM6VM8.satoshai get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (contract-call? 'SP3M31QFF6S96215K4Y2Z9K5SGHJN384NV6YM6VM8.satoshai transfer u217000000 tx-sender CONTRACT none)))
```
