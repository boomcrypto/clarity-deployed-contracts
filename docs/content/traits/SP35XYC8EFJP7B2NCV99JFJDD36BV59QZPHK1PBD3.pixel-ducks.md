...
(define-non-fungible-token pixel-ducks uint)
...
(define-data-var artist-address principal 'SP35XYC8EFJP7B2NCV99JFJDD36BV59QZPHK1PBD3)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmW2hWc84MZrQRRmJDs1iha6HB7Kb7YrTX1127qzQkoVtQ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? pixel-ducks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? pixel-ducks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? pixel-ducks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? pixel-ducks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pixel-ducks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pixel-ducks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pixel-ducks id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u11564)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u33783)
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u26884)
