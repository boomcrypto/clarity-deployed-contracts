---
title: "Trait f-claim-liquidation-rewards-xstx-j-v1-10"
draft: true
---
```
...
(define-read-only (check-job)
  (let (
    ...
    (last-id (contract-call? .f-claim-liquidation-rewards-xstx-e-v1-10 get-last-total-reward-ids))
    (next-unlock-block (contract-call? .f-claim-liquidation-rewards-xstx-e-v1-10 get-next-unlock-block))
  )
    ...
    (if (is-eq next-unlock-block u0)
      ...
      (ok (> burn-block-height (+ next-unlock-block u144)))
    )
  )
)
...
(define-public (run-job)
  (begin
    ...
    (try! (contract-call? .lydian-dao-executor-v1-1 execute-proposal .f-claim-liquidation-rewards-xstx-e-v1-10))
    (ok true)
  )
)
