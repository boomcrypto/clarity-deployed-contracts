...
(define-non-fungible-token the-1o1-gentlemen uint)
...
(define-data-var artist-address principal 'SP3NY9297ZHA3GNB1JMV1T545S9GPHAT4NQ4RFD47)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTLA5nEaMAtjFwmjWVEzfUHxzdoR6xLvL8XVVdrXE2A52/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-1o1-gentlemen next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-1o1-gentlemen token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-1o1-gentlemen token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-1o1-gentlemen token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-1o1-gentlemen id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-1o1-gentlemen id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-1o1-gentlemen id) (err ERR-NOT-FOUND)))
