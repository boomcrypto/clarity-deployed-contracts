...
(define-non-fungible-token stacks-catz uint)
...
;; Internal variables
(define-data-var mint-limit uint u3550)
(define-data-var last-id uint u1)
(define-data-var total-price uint u3000000)
(define-data-var artist-address principal 'SP1PQY9NQTR66CEHT08591ENCMKWYQZQV8GHTF55)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmeh7A8jQSEhLqha4Vh6Fyd7xonoSzadCPtTJRjkxbaPyn/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-catz next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-catz token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-catz token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-catz token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-catz id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-catz id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-catz id) (err ERR-NOT-FOUND)))
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? stacks-catz (+ last-nft-id u0) 'SP2XA59ZFEVYVDS24AD94XRJ60GNXCCE0AD3XG1ZP))
      (map-set token-count 'SP2XA59ZFEVYVDS24AD94XRJ60GNXCCE0AD3XG1ZP (+ (get-balance 'SP2XA59ZFEVYVDS24AD94XRJ60GNXCCE0AD3XG1ZP) u1))
