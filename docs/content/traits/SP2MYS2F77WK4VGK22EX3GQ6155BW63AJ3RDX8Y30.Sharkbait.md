...
;; Variables
(define-fungible-token SHARKBAIT MAXSUPPLY)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://gaia.hiro.so/hub/1NGN9Z8NxdYAyeZqzTuNcrxsp13U3hWtBn/f3762b55-528e-44e9-8331-aeb0ef442440-metadata.json"))
(define-data-var token-name (string-ascii 32) "Sharkbait")
(define-data-var token-symbol (string-ascii 32) "SHARKBAIT")
(define-data-var token-decimals uint u6)

;; Read-Only Functions
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance SHARKBAIT owner))
)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance SHARKBAIT from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? SHARKBAIT amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
(begin
  ...
  (let ((result (ft-mint? SHARKBAIT MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
