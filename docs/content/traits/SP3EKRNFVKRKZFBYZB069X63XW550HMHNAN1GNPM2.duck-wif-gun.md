---
title: "Trait duck-wif-gun"
draft: true
---
```
(impl-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)

(define-fungible-token duck-wif-gun)
...
(define-constant dev-principal 'SP3EKRNFVKRKZFBYZB069X63XW550HMHNAN1GNPM2)
...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://bafybeicc7pxjbylxbaakl2poenh57hn3xqpt24zw4bmjinzb4pkk53wgui.ipfs.dweb.link/")) 
...
(define-read-only (get-name) (ok "duck wif gun"))
(define-read-only (get-symbol) (ok "DUCK"))
(define-read-only (get-decimals) (ok u0))
(define-read-only (get-balance (user principal)) (ok (ft-get-balance duck-wif-gun user)))
(define-read-only (get-total-supply) (ok (ft-get-supply duck-wif-gun)))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? duck-wif-gun amount from to)
    )
)
...
(define-public (swap-stx-for-token (stx-amount uint) (min-tokens-out uint)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? duck-wif-gun tokens-out (as-contract tx-sender) tx-sender))
      (if (>= (/ (* (+ (var-get real-stx-reserve) (var-get virtual-stx-reserve)) token-supply) (var-get token-reserve)) market-cap-threshold)
        (begin
          (let (
            ...
            (try! (ft-transfer? duck-wif-gun burn-amount (as-contract tx-sender) burn-address))
            (try! (ft-transfer? duck-wif-gun remaining-tokens (as-contract tx-sender) liquidity-builder-address))
...
(define-public (swap-token-for-stx (token-amount uint) (min-stx-out uint) (recipient principal)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? duck-wif-gun token-amount tx-sender (as-contract tx-sender)))
...
(begin
  (try! (ft-mint? duck-wif-gun token-supply (as-contract tx-sender)))
