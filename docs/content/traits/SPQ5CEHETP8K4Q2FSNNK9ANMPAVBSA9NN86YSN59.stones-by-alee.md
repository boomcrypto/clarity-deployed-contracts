...
(define-non-fungible-token stones-by-alee uint)
...
;; Internal variables
(define-data-var mint-limit uint u5092)
(define-data-var last-id uint u1)
(define-data-var total-price uint u150920)
(define-data-var artist-address principal 'SP2X8WZK0A2X4BJC35JFX231CAHEQER5TMRFQBZ3K)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWig12xRJ1pHjZ9jGwTs4aFzuQtPyXxozMj1xmDs4a21M/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? stones-by-alee next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stones-by-alee token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stones-by-alee token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stones-by-alee token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stones-by-alee id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stones-by-alee id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stones-by-alee id) (err ERR-NOT-FOUND)))
