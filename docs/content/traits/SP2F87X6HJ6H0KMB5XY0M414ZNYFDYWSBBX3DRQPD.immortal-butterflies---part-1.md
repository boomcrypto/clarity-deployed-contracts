---
title: "Trait immortal-butterflies---part-1"
draft: true
---
```
;; immortal-butterflies---part-1

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token immortal-butterflies---part-1 uint)
...
(define-data-var artist-address principal 'SP2F87X6HJ6H0KMB5XY0M414ZNYFDYWSBBX3DRQPD)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmf1QFW2kDvqptr6Nnr5AvQnLR89CiD4FZTfbAQj6CiF3D/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? immortal-butterflies---part-1 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? immortal-butterflies---part-1 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? immortal-butterflies---part-1 id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? immortal-butterflies---part-1 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? immortal-butterflies---part-1 id) (err ERR-NOT-FOUND)))
...
(define-data-var total-price-mia uint u10000)
...
;; Alt Minting Mintpass
(define-data-var total-price-nyc uint u10000)
...
(define-private (mint-many-nyc (orders (list 10 bool )))  
  (let 
    ...
    (ok id-reached)))
(map-set mint-passes 'SP2F87X6HJ6H0KMB5XY0M414ZNYFDYWSBBX3DRQPD u10)
(map-set mint-passes 'SP38GBVK5HEJ0MBH4CRJ9HQEW86HX0H9AP1HZ3SVZ u5)
