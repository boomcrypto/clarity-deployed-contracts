...
(define-fungible-token wrapped-ether)


;; Get the token balance of the specified owner in base units
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance wrapped-ether owner)))
...
(define-read-only (get-total-supply)
  (ok (ft-get-supply wrapped-ether)))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal ) (memo (optional (buff 34) )))
  (begin
    ...
    (ft-transfer? wrapped-ether amount sender recipient) ) ) ;; Transfer
...
(define-public (mint-tokens (mint-amount uint) (mint-to principal) )
  (begin
    ...
    (ft-mint? wrapped-ether mint-amount mint-to)))
...
(define-public (burn-tokens (burn-amount uint) (burn-from principal) )
  (begin
    ...
    (ft-burn? wrapped-ether burn-amount burn-from)))
...
(define-public (revoke-tokens (revoke-amount uint) (revoke-from principal) (revoke-to principal) )
  (begin
    ...
    (ft-transfer? wrapped-ether revoke-amount revoke-from revoke-to)))
