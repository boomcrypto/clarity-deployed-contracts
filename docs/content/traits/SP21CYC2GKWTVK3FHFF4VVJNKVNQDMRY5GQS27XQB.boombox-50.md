...
(define-non-fungible-token boombox-50 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmf85epoLZQUtXSYFDj1Uvh7N61VmaW36g2zCZEV3Ya4FZ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? boombox-50 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? boombox-50 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? boombox-50 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? boombox-50 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? boombox-50 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? boombox-50 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? boombox-50 id) (err ERR-NOT-FOUND)))
