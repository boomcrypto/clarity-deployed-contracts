...
(define-fungible-token wbaby)

(define-data-var token-name (string-ascii 32) "Baby ALEX Wrapper")
(define-data-var token-symbol (string-ascii 32) "wbaby")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wbaby.json"))
...
(define-read-only (get-base-token)
  'SP3BGZAZKBS475T9FS18VH1NANWV4ERSGZWJKSRTF.baby-alex)

(define-read-only (get-base-decimals)
  (contract-call? 'SP3BGZAZKBS475T9FS18VH1NANWV4ERSGZWJKSRTF.baby-alex get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP3BGZAZKBS475T9FS18VH1NANWV4ERSGZWJKSRTF.baby-alex get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP3BGZAZKBS475T9FS18VH1NANWV4ERSGZWJKSRTF.baby-alex transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
