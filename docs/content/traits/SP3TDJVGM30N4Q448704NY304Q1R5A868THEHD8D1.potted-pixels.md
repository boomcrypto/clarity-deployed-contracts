...
(define-non-fungible-token potted-pixels uint)
...
(define-data-var artist-address principal 'SP2DY26M87BW1DE1962K2Z7Y4EGY1Z4BJXR3TBAZA)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmariKLUUmyX4UDQDs6TWSQLJ2mj36SPxZKgLE4cJS2sDd/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? potted-pixels next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? potted-pixels token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? potted-pixels token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? potted-pixels token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? potted-pixels id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? potted-pixels id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? potted-pixels id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u75000)
