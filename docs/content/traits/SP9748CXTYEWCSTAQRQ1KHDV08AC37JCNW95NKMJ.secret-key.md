...
(define-non-fungible-token secret-key uint)
...
(define-data-var artist-address principal 'SP9748CXTYEWCSTAQRQ1KHDV08AC37JCNW95NKMJ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVd93EWK2JGRaT3v51D3nVjBshZV7QomGRbnCp6EKJpFZ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? secret-key next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? secret-key token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? secret-key token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? secret-key token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? secret-key id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? secret-key id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? secret-key id) (err ERR-NOT-FOUND)))
...
(define-data-var total-price-mia uint u50000)
...
;; Alt Minting Mintpass
(define-data-var total-price-nyc uint u90000)
...
;; Alt Minting Mintpass
(define-data-var total-price-xbtc uint u300)
...
;; Alt Minting Mintpass
(define-data-var total-price-usda uint u90000)
...
(map-set mint-passes 'SP23RN5F2EGG89SD7ER61ZQFK5FZ7HCTN1SQEQXJN u1)
(map-set mint-passes 'SP7Q3SD4SGKPQD8YGSWZNVGYT98KJDS7NYJYBJ5W u10)
(map-set mint-passes 'SP3QKAQS3J0YS3ZAZPSZM5ZSZZRYRYV72N6A9ZPZT u10)
(map-set mint-passes 'SP3NDGET2RK5VKRTM47EJ8444MB3PKKJTJR3RPS0X u10)
;; Airdrop
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? secret-key (+ last-nft-id u0) 'SP23RN5F2EGG89SD7ER61ZQFK5FZ7HCTN1SQEQXJN))
      (map-set token-count 'SP23RN5F2EGG89SD7ER61ZQFK5FZ7HCTN1SQEQXJN (+ (get-balance 'SP23RN5F2EGG89SD7ER61ZQFK5FZ7HCTN1SQEQXJN) u1))
      (try! (nft-mint? secret-key (+ last-nft-id u1) 'SP23RN5F2EGG89SD7ER61ZQFK5FZ7HCTN1SQEQXJN))
      (map-set token-count 'SP23RN5F2EGG89SD7ER61ZQFK5FZ7HCTN1SQEQXJN (+ (get-balance 'SP23RN5F2EGG89SD7ER61ZQFK5FZ7HCTN1SQEQXJN) u1))
      (try! (nft-mint? secret-key (+ last-nft-id u2) 'SP23RN5F2EGG89SD7ER61ZQFK5FZ7HCTN1SQEQXJN))
      (map-set token-count 'SP23RN5F2EGG89SD7ER61ZQFK5FZ7HCTN1SQEQXJN (+ (get-balance 'SP23RN5F2EGG89SD7ER61ZQFK5FZ7HCTN1SQEQXJN) u1))
      (try! (nft-mint? secret-key (+ last-nft-id u3) 'SP7Q3SD4SGKPQD8YGSWZNVGYT98KJDS7NYJYBJ5W))
