---
title: "Trait wildlife-collection"
draft: true
---
```
;; wildlife-collection
...
(define-non-fungible-token wildlife-collection uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmc4YofNx2WVptso3iVmmEfvknhiEWcLaQuL4gAbT6L7rQ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? wildlife-collection next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? wildlife-collection token-id sender recipient)))

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? wildlife-collection token-id)))
