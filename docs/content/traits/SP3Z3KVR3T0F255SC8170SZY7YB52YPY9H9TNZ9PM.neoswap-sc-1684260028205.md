---
title: "Trait neoswap-sc-1684260028205"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/kP1L7Ot8CsZDZ0FoSmQ6
;; SwapId: 8YF4V8UXWcIkjsKZFdlx
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u27000000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u1350000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u25650000 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u27000000 tx-sender TRADER-2)))
	    )
            true
        )
