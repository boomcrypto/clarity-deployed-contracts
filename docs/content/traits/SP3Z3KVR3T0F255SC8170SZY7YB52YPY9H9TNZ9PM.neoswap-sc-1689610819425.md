---
title: "Trait neoswap-sc-1689610819425"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/57FHZ7SyB2WSeBOUUEus
;; SwapId: CzeNSvFwhpRyo65SJciG

;; traders
(define-constant TRADER-1 'SP136AXDAQ41R31GJWJX8KX14E2T4K8PA08NCE6Q5)
(define-constant TRADER-2 'SP3QC4R6M7M0DAZBXSZCW4FWGDCNDD05FV8Y0AY8C)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u302889 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP125J1ADVYWGWB9NQRCVGKYAG73R17ZNMV17XEJ7.mutant-monkeys transfer u1396 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP125J1ADVYWGWB9NQRCVGKYAG73R17ZNMV17XEJ7.mutant-monkeys transfer u1396 tx-sender TRADER-1)))
	...
	(unwrap-panic (as-contract (stx-transfer? u302889 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u302889 tx-sender TRADER-1)))
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP125J1ADVYWGWB9NQRCVGKYAG73R17ZNMV17XEJ7.mutant-monkeys transfer u1396 tx-sender TRADER-2)))
	    )
            true
        )
