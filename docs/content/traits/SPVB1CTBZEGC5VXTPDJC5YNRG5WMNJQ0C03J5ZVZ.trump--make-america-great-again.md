---
title: "Trait trump--make-america-great-again"
draft: true
---
```
...
(define-non-fungible-token trump--make-america-great-again uint)
...
;; Internal variables
(define-data-var mint-limit uint u994)
(define-data-var last-id uint u1)
(define-data-var total-price uint u2024000)
(define-data-var artist-address principal 'SPVB1CTBZEGC5VXTPDJC5YNRG5WMNJQ0C03J5ZVZ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmX9vcQXpGSYLMp9P77pCQ5xBRN1eZXEcAa6Zden8zvaW3/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? trump--make-america-great-again next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? trump--make-america-great-again token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? trump--make-america-great-again token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? trump--make-america-great-again token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? trump--make-america-great-again id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? trump--make-america-great-again id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? trump--make-america-great-again id) (err ERR-NOT-FOUND)))
