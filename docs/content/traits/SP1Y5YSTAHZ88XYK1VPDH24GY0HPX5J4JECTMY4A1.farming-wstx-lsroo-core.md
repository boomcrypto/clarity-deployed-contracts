---
title: "Trait farming-wstx-lsroo-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-lsroo)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-lsroo transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-lsroo get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-lsroo transfer amt protocol user none)))
