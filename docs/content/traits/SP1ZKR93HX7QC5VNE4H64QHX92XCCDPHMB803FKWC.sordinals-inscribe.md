---
title: "Trait sordinals-inscribe"
draft: true
---
```
...
(define-private (transfer-fold (id-to-send uint) (state { memos: (list 1000 (buff 34)), recipients: (list 1000 principal), sent: (list 1000 uint) }))
    (let
        (
            (memo (unwrap! (element-at? (get memos state) id-to-send) state))
            (recipient (unwrap! (element-at? (get recipients state) id-to-send) state))
        )
        (unwrap! (stx-transfer-memo? u1 tx-sender recipient memo) state)
        (merge state { sent: (unwrap! (as-max-len? (append (get sent state) id-to-send) u1000) state) })
    )
)
...
(define-public (batch-transfer-memo (recipients (list 1000 principal)) (memos (list 1000 (buff 34))))
    (let
        (
            ...
            (res (fold transfer-fold it {memos: memos, recipients: recipients, sent: (list)}))
        )
        (asserts! (> (len memos) u0) ERR-EMPTY-ARRAY)
        (asserts! (is-eq (len (get sent res)) (len memos)) ERR-TRANSFER)
        (asserts! (is-eq (len recipients) (len memos)) ERR-WRONG-ARRAY-SIZE)
        (ok true)
    )
)

(define-public (transfer-memo-single (recipient principal) (memo (buff 34)))
    (stx-transfer-memo? u1 tx-sender recipient memo)
)

(define-public (sordinals--inscribe-single
    (sordinals--recipient principal)
    (sordinals--parent-id (buff 32))
    (sordinals--inscribe-events-count uint)
    (sordinals--metadata (buff 1048576))
    (sordinals--encoding (buff 7))
    (sordinals--content-type (buff 100))
    (sordinals--data (buff 1048576))
  )
    (ok true)
)

(define-public (sordinals--append-inscribe-event
    (sordinals--inscription-id (buff 32))
    (sordinals--data (buff 1048576))
  )
    (ok true)
)
...
(define-public (sordinals--inscribe-multiple
    (sordinals--recipients (list 1000 principal))
    (sordinals--parent-id (buff 32))
    (sordinals--metadata (buff 1048576))
    (sordinals--encoding (buff 7))
    (sordinals--content-types (list 1000 (buff 100)))
    (sordinals--data-lengths (list 1000 uint))
    (sordinals--data (buff 1048576))
  )
    (ok true)
)
