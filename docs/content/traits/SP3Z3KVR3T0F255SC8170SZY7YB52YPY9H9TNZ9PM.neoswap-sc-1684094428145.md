---
title: "Trait neoswap-sc-1684094428145"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/Q16zB543hoSYTfsnpgl8
;; SwapId: 79EZtogdpmG4pIxmaAHV
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (contract-call? 'SP2W9RM9KBQFWVSX4F532PSHJ35N2PPS230380CRH.zyoolah transfer u5 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
		(unwrap! (contract-call? 'SP1XJ6GNTDVF6HR1VHPQDMFZJY87D9W6TGMH3QCP.passion-of-christ-v3 transfer u3 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u3130000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP2W9RM9KBQFWVSX4F532PSHJ35N2PPS230380CRH.zyoolah transfer u5 tx-sender TRADER-2)))
	(unwrap-panic (as-contract (contract-call? 'SP1XJ6GNTDVF6HR1VHPQDMFZJY87D9W6TGMH3QCP.passion-of-christ-v3 transfer u3 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u156500 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u2973500 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2W9RM9KBQFWVSX4F532PSHJ35N2PPS230380CRH.zyoolah transfer u5 tx-sender TRADER-1)))
		(unwrap-panic (as-contract (contract-call? 'SP1XJ6GNTDVF6HR1VHPQDMFZJY87D9W6TGMH3QCP.passion-of-christ-v3 transfer u3 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u3130000 tx-sender TRADER-2)))
	    )
            true
        )
