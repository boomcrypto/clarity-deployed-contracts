...
(define-non-fungible-token block-cadence uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmU6o8TCarREJjTCoy5PUuyxaSfCSa6mP14uV8g7YVWAqV/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? block-cadence next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? block-cadence token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? block-cadence token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? block-cadence token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? block-cadence id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? block-cadence id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? block-cadence id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-usda uint u35000000)
