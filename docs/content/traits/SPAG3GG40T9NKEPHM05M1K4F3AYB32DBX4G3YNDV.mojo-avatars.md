...
(define-non-fungible-token mojo-avatars uint)
...
(define-data-var artist-address principal 'SP31D40V6DGQ02M1GRZJKKPCTQB8DZKXZVR0CT388)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYncKMzNckwRM8p2eVEdv32mcMiv22UjkYtxXdrgBoCpF/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? mojo-avatars next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? mojo-avatars token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? mojo-avatars token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? mojo-avatars token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? mojo-avatars id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? mojo-avatars id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? mojo-avatars id) (err ERR-NOT-FOUND)))
