...
;; Constants
(define-constant MAXSUPPLY u12138000000000)

;; Variables
(define-fungible-token RBGOAT MAXSUPPLY)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://gaia.hiro.so/hub/1A3iaaybbiBBe8LsQbSnRDbx3VASSDYeU7/350876fd-3ecc-4962-a5df-b0205391e3b2-metadata.json"))
(define-data-var token-name (string-ascii 32) "REDBULL GOAT")
(define-data-var token-symbol (string-ascii 32) "RBGOAT")
(define-data-var token-decimals uint u0)

;; Read-Only Functions
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance RBGOAT owner))
)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance RBGOAT from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? RBGOAT amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
;; Mint Tokens
(begin
  (let ((result (ft-mint? RBGOAT MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
