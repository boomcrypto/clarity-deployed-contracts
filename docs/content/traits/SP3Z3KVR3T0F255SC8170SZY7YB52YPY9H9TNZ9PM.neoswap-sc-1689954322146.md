---
title: "Trait neoswap-sc-1689954322146"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/V69gcQdz5hyr6Klfc6jX
;; SwapId: Qt2sxYgwXxhcDG6cLVPy
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u8530000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
		(unwrap! (contract-call? 'SP3BRRCHKMPBR60V8ES9J5YF40VXWMABWXK4SEB9G.loop-isle transfer u70 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (contract-call? 'SP3BRRCHKMPBR60V8ES9J5YF40VXWMABWXK4SEB9G.loop-isle transfer u70 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u8530000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u8530000 tx-sender TRADER-1)))
		(unwrap-panic (as-contract (contract-call? 'SP3BRRCHKMPBR60V8ES9J5YF40VXWMABWXK4SEB9G.loop-isle transfer u70 tx-sender TRADER-1)))
	    )
            true
        )
