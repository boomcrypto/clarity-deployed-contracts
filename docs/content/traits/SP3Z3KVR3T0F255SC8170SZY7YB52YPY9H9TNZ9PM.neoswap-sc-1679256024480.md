---
title: "Trait neoswap-sc-1679256024480"
draft: true
---
```
;; multiway-swap
...
(define-constant TRADER-3 'SP2M668YFWEG6VW66MA6TW4YR8P4RNMYNBSDSE0N6)
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u9433000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u934000 tx-sender TRADER-1)))
	    )
            true
        )
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-3)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u9433000 tx-sender TRADER-3)))
	    )
            true
        )
