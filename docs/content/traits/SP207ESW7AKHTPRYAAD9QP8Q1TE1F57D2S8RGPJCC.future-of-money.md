...
(define-non-fungible-token future-of-money uint)
...
(define-data-var artist-address principal 'SP3TYAQVV9378DDA1HF118CH6521NRCY7YBCWAX6G)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcQxi8dPaieBmi9YJ2eGjwfVpWM4iJZ487z3BQUSueiNv/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? future-of-money next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? future-of-money token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? future-of-money token-id)))
