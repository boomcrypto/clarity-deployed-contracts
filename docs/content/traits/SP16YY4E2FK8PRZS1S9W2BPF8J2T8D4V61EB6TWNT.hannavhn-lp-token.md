---
title: "Trait hannavhn-lp-token"
draft: true
---
```
;; Title: Hanna-VHN LP Token
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token Hanna-VHN)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP16YY4E2FK8PRZS1S9W2BPF8J2T8D4V61EB6TWNT.hannavhn-lp-token"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? Hanna-VHN amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "Hanna-VHN LP Token"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance Hanna-VHN who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply Hanna-VHN)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP16YY4E2FK8PRZS1S9W2BPF8J2T8D4V61EB6TWNT.vhanna transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP16YY4E2FK8PRZS1S9W2BPF8J2T8D4V61EB6TWNT.vhanna transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP16YY4E2FK8PRZS1S9W2BPF8J2T8D4V61EB6TWNT.vhanna transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? Hanna-VHN (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? Hanna-VHN (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP16YY4E2FK8PRZS1S9W2BPF8J2T8D4V61EB6TWNT.vhanna get-balance CONTRACT))
    })
...
(define-read-only (get-reserves-quote)
    (let (
        (reserves (get-reserves))
        (supply (ft-get-supply Hanna-VHN)))
        {
          dx: (get a reserves),
          dy: (get b reserves),
          dk: supply
...
(begin
    ...
    (try! (add-liquidity u1500000000000))
    
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SP16YY4E2FK8PRZS1S9W2BPF8J2T8D4V61EB6TWNT.vhanna transfer u2500000000000 tx-sender CONTRACT none)))
```
