...
(define-constant ITEM-COUNT u201)

;; variables
(define-data-var metadata-frozen bool false)
(define-data-var keys-counter uint u0)
(define-data-var keys-index uint u0)
(define-data-var token-uri (string-ascii 256) "ipfs://QmVrMiVJcZGvpXWfv9tVgBMMttDegdDdQ6ERJ7ZCzxqQNh/{id}.json")
(define-data-var cost-per-mint uint u30000000)
(define-data-var marketplace-commission uint u5000000)
(define-data-var creator-address principal 'SP1BYA9WZMNHN0QK4EJ4HEHEVC2994Z2W13B0FBMR)
...
(define-private (mint-next)
  (let (
    ...
    (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
      success (begin
        (try! (nft-mint? stacks-keys random-keys-id tx-sender))
        (var-set keys-counter (+ count u1))
        (if (is-eq u0 (var-get rotation))
          (begin
            (var-set rotation u1)
            (var-set keys-index (+ u1 (var-get keys-index)))
          )
          (var-set rotation u0)
        )
        (try!
...
(define-public (burn (index uint))
  (let (
    ...
    (match (nft-burn? stacks-keys index tx-sender)
      success (begin
        (map-set token-count tx-sender (- current-balance u1))
        (ok true)
      )
...
(define-read-only (get-owner (index uint))
  (ok (nft-get-owner? stacks-keys index))
)
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let (
    (owner (unwrap! (nft-get-owner? stacks-keys id) (err ERR-NOT-FOUND)))
...
(define-private (is-sender-owner (id uint))
  (let (
    (owner (unwrap! (nft-get-owner? stacks-keys id) false))
  )
