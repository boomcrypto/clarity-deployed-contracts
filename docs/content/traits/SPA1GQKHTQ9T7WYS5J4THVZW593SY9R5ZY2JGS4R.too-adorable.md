...
(define-non-fungible-token too-adorable uint)
...
;; Internal variables
(define-data-var mint-limit uint u232)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZ7FvfMdk57Ca1yoJvXZZYmL72Qj8bTRPhL4PVchn8cPh/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? too-adorable next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? too-adorable token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? too-adorable token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? too-adorable token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? too-adorable id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? too-adorable id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? too-adorable id) (err ERR-NOT-FOUND)))
