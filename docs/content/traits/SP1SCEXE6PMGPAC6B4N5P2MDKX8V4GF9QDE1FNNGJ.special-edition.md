...
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)
    ...
    (define-non-fungible-token special-edition uint)
    ...
    (define-constant mint-limit u2)

    ;; Internal variables
    (define-data-var last-id uint u0)
    (define-data-var commission-master uint u500)
    (define-data-var commission uint u500)
    (define-data-var total-price uint u30000)
    (define-data-var artist-address principal 'SPFRGJ8117Y5H9Y6SYBF75H6RSB1FMJQYA27RST9)
    (define-data-var commission-address principal 'SPFRGJ8117Y5H9Y6SYBF75H6RSB1FMJQYA27RST9)
    (define-data-var ipfs-root (string-ascii 80) "ipfs://QmcbGRP3sfjpAeewBoWHrJpWW6qF4CdNRVBo4X6BNrL6fn/")
    ...
    (define-private (mint-helper (new-owner principal) (next-id uint))
        (match (nft-mint? special-edition next-id new-owner)
                success
                (begin
    ...
    (if (and
        ...
        (match (nft-transfer? special-edition token-id sender recipient)
            success (ok success)
            error (err error))
        (err err-invalid-user))
    )
    ...
    (ok (nft-get-owner? special-edition token-id))
    )
