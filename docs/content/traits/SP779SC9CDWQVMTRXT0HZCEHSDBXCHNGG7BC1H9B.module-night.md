...
(define-non-fungible-token module-night uint)
...
(define-data-var artist-address principal 'SPK0JQJ8XZ4DSEWEBKVQ54YH2VT4T51HDPNBCQQE)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmfEijQGiMV9hrMBPZrnEZBbfGnWSyNx16C2RFfw74yAQt/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? module-night next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? module-night token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? module-night id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? module-night id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? module-night id) (err ERR-NOT-FOUND)))
