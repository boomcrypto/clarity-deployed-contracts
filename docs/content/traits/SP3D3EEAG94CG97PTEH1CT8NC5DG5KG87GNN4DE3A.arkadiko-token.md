...
(define-constant TOKEN_URI u"https://ipfs.io/ipfs/QmXV8EA4Y21h6BZW65EemJk9S74KnwfPwDE7g8VoT8TYye") ;; utf-8 string with token metadata host
(define-constant TOKEN_NAME "Arkadiko Token")
(define-constant TOKEN_SYMBOL "DIKO")
...
(begin
    ...
    ;; Transfer the tokens
    (let ((result (ft-transfer? diko amount sender recipient)))
      (if (is-ok result)
        (begin
...
(define-public (transfers (recipients (list 1000 { to: principal, amount: uint}) ))
...
(define-private (send-token-with-memo (amount uint) (to principal))
  (let ((transferOk (try! (transfer amount tx-sender to (some 0x00)))))
    (ok transferOk)
  )
)
...
(begin
  (try! (ft-mint? diko u50000000000000 CONTRACT_OWNER)) 
  ;; (try! (ft-mint? diko u100000000000 'ST18VZNQWK7R44BFN75X3RANYRAT4B9ZA8S47KZ24)) 
) 
