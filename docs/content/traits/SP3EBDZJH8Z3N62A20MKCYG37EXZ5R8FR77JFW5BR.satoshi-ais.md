...
(define-non-fungible-token satoshi-ais uint)
...
(define-data-var artist-address principal 'SP3EBDZJH8Z3N62A20MKCYG37EXZ5R8FR77JFW5BR)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbVsqni9ASgcxN6PbYEXcpPsAr4eeU5jUhh3HRNeFsafX/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? satoshi-ais next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? satoshi-ais token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? satoshi-ais token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? satoshi-ais token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? satoshi-ais id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? satoshi-ais id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? satoshi-ais id) (err ERR-NOT-FOUND)))
