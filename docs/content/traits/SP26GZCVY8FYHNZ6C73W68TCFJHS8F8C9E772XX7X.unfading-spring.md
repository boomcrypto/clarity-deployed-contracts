...
(define-non-fungible-token unfading-spring uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qma7svdtfPr5UQoGkjjcntZ31zsSqnvG2o8WcfrMS42Vt8/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? unfading-spring next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? unfading-spring token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? unfading-spring token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? unfading-spring token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? unfading-spring id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? unfading-spring id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? unfading-spring id) (err ERR-NOT-FOUND)))
