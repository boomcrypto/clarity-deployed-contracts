---
title: "Trait prediction-test-12"
draft: true
---
```
...
(define-public (resolve-market (market-id uint) (winning-outcome uint))
    (let (
        ...
        ;; Calculate admin fee - 5% of the total pool
        (let (
            (admin-fee (/ (* (get total-pool market) ADMIN_FEE) PRECISION))
            ;; Calculate the winning outcome pool total
            (winning-pool (default-to u0 (element-at? (get outcome-pools market) winning-outcome)))
        )
            ;; Calculate admin's virtual bet amount that would yield the 5% fee
            ;; This is: (winning_pool * admin_fee) / (total_pool - admin_fee)
            ;; This gives the admin the exact amount they need to claim 5% of the pot
            (let (
                (admin-amount (if (> (- (get total-pool market) admin-fee) u0)
                               (/ (* winning-pool admin-fee) (- (get total-pool market) admin-fee))
                               u0))
            )
                ;; Store receipt data for admin's prediction
                (map-set predictions receipt-id {
                    market-id: market-id,
                    outcome-id: winning-outcome,
                    amount: admin-amount
                })
                
                ;; Mint NFT receipt to admin
                (try! (nft-mint? prediction-receipt receipt-id tx-sender))
                
                ;; Increment receipt ID counter
                (var-set next-receipt-id (+ receipt-id u1))
                
                ;; Update market state - IMPORTANT: Don't change the pools or total
                ;; This ensures the reward calculation for users remains accurate
