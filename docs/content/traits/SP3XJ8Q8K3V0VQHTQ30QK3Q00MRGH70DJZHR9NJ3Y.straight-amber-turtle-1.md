---
title: "Trait straight-amber-turtle-1"
draft: true
---
```
...
(define-public (get-user-balance-reserve-data
  ...
  (let ((reserve-data (try! (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-0-reserve-v2-0 get-reserve-state (contract-of asset)))))
    (asserts! (is-eq (contract-of lp-token) (get a-token-address reserve-data)) ERR_INVALID_Z_TOKEN)
    (let (
      (user-data (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-0-reserve-v2-0 get-user-reserve-data user (contract-of asset)))
