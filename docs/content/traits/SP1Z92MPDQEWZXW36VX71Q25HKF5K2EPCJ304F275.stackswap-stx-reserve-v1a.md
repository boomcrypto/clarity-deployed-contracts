---
title: "Trait stackswap-stx-reserve-v1a"
draft: true
---
```
(impl-trait .stackswap-vault-trait-v1a.vault-trait)
  ...
  (begin
    ...
    (let ((max-new-debt (- (unwrap-panic (calculate-lbtc-count token-string ustx-amount collateralization-ratio oracle)) current-debt)))
      (if (>= max-new-debt extra-debt)
        (match (print (as-contract (contract-call? .lbtc-token-v1a mint-for-dao extra-debt vault-owner)))
          success (ok true)
