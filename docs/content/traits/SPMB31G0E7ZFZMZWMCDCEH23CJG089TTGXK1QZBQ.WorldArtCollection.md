---
title: "Trait WorldArtCollection"
draft: true
---
```
...
(define-non-fungible-token WorldArtCollection uint)
...
(define-data-var base-uri (string-ascii 80) "Qmd7MmwraGxsgCinrRd6Ug4hTMz9NdMfds52Hh2uSyK3E6/")
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? WorldArtCollection next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? WorldArtCollection next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? WorldArtCollection id sender recipient)
        success
          (let
            (
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? WorldArtCollection id))
)
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? WorldArtCollection id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? WorldArtCollection id) ERR_NO_OWNER))
