---
title: "Trait arkadiko-stake-pool-xbtc-usda-v1-1"
draft: true
---
```
;; @contract Stake Pool - Stake xBTC-USDA LP tokens
...
;; Constants
(define-constant POOL-TOKEN .arkadiko-swap-token-xbtc-usda)
...
(define-public (stake (registry-trait <stake-registry-trait>) (token <ft-trait>) (staker principal) (amount uint))
  (begin
    ...
    (let (
      ...
      (try! (contract-call? .arkadiko-swap-token-xbtc-usda transfer amount staker (as-contract tx-sender) none))
...
(define-public (unstake (registry-trait <stake-registry-trait>) (token <ft-trait>) (staker principal) (amount uint))
  (let (
    ...
    (let (
      ...
      (try! (as-contract (contract-call? .arkadiko-swap-token-xbtc-usda transfer amount tx-sender staker none)))
...
(define-public (emergency-withdraw (registry-trait <stake-registry-trait>))
  (begin
    ...
    (let (
      ...
      ;; Transfer LP token back from this contract to the user
      (try! (as-contract (contract-call? .arkadiko-swap-token-xbtc-usda transfer stake-amount tx-sender staker none)))
...
(define-public (calculate-cumm-reward-per-stake (registry-trait <stake-registry-trait>))
  (let (
    (rewards-per-block (unwrap-panic (contract-call? registry-trait get-rewards-per-block-for-pool .arkadiko-stake-pool-xbtc-usda-v1-1)))
...
(define-private (get-last-block-height (registry-trait <stake-registry-trait>))
  (let (
    (deactivated-block (unwrap-panic (contract-call? registry-trait get-pool-deactivated-block .arkadiko-stake-pool-xbtc-usda-v1-1)))
...
(begin
  (var-set last-reward-increase-block u43840)
)
