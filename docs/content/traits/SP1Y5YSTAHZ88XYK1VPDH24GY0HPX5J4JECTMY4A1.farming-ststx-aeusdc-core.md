---
title: "Trait farming-ststx-aeusdc-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .ststx-aeusdc)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .ststx-aeusdc transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .ststx-aeusdc get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .ststx-aeusdc transfer amt protocol user none)))
