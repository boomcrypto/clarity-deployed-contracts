...
;; Define LP token
(define-fungible-token DMG-RUN)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.dmg-runes"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? DMG-RUN amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance DMG-RUN who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply DMG-RUN)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP14NS8MVBRHXMM96BQY0727AJ59SWPV7RMHC0NCG.pontis-bridge-ROONS transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP14NS8MVBRHXMM96BQY0727AJ59SWPV7RMHC0NCG.pontis-bridge-ROONS transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP14NS8MVBRHXMM96BQY0727AJ59SWPV7RMHC0NCG.pontis-bridge-ROONS transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? DMG-RUN (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? DMG-RUN (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP14NS8MVBRHXMM96BQY0727AJ59SWPV7RMHC0NCG.pontis-bridge-ROONS get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (add-liquidity u550000000))
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SP14NS8MVBRHXMM96BQY0727AJ59SWPV7RMHC0NCG.pontis-bridge-ROONS transfer u144321708 tx-sender CONTRACT none)))
```
