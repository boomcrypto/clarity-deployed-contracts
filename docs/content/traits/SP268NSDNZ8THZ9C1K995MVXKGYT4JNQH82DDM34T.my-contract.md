...
;; Define the SIP-010 trait for fungible tokens
...
(define-public (tip-ft (token <sip010-trait>) (recipient principal) (amount uint) (memo (optional (buff 34))))
  (begin
    ;; Prevent sender from tipping themselves
    (asserts! (not (is-eq tx-sender recipient)) ERR_SAME_SENDER_RECIPIENT)
    ;; Prevent tipping to the contract itself
    ...
    ;; Transfer full token amount to recipient
    (try! (contract-call? token transfer amount tx-sender recipient))
    ;; Transfer fee to collector
    (try! (stx-transfer? tip-fee tx-sender fee-collector))
    ;; Print memo if provided
    (if (is-some memo)
