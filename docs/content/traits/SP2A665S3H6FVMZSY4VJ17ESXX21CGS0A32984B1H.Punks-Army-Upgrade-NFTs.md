---
title: "Trait Punks-Army-Upgrade-NFTs"
draft: true
---
```
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token Punks-Army-Upgrade uint)
...
(define-private (mint_punk (punk-id uint) (new-owner principal))
	(begin
		...
		(nft-mint? Punks-Army-Upgrade punk-id new-owner)
	)
)
...
(define-public 
	...
	(begin     
		...
		(nft-transfer? Punks-Army-Upgrade token-id sender recipient))
)
;; BURN A TOKEN
(define-public 
	(burn-token 
	...
	(begin     
		(asserts! (is-eq (some tx-sender) (nft-get-owner? Punks-Army-Upgrade token-id) ) (err u403))     
		(nft-burn? Punks-Army-Upgrade token-id tx-sender))
)

(
	define-read-only (get-owner (punk-id uint))  
	(ok (nft-get-owner? Punks-Army-Upgrade punk-id))
)
