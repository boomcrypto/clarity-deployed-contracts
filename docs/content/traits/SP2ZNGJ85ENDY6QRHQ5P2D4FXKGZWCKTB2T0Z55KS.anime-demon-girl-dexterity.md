---
title: "Trait anime-demon-girl-dexterity"
draft: true
---
```
;; Anime Demon Girl - LP Token, AMM DEX and Hold-to-Earn Engine
;; SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.anime-demon-girl-dexterity
...
(define-data-var swap-fee uint u64746) ;; Default to 6.5%
(define-data-var token-uri (optional (string-utf8 256)) 
  (some u"https://charisma.rocks/api/v0/indexes/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.anime-demon-girl-dexterity"))
...
(define-public (swap (forward bool) (amt-in uint))
  (let (
    (sender tx-sender)
    (reserve-in (unwrap-panic (if forward (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve get-balance CONTRACT) 
    ...
    (reserve-out (unwrap-panic (if forward (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token get-balance CONTRACT) 
      (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve get-balance CONTRACT))))
    ...
    ;; Execute the swap
    (try! (if forward (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve transfer amt-in sender CONTRACT none) 
    ...
    (try! (as-contract (if forward (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token transfer amt-out CONTRACT sender none)
      (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve transfer amt-out CONTRACT sender none))))
    (ok {amt-in: amt-in, amt-out: amt-out})))

(define-public (mint (who principal) (amount uint))
  (let (
    (reserve0 (unwrap-panic (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve get-balance CONTRACT)))
    ...
    (try! (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve transfer token0-amount who CONTRACT none))
...
(define-public (burn (who principal) (amount uint))
  (let (
    ...
    (try! (as-contract (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve transfer token0-amount CONTRACT who none)))
...
(define-read-only (get-tokens)
  (ok {token0: 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve, token1: 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token}))
...
(define-read-only (get-reserves)
  (ok {
    token0: (unwrap-panic (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve get-balance CONTRACT)),
