---
title: "Trait wrapped-charisma"
draft: true
---
```
;; Title: Wrapped Charisma
...
(define-constant unlock-block u155855)
(define-constant token-a-ratio u1)
(define-constant token-b-ratio u1)
(define-constant index-token-ratio u2)

(define-fungible-token index-token)

(define-data-var token-name (string-ascii 32) "Wrapped Charisma")
(define-data-var token-symbol (string-ascii 10) "wCHA")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.wrapped-charisma.json"))
(define-data-var token-decimals uint u6)

(define-data-var blocks-per-tx uint u0)
...
(define-public (add-liquidity (amount uint))
    (let
        ...
        (try! (is-unlocked))
        (try! (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token transfer amount-a tx-sender contract none))
        (try! (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token transfer amount-b tx-sender contract none))
...
(define-public (remove-liquidity (amount uint))
    (let
        ...
        (try! (as-contract (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token transfer amount-a contract sender none)))
        (try! (as-contract (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token transfer amount-b contract sender none)))
        (var-set block-counter (+ (var-get block-counter) (var-get blocks-per-tx)))
        (ok true)
    )
)
