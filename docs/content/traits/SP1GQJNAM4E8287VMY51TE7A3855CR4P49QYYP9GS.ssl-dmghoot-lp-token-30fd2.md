---
title: "Trait ssl-dmghoot-lp-token-30fd2"
draft: true
---
```
...
(define-read-only (get-name) (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token get-name))
(define-read-only (get-symbol) (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token get-symbol))
(define-read-only (get-base-decimals) (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token get-decimals))
(define-read-only (get-token-uri) (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token get-token-uri))
(define-read-only (get-balance (who principal)) (if (is-eq who VAULT) (ok (ft-get-balance wrapped-token who)) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token get-balance who))))))
(define-read-only (get-total-supply) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token get-total-supply)))))
  ...
  (begin
    ...
    (if (is-eq sender VAULT)
			(begin
				...
				(as-contract (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token transfer (fixed-to-decimals amount) tx-sender recipient memo)))
			(begin
				(and (is-eq recipient VAULT) (try! (ft-mint? wrapped-token amount recipient)))
				(contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token transfer (fixed-to-decimals amount) tx-sender (if (is-eq recipient VAULT) (as-contract tx-sender) recipient) memo)))))
