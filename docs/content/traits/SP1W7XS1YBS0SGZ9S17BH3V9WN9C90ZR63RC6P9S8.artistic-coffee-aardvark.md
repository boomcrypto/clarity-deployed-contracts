---
title: "Trait artistic-coffee-aardvark"
draft: true
---
```
...
(define-non-fungible-token test-uap uint)
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? test-uap token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? test-uap token-id sender recipient)
  )
)
...
(define-public (mint (recipient principal))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? test-uap token-id recipient))
