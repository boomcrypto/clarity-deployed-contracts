---
title: "Trait muddietoes-21-again"
draft: true
---
```
;; muddietoes-21-again
...
(define-non-fungible-token muddietoes-21-again uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTiczeuVLpq1p1Xp8EhFQUbm5W7UFwfhxLv2JgpLofE8M/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? muddietoes-21-again next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? muddietoes-21-again token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? muddietoes-21-again token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? muddietoes-21-again token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? muddietoes-21-again id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? muddietoes-21-again id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? muddietoes-21-again id) (err ERR-NOT-FOUND)))
