...
(define-non-fungible-token forever-punks uint)
...
(define-data-var artist-address principal 'SPJCKYVNBGAZXYFWB22YAVVWQBZ3S803TZX2S3E3)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPikDe3kNANAFPvGSbpUGfPJSrUrPAdHq6MYYLsa64KQp/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? forever-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? forever-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? forever-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? forever-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? forever-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? forever-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? forever-punks id) (err ERR-NOT-FOUND)))
