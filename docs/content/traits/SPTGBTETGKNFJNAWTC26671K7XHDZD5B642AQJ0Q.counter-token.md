...
(define-constant TOKEN_URI u"count.io") ;; utf-8 string with token metadata host
(define-constant TOKEN_NAME "The Counter Token")
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance counter-token who))
)
...
(define-public (mint)
  (begin
    (ft-mint? counter-token u1 tx-sender)
  )
)
