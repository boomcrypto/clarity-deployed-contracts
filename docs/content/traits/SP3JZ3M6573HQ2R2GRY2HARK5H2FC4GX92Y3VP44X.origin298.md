...
(define-non-fungible-token origin298 uint)
...
(define-data-var artist-address principal 'SP3JZ3M6573HQ2R2GRY2HARK5H2FC4GX92Y3VP44X)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaPS9Fu7Mf3679QJKJdSeu2yjthFwpdeHL1Z3X8rrdf67/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? origin298 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? origin298 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? origin298 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? origin298 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? origin298 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? origin298 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? origin298 id) (err ERR-NOT-FOUND)))
