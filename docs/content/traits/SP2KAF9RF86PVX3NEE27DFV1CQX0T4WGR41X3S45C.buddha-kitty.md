...
(define-non-fungible-token buddha-kitty uint)
...
(define-constant MINT-LIMIT u17)
...
(define-data-var ipfs-full-metadata (string-ascii 80) "ipfs://QmQUmr9832tVqhZRLkKoABDzagLZQ92YiSSJ5zYsyRAXRW/buddha_kitty_metadata.json")
(define-data-var ipfs-root (string-ascii 67) "ipfs://QmQUmr9832tVqhZRLkKoABDzagLZQ92YiSSJ5zYsyRAXRW/buddha_kitty_")
(define-data-var artist-address principal 'SP14YEEA2S46F5EE11G6Z7PJFDW5JAZFG86BACFXB)
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? buddha-kitty next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? buddha-kitty token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? buddha-kitty token-id)))
