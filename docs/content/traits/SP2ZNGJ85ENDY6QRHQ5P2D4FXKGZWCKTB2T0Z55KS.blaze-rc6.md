...
(define-constant message-domain-hash (sha256 (unwrap-panic (to-consensus-buff? { name: "BLAZE_PROTOCOL", version: "rc5", chain-id: chain-id }))))
...
(define-public (submit
  ...
  (let
    (
      ...
      (contract-param (if use-caller-auth (some contract-caller) none))
      ;; Generate the hash using the opcode, uuid, and optional contract-caller
      (hash (try! (hash-args contract-param opcode uuid)))
    )
...
;; The contract parameter is now optional.
