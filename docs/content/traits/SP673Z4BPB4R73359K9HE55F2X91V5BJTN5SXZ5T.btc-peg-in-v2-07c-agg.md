---
title: "Trait btc-peg-in-v2-07c-agg"
draft: true
---
```
...
(define-read-only (validate-tx-agg (commit-tx { tx: (buff 32768), output-idx: uint }) (reveal-tx { tx: (buff 32768), order-idx: uint }))
	(let (
		...
		(asserts! (or 
			(is-eq (get swap-token-in order-details) 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-abtc) 
			(is-eq (try! (contract-call? .cross-router-v2-03 get-approved-wrapped-or-fail (get swap-token-in order-details))) 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-abtc)) err-invalid-token)
		(asserts! (or 
			(is-eq (get swap-token-out order-details) (get token-out order-details)) 
			(is-eq (try! (contract-call? .cross-router-v2-03 get-approved-wrapped-or-fail (get swap-token-out order-details))) (get token-out order-details))) err-invalid-token)		
...
(define-public (finalize-peg-in-agg
	...
	(let (
		...
		(as-contract (try! (contract-call? .cross-peg-out-v2-01a-agg transfer-to-swap (get amount-net validation-data) swap-token-in-trait (get swap-token-out order-details) (get min-amount-out order-details) (get dest-chain-id order-details) { address: (get to order-details), chain-id: (get chain-id order-details) } { address: (get from order-details), chain-id: (some u0) })))			
