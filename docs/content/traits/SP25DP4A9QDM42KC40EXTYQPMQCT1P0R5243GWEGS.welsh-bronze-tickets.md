---
title: "Trait welsh-bronze-tickets"
draft: true
---
```
;; welsh-bronze-tickets
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token welsh-bronze-tickets uint)
...
(define-data-var artist-address principal 'SP3J0EHZHEQSAH6TRTZE0MGRR6NXFS67S6F3MBD8C)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXHUYnGV235XBXVmjvpFpUvxvhEiBisF8jfJdYSCQMTyp/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? welsh-bronze-tickets next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? welsh-bronze-tickets token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? welsh-bronze-tickets token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? welsh-bronze-tickets token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? welsh-bronze-tickets id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? welsh-bronze-tickets id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? welsh-bronze-tickets id) (err ERR-NOT-FOUND)))
