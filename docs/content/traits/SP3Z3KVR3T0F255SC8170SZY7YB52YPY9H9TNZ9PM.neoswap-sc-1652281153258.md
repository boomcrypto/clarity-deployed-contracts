---
title: "Trait neoswap-sc-1652281153258"
draft: true
---
```
...
(begin
	(unwrap-panic (begin
		...
		(as-contract (stx-transfer? u3600000 tx-sender agent-1)))
	)
	(unwrap-panic (begin
		...
		(as-contract (stx-transfer? u1000018 tx-sender agent-2)))
	)
	...
	(unwrap-panic (begin
		...
		(as-contract (stx-transfer? u1689964 tx-sender agent-4)))
	)
	(unwrap-panic (begin
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u3060000 tx-sender agent-0)))
	)
...
(begin        
	...
	(unwrap-panic (begin
		...
		(as-contract (stx-transfer? u14349982 tx-sender agent-3)))
	)
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SP32AEEF6WW5Y0NMJ1S8SBSZDAY8R5J32NBZFPKKZ.free-punks-v0 transfer u5869 tx-sender agent-4)))
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u14349982 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
