---
title: "Trait project-indigo-container-practice"
draft: true
---
```
;; Final Practice Container(s) NFT
;; Final Practice Container NFT for Project Indigo
;; Written by Setzeus / ClarityClear

;;;;;; IGNORE CONTRACT - NOT FUNCTIONAL ;;;;;;;;;

;; Container(s)
;; Containers are burned for other NFTs within the Project-Indigo ecosystem (equipment, artifacts, treasure, etc...)
...
;; Gameplay Earn
;; 434 Containers will be reserved for server-side airdrops
...
(define-constant purchase-price u300000)
...
;; Container Initial Collection Size (8k)
(define-data-var collection-limit-total uint u15)

;; Uint var that keeps track of containers nft index
(define-data-var containers-index uint u1)

;; Container Initial Earn Size (434)
(define-data-var collection-limit-earn uint u435)

;; Uint var that keeps track of earned
(define-data-var containers-earned-index uint u1)

;; Container Initial Earn Size (6001)
(define-data-var collection-limit-purchase uint u6001)

;; Uint var that keeps track of earned
(define-data-var containers-purchased-index uint u1)

;; Container Initial Snapshot Airdrop Size (~1.5k) -> need final figure
(define-data-var collection-limit-snapshot uint u1565)

;; Uint var that keeps track of snapshot airdrops
(define-data-var containers-snapshot-index uint u1)

;; Container Total = Earn + Snapshot + Purchase (leftover)
;; Total is the collection total at any given time. When collection is extended total increases by the total new amount of Containers provided.
;; Earn is the amount of Containers that are earned by users during gameplay. When collection is extended, the Earn amount & earn amount index are both reset to the new value & u1 respectively.
;; Snapshot is the amount of Containers that are airdropped to users based on a snapshot of the community. We need to keep track of index since we might not be able to airdrop them all at once (likely if need to drop > 250).
;; Purchase is the amount of Containers that are available for purchase. Purchased also needs to be checked & updated when collection is extended.
...
(define-read-only (get-purchase-remaining) 
    (- (var-get collection-limit-purchase) (var-get containers-purchased-index))
)

;; Get Airdrops Remaining
(define-read-only (get-earn-remaining) 
    (- (var-get collection-limit-earn) (var-get containers-earned-index))
)
...
(define-read-only (get-last-token-id)
  (ok (var-get containers-index))
)

(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? container id))
)
...
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? container id sender recipient)
  )
)
...
(define-private (is-sender-owner (id uint))
  (let
    (
      (owner (unwrap! (nft-get-owner? container id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let
    (
      (owner (unwrap! (nft-get-owner? container id) ERR-NOT-AUTH))
...
(define-public (mint-container-purchase)
  (let
    (
      (current-id (var-get containers-index))
      (next-id (+ current-id u1))
      (current-purchase-id (var-get containers-purchased-index))
    ...
    (try! (nft-mint? container current-id tx-sender))

    ;; update purchase index
    (var-set containers-purchased-index next-purchased)

    ;; update containers-index
    (ok (var-set containers-index next-id))

  )
)

;; Purchase-Mint x3
(define-public (mint-container-purchase-x3) 
  (begin
    (try! (mint-container-purchase))
    (try! (mint-container-purchase))
    (ok (try! (mint-container-purchase)))
  )
)

;; Purchase-Mint x10
(define-public (mint-container-purchase-x10) 
  (begin 
    (try! (mint-container-purchase))
    (try! (mint-container-purchase))
    (try! (mint-container-purchase))
...
;; @desc function for admin-server to airdrop a Container to a user that earned it in the ecosystem
;; @param bud-id: the ID of the Bud being used by client to claim a bud
(define-public (mint-container-earn (user principal))
  (let
    (
      (current-id (var-get containers-index))
      (next-id (+ current-id u1))
      (current-earn-id (var-get containers-earned-index))
    ...
    ;; mint container to user
    (try! (nft-mint? container current-id user))

    ;; update containers-index
    (var-set containers-index next-id)

    ;; update containers-earned
    (ok (var-set containers-earned-index next-earned))
  )
)
...
(define-public (extend-collection (new-purchase uint) (new-snapshot uint) (new-earn uint) (new-ipfs (string-ascii 144))) 
  (let 
    ...
    (var-set containers-snapshot-index u1)

    ;; set new earn
    (var-set collection-limit-earn next-earn)

    ;; reset earn index
    (var-set containers-earned-index u1)

    ;; set new purchase
    (var-set collection-limit-purchase next-purchase)

    ;; reset purchase index
    (var-set containers-purchased-index u1)
...
;; Snapshot Airdrops ;;
;;;;;;;;;;;;;;;;;;;;;;;
;; @desc function for any admin to mass-airdrop a Container to a list of of recipients/principals - up to 250 recipients per call.  
;; @param recipients: list of all principals to receive a Container during this airdrop
(define-public (snapshot-airdrops (recipients (list 250 principal)))
  (let 
    (
      ...
      (current-snapshot-index (var-get containers-snapshot-index))
    ...
    ;; mass-airdrop a Container to each recipient in the list
...
;; Map over the list of recipients (principal) & mint a Container for each recipient
(define-private (mass-airdrop (recipient principal)) 
  (let 
    ...
    ;; mint a Container for each recipient
    (try! (nft-mint? container current-id tx-sender))

    ;; increment snapshot index
    (var-set containers-snapshot-index new-snapshot-index) 

    ;; increment containers index
    (ok (var-set containers-index next-id))
  )
)
...
;; burn function
(define-public (burn-container (container-id uint))
  (begin 

    ;; assert tx-sender is owner
    (asserts! (is-eq (some tx-sender) (unwrap! (get-owner container-id) ERR-ALREADY-WHITELISTED)) ERR-ALREADY-WHITELISTED)

    ;; assert that contract-caller is containers.clar 
    (asserts! (is-eq contract-caller .equipment) ERR-NOT-AUTH)

    ;; burn container
    (nft-burn? container container-id tx-sender)
  )
)
