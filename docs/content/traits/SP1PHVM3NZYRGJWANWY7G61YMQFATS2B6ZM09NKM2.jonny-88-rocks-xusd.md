---
title: "Trait jonny-88-rocks-xusd"
draft: true
---
```
...
(define-non-fungible-token jonny-88-rocks-xusd uint)
...
(define-data-var private-target-block-height uint u64110)
(define-data-var public-target-block-height uint u64398)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u60000000000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? jonny-88-rocks-xusd id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace jonny-88-rocks-xusd
  (ok (nft-get-owner? jonny-88-rocks-xusd id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? jonny-88-rocks-xusd next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? jonny-88-rocks-xusd id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? jonny-88-rocks-xusd id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? jonny-88-rocks-xusd id) ERR-NOT-FOUND))
...
(define-data-var xusd-cost-per-mint uint u45000000000000)
