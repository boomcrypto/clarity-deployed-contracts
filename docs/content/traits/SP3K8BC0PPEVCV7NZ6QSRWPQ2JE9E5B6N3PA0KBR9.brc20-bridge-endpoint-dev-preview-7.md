---
title: "Trait brc20-bridge-endpoint-dev-preview-7"
draft: true
---
```
...
(define-constant err-asset-not-supported-on-b20 (err u1012))
(define-constant MAX_UINT u340282366920938463463374607431768211455)
(define-constant ONE_8 u100000000)
(define-constant gas-fee-token .token-susdt)
(define-constant stxdx-registry .stxdx-registry)
(define-constant stxdx-wallet-zero .stxdx-wallet-zero)
...
(define-public (finalize-peg-in (tx (buff 4096)) (output-idx uint) (offset-idx uint) (order-idx uint) (token-trait <sip010-trait>))
	(let (
		...
		(ok true)))
(define-public (finalize-peg-in-and-transfer-in (tx (buff 4096)) (output-idx uint) (offset-idx uint) (order-idx uint) (token-trait <sip010-trait>))
	(let (
			(token (contract-of token-trait))
			(validation-data (try! (validate-tx tx output-idx offset-idx order-idx token)))
			(tx-idxed (get tx-idxed validation-data))
			...
			(fee (get fee validation-data))
			(amt-net (get amt-net validation-data))
			(asset-id (unwrap! (contract-call? stxdx-registry get-asset-id token) err-asset-not-supported-on-b20))
			(user-id (try! (contract-call? stxdx-registry get-user-id-or-fail order-address))))
		(asserts! (not (get peg-in-paused token-details)) err-paused)
		(as-contract (try! (contract-call? .brc20-bridge-registry-dev-preview-2 set-peg-in-sent { tx: tx, output: output-idx, offset: offset-idx } true)))
		(and (> fee u0) (as-contract (try! (contract-call? token-trait mint-fixed fee (var-get fee-address)))))
		(and (> amt-net u0) (as-contract (try! (contract-call? token-trait mint-fixed amt-net tx-sender))))
		(and (> amt-net u0) (as-contract (try! (contract-call? stxdx-wallet-zero transfer-in amt-net user-id asset-id token-trait))))
		(print (merge tx-idxed { type: "peg-in-and-transfer-in", order-address: order-address, fee: fee, amt-net: amt-net, user-id: user-id, asset-id: asset-id }))
		(ok true)))
