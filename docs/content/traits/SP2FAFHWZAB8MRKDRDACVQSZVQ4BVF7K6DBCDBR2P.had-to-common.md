...
(define-non-fungible-token had-to-common uint)
...
(define-data-var total-price uint u400000)
(define-data-var artist-address principal 'SP2FAFHWZAB8MRKDRDACVQSZVQ4BVF7K6DBCDBR2P)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmexkGuZ5bmXyQ2wdhuQHBB6Mjmd5DnX9Fsd9HEuyqG6m4/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? had-to-common next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? had-to-common token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? had-to-common token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? had-to-common token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? had-to-common id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? had-to-common id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? had-to-common id) (err ERR-NOT-FOUND)))
