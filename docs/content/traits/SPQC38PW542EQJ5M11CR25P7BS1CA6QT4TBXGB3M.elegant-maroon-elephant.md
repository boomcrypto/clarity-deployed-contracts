---
title: "Trait elegant-maroon-elephant"
draft: true
---
```
;; xyk-sbtc-reader-pool-21-plus-staking-v-1-1
...
;; Get total sBTC balance in sBTC-STX XYK pool
(define-read-only (get-total-sbtc-balance)
  (let (
    ...
    (pool-sbtc-balance (unwrap! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token get-balance
                                'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-pool-sbtc-stx-v-1-1) ERR_NO_POOL_DATA))
  )
...
;; Get a user's sBTC balance in sBTC-STX XYK pool
(define-read-only (get-user-sbtc-balance (user principal))
  (let (
    ;; Gather all pool data
    (pool-data (unwrap! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-pool-sbtc-stx-v-1-1 get-pool) ERR_NO_POOL_DATA))
    ...
    ;; Get user's LP Token balance and staked LP Token balance
    (user-lp-balance (unwrap! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.xyk-pool-sbtc-stx-v-1-1 get-balance
                              user) ERR_NO_USER_DATA))
    (user-staking-data (unwrap! (contract-call? 'SPQC38PW542EQJ5M11CR25P7BS1CA6QT4TBXGB3M.xyk-staking-sbtc-stx-v-1-1 get-user
                                user) ERR_NO_USER_DATA))
    (user-lp-staked-balance (default-to u0 (get lp-staked user-staking-data)))

    ;; Calculate user's total LP Token balance and proportional sBTC balance
    (user-total-lp-balance (+ user-lp-balance user-lp-staked-balance))
