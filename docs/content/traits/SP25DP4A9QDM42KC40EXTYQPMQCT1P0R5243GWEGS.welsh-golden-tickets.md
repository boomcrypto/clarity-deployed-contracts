---
title: "Trait welsh-golden-tickets"
draft: true
---
```
;; welsh-golden-tickets
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token welsh-golden-tickets uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZ4K58VLwiWwwndh9rHtzfp3ekxS9HBBXJcBoyRRmrUbS/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? welsh-golden-tickets next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? welsh-golden-tickets token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? welsh-golden-tickets token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? welsh-golden-tickets token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? welsh-golden-tickets id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? welsh-golden-tickets id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? welsh-golden-tickets id) (err ERR-NOT-FOUND)))
