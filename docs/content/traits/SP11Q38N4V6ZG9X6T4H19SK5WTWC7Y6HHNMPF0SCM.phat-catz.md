...
(define-non-fungible-token phat-catz uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVBGdMwbC7LSTvKxvGvUFD9nRNj1h5Ygs7Sk1TomsZ58R/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? phat-catz next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? phat-catz token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? phat-catz token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? phat-catz token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? phat-catz id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? phat-catz id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? phat-catz id) (err ERR-NOT-FOUND)))
