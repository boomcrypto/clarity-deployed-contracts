...
(define-non-fungible-token bitcowboys uint)
...
(define-constant ITEM-COUNT u512)

;; variables
(define-data-var metadata-frozen bool false)
(define-data-var bitcowboys-counter uint u0)
(define-data-var bitcowboys-index uint u0)
(define-data-var token-uri (string-ascii 256) "ipfs://QmQFuUrY9RnCWGZ28uqJsgQW1C8CmBoi8K78sWNTgVCtQi")
(define-data-var cost-per-mint uint u25000000)
(define-data-var creator-address principal 'SPZ7MME7JAPGGQP3SMEXDPCGGP00S0KC3P1KSEFM)
...
(define-public (mint)
  (let (
    (count (var-get bitcowboys-counter))
  )
...
(define-private (mint-next)
  (let (
    (count (var-get bitcowboys-counter))
    (idx (var-get bitcowboys-index))
    (random-bitcowboys-id
    ...
    (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
      success (begin
        (try! (nft-mint? bitcowboys random-bitcowboys-id tx-sender))
        (var-set bitcowboys-counter (+ count u1))
        (if (is-eq u0 (var-get rotation))
          (begin
            (var-set rotation u1)
            (var-set bitcowboys-index (+ u1 (var-get bitcowboys-index)))
          )
          (var-set rotation u0)
        )
...
(define-public (burn (index uint))
  (let (
    ...
    (if (is-owner index tx-sender)
      (match (nft-burn? bitcowboys index tx-sender)
        success (begin
          (map-set token-count tx-sender (- current-balance u1))
          (ok true)
        )
...
(define-public (transfer (index uint) (owner principal) (recipient principal))
  (let (
    ...
    (if (and (is-owner index owner) (is-owner index tx-sender))
      (match (nft-transfer? bitcowboys index owner recipient)
        success (begin
...
(define-read-only (get-owner (index uint))
  (ok (nft-get-owner? bitcowboys index))
)
...
(define-private (is-owner (index uint) (user principal))
  (is-eq user (unwrap! (nft-get-owner? bitcowboys index) false))
)
