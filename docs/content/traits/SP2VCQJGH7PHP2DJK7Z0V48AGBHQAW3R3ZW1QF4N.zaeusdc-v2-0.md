...
(define-read-only (get-total-supply)
  (contract-call? .zaeusdc-token get-total-supply))

(define-read-only (get-name)
  (contract-call? .zaeusdc-token get-name))

(define-read-only (get-symbol)
  (contract-call? .zaeusdc-token get-symbol))

(define-read-only (get-decimals)
  (contract-call? .zaeusdc-token get-decimals))

(define-read-only (get-token-uri)
  (contract-call? .zaeusdc-token get-token-uri))
...
(define-private (transfer-internal (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (contract-call? .zaeusdc-token transfer amount sender recipient memo)
)

(define-private (mint-internal (amount uint) (owner principal))
  (contract-call? .zaeusdc-token mint amount owner)
)

(define-private (burn-internal (amount uint) (owner principal))
  (contract-call? .zaeusdc-token burn amount owner)
)
...
(define-read-only (get-principal-balance (account principal))
  (ok
    (contract-call? .zaeusdc-token get-balance account)
  )
)
