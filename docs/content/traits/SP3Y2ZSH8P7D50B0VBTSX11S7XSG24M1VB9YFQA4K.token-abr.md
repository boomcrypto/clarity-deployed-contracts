...
(define-data-var token-uri (string-utf8 256) u"https://allbridge.io/.well-known/stx-abr.json")
(define-data-var name (string-ascii 32) "Allbridge")
(define-data-var symbol (string-ascii 32) "ABR")
...
(define-fungible-token abr)

(define-read-only 
	(get-total-supply)
  	(ok (ft-get-supply abr))
)
...
(define-private 
	...
	(begin 
		...
		(match 
			(ft-transfer? abr amount sender recipient)
			response (begin
				(print memo)
				(ok response)
			)
