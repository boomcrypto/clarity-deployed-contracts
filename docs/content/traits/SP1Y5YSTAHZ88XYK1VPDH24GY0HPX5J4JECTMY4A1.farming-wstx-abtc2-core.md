---
title: "Trait farming-wstx-abtc2-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-abtc2)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-abtc2 transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-abtc2 get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-abtc2 transfer amt protocol user none)))
