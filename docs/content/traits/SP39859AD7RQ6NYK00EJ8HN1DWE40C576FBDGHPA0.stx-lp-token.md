...
(define-constant LP_REBATE u15000)
...
;; Define LP token
(define-fungible-token STXLP)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.stx-lp-token"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? STXLP amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance STXLP who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply STXLP)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.ukrainian-hryvnia-stxcity transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.ukrainian-hryvnia-stxcity transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.ukrainian-hryvnia-stxcity transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? STXLP (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? STXLP (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.ukrainian-hryvnia-stxcity get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.ukrainian-hryvnia-stxcity transfer u4999999000000 tx-sender CONTRACT none)))
```
