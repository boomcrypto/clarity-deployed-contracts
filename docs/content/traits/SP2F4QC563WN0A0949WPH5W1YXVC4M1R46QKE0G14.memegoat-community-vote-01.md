---
title: "Trait memegoat-community-vote-01"
draft: true
---
```
;;
...
(define-public (execute (sender principal) (opt uint))
  (begin
    (try! (is-dao-or-extension))
    (if (is-eq opt u0)
      (contract-call? .memegoat-game-record set-ticket-price u100000)
      (if (is-eq opt u1)
        (contract-call? .memegoat-game-record set-ticket-price u200000)
        (if (is-eq opt u2)
          (contract-call? .memegoat-game-record set-ticket-price u300000)
          (if (is-eq opt u3)
           (contract-call? .memegoat-game-record set-ticket-price u400000)
            (ok false)
          )
        )
      )
