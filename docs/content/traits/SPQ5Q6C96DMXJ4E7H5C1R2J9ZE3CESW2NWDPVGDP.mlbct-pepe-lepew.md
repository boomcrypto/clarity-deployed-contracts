---
title: "Trait mlbct-pepe-lepew"
draft: true
---
```
(impl-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)

(define-fungible-token pepe-lepew)
...
(define-constant dev-principal 'SPQ5Q6C96DMXJ4E7H5C1R2J9ZE3CESW2NWDPVGDP)
(define-constant token-supply u333000777)
...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://bafybeifu3upluvbefqgogcycyg5qz5kke4cf3gtqehdtmar7li5zeuck3e.ipfs.dweb.link/")) 
...
(define-read-only (get-name) (ok "Pepe LePew"))
(define-read-only (get-symbol) (ok "LePew"))
(define-read-only (get-decimals) (ok u0))
(define-read-only (get-balance (user principal)) (ok (ft-get-balance pepe-lepew user)))
(define-read-only (get-total-supply) (ok (ft-get-supply pepe-lepew)))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? pepe-lepew amount from to)
    )
)
...
(define-public (swap-stx-for-token (stx-amount uint) (min-tokens-out uint)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? pepe-lepew tokens-out (as-contract tx-sender) tx-sender))
      (if (>= (/ (* (+ (var-get real-stx-reserve) (var-get virtual-stx-reserve)) token-supply) (var-get token-reserve)) market-cap-threshold)
        (begin
          (let (
            ...
            (try! (ft-transfer? pepe-lepew burn-amount (as-contract tx-sender) burn-address))
            (try! (ft-transfer? pepe-lepew remaining-tokens (as-contract tx-sender) liquidity-builder-address))
...
(define-public (swap-token-for-stx (token-amount uint) (min-stx-out uint) (recipient principal)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? pepe-lepew token-amount tx-sender (as-contract tx-sender)))
...
(begin
  (try! (ft-mint? pepe-lepew token-supply (as-contract tx-sender)))
