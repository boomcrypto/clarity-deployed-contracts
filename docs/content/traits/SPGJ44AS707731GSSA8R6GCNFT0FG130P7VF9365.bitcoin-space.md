...
(define-non-fungible-token bitcoin-space uint)
...
(define-data-var artist-address principal 'SPGJ44AS707731GSSA8R6GCNFT0FG130P7VF9365)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmR43KuCLvdpgoWmV7gwouMKBofA7wnZxGnHrUzFtA7aS2/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-space next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? bitcoin-space token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-space token-id)))
