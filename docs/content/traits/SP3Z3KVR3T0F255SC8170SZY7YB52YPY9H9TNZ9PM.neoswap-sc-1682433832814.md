---
title: "Trait neoswap-sc-1682433832814"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/yJxW3qLUjQz0Yt7GH7d3
;; SwapId: n5R89jKlwADKNj7IY3ge
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u6560748 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u6560748 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u6560748 tx-sender TRADER-2)))
	    )
            true
        )
