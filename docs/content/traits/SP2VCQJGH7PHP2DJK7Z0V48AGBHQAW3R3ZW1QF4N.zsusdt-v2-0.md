...
(define-constant asset-addr 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt)
(define-constant decimals u8)

(define-read-only (get-total-supply)
  (contract-call? .zsusdt-token get-total-supply))

(define-read-only (get-name)
  (contract-call? .zsusdt-token get-name))

(define-read-only (get-symbol)
  (contract-call? .zsusdt-token get-symbol))

(define-read-only (get-decimals)
  (contract-call? .zsusdt-token get-decimals))

(define-read-only (get-token-uri)
  (contract-call? .zsusdt-token get-token-uri))
...
(define-private (transfer-internal (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (contract-call? .zsusdt-token transfer amount sender recipient memo)
)

(define-private (mint-internal (amount uint) (owner principal))
  (contract-call? .zsusdt-token mint amount owner)
)

(define-private (burn-internal (amount uint) (owner principal))
  (contract-call? .zsusdt-token burn amount owner)
)
...
(define-read-only (get-principal-balance (account principal))
  (ok
    (contract-call? .zsusdt-token get-balance account)
  )
)
...
(define-public (set-contract-owner (owner principal))
  (begin
    ...
    (print { type: "set-contract-owner-zsusdt", payload: owner })
