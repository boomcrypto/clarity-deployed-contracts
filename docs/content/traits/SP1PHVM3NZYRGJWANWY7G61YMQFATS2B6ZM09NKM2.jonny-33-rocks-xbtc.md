---
title: "Trait jonny-33-rocks-xbtc"
draft: true
---
```
...
(define-non-fungible-token jonny-33-rocks-xbtc uint)
...
(define-data-var private-target-block-height uint u64115)
(define-data-var public-target-block-height uint u64403)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? jonny-33-rocks-xbtc id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace jonny-33-rocks-xbtc
  (ok (nft-get-owner? jonny-33-rocks-xbtc id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? jonny-33-rocks-xbtc next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? jonny-33-rocks-xbtc id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? jonny-33-rocks-xbtc id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? jonny-33-rocks-xbtc id) ERR-NOT-FOUND))
