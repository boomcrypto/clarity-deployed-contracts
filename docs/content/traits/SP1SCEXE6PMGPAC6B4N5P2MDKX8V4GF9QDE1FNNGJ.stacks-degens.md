...
(define-non-fungible-token STACKS-DEGENS uint)
...
(define-data-var ipfs-root (string-ascii 80) "https://stacksdegens.com/SDGU/StacksDegens/jsons/")
...
(match (nft-mint? STACKS-DEGENS next-id new-owner)
  success
  (begin
...
(if (and
    ...
    (match (nft-transfer? STACKS-DEGENS token-id sender recipient)
      success (ok success)
      error (err error)
    )
    (err err-invalid-user)
...
(ok (nft-get-owner? STACKS-DEGENS token-id)))
