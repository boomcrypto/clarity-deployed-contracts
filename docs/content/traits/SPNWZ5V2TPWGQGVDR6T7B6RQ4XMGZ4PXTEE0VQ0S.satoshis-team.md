...
(define-non-fungible-token satoshis-team uint)

;; Constants
(define-constant err-no-more-nfts u300)
(define-constant err-invalid-user u500)

(define-constant mint-limit u14500)
...
(define-data-var artist-address principal 'SP3BJSVA9E8MJAKK3ZNAGZKG91KY82G6299DY1YBW)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWDSEa8HSwutmBDY129xJbet6t9GHHxAPiuWPAzw9bQ92/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? satoshis-team next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? satoshis-team token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? satoshis-team token-id)))
