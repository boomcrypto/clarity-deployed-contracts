---
title: "Trait ht-vault-ststx-earn-v3"
draft: true
---
```
;; @contract Vault
...
(define-read-only (get-total-pending-withdrawals) 
  (unwrap-panic (as-contract (contract-call? .ht-token-ststx-earn-v3 get-balance tx-sender))))

(define-read-only (get-total-tokens)
  (unwrap-panic (contract-call? .ht-token-ststx-earn-v3 get-total-supply)))
...
(define-public (queue-deposit
  ...
  (let (
    ...
    (hq-data (contract-call? .ht-hq-ststx-earn-v3 get-deposit-data)))
...
(define-public (activate-pending-deposit-claims)
  (begin
    (asserts! (is-eq tx-sender .core-ststx-v1) ERR_ONLY_CORE_CONTRACT_ALLOWED)
...
(define-public (queue-withdrawal 
  ...
  (let (
    (claim-id (+ (get-current-claim-id) u1)))
    (asserts! (> amount u0) ERR_INVALID_AMOUNT)
    (try! (contract-call? .ht-token-ststx-earn-v3 transfer amount tx-sender vault none))
    (map-insert claims { claim-id: claim-id } 
      {
        address: tx-sender,
        epoch-id: (get-current-epoch-id),
...
(define-public (activate-pending-withdrawal-claims)
  (let (
    ...
    (if (> current-total-pending-withdrawals u0)
      (try! (as-contract (contract-call? .ht-token-ststx-earn-v3 burn-for-vault current-total-pending-withdrawals (as-contract tx-sender))))
      true
    )
    (ok true)))
  ...
  (let (
    ...
    (if (and (> (len deposit-claims) u0) (get claim-deposits entry))
      (begin
        ...
        (if (> (var-get claim-amount-helper) u0) (try! (as-contract (contract-call? .ht-token-ststx-earn-v3 mint-for-vault (var-get claim-amount-helper) address))) true)
    ...
    (if (and (> (len withdrawal-claims) u0) (get claim-withdrawals entry))
      (begin
        ...
        (let (
          (current-withdrawal-fee (get current (try! (contract-call? .ht-hq-ststx-earn-v3 get-fees "withdrawal"))))
