...
(define-non-fungible-token theopatra-king uint)
...
(define-data-var total-price uint u444000000)
(define-data-var artist-address principal 'SP45EDJKSKGCZVDPN14210Y5RBRWV4MYKT3K6NVX)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeF8y7xyXobdUvTpymbfebmFh3FW897zeqH8bigbTZWfG/king/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? theopatra-king next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? theopatra-king token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? theopatra-king token-id)))
