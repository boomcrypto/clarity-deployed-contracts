...
(define-non-fungible-token picasso-magic-2 uint)
...
;; Internal variables
(define-data-var mint-limit uint u238)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZsBBnsFG1GdeMG6uGTqLSMELJsipCehWrPBSY9MGNNQv/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? picasso-magic-2 next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? picasso-magic-2 token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? picasso-magic-2 token-id)))
