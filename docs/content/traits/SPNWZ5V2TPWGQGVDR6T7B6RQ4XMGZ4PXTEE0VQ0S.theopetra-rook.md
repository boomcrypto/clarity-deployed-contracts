...
(define-non-fungible-token theopetra-rook uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeF8y7xyXobdUvTpymbfebmFh3FW897zeqH8bigbTZWfG/rook/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? theopetra-rook next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? theopetra-rook token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? theopetra-rook token-id)))
