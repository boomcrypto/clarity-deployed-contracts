...
(define-public (stake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    (begin 
        ...
        (try! (contract-call? .test-bonus-v3 check-bonuses tx-sender))
        (ok true)
    )
)
...
(define-public (collect)
  (let (
      ...
      (bonus (contract-call? .test-bonus-v3 check-bonuses tx-sender))
      (bonus-balance (get points-balance (contract-call? .test-bonus-v3 get-staked-bonuses tx-sender)))      
    )
    (asserts! (is-eq (var-get shutoff-valve) false) (err ERR-NOT-AUTHORIZED))
    (try! (contract-call? .test-bonus-v3 bonus-collect tx-sender))
...
(try! (contract-call? .test-bonus-v3 helper-change (as-contract tx-sender)))
```
