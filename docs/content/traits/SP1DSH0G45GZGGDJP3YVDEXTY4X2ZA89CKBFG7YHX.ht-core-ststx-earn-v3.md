---
title: "Trait ht-core-ststx-earn-v3"
draft: true
---
```
;; @contract Core
...
(define-private (settle) 
  (begin 
    ...
    (try! (as-contract (contract-call? .ht-hq-ststx-earn-v3 update-fees-and-settings)))
    (unwrap-panic (reset-data))
    (ok true)))

(define-private (charge-fees)
  (let (
    (current-performance-fee (get current (try! (contract-call? .ht-hq-ststx-earn-v3 get-fees "performance"))))
    (current-management-fee (get current (try! (contract-call? .ht-hq-ststx-earn-v3 get-fees "management"))))
    (fee-address (contract-call? .ht-hq-ststx-earn-v3 get-fee-address))
    (no-profit-management-fee (contract-call? .ht-hq-ststx-earn-v3 get-no-profit-management-fee))
    ...
    (if (and strike-set (not (var-get delinquent-epoch)))
      (if (> (* unit-pnl units-transacted) total-premium)
        (begin
          (try! 
            (as-contract (contract-call? .ht-vault-ststx-earn-v3 payout-funds
...
(define-private (settle-epoch-info)
  (let (
    (underlying-per-token (contract-call? .ht-vault-ststx-earn-v3 get-underlying-per-token))
    (current-epoch-info (unwrap-panic (get-current-epoch-info)))) 
    ...
    (as-contract (contract-call? .ht-vault-ststx-earn-v3 update-epoch-info-for-claims))))

(define-private (activate-pending-claims) 
  (begin
    (try! (as-contract (contract-call? .ht-vault-ststx-earn-v3 activate-pending-deposit-claims)))
    (try! (as-contract (contract-call? .ht-vault-ststx-earn-v3 activate-pending-withdrawal-claims)))
    (ok true)))
...
(define-private (init-trading-funds-available)
  (begin
    ...
    (/ 
      (* 
        (contract-call? .ht-vault-ststx-earn-v3 get-total-underlying-active)
        (contract-call? .ht-hq-ststx-earn-v3 get-epoch-risk)) 
      bps-base))
    (ok true)))
...
(define-public (determine-pnl
  ...
  (let (
    ...
    (option-type (contract-call? .ht-hq-ststx-earn-v3 get-option-type))
    (strategy-type (contract-call? .ht-hq-ststx-earn-v3 get-strategy-type))
    (unit-size (contract-call? .ht-hq-ststx-earn-v3 get-unit-size))
    ...
    (total-premium (get total-premium current-epoch-info)))
    (try! (contract-call? .ht-hq-ststx-earn-v3 check-is-updater tx-sender))
    (try! (contract-call? .ht-hq-ststx-earn-v3 check-is-pnl-calculator-active (contract-of pnl-calculator-contract)))
    ...
    (asserts! (<= timestamp (+ current-epoch-expiry (contract-call? .ht-hq-ststx-earn-v3 get-pnl-data-window))) ERR_SETTLEMENT_WINDOW_CLOSED)
...
(define-public (start-new-epoch
  ...
  (let (
    ...
    (try! (contract-call? .ht-hq-ststx-earn-v3 check-is-trader tx-sender))
    (try! (contract-call? .ht-hq-ststx-earn-v3 check-is-pnl-calculator-active (contract-of pnl-calculator-contract)))
    (asserts! (contract-call? .ht-hq-ststx-earn-v3 get-trading-allowed) ERR_TRADING_NOT_ALLOWED)
    ...
    (if (and (is-eq current-epoch-expiry expiry) (not current-epoch-settled))
      ...
      (begin 
        ...
        (asserts! (>= expiry (+ current-epoch-expiry (contract-call? .ht-hq-ststx-earn-v3 get-min-epoch-duration))) ERR_VIOLATES_MIN_EPOCH_DURATION)
        (asserts! (<= expiry (+ current-epoch-expiry (contract-call? .ht-hq-ststx-earn-v3 get-max-epoch-duration))) ERR_VIOLATES_MAX_EPOCH_DURATION)
        (map-set epoch-info { epoch-id: new-epoch-id } 
          {
            ...
            total-premium: u0,
            total-underlying-active: (contract-call? .ht-vault-ststx-earn-v3 get-total-underlying-active), 
            underlying-per-token-settled: none,
            epoch-risk: (contract-call? .ht-hq-ststx-earn-v3 get-epoch-risk),
            unit-size: (contract-call? .ht-hq-ststx-earn-v3 get-next-unit-size)
          }
        )
        (try! (as-contract (contract-call? .ht-vault-ststx-earn-v3 create-epoch-info-for-claims)))
...
(define-public (register-trade
  ...
  (let (
    (current-epoch-info (unwrap-panic (get-current-epoch-info)))
    (funds-outgoing (* amount (/ (* option-price-per-underlying (contract-call? .ht-hq-ststx-earn-v3 get-unit-size)) underlying-base))))
    (try! (contract-call? .ht-hq-ststx-earn-v3 check-is-trader tx-sender))
    ...
    (asserts! (<= burn-block-height (+
      ...
      (contract-call? .ht-hq-ststx-earn-v3 get-registration-window))) 
...
(define-public (confirm-trade
  ...
  (let (
    ...
    (funds-outgoing (/ (* (* option-price-per-underlying amount) (contract-call? .ht-hq-ststx-earn-v3 get-unit-size)) underlying-base))
    (current-epoch-info (unwrap-panic (get-current-epoch-info))))
    ...
    (asserts! (<= burn-block-height (+ block-height-registered (contract-call? .ht-hq-ststx-earn-v3 get-confirmation-window))) ERR_CONFIRMATION_WINDOW_CLOSED)
    (try! (as-contract (contract-call? .ht-vault-ststx-earn-v3 payout-funds funds-outgoing counterparty)))
...
(define-public (make-payment
  ...
  (let (
    ...
    (payment-window (contract-call? .ht-hq-ststx-earn-v3 get-payment-window)))
    ...
    (try! (contract-call? .ht-vault-ststx-earn-v3 deposit-funds amount tx-sender))
...
(define-public (initialize
  (first-expiry uint))
  (begin
    (try! (contract-call? .ht-hq-ststx-earn-v3 check-is-admin tx-sender))
...
(define-public (force-settle)
  (let (
    ...
    (asserts! (> burn-block-height (+ block-height-pnl (contract-call? .ht-hq-ststx-earn-v3 get-payment-window))) ERR_PAYMENT_WINDOW_STILL_OPEN)
...
(define-public (force-determine-pnl
  ...
  (let (
    ...
    (current-epoch-info (unwrap-panic (get-current-epoch-info)))
    (current-epoch-expiry (get epoch-expiry current-epoch-info))
    (pnl-calculation-window (contract-call? .ht-hq-ststx-earn-v3 get-pnl-calculation-window)))
...
(define-public (force-activate-pending-deposit-claims)
  (begin
    (try! (contract-call? .ht-hq-ststx-earn-v3 check-is-admin tx-sender))
    (as-contract (contract-call? .ht-vault-ststx-earn-v3 activate-pending-deposit-claims))))

(define-public (force-activate-pending-withdrawal-claims)
  (begin
    (try! (contract-call? .ht-hq-ststx-earn-v3 check-is-admin tx-sender))
    (as-contract (contract-call? .ht-vault-ststx-earn-v3 activate-pending-withdrawal-claims))))
```
