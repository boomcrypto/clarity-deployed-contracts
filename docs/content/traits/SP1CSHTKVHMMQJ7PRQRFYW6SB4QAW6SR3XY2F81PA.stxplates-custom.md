---
title: "Trait stxplates-custom"
draft: true
---
```
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)
(define-non-fungible-token stxplates-custom uint)
...
(define-map uris uint (string-ascii 64))
(define-map attributes uint { text: (string-ascii 25), text-color: (string-ascii 10), background-color: (string-ascii 10), border-color: (string-ascii 10) })
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stxplates-custom token-id)))

(define-read-only (get-token-uri (token-id uint)) 
  (ok (some (concat "ipfs://" (unwrap-panic (map-get? uris token-id))))))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stxplates-custom token-id tx-sender)))

(define-public (mint (uri (string-ascii 64)) (text (string-ascii 25)) (text-color (string-ascii 10)) (background-color (string-ascii 10)) (border-color (string-ascii 10)) (owned-tokens (list 10 uint)))
  (let 
    (            
    ...
    (unwrap! (nft-mint? stxplates-custom token-id tx-sender) ERR-NFT-MINT)
...
(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stxplates-custom token-id) false)))
...
(define-public (set-uri (uri (string-ascii 64)) (token-id uint))
  (begin
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stxplates-custom id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stxplates-custom id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stxplates-custom id) ERR-PLATE-NOT-FOUND))
