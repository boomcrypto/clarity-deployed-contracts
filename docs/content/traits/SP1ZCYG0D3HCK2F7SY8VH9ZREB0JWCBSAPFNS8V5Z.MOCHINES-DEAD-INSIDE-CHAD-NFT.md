---
title: "Trait MOCHINES-DEAD-INSIDE-CHAD-NFT"
draft: true
---
```
;; nft Contract
...
(define-non-fungible-token Chad uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://QmYKq5ifnRjYGpn6M1jgW5tqoBFxHryfwB7nhbvoV3JiSL")
...
(define-public (mint (recipient principal))
    (begin 
        ...
        (asserts! (< (var-get last-id) SUPPLY) (err ERR-NO-MORE-NFTS))
            ...
            (try! (nft-mint? Chad u1 tx-sender))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Chad token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Chad token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Chad token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Chad id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Chad id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Chad id) (err ERR-NOT-FOUND)))
