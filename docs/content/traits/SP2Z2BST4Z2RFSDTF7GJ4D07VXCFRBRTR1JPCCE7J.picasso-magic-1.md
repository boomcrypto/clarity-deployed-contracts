...
(define-non-fungible-token picasso-magic-1 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSbK719Y51mETJ3YRqucZ2Z6KJxKu7CLAVxBBic57pDZy/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? picasso-magic-1 next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? picasso-magic-1 token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? picasso-magic-1 token-id)))
