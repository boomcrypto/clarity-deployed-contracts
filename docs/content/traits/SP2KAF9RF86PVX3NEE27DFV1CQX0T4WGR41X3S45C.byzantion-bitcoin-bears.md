---
title: "Trait byzantion-bitcoin-bears"
draft: true
---
```
...
(define-data-var ipfs-full-metadata (string-ascii 81) "ipfs://QmQwWYkGfhoGPbjcowqua55Ajp3w3BjFHWofxE5UpPd6S4/bitcoin_bears_metadata.json")
(define-data-var ipfs-root (string-ascii 68) "ipfs://QmQwWYkGfhoGPbjcowqua55Ajp3w3BjFHWofxE5UpPd6S4/bitcoin_bears_")
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? bitcoin-bears next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? bitcoin-bears token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
