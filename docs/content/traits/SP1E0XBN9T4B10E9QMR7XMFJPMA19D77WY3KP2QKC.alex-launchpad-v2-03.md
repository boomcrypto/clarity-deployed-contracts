---
title: "Trait alex-launchpad-v2-03"
draft: true
---
```
...
(define-public (create-pool
	...
	(let (
		...
		(asserts! (map-set offerings launch-id (merge offering { max-size-factor: u15, launch-token: (contract-of launch-token-trait), payment-token: (contract-of payment-token-trait), total-tickets: u0})) err-update-failed)
		(print { type: "create-pool", launch-id: launch-id, offering: offering, launch-token: (contract-of launch-token-trait), payment-token: (contract-of payment-token-trait) })
...
(define-public (claim (launch-id uint) (input (list 200 { address: (buff 128), chain-id: (optional uint) })) (launch-token-trait <ft-trait>) (payment-token-trait <ft-trait>) (memo (optional (buff 512))))
	(let (
			...
			(tm-amount (* ONE_8 (get launch-tokens-per-ticket claim-details))))
		(fold transfer-many-iter input { launch-id: launch-id, chain-id: (get chain-id claim-details), token-trait: launch-token-trait, memo: memo, tm-amount: tm-amount } )
...
(define-private (transfer-many-iter (recipient { address: (buff 128), chain-id: (optional uint) }) (details { launch-id: uint, chain-id: (optional uint), token-trait: <ft-trait>, memo: (optional (buff 512)), tm-amount: uint }))
	(begin
		(as-contract (unwrap-panic (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.cross-router-v2-03 route 
			(get tm-amount details) 
			(list (get token-trait details)) 
			(list ) 
			(get token-trait details)
			none 
			{ address: (unwrap-panic (get-tickets-dest-or-fail (get launch-id details) recipient)), chain-id: (get chain-id details) })))
		details))
...
(define-private (claim-process (launch-id uint) (input (list 200 { address: (buff 128), chain-id: (optional uint) })) (launch-token-trait principal) (payment-token-trait <ft-trait>))
	(let (
		...
		(as-contract (try! (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.cross-router-v2-03 route 
			(* (len input) net-price-per-ticket) 
			(list payment-token-trait) 
			(list ) 
			payment-token-trait
			none 
			(get launch-owner offering))))
