---
title: "Trait morphing-horse-gifs"
draft: true
---
```
;; morphing-horse-gifs
...
(define-non-fungible-token morphing-horse-gifs uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmbm7b4CXPFULV4G8tB6gBidKXN4fgovFfxdJpZLSqxUfL/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? morphing-horse-gifs next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? morphing-horse-gifs token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? morphing-horse-gifs token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? morphing-horse-gifs token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? morphing-horse-gifs id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? morphing-horse-gifs id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? morphing-horse-gifs id) (err ERR-NOT-FOUND)))
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? morphing-horse-gifs (+ last-nft-id u0) 'SP156CPYZP5VV2C09NWYWQT4CP0T9EWJP76Y18E3T))
      (map-set token-count 'SP156CPYZP5VV2C09NWYWQT4CP0T9EWJP76Y18E3T (+ (get-balance 'SP156CPYZP5VV2C09NWYWQT4CP0T9EWJP76Y18E3T) u1))
      (try! (nft-mint? morphing-horse-gifs (+ last-nft-id u1) 'SP156CPYZP5VV2C09NWYWQT4CP0T9EWJP76Y18E3T))
      (map-set token-count 'SP156CPYZP5VV2C09NWYWQT4CP0T9EWJP76Y18E3T (+ (get-balance 'SP156CPYZP5VV2C09NWYWQT4CP0T9EWJP76Y18E3T) u1))
