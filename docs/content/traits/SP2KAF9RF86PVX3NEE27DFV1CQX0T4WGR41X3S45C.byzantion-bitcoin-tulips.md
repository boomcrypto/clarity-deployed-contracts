---
title: "Trait byzantion-bitcoin-tulips"
draft: true
---
```
...
(define-non-fungible-token bitcoin-tulips uint)
...
(define-constant MINT-LIMIT u1637)

;; Internal variables
(define-data-var last-id uint u0)
(define-data-var uri-prefix (string-ascii 256) "")
(define-data-var cost-per-mint uint u1000000) ;;initial. Will be changed to 25 stx after launch of contract
(define-data-var commission-per-mint uint u625000) 
(define-data-var payout uint u0)
(define-data-var ipfs-full-metadata (string-ascii 115) "ipfs://ipfs/bafybeihwnqwknymmw5raj5x3xmo3c34kuzkd6g4ahtgmqvw6iacuwfyzkq/bitcoin_tulips/bitcoin_tulips_metadata.json")
(define-data-var ipfs-root (string-ascii 102) "ipfs://ipfs/bafybeiguyb3cbufa7ki4vqxuyugjjmr5k2ebxmenxgmu7ostplzmy7cv5e/bitcoin_tulips/bitcoin_tulips_")
(define-data-var artist-address principal 'SP21JJ0KVPTGAA1PD8RG1QR0ZSCFX39Q86T5NNFCY)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? bitcoin-tulips next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? bitcoin-tulips token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-tulips token-id)))
