...
(define-non-fungible-token welsh-corgi uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNd2eNigHP2yHCaLJqbmJ2yM7XKasQuLS2Rjeg7nfaJSW/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? welsh-corgi next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? welsh-corgi token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? welsh-corgi token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? welsh-corgi token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? welsh-corgi id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? welsh-corgi id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? welsh-corgi id) (err ERR-NOT-FOUND)))
