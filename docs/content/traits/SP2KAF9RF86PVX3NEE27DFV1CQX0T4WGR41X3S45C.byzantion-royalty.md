...
(define-non-fungible-token royalty uint)
...
(define-data-var ipfs-full-metadata (string-ascii 75) "ipfs://QmaZzAEwvaphyorCaujfa2mYXp74DscFjHPFwMTTZ5YuVG/royalty_metadata.json")
(define-data-var ipfs-root (string-ascii 62) "ipfs://QmaZzAEwvaphyorCaujfa2mYXp74DscFjHPFwMTTZ5YuVG/royalty_")
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? royalty next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? royalty token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? royalty token-id)))
