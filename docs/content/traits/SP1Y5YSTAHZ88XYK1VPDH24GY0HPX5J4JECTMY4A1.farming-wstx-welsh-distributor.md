---
title: "Trait farming-wstx-welsh-distributor"
draft: true
---
```
...
(define-public
  ...
  (let ((epoch (contract-call? .farming-wstx-welsh-core current-epoch))
...
(define-read-only
  ...
  (let ((share   (try! (contract-call? .farming-wstx-welsh-core get-share-at user epoch)))
...
(define-read-only (mkepochs (start-epoch uint))
  (let ((current (contract-call? .farming-wstx-welsh-core current-epoch))
