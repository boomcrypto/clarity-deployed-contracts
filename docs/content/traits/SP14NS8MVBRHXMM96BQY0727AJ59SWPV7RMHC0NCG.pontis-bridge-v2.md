---
title: "Trait pontis-bridge-v2"
draft: true
---
```
(impl-trait .bridge-trait-v1.bridge-trait-v1)
...
(define-public (mint-runes-batch-from-btc (rune-contracts (list 100 <bridge-ft-trait>)) (amounts (list 100 uint)) (recipients (list 100 principal)) (tx-hashes-and-vout (list 100 (buff 36))) (is-btc bool) (burn-hash (buff 32)) (burn-height uint))
	(let
		(
		...
		;; (asserts! (or (not is-btc) (is-eq (some burn-hash) (get-burn-header burn-height))) ERR_INVALID_BURN_HASH)
...
(define-public (mint-btc-from-btc (btc-contract <bridge-ft-trait>) (amount uint) (recipient principal) (tx-hash-and-vout (buff 36)) (is-btc bool) (burn-hash (buff 32)) (burn-height uint))
	(begin
		(try! (contract-call? .pontis-bridge-controller authorize-bridge-owner))
		(try! (check-if-exists-and-mark tx-hash-and-vout))
		;; Verify that Bitcoin hasn't forked by comparing the burn hash provided
...
(define-public (mint-ordinals-batch-from-btc (ordinals-contract <bridge-nft-trait>) (ids (list 1000 uint)) (recipients (list 1000 principal)) (tx-hashes-and-vout (list 1000 (buff 36))) (is-btc bool) (burn-hash (buff 32)) (burn-height uint))
	(let
		(
...
(define-public (bridge-out-runes (rune (buff 26)) (rune-contract <bridge-ft-trait>) (amount uint) (recipient (buff 64)) (network (buff 12)))
	(let
		(
			(percent-fee (contract-call? .pontis-bridge-fee-manager calculate-runes-percent-fee amount))
		...
		(asserts! (default-to false (map-get? rune-tokens-active (contract-of rune-contract))) ERR-RUNE-NOT-ACTIVE)
		(asserts! (>= amount (unwrap! (contract-call? .pontis-bridge-fee-manager get-min-runes-bridge (contract-of rune-contract)) ERR-MIN-AMOUNT-UNWRAP)) ERR-MIN-AMOUNT)

		(try! (contract-call? .pontis-bridge-fee-manager pay-stx-fee (unwrap! (contract-call? .pontis-bridge-fee-manager get-runes-base-fee network) ERR-BASE-FEE-UNWRAP)))
		(try! (contract-call? .pontis-bridge-fee-manager pay-ft-fee percent-fee rune-contract))
...
(define-public (bridge-out-btc (btc-contract <bridge-ft-trait>) (amount uint) (recipient (buff 64)) (network (buff 12)))
	(let
		(
			(percent-fee (contract-call? .pontis-bridge-fee-manager calculate-btc-percent-fee amount))
			(key-utxo (try! (check-utxo-increment network)))
			(sender tx-sender)
		)
		...
		(asserts! (is-eq (var-get btc-token) (contract-of btc-contract)) ERR-WRONG-BTC-CONTRACT)
		(asserts! (not (var-get btc-paused)) ERR-PAUSED)
		(asserts! (>= amount (unwrap! (contract-call? .pontis-bridge-fee-manager get-min-btc-bridge) ERR-MIN-AMOUNT-UNWRAP)) ERR-MIN-AMOUNT)

		(try! (contract-call? .pontis-bridge-fee-manager pay-stx-fee (unwrap! (contract-call? .pontis-bridge-fee-manager get-btc-base-fee network) ERR-BASE-FEE-UNWRAP)))
		(try! (contract-call? .pontis-bridge-fee-manager pay-ft-fee percent-fee btc-contract))
...
(define-public (peg-out-ordinals-batch (ordinals-contract <bridge-nft-trait>) (ids (list 1000 uint)) (recipients (list 1000 (buff 64))))
	(let
		...
		(try! (contract-call? .pontis-bridge-fee-manager pay-ordinals-stx-fee (len ids)))
		(asserts! (default-to false (map-get? ordinals-contracts (contract-of ordinals-contract))) ERR-ORDINALS-CONTRACT-NOT-ACTIVE)
