---
title: "Trait PunksArmy-Friends-NFTs"
draft: true
---
```
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token PunksArmy-Friends uint)
...
(define-private (mint_punk (punk-id uint) (new-owner principal))
	(begin
		...
		(nft-mint? PunksArmy-Friends punk-id new-owner)
	)
)
...
(define-public 
	...
	(begin     
		...
		(nft-transfer? PunksArmy-Friends token-id sender recipient))
)
;; BURN A TOKEN
(define-public 
	(burn-token 
	...
	(begin     
		(asserts! (is-eq (some tx-sender) (nft-get-owner? PunksArmy-Friends token-id) ) (err u403))     
		(nft-burn? PunksArmy-Friends token-id tx-sender))
)

(
	define-read-only (get-owner (punk-id uint))  
	(ok (nft-get-owner? PunksArmy-Friends punk-id))
)
