---
title: "Trait trump-is-alive"
draft: true
---
```
(impl-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)

(define-fungible-token trump-is-alive)
...
(define-constant token-supply u420000000)
...
(define-read-only (get-name) (ok "Trump is alive"))
(define-read-only (get-symbol) (ok "FIGHT"))
(define-read-only (get-decimals) (ok u0))
(define-read-only (get-balance (user principal)) (ok (ft-get-balance trump-is-alive user)))
(define-read-only (get-total-supply) (ok (ft-get-supply trump-is-alive)))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? trump-is-alive amount from to)
    )
)
...
(define-public (swap-stx-for-token (stx-amount uint) (min-tokens-out uint)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? trump-is-alive tokens-out (as-contract tx-sender) tx-sender))
      (if (>= (/ (* (+ (var-get real-stx-reserve) (var-get virtual-stx-reserve)) token-supply) (var-get token-reserve)) market-cap-threshold)
        (begin
          (let (
            ...
            (try! (ft-transfer? trump-is-alive burn-amount (as-contract tx-sender) burn-address))
            (try! (ft-transfer? trump-is-alive remaining-tokens (as-contract tx-sender) liquidity-builder-address))
...
(define-public (swap-token-for-stx (token-amount uint) (min-stx-out uint) (recipient principal)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? trump-is-alive token-amount tx-sender (as-contract tx-sender)))
...
(begin
  (try! (ft-mint? trump-is-alive token-supply (as-contract tx-sender)))
