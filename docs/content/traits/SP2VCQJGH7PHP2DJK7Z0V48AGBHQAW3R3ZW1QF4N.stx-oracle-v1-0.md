...
(define-public (get-asset-price (token <ft>))
  (let (
    ...
    (ok (from-fixed-to-precision (get last-price oracle-data) (get decimals oracle-data)))
  )
)
...
(define-read-only (get-price)
  (let (
    ...
    ;; convert to fixed precision
    (from-fixed-to-precision (get last-price oracle-data) (get decimals oracle-data))
  )
)
