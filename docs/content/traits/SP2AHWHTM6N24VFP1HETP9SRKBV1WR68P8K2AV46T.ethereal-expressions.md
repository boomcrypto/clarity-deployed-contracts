---
title: "Trait ethereal-expressions"
draft: true
---
```
...
(define-non-fungible-token ethereal-expressions uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQuYxUznDKueHSqRcATYhziM2zmfBoDPvtpxYqMDdLNUv/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ethereal-expressions next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ethereal-expressions token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ethereal-expressions token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ethereal-expressions token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ethereal-expressions id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ethereal-expressions id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ethereal-expressions id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u27630)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u23000000)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u38821000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u35342000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u75000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u989000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u5300000000)
