...
(define-non-fungible-token stx-masks uint)
...
(define-data-var base-uri (string-ascii 80) "QmS7wcwy1RAyQJFWnhDyVph7tBc1ejqziQ2n1iaE1vmjpx/")
(define-data-var mint-price uint u2000000)
;; Percent u100 = 1%
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? stx-masks next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? stx-masks next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-masks id sender recipient)
        success
          (let
            (
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? stx-masks id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? stx-masks id) ERR_NO_OWNER))
