---
title: "Trait dmgsbtc-lp-token"
draft: true
---
```
;; Title: DMG-sbtc LP Token
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token DMG-sbtc)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP26PZG61DH667XCX51TZNBHXM4HG4M6B2HWVM47V.dmgsbtc-lp-token"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? DMG-sbtc amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "DMG-sbtc LP Token"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance DMG-sbtc who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply DMG-sbtc)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? DMG-sbtc (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? DMG-sbtc (get dk delta) sender))
...
(begin
    ...
    (try! (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token transfer u99990000 tx-sender CONTRACT none)))
```
