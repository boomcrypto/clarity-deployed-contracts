---
title: "Trait sbtc-crys"
draft: true
---
```

;; Title: sBTC-CRYS Liquidity
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token sBTC-CRYS)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? sBTC-CRYS amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "sBTC-CRYS Liquidity"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance sBTC-CRYS who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply sBTC-CRYS)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.crystals transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.crystals transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.crystals transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? sBTC-CRYS (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? sBTC-CRYS (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.crystals get-balance CONTRACT))
    })
...
(define-read-only (get-reserves-quote)
    (let (
        (reserves (get-reserves))
        (supply (ft-get-supply sBTC-CRYS)))
        {
          dx: (get a reserves),
          dy: (get b reserves),
          dk: supply
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://charisma-metadata.vercel.app/api/v1/metadata/SP3619DGWH08262BJAG0NPFHZQDPN4TKMXHC0ZQDN.sbtc-crys"))
)
```
