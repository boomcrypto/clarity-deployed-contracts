---
title: "Trait tstnfts-three-mint"
draft: true
---
```
;; Storage
...
(define-private (og-mintpass-mint (new-owner principal))
  (let ((mint-count (get-og-presale-balance new-owner)))
    ...
    (map-set og-presale-count
              new-owner
              (+ mint-count u1))
    (contract-call? .tstnfts-three stx-mint new-owner)
  ))
...
(define-private (og-mintpass-banana-mint (new-owner principal))
  (let ((mint-count (get-og-presale-balance new-owner)))
    ...
    (map-set og-presale-count
              new-owner
              (+ mint-count u1))
    (contract-call? .tstnfts-three banana-mint new-owner)
  ))
...
(as-contract (contract-call? .tstnfts-three set-mint-address))
