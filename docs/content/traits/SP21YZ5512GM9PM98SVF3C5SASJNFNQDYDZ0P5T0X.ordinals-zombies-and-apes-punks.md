---
title: "Trait ordinals-zombies-and-apes-punks"
draft: true
---
```
;; ordinals-zombies-and-apes-punks
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token ordinals-zombies-and-apes-punks uint)
...
;; Internal variables
(define-data-var mint-limit uint u72)
(define-data-var last-id uint u1)
(define-data-var total-price uint u10000000)
(define-data-var artist-address principal 'SP21YZ5512GM9PM98SVF3C5SASJNFNQDYDZ0P5T0X)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeR73Z2zDjN1SHB8sDttX37z5nLhTKsnKGnU1JF7c1ESS/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinals-zombies-and-apes-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinals-zombies-and-apes-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinals-zombies-and-apes-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinals-zombies-and-apes-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinals-zombies-and-apes-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinals-zombies-and-apes-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinals-zombies-and-apes-punks id) (err ERR-NOT-FOUND)))
