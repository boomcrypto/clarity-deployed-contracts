---
title: "Trait farming-abtc-aeusdc-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .abtc-aeusdc)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .abtc-aeusdc transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .abtc-aeusdc get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .abtc-aeusdc transfer amt protocol user none)))
