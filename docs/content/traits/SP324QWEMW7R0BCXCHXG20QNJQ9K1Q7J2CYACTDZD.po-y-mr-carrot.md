...
(define-non-fungible-token po-y-mr-carrot uint)
...
;; Internal variables
(define-data-var mint-limit uint u233)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSautqBrzCLJBs9TPdzMxER7zhk7uXStEh8LJvyjsKFP2/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? po-y-mr-carrot next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? po-y-mr-carrot token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? po-y-mr-carrot token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? po-y-mr-carrot token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? po-y-mr-carrot id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? po-y-mr-carrot id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? po-y-mr-carrot id) (err ERR-NOT-FOUND)))
