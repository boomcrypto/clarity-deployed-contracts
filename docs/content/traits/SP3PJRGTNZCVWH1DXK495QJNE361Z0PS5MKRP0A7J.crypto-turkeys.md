...
(define-non-fungible-token crypto-turkeys uint)
...
(define-data-var artist-address principal 'SP3PJRGTNZCVWH1DXK495QJNE361Z0PS5MKRP0A7J)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmb3WuSLfZMNzvmbQmSEVWdPT73rjipvgdi4B9pp2X7Wnc/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? crypto-turkeys next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? crypto-turkeys token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? crypto-turkeys token-id)))
