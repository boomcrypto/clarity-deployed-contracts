---
title: "Trait sponge-bob-mocking"
draft: true
---
```
...
;; Variables
(define-fungible-token SBOB MAXSUPPLY)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://mcdn.wallpapersafari.com/medium/3/98/ZHAOS5.jpg"))
(define-data-var token-name (string-ascii 32) "SpongeBobMocking")
(define-data-var token-symbol (string-ascii 32) "SBOB")
(define-data-var token-decimals uint u6)

;; Read-Only Functions
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance SBOB owner))
)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance SBOB from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? SBOB amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
;; Mint Tokens
(begin
  (let ((result (ft-mint? SBOB MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
