---
title: "Trait stackswap-governance-v5j"
draft: true
---
```
...
(define-public (add-governance-council (user principal))
  (begin
    (asserts! (is-eq contract-caller (contract-call? .stackswap-dao-v5j get-dao-owner)) (err ERR_NOT_AUTHORIZED))
...
(define-public (propose
  ...
  (let (
    (proposer-balance (unwrap-panic (contract-call? .vstsw-token-v1j get-balance contract-caller)))
    ...
    (asserts! (is-eq (unwrap! (contract-call? .stackswap-dao-v5j get-qualified-name-by-name "governance") (err ERR_DAO_GET)) (as-contract tx-sender)) (err ERR_NOT_AUTHORIZED))
...
(define-public (vote-for (proposal-id uint) (amount uint))
  (let (
    ...
    (unwrap! (contract-call? .vstsw-token-v1j transfer amount user (as-contract tx-sender) none) (err ERR_TRANSFER_FAIL))
    (map-set proposals
      proposal-id 
...
(define-public (return-votes-to-member (proposal-id uint) (member principal))
  (let (
    ...
    (as-contract (contract-call? .vstsw-token-v1j transfer vote-count tx-sender member none))
  )
)
...
(define-private (execute-proposal (proposal-id uint))
  (let (
    ...
    (asserts! (> (len contract-changes) u0) (err ERR_NO_CONTRACT_CHANGES))
    (unwrap! (as-contract (contract-call? .stackswap-dao-v5j execute-proposals contract-changes)) (err ERR_NO_CONTRACT_CHANGES))
    (ok true)
  )
)
...
(define-public (add-contract-address (name (string-ascii 256)) (address principal) (qualified-name principal))
  (begin
    ...
    (if (is-some (contract-call? .stackswap-dao-v5j get-contract-address-by-name name))
      (ok false)
      (begin
        (try! (as-contract (contract-call? .stackswap-dao-v5j set-contract-address name address qualified-name)))
        (ok true)
      )
    )
  )
