...
(define-non-fungible-token viosam-back-packs uint)
...
(define-data-var artist-address principal 'SP14X8VX9F3QFG9DYTNJE3KZPY4ZSAFHNR29AYS70)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTGyGr21sH4T5ENwGbogFL2w4Lg5L71Roxb6B9r7FqDJA/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? viosam-back-packs next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? viosam-back-packs token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? viosam-back-packs token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? viosam-back-packs token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? viosam-back-packs id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? viosam-back-packs id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? viosam-back-packs id) (err ERR-NOT-FOUND)))
...
(define-data-var total-price-xbtc uint u35515)
...
;; Alt Minting Mintpass
(define-data-var total-price-mia2 uint u16694000000)
...
;; Alt Minting Mintpass
(define-data-var total-price-nyc2 uint u24288000000)
...
;; Alt Minting Mintpass
(define-data-var total-price-banana uint u95000000)
...
;; Alt Minting Mintpass
(define-data-var total-price-slime uint u1843000000)
...
;; Alt Minting Mintpass
(define-data-var total-price-alex uint u47400000000)
...
(map-set mint-passes 'SP14X8VX9F3QFG9DYTNJE3KZPY4ZSAFHNR29AYS70 u3)
;; Airdrop
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? viosam-back-packs (+ last-nft-id u0) 'SP14X8VX9F3QFG9DYTNJE3KZPY4ZSAFHNR29AYS70))
      (map-set token-count 'SP14X8VX9F3QFG9DYTNJE3KZPY4ZSAFHNR29AYS70 (+ (get-balance 'SP14X8VX9F3QFG9DYTNJE3KZPY4ZSAFHNR29AYS70) u1))
