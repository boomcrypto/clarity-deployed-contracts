...
(define-public (set-token-uri (value (string-utf8 256)))
  (begin
    ...
    ;; update token uri
    (try! (as-contract (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-stxcity set-token-uri value)))
    (ok true)
  )
)
...
(define-public (transfer-ownership (new-owner principal))
  (begin
    ...
    (try! (is-dao-or-extension))
    ;; transfer ownership
    (try! (as-contract (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-stxcity transfer-ownership new-owner)))
    (ok true)
  )
)
