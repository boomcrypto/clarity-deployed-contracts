---
title: "Trait memegoat-stake-pool-v3e"
draft: true
---
```
;;
...
(define-private (move-p (stake-id uint) (acc uint))
  (let
    (
      ...
      (stake-pool (unwrap-panic (contract-call? .memegoat-stake-pool-v2 get-stake-pool stake-id)))
    )
...
(define-public (move-user-records (stake-id uint) (users (list 500 principal)))
  (begin 
    (try! (check-is-owner))
    (let
      (
        (stake-pool (try! (contract-call? .memegoat-stake-pool-v2 get-stake-pool stake-id)))
      )
...
(define-private (move (user principal) (stake-id uint))
  (let
    (
      (u-has-stake (contract-call? .memegoat-stake-pool-v2 get-user-stake-has-stake stake-id user))
      (user-stake (unwrap-panic (contract-call? .memegoat-stake-pool-v2 get-user-staking-data stake-id user)))
    )
    (if u-has-stake
      (begin
