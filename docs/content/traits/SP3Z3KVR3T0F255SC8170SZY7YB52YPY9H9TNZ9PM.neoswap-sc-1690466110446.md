---
title: "Trait neoswap-sc-1690466110446"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/32wFbP6Qh6JGqjkNS09M
;; SwapId: 4GU3v93NvEBxZudJGanc
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u12000000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP2VF3564DKVF2T39NWAT6ZMAQ5X3HHBW0GHN8PWY.stacks-animals-lovers transfer u13 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
		(unwrap! (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.stacks-mandala transfer u521 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP2VF3564DKVF2T39NWAT6ZMAQ5X3HHBW0GHN8PWY.stacks-animals-lovers transfer u13 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.stacks-mandala transfer u521 tx-sender TRADER-1)))

	(unwrap-panic (as-contract (stx-transfer? u600000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u11400000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u12000000 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2VF3564DKVF2T39NWAT6ZMAQ5X3HHBW0GHN8PWY.stacks-animals-lovers transfer u13 tx-sender TRADER-2)))
		(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.stacks-mandala transfer u521 tx-sender TRADER-2)))
	    )
            true
        )
