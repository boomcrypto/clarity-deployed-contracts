...
(define-public (refresh-wallet (holder principal) (balance uint)) 
  (begin
    ...
    (try! (claim-pending-rewards holder holder))
    (try! (contract-call? .ststxbtc-tracking-data update-holder-position-amount holder holder balance))
...
(define-public (refresh-position (holder principal) (position <position-trait>)) 
  (let (
    ...
    (supported-position (contract-call? .ststxbtc-tracking-data get-supported-positions position-address))

    (supported-position-reserve (get amount (contract-call? .ststxbtc-tracking-data get-holder-position (get reserve supported-position) (get reserve supported-position))))

    (new-position-balance (try! (contract-call? position get-holder-balance holder)))
    (prev-position-balance (get amount (contract-call? .ststxbtc-tracking-data get-holder-position holder position-address)))
    ...
    (try! (claim-pending-rewards holder position-address))
    (try! (contract-call? .ststxbtc-tracking-data update-holder-position-amount holder position-address new-position-balance))
    (try! (contract-call? .ststxbtc-tracking-data update-supported-positions-total position-address supported-position-new-total))
...
(define-public (add-rewards (amount uint))
  (let (
    (reward-added-per-token (/ (* amount u10000000000) (contract-call? .ststxbtc-tracking-data get-total-supply)))
    (new-cumm-reward (+ (contract-call? .ststxbtc-tracking-data get-cumm-reward) reward-added-per-token))
  )
    ...
    (try! (contract-call? .ststxbtc-tracking-data set-cumm-reward new-cumm-reward))
...
(define-read-only (get-pending-rewards (holder principal) (position principal))
  (let (
    (holders-info (contract-call? .ststxbtc-tracking-data get-holder-position holder position))

    (supported-position (contract-call? .ststxbtc-tracking-data get-supported-positions position))
    (cumm-reward (if 
      (and 
        ...
        (contract-call? .ststxbtc-tracking-data get-cumm-reward)
    ))
    ...
    (is-holder-position (get active (contract-call? .ststxbtc-tracking-data get-supported-positions holder)))
  )
  ...
  (let (
    ...
    (if (>= pending-rewards u1)
      (begin
        ...
        (try! (contract-call? .ststxbtc-tracking-data update-holder-position holder position))
  ...
  (let (
    ...
    (if active
      (begin
        ...
        (contract-call? .ststxbtc-tracking-data set-supported-positions position-address active reserve u0 u0)
      )
      (begin
        (try! (contract-call? .ststxbtc-tracking-data update-holder-position position-address position-address))
        (contract-call? .ststxbtc-tracking-data set-supported-positions position-address active (get reserve supported-position) (get total supported-position) (contract-call? .ststxbtc-tracking-data get-cumm-reward))
      )
    )
  )
)
