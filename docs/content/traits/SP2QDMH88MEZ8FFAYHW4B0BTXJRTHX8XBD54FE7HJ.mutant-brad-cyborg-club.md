---
title: "Trait mutant-brad-cyborg-club"
draft: true
---
```
;; mutant-brad-cyborg-club
...
(define-non-fungible-token mutant-brad-cyborg-club uint)
...
(define-data-var artist-address principal 'SP1RKXDEBZ3WSX0F749K0415CHEWBEQ82ZYDZRD85)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWaMH4f5vuu45d4KsNQTojxaHPhkFZEyZWJj28xLqFSjE/json/")
...
(define-private (mint-many (orders (list 10 bool )))  
  (let (
    ...
    (if (or (is-eq tx-sender art-addr) (is-eq tx-sender COMM_ADDR))
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? mutant-brad-cyborg-club next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? mutant-brad-cyborg-club token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? mutant-brad-cyborg-club token-id)))
