...
(define-private (charge-stx (amount uint))
  (if (>
    ...
    (match (stx-transfer? (/ (* (get mint_price ( var-get mint_event ) ) amount) u1000000) tx-sender (as-contract tx-sender) )
      success true
      error false)
    true)
)
...
(define-public (buy (amount uint) 
  ...
  (let (
      (recipient_address tx-sender)
    ) 
    (asserts! ( is_open_minting ) err-minting-closed )
    (asserts! ( is_avaible_address_minting amount ) err-minting-closed )
    (asserts! (<= (+ (get minted (var-get mint_event)) amount) (get max-token (var-get mint_event)) ) (err u407))
    (asserts! (> (stx-get-balance tx-sender) (/ (* (get mint_price ( var-get mint_event ) ) amount) u1000000) ) (err u406) )
    (asserts! (charge-stx amount) (err u402) )
    (asserts! 
      (is-ok (as-contract (contract-call? 'SP2A665S3H6FVMZSY4VJ17ESXX21CGS0A32984B1H.romatoken mint recipient_address amount )))
      (err u0))
    (var-set current-commissioning-amount (/ (* (get mint_price ( var-get mint_event ) ) amount) u1000000) )
    (map give-commission (var-get commission-contracts))
    (var-set mint_event (merge (var-get mint_event) {minted: (+ (get minted (var-get mint_event)) amount) } ))
    (ok amount)
  )
)
...
(define-read-only 
  ...
  (ok {
    ...
    can_mint_address: (is_avaible_address_minting u1),
