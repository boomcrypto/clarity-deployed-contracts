...
(define-non-fungible-token fruits-2 uint)
...
(define-data-var artist-address principal 'SPG34S51QV6YTZQGVRPZY9323MY4BTCFAFP1HR25)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcxMRJdhGnMtJs5SM72UDL6dxEtC899oGy6vnHGVwkbyM/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? fruits-2 next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? fruits-2 token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? fruits-2 token-id)))
