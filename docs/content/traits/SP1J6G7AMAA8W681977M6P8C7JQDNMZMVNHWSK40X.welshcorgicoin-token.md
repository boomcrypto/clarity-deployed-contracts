---
title: "Trait welshcorgicoin-token"
draft: true
---
```
...
(define-constant TOKEN_URI u"https://raw.githubusercontent.com/Welshcorgicoin/Welshcorgicoin/main/packages/welshcorgicoin.json") ;; utf-8 string with token metadata host
(define-constant TOKEN_NAME "Welshcorgicoin")
(define-constant TOKEN_SYMBOL "WELSH")
(define-constant TOKEN_DECIMALS u6) ;; 6 units displayed past decimal, e.g. 1._000_000 = 1 token
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance welshcorgicoin who))
)
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT_OWNER) ERR_OWNER_ONLY)
    (ft-mint? welshcorgicoin amount recipient)
  )
)
...
(begin
    ...
    ;; Transfer the tokens
    (let ((result (ft-transfer? welshcorgicoin amount sender recipient)))
      (if (is-ok result)
        (begin
...
(begin
  (try! (ft-mint? welshcorgicoin u3000000000000000 CONTRACT_OWNER)) 
  ;; (try! (ft-mint? welshcorgicoin u100000000000 'ST18VZNQWK7R44BFN75X3RANYRAT4B9ZA8S47KZ24)) 
) 
```
