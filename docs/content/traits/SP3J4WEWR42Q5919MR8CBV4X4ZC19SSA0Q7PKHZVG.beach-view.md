...
(define-non-fungible-token beach-view uint)
...
(define-data-var artist-address principal 'SP3J4WEWR42Q5919MR8CBV4X4ZC19SSA0Q7PKHZVG)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcmUDtWiFPE84eTKZfAFVGpp7kpJpevAVepidJPc19m4a/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? beach-view next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? beach-view token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? beach-view token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? beach-view token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? beach-view id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? beach-view id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? beach-view id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-usda uint u31000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u384000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u5697000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u163700000000)
