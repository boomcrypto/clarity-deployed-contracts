---
title: "Trait test-token-7-stxcity-dex"
draft: true
---
```
...
(define-constant token-supply u98888812318000000) ;; match with the token's supply (6 decimals)
(define-constant BONDING-DEX-ADDRESS (as-contract tx-sender)) ;; one contract per token

;; bonding curve config
(define-constant STX_TARGET_AMOUNT u50000000000)
(define-constant VIRTUAL_STX_VALUE u10000000000) ;; 1/5 of STX_TARGET_AMOUNT
(define-constant COMPLETE_FEE u1000000000) ;; 2% of STX_TARGET_AMOUNT
...
(define-constant allow-token 'SP1FQ3DQDR5N9HJX3XC5DNKFCG4DHH48EFJQV6QH0.test-token-7-stxcity)
...
(begin
  (var-set virtual-stx-amount VIRTUAL_STX_VALUE)
    ...
    (var-set token-balance u98882220169988672)
