---
title: "Trait meta-stacks-vehicles"
draft: true
---
```
;; meta-stacks-vehicles

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token meta-stacks-vehicles uint)
...
(define-data-var artist-address principal 'SP65CAZ20QFDRTP5X7A9KGJ5MFCE2QBZ24XS0G2S)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPgbzFydwJwYdUJAKATW2UQ2Ht6kKK4F11cbS3XFuiYLt/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? meta-stacks-vehicles next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? meta-stacks-vehicles token-id sender recipient)))

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? meta-stacks-vehicles token-id)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u1700)
