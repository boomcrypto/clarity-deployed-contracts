---
title: "Trait stacking-dao-fomo-eoy-nft"
draft: true
---
```
;; @contract Stacking DAO FOMO EOY NFT
;; @version 1
;;
;; Stacking DAO FOMO NFT for people who participate in the FOMO EOY treasure hunt

(impl-trait .nft-trait.nft-trait)
(use-trait commission-trait .commission-trait.commission)

(define-non-fungible-token stacking-dao-fomo-eoy uint)
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacking-dao-fomo-eoy token-id))
)
...
(define-public (mint-for-protocol (recipient principal))
  (let (
    ...
    (try! (nft-mint? stacking-dao-fomo-eoy (var-get last-id) recipient))
...
(define-public (burn-for-protocol (token-id uint))
  (let (
    ...
    (try! (nft-burn? stacking-dao-fomo-eoy token-id owner))
...
(define-private (is-sender-owner (id uint))
  (let (
    (owner (unwrap! (nft-get-owner? stacking-dao-fomo-eoy id) false))
  )
...
(define-public (buy-in-ustx (id uint) (commission-contract <commission-trait>))
  (let (
    (owner (unwrap! (nft-get-owner? stacking-dao-fomo-eoy id) (err ERR_NFT_NOT_FOUND)))
...
(define-private (transfer-helper (id uint) (sender principal) (recipient principal))
  (begin
    (try! (nft-transfer? stacking-dao-fomo-eoy id sender recipient))
