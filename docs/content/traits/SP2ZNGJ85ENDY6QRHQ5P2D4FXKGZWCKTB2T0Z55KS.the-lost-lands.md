...
;; Define LP token
(define-fungible-token LOST)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.the-lost-lands"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? LOST amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance LOST who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply LOST)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? LOST (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? LOST (get dk delta) sender))
...
(begin
    ...
    (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.dexterity-pool-v1 transfer u8000000 tx-sender CONTRACT none)))
```
