...
(define-non-fungible-token glitched-mrks uint)
...
(define-data-var artist-address principal 'SP21H62TFEJATYN9QJZEJEXRXMRBKBCDRH038GJ7N)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbS9d6BjSsLBFWPQ1zcBDyutfNLWNovJeYhX4EqAtnFBe/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? glitched-mrks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? glitched-mrks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? glitched-mrks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? glitched-mrks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? glitched-mrks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? glitched-mrks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? glitched-mrks id) (err ERR-NOT-FOUND)))
