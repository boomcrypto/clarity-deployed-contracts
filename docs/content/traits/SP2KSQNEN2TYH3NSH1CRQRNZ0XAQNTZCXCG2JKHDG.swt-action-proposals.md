---
title: "Trait swt-action-proposals"
draft: true
---
```
...
(define-constant VOTING_TOKEN_DEX 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-faktory-dex)
(define-constant VOTING_TOKEN_POOL 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.xyk-pool-stx-swt-v-1-1)
(define-constant VOTING_TREASURY 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-treasury)
...
(define-public (propose-action (action <action-trait>) (parameters (buff 2048)))
  (let
    (
      ...
      (voterBalance (unwrap! (contract-call? 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-faktory get-balance tx-sender) ERR_FETCHING_TOKEN_DATA))
...
(define-public (vote-on-proposal (proposalId uint) (vote bool))
  (let
    (
      ...
      (senderBalance (unwrap! (at-block proposalBlockHash (contract-call? 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-faktory get-balance tx-sender)) ERR_FETCHING_TOKEN_DATA))
    )
    ;; caller has the required balance
...
(define-read-only (get-voting-power (who principal) (proposalId uint))
  (let
    ...
    (at-block proposalBlockHash (contract-call? 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-faktory get-balance who))
  )
)
...
(define-private (is-dao-or-extension)
  (ok (asserts! (or (is-eq tx-sender 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-base-dao)
    (contract-call? 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-base-dao is-extension contract-caller)) ERR_NOT_DAO_OR_EXTENSION
  ))
)
...
(define-private (get-liquid-supply (blockHeight uint))
  (let
    (
      ...
      (totalSupply (unwrap! (at-block blockHash (contract-call? 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-faktory get-total-supply)) ERR_FETCHING_TOKEN_DATA))
      (dexBalance (unwrap! (at-block blockHash (contract-call? 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-faktory get-balance VOTING_TOKEN_DEX)) ERR_FETCHING_TOKEN_DATA))
      (poolBalance (unwrap! (at-block blockHash (contract-call? 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-faktory get-balance VOTING_TOKEN_POOL)) ERR_FETCHING_TOKEN_DATA))
      (treasuryBalance (unwrap! (at-block blockHash (contract-call? 'SP2KSQNEN2TYH3NSH1CRQRNZ0XAQNTZCXCG2JKHDG.swt-faktory get-balance VOTING_TREASURY)) ERR_FETCHING_TOKEN_DATA))
    )
