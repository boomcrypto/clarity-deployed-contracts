---
title: "Trait memegoat-staking-pool-v1-1"
draft: true
---
```
;;
...
(define-public (move-stakes-from-v1 
  ...
  (end-block uint)
  (reward-per-block uint)
  (addresses (list 200 {user: principal, stake: uint, stake-token: <ft-trait>})
))
  (begin 
    (try! (check-is-owner))
    (let 
      (
        (stake-id  (get-next-stake-id))
      ) 
        ...
        (fold move-stake addresses stake-id)  
    )
    (ok true)
  )
)
...
(define-private (do-withdraw-stake (stake-id uint) (user-addr principal) (amount uint) (stake-token <ft-trait>))
  (begin 
    (let
      ...
      (as-contract (try! (contract-call? .memegoat-stakepool-vault-v1 transfer-ft stake-token amount user-addr)))   
