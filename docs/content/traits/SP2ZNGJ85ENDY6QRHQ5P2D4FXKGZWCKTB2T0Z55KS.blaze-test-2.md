...
;; Constants
(define-constant SIP10_TOKEN 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token)
...
(define-public (transfer
  ...
  (let 
    ...
    ;; Verify signature
    (asserts! (verify-signature signature from to amount nonce) ERR_INVALID_SIGNATURE)
...
(define-read-only (make-structured-data-hash
  ...
  (let (
      (structured-data { token: SIP10_TOKEN, to: to, amount: amount, nonce: nonce })
