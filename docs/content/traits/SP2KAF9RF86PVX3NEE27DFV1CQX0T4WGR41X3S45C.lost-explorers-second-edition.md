---
title: "Trait lost-explorers-second-edition"
draft: true
---
```
...
(define-non-fungible-token lost-explorers-second-edition uint)
...
(define-constant MINT-LIMIT u16)
...
(define-data-var target-block uint u41070)
(define-data-var ipfs-full-metadata (string-ascii 97) "ipfs://QmVHcqj3p9mE5HJN7XcDRwZzMCpJnpKGsx7H3d9F7bz8zp/lost_explorers_second_edition_metadata.json")
(define-data-var ipfs-root (string-ascii 54) "ipfs://QmVHcqj3p9mE5HJN7XcDRwZzMCpJnpKGsx7H3d9F7bz8zp/")
(define-data-var artist-address principal 'SP2Y5FQMEG8XKBZJREJZQ3DSM1B5DC3YRDBYTMNVY)
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? lost-explorers-second-edition next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? lost-explorers-second-edition token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? lost-explorers-second-edition token-id)))
