---
title: "Trait Cerulean-Marketplace-TEST-NOTH-fixed"
draft: true
---
```
;; title: marketplace
...
;; summary:
;; description:
;; this is a test contract interact at your own risk
;; deployed by hodlstx -see zeroauthority profile
...
(define-constant nope-token-address 'SP32AEEF6WW5Y0NMJ1S8SBSZDAY8R5J32NBZFPKKZ.nope)
...
;; create gig
(define-public (create-gig (artist principal) (amount uint) (pay-on <token>) (job-title (string-ascii 20)) (period uint))
  (let ((gig-id (increment-gig-count))
        (fee (/ (* amount commission) u1000))
        (gig-amount (- amount fee)))
    (asserts! (is-eq (contract-of pay-on) nope-token-address) ERR_WRONG_TOKEN)
...
;; check if pending
;; send back the stx
(define-public (decline-gig (gig-id uint) (token-name <token>)) 
  (begin
    (asserts! (is-eq (contract-of token-name) nope-token-address) ERR_WRONG_TOKEN) 
    (asserts! (is-eq tx-sender (get to (unwrap! (map-get? gig gig-id) ERR_NOT_FOUND))) ERR_NOT_ARTIST)
    (let ((gig-info (unwrap-panic (map-get? gig gig-id)))) 
      (asserts! (is-eq (get status gig-info) "pending") ERR_NOT_PENDING)
      (asserts! (<= block-height (+ (get block-created gig-info) (* u144 days))) ERR_EXPIRED)
      (try! (as-contract (contract-call? token-name transfer (get amount gig-info)  tx-sender (get from gig-info) none)))
...
(define-public (redeem-back (gig-id uint) (token-name <token>)) 
  (begin 
    (asserts! (is-eq (contract-of token-name) nope-token-address) ERR_WRONG_TOKEN)
...
(define-public (satisfaction-vote-gig-as-client (gig-id uint) (satisfaction-vote (string-ascii 20)) (token-name <token>))
  (begin 
    (asserts! (is-eq (contract-of token-name) nope-token-address) ERR_WRONG_TOKEN) 
    ...
    (if (is-eq satisfaction-vote vote-1)
      (begin
        (as-contract (try! (contract-call? token-name transfer (get amount gig-info)  tx-sender (get to gig-info) none)))
...
(define-public (satisfaction-acceptance-as-artist (gig-id uint) (acceptance bool) (token-name <token>)) 
  (let ((gig-info (unwrap-panic (map-get? gig gig-id))))
    (asserts! (is-eq (contract-of token-name) nope-token-address) ERR_WRONG_TOKEN) 
    ...
    (if acceptance 
      (if (is-eq (get satisfaction gig-info) vote-2) ;; pay 75% and 25%
        (begin
          (as-contract (try! (contract-call? token-name transfer (/ (* (get amount gig-info) u75) u100)  tx-sender (get to gig-info) none)))
          (as-contract (try! (contract-call? token-name transfer (/ (* (get amount gig-info) u25) u100)  tx-sender (get from gig-info) none))) 
          (ok (map-set gig gig-id (merge gig-info {status: "completed", completely-paid: true}))))
        ;; else it is 50% 50% - vote-3
        (begin
          (as-contract (try! (contract-call? token-name transfer (/ (* (get amount gig-info) u50) u100) tx-sender (get to gig-info) none)))
          (as-contract (try! (contract-call? token-name transfer (/ (* (get amount gig-info) u50) u100) tx-sender (get from gig-info) none)))
          (ok (map-set gig gig-id (merge gig-info {status: "completed", completely-paid: true})))))
...
(define-public (dao-vote-satisfaction (gig-id uint) (satisfaction-vote (string-ascii 20)) (token-name <token>))
  (begin 
    (asserts! (is-eq (contract-of token-name) nope-token-address) ERR_WRONG_TOKEN) 
        ...
        (if (is-eq satisfaction-vote vote-2) 
          ...
          (if (is-eq satisfaction-vote vote-3) 
            (begin
              ...
              (as-contract (try! (contract-call? token-name transfer (/ (* (get amount gig-info) u50) u100) tx-sender (get from gig-info) none))))
            ;; satisfaction-vote-4    
            (as-contract (try! (contract-call? token-name transfer (get amount gig-info) tx-sender (get from gig-info) none))))))        
