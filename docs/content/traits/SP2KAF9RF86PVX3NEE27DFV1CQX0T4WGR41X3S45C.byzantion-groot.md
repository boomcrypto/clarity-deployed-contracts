...
(define-non-fungible-token groot uint)
...
(define-data-var cost-per-mint uint u85000000)
(define-data-var commission uint u1500)
(define-data-var payout uint u0)
(define-data-var ipfs-full-metadata (string-ascii 92) "ipfs://bafybeigtw2z37gibnmjfzk7lujn7kvttujer4tvrydptslxcsxnfbdi44i/groot/groot_metadata.json")
(define-data-var ipfs-root (string-ascii 79) "ipfs://bafybeigtw2z37gibnmjfzk7lujn7kvttujer4tvrydptslxcsxnfbdi44i/groot/groot_")
(define-data-var artist-address principal 'SP1SS16QBKNXYNPH49RB8HJTJK8J2Q3X3AE55BKFB)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? groot next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? groot token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? groot token-id)))
