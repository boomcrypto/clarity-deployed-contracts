...
(define-non-fungible-token bricks-stackers uint)
...
(define-data-var artist-address principal 'SP2HXK6WZXRFNWK9C3DE36HBM7K5J9QFWMBG70G31)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmT7zahomyQTzexb3EWDZBFjcaUif3EyfR9smSmtgXDgjB/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bricks-stackers next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bricks-stackers token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bricks-stackers token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bricks-stackers token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bricks-stackers id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bricks-stackers id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bricks-stackers id) (err ERR-NOT-FOUND)))
