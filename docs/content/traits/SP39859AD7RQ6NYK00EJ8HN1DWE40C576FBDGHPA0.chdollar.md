...
;; Define LP token
(define-fungible-token CHD)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.chdollar"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? CHD amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance CHD who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply CHD)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? CHD (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? CHD (get dk delta) sender))
...
(begin
    ...
    (try! (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token transfer u900000 tx-sender CONTRACT none)))
```
