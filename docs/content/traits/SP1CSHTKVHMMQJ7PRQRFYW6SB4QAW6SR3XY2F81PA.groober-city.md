...
(define-non-fungible-token groober-city uint)
...
(define-constant TOKEN-LIMIT u400)
(define-constant URI-CID "QmbGWLCXAT7fGDoUa7ad43RLUmtbtcGmCcnYKgCbuBnndp/")
...
;; mainnet
(define-data-var artist-address principal 'SPJAPA3VKBC4CJBGG2B3N6GSW5AWPVFV3R5FRBM1)
...
(define-read-only (get-owner (token-id uint)) 
    (ok (nft-get-owner? groober-city token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
    (begin 
        ...
        (nft-transfer? groober-city token-id sender recipient)
    )
)
...
(define-public (burn (token-id uint))
    (begin 
        ...
        (nft-burn? groober-city token-id tx-sender)
    )
)
...
(define-private (mint (recipient principal))
    (let 
        ...
        (unwrap! (stx-transfer? u28800000 tx-sender (var-get artist-address)) (err ERR-STX-TRANSFER))
        (unwrap! (stx-transfer? u1200000 tx-sender (var-get helper-address)) (err ERR-STX-TRANSFER))
        (unwrap! (nft-mint? groober-city token-id recipient) (err ERR-NFT-MINT))
...
(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? groober-city token-id) false))
)
