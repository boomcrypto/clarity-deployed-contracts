...
(define-constant asset-addr 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token)
(define-constant decimals u8)

(define-read-only (get-total-supply)
  (contract-call? .zsbtc-token get-total-supply))

(define-read-only (get-name)
  (contract-call? .zsbtc-token get-name))

(define-read-only (get-symbol)
  (contract-call? .zsbtc-token get-symbol))

(define-read-only (get-decimals)
  (contract-call? .zsbtc-token get-decimals))

(define-read-only (get-token-uri)
  (contract-call? .zsbtc-token get-token-uri))
...
(define-private (transfer-internal (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (contract-call? .zsbtc-token transfer amount sender recipient memo)
)

(define-private (mint-internal (amount uint) (owner principal))
  (contract-call? .zsbtc-token mint amount owner)
)

(define-private (burn-internal (amount uint) (owner principal))
  (contract-call? .zsbtc-token burn amount owner)
)
...
(define-read-only (get-principal-balance (account principal))
  (ok
    (contract-call? .zsbtc-token get-balance account)
  )
)
...
(define-public (set-contract-owner (owner principal))
  (begin
    ...
    (print { type: "set-contract-owner-zsbtc", payload: owner })
