---
title: "Trait brc20-bridge-endpoint-v1-09"
draft: true
---
```
...
(define-read-only (get-token-to-tick-or-fail (token principal))
	(contract-call? .brc20-bridge-registry-v1-03 get-token-to-tick-or-fail token))
(define-read-only (get-token-details-or-fail (tick (string-utf8 4)))
	(contract-call? .brc20-bridge-registry-v1-03 get-token-details-or-fail tick))
(define-read-only (get-token-details-or-fail-by-address (token principal))
	(contract-call? .brc20-bridge-registry-v1-03 get-token-details-or-fail-by-address token))
(define-read-only (is-approved-token (tick (string-utf8 4)))
	(contract-call? .brc20-bridge-registry-v1-03 is-approved-token tick))
(define-read-only (get-request-or-fail (request-id uint))
	(contract-call? .brc20-bridge-registry-v1-03 get-request-or-fail request-id))
...
(define-public (finalize-peg-in (tx (buff 8192)) (output-idx uint) (offset-idx uint) (order-idx uint) (token-trait <sip010-trait>))
	(let (
		...
		(as-contract (try! (contract-call? .brc20-bridge-registry-v1-03 set-peg-in-sent { tx: tx, output: output-idx, offset: offset-idx } true)))
...
(define-public (request-peg-out (tick (string-utf8 4)) (amount uint) (peg-out-address (buff 128)) (token-trait <sip010-trait>))
	(let (
			...
			(request-id (as-contract (try! (contract-call? .brc20-bridge-registry-v1-03 set-request u0 request-details)))))
...
(define-public (claim-peg-out (request-id uint) (fulfilled-by (buff 128)))
	(let (
		...
		(as-contract (try! (contract-call? .brc20-bridge-registry-v1-03 set-request request-id (merge request-details { claimed: (+ block-height (get-request-claim-grace-period)), claimed-by: claimer, fulfilled-by: fulfilled-by }))))
...
(define-public (finalize-peg-out (request-id uint) (tx (buff 8192)) (output-idx uint) (offset-idx uint) (token-trait <sip010-trait>))
	(let (
		...
		(asserts! (not (get finalized request-details)) err-request-already-finalized)
		(as-contract (try! (contract-call? .brc20-bridge-registry-v1-03 set-peg-in-sent { tx: tx, output: output-idx, offset: offset-idx } true)))
		(as-contract (try! (contract-call? .brc20-bridge-registry-v1-03 set-request request-id (merge request-details { finalized: true }))))
...
(define-public (revoke-peg-out (request-id uint) (token-trait <sip010-trait>))
	(let (
		...
		(asserts! (not (get revoked request-details)) err-request-already-revoked)
		(asserts! (not (get finalized request-details)) err-request-already-finalized)
		(as-contract (try! (contract-call? .brc20-bridge-registry-v1-03 set-request request-id (merge request-details { revoked: true }))))
