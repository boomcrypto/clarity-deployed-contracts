---
title: "Trait rapha-mac-bear-with-extra-anti-war"
draft: true
---
```
...
(define-non-fungible-token rapha-mac-bear-with-extra-anti-war uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmPwD6FftdHiSiKYYxZYdj4BVeLZ7cpQQLJiiuWbn8hKv1/rapha-mac-bear-with-extra-anti-war/")
...
(define-data-var artist-address (list 1000 principal) (list 'SPKYZ54NDRA04F3HJA2HMCCYX9J63M6FFD3TKZ9M )) 
...
(define-data-var public-target-block-height uint u66001)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u10000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rapha-mac-bear-with-extra-anti-war id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace rapha-mac-bear-with-extra-anti-war
  (ok (nft-get-owner? rapha-mac-bear-with-extra-anti-war id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? rapha-mac-bear-with-extra-anti-war next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? rapha-mac-bear-with-extra-anti-war id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rapha-mac-bear-with-extra-anti-war id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rapha-mac-bear-with-extra-anti-war id) ERR-NOT-FOUND))
