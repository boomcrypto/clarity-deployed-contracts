...
(define-non-fungible-token pixel-muneeb-nft uint)
...
(define-data-var artist-address principal 'SP2VQF7ECJJHGWF6T9XVCR4A8NVTVGJYV20NDKG8D)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qma1CMrTDDi2NjHyCMnW4sD9WozSrPCAtvZ4FpaHub7va6/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? pixel-muneeb-nft next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? pixel-muneeb-nft token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? pixel-muneeb-nft token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? pixel-muneeb-nft token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pixel-muneeb-nft id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pixel-muneeb-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pixel-muneeb-nft id) (err ERR-NOT-FOUND)))
