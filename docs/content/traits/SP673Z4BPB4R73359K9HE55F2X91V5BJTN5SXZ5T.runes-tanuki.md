...
(define-fungible-token runes-tanuki)

(define-data-var token-name (string-ascii 32) "TANUKI WISDOM")
(define-data-var token-symbol (string-ascii 10) "tanuki")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/runes-tanuki.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(try! (ft-transfer? runes-tanuki amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance runes-tanuki who)))

(define-read-only (get-total-supply)
	(ok (ft-get-supply runes-tanuki)))
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? runes-tanuki amount recipient)))
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? runes-tanuki amount sender)))
