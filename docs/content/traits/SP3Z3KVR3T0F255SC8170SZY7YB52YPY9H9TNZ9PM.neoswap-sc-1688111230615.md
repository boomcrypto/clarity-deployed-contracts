---
title: "Trait neoswap-sc-1688111230615"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/y2Ce00irzRZOKjHJtkmR
;; SwapId: pTfptnL1ztpogKo6MnQd
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u5613207 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u5613207 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u5613207 tx-sender TRADER-1)))
	    )
            true
        )
