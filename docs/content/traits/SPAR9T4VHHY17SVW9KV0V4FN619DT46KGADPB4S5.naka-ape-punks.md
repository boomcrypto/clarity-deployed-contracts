...
(define-non-fungible-token naka-ape-punks uint)
...
(define-data-var artist-address principal 'SPAR9T4VHHY17SVW9KV0V4FN619DT46KGADPB4S5)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbQXzPCgS1TaBjGgihMg671dj5diniBtiugJY4N1HxShY/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? naka-ape-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? naka-ape-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? naka-ape-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? naka-ape-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? naka-ape-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? naka-ape-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? naka-ape-punks id) (err ERR-NOT-FOUND)))
