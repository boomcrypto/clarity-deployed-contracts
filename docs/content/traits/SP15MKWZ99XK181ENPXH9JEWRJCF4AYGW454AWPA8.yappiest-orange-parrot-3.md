---
title: "Trait yappiest-orange-parrot-3"
draft: true
---
```
...
(define-read-only (get-sbtc-rewards
    (who principal)
    (sbtc-balance uint)
    )
    (let (
        ;; gets with interest
        (reward-balance (unwrap-panic (convert-to sbtc wstx sbtc-balance)))
...
(define-read-only (get-apy
    ...
    (let ((reward-program-income-state (unwrap! (get-reward-program-income supplied-asset reward-asset) err-not-found)))
    (ok { liquidity-rate: (get liquidity-rate reward-program-income-state) })
    )
)
