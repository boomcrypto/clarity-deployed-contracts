...
(define-non-fungible-token kelsons-nightmare uint)
...
(define-data-var admin (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF 'SP1B6VYHGCR4E8NG8VDF38S7H4BC7E0D0T8CQR1BZ ))
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? kelsons-nightmare item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? kelsons-nightmare item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? kelsons-nightmare item-id)))
      (begin
        (try! (nft-burn? kelsons-nightmare item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? kelsons-nightmare token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? kelsons-nightmare id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? kelsons-nightmare id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? kelsons-nightmare id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP1B6VYHGCR4E8NG8VDF38S7H4BC7E0D0T8CQR1BZ u1000 "Kelsons nightmare" u600000 u64010 u64154 "ipfs://QmSqYDadyqosjtLZzx7evWVcQc2CD9Qm2Cir1PxTSipocn/kelsons-nightmare/1.json"))
  (ok true)
)
