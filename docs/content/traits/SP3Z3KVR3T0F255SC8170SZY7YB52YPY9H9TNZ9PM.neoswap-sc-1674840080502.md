---
title: "Trait neoswap-sc-1674840080502"
draft: true
---
```
...
(define-constant TRADER-4 'SPK9KP81Q281Q84SPGAMK8J12X2AQQJGT0XFPPCX)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-3)
		...
		(unwrap! (contract-call? 'SPWGSV28BN5QMQ618CZAPDD18XPSN6EK776CDS84.magic-sound transfer u116 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SPJSCH3DDEJ8GQPGYZBHSB4F3HX5Q222CG89PSAB.guest-wins transfer u87 tx-sender TRADER-3)))
	(unwrap-panic (as-contract (contract-call? 'SP33SE5XKAY043VN4ZNYCZB7G7AQ2QT1K9K3J70Z2.anime-contest-2021 transfer u31 tx-sender TRADER-3)))
	(unwrap-panic (as-contract (contract-call? 'SPWGSV28BN5QMQ618CZAPDD18XPSN6EK776CDS84.magic-sound transfer u116 tx-sender TRADER-4)))

	(unwrap-panic (as-contract (stx-transfer? u7714000 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (stx-transfer? u11000000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP33SE5XKAY043VN4ZNYCZB7G7AQ2QT1K9K3J70Z2.anime-contest-2021 transfer u31 tx-sender TRADER-2)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
		...
		(unwrap-panic (as-contract (contract-call? 'SPWGSV28BN5QMQ618CZAPDD18XPSN6EK776CDS84.magic-sound transfer u116 tx-sender TRADER-3)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
