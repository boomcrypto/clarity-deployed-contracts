...
(define-non-fungible-token find-your-path uint)
...
(define-data-var artist-address principal 'SP208XD20NAHGXA2KE0442F9F03GY23MHEBC60C2K)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPgb2VyzJAjtKkv8GNC14FUxY7ECJXzLPnrD5kHWYCjFs/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? find-your-path next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? find-your-path token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? find-your-path token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? find-your-path token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? find-your-path id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? find-your-path id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? find-your-path id) (err ERR-NOT-FOUND)))
