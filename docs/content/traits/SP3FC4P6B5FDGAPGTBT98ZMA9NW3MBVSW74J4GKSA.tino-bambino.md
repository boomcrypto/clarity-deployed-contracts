...
(define-non-fungible-token tino-bambino uint)
...
(define-data-var artist-address principal 'SP3FC4P6B5FDGAPGTBT98ZMA9NW3MBVSW74J4GKSA)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRSZztyFex2iJVXHfZgysPmWvfPDFeDfqYwhdCk9yjBGp/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? tino-bambino next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? tino-bambino token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? tino-bambino token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tino-bambino token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? tino-bambino id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? tino-bambino id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? tino-bambino id) (err ERR-NOT-FOUND)))
