---
title: "Trait sdags-doken"
draft: true
---
```
(impl-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)

(define-fungible-token sdags-doken)
...
(define-constant token-supply u1818000000)
...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://bafybeifd2tdwdeutz5xvzb4gapt3ihf5hkvp42434sb6kbodon6brpskoy.ipfs.dweb.link/")) 
...
(define-read-only (get-name) (ok "Sdags Doken"))
(define-read-only (get-symbol) (ok "SDAGS"))
(define-read-only (get-decimals) (ok u0))
(define-read-only (get-balance (user principal)) (ok (ft-get-balance sdags-doken user)))
(define-read-only (get-total-supply) (ok (ft-get-supply sdags-doken)))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? sdags-doken amount from to)
    )
)
...
(define-public (swap-stx-for-token (stx-amount uint) (min-tokens-out uint)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? sdags-doken tokens-out (as-contract tx-sender) tx-sender))
      (if (>= (/ (* (+ (var-get real-stx-reserve) (var-get virtual-stx-reserve)) token-supply) (var-get token-reserve)) market-cap-threshold)
        (begin
          (let (
            ...
            (try! (ft-transfer? sdags-doken burn-amount (as-contract tx-sender) burn-address))
            (try! (ft-transfer? sdags-doken remaining-tokens (as-contract tx-sender) liquidity-builder-address))
...
(define-public (swap-token-for-stx (token-amount uint) (min-stx-out uint) (recipient principal)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? sdags-doken token-amount tx-sender (as-contract tx-sender)))
...
(begin
  (try! (ft-mint? sdags-doken token-supply (as-contract tx-sender)))
