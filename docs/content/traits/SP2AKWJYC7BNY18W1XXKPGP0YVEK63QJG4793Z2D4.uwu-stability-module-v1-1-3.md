---
title: "Trait uwu-stability-module-v1-1-3"
draft: true
---
```
;; UWU STABILITY MODULE CONTRACT Version 1.1.3
...
(define-map tokens principal {approved: bool, factor: uint, x-fee: uint, y-fee: uint, price: uint, price-height: uint, oracle: principal})
...
(define-public (add-token (token-trait <ft-trait>) (approved bool) (factor uint) (x-fee uint) (y-fee uint) (price uint) (oracle principal))
  (let (
    (token-contract (contract-of token-trait))
    ...
    (begin
      ...
      (map-set tokens token-contract {approved: approved, factor: factor, x-fee: x-fee, y-fee: y-fee, price: price, price-height: block-height, oracle: oracle})
      (print {action: "add-token", sender: sender, token: token-contract, approved: approved, factor: factor, x-fee: x-fee, y-fee: y-fee, price: price, price-height: block-height, oracle: oracle})
      (ok true)
    )
  )
)
...
(define-public (set-token-price (token-trait <ft-trait>) (price uint))
  (let (
    (token-contract (contract-of token-trait))
    (token-data (unwrap! (map-get? tokens token-contract) ERR_INVALID_TOKEN))
    (sender tx-sender)
    ...
    (begin
      ...
      (asserts! (> price u0) ERR_INVALID_AMOUNT)
      (map-set tokens token-contract (merge {price: price, price-height: block-height} token-data))
      (print {action: "set-token-price", sender: sender, token: token-contract, price: price, price-height: block-height})
      (ok true)
    )
  )
)
...
(define-public (swap-x-for-y (token-trait <ft-trait>) (amount uint) (min-dy uint))
  (let (
    ...
    (begin
      ...
      (print {action: "swap-x-for-y", sender: sender, token: token-contract, amount: amount, dy: dy, min-dy: min-dy, fee: fee, fee-rate: fee-rate, price: price, factor: factor})
      (ok dy)
    )
  )
)
...
(define-public (swap-y-for-x (token-trait <ft-trait>) (amount uint) (min-dx uint))
  (let (
    ...
    (begin
      ...
      (print {action: "swap-y-for-x", sender: sender, token: token-contract, amount: amount, dx: dx, min-dx: min-dx, fee: fee, fee-rate: fee-rate, price: price, factor: factor})
      (ok dx)
    )
  )
)
