...
(define-non-fungible-token equipment-series-2 uint)
...
(define-constant ERR-UNWRAP (err u110))
...
;; Equipment NFT Index
(define-data-var equipment-series-2-index uint u1)
...
(define-read-only (get-last-token-id)
  (ok (var-get equipment-series-2-index))
)

(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? equipment-series-2 id))
)
...
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? equipment-series-2 id sender recipient)
  )
)
...
(define-private (is-sender-owner (id uint))
  (let
    (
      (owner (unwrap! (nft-get-owner? equipment-series-2 id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let
    (
      (owner (unwrap! (nft-get-owner? equipment-series-2 id) ERR-NOT-AUTH))
    ...
    (try! (nft-transfer? equipment-series-2 id owner tx-sender))
    (map-delete market id)
...
(define-public (mint-equipment (lost-shipment-id uint))
  (let
    (
      (current-id (var-get equipment-series-2-index))
    ...
    ;; (asserts! (is-eq (some tx-sender) (unwrap! (contract-call? .project-indigo-lost-shipments-series-2 get-owner lost-shipment-id) ERR-UNWRAP)) ERR-GET-OWNER-FAILED)
    (asserts! (is-eq (some tx-sender) (unwrap! (contract-call? .project-i-l-s-s-2-1 get-owner lost-shipment-id) ERR-UNWRAP)) ERR-GET-OWNER-FAILED)

    ;; burn lost-shipments NFTs
    ;; (try! (contract-call? .project-indigo-lost-shipments-series-2 burn-lost-shipment lost-shipment-id))
    (try! (contract-call? .project-i-l-s-s-2-1 burn-lost-shipment lost-shipment-id))

    ;; mint lost-shipment to tx-sender
    (try! (nft-mint? equipment-series-2 current-id tx-sender))

    ;; update lost-shipments-index
    (ok (var-set equipment-series-2-index next-id))
