...
;; Variables
(define-fungible-token SHAKEBAIT MAXSUPPLY)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://gaia.hiro.so/hub/1KM1pSBPdfDL1XXpKZiv6trJTtzc6AscNY/45470546-4bd1-4bb9-9649-89d0663f7312-metadata.json"))
(define-data-var token-name (string-ascii 32) "Shakebait")
(define-data-var token-symbol (string-ascii 32) "SHAKEBAIT")
(define-data-var token-decimals uint u6)

;; Read-Only Functions
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance SHAKEBAIT owner))
)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance SHAKEBAIT from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? SHAKEBAIT amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
(begin
  ...
  (let ((result (ft-mint? SHAKEBAIT MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
