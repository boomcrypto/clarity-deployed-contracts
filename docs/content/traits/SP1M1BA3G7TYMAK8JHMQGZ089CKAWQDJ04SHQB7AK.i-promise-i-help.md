...
(define-non-fungible-token i-promise-i-help uint)
...
(define-data-var artist-address principal 'SP1M1BA3G7TYMAK8JHMQGZ089CKAWQDJ04SHQB7AK)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmedUeYXYnrom4TYyKiMt7nMv91QawkvmxoVvB4Qyga83Y/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? i-promise-i-help next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? i-promise-i-help token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? i-promise-i-help token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? i-promise-i-help token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? i-promise-i-help id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? i-promise-i-help id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? i-promise-i-help id) (err ERR-NOT-FOUND)))
