...
(define-non-fungible-token phoenix-bird uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qme1ieBtE3bg3jjDX2LYyN89Z1SoLZ83GUJbz8NDWeCb4p/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? phoenix-bird next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? phoenix-bird token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? phoenix-bird token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? phoenix-bird token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? phoenix-bird id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? phoenix-bird id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? phoenix-bird id) (err ERR-NOT-FOUND)))
