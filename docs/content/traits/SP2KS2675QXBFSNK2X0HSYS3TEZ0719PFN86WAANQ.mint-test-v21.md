---
title: "Trait mint-test-v21"
draft: true
---
```
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token test-token-name-v21 uint)
...
(define-private (mint-many-iter (ignore bool) (next-id uint)) 
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? test-token-name-v21 next-id tx-sender) next-id)
      (+ next-id u1) 
    )
    next-id)) 
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? test-token-name-v21 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? test-token-name-v21 id sender recipient)
    success
      (let (
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? test-token-name-v21 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? test-token-name-v21 id) (err ERR-NOT-FOUND)))
...
(define-private (mint-many-stsw (orders (list 5 bool ))) 
  (let ((last-nft-id (var-get last-id)) 
    ...
    (if (is-eq tx-sender DEPLOYER)
      ...
      (begin (var-set last-id id-reached)
        (map-set users-tokens tx-sender (+ current-balance (- id-reached last-nft-id)))
        (try! (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.stsw-token-v4a transfer u1 tx-sender WALLET_1 (some 0x00))) ))
    (ok true))
    )
```
