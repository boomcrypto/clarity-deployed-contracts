---
title: "Trait keeper-1-t595sq9ow-v-1-1"
draft: true
---
```
;; keeper-1-t595sq9ow-v-1-1
...
(define-data-var owner-address principal 'SP38GBVK5HEJ0MBH4CRJ9HQEW86HX0H9AP1HZ3SVZ)
...
;; Swap psBTC to STX and then transfer STX tokens to owner address
(define-public (execute-action-a
  ...
  (let (
    ...
    (keeper-check (asserts! (unwrap! (contract-call? .keeper-1-helper-v-1-1 get-keeper-status) ERR_INVALID_HELPER_DATA) ERR_KEEPER_STATUS))
    ...
    (keeper-fee-amount (unwrap! (contract-call? .keeper-1-helper-v-1-1 get-keeper-fee-amount amount) ERR_INVALID_HELPER_DATA))
    ...
    ;; Perform psBTC to STX swap via XYK Core
    (swap-psbtc-to-stx (try! (as-contract (contract-call?
    ...
    (begin
      ;; Assert swap-psbtc-to-stx is greater than or equal to min-received
      (asserts! (>= swap-psbtc-to-stx min-received) ERR_MINIMUM_RECEIVED)

      ;; Transfer STX tokens from the contract to owner-address
      (try! (as-contract (stx-transfer? swap-psbtc-to-stx tx-sender (var-get owner-address))))
