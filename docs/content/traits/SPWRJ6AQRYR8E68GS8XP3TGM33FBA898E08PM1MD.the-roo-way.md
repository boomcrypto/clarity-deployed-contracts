...
(define-non-fungible-token the-roo-way uint)
...
;; Internal variables
(define-data-var mint-limit uint u84)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZDc4Q7c7dQbCKCoaqGUsBFfqEbHRQw9BBoVigLEKAni1/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-roo-way next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-roo-way token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-roo-way token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-roo-way token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-roo-way id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-roo-way id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-roo-way id) (err ERR-NOT-FOUND)))
