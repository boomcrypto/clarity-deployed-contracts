---
title: "Trait farming-welshcorgicoin-aeusdc-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .welshcorgicoin-aeusdc)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .welshcorgicoin-aeusdc transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .welshcorgicoin-aeusdc get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .welshcorgicoin-aeusdc transfer amt protocol user none)))
