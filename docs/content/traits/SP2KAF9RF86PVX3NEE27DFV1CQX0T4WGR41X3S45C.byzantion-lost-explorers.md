---
title: "Trait byzantion-lost-explorers"
draft: true
---
```
...
(define-non-fungible-token lost-explorers uint)
...
(define-data-var ipfs-full-metadata (string-ascii 110) "ipfs://bafybeicusmy7vnurssypqyqow2eo653ch3eybzp3es3dyl4qts6k5qqc5e/lost_explorers/lost_explorers_metadata.json")
(define-data-var ipfs-root (string-ascii 97) "ipfs://bafybeicusmy7vnurssypqyqow2eo653ch3eybzp3es3dyl4qts6k5qqc5e/lost_explorers/lost_explorers_")
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? lost-explorers next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? lost-explorers token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? lost-explorers token-id)))
