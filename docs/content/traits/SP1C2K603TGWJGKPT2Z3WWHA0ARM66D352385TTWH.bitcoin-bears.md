...
(define-non-fungible-token Bitcoin-Bears uint)
...
(define-data-var total-spoints-price uint u4200000000)
(define-data-var artist-address principal 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/bafybeifmzuycuuz75cxvr3so4xpsfgescvm4gsoysbycaniv2rkq3itqfm/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? Bitcoin-Bears next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Bitcoin-Bears token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Bitcoin-Bears token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Bitcoin-Bears token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Bitcoin-Bears id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Bitcoin-Bears id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Bitcoin-Bears id) (err ERR-NOT-FOUND)))
