...
(define-non-fungible-token stacks-lego-zoo uint)
...
(define-data-var artist-address principal 'SPVJXZSNNM6N35JCDSVH58MA1JCNB8JHXQ1J9ECV)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWxBBgVKwAiYTNe1ec5aCPe12mitSUVcVoZ3a2R9GY9Dg/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-lego-zoo next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-lego-zoo token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-lego-zoo token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-lego-zoo token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-lego-zoo id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-lego-zoo id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-lego-zoo id) (err ERR-NOT-FOUND)))
