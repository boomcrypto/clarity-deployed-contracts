---
title: "Trait arkadiko-stacker-payer-v3-8"
draft: true
---
```
...
(define-public (initiate-unlock (vault-id uint))
  (let (
    ...
    (map-set unlocks { vault-id: vault-id } { unlocked-at-burn-height: (unwrap-panic (contract-call? .arkadiko-stacker-v3-1 get-stacking-unlock-burn-height)) })
    (ok true)
  )
)
