...
(define-non-fungible-token pizza-day uint)
...
(define-data-var artist-address principal 'SPY0JQ4TCN1QEZHXY98HV397323CNKBSTRCN8V2W)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeVqijcuQwwm6LXS5qEJB8j3chKZp8aTFxk2w81g4BnCt/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? pizza-day next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? pizza-day token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? pizza-day token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? pizza-day token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pizza-day id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pizza-day id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pizza-day id) (err ERR-NOT-FOUND)))
...
(define-data-var total-price-xbtc uint u44903)
