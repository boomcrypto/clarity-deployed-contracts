...
;; get version
(define-read-only (get-version (version (buff 1)))
    (unwrap-panic (element-at? BTC_VER (unwrap-panic (index-of? STX_VER version))))
)

;; get versioned-hash-bytes
(define-read-only (get-versioned-hash-bytes (version (buff 1)) (hash-bytes (buff 20)))
    (concat version hash-bytes)
)

;; get to-encode
(define-read-only (get-to-encode (versioned-hash-bytes (buff 25)))
    (map hex-to-uint (concat 
...
(define-read-only (convert-inner (data {hash-bytes: (buff 20), name: (optional (string-ascii 40)), version:(buff 1)}))
    (let (
        ...
        (version (get-version (get version data)))
        ;; concat BTC version & hash160 
        (versioned-hash-bytes (get-versioned-hash-bytes version (get hash-bytes data)))
        ;; concat hash-bytes & 4 bytes checksum, and convert hext to uint
        (to-encode (get-to-encode versioned-hash-bytes))
