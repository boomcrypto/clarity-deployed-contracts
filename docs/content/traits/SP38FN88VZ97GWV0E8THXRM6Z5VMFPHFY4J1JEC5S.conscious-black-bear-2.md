---
title: "Trait conscious-black-bear-2"
draft: true
---
```
...
;; Private functions
(define-public (hasnt-bred-public (parent-armadillo-id uint))
...
(define-read-only (get-kitty-armadillos (kitty-id uint))
    (ok (print (map-get? kitty-armadillos kitty-id)))
)

;; Get User Heart Balance
(define-read-only (get-heart-balance (user principal))
    (ok (print (map-get? heart-balance user)))
)
