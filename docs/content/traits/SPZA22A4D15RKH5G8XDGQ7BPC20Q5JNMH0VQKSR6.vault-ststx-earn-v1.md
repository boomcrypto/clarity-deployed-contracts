---
title: "Trait vault-ststx-earn-v1"
draft: true
---
```
;; @contract Vault
...
(define-read-only (get-total-pending-withdrawals) 
  (unwrap-panic (as-contract (contract-call? .token-ststx-earn-v1 get-balance tx-sender))))

(define-read-only (get-total-tokens)
  (unwrap-panic (contract-call? .token-ststx-earn-v1 get-total-supply)))
...
(define-read-only (get-total-underlying-active)
  (if (> (+ (get-total-pending-deposits) (get-underlying-reserved-for-claims)) (get-total-underlying))
    u0
...
(define-public (queue-deposit
  ...
  (let (
    ...
    (hq-data (contract-call? .hq-ststx-earn-v1 get-deposit-data)))
...
(define-public (queue-withdrawal 
  ...
  (let (
    (claim-id (+ (get-current-claim-id) u1)))
    (asserts! (> amount u0) ERR_INVALID_AMOUNT)
    (try! (contract-call? .token-ststx-earn-v1 transfer amount tx-sender vault none))
    (map-insert claims { claim-id: claim-id } 
      {
        address: tx-sender,
        epoch-id: (get-current-epoch-id),
...
(define-public (activate-pending-withdrawal-claims)
  (let (
    ...
    (if (> current-total-pending-withdrawals u0)
      (try! (as-contract (contract-call? .token-ststx-earn-v1 burn-for-vault current-total-pending-withdrawals (as-contract tx-sender))))
      true
    )
    (ok true)))
  ...
  (let (
    ...
    (if (and (> (len deposit-claims) u0) (get claim-deposits entry))
      (begin
        ...
        (if (> (var-get claim-amount-helper) u0) (try! (as-contract (contract-call? .token-ststx-earn-v1 mint-for-vault (var-get claim-amount-helper) address))) true)
    ...
    (if (and (> (len withdrawal-claims) u0) (get claim-withdrawals entry))
      (begin
        ...
        (let (
          (current-withdrawal-fee (get current (try! (contract-call? .hq-ststx-earn-v1 get-fees "withdrawal"))))
...
(define-public (deposit-funds
  ...
  (depositor principal)) 
  (begin
    (asserts! (is-eq contract-caller .core-ststx-earn-v1) ERR_ONLY_CORE_CONTRACT_ALLOWED)
    (asserts! (> amount u0) ERR_INVALID_AMOUNT)
