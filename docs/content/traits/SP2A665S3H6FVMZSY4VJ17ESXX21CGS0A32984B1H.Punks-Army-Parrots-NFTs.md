---
title: "Trait Punks-Army-Parrots-NFTs"
draft: true
---
```
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token Punks-Army-Parrots uint)

(define-constant cntrct-owner tx-sender)

(define-constant avaible_multiple_mint false)
...
(define-private (mint_punk (punk-id uint) (new-owner principal))
	(begin
		...
		(nft-mint? Punks-Army-Parrots punk-id new-owner)
	)
)
...
(define-public 
	...
	(begin     
		...
		(nft-transfer? Punks-Army-Parrots token-id sender recipient))
)
;; BURN A TOKEN
(define-public 
	(burn-token 
	...
	(begin     
		(asserts! (is-eq (some tx-sender) (nft-get-owner? Punks-Army-Parrots token-id) ) (err u403))     
		(nft-burn? Punks-Army-Parrots token-id tx-sender))
)

(
	define-read-only (get-owner (punk-id uint))  
	(ok (nft-get-owner? Punks-Army-Parrots punk-id))
)
