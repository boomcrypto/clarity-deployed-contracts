---
title: "Trait dark-azure-marlin"
draft: true
---
```
;; bitflow-sbtc-reader-v-1-1

;; Use Stableswap and XYK pool trait
...
;; Get a pool's total sBTC balance
...
(define-public (get-user-sbtc-balance-stableswap (pool-trait <stableswap-pool-trait>) (user principal))
  (let (
...
(define-public (get-user-sbtc-balance-xyk (pool-trait <xyk-pool-trait>) (user principal))
  (let (
    ;; Gather all pool data
    (pool-data (unwrap! (contract-call? pool-trait get-pool) ERR_NO_POOL_DATA))
    (x-token (get x-token pool-data))
...
;; Check if a pool is a valid sBTC pool
