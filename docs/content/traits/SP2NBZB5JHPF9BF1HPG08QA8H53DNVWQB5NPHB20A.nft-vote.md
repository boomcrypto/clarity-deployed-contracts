...
(define-non-fungible-token Stacks-vote-nft uint)
...
(define-data-var base-uri (string-ascii 80) "ipfs://QmS9aYDZu1PniS9pxS3M3G6yNAom9gkzX1qHvf9pH3tLqZ/json/1.json")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Stacks-vote-nft token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? Stacks-vote-nft token-id sender recipient)
  )
)
...
(define-public (mint (recipient principal))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? Stacks-vote-nft token-id recipient))
...
(define-public (burn (token-id uint))
  (begin 
    (asserts! (is-eq tx-sender CONTRACT_OWNER) ERR_OWNER_ONLY)
    (nft-burn? Stacks-vote-nft token-id tx-sender)))
```
