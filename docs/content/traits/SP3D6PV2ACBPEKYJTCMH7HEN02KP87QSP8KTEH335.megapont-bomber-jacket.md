---
title: "Trait megapont-bomber-jacket"
draft: true
---
```
...
(define-read-only (mint-permitted (sender principal))
    (begin
        (asserts! (> (contract-call? .megapont-ape-club-nft get-balance tx-sender) u0) false)
        (asserts! (> (unwrap! (contract-call? .mega get-balance tx-sender) false) u0) false)))
