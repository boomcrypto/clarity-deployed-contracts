...
(define-non-fungible-token pixeleton-club uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNZxbVSXNfZrGRQKjj1izLHBRi14CBMncgUyqgu1FEipc/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? pixeleton-club next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? pixeleton-club token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? pixeleton-club token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? pixeleton-club token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pixeleton-club id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pixeleton-club id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pixeleton-club id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u4000)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u3500)
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u13000)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u5)
