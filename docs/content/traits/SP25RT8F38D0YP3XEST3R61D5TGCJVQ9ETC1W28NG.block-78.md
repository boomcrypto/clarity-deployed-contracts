...
(define-non-fungible-token block-78 uint)
...
;; Internal variables
(define-data-var mint-limit uint u78)
(define-data-var last-id uint u1)
(define-data-var total-price uint u30000000)
(define-data-var artist-address principal 'SP25RT8F38D0YP3XEST3R61D5TGCJVQ9ETC1W28NG)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcU7TcUMHcbd3Xk4zoNhCFqYZQvqAZJ6mJSeKt8iya7om/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? block-78 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? block-78 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? block-78 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? block-78 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? block-78 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? block-78 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? block-78 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u59717)
