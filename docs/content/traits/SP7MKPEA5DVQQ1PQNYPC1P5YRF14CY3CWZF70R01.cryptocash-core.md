---
title: "Trait cryptocash-core"
draft: true
---
```
;; CryptoCash CORE CONTRACT
;; CryptoCash Protocol Version 1.0.1

;; GENERAL CONFIGURATION
 
(impl-trait 'SP7MKPEA5DVQQ1PQNYPC1P5YRF14CY3CWZF70R01.cryptocash-core-trait.cryptocash-core)
...
(define-data-var foundationWallet principal 'SP2JTEQ0FND1PRRB8TC86272GT1F49H6HHACGWQVE)
...
;; returns Stacks block activation block height if activated
...
(define-public (mine (amounts (list 200 uint)))
  (begin
    ...
    (match (fold mine-block amounts (ok { userId: (get-or-create-user-id tx-sender), toStackers: u0, toFoundation: u0, stacksHeight: block-height }))
      okReturn 
      (begin
...
(define-private (mine-block 
  ...
  (match return okReturn
    (let
      ...
      (try! (set-tokens-mined (get userId okReturn) stacksHeight amountUstx toStackers toFoundation))
      (ok (merge okReturn 
        {
...
(define-private (mine-tokens-at-block (userId uint) (stacksHeight uint) (amountUstx uint) (memo (optional (buff 34))))
  (let
    ...
    (try! (set-tokens-mined userId stacksHeight amountUstx toStackers toFoundation))
    (if (is-some memo)
      (print memo)
      none
    )
...
(define-private (set-tokens-mined (userId uint) (stacksHeight uint) (amountUstx uint) (toStackers uint) (toFoundation uint))
  (let
    (
...
(define-private (mint-coinbase (recipient principal) (stacksHeight uint))
  (begin 
    (print {recipient:recipient, stacksHeight:stacksHeight, coinbaseAmount:(get-coinbase-amount stacksHeight)})
    (as-contract (contract-call? .cryptocash-token mint (get-coinbase-amount stacksHeight) recipient))
  )
)
...
(define-public (activate-cryptocash)
  (let
    (
      (activationBlockVal (+ block-height u120))
    )
