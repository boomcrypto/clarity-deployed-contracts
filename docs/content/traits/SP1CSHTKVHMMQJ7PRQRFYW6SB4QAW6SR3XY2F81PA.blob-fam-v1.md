...
(define-non-fungible-token blob-fam-v1 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNsDLKfQyHn5NCTitT21qrFLVoMw5iLhKYxjt5XxVNP82/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? blob-fam-v1 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? blob-fam-v1 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? blob-fam-v1 id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? blob-fam-v1 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? blob-fam-v1 id) (err ERR-NOT-FOUND)))
