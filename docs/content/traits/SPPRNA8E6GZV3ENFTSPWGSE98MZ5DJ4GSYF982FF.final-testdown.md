---
title: "Trait final-testdown"
draft: true
---
```
(impl-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)

(define-fungible-token final-testdown)
...
(define-constant dev-principal 'SPPRNA8E6GZV3ENFTSPWGSE98MZ5DJ4GSYF982FF)
(define-constant token-supply u342534534)
...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://bafybeih7h5diwxpkpjxk3h7lvzgfeuuhbsjb7kbokzflv5xi5nehu5eo2i.ipfs.dweb.link/")) 
...
(define-read-only (get-name) (ok "Final-testdown"))
(define-read-only (get-symbol) (ok "FINALTEST"))
(define-read-only (get-decimals) (ok u0))
(define-read-only (get-balance (user principal)) (ok (ft-get-balance final-testdown user)))
(define-read-only (get-total-supply) (ok (ft-get-supply final-testdown)))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? final-testdown amount from to)
    )
)
...
(define-public (swap-stx-for-token (stx-amount uint) (min-tokens-out uint)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? final-testdown tokens-out (as-contract tx-sender) tx-sender))
      (if (>= (/ (* (+ (var-get real-stx-reserve) (var-get virtual-stx-reserve)) token-supply) (var-get token-reserve)) market-cap-threshold)
        (begin
          (let (
            ...
            (try! (ft-transfer? final-testdown burn-amount (as-contract tx-sender) burn-address))
            (try! (ft-transfer? final-testdown remaining-tokens (as-contract tx-sender) liquidity-builder-address))
...
(define-public (swap-token-for-stx (token-amount uint) (min-stx-out uint) (recipient principal)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? final-testdown token-amount tx-sender (as-contract tx-sender)))
...
(begin
  (try! (ft-mint? final-testdown token-supply (as-contract tx-sender)))
