...
(define-non-fungible-token stacks-army uint)
...
(define-data-var artist-address principal 'SP3QYTYDMFBMBE398NB0770HJM65TAHRE86PD1KSS)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZyJAF2wmRbiXCReGptsfMgHjn6eonC9WYEQUs9Wmj9tE/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? stacks-army next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? stacks-army token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-army token-id)))
