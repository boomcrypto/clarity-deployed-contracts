...
(define-fungible-token wstxarmy)

(define-data-var token-name (string-ascii 32) "STX Army Wrapper")
(define-data-var token-symbol (string-ascii 32) "wstxarmy")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wstxarmy.json"))
...
(define-read-only (get-base-token)
  'SP2ZYR2314Z50MZF2K5N0266R1JX6VDZR9P501Z9B.stacks-army-token)

(define-read-only (get-base-decimals)
  (contract-call? 'SP2ZYR2314Z50MZF2K5N0266R1JX6VDZR9P501Z9B.stacks-army-token get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP2ZYR2314Z50MZF2K5N0266R1JX6VDZR9P501Z9B.stacks-army-token get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP2ZYR2314Z50MZF2K5N0266R1JX6VDZR9P501Z9B.stacks-army-token transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
