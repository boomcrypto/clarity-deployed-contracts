---
title: "Trait prediction-test-11"
draft: true
---
```
...
(define-public (resolve-market (market-id uint) (winning-outcome uint))
    (let (
        ...
        (receipt-id (var-get next-receipt-id))  ;; Get the next receipt ID
        ...
        ;; Admin collects 5% fee in the form of a prediction on the winning outcome
        ;; 1. Store receipt data
        ...
        ;; 2. Mint NFT receipt to admin
        (try! (nft-mint? prediction-receipt receipt-id tx-sender))
        
        ;; 3. Update outcome pools to include admin's prediction
        (let (
            (current-pools (get outcome-pools market))
            (current-pool (default-to u0 (element-at? current-pools winning-outcome)))
            (updated-pool (+ current-pool admin-fee))
            (updated-pools (replace-at? current-pools winning-outcome updated-pool))
        )
            ;; Update market state with new pools and increased total pool
            (map-set markets market-id (merge market {
                outcome-pools: (unwrap-panic updated-pools),
                total-pool: (+ (get total-pool market) admin-fee),
                is-resolved: true,
            ...
            ;; 4. Increment receipt ID counter
