...
(define-non-fungible-token stx-owl uint)
...
(define-data-var artist-address principal 'SPBDJ730SVMM5EWEZ6F4ARP7PNXYXPR4KCC9HP96)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPcRm7SuQfgoJ7GXuA4coVwdKm5rsuAfGjBNjnzcN7CiB/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stx-owl next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-owl token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-owl token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-owl token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-owl id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-owl id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-owl id) (err ERR-NOT-FOUND)))
