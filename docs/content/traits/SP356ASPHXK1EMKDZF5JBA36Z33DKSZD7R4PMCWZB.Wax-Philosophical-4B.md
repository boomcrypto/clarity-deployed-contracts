---
title: "Trait Wax-Philosophical-4B"
draft: true
---
```
...
(define-non-fungible-token Wax-Philosophical-4B uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafyreihbuf6i7dm3x66zgde6ba6blr76z5rhkwe5nsqfo5l4576crzl3py/metadata.json")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Wax-Philosophical-4B token-id)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Wax-Philosophical-4B token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Wax-Philosophical-4B token-id) false)))
...
(define-private (iterate-minting (not-to-be-used bool) (next-id uint))
  (if (<= next-id (var-get max-supply))
    (begin
      (unwrap! (nft-mint? Wax-Philosophical-4B next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Wax-Philosophical-4B id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Wax-Philosophical-4B id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Wax-Philosophical-4B id) (err err-not-found)))
...
(define-private (airdrop-iter (collector principal) (next-id uint)) 
  (if (<= next-id (var-get max-supply))
    (let (
      ...
      (unwrap! (nft-mint? Wax-Philosophical-4B next-id collector) next-id)
