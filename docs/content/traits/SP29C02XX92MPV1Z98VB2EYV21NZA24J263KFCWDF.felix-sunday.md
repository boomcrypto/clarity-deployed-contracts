...
(define-constant slot-size u1000000000)
(define-constant number-of-slots u1)
(define-constant start-block-height u169903)
(define-constant start-block-buffer u10)
(define-constant end-block-height u170903)
(define-constant end-cooldown u6)
(define-non-fungible-token felix-sunday uint)
...
(define-read-only (get-owner (token-id uint))
    (ok (nft-get-owner? felix-sunday token-id)))
...
(define-public (buy-ticket (recipient principal) (selected-nums uint))
    ...
    ;; #[allow(unchecked_data)]
    (try! (nft-mint? felix-sunday ticket-id recipient))
...
(define-public (claim-prize (ticket-id uint))
    ...
    (asserts! (is-eq (unwrap! (nft-get-owner? felix-sunday ticket-id) err-inexistent-ticket-id) contract-caller) err-not-ticket-owner)
    ...
    (try! (nft-burn? felix-sunday ticket-id alleged-winner-principal))
    (ok true)))
...
(define-public (get-ticket-refund (ticket-id uint))
    ...
    (try! (nft-burn? felix-sunday ticket-id ticket-owner))
    (ok ticket-id)))
...
(define-public (burn-ticket (ticket-id uint))
    (begin
        ...
        ;; #[allow(unchecked_data)]
        (nft-burn? felix-sunday ticket-id contract-caller)))
