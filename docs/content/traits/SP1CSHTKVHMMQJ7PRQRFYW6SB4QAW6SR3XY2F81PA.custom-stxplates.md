---
title: "Trait custom-stxplates"
draft: true
---
```
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)
(define-non-fungible-token custom-stxplates uint)
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? custom-stxplates token-id)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? custom-stxplates token-id tx-sender)))
...
(define-public (mint (uri (string-ascii 64)) (text (string-ascii 25)) (text-color (string-ascii 10)) (background-color (string-ascii 10)) (border-color (string-ascii 10)) (owned-tokens (list 10 uint)))
  (let 
    ...
    (unwrap! (nft-mint? custom-stxplates token-id tx-sender) ERR-NFT-MINT)
...
(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? custom-stxplates token-id) false)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? custom-stxplates id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? custom-stxplates id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? custom-stxplates id) ERR-PLATE-NOT-FOUND))
