---
title: "Trait btc-peg-out-endpoint-v1-12"
draft: true
---
```
...
(define-read-only (get-request-revoke-grace-period)
	(contract-call? .btc-bridge-registry-v1-04 get-request-revoke-grace-period))
(define-read-only (get-request-claim-grace-period)
	(contract-call? .btc-bridge-registry-v1-04 get-request-claim-grace-period))
(define-read-only (get-request-or-fail (request-id uint))
	(contract-call? .btc-bridge-registry-v1-04 get-request-or-fail request-id))
...
(define-read-only (validate-peg-out-1 (token principal) (factor uint) (dx uint) (min-dy uint))
	(let (
			(dy (try! (contract-call? .amm-swap-pool-v1-1 get-helper token .token-abtc factor dx))))
		(asserts! (>= dy min-dy) err-slippage)
		(validate-peg-out-0 dy)))
(define-read-only (validate-peg-out-2 (token1 principal) (token2 principal) (factor1 uint) (factor2 uint) (dx uint) (min-dz uint))
	(let (
			(dz (try! (contract-call? .amm-swap-pool-v1-1 get-helper-a token1 token2 .token-abtc factor1 factor2 dx))))
		(asserts! (>= dz min-dz) err-slippage)
		(validate-peg-out-0 dz)))
(define-read-only (validate-peg-out-3 (token1 principal) (token2 principal) (token3 principal) (factor1 uint) (factor2 uint) (factor3 uint) (dx uint) (min-dw uint))
	(let (
			(dw (try! (contract-call? .amm-swap-pool-v1-1 get-helper-b token1 token2 token3 .token-abtc factor1 factor2 factor3 dx))))
		(asserts! (>= dw min-dw) err-slippage)
		(validate-peg-out-0 dw)))
...
(define-public (request-peg-out-0 (peg-out-address (buff 128)) (amount uint))
	(let (
			...
			(request-id (as-contract (try! (contract-call? .btc-bridge-registry-v1-04 set-request u0 request-details)))))
...
(define-public (claim-peg-out (request-id uint) (fulfilled-by (buff 128)))
	(let (
		...
		(as-contract (try! (contract-call? .btc-bridge-registry-v1-04 set-request request-id (merge request-details { claimed: (+ block-height (get-request-claim-grace-period)), claimed-by: claimer, fulfilled-by: fulfilled-by }))))
...
(define-public (finalize-peg-out
	...
	(let (
		...
		(as-contract (try! (contract-call? .btc-bridge-registry-v1-04 set-request request-id (merge request-details { finalized: true }))))
...
(define-public (revoke-peg-out (request-id uint))
	(let (
		...
		(asserts! (not (get revoked request-details)) err-request-already-revoked)
		(asserts! (not (get finalized request-details)) err-request-already-finalized)
		(as-contract (try! (contract-call? .btc-bridge-registry-v1-04 set-request request-id (merge request-details { revoked: true }))))
