...
(define-public (cancel (id uint) (ft <fungible-token>) (fees <fees-trait>))
  (let ((swap (unwrap! (map-get? swaps id) ERR_INVALID_ID))
    (ustx (get ustx swap)))
      ...
      (asserts! (< (+ (get when swap) expiry) block-height) ERR_TOO_EARLY) ;; anyone can cancel after expiry and get the worm
...
;; stx-sender can cancel the swap before expiry
