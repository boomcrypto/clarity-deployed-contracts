...
(define-non-fungible-token ethereal-essence uint)
...
;; Internal variables
(define-data-var mint-limit uint u207)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbMxdxE5uBQRxnGpGgjmwKShfecbeUM5mosbHiLDzWmre/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ethereal-essence next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ethereal-essence token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ethereal-essence token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ethereal-essence token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ethereal-essence id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ethereal-essence id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ethereal-essence id) (err ERR-NOT-FOUND)))
