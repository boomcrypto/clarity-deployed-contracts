...
(define-non-fungible-token cuneiforms uint)
...
(define-data-var artist-address principal 'SP2AHWHTM6N24VFP1HETP9SRKBV1WR68P8K2AV46T)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUQhx55Dq9LorxyeVPuBjeYmezrTLTUSEfyEG1p7bNags/json/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u700)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? cuneiforms next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? cuneiforms token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? cuneiforms token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? cuneiforms token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? cuneiforms id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? cuneiforms id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? cuneiforms id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u4230)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u5211000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u4576000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u10000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u123000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u800000000)
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? cuneiforms (+ last-nft-id u0) 'SP28WFMAB4WJ1HSPEM1Z5S8TMHPR9086H802WPEP6))
      (map-set token-count 'SP28WFMAB4WJ1HSPEM1Z5S8TMHPR9086H802WPEP6 (+ (get-balance 'SP28WFMAB4WJ1HSPEM1Z5S8TMHPR9086H802WPEP6) u1))
      (try! (nft-mint? cuneiforms (+ last-nft-id u1) 'SP2MP2EY11MDBJSRFG4EYC67VTVGGN78NWJGDR391))
      (map-set token-count 'SP2MP2EY11MDBJSRFG4EYC67VTVGGN78NWJGDR391 (+ (get-balance 'SP2MP2EY11MDBJSRFG4EYC67VTVGGN78NWJGDR391) u1))
      (try! (nft-mint? cuneiforms (+ last-nft-id u2) 'SPNEPYMEX3VEJ5CNHBQ66ESV1GERVKFP59EDSMJR))
      (map-set token-count 'SPNEPYMEX3VEJ5CNHBQ66ESV1GERVKFP59EDSMJR (+ (get-balance 'SPNEPYMEX3VEJ5CNHBQ66ESV1GERVKFP59EDSMJR) u1))
      (try! (nft-mint? cuneiforms (+ last-nft-id u3) 'SP3S9KXPRG4AQB3ME8BXT4PTGY275CY06GPFWSJ3X))
      (map-set token-count 'SP3S9KXPRG4AQB3ME8BXT4PTGY275CY06GPFWSJ3X (+ (get-balance 'SP3S9KXPRG4AQB3ME8BXT4PTGY275CY06GPFWSJ3X) u1))
      (try! (nft-mint? cuneiforms (+ last-nft-id u4) 'SP6Y4T2QESRP24QR0HJJD1R4WP2CSV4DRXSZRX77))
      (map-set token-count 'SP6Y4T2QESRP24QR0HJJD1R4WP2CSV4DRXSZRX77 (+ (get-balance 'SP6Y4T2QESRP24QR0HJJD1R4WP2CSV4DRXSZRX77) u1))
