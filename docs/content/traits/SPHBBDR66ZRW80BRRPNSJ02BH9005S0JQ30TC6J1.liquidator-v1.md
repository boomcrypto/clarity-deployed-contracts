...
(define-data-var liquidation-buffer uint u5000)
...
(define-public (set-liquidation-buffer (buffer uint))
  (begin 
    (print {
      action: "update-liquidation-buffer",
      prev: (var-get liquidation-buffer),
      current: buffer,
    })

    (var-set liquidation-buffer buffer)
    SUCCESS
  )
)
...
(define-private (execute-liquidation (user principal) (collateral <token-trait>) (liquidator-repay-amount uint) (min-collateral-expected uint))
  (let
      ...
      (asserts! (<= (get position-health (try! (account-health user none none))) (+ (var-get liquidation-buffer) SCALING-FACTOR)) ERR-LIQUIDATED-TOO-MUCH)
