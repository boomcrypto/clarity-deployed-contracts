...
(define-non-fungible-token anime-girls uint)
...
(define-data-var artist-address principal 'SP2CC8YR1SV8VT1H3RBZJFBD2V8HA36S9C7JZKP2B)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmU13ueRvBksK5GAJdL8AbNFXz4qjUTPX8cNwsctxUYXNd/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? anime-girls next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? anime-girls token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? anime-girls token-id)))
