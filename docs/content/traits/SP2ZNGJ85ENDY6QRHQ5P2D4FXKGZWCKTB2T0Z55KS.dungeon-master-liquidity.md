---
title: "Trait dungeon-master-liquidity"
draft: true
---
```
;; Title: Dungeon Master Liquidity
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token DML)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.dungeon-master-liquidity"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? DML amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "Dungeon Master Liquidity"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance DML who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply DML)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? DML (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? DML (get dk delta) sender))
...
(begin
    ...
    (try! (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token transfer u91000000 tx-sender CONTRACT none)))
```
