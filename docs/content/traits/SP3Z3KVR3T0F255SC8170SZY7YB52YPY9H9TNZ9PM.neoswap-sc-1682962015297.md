---
title: "Trait neoswap-sc-1682962015297"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/NeTUGgs6iSeO4D2dp2Th
;; SwapId: jWmHbDQlhs8Z8QvotHgL
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u14820000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u14820000 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u14820000 tx-sender TRADER-2)))
	    )
            true
        )
