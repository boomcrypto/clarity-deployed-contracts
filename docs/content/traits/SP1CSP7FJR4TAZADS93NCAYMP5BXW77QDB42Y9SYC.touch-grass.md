...
(define-non-fungible-token touch-grass uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcD9rURQCZq5GdDZiSRxB1NVkdugEBumGp9vUTzfE7Sfn/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? touch-grass next-id tx-sender) next-id)
      (unwrap! (nft-transfer? touch-grass next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? touch-grass token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? touch-grass token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? touch-grass token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? touch-grass id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? touch-grass id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? touch-grass id) (err ERR-NOT-FOUND)))
