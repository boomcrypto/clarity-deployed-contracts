---
title: "Trait cross-router-v2-01"
draft: true
---
```
...
(define-read-only (destruct-principal (address principal))
	(contract-call? .bridge-common-v2-01 destruct-principal address))

(define-read-only (construct-principal (hash-bytes (buff 20)))
	(contract-call? .bridge-common-v2-01 construct-principal hash-bytes))

(define-read-only (validate-route (amount uint) (routing-tokens (list 5 principal)) (routing-factors (list 4 uint)) (min-amount-out (optional uint)) (recipient { address: (buff 128), chain-id: (optional uint) }))
	(let (
		...
		(match (get chain-id recipient)
			...
			(if (> some-value u1000)
				(begin 
					(try! (contract-call? .meta-peg-out-endpoint-v2-02 validate-peg-out (get amount route-details) { token: (get token route-details), chain-id: some-value })) 
					(ok true))
				(if (is-eq some-value u0)
					(begin 
						(asserts! (is-eq (get token route-details) 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-abtc) err-invalid-token)
						(try! (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.btc-peg-out-endpoint-v2-01 validate-peg-out-0 (get amount route-details))) 
						(ok true))
					(begin 
						(try! (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.cross-peg-out-endpoint-v2-01 validate-transfer-to-unwrap tx-sender (get token route-details) (get amount route-details) some-value))
						(ok true))))
			(begin 
				(unwrap! (construct-principal (unwrap! (as-max-len? (get address recipient) u20) err-principal-construct)) err-principal-construct)
				(ok true)))))	

;; @dev failure and fallback should be handled by the consumer
(define-public (route (amount uint) (routing-tokens (list 5 <ft-trait>)) (routing-factors (list 4 uint)) (min-amount-out (optional uint)) (recipient { address: (buff 128), chain-id: (optional uint) }))
	(let (
			(routing-len-check (asserts! (is-eq (len routing-tokens) (+ (len routing-factors) u1)) err-invalid-routing))
			(token-trait (if (is-eq (len routing-tokens) u1) (unwrap-panic (element-at? routing-tokens u0))
				(if (is-eq (len routing-tokens) u2) (unwrap-panic (element-at? routing-tokens u1))
					(if (is-eq (len routing-tokens) u3) (unwrap-panic (element-at? routing-tokens u2))
						(if (is-eq (len routing-tokens) u4) (unwrap-panic (element-at? routing-tokens u3)) (unwrap-panic (element-at? routing-tokens u4)))))))
		...
		(begin
			(match (get chain-id recipient) some-value
				(if (> some-value u1000)
					(try! (contract-call? .meta-peg-out-endpoint-v2-02 request-peg-out amount-out (get address recipient) token-trait some-value))
					(if (is-eq some-value u0)
						(begin 
							(asserts! (is-eq (contract-of token-trait) 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-abtc) err-invalid-token)
							(try! (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.btc-peg-out-endpoint-v2-01 request-peg-out-0 (get address recipient) amount-out)) 
							true)
						(try! (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.cross-peg-out-endpoint-v2-01 transfer-to-unwrap token-trait amount-out some-value (get address recipient)))))		
				(try! (contract-call? token-trait transfer-fixed amount tx-sender (unwrap! (construct-principal (unwrap! (as-max-len? (get address recipient) u20) err-principal-construct)) err-principal-construct) none)))
			(ok true))))
