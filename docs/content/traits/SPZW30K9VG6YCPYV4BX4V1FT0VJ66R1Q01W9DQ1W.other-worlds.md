...
(define-non-fungible-token other-worlds uint)
...
(define-data-var artist-address principal 'SP3JEJE6WXYR9WAQP2F0N76093Y01V20X26EY0Y4E)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdPqNfawLKLdc7zUDqh5WkrZFdZcUoFn2mLz1tco7PU1u/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? other-worlds next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (set-price (price uint))
  (begin
    (asserts! (is-eq tx-sender COMM_ADDR1) (err err-invalid-user))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? other-worlds token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? other-worlds token-id)))
