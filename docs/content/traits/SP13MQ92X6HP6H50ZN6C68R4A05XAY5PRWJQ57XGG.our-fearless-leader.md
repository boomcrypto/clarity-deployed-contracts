---
title: "Trait our-fearless-leader"
draft: true
---
```
;; our-fearless-leader
...
(define-non-fungible-token our-fearless-leader uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVJxeNdCRWcMLEVcub7xgGPFgyyM3jQhpA6SbQQTKbMPS/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? our-fearless-leader next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? our-fearless-leader token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? our-fearless-leader token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? our-fearless-leader token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? our-fearless-leader id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? our-fearless-leader id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? our-fearless-leader id) (err ERR-NOT-FOUND)))
