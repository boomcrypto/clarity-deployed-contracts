...
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)
  ...
  (define-fungible-token heater MAX_SUPPLY)
  ...
  (define-public
    ...
    (begin
      ...
      (ft-transfer? heater amt from to)))


  (define-public (mint (amt uint) (to principal))
    (begin
      (try! (check-owner))
      (ft-mint? heater amt to) ))

  (define-read-only (get-name)                   (ok "heater"))
  (define-read-only (get-symbol)                 (ok "HEAT"))
  (define-read-only (get-decimals)               (ok u5))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance heater of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply heater)))
