---
title: "Trait donald-trump-iconic-memories"
draft: true
---
```
;; donald-trump-iconic-memories
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token donald-trump-iconic-memories uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaCGAvGbR4CRvwuERZH8dLdoyt5BcQ3LtA4ZDWThAndRE/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? donald-trump-iconic-memories next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? donald-trump-iconic-memories token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? donald-trump-iconic-memories token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? donald-trump-iconic-memories token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? donald-trump-iconic-memories id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? donald-trump-iconic-memories id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? donald-trump-iconic-memories id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u9483)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u4300000000)
