...
(define-data-var flexible-strategy bool true)
(define-data-var option-type uint u0) ;; type of the option (u1 = vanilla, u2 = ERKO, u3 = ERKI, u4 = spread, u5 = binary)
(define-data-var strategy-type uint u0) ;; type of the strategy (u1 = call, u2 = put, u3 = callput, u4 = callstrip, u5 = putstrip, u6 = shortcall, u7 = shortput)
(define-data-var min-epoch-duration uint week-in-ms)
(define-data-var max-epoch-duration uint (* u8 week-in-ms))

(define-data-var epoch-risk uint u200) ;; bps of total-underlying in vault that can be deployed into trades every epoch
(define-data-var max-epoch-risk uint u250) ;; max bps of total-underlying in vault that can be deployed into trades every epoch
(define-data-var next-epoch-risk uint u200)
...
(define-data-var no-profit-management-fee bool true)
...
(define-data-var pnl-calculation-window uint (* (* u72 u60) minute-in-ms)) ;; 72 hours (in ms)
(define-data-var max-pnl-calculation-window uint (* (* u72 u60) minute-in-ms))

(define-data-var vault-capacity uint u1000000000000) ;; underlying
...
(define-data-var option-asset-symbol (buff 32) 0xec7a775f46379b5e943c3526b1c8d54cd49749176b0b98e02dde68d1bd335c17) ;; "STX" price feed id (as buff)
...
(define-read-only (get-option-asset-symbol)
  (var-get option-asset-symbol))
...
(define-public (set-option-asset-symbol (new-option-asset-symbol (buff 32))) 
  (begin
    (try! (check-is-admin tx-sender))
    (ok (var-set option-asset-symbol new-option-asset-symbol))))
...
(define-public (update-fees-and-settings)
  (let (
    ...
    (asserts! (is-eq tx-sender .core-ststx-earn-v1) ERR_ONLY_CORE_CONTRACT_ALLOWED)
