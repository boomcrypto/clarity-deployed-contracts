...
(define-fungible-token lottery-t-alex-v1)
...
(define-read-only (get-total-supply)
  (ok (ft-get-supply lottery-t-alex-v1))
)

(define-read-only (get-name)
  (ok "lottery-t-alex-v1")
)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance lottery-t-alex-v1 account))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? lottery-t-alex-v1 amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? lottery-t-alex-v1 amount recipient)
  )
)
...
(define-public (burn (amount uint) (sender principal))
  (begin
    (try! (check-is-approved contract-caller))
    (ft-burn? lottery-t-alex-v1 amount sender)
  )
)
...
(define-read-only (get-total-supply-fixed)
  (ok (decimals-to-fixed (ft-get-supply lottery-t-alex-v1)))
)

(define-read-only (get-balance-fixed (account principal))
  (ok (decimals-to-fixed (ft-get-balance lottery-t-alex-v1 account)))
)
...
(map-set approved-contracts .test-launchpad-v1 true)
```
