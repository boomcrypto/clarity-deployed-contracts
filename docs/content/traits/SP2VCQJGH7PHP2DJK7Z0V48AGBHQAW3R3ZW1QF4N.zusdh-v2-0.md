...
(define-constant asset-addr 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1)
(define-constant decimals u8)

(define-read-only (get-total-supply)
  (contract-call? .zusdh-token get-total-supply))

(define-read-only (get-name)
  (contract-call? .zusdh-token get-name))

(define-read-only (get-symbol)
  (contract-call? .zusdh-token get-symbol))

(define-read-only (get-decimals)
  (contract-call? .zusdh-token get-decimals))

(define-read-only (get-token-uri)
  (contract-call? .zusdh-token get-token-uri))
...
(define-private (transfer-internal (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (contract-call? .zusdh-token transfer amount sender recipient memo)
)

(define-private (mint-internal (amount uint) (owner principal))
  (contract-call? .zusdh-token mint amount owner)
)

(define-private (burn-internal (amount uint) (owner principal))
  (contract-call? .zusdh-token burn amount owner)
)
...
(define-read-only (get-principal-balance (account principal))
  (ok
    (contract-call? .zusdh-token get-balance account)
  )
)
