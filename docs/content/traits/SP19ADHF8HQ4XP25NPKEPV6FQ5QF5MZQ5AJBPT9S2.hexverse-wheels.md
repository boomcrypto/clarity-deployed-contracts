...
(define-non-fungible-token hexverse-wheels uint)
...
(define-data-var artist-address principal 'SP3DRY14EVZN9JVYATAKEAVSNN5ZMWJ47TMSKRDPS)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRYiW3CaGwuHLojBWwJsWBRPeFZz7yGTXGCWme1A7h3uS/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? hexverse-wheels next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? hexverse-wheels token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? hexverse-wheels token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? hexverse-wheels token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? hexverse-wheels id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? hexverse-wheels id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? hexverse-wheels id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u455766)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u90000000)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u12835100000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u26119000000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u369000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u2154000000)
