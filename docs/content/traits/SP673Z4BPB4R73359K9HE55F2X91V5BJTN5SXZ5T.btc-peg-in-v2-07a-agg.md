---
title: "Trait btc-peg-in-v2-07a-agg"
draft: true
---
```
...
(define-data-var btc-peg-outfee uint u0)
(define-data-var btc-peg-out-min-fee uint u0)
...
(define-public (set-btc-peg-out-fee (fee uint))
	(begin
		(try! (is-dao-or-extension))
		(ok (var-set btc-peg-outfee fee))))

(define-public (set-btc-peg-out-min-fee (fee uint))
	(begin
		(try! (is-dao-or-extension))
		(ok (var-set btc-peg-out-min-fee fee))))
...
(define-read-only (get-btc-peg-out-fee)
	(var-get btc-peg-outfee))

(define-read-only (get-btc-peg-out-min-fee)
	(var-get btc-peg-out-min-fee))
...
(define-read-only (create-order-agg-or-fail (order { from: (buff 128), to: (buff 128), token-out: principal, min-amount-out: (optional uint), chain-id: uint }))
	(ok (unwrap! (to-consensus-buff? { f: (get from order), r: (get to order), o: (get token-out order), m: (match (get min-amount-out order) some-value (int-to-ascii some-value) "none"), c: (int-to-ascii (get chain-id order)) }) err-invalid-input)))

(define-read-only (decode-order-agg-or-fail (order-script (buff 512)) (offset uint))
	(let (
			(raw-order (unwrap! (from-consensus-buff? { f: (buff 128), r: (buff 128), o: principal, m: (string-ascii 40), c: (string-ascii 40) } (unwrap-panic (slice? order-script offset (len order-script)))) err-invalid-input)))
		(ok { from: (get f raw-order), to: (get r raw-order), token-out: (get o raw-order), min-amount-out: (if (is-eq (get m raw-order) "none") none (some (unwrap-string-to-uint (get m raw-order)))), chain-id: (unwrap-string-to-uint (get c raw-order)) })))
...
(define-read-only (validate-tx-agg (commit-tx { tx: (buff 32768), output-idx: uint }) (reveal-tx { tx: (buff 32768), order-idx: uint }))
	(let (
		...
		(ok { order-details: (try! (decode-order-agg-or-fail (get order-script validation-data) u0)), fee: (get fee validation-data), amount-net: (get amount-net validation-data) })))
...
(define-public (finalize-peg-in-agg
	...
	(let (
		...
		(as-contract (try! (contract-call? .cross-peg-out-v2-01-agg transfer-to-swap (get amount-net validation-data) 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-abtc (get token-out order-details) (get min-amount-out order-details) (get chain-id order-details) { address: (get to order-details), chain-id: (some u0) })))			
