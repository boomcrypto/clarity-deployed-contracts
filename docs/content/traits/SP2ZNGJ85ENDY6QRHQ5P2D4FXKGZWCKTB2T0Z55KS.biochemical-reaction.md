---
title: "Trait biochemical-reaction"
draft: true
---
```
;; Title: ALEX-DEX ALEX Wrapper
...
(define-constant TOKEN-NAME "Biochemical Reaction")
(define-constant TOKEN-SYMBOL "BCR")
(define-constant TOKEN-URI (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.biochemical-reaction"))
...
(define-private (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01 swap-helper
            ...
            'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.token-wdex
            ALEX-FACTOR
            (* amount u1)
            none))
        (ok delta)))
...
(define-read-only (get-swap-quote (amount uint) (opcode (optional (buff 16))))
    (let (
        ...
        (alex-out (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01 get-helper
            (if is-a-in 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.token-wdex)
            (if is-a-in 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.token-wdex 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex)
            ALEX-FACTOR
            (* amount (if is-a-in u1 u100))))))
        {
            dx: amount,
            dy: (/ alex-out (if is-a-in u100 u1)),
            dk: u0
        }))
...
(define-read-only (get-reserves-quote)
    (let (
        (pool (get-pool)))
        {
            dx: (/ (get balance-x pool) u1),
            dy: (/ (get balance-y pool) u100),
