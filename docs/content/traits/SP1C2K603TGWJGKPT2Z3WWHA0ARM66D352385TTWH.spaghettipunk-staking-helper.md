---
title: "Trait spaghettipunk-staking-helper"
draft: true
---
```
...
(define-public (collect)
  (let (
    ...
    (if (> bonus-balance u0)
        (begin 
            (try! (contract-call? .spaghettipunk-staking mint-sp .test-token bonus-balance tx-sender))
...
(define-private (collect-one (collection principal))
    (let (
        ...
        (if (> to-collect u0)
            (begin
                (try! (contract-call? .spaghettipunk-staking collect collection .test-token))
                (ok true)
            )
            (ok false)
        )       
