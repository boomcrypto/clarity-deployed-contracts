---
title: "Trait wrapped-lydian-helper-v1-3"
draft: true
---
```
...
(define-public (redeem-bond-wldn (bond-type uint) (bond-id uint))
  (let (
    (sldn-amount (unwrap-panic (contract-call? .bond-teller-v1-3 redeem bond-type bond-id)))
  )
...
(define-public (redeem-all-bonds-wldn (bond-type uint))
  (let (
    (sldn-amount (unwrap-panic (contract-call? .bond-teller-v1-3 redeem-all bond-type)))
  )
    (contract-call? .wrapped-lydian-creator-v1-1 wrap sldn-amount)
  )
)
