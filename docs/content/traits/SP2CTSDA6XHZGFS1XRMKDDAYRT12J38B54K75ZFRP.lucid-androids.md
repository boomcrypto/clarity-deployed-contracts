...
(define-non-fungible-token lucid-androids uint)
...
;; Internal variables
(define-data-var mint-limit uint u889)
(define-data-var last-id uint u1)
(define-data-var total-price uint u2000000)
(define-data-var artist-address principal 'SP2CTSDA6XHZGFS1XRMKDDAYRT12J38B54K75ZFRP)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTdo8gqfZYif9qQmzqcaQ8Xob4Tp4QSav3je4ery9TSrG/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? lucid-androids next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? lucid-androids token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? lucid-androids token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? lucid-androids token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? lucid-androids id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? lucid-androids id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? lucid-androids id) (err ERR-NOT-FOUND)))
