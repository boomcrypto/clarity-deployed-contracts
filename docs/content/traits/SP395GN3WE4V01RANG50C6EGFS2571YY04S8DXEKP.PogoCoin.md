...
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)
  ...
  (define-fungible-token PogoCoin MAX_SUPPLY)
  ...
  (define-public
    ...
    (begin
      ...
      (ft-transfer? PogoCoin amt from to)))


  (define-public (mint (amt uint) (to principal))
    (begin
      (try! (check-owner))
      (ft-mint? PogoCoin amt to) ))

  (define-read-only (get-name)                   (ok "PogoCoin"))
  (define-read-only (get-symbol)                 (ok "POGO"))
  (define-read-only (get-decimals)               (ok u5))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance PogoCoin of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply PogoCoin)))
