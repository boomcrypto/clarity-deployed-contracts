---
title: "Trait laser-eyes-market-v3"
draft: true
---
```
;; Laser eyes NFT market (visit https://onedotbtcus.bitbucket.io/ or 1.btc.us)
...
(define-data-var m_min_price uint u1000000)
(define-data-var m_max_price uint u999999990000)
...
(define-data-var m_stat_switch bool true)
...
(define-public (list_nft (tid uint) (price uint))
  (let
    (
      (sys_owner (unwrap! (unwrap-panic (contract-call? .laser-eyes-v3 get-owner tid)) (err ERR_TOKEN_NOT_EXIST)))
      (caller tx-sender)
    )
    ...
    (and (var-get m_stat_switch) (stat_list tid price))
    (contract-call? .laser-eyes-v3 transfer_by_market_list tid sys_owner (as-contract tx-sender))
  )
)
...
(define-public (cancel_list (tid uint))
  (let
    ...
    (and (var-get m_stat_switch) (stat_list tid u0))
    (contract-call? .laser-eyes-v3 transfer_by_market_list tid (as-contract tx-sender) owner)
  )
)
...
(define-public (accept_bid (tid uint) (bidder principal))
  (let
    (
      (owner (unwrap! (contract-call? .laser-eyes-v3 get_player_by_id tid) (err ERR_TOKEN_NOT_EXIST)))
      ...
      (fee (/ (* price (var-get m_fee)) u1000))
      (seller_get (- price fee))
      (deposit (default-to u0 (map-get? map_bidder_deposit bidder)))
      (caller tx-sender)
      (bh block-height)
    )
    (asserts! (is-none (contract-call? .laser-eyes-v3 get_id_by_player bidder)) (err ERR_ACCOUNT_ALREADY_ASSOCIATED))
    ...
    (try! (as-contract (contract-call? .laser-eyes-v3 deduct fee)))
    ;;
    (if b_list
      (try! (contract-call? .laser-eyes-v3 transfer_by_market_trade tid tx-sender bidder))
      (try! (contract-call? .laser-eyes-v3 transfer tid tx-sender bidder))
    )
    ...
    (and (var-get m_stat_switch) (stat_list tid u0) (stat_trade tid price))
    (ok true)
  )
)
...
(define-public (buy (tid uint))
  (let
    ...
    (asserts! (is-none (contract-call? .laser-eyes-v3 get_id_by_player caller)) (err ERR_ACCOUNT_ALREADY_ASSOCIATED))
    ...
    (try! (contract-call? .laser-eyes-v3 deduct fee))
    ;;
    (try! (contract-call? .laser-eyes-v3 transfer_by_market_trade tid (get owner list_info) caller))
    ;;
    (map-delete map_list_info tid)
...
(define-public (bid (tid uint) (price uint) (days uint))
  (let
    ...
    (asserts! (is-some (contract-call? .laser-eyes-v3 get_player_by_id tid)) (err ERR_TOKEN_NOT_EXIST))
    (asserts! (is-none (contract-call? .laser-eyes-v3 get_id_by_player tx-sender)) (err ERR_ACCOUNT_ALREADY_ASSOCIATED))
    (asserts! (and (>= price (var-get m_min_price)) (<= price (var-get m_max_price))) (err ERR_PRICE_INVALID))
          ...
          (if (> block-height (get expiration bi))
            ...
            (if (> (get up ud) u0) ud
              (if (< (default-to u0 (map-get? map_bidder_deposit (get player bi))) (get price bi))
                ...
                (if (> (get ap ud) u0)
                  ud
                  (if (is-some (contract-call? .laser-eyes-v3 get_id_by_player (get player bi)))
                    (merge ud { ap: i }) 
...
(define-public (update_stat_switch (b_open bool))
  (begin
    (asserts! (is-eq tx-sender OWNER) (err ERR_NO_AUTHORITY))
    (var-set m_stat_switch b_open)
    (ok true)
  )
)
...
(define-read-only (get_summary (player_opt (optional principal)))
  (match player_opt player
    (let
      (
        (resolve_info (contract-call? .laser-eyes-v3 resolve_player player))
...
(define-read-only (get_token_brief (tid uint))
  (let
    (
      (info (contract-call? .laser-eyes-v3 resolve_tid tid))
      (ud (get ud info))
    )
    {
      bh: block-height,
...
(define-read-only (get_pre_act_info (player principal))
  {
    tid: (contract-call? .laser-eyes-v3 get_id_by_player player),
    deposit: (default-to u0 (map-get? map_bidder_deposit player)),
  }
)
...
(define-public (update_price (min_price uint) (max_price uint) (fee uint))
  (begin
    (asserts! (is-eq tx-sender OWNER) (err ERR_NO_AUTHORITY))
...
(define-public (update_limitations (min_bid_days uint) (max_bid_days uint))
  (begin
    (asserts! (is-eq tx-sender OWNER) (err ERR_NO_AUTHORITY))
