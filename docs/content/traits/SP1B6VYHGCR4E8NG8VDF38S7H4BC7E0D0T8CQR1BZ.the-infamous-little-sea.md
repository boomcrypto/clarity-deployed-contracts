---
title: "Trait the-infamous-little-sea"
draft: true
---
```
...
(define-non-fungible-token the-infamous-little-sea uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmfGYEGiFGwJunxVLLt8Q9Fn9tFo7ybXjZ7SK2bDaEiK1w/the-infamous-little-sea/")
...
(define-data-var private-target-block-height uint u67376)
(define-data-var public-target-block-height uint u71840)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-infamous-little-sea id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace the-infamous-little-sea
  (ok (nft-get-owner? the-infamous-little-sea id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? the-infamous-little-sea next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? the-infamous-little-sea id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-infamous-little-sea id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-infamous-little-sea id) ERR-NOT-FOUND))
...
(define-data-var xusd-cost-per-mint uint u10000)
...
(begin
  ...
  (try! (set-private-wl 'SP2WQ19W5JBMFFPW6KYYEKTFWMCYAAKQHYVH4P6SV u10))
  (ok true)
)
