---
title: "Trait neoswap-sc-1681684389717"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/g8FqJSMDt2ngX2Cs3I4w
;; SwapId: H7Z36CIiVb3UYnYH6E3i
...
(define-constant TRADER-3 'SP398XE371G08T84A99TCBD8XKWY3S7VVX6JKJWKY)
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.steady-lads transfer u4995 tx-sender TRADER-3)))

	(unwrap-panic (as-contract (stx-transfer? u453000 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (stx-transfer? u508000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u3869000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.steady-lads transfer u4995 tx-sender TRADER-2)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-3)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u4830000 tx-sender TRADER-3)))
	    )
            true
        )
