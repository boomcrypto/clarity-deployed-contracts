---
title: "Trait fractal-generation"
draft: true
---
```
;; fractal-generation
...
(define-non-fungible-token fractal-generation uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQcp8Nn2vCLA224wkCtER3AJuXd8CFeDKcgwCFthTy5FB/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? fractal-generation next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? fractal-generation token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? fractal-generation token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? fractal-generation token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? fractal-generation id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? fractal-generation id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? fractal-generation id) (err ERR-NOT-FOUND)))
