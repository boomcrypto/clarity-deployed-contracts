...
(define-fungible-token runes-trump)

(define-data-var token-name (string-ascii 32) "THE REAL TRUMP RUNE")
(define-data-var token-symbol (string-ascii 10) "TRUMP")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/runes-trump.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(try! (ft-transfer? runes-trump amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance runes-trump who)))

(define-read-only (get-total-supply)
	(ok (ft-get-supply runes-trump)))
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? runes-trump amount recipient)))
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? runes-trump amount sender)))
