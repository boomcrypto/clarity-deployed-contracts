...
(define-non-fungible-token the-earth-rase uint)
...
(define-data-var artist-address principal 'SPGYVECG634WDDNT34A5RRBB837F3GJCV8MG8Q23)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWsVgNx1DAPFyhpyssPdumDhLNG1UNCzkfvSm8iS1p4cN/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-earth-rase next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-earth-rase token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-earth-rase token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-earth-rase token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-earth-rase id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-earth-rase id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-earth-rase id) (err ERR-NOT-FOUND)))
