...
(define-non-fungible-token stx-skull-army uint)
...
(define-data-var artist-address principal 'SP2Y5Y9QJ69679NJ2HPBBVEJKGYZMSX4E46JEQQKE)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPhuiVbPCHyDZWvCY8xE9GXyY3SG4YLsx22A4jWRXLpUR/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stx-skull-army next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-skull-army token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-skull-army token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-skull-army token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-skull-army id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-skull-army id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-skull-army id) (err ERR-NOT-FOUND)))
