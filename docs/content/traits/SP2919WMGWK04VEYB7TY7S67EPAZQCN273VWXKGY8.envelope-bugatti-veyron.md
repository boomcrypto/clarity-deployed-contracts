---
title: "Trait envelope-bugatti-veyron"
draft: true
---
```
;; envelope-bugatti-veyron
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token envelope-bugatti-veyron uint)
...
(define-data-var artist-address principal 'SP2919WMGWK04VEYB7TY7S67EPAZQCN273VWXKGY8)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmX3DTjAD4Qw6zPcnBYVVVeRPtGg73bN3F5egMeg3tsX24/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? envelope-bugatti-veyron next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? envelope-bugatti-veyron token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? envelope-bugatti-veyron token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? envelope-bugatti-veyron token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? envelope-bugatti-veyron id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? envelope-bugatti-veyron id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? envelope-bugatti-veyron id) (err ERR-NOT-FOUND)))
