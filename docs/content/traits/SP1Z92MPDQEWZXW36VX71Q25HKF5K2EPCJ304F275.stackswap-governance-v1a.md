---
title: "Trait stackswap-governance-v1a"
draft: true
---
```
...
(define-read-only (is-token-accepted (token <ft-trait>))
  (let (
    (is-stsw (is-eq (contract-of token) 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.stsw-token-v1a))
...
(define-public (propose
  ...
  (let (
    (proposer-balance (unwrap-panic (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.stsw-token-v1a get-balance tx-sender)))
    (supply (- (unwrap-panic (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.stsw-token-v1a get-total-supply)) u200000000))
...
(define-private (execute-proposal-change-contract (change (tuple (name (string-ascii 256)) (address principal) (qualified-name principal) (can-mint bool) (can-burn bool))))
  (let (
    ...
    (if (not (is-eq name ""))
      (begin
        (try! (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.stackswap-dao-v1a set-contract-address name address qualified-name can-mint can-burn))
        (ok true)
      )
      (ok false)
    )
...
(define-public (add-contract-address (name (string-ascii 256)) (address principal) (qualified-name principal) (can-mint bool) (can-burn bool))
  (begin
    (asserts! (is-eq tx-sender (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.stackswap-dao-v1a get-dao-owner)) (err ERR-NOT-AUTHORIZED))

    (if (is-some (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.stackswap-dao-v1a get-contract-address-by-name name))
      (ok false)
      (begin
        (try! (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.stackswap-dao-v1a set-contract-address name address qualified-name can-mint can-burn))
        (ok true)
