---
title: "Trait keeper-3-helper-v-1-1"
draft: true
---
```
;; keeper-3-helper-v-1-1
...
(define-read-only (get-psbtc-swap-amount)
  (let (
    ...
    (pontis-stx-fee (unwrap! (contract-call? 'SP14NS8MVBRHXMM96BQY0727AJ59SWPV7RMHC0NCG.pontis-bridge-fee-manager get-btc-base-fee 0x425443) ERR_NO_PONTIS_FEE_DATA))
...
(define-public (get-action-a
  ...
  (let (
    ...
    (psbtc-quote-amount-check (asserts! (> amount-after-keeper-fee psbtc-swap-amount) ERR_INSUFFICIENT_PSBTC_AVAILABLE))
    ...
    ;; Get quote for psBTC to rune token swap via XYK Core
    (quote-psbtc-to-rune (try! (xyk-quote (- amount-after-keeper-fee psbtc-swap-amount) 'SP14NS8MVBRHXMM96BQY0727AJ59SWPV7RMHC0NCG.pontis-bridge-psBTC (get a xyk-tokens) (get a xyk-pools))))

    ;; Calculate percent fee required by Pontis based on amount
    (pontis-ft-fee (contract-call? 'SP14NS8MVBRHXMM96BQY0727AJ59SWPV7RMHC0NCG.pontis-bridge-fee-manager calculate-btc-percent-fee quote-psbtc-to-rune))
...
;; Get output for execute-action-b function
(define-public (get-action-b
    (amount uint)
    (xyk-tokens (tuple (a <xyk-ft-trait>)))
    (xyk-pools (tuple (a <xyk-pool-trait>)))
  )
  (let (
    ...
    ;; Get quote for rune token to psBTC swap via XYK Core
    ...
    ;; Calculate percent fee required by Pontis based on amount
    (pontis-ft-fee (contract-call? 'SP14NS8MVBRHXMM96BQY0727AJ59SWPV7RMHC0NCG.pontis-bridge-fee-manager calculate-btc-percent-fee (- quote-rune-to-psbtc psbtc-swap-amount)))

    ;; Calculate amount bridged out after subtracting the percent fee
    (amount-after-pontis-ft-fee (- quote-rune-to-psbtc pontis-ft-fee))
  )
    ;; Return action data
    (ok {
      amount: amount,
