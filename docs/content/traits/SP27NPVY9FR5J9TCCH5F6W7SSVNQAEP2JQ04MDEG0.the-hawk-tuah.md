---
title: "Trait the-hawk-tuah"
draft: true
---
```
(impl-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)

(define-fungible-token the-hawk-tuah)
...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://bafybeiguis2mg4qc5xsnrwy7eooiyj76ovni4jkrjruvr46en3x4krc5le.ipfs.dweb.link/")) 
...
(define-read-only (get-name) (ok "The Hawk Tuah"))
(define-read-only (get-symbol) (ok "HAWK"))
(define-read-only (get-decimals) (ok u0))
(define-read-only (get-balance (user principal)) (ok (ft-get-balance the-hawk-tuah user)))
(define-read-only (get-total-supply) (ok (ft-get-supply the-hawk-tuah)))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? the-hawk-tuah amount from to)
    )
)
...
(define-public (swap-stx-for-token (stx-amount uint) (min-tokens-out uint)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? the-hawk-tuah tokens-out (as-contract tx-sender) tx-sender))
      (if (>= (/ (* (+ (var-get real-stx-reserve) (var-get virtual-stx-reserve)) token-supply) (var-get token-reserve)) market-cap-threshold)
        (begin
          (let (
            ...
            (try! (ft-transfer? the-hawk-tuah burn-amount (as-contract tx-sender) burn-address))
            (try! (ft-transfer? the-hawk-tuah remaining-tokens (as-contract tx-sender) liquidity-builder-address))
...
(define-public (swap-token-for-stx (token-amount uint) (min-stx-out uint) (recipient principal)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? the-hawk-tuah token-amount tx-sender (as-contract tx-sender)))
...
(begin
  (try! (ft-mint? the-hawk-tuah token-supply (as-contract tx-sender)))
