...
(define-non-fungible-token gambler-boys uint)
...
(define-data-var total-price uint u1256000)
(define-data-var artist-address principal 'SP1G7DM17XAVQMVFQX6E8BPAB1KFSJ93KP3GM8N75)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmf4eaLaUxksXoyNQV7UycW1enNpCrZkSvrqkGDV8a6TiK/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? gambler-boys next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gambler-boys token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gambler-boys token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gambler-boys token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gambler-boys id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gambler-boys id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gambler-boys id) (err ERR-NOT-FOUND)))
