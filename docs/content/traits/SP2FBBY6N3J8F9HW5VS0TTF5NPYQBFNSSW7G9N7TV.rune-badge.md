...
(define-non-fungible-token rune-badge uint)
...
(define-data-var artist-address principal 'SP2FBBY6N3J8F9HW5VS0TTF5NPYQBFNSSW7G9N7TV)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeF1iGQNasbEbX2CQ5urtFxZcfPjPiGRn5ERhTQw8aMpJ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? rune-badge next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? rune-badge token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? rune-badge token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? rune-badge token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rune-badge id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rune-badge id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rune-badge id) (err ERR-NOT-FOUND)))
