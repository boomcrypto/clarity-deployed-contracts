---
title: "Trait ordinals-cat-phunks"
draft: true
---
```
;; ordinals-cat-phunks
...
(define-non-fungible-token ordinals-cat-phunks uint)
...
;; Internal variables
(define-data-var mint-limit uint u802)
(define-data-var last-id uint u1)
(define-data-var total-price uint u15000000)
(define-data-var artist-address principal 'SP16G38CAZQSVK5TW87K8ZH9G7XK5NBNA98RAGGP2)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXooC6b6iftNuJQodS3mapWmiuhha9nd64N36ow7nCXXL/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinals-cat-phunks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinals-cat-phunks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinals-cat-phunks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinals-cat-phunks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinals-cat-phunks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinals-cat-phunks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinals-cat-phunks id) (err ERR-NOT-FOUND)))
