---
title: "Trait woo-meme-world-champion"
draft: true
---
```
;; Title: Woo! Meme World Champion
;; Author: rozar.btc
...
(define-fungible-token woo)

(define-data-var token-name (string-ascii 32) "Woo! Meme World Champion")
(define-data-var token-symbol (string-ascii 10) "WOO")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/woo.json"))
...
(define-public (craft (amount uint) (recipient principal) (lft-a <liquid-ft-trait>) (lft-b <liquid-ft-trait>))
    (let
        (
            ...
            (craft-fee-a (/ (* craft-fee (get-exchange-rate lft-a)) ONE_6))
            (craft-fee-b (/ (* craft-fee (get-exchange-rate lft-b)) ONE_6))
        )
		(try! (is-dao-or-extension))
        ;; if craft-fee is greater than 0 then burn base tokens
        (and (> craft-fee u0) 
            (begin
                (print {craft-fee: craft-fee, craft-fee-a: craft-fee-a, craft-fee-b: craft-fee-b})
                (try! (contract-call? lft-a deflate craft-fee-a))
                (try! (contract-call? lft-b deflate craft-fee-b))
            )
        )
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(let
		...
		(ft-transfer? woo amount-after-fee sender recipient)
	)
)
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance woo who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply woo))
)
...
(define-private (get-exchange-rate (lft <liquid-ft-trait>))
	(/ (* (get-total-in-pool lft) ONE_6) (ft-get-supply woo))
)
...
(define-private (join (amount uint) (recipient principal) (lft-a <liquid-ft-trait>) (lft-b <liquid-ft-trait>))
    (let
        ...
        (try! (ft-mint? woo amount recipient))
        (ok true)
    )
)
...
(define-private (split (amount uint) (recipient principal) (lft-a <liquid-ft-trait>) (lft-b <liquid-ft-trait>))
    (let
        ...
        (print {exchange-rate-a: (get-exchange-rate lft-a), exchange-rate-b: (get-exchange-rate lft-b)})
        (try! (ft-burn? woo amount tx-sender))
