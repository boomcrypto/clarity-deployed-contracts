---
title: "Trait ststxbtc-withdraw-nft"
draft: true
---
```
;; @contract stSTXbtc withdraw NFT
;; @version 1
;;
;; To convert stSTXbtc back into STX, a user must wait until the ongoing stacking cycle ends.
;; When initiating a withdrawal, the stSTXbtc tokens are already taken, while the user has not yet received STX.
;; That's why this NFT is introduced, so the user has a token representation of the withdrawal initiation.

(impl-trait .nft-trait.nft-trait)
(use-trait commission-trait .commission-trait.commission)

(define-non-fungible-token ststxbtc-withdraw uint)
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ststxbtc-withdraw token-id))
)
...
(define-public (mint-for-protocol (recipient principal))
  (let (
    ...
    (try! (nft-mint? ststxbtc-withdraw (var-get last-id) recipient))
...
(define-public (burn-for-protocol (token-id uint))
  (let (
    ...
    (try! (nft-burn? ststxbtc-withdraw token-id owner))
...
(define-private (is-sender-owner (id uint))
  (let (
    (owner (unwrap! (nft-get-owner? ststxbtc-withdraw id) false))
  )
...
(define-public (buy-in-ustx (id uint) (commission-contract <commission-trait>))
  (let (
    (owner (unwrap! (nft-get-owner? ststxbtc-withdraw id) (err ERR_NFT_NOT_FOUND)))
...
(define-private (transfer-helper (id uint) (sender principal) (recipient principal))
  (begin
    (try! (nft-transfer? ststxbtc-withdraw id sender recipient))
