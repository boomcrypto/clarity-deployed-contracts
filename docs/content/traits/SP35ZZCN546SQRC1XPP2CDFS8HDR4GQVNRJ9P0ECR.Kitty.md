...
(define-read-only (get-balance (address principal))
  (ok (ft-get-balance Kitty address)))

(define-read-only (get-total-supply)
  (ok (ft-get-supply Kitty)))
...
(define-read-only (get-token-uri)
  (ok (some u"https://ipfs.io/ipfs/QmX6TsG3fCQrd7wGRt28NKWBuh8nw1HJ43eJGsB987KLAa")))
...
(define-public (transfer (amount uint) (sender principal) (receiver principal) (memo (optional (buff 34))))
  (begin
        ...
        (try! (ft-transfer? Kitty amount sender receiver))
...
(define-public (burn (count uint))
    (ft-burn? Kitty count tx-sender))

(begin
  (map-set receiver-users tx-sender true)
  (try! (ft-mint? Kitty u10000000000000000 tx-sender))
)
```
