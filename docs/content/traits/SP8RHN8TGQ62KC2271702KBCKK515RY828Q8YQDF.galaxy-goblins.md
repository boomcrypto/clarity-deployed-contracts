...
(define-non-fungible-token galaxy-goblins uint)
...
(define-data-var artist-address principal 'SP8RHN8TGQ62KC2271702KBCKK515RY828Q8YQDF)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmS9WYD4kEtVsJu8KG6Pi95nszeyJnCu8H8ii5DDbZVXEQ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? galaxy-goblins next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? galaxy-goblins token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? galaxy-goblins token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? galaxy-goblins token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? galaxy-goblins id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? galaxy-goblins id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? galaxy-goblins id) (err ERR-NOT-FOUND)))
