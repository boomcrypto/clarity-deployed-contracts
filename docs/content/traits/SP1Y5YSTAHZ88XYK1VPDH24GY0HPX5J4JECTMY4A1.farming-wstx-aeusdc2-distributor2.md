---
title: "Trait farming-wstx-aeusdc2-distributor2"
draft: true
---
```
...
(define-public
  ...
  (let ((epoch (contract-call? .farming-wstx-aeusdc2-core current-epoch))
...
(define-read-only
  ...
  (let ((share   (try! (contract-call? .farming-wstx-aeusdc2-core get-share-at user epoch)))
...
(define-read-only (mkepochs (start-epoch uint))
  (let ((current (contract-call? .farming-wstx-aeusdc2-core current-epoch))
