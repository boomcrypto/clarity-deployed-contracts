...
(define-public (swap-x-for-y (amountIn uint))
  (let (
    ...
    (b2 (unwrap-panic (contract-call? .test-hello-world-v4 swap-usda-diko b1)))
    (b3 (unwrap-panic (contract-call? .test-hello-world-v4 swap-diko-wstx b2)))
  )
    (begin
...
(define-public (swap-y-for-x (amountIn uint))
  (let (
    ...
    (b2 (unwrap-panic (contract-call? .test-hello-world-v4 swap-diko-usda b1)))
    (b3 (unwrap-panic (contract-call? .test-hello-world-v4 swap-usda-wstx b2)))
  )
    (begin
      (asserts! (> b3 amountIn) (err ERR-COOLDOWN))
      (asserts! (is-eq CONTRACT-OWNER tx-sender) (err ERR-ALL-MINTED))
