...
(define-non-fungible-token orange-pill uint)
...
(define-data-var artist-address principal 'SP1T4T19MA76MBESFM3QJAYY6QZHSP9Q525VGTS8J)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPaGLRxYXkLSPqPzE12U85iEV2eLBZyZov32XPrRCsyRc/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? orange-pill next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? orange-pill token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? orange-pill token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? orange-pill token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? orange-pill id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? orange-pill id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? orange-pill id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u34096)
