---
title: "Trait neoswap-sc-1685998835370"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/PPcFE2XQA20DyIbnJ9nI
;; SwapId: BYdz1VOe5X8QDaGpmVsV

;; traders
(define-constant TRADER-1 'SP15AQJRCXFD3BMEPK682XC99HRVYSG8F4N9G8T44)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-robot-expansion-nft transfer u1895 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-robot-expansion-nft transfer u1895 tx-sender TRADER-1)))

	(unwrap-panic (as-contract (stx-transfer? u4500000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u85500000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u90000000 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-robot-expansion-nft transfer u1895 tx-sender TRADER-2)))
	    )
            true
        )
