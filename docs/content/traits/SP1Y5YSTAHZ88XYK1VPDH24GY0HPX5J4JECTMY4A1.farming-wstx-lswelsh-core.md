---
title: "Trait farming-wstx-lswelsh-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-lswelsh)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-lswelsh transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-lswelsh get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-lswelsh transfer amt protocol user none)))
