---
title: "Trait ordamoosement-park"
draft: true
---
```
;; ordamoosement-park
...
(define-non-fungible-token ordamoosement-park uint)
...
(define-data-var artist-address principal 'SP2TKS065J48YZDRE76YP2E3XH6A5VY298Y9ZMNBR)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmT5AbZcu9JTSEJf2wxuMKAky2cm6WF8zZtuJoXHWh5uRe/json/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u5000)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordamoosement-park next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordamoosement-park token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordamoosement-park token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordamoosement-park token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordamoosement-park id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordamoosement-park id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordamoosement-park id) (err ERR-NOT-FOUND)))
