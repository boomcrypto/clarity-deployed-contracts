---
title: "Trait meme-token"
draft: true
---
```
;; $MEME (https://lasereye.vip/#/x/tokens)
;; Total: 2,100,000,000,000,000 Distribution as below:
;; Airdrop: 72,000,000,000,000 (for current 18 members, each get 4,000,000,000,000)
;; Members: 800,000,000,000,000 (first 200 members can claim, each get 4,000,000,000,000)
;; BNS: 1,228,000,000,000,000 (first 61,400 names, each get 20,000,000,000. Only available for .btc, .id(length<=2), .stx(length<=2), .app(length<=2), .stacks(length<=2))
...
(define-constant DECIMAL u1)
(define-constant ONE_COIN (pow u10 DECIMAL))
(define-constant MAX_SUPPLY (* u2100000000000000 ONE_COIN))
(define-constant AIRDROP_COUNT_PER_MEMBER (* u4000000000000 ONE_COIN))
(define-constant CLAIM_COUNT_PER_MEMBER (* u4000000000000 ONE_COIN))
(define-constant MAX_CLAIM_MEMBER_COUNT u200)
(define-constant CLAIM_COUNT_PER_BNS (* u20000000000 ONE_COIN))    
(define-constant MAX_CLAIM_BNS_COUNT u61400)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (if (is-eq tx-sender sender)
    (begin
      (try! (ft-transfer? MEME amount sender recipient))
      (print memo)
      (ok true)
    )
    (err u4)
...
(define-public (get-token-uri)
  (ok (some u"https://ipfs.io/ipfs/QmSMgLSZR4pQXbBL2TEGmWJ9JGpEbaKqDAqTB27TAHrRTU"))
)
...
(define-public (member_claim)
  (let
    ...
    (ft-mint? MEME CLAIM_COUNT_PER_MEMBER tx-sender)
  )
)
...
(define-public (bns_claim)
  (let
    ...
    (ft-mint? MEME CLAIM_COUNT_PER_BNS tx-sender)
  )
)
...
(define-private (airdrop (tid uint))
  (match (contract-call? .laser-eyes-v5 get_player_by_id tid) user
    (is-ok (ft-mint? MEME AIRDROP_COUNT_PER_MEMBER user))
    false
  )
)

;; Airdrop to early members
