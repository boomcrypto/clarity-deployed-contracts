...
(define-non-fungible-token the-xrp-anthem uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeriGSRd9M8QBuoaAmGmSYjVAmGGG2Fz18KKggxAKhQk6/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? the-xrp-anthem next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-xrp-anthem token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-xrp-anthem token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-xrp-anthem token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-xrp-anthem id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-xrp-anthem id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-xrp-anthem id) (err ERR-NOT-FOUND)))
