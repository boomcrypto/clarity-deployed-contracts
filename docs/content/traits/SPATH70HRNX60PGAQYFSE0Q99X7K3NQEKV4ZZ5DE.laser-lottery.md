...
(define-constant ERR_TRANSFER_STX 1004)
...
(define-constant ORDI_PRICE_PER_BET u5000000)     ;; 0.05 $ORDI/BET
(define-constant BET_PRICE_NORMAL u150000)        ;; Non-member 0.15 STX/BET
(define-constant BET_PRICE_FOR_MEMBER u100000)    ;; Member 0.1 STX/BET
(define-constant CALL_END_AT_REWARD u50000)       ;; Reward 0.05 STX to whom clicks end-bet button
(define-constant CALL_DRAW_REWARD u100000)        ;; Reward 0.1 STX to whom clicks draw button
...
(define-constant BET_BLOCKS u2)                   ;; 1) 72 blocks for user to bet(about 12 hours)
(define-constant WAIT_TO_END_BET_BLOCKS u2)       ;; 2) Wait for 6 blocks(do nothing), then user can call end_bet to enter wait-for-draw state
(define-constant WAIT_TO_DRAW_BLOCKS u2)         ;; 3) Wait 17 blocks(6 + 5 + 6), then user can call draw
(define-constant BYTE_INDEX_FOR_GEN_JACKPOT u25)  ;; use the header-hash's 25th byte to generate jockpot
(define-constant RETURN_NAME_MIN_BLOCK u20)   ;; Can get back name after this block
(define-constant RETURN_NAME_MAX_ROUND_BLOCKS u10)  ;; If round still not over after 1000 blocks since start, founder can get back name
(define-constant RETURN_NAME_ADDRESS 'SP3ZZS16ERHM41WBCS5H0VNMMAWSWPKWRT3J8M4BV)  ;;Account 4
(define-constant FOUNDER 'SP3KBNJR3CX64CDAB81M00ZXRZTDP3BZS4TH5A159)  ;;Account 5
...
(define-public (bet (bet_list (list 200 (string-ascii 5))))
  (let
    (
      ...
      (tid (default-to u0 (contract-call? 'SP3XYJ8XFZYF7MD86QQ5EF8HBVHHZFFQ9HM6SPJNQ.laser-eyes-v5 get_id_by_player caller)))
      (stx_cost (* item_count (if (> tid u0) BET_PRICE_FOR_MEMBER BET_PRICE_NORMAL)))
    ...
    (if (is-eq round_bet_count u0)
      (begin
        (unwrap! (stx-transfer? (+ CALL_END_AT_REWARD CALL_DRAW_REWARD) caller (as-contract tx-sender)) (err ERR_TRANSFER_STX)) ;; one for end_bet, one for draw
        (unwrap! (stx-transfer? (- stx_cost CALL_END_AT_REWARD CALL_DRAW_REWARD) caller FOUNDER) (err ERR_TRANSFER_STX))
      )
      (unwrap! (stx-transfer? stx_cost caller FOUNDER) (err ERR_TRANSFER_STX))
    )
    (unwrap! (contract-call? 'SP3XYJ8XFZYF7MD86QQ5EF8HBVHHZFFQ9HM6SPJNQ.ordi transfer ordi_cost caller FOUNDER none) (err ERR_TRANSFER_ORDI))
...
(define-public (end_bet (btc_height uint))
  (let
    ...
    (as-contract (unwrap! (stx-transfer? CALL_END_AT_REWARD tx-sender caller) (err ERR_TRANSFER_STX)))
    (ok true)
  )
)
...
(define-public (draw)
  (let
    ...
    (as-contract (unwrap! (stx-transfer? CALL_DRAW_REWARD tx-sender caller) (err ERR_TRANSFER_STX)))
    (print {
      title: "draw",
      jackpot: jackpot,
      winners: winners,
    ...
    (if (> (len winners) u0)
      (begin
        (try! (contract-call? .laser-lottery-prize set_winners winners))
...
;; Can return name in several situations:
;; 1. block-height exceed RETURN_NAME_MIN_BLOCK, and in the idle state
;; 2. 1000 blocks passed since round-start block. Probably due to bug, allow FOUNDER to get back name to rescue.
(define-public (return_name)
  (if (is-eq (var-get m_state) STATE_HAS_WINNER)
    ...
    (if (is-eq (var-get m_state) STATE_IDLE)
      (if (>= block-height RETURN_NAME_MIN_BLOCK)
        (begin
          ...
          (contract-call? .laser-lottery-prize return_name RETURN_NAME_ADDRESS)
        )
        (err ERROR_RETURN_NAME)
      )
      (if (>= block-height (+ (var-get m_stx_start_at) RETURN_NAME_MAX_ROUND_BLOCKS)) ;; stucked
        (begin
          (var-set m_state STATE_FREEZED)
          (contract-call? .laser-lottery-prize return_name RETURN_NAME_ADDRESS)
        )
...
(define-read-only (get_summary_with_user (user principal))
  {
    base: (get_summary),
    tid: (contract-call? 'SP3XYJ8XFZYF7MD86QQ5EF8HBVHHZFFQ9HM6SPJNQ.laser-eyes-v5 get_id_by_player user),
    stx: (stx-get-balance user),
    ordi: (unwrap-panic (contract-call? 'SP3XYJ8XFZYF7MD86QQ5EF8HBVHHZFFQ9HM6SPJNQ.ordi get-balance user)),
...
(contract-call? .laser-lottery-prize set_lottery_contract (as-contract tx-sender))

(define-read-only (get_info (height uint))
  {
    btc_end_at: (fold f LIST_13 (+ height u6)),   ;; search the latest bitcoin height in btc_height + 6 => btc_height - 6
    height: height, 
