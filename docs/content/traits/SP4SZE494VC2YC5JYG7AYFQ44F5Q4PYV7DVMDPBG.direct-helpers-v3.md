...
(define-constant LIST_INDICES_50 
...
(define-public (get-user-balance-in-protocol (user principal) (protocol <protocol-trait>) (index uint)) 
  (let (
    ...
    (asserts! (is-eq index (unwrap-panic protocol-index)) (err ERR_UNKNOWN_PROTOCOL))
...
(define-public (calculate-direct-stacking-info (reserve <reserve-trait>) (protocols (list 50 <protocol-trait>)) (user principal)) 
  (let (
    ...
    (ratio (try! (contract-call? .data-core-v2 get-stx-per-ststx reserve)))
    ...
    (protocol-balances (map get-user-balance-in-protocol user-list protocols LIST_INDICES_50))
    ...
    (asserts! (is-eq (len supported-protocols) (len protocols)) (err ERR_UNKNOWN_PROTOCOL))
    ...
    (let (
      ...
      (balance-ststx (+ wallet-ststx protocol-ststx))
      (balance-stx (/ (* balance-ststx ratio) DENOMINATOR_6))
    )
      (ok { direct-stacking-stx: direct-stacking, direct-stacking-ststx: direct-stacking-ststx, balance-stx: balance-stx, balance-ststx: balance-ststx })
    )
  )
)
...
(define-public (update-direct-stacking (reserve <reserve-trait>) (protocols (list 50 <protocol-trait>)) (user principal))
  (let (
    ...
    (balance-stx (get balance-stx info))

    (diff (if (> stacking-stx balance-stx)
      (- stacking-stx balance-stx)
      u0
    ))
  )
