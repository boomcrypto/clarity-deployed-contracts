...
(define-non-fungible-token obama-ordinals uint)
...
(define-data-var total-price uint u38000000)
(define-data-var artist-address principal 'SPHHWD9EVGQ3H5PDFX41EDNH0FPTBZRCN7RNAT49)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUsUmoJjJtrsQgXAdo7yoESwBiHdUgR8hdBvG95maaDEF/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? obama-ordinals next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? obama-ordinals token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? obama-ordinals token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? obama-ordinals token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? obama-ordinals id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? obama-ordinals id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? obama-ordinals id) (err ERR-NOT-FOUND)))
