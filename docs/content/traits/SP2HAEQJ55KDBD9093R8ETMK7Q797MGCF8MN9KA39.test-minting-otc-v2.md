---
title: "Trait test-minting-otc-v2"
draft: true
---
```
...
(define-public (claim-unconfirmed-redeem (request-id (string-ascii 36)))
  (let (
    ...
    (try! (contract-call? .test-usdh-token-final transfer (get amount-usdh redeem-request) this-contract requester none))
...
(define-public (confirm-mint (request-id (string-ascii 36)) (requester principal) (amount-asset uint) (price uint))
  (let (
    ...
    (if (> amount-usdh-fee u0) (try! (contract-call? .test-usdh-token-final mint-for-protocol amount-usdh-fee (get fee-address state))) true)
...
(define-public (confirm-redeem (request-id (string-ascii 36)) (price  uint) (amount-usdh-confirmed uint))
  (let (
    ...
    (try! (contract-call? .test-usdh-token-final burn-for-protocol amount-usdh-confirmed-minus-fee this-contract))
    (if (> amount-usdh-fee u0) (try! (contract-call? .test-usdh-token-final transfer amount-usdh-fee this-contract (get fee-address state) none)) true)
    (if (not (is-eq amount-usdh-requested amount-usdh-confirmed))
      (try! (contract-call? .test-usdh-token-final transfer (- amount-usdh-requested amount-usdh-confirmed) this-contract (get requester redeem-request) none))
      true
    )
