---
title: "Trait neoswap-sc-1697824808197"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/VaZn0G1VhJkqHwVAjJY1
;; SwapId: xqeWkccUYiCSHSASbOCQ
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u32760000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SPJJYJVZ4H7B34GG8D3SSN70WVWDYSHCC9E9ZV4V.bitcoin-toadz transfer u1362 tx-sender TRADER-2)))
	...
	(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-satoshi-knights transfer u78 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u31122000 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (stx-transfer? u1638000 tx-sender RECEIVER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
		...
		(unwrap-panic (as-contract (contract-call? 'SP2J4TG4H9KYMTTG267KMR82JDM20WHET6XNR0PWQ.bitcoin-owlz transfer u1539 tx-sender TRADER-1)))
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u32760000 tx-sender TRADER-2)))
	    )
            true
        )
