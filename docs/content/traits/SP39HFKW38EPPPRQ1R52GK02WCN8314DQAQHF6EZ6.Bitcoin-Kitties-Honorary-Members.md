---
title: "Trait Bitcoin-Kitties-Honorary-Members"
draft: true
---
```
...
(define-non-fungible-token Bitcoin-Kitties-Honorary-Members uint)
...
(define-data-var ipfs-full-metadata (string-ascii 85) "https://gateway.pinata.cloud/ipfs/QmU7Nsu7ChdBqSvkt9fWaovS1HJkDnvs8EinQMLatASFSR")
(define-data-var ipfs-root (string-ascii 120) "https://gateway.pinata.cloud/ipfs/QmeN9HGojrgL5ZEb3gDwkYdihLZGrpRfah9NkAZVyJXmgT/bitcoin_kitties_honorary_")
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint?  Bitcoin-Kitties-Honorary-Members next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer?  Bitcoin-Kitties-Honorary-Members token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner?  Bitcoin-Kitties-Honorary-Members token-id)))
