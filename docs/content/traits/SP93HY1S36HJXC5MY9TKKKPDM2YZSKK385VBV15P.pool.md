...
(define-private (swap-single (ti principal) (to principal) (logic <pair-logic-trait>) (amount-in uint) (context {amount-out: uint, user: principal, global: {fee: uint, pfee: uint, block-height-provider: principal}, token: <asset-wrapper-trait>}))
  (let
    ...
    (print {event: "swap", maker: (get user context), tkn-in: ti, tkn-out: to, logic: (contract-of logic), amount-in: amount-in, amount-out: amount-out, lpfee: (- fee pfee), pfee: pfee, reserve-in: (+ ri amount-in), reserve-out: (- ro amount-out)})
    (ok amount-out)))
