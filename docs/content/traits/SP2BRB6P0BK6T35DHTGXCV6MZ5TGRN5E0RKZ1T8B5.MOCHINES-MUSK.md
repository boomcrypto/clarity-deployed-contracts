...
(define-non-fungible-token Musk uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafkreid34gfaipi42iflei567jtdj47nbviu5dwx6xxtqbx5dikd6hmjou")
...
(define-public (mint (recipient principal))
    (begin 
        ...
        (asserts! (< (var-get last-id) SUPPLY) (err ERR-NO-MORE-NFTS))
            ...
            (try! (nft-mint? Musk u1 tx-sender))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Musk token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Musk token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Musk token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Musk id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Musk id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Musk id) (err ERR-NOT-FOUND)))
