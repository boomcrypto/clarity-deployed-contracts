---
title: "Trait neoswap-sc-1682190038659"
draft: true
---
```
;; multiway-swap
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u18900000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP3X1CG6HJQWTHZ85DQVES9WMSGCNYASCZ20VVGPW.banana-ape-town transfer u3 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP3X1CG6HJQWTHZ85DQVES9WMSGCNYASCZ20VVGPW.banana-ape-town transfer u3 tx-sender TRADER-1)))

	(unwrap-panic (as-contract (stx-transfer? u945000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u17955000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u18900000 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP3X1CG6HJQWTHZ85DQVES9WMSGCNYASCZ20VVGPW.banana-ape-town transfer u3 tx-sender TRADER-2)))
	    )
            true
        )
