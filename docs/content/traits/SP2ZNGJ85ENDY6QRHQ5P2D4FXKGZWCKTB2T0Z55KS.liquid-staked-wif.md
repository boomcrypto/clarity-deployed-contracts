---
title: "Trait liquid-staked-wif"
draft: true
---
```
;; Title: Liquid Staked Wif
;; Author: rozar.btc
;; Synopsis:
;; This contract implements a liquid staking solution for Wif.
;; It provides users with liquid tokens (sWIF) that represent staked Wif. 
...
(define-data-var token-name (string-ascii 32) "Liquid Staked Dogwifhat")
(define-data-var token-symbol (string-ascii 10) "sWIF")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/liquid-staked-wif.json"))
...
(define-public (stake (amount uint))
	(begin
		(let
			...
			(try! (contract-call? 'SP3WPNAEBYMX06RQNNYTH5PTJ1FRGX5A13ZZMZ01D.dogwifhat-token transfer amount sender contract none))
...
(define-public (unstake (amount uint))
	(begin
		(let
			...
			(try! (as-contract (contract-call? 'SP3WPNAEBYMX06RQNNYTH5PTJ1FRGX5A13ZZMZ01D.dogwifhat-token transfer amount-token contract sender none)))
		)
		(ok true)
	)
)
...
(define-public (deposit (amount uint))
    (contract-call? 'SP3WPNAEBYMX06RQNNYTH5PTJ1FRGX5A13ZZMZ01D.dogwifhat-token transfer amount tx-sender contract none)
)
...
(define-read-only (get-total-in-pool)
	(unwrap-panic (contract-call? 'SP3WPNAEBYMX06RQNNYTH5PTJ1FRGX5A13ZZMZ01D.dogwifhat-token get-balance contract))
)
