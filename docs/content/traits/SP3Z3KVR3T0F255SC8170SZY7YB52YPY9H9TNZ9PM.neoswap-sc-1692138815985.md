---
title: "Trait neoswap-sc-1692138815985"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/4Lqkx7JmS0OoA95MeiAS
;; SwapId: vyzqLAfxosNMxIawVFxT
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u4038462 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u4038462 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u4038462 tx-sender TRADER-2)))
	    )
            true
        )
