---
title: "Trait liquid-staked-not"
draft: true
---
```
;; Title: Liquid Staked Not
;; Author: rozar.btc
;; Synopsis:
;; This contract implements a liquid staking solution for Not.
;; It provides users with liquid tokens (sNOT) that represent staked Not. 
...
(define-data-var token-name (string-ascii 32) "Liquid Staked Nothing")
(define-data-var token-symbol (string-ascii 10) "sNOT")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/liquid-staked-not.json"))
...
(define-public (unstake (amount uint))
	(begin
		(let
			...
			(try! (as-contract (contract-call? 'SP32AEEF6WW5Y0NMJ1S8SBSZDAY8R5J32NBZFPKKZ.nope transfer amount-token contract sender none)))
		)
		(ok true)
	)
)
...
(define-public (deposit (amount uint))
    (contract-call? 'SP32AEEF6WW5Y0NMJ1S8SBSZDAY8R5J32NBZFPKKZ.nope transfer amount tx-sender contract none)
)
...
(define-read-only (get-total-in-pool)
	(unwrap-panic (contract-call? 'SP32AEEF6WW5Y0NMJ1S8SBSZDAY8R5J32NBZFPKKZ.nope get-balance contract))
)
