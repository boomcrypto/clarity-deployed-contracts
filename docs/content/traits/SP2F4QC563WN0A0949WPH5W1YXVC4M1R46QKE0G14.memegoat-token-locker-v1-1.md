---
title: "Trait memegoat-token-locker-v1-1"
draft: true
---
```
...
(define-public 
  ...
  (begin     
    ...
    (let 
      ...
      (if fee-in-stx
        ;; Pay fee in STX
        (try! (stx-transfer? stxfee tx-sender .memegoat-locker-vault-v1-1))
        ;; Burn token
        (begin
      ...
      (try! (contract-call? locked-token transfer total-amount sender .memegoat-locker-vault-v1-1 none))
...
;; relockToken
(define-public (relock-token (index uint) (new-unlock-block uint) (fee-in-stx bool) (secondary-token-trait <ft-trait-ext>) ) 
  (begin 
    (let
      (
        (sender tx-sender)
...
(define-public (withdraw-token (locked-token <ft-trait>) (index uint)) 
  (begin
    (let
      ...
      (as-contract (try! (contract-call? .memegoat-locker-vault-v1-1 transfer-ft locked-token unlock-amount withdrawal-address)))
...
(define-public (increment-lock (locked-token <ft-trait>) (index uint) (amount uint)) 
  (begin 
    ...
    (let
      ...
      ;; transfer token to vault
      (try! (contract-call? locked-token transfer amount sender .memegoat-locker-vault-v1-1 none))
