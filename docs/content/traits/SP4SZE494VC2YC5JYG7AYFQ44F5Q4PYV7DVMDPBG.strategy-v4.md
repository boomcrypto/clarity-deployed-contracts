...
(define-constant ERR_UNAUTHORISED u40001)
...
(define-public (perform-pool-delegation (pool principal) (delegates-info (list 10 { delegate: <stacking-delegate-trait>, amount: uint})))
  (let (
    ...
    (asserts! (is-eq contract-caller (get-manager)) (err ERR_UNAUTHORISED))
...
(define-private (perform-pool-delegation-helper (delegate-info { delegate: <stacking-delegate-trait>, amount: uint}) (delegate-to principal) (until-burn-ht uint))
  (let (
    ...
    (if (is-eq delegate-amount u0)
      (try! (contract-call? .delegates-handler-v1 revoke delegate-contract .reserve-v1 .rewards-v2))
      (try! (contract-call? .delegates-handler-v1 revoke-and-delegate delegate-contract .reserve-v1 .rewards-v2 delegate-amount delegate-to until-burn-ht))
    )
