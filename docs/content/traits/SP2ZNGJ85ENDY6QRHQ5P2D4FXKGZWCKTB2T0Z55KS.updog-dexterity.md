---
title: "Trait updog-dexterity"
draft: true
---
```
;; Updog - LP Token, AMM DEX and Hold-to-Earn Engine
...
(define-data-var swap-fee uint u91028) ;; Default to 9.1%
(define-data-var token-uri (optional (string-utf8 256)) 
  (some u"https://charisma.rocks/api/v0/indexes/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.updog-dexterity"))
...
(define-public (swap (forward bool) (amt-in uint))
  (let (
    ...
    (reserve-in (unwrap-panic (if forward (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token get-balance CONTRACT) 
      (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.runes-dog get-balance CONTRACT))))
    (reserve-out (unwrap-panic (if forward (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.runes-dog get-balance CONTRACT) 
    ...
    (try! (if forward (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer amt-in sender CONTRACT none) 
      (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.runes-dog transfer amt-in sender CONTRACT none)))
    (try! (as-contract (if forward (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.runes-dog transfer amt-out CONTRACT sender none)
...
(define-public (mint (who principal) (amount uint))
  (let (
    ...
    (reserve1 (unwrap-panic (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.runes-dog get-balance CONTRACT)))
    ...
    (try! (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.runes-dog transfer token1-amount who CONTRACT none))
...
(define-public (burn (who principal) (amount uint))
  (let (
    ...
    (try! (as-contract (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.runes-dog transfer token1-amount CONTRACT who none)))
    (ok {token0-amount: token0-amount, token1-amount: token1-amount, lp-amount: amount})))
...
(define-read-only (get-tokens)
  (ok {token0: 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token, token1: 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.runes-dog}))
...
(define-read-only (get-reserves)
  (ok {
    ...
    token1: (unwrap-panic (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.runes-dog get-balance CONTRACT))
  }))
