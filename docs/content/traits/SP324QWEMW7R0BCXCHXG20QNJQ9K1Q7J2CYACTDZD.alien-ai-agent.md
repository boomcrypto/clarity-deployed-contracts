...
(define-non-fungible-token alien-ai-agent uint)
...
;; Internal variables
(define-data-var mint-limit uint u126)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPL5vu5poiF3Ea1euGzLZrbMLvh7SHuQp7bfhTE21MDze/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? alien-ai-agent next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? alien-ai-agent token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? alien-ai-agent token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? alien-ai-agent token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? alien-ai-agent id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? alien-ai-agent id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? alien-ai-agent id) (err ERR-NOT-FOUND)))
