---
title: "Trait liquid-staked-max"
draft: true
---
```
;; Title: Liquid Staked Max
;; Author: rozar.btc
;; Synopsis:
;; This contract implements a liquid staking solution for Max.
;; It provides users with liquid tokens (sMAX) that represent staked Max. 
...
(define-data-var token-name (string-ascii 32) "Liquid Staked Max")
(define-data-var token-symbol (string-ascii 10) "sMAX")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/liquid-staked-max.json"))
...
(define-public (stake (amount uint))
	(begin
		(let
			...
			(try! (contract-call? 'SP7V1SE7EA3ZG3QTWSBA2AAG8SRHEYJ06EBBD1J2.max-token transfer amount sender contract none))
...
(define-public (unstake (amount uint))
	(begin
		(let
			...
			(try! (as-contract (contract-call? 'SP7V1SE7EA3ZG3QTWSBA2AAG8SRHEYJ06EBBD1J2.max-token transfer amount-token contract sender none)))
		)
		(ok true)
	)
)
...
(define-public (deposit (amount uint))
    (contract-call? 'SP7V1SE7EA3ZG3QTWSBA2AAG8SRHEYJ06EBBD1J2.max-token transfer amount tx-sender contract none)
)
...
(define-read-only (get-total-in-pool)
	(unwrap-panic (contract-call? 'SP7V1SE7EA3ZG3QTWSBA2AAG8SRHEYJ06EBBD1J2.max-token get-balance contract))
)
