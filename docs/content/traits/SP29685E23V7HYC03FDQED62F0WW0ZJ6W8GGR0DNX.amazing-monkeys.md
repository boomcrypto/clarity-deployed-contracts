...
(define-non-fungible-token amazing-monkeys uint)
...
(define-data-var artist-address principal 'SP29685E23V7HYC03FDQED62F0WW0ZJ6W8GGR0DNX)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcFQWNV59wsfvuJWU22DTX5c5E6ks4WKvbKPNaC4cy4c3/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? amazing-monkeys next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? amazing-monkeys token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? amazing-monkeys token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? amazing-monkeys token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? amazing-monkeys id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? amazing-monkeys id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? amazing-monkeys id) (err ERR-NOT-FOUND)))
