...
(define-non-fungible-token crow-squad uint)
...
(define-data-var artist-address principal 'SP3NWB8X5VKD6E0W8NJDZK8PNNENWDGQV8CQWRXPC)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbiaUSwmvXhSk77m1EuuTGF3xNnCHbcCaVqX5kNj6HRfY/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? crow-squad next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? crow-squad token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? crow-squad token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? crow-squad token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? crow-squad id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? crow-squad id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? crow-squad id) (err ERR-NOT-FOUND)))
