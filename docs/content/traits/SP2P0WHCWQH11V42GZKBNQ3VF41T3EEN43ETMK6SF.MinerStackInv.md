...
(define-non-fungible-token MinerStackInv uint)
...
(define-data-var base-uri (string-ascii 80) "bafybeiezaasub3sklvclzyzso7mpkn4tnlzs3fvpmfqfyqohh4wj4k5qcm/")
(define-data-var mint-price uint u70000000)
;; Percent u100 = 1%
(define-data-var royalty-percent uint u500)
(define-data-var mint-limit uint u3)
(define-data-var max-supply uint u251)
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? MinerStackInv next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? MinerStackInv next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? MinerStackInv id sender recipient)
        success
          (let
            (
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? MinerStackInv id))
)
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? MinerStackInv id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? MinerStackInv id) ERR_NO_OWNER))
