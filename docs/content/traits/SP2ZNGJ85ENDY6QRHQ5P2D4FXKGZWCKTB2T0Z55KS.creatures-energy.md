---
title: "Trait creatures-energy"
draft: true
---
```
(use-trait sip10 .creatures.sip010-transferable-trait)
...
(define-public (recruit (amount uint) (sip10-contract <sip10>))
    (let
        ...
        (try! (contract-call? .creatures summon amount sip10-contract))
        (ok stored-out)
    )
)
...
(define-public (dismiss (amount uint) (sip10-contract <sip10>))
    (let
        ...
        (try! (contract-call? .creatures unsummon amount sip10-contract))
        (ok stored-out)
    )
)
...
(define-public (tap (creature-id uint))
    (let
        (
            ...
            (creature-amount (unwrap-panic (contract-call? .creatures get-balance creature-id tx-sender)))
...
(define-public (store (sip10-contract <sip10>))
    (let
        (
            (creature-id (try! (contract-call? .creatures get-or-create-creature-id sip10-contract)))
...
(define-read-only (get-energy-per-block (creature-id uint) (user principal))
    (let
        (
            (users-creatures (unwrap-panic (contract-call? .creatures get-balance creature-id user)))
            (creatures-power (contract-call? .creatures get-creature-power creature-id))
        )
