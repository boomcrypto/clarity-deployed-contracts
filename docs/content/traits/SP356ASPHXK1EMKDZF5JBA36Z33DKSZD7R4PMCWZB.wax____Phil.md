...
```
    ...
    (define-non-fungible-token wax____Phil uint)
    ...
    (define-data-var max-supply uint u5)
    ...
    (define-read-only (get-owner (token-id uint))
        (ok (nft-get-owner? wax____Phil token-id)))
    ...
    (define-public (burn (token-id uint))
       (begin 
        ...
        (nft-burn? wax____Phil token-id tx-sender)))
    
    (define-private (is-owner (token-id uint) (user principal))
        (is-eq user (unwrap! (nft-get-owner? wax____Phil token-id) false)))
    ...
    (define-private (iterate-minting (not-to-be-used bool) (next-id uint))
        ...
        (begin
            (unwrap! (nft-mint? wax____Phil next-id tx-sender) next-id)
            (+ next-id u1)    
        )
        next-id))
    ...
    (define-private (trnsfr (id uint) (sender principal) (recipient principal))
        (match (nft-transfer? wax____Phil id sender recipient)
        success
            (let
    ...
    (define-private (is-sender-owner (id uint))
        (let ((owner (unwrap! (nft-get-owner? wax____Phil id) false)))
    ...
    (define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
        (let ((owner (unwrap! (nft-get-owner? wax____Phil id) (err err-not-found)))
