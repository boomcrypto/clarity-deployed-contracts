---
title: "Trait the-girl-from-stacks"
draft: true
---
```
;; the-girl-from-stacks
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token the-girl-from-stacks uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUHZuMhiM9xcfXF3AVgsaDXLuiis3rVottQbjk3Sk9VP5/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-girl-from-stacks next-id tx-sender) next-id)
      (unwrap! (nft-transfer? the-girl-from-stacks next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-girl-from-stacks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-girl-from-stacks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-girl-from-stacks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-girl-from-stacks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-girl-from-stacks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-girl-from-stacks id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u8793)
