...
(define-non-fungible-token cyber-roo uint)
...
;; Internal variables
(define-data-var mint-limit uint u54)
(define-data-var last-id uint u1)
(define-data-var total-price uint u2500000)
(define-data-var artist-address principal 'SP2QEWK2Y4XR8Y2XT9ZJWVQAT1W94M5ZM35Q1QPBK)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeoyB66jeoaiWbQ2yeLGWsLsr7346JL5gAXTBZM7divmc/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? cyber-roo next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? cyber-roo token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? cyber-roo token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? cyber-roo token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? cyber-roo id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? cyber-roo id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? cyber-roo id) (err ERR-NOT-FOUND)))
