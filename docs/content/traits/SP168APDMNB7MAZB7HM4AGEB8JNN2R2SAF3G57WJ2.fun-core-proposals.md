---
title: "Trait fun-core-proposals"
draft: true
---
```
...
(define-constant VOTING_TOKEN_DEX 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-stxcity-dex)
(define-constant VOTING_TOKEN_POOL 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.xyk-pool-stx-fun-v-1-1)
(define-constant VOTING_TREASURY 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-treasury)
...
(define-public (create-proposal (proposal <proposal-trait>))
  (let
    ...
    (asserts! (> (unwrap! (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-stxcity get-balance tx-sender) ERR_FETCHING_TOKEN_DATA) u0) ERR_INSUFFICIENT_BALANCE)
    ;; proposal was not already executed
    (asserts! (is-none (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-base-dao executed-at proposal)) ERR_PROPOSAL_ALREADY_EXECUTED)
...
(define-public (vote-on-proposal (proposal <proposal-trait>) (vote bool))
  (let
    (
      ...
      (senderBalance (unwrap! (at-block proposalBlockHash (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-stxcity get-balance tx-sender)) ERR_FETCHING_TOKEN_DATA))
    )
    ;; caller has the required balance
...
(define-public (conclude-proposal (proposal <proposal-trait>))
  (let
    ...
    (and votePassed (try! (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-base-dao execute proposal tx-sender)))
    ;; return the result
    (ok votePassed)
  )
)
...
(define-read-only (get-voting-power (who principal) (proposal <proposal-trait>))
  (let
    ...
    (at-block proposalBlockHash (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-stxcity get-balance who))
  )
)
...
(define-private (is-dao-or-extension)
  (ok (asserts! (or (is-eq tx-sender 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-base-dao)
    (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-base-dao is-extension contract-caller)) ERR_NOT_DAO_OR_EXTENSION
  ))
)
...
(define-private (get-liquid-supply (blockHeight uint))
  (let
    (
      ...
      (totalSupply (unwrap! (at-block blockHash (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-stxcity get-total-supply)) ERR_FETCHING_TOKEN_DATA))
      (dexBalance (unwrap! (at-block blockHash (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-stxcity get-balance VOTING_TOKEN_DEX)) ERR_FETCHING_TOKEN_DATA))
      (poolBalance (unwrap! (at-block blockHash (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-stxcity get-balance VOTING_TOKEN_POOL)) ERR_FETCHING_TOKEN_DATA))
      (treasuryBalance (unwrap! (at-block blockHash (contract-call? 'SP168APDMNB7MAZB7HM4AGEB8JNN2R2SAF3G57WJ2.fun-stxcity get-balance VOTING_TREASURY)) ERR_FETCHING_TOKEN_DATA))
    )
