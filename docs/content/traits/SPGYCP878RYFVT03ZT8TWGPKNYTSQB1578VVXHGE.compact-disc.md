...
;; Define LP token
(define-fungible-token CD)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SPGYCP878RYFVT03ZT8TWGPKNYTSQB1578VVXHGE.compact-disc"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? CD amount sender recipient))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? CD (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? CD (get dk delta) sender))
...
(begin
    ...
    (try! (add-liquidity u1000000000))
    
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.dexterity-pool-v1 transfer u3300000000 tx-sender CONTRACT none)))
```
