...
(define-non-fungible-token fuji-visions uint)
...
;; Internal variables
(define-data-var mint-limit uint u70)
(define-data-var last-id uint u1)
(define-data-var total-price uint u25000000)
(define-data-var artist-address principal 'SP3Z0WAQN1YB6N80PZK1CNJB6CMD1F89Y093CWY5W)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZMircy5QKJWrNGAutW8hKwm2XLYZHNiqCnP4rp1Zi8Mo/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? fuji-visions next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? fuji-visions token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? fuji-visions token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? fuji-visions token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? fuji-visions id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? fuji-visions id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? fuji-visions id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u5000)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u4000)
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u120588)
