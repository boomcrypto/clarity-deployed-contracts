...
(define-non-fungible-token jungleking-ai uint)
...
(define-data-var artist-address principal 'SP3Z2Q06Y9F4W1JHVDQ0C414211D59DFNJSWSK9V2)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQ7PKASJteMb8rgBvDABD5j2gRifB98BoUpQegsYuZqFY/json/")
(define-data-var mint-paused bool true)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u190)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? jungleking-ai next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? jungleking-ai token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? jungleking-ai token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? jungleking-ai token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? jungleking-ai id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? jungleking-ai id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? jungleking-ai id) (err ERR-NOT-FOUND)))
...
(map-set mint-passes 'SP3Z2Q06Y9F4W1JHVDQ0C414211D59DFNJSWSK9V2 u10)
