---
title: "Trait neoswap-sc-1684510527093"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/4Cv7vIGx1DcVu0MrGtDR
;; SwapId: ugdH9kKDz9hVaDu02a3j
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u24000000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (contract-call? 'SP28V7K7AJX74RD3RAKFMJ1TGFFKXHMJA51RC3RNN.gamma-dos-punks transfer u7 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u22800000 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (stx-transfer? u1200000 tx-sender RECEIVER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u24000000 tx-sender TRADER-2)))
	    )
            true
        )
