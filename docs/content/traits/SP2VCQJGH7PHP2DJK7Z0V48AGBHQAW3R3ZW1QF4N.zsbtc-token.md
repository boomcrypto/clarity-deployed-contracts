...
(define-data-var token-uri (string-utf8 256) u"https://token-meta.s3.eu-central-1.amazonaws.com/zsBTC.json")
(define-data-var token-name (string-ascii 32) "Zest sBTC")
(define-data-var token-symbol (string-ascii 32) "zsBTC")

(define-constant decimals u8)

(define-read-only (get-total-supply)
  (ok (ft-get-supply zsbtc)))
...
(define-private (transfer-internal (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (match (ft-transfer? zsbtc amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
