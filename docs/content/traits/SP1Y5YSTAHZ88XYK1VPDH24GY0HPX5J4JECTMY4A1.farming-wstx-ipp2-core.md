---
title: "Trait farming-wstx-ipp2-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-ipp2)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-ipp2 transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-ipp2 get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-ipp2 transfer amt protocol user none)))
