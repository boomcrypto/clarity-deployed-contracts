---
title: "Trait test-staking-cp-contract"
draft: true
---
```
...
;; @desc - Var (uint) that keeps track of the max token supply
  ...
  (let
    ...
    (if
      (is-some (index-of custodial-list (contract-of collection))) 
        ...
        (unwrap! (contract-call? collection transfer id tx-sender 'SP3D03X5BHMNSAAW71NN7BQRMV4DW2G4JB3MZAGJ8.test-staking-cp-contract) (err u401))
    false
    )
...
(define-public (claim-item-stake (collection-collective <nft-trait>) (staked-id uint))
  (let
    ...
    (unwrap! (contract-call? 'SP3D03X5BHMNSAAW71NN7BQRMV4DW2G4JB3MZAGJ8.test-snow-contract mint (* this-collection-multiplier-normalized blocks-staked) tx-sender) ERR-UNWRAP)
...
(define-public (claim-all-stake)
  (let
    ...
    (unwrap! (contract-call? 'SP3D03X5BHMNSAAW71NN7BQRMV4DW2G4JB3MZAGJ8.test-snow-contract mint unclaimed-balance-total tx-sender) ERR-UNWRAP)
...
(define-public (unstake-item (collection <nft-trait>) (staked-id uint))
  (let
    ...
    (if
        ...
        (as-contract (unwrap! (contract-call? collection transfer staked-id 'SP3D03X5BHMNSAAW71NN7BQRMV4DW2G4JB3MZAGJ8.test-staking-cp-contract (var-get function-caller-helper-to-unstake)) (err u401)))
...
(define-public (admin-emergency-unstake (collection <nft-trait>) (staked-id uint) (original-owner principal))
  (let
    ...
    (if (> blocks-staked u0)
      ...
      (unwrap! (contract-call? 'SP3D03X5BHMNSAAW71NN7BQRMV4DW2G4JB3MZAGJ8.test-snow-contract mint (* this-collection-multiplier-normalized blocks-staked) original-owner) ERR-UNWRAP)
    ...
    (if
        ...
        (as-contract (unwrap! (contract-call? collection transfer staked-id 'SP3D03X5BHMNSAAW71NN7BQRMV4DW2G4JB3MZAGJ8.test-staking-cp-contract original-owner) (err u401)))
  ...
  (match (var-get token-max-supply)
    ...
    (let
      (
        ...
        (new-token-max-supply (unwrap! (contract-call? 'SP3D03X5BHMNSAAW71NN7BQRMV4DW2G4JB3MZAGJ8.test-snow-contract get-max-supply) u0))
      )
