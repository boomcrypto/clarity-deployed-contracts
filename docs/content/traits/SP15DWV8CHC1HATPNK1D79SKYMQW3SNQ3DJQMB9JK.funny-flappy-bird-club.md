---
title: "Trait funny-flappy-bird-club"
draft: true
---
```
;; funny-flappy-bird-club
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token funny-flappy-bird-club uint)
...
;; Internal variables
(define-data-var mint-limit uint u3850)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTH5C77x4cjsrR32RZovSCujoXjn6bGkC9s45bB5ttcW8/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? funny-flappy-bird-club next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? funny-flappy-bird-club token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? funny-flappy-bird-club token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? funny-flappy-bird-club token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? funny-flappy-bird-club id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? funny-flappy-bird-club id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? funny-flappy-bird-club id) (err ERR-NOT-FOUND)))
