...
(define-non-fungible-token testing uint)

(define-constant CONTRACT-CREATOR tx-sender)

(define-constant ERR-NOT-AUTHORIZED (err u0))

(define-data-var token-uri (string-ascii 256) "https://sspblsjcforgiuatmnbp.nhost.run/v1/functions/rewards/stacks/metadata?id=8236232")
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? testing token-id sender recipient)
  )
)

(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? testing token-id))
)
...
(define-public (mint (token-id uint) (recipient principal))
  (begin
    ...
    (try! (nft-mint? testing token-id recipient))
