---
title: "Trait neoswap-sc-1689465055215"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/e31AYYMETAuRMEbbo1rl
;; SwapId: VMcoo93PzdtIQO9XByRR
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u39472222 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
		(unwrap! (contract-call? 'SP2BE8TZATXEVPGZ8HAFZYE5GKZ02X0YDKAN7ZTGW.btc-sports-vip-pass transfer u150 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (contract-call? 'SP2BE8TZATXEVPGZ8HAFZYE5GKZ02X0YDKAN7ZTGW.btc-sports-vip-pass transfer u150 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u39472222 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u39472222 tx-sender TRADER-1)))
		(unwrap-panic (as-contract (contract-call? 'SP2BE8TZATXEVPGZ8HAFZYE5GKZ02X0YDKAN7ZTGW.btc-sports-vip-pass transfer u150 tx-sender TRADER-1)))
	    )
            true
        )
