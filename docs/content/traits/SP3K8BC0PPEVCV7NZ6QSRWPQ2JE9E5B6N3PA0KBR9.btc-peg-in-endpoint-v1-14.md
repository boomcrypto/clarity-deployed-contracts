---
title: "Trait btc-peg-in-endpoint-v1-14"
draft: true
---
```
...
(define-public (finalize-peg-in-cross
	...
	(let (
		...
		(match (validate-tx-cross-extra validation-data)
			ok-value
			(begin 
				(as-contract (try! (contract-call? .cross-peg-out-endpoint-v2-02 transfer-to-unwrap .token-abtc (get amount-net validation-data) (get chain-id order-details) (get recipient order-details))))
	...
	(let (
		...
		(match (validate-tx-cross-swap-extra validation-data token)
			...
			(let (
				...
				(as-contract (try! (contract-call? .cross-peg-out-endpoint-v2-02 transfer-to-unwrap token-trait swapped (get chain-id order-details) (get recipient order-details))))
				(print (merge print-msg { success: true, swapped-amount: swapped }))
				(ok true))
			err-value
			(begin				
...
(define-private (validate-tx-cross-extra (validation-data { order-details: { recipient: (buff 256), chain-id: uint }, fee: uint, amount-net: uint }))
	(begin 
		(try! (contract-call? .cross-peg-out-endpoint-v2-02 validate-transfer-to-unwrap tx-sender .token-abtc (get amount-net validation-data) (get chain-id (get order-details validation-data))))
		(ok validation-data)))
...
(define-private (validate-tx-cross-swap-extra (validation-data { order-details: { recipient: (buff 256), chain-id: uint, pool-id: uint, min-dy: uint }, fee: uint, amount-net: uint }) (token principal))
	(let (
		...
		(asserts! (>= dy (get min-dy order-details)) err-slippage)
		(asserts! (is-eq token-y token) err-token-mismatch)		
		(try! (contract-call? .cross-peg-out-endpoint-v2-02 validate-transfer-to-unwrap tx-sender token dy (get chain-id order-details)))
		(ok { validation-data: validation-data, token-y: token-y, factor: factor })))
