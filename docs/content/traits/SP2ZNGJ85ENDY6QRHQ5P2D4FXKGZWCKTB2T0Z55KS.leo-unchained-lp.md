---
title: "Trait leo-unchained-lp"
draft: true
---
```
;; Title: Leo Unchained LP
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token LUCID)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.leo-unchained-lp"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? LUCID amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance LUCID who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply LUCID)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? LUCID (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? LUCID (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (add-liquidity u105000000))
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer u199895000000 tx-sender CONTRACT none)))
```
