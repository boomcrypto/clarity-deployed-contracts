---
title: "Trait launchpad-with-pool-lock-v1-5"
draft: true
---
```
...
(define-public (create-pool
	...
	(let 
		(
			(launch-id (try! (contract-call? .alex-launchpad-v1-5 create-pool launch-token-trait payment-token-trait (merge offering { launch-owner: (as-contract tx-sender) }))))
		)
...
(define-public (add-to-position (launch-id uint) (tickets uint) (launch-token-trait <ft-trait>))
	(let
		...
		(as-contract (try! (contract-call? .alex-launchpad-v1-5 add-to-position launch-id tickets launch-token-trait)))
...
(define-read-only (get-launch-or-fail (launch-id uint))
	(contract-call? .alex-launchpad-v1-5 get-launch-or-fail launch-id))
...
(define-public (claim (launch-id uint) (input (list 200 principal)) (launch-token-trait <ft-trait>) (payment-token-trait <ft-trait>))
	(let 
		...
		(try! (contract-call? .alex-launchpad-v1-5 claim launch-id input launch-token-trait payment-token-trait))
