---
title: "Trait syvita-mining-mia-v2"
draft: true
---
```
;;                __   __
...
(define-private (contract-claim-mining-reward (block uint))
    (let
      (
        ...
        (coinbaseAmount (as-contract (contract-call? 'SP1H1733V5MZ3SZ9XRW9FKYGEZT0JDGEB8Y634C7R.miamicoin-core-v2 get-coinbase-amount block)))
        (isWinner (as-contract (contract-call? 'SP1H1733V5MZ3SZ9XRW9FKYGEZT0JDGEB8Y634C7R.miamicoin-core-v2 can-claim-mining-reward POOL_CONTRACT_ADDRESS block)))
      )
      (if isWinner
        (begin
          (try! (as-contract (contract-call? 'SP1H1733V5MZ3SZ9XRW9FKYGEZT0JDGEB8Y634C7R.miamicoin-core-v2 claim-mining-reward block)))
...
(define-private (payout-fee (mineManyId uint))
    (let
      ...
      (try! (as-contract (contract-call? 'SP1H1733V5MZ3SZ9XRW9FKYGEZT0JDGEB8Y634C7R.miamicoin-token-v2 transfer (* (/ coinsWon u100) fee) POOL_CONTRACT_ADDRESS (var-get feeAddress) none)))
      (ok true)
    )
)
...
(define-public (contributor-claim-single-reward-from-pool (mineManyId uint))
   (let
      ...
      (try! (as-contract (contract-call? 'SP1H1733V5MZ3SZ9XRW9FKYGEZT0JDGEB8Y634C7R.miamicoin-token-v2 transfer (/ (* (* (/ coinsWon u100) (- u100 fee)) currentContribution) totalContributed) POOL_CONTRACT_ADDRESS callerAddress none)))
...
(define-public (contract-mine-many (poolId uint) (amounts (list 200 uint)))
    (let
      ...
      (try! (as-contract (contract-call? 'SP1H1733V5MZ3SZ9XRW9FKYGEZT0JDGEB8Y634C7R.miamicoin-core-v2 mine-many amounts)))
