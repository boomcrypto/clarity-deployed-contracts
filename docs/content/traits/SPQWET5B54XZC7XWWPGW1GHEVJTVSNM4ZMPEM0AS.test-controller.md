...
(define-public (check-is-rewarder (contract principal))
...
(define-public (log-reward (reward-usdh uint))
  (let (
    ...
    (total-usdh-supply-staked (unwrap-panic (contract-call? .test-usdh-token get-balance .test-staking-reserve)))
  )
    ...
    (if (> reward-usdh u0)
      (begin
        ...
        (try! (contract-call? .test-usdh-token mint-for-protocol reward-usdh .test-staking-reserve))
      )
      true
    )
