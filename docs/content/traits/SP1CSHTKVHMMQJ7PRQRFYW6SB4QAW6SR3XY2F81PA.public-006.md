...
(define-non-fungible-token public-006 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUVmxDF2wpKDEL8EcPjLgknwKRxawkdV1VQDrRmzNP4n8/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? public-006 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? public-006 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? public-006 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? public-006 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? public-006 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? public-006 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? public-006 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u9632)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u275100000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u466500000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u35000000)
