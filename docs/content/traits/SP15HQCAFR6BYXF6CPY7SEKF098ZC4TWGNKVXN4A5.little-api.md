...
(define-non-fungible-token little-api uint)
...
(define-data-var artist-address principal 'SP15HQCAFR6BYXF6CPY7SEKF098ZC4TWGNKVXN4A5)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQ3aKR4F3wxMRLhBbqWT7cHvxjxB4VpK4yMnj3buXtEph/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? little-api next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? little-api token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? little-api token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? little-api token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? little-api id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? little-api id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? little-api id) (err ERR-NOT-FOUND)))
