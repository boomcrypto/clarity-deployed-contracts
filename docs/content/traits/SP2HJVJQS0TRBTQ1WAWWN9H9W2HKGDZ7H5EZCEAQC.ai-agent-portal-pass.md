---
title: "Trait ai-agent-portal-pass"
draft: true
---
```
;; ai-agent-portal-pass
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token ai-agent-portal-pass uint)
...
;; Internal variables
(define-data-var mint-limit uint u1250)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmc7Cy8rPLov97SeP7485fSiWEz92T9d499dTojFGWfzyw/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? ai-agent-portal-pass next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ai-agent-portal-pass token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ai-agent-portal-pass token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ai-agent-portal-pass token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ai-agent-portal-pass id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ai-agent-portal-pass id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ai-agent-portal-pass id) (err ERR-NOT-FOUND)))
