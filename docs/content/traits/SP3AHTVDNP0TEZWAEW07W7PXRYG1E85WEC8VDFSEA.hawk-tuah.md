...
(define-non-fungible-token hawk-tuah uint)
...
(define-data-var artist-address principal 'SP3AHTVDNP0TEZWAEW07W7PXRYG1E85WEC8VDFSEA)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWyyaN6pojCMwNGrfA3ApUpw1KZuhWJaPgRF3EG8dzsE5/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? hawk-tuah next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? hawk-tuah token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? hawk-tuah token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? hawk-tuah token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? hawk-tuah id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? hawk-tuah id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? hawk-tuah id) (err ERR-NOT-FOUND)))
