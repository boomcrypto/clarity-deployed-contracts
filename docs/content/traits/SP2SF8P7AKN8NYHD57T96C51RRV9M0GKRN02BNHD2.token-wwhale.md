...
(define-fungible-token wwhale)

(define-data-var token-name (string-ascii 32) "Whale Wrapper")
(define-data-var token-symbol (string-ascii 32) "wwhale")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wwhale.json"))
...
(define-read-only (get-base-token)
  'SP25JXH48YHJPE8R48S3AWVYPSB3N3ZG9PRKQZHSF.whales-coin)

(define-read-only (get-base-decimals)
  (contract-call? 'SP25JXH48YHJPE8R48S3AWVYPSB3N3ZG9PRKQZHSF.whales-coin get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP25JXH48YHJPE8R48S3AWVYPSB3N3ZG9PRKQZHSF.whales-coin get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP25JXH48YHJPE8R48S3AWVYPSB3N3ZG9PRKQZHSF.whales-coin transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
