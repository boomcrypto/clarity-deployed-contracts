---
title: "Trait farming-wstx-hshko-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-hshko)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-hshko transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-hshko get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-hshko transfer amt protocol user none)))
