---
title: "Trait keepers-petition-rc4"
draft: true
---
```
;; Keepers' Petition Interaction
;;
...
;; Data Variables
(define-data-var token-amount uint u15000000)
(define-data-var token-principal principal .charisma-token)
...
(define-private (petition-action (sender principal))
  (let ((fatigue-response (unwrap-panic (contract-call? .fatigue-rc4 execute "BURN"))))
...
(define-private (handle-petition-attempt (sender principal))
  (begin
    (match (contract-call? .dungeon-keeper-rc5 transfer (var-get token-amount) (var-get token-principal) sender)
      success (begin 
