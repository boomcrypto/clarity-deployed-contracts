...
(define-non-fungible-token stx-koala-squad uint)
...
;; Internal variables
(define-data-var mint-limit uint u5385)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmW8XiGJjiejSD6wyeAnVUG8VVwzKeB1uvr8EVPMXf3wUa/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stx-koala-squad next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-koala-squad token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-koala-squad token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-koala-squad token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-koala-squad id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-koala-squad id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-koala-squad id) (err ERR-NOT-FOUND)))
