...
(define-non-fungible-token enemy-frogs uint)
...
;; Internal variables
(define-data-var mint-limit uint u747)
(define-data-var last-id uint u1)
(define-data-var total-price uint u2000000)
(define-data-var artist-address principal 'SP1WTNB3BKE23XWAXRS5QXWYF3V9A263R061Q1CQS)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRdn9nNGHrvEP4VmcgT8B7PD75EywzV2QYNHKHKP4bwVu/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? enemy-frogs next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? enemy-frogs token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? enemy-frogs token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? enemy-frogs token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? enemy-frogs id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? enemy-frogs id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? enemy-frogs id) (err ERR-NOT-FOUND)))
