---
title: "Trait arkadiko-swap-token-ldn-usda"
draft: true
---
```
(impl-trait .arkadiko-swap-trait-v1.swap-trait)

(define-fungible-token ldn-usda)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? ldn-usda amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-read-only (get-name)
  (ok "Arkadiko V1 LDN USDA LP Token")
)
...
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance ldn-usda owner))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply ldn-usda))
)

(define-read-only (get-token-uri)
  (ok (some u"https://arkadiko.finance/tokens/ldn-usda-token.json"))
)
...
(define-public (mint (recipient principal) (amount uint))
  (begin
    ...
    (ft-mint? ldn-usda amount recipient)
  )
)
...
(define-public (burn (recipient principal) (amount uint))
  (begin
    ...
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .arkadiko-dao get-qualified-name-by-name "swap"))) (err ERR-NOT-AUTHORIZED))
    (ft-burn? ldn-usda amount recipient)
  )
)
