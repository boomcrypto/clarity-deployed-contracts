---
title: "Trait byzantion-stacks-parrots"
draft: true
---
```
...
(define-non-fungible-token stacks-parrots uint)
...
(define-data-var artist-address principal 'SP2WXZ49XFP2T9MYHSM7PJBBQHF68NNJGREHF03K7)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? stacks-parrots next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? stacks-parrots token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-parrots token-id)))
