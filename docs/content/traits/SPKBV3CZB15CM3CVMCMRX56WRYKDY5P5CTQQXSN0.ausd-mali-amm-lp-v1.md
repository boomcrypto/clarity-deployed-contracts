---
title: "Trait ausd-mali-amm-lp-v1"
draft: true
---
```

;; Title: aUSD-MALi Liquidity Pool
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token aUSD-MALi-LP)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? aUSD-MALi-LP amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "aUSD-MALi Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance aUSD-MALi-LP who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply aUSD-MALi-LP)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? aUSD-MALi-LP (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? aUSD-MALi-LP (get dk delta) sender))
...
(define-read-only (get-reserves-quote)
    (let (
        (reserves (get-reserves))
        (supply (ft-get-supply aUSD-MALi-LP)))
        {
          dx: (get a reserves),
          dy: (get b reserves),
          dk: supply
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://charisma-metadata.vercel.app/api/v1/metadata/SPKBV3CZB15CM3CVMCMRX56WRYKDY5P5CTQQXSN0.ausd-mali-amm-lp-v1"))
)
```
