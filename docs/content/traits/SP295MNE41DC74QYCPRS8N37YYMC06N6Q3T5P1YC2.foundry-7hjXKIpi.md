...
;; Variables
(define-fungible-token FOUND MAXSUPPLY)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://gaia.hiro.so/hub/1EXQXFcsYMHxN35n7eqLJ4L4tBgEEZ9xSA/5db9de5a-5ac2-4eb2-ad36-ff1629bddf56-metadata.json"))
(define-data-var token-name (string-ascii 32) "Foundry")
(define-data-var token-symbol (string-ascii 32) "FOUND")
(define-data-var token-decimals uint u6)

;; Read-Only Functions
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance FOUND owner))
)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance FOUND from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? FOUND amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
(begin
  ...
  (let ((result (ft-mint? FOUND MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
