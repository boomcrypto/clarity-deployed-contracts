---
title: "Trait power-to-the-people"
draft: true
---
```
;; power-to-the-people
...
(define-non-fungible-token power-to-the-people uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUg5Gzw9dQ5qAjRp9rJwCSkF35YVyuE482CrcoJpZTp1c/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? power-to-the-people next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? power-to-the-people token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? power-to-the-people token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? power-to-the-people token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? power-to-the-people id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? power-to-the-people id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? power-to-the-people id) (err ERR-NOT-FOUND)))
