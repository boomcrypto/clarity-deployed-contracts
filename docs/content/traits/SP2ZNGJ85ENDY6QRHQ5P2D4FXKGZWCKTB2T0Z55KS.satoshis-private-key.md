---
title: "Trait satoshis-private-key"
draft: true
---
```
;; Title: Satoshi's Private Key
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token sBTC-STX)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.satoshis-private-key"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? sBTC-STX amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "Satoshi's Private Key"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance sBTC-STX who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply sBTC-STX)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (stx-transfer? (get dy delta) sender CONTRACT))
        (try! (ft-mint? sBTC-STX (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? sBTC-STX (get dk delta) sender))
