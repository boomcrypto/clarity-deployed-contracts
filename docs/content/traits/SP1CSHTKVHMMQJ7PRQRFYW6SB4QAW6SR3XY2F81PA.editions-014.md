...
(define-non-fungible-token editions-014 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmc3FLeMQbbkDNRzzXvoR2g642QP5dEnchcHLiRydeFhyq/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? editions-014 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? editions-014 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? editions-014 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? editions-014 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? editions-014 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? editions-014 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? editions-014 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u290300000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u390400000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u16000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u114000000)
