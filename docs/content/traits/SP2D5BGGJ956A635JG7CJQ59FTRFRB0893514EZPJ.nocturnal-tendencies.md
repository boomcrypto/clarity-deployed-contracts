---
title: "Trait nocturnal-tendencies"
draft: true
---
```
;; Title: Nocturnal Tendencies
;; Version: 1.0.0
;; Description: 
...
(define-constant LP_REBATE u2500)
...
;; Define LP token
(define-fungible-token NTEN)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.nocturnal-tendencies"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? NTEN amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "Nocturnal Tendencies"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance NTEN who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply NTEN)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token transfer amount sender CONTRACT none))
        ;; Transfer token B to sender
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.dexterity-pool-v1 transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.dexterity-pool-v1 transfer amount sender CONTRACT none))
        ;; Transfer token A to sender
        (try! (as-contract (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token transfer (get dx delta) sender CONTRACT none))
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.dexterity-pool-v1 transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? NTEN (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? NTEN (get dk delta) sender))
        (try! (as-contract (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token transfer (get dx delta) CONTRACT sender none)))
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.dexterity-pool-v1 transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-private (get-reserves)
    { 
      a: (unwrap-panic (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token get-balance CONTRACT)), 
      b: (unwrap-panic (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.dexterity-pool-v1 get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.dmghoot-lp-token transfer u14000000 tx-sender CONTRACT none)))
```
