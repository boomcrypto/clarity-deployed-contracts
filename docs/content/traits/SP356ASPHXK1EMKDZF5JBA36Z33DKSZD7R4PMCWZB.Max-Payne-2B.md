...
(define-non-fungible-token Max-Payne-2B uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafyreigscx2ep5dj25upqlzas7qi5odyxjk6hurgch5knu7zvb26nsicsu/metadata.json")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Max-Payne-2B token-id)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Max-Payne-2B token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Max-Payne-2B token-id) false)))
...
(define-private (iterate-minting (not-to-be-used bool) (next-id uint))
  (if (<= next-id (var-get max-supply))
    (begin
      (unwrap! (nft-mint? Max-Payne-2B next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Max-Payne-2B id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Max-Payne-2B id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Max-Payne-2B id) (err err-not-found)))
...
(define-private (airdrop-iter (collector principal) (next-id uint)) 
  (if (<= next-id (var-get max-supply))
    (let (
      ...
      (unwrap! (nft-mint? Max-Payne-2B next-id collector) next-id)
