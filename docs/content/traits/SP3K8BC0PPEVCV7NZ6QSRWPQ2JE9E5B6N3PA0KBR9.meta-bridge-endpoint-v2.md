---
title: "Trait meta-bridge-endpoint-v2"
draft: true
---
```
...
(define-read-only (validate-tx (tx (buff 8192)) (output-idx uint) (offset-idx uint) (order-idx uint) (token principal))
	(let (
			(tx-idxed (try! (contract-call? .oracle-v2 get-bitcoin-tx-indexed-or-fail tx output-idx offset-idx)))
			...
			(amt-in-fixed (decimals-to-fixed (get amt tx-idxed) (contract-call? .oracle-v2 get-tick-decimals-or-default (get tick tx-idxed))))
...
(define-public (finalize-peg-out (request-id uint) (tx (buff 8192)) (output-idx uint) (offset-idx uint) (token-trait <sip010-trait>))
	(let (
			...
			(token-details (try! (get-pair-details-or-fail { tick: (get tick request-details), token: (get token request-details) })))
			(tx-idxed (try! (contract-call? .oracle-v2 get-bitcoin-tx-indexed-or-fail tx output-idx offset-idx)))
			(tx-mined-height (try! (contract-call? .oracle-v2 get-bitcoin-tx-mined-or-fail tx)))
			(amount-in-fixed (decimals-to-fixed (get amt tx-idxed) (contract-call? .oracle-v2 get-tick-decimals-or-default (get tick tx-idxed))))
...
(define-private (index-tx
	...
	(begin 
		(and 
			(not (is-ok (contract-call? .oracle-v2 get-bitcoin-tx-indexed-or-fail (get bitcoin-tx tx) (get output tx) (get offset tx))))
			(as-contract (try! (contract-call? .oracle-v2 index-tx-many (list { tx: tx, block: block, proof: proof, signature-packs: signature-packs })))))
