---
title: "Trait sbtc-yield-rewards-v3"
draft: true
---
```
...
(if is-in-mainnet 
  (begin 
    (var-set current-cycle-tenure-height u182700) 
...
;; verify at get-stacks-block-info state of map when doing calculations
...
(define-private (distribute-reward-user (user principal)) 
  (let 
    (
      ...
      (already-rewarded-amount (default-to u0 (get amount (map-get? rewards-holding {rewarded-address: rewarded-address, cycle-id: cycle-id-current}))))
      (reward-amount 
        (/
          (* 
