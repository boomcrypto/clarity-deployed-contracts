...
(define-constant ERR_MINT_DISABLED u1003)
...
(define-constant ERR_NAME_MODIFY_DISABLED u2003)
(define-constant ERR_NAME_MINORNAME_NOT_MATCH u2004)
(define-constant ERR_MINOR_NAME_ALREADY_TAKEN u2005)
...
;;
(define-data-var m_update_cb_price uint u8000000)
(define-data-var m_update_name_price uint u40000000)
(define-data-var m_update_name_switch bool false)
;;
(define-data-var m_award_step uint u20090103)
(define-data-var m_market_contract (optional principal) none)
(define-data-var m_blacklist_contracts (list 5 principal) (list))
...
(define-public (real_mint (name (buff 25)) (minor_name (buff 25)) (cid (string-ascii 64)) (bio (buff 80)) (receiver principal) (bh uint))
  (let
    (
    ...
    (asserts! (and (is-eq tx-sender OWNER) (<= tid u4)) (err ERR_NO_AUTHORITY))
    (asserts! (var-get m_mint_switch) (err ERR_MINT_DISABLED))
    ...
    ;;
    (and (> tid u4) (try! (deduct (var-get m_mint_price))))
    (try! (contract-call? .laser_token reward_minter receiver))
...
(define-public (update (cid (string-ascii 64)) (bio (buff 80)))
  (let
    ...
    ;;
    (try! (deduct (var-get m_update_cb_price)))
...
(define-public (transfer (tid uint) (sender principal) (recipient principal))
  (let
    ...
    (if (is_called_by_market)
      true
      (asserts! (and (is-eq sender contract-caller) (is-none (index-of (var-get m_blacklist_contracts) sender))) (err ERR_UNAUTHORIZED_CONTRACT))
    )
...
(define-public (update_name (name (buff 25)) (minor_name (buff 25)))
  (let
    ...
    (asserts! (var-get m_update_name_switch) (err ERR_NAME_MODIFY_DISABLED))
    (asserts! (is-eq tx-sender sys_owner logic_owner) (err ERR_NO_AUTHORITY))
    (asserts! (is-none (map-get? map_name2id lower_name)) (err ERR_NAME_ALREADY_TAKEN))
    (if b_minor_name
      (begin
    ...
    ;;
    (try! (deduct (var-get m_update_name_price)))
...
(define-public (deduct (amount uint))
  (let
    (
      ...
      (per_holder_award (/ (- amount owner_award) AWARD_HOLDERS_COUNT))
...
(define-public (update_switches (mint_switch bool) (update_name_switch bool))
  (begin
    (asserts! (is-eq tx-sender OWNER) (err ERR_NO_AUTHORITY))
    (var-set m_mint_switch mint_switch)
    (var-set m_update_name_switch update_name_switch)
    (ok true)
  )
)

(define-public (update_prices (mint_price uint) (update_cb_price uint) (update_name_price uint))
  (begin
    (asserts! (is-eq tx-sender OWNER) (err ERR_NO_AUTHORITY))
    (var-set m_mint_price mint_price)
    (var-set m_update_cb_price update_cb_price)
    (var-set m_update_name_price update_name_price)
    (ok true)
  )
)

(define-public (update_blacklist_contracts (blacklist_contracts (list 5 principal)))
  (begin
    (asserts! (is-eq tx-sender OWNER) (err ERR_NO_AUTHORITY))
    (var-set m_blacklist_contracts blacklist_contracts)
    (ok true)
  )
)
...
(define-read-only (get_summary (player (optional principal)) (name (optional (buff 25))) (tid uint))
  (let
    ...
    )
    {
      ...
      mp: (var-get m_mint_price),
      ucbp: (var-get m_update_cb_price),
      ptid: ptid,
...
(define-read-only (get_player_summary (player principal))
    ...
    )
  {
    ...
    time: (if (is-some meta) (get-block-info? time (mod (unwrap-panic (get ud meta)) u1000000)) none),
    bown: (and (is-some owner_opt) (is-eq player (unwrap-panic owner_opt))),
}))

(define-read-only (get_minor_summary)
  {
    mint_switch: (var-get m_mint_switch),
    update_name_switch: (var-get m_update_name_switch),
    update_name_price: (var-get m_update_name_price),
  }
)
...
(define-read-only (resolve_name (name (buff 25)))
  (let
    (
      (tid (default-to u0 (map-get? map_name2id name)))
    )
    {
      tid: tid,
      meta: (map-get? map_meta tid),
...
(define-public (set_ext_contract (ext_contract principal) (b_add bool))
  (begin
    (asserts! (is-eq tx-sender OWNER) (err ERR_NO_AUTHORITY))
    (if b_add
      (map-set map_ext_contracts ext_contract true)
      (map-delete map_ext_contracts ext_contract)
    )
    (ok true)
  )
)
...
(define-public (set_market_contract (market_contract principal))
  (begin
    (asserts! (is-eq tx-sender OWNER) (err ERR_NO_AUTHORITY))
    (and (is-none (var-get m_market_contract))
      (var-set m_market_contract (some market_contract))
      (try! (set_ext_contract market_contract true))
    )
    (ok true)
  )
)

(define-private (is_called_by_market)
  (is-eq contract-caller (unwrap-panic (var-get m_market_contract)))
)

(define-public (transfer_by_market_list (tid uint) (sender principal) (recipient principal))
  (begin
    (asserts! (is_called_by_market) (err ERR_UNAUTHORIZED_CONTRACT))
