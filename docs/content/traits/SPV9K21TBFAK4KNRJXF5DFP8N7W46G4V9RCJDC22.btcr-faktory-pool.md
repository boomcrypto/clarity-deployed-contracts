---
title: "Trait btcr-faktory-pool"
draft: true
---
```

      ;; Title: sBTC-BTCR lp-token
      ;; Version: 1.0.0
      ...
      (define-constant CONTRACT_INITIALIZER 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.btcr-faktory-dex)
      (define-constant SAINT 'SP000000000000000000002Q6VF78)
  
      ;; Define LP token
      (define-fungible-token sBTC-BTCR)
      ...
      (define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
          (begin
              ...
              (try! (ft-transfer? sBTC-BTCR amount sender recipient))
      ...
      (define-read-only (get-balance (who principal))
          (ok (ft-get-balance sBTC-BTCR who)))
  
      (define-read-only (get-total-supply)
          (ok (ft-get-supply sBTC-BTCR)))
      ...
      (define-public (swap-a-to-b (amount uint))
          (let (
              ...
              (try! (as-contract (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.btcr-faktory transfer dy-d CONTRACT sender none)))
              (print {
                  type: "buy",
                  sender: sender,
                  token-in: "sbtc",
      ...
      (define-public (swap-b-to-a (amount uint))
          (let (
              ...
              (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.btcr-faktory transfer amount sender CONTRACT none))
      ...
      (define-public (add-liquidity (amount uint))
          (let (
              ...
              (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.btcr-faktory transfer dy-d sender CONTRACT none))
              (try! (ft-mint? sBTC-BTCR dk-d sender))
              (print {
                  type: "add-liquidity",
                  sender: sender,
                  sbtc-amount: dx-d,
      ...
      (define-private (add-liquidity-and-burn (amount uint))
          (let (
              ...
              (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer dx-d sender CONTRACT none))
              (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.btcr-faktory transfer dy-d sender CONTRACT none))
              (try! (ft-mint? sBTC-BTCR dk-d SAINT))
              (print {
      ...
      (define-public (remove-liquidity (amount uint))
          (let (
              ...
              (dy-d (get dy delta))
              (dk-d (get dk delta)))
              (try! (ft-burn? sBTC-BTCR dk-d sender))
      ...
      (define-private (get-reserves)
          { 
            ...
            b: (unwrap-panic (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.btcr-faktory get-balance CONTRACT))
          })
      ...
      (define-read-only (get-reserves-quote)
          (let (
              (reserves (get-reserves))
              (supply (ft-get-supply sBTC-BTCR)))
              {
                dx: (get a reserves),
                dy: (get b reserves),
                dk: supply
      ...
      (define-public (initialize-pool (lowest uint) (highest uint)) ;; highest = origin minus lowest
        (begin
           ...
           (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.btcr-faktory transfer highest contract-caller CONTRACT none))
           (ok true)
        )
      )
```
