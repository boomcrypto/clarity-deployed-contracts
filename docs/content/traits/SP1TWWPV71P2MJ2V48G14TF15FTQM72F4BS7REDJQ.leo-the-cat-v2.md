...
(define-non-fungible-token leo-the-cat-v2 uint)
...
(define-data-var artist-address principal 'SP1TWWPV71P2MJ2V48G14TF15FTQM72F4BS7REDJQ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qma45G4JwxLkZsWXNQhMtc1Z7DTsnp9XrDCjf1c9VMW4N3/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? leo-the-cat-v2 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? leo-the-cat-v2 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? leo-the-cat-v2 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? leo-the-cat-v2 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? leo-the-cat-v2 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? leo-the-cat-v2 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? leo-the-cat-v2 id) (err ERR-NOT-FOUND)))
