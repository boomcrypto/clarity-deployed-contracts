...
(define-constant INVALID-STX-CALCULATION (err u408))
(define-constant ERR-TRANSFER-MULTISIG (err u409))

(define-constant ERR-TRANSFER-CRYPTOMIND (err u501))
...
(define-data-var CRYPTOMIND principal 'SPVBZEFW7X123Q3GXC2YH4XV1SS0DW1MHTNJD6KE);;3
(define-data-var BURN_WALLET principal 'SP362TJX91ATWS1NJMRZVFEXHAQX6PR6GCBRJWVY2);;4


(define-data-var total-stx-swapped uint u0) ;; Total STX swapped in DEX
...
(define-public (set-cryptomind (new-wallet principal))
  (begin
    (try! (check-is-owner))
    (asserts! (not (is-eq new-wallet (var-get CRYPTOMIND))) ERR-ENTER-NEW-WALLET)
    (var-set CRYPTOMIND new-wallet)
    (ok new-wallet)))
...
(define-read-only (get-cryptomind) (ok (var-get CRYPTOMIND)))
...
(define-public (buy-diamonds (diamond-count uint))
  (begin
    ...
    (let ((required-stx
      ...
      (let ((share-to-multisig (/ (* required-stx (var-get percentage-multisig)) u100))
            (share-to-burn (/ (* required-stx (var-get percentage-burn)) u100))
            (share-to-cryptomind (/ (* required-stx (var-get percentage-cryptomind)) u100))
        ...
        (unwrap! (stx-transfer? share-to-cryptomind tx-sender (var-get CRYPTOMIND)) ERR-TRANSFER-CRYPTOMIND)
...
(define-public (buy-hearts (heart-count uint))
  ...
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Define public variables for the percentages
(define-data-var percentage-multisig uint u45)
(define-data-var percentage-burn uint u35)
(define-data-var percentage-cryptomind uint u20)
...
(define-read-only (get-percentage-cryptomind)
  (ok (var-get percentage-cryptomind)))

;; Setter functions for percentage (only owner can update)
...
(define-public (set-percentage-cryptomind (new-percentage uint))
  (begin
    (try! (check-is-owner))
     (asserts! (not (is-eq new-percentage (var-get percentage-cryptomind))) ERR-SAME-PERCENTAGE)
    (var-set percentage-cryptomind new-percentage)
    (ok new-percentage)))
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define-read-only (get-progressive-pool)
