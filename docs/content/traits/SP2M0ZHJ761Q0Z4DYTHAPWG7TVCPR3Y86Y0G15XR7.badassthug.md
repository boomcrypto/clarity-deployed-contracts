...
(define-non-fungible-token badassthug uint)
...
;; Internal variables
(define-data-var mint-limit uint u646)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYXLJGFQD7X54WQCPsRV934QZ6SZyxgFmkADFbAFjBdju/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? badassthug next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? badassthug token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? badassthug token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? badassthug token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? badassthug id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? badassthug id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? badassthug id) (err ERR-NOT-FOUND)))
