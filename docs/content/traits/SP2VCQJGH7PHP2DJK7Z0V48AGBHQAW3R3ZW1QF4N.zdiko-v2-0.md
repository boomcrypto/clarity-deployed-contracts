...
(define-constant asset-addr 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-token)
(define-constant decimals u6)

(define-read-only (get-total-supply)
  (contract-call? .zdiko-token get-total-supply))

(define-read-only (get-name)
  (contract-call? .zdiko-token get-name))

(define-read-only (get-symbol)
  (contract-call? .zdiko-token get-symbol))

(define-read-only (get-decimals)
  (contract-call? .zdiko-token get-decimals))

(define-read-only (get-token-uri)
  (contract-call? .zdiko-token get-token-uri))
...
(define-private (transfer-internal (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (contract-call? .zdiko-token transfer amount sender recipient memo)
)

(define-private (mint-internal (amount uint) (owner principal))
  (contract-call? .zdiko-token mint amount owner)
)

(define-private (burn-internal (amount uint) (owner principal))
  (contract-call? .zdiko-token burn amount owner)
)
...
(define-read-only (get-principal-balance (account principal))
  (ok
    (contract-call? .zdiko-token get-balance account)
  )
)
...
(define-public (set-contract-owner (owner principal))
  (begin
    ...
    (print { type: "set-contract-owner-zdiko", payload: owner })
