---
title: "Trait brc20-bridge-registry-dev-preview-2"
draft: true
---
```
	...
	(begin
		...
		(ok
			(match (map-get? approved-tokens tick)
				...
				(map-set approved-tokens tick { token: token, approved: approved, peg-in-paused: true, peg-out-paused: true, peg-in-fee: u0, peg-out-fee: u0, peg-out-gas-fee: u0 })))))
...
(define-public (set-request (request-id uint) (details { requested-by: principal, peg-out-address: (buff 128), tick: (string-utf8 4), amount-net: uint, fee: uint, gas-fee: uint, claimed: uint, claimed-by: principal, fulfilled-by: (buff 128), revoked: bool, finalized: bool, requested-at: uint}))
	(let 
		(
