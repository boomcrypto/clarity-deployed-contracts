---
title: "Trait bitcoin-monkeys-coupon-20pc"
draft: true
---
```
...
(define-non-fungible-token bitcoin-monkeys-coupon-20pc uint)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-monkeys-coupon-20pc id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace bitcoin-monkeys-coupon-20pc
  (ok (nft-get-owner? bitcoin-monkeys-coupon-20pc id)))
...
(define-public (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? bitcoin-monkeys-coupon-20pc next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? bitcoin-monkeys-coupon-20pc id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-monkeys-coupon-20pc id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-monkeys-coupon-20pc id) ERR-NOT-FOUND))
