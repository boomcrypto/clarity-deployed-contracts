...
(define-non-fungible-token light-of-love uint)
...
(define-data-var artist-address principal 'SP1Y5TBRJK8GX2Q8DJK66SBHQR1ZBVEM4ZB4PTYCE)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRbAMAXJgqTqsMzWHbrBLDFUdC64tU8DmJxKEn94EzW1w/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? light-of-love next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? light-of-love token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? light-of-love token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? light-of-love token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? light-of-love id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? light-of-love id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? light-of-love id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u4900)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u4612000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u3153000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u13000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u165000000)
