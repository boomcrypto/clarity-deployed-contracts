...
```
    ...
    (define-non-fungible-token PaperPlanes uint)
    ...
    (define-data-var max-supply uint u150)
    ...
    (define-data-var ipfs (string-ascii 80) "ipfs://bafyreidgxkzpsow4p52xmepiqvaio4xl5n37rrgodburwmiknbzroblu7a/metadata.json")
    ...
    (define-read-only (get-owner (token-id uint))
        (ok (nft-get-owner? PaperPlanes token-id)))
    ...
    (define-public (burn (token-id uint))
       (begin 
        ...
        (nft-burn? PaperPlanes token-id tx-sender)))
    
    (define-private (is-owner (token-id uint) (user principal))
        (is-eq user (unwrap! (nft-get-owner? PaperPlanes token-id) false)))
    ...
    (define-private (iterate-minting (not-to-be-used bool) (next-id uint))
        ...
        (begin
            (unwrap! (nft-mint? PaperPlanes next-id tx-sender) next-id)
            (+ next-id u1)    
        )
        next-id))
    ...
    (define-private (trnsfr (id uint) (sender principal) (recipient principal))
        (match (nft-transfer? PaperPlanes id sender recipient)
        success
            (let
    ...
    (define-private (is-sender-owner (id uint))
        (let ((owner (unwrap! (nft-get-owner? PaperPlanes id) false)))
    ...
    (define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
        (let ((owner (unwrap! (nft-get-owner? PaperPlanes id) (err err-not-found)))
