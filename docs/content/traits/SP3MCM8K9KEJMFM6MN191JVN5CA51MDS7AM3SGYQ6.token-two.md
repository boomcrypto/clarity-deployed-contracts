...
(define-fungible-token token-two)

(define-read-only (get-total-supply)
  (ok (ft-get-supply token-two))
)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance token-two account))
)

(define-read-only (get-balance-simple (account principal))
  (ft-get-balance token-two account)
)
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    (ft-mint? token-two amount recipient)
  )
)

(define-public (burn (amount uint))
  (begin
    (ft-burn? token-two amount tx-sender)
  )
)
```
