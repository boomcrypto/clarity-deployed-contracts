...
(define-non-fungible-token neon-ghosts uint)
...
(define-data-var artist-address principal 'SP28WFMAB4WJ1HSPEM1Z5S8TMHPR9086H802WPEP6)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPevdUT9KxJp7eCKyYYk5j5Z6iCSkfiRdbbFLVPmSiC1n/json/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u30)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? neon-ghosts next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? neon-ghosts token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? neon-ghosts token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? neon-ghosts token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? neon-ghosts id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? neon-ghosts id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? neon-ghosts id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u31609)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u10944000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u4315000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u372000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u3571000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u14500000000)
