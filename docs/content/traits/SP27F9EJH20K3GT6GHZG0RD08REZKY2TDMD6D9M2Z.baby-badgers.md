---
title: "Trait baby-badgers"
draft: true
---
```
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)
(define-non-fungible-token baby-badgers uint)

;; constants
(define-constant contract-owner tx-sender)
(define-constant baby-badgers-nft-limit u1050)
...
(define-data-var ipfs-root (string-ascii 102) "ipfs://ipfs/QmeseZLhWcTVAPR9GmCpvHHMXoK12AaBBKdMThRpGnXWVA/baby_badger_")
...
;; Private functions
(define-private (hasnt-bred (parent-badger-id uint))
  (default-to true (map-get? has-bred parent-badger-id))
)

;; Read Only
;; Hasn't Bred
(define-read-only (hasnt-bred-public (parent-badger-id uint))
  (ok (default-to true (map-get? has-bred parent-badger-id)))
)
...
(define-read-only (get-baby-badger (baby-id uint))
    (ok (print (map-get? baby-badger baby-id)))
)
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? baby-badgers id))
)
...
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? baby-badgers id sender recipient)
  )
)

;; Core Functions
(define-public (breed (baby-name (string-ascii 48)) (parent-badger-1 uint) (parent-badger-2 uint) (parent-1-trait-01 uint) (parent-1-trait-02 uint) (parent-1-trait-03 uint) (parent-1-trait-04 uint) (parent-1-trait-05 uint) (parent-1-trait-06 uint) (parent-1-trait-07 uint) (parent-1-trait-08 uint) (parent-1-trait-09 uint) (parent-1-trait-10 uint) (parent-2-trait-01 uint) (parent-2-trait-02 uint) (parent-2-trait-03 uint) (parent-2-trait-04 uint) (parent-2-trait-05 uint) (parent-2-trait-06 uint) (parent-2-trait-07 uint) (parent-2-trait-08 uint) (parent-2-trait-09 uint) (parent-2-trait-10 uint))
  (let (
    ...
    (asserts! (is-eq tx-sender (unwrap-panic (unwrap-panic (contract-call? .btc-badgers-v2 get-owner parent-badger-1)))) ERR-NOT-BDGR-1-OWNER)
    (asserts! (is-eq tx-sender (unwrap-panic (unwrap-panic (contract-call? .btc-badgers-v2 get-owner parent-badger-2)))) ERR-NOT-BDGR-2-OWNER)
    (asserts! (hasnt-bred parent-badger-1) ERR-ADULT-ALREADY-BRED-1)
    (asserts! (hasnt-bred parent-badger-2) ERR-ADULT-ALREADY-BRED-2)
    (asserts! (<= total-hearts-applied user-hearts-balance) ERR-NOT-ENOUGH-HEARTS)

    (map-set baby-badger (var-get last-id)
    {
      baby-name: baby-name,
      parent-badger-1: parent-badger-1,
      parent-badger-2: parent-badger-2,
   ...
   (map-set has-bred parent-badger-1 false)
   (map-set has-bred parent-badger-2 false)
   (map-set heart-balance tx-sender {first-time: false, purchased-count: (- user-hearts-balance total-hearts-applied)})
   (try! (nft-mint? baby-badgers (var-get last-id) tx-sender))
