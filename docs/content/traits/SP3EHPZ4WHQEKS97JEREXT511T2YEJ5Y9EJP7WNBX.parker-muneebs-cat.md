---
title: "Trait parker-muneebs-cat"
draft: true
---
```
;; parker-muneebs-cat
...
(define-non-fungible-token parker-muneebs-cat uint)
...
(define-data-var artist-address principal 'SP3EHPZ4WHQEKS97JEREXT511T2YEJ5Y9EJP7WNBX)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYJasYFckMj1Wwz3QCji84zTA7EkyXvx3skErSvNBPzB4/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? parker-muneebs-cat next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? parker-muneebs-cat token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? parker-muneebs-cat token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? parker-muneebs-cat token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? parker-muneebs-cat id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? parker-muneebs-cat id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? parker-muneebs-cat id) (err ERR-NOT-FOUND)))
