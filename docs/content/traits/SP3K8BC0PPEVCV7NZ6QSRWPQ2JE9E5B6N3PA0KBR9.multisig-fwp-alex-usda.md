---
title: "Trait multisig-fwp-alex-usda"
draft: true
---
```
...
(define-public (propose (start-block-height uint) (title (string-utf8 256)) (url (string-utf8 256)) (new-fee-rate-x uint) (new-fee-rate-y uint))
  (let 
    (
      (proposer-balance (unwrap-panic (contract-call? .fwp-alex-usda get-balance tx-sender)))
      (total-supply (unwrap-panic (contract-call? .fwp-alex-usda get-total-supply)))
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .simple-weight-pool-alex set-fee-rate-x .age000-governance-token .token-wusda new-fee-rate-x)))
    (as-contract (try! (contract-call? .simple-weight-pool-alex set-fee-rate-y .age000-governance-token .token-wusda new-fee-rate-y)))
    (ok true)
  )
)
