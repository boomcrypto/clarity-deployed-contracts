---
title: "Trait ede007-snapshot-proposal-voting-v4"
draft: true
---
```
...
(define-read-only (get-historical-values (height uint) (who principal))
	(at-block (unwrap! (get-block-info? id-header-hash height) none)
		(some {
				...
				locked-balance: (default-to u0 (get amount-ustx (contract-call? 'SP000000000000000000002Q6VF78.pox get-stacker-info who))), 
...
(define-public (vote (amount uint) (for bool) (proposal principal))
	(let
		...
		(asserts!
			(<= new-total-votes (- (get user-balance historical-values) (get locked-balance historical-values)))
