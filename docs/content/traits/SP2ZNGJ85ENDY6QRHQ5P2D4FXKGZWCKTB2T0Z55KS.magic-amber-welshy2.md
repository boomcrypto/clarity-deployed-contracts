---
title: "Trait magic-amber-welshy2"
draft: true
---
```
...
(define-private (execute-legacy-quote (amount uint) (token-in <ft-trait>) (token-out <ft-trait>))
    ;; Determine direction based on tokens
    (if (is-eq (contract-of token-out) 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststx-token)
        (unwrap-panic (quote-legacy-stx-to-ststx amount))
        (unwrap-panic (quote-legacy-ststx-to-stx amount)))
)
...
;; (define-private (execute-legacy-swap (amount uint) (token-in <ft-trait>) (token-out <ft-trait>))
;;     ;; Determine direction based on tokens
;;     (if (is-eq (contract-of token-out) 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststx-token)
;;         (unwrap-panic (swap-legacy-stx-to-ststx amount))
;;         (unwrap-panic (swap-legacy-ststx-to-stx amount)))
;; )
