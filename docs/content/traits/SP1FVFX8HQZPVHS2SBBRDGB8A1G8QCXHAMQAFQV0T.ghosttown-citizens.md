---
title: "Trait ghosttown-citizens"
draft: true
---
```
;; ghosttown-citizens
...
(define-non-fungible-token ghosttown-citizens uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWbzyVm6KSQdFs7EM5zj6iUVP7e17foN8EEtEBx5m2sZj/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ghosttown-citizens next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? ghosttown-citizens token-id sender recipient)))

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ghosttown-citizens token-id)))
