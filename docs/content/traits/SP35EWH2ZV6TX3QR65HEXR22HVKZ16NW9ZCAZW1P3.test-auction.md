...
(define-non-fungible-token test-auction uint)
...
(define-data-var admin (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF 'SP35EWH2ZV6TX3QR65HEXR22HVKZ16NW9ZCAZW1P3 ))
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? test-auction item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? test-auction item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? test-auction item-id)))
      (begin
        (try! (nft-burn? test-auction item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? test-auction token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? test-auction id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? test-auction id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? test-auction id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP35EWH2ZV6TX3QR65HEXR22HVKZ16NW9ZCAZW1P3 u1000 "Test auction" u1000000 u63652 u64060 "ipfs://QmY4XWemdQnfU2BGqGFzjkfQSKD48xJsg8hGvU3C2aTRiN/test-auction/1.json"))
  (ok true)
)
