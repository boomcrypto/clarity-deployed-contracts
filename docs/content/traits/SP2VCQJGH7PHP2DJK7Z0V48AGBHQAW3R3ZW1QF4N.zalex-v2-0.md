...
(define-constant asset-addr 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex)
(define-constant decimals u8)

(define-read-only (get-total-supply)
  (contract-call? .zalex-token get-total-supply))

(define-read-only (get-name)
  (contract-call? .zalex-token get-name))

(define-read-only (get-symbol)
  (contract-call? .zalex-token get-symbol))

(define-read-only (get-decimals)
  (contract-call? .zalex-token get-decimals))

(define-read-only (get-token-uri)
  (contract-call? .zalex-token get-token-uri))
...
(define-private (transfer-internal (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (contract-call? .zalex-token transfer amount sender recipient memo)
)

(define-private (mint-internal (amount uint) (owner principal))
  (contract-call? .zalex-token mint amount owner)
)

(define-private (burn-internal (amount uint) (owner principal))
  (contract-call? .zalex-token burn amount owner)
)
...
(define-read-only (get-principal-balance (account principal))
  (ok
    (contract-call? .zalex-token get-balance account)
  )
)
...
(define-public (set-contract-owner (owner principal))
  (begin
    ...
    (print { type: "set-contract-owner-zalex", payload: owner })
