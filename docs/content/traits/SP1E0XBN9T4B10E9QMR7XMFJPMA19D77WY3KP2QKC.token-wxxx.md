...
(define-fungible-token wxxx)

(define-data-var token-name (string-ascii 32) "X Wrapper")
(define-data-var token-symbol (string-ascii 32) "wxxx")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wxxx.json"))
...
(define-read-only (get-base-token)
  'SPX7Z21BD3HR8VXQE6NS38KX6PY7V184KDVVYMZW.x)

(define-read-only (get-base-decimals)
  (contract-call? 'SPX7Z21BD3HR8VXQE6NS38KX6PY7V184KDVVYMZW.x get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SPX7Z21BD3HR8VXQE6NS38KX6PY7V184KDVVYMZW.x get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SPX7Z21BD3HR8VXQE6NS38KX6PY7V184KDVVYMZW.x transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
