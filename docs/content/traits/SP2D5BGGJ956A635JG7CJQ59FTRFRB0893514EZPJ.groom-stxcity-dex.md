...
(define-constant token-supply u10000000) ;; match with the token's supply (6 decimals)
(define-constant BONDING-DEX-ADDRESS (as-contract tx-sender)) ;; one contract per token

;; bonding curve config
(define-constant STX_TARGET_AMOUNT u10000000)
(define-constant VIRTUAL_STX_VALUE u2000000) ;; 1/5 of STX_TARGET_AMOUNT
(define-constant COMPLETE_FEE u200000) ;; 2% of STX_TARGET_AMOUNT
...
(define-constant allow-token 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.groom-stxcity)
...
(define-data-var burn-percent uint u0)
(define-data-var deployer-percent uint u0)
...
(begin
  ...
  (var-set token-balance u1000000)
  (var-set stx-balance u1000000)
  (var-set tradable true)
  (var-set burn-percent u0)
  (var-set deployer-percent u0) ;; based on the burn-amount. It's about ~0.1 to 0.5% supply
