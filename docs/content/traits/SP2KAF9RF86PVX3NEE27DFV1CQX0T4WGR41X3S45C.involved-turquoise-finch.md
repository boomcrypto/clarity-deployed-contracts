---
title: "Trait involved-turquoise-finch"
draft: true
---
```
...
(define-private (mint-in-nyc (new-owner principal))
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
      (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
        ;; (match (unwrap! (contract-call? 'SP2H8PY27SEZ03MWRKS5XABZYQN17ETGQS3527SA5.newyorkcitycoin-token transfer (var-get cost-per-mint-mia) tx-sender (as-contract tx-sender) (some 0x00)) (err u102))
          success (begin
            ;; (try! (as-contract (unwrap! (contract-call? 'SP2H8PY27SEZ03MWRKS5XABZYQN17ETGQS3527SA5.newyorkcitycoin-token transfer (- (var-get cost-per-mint-mia) (* (var-get commission-per-mint-mia) u2)) (as-contract tx-sender) artist-address (some 0x00)) (err u102))))
            ;; (try! (as-contract (unwrap! (contract-call? 'SP2H8PY27SEZ03MWRKS5XABZYQN17ETGQS3527SA5.newyorkcitycoin-token transfer (var-get commission-per-mint-mia) (as-contract tx-sender) CONTRACT-OWNER (some 0x00)) (err u102))))
            ;; (try! (as-contract (unwrap! (contract-call? 'SP2H8PY27SEZ03MWRKS5XABZYQN17ETGQS3527SA5.newyorkcitycoin-token transfer (var-get commission-per-mint-mia) (as-contract tx-sender) (var-get commission-address) (some 0x00)) (err u102))))
            (try! (nft-mint? test-nfts next-id new-owner))
            (var-set last-id next-id)
            (ok next-id)
          ) 
