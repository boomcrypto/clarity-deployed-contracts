---
title: "Trait stx-test-collection-v1"
draft: true
---
```
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token stx-test-collection-v1 uint)

;; Constants
(define-constant DEPLOYER tx-sender )
;;deployer: 'SPGYVECG634WDDNT34A5RRBB837F3GJCV8MG8Q23)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stx-test-collection-v1 next-id tx-sender) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-test-collection-v1 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-test-collection-v1 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-test-collection-v1 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-test-collection-v1 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-test-collection-v1 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-test-collection-v1 id) (err ERR-NOT-FOUND)))
