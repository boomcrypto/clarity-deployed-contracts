...
(define-non-fungible-token big-cat-george uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmSVd12SQY7TPjdC4LRXGjwifNLw2B2oHFKoVc6Uz4CQCV/big-cat-george/")
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? big-cat-george id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace big-cat-george
  (ok (nft-get-owner? big-cat-george id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? big-cat-george next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? big-cat-george id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? big-cat-george id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? big-cat-george id) ERR-NOT-FOUND))
...
(define-data-var xbtc-cost-per-mint uint u20000)
...
;; ###### BEGIN: fungible token mint template code ##################################################
(define-data-var xusd-cost-per-mint uint u30000)
