...
(define-non-fungible-token drop-dead-damsels uint)
...
;; Internal variables
(define-data-var mint-limit uint u208)
(define-data-var last-id uint u1)
(define-data-var total-price uint u100000000)
(define-data-var artist-address principal 'SP1PDAC0E02FAQX4B3WJN6TSEMEBGXA9VPA43QXJ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdzcjkyETX4ixvUqSYdY8JzDrdRgmVpXfFWhiZKXYgcme/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? drop-dead-damsels next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? drop-dead-damsels token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? drop-dead-damsels token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? drop-dead-damsels token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? drop-dead-damsels id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? drop-dead-damsels id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? drop-dead-damsels id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u223594)
