---
title: "Trait wellness-x-web-3-giveaway"
draft: true
---
```
;; wellness-x-web-3-giveaway
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token wellness-x-web-3-giveaway uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVAmehwFBdcewfyXicVrXAR53SckCjr5DSfbx4HykdZDQ/json/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? wellness-x-web-3-giveaway next-id tx-sender) next-id)
      (unwrap! (nft-transfer? wellness-x-web-3-giveaway next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? wellness-x-web-3-giveaway token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? wellness-x-web-3-giveaway token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? wellness-x-web-3-giveaway token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? wellness-x-web-3-giveaway id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? wellness-x-web-3-giveaway id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? wellness-x-web-3-giveaway id) (err ERR-NOT-FOUND)))
