---
title: "Trait memegoat-staking-v1-2"
draft: true
---
```
...
(define-constant ERR-WITH-V1 (err u4000))
...
;; @desc transfer-v1-stakes:
;; @params user-addr whose stake is to be transferred
(define-public (transfer-stake-from-v1 (user-addr principal))
  (begin
    (try! (check-is-owner))
    (asserts! (contract-call? .memegoat-staking-v1 get-user-stake-has-staked user-addr) ERR-NO-STAKE-FOUND)

    (let
      (
        (user-stake-data (try! (contract-call? .memegoat-staking-v1 get-user-staking-data user-addr)))
        (deposit-amount (get deposit-amount user-stake-data))
        (deposit-block (get deposit-block user-stake-data))
        (end-block (get end-block user-stake-data))
        (lock-rewards (get lock-rewards user-stake-data))
        (stake-index (get stake-index user-stake-data))
        (paid (get paid user-stake-data))
      )

      (map-set deposit-map 
        { user-addr: user-addr } 
        { deposit-amount: deposit-amount, deposit-block: deposit-block, end-block: end-block, lock-rewards: lock-rewards, stake-index: stake-index, paid: paid }
      )
...
(define-public (transfer-balances-from-v1 (no-of-participants uint))
  (begin 
   (try! (check-is-owner))
    (let
      (
        (total-reward-from-v1 (unwrap! (contract-call? .memegoat-staking-v1 get-total-rewards) ERR-WITH-V1))
        (total-staked-from-v1 (unwrap! (contract-call? .memegoat-staking-v1 get-total-staked) ERR-WITH-V1))
      )

      (var-set total-reward total-reward-from-v1)
      (var-set reward-balance total-reward-from-v1)
      (var-set staked-total total-staked-from-v1)
      (var-set staked-balance total-staked-from-v1)
      (var-set total-participants no-of-participants)
    )
    (ok true)
  )
)
...
(define-public (stake (amount uint) (stake-index uint))
  (begin
    ...
    ;; transfer memegoat to vault
    (try! (contract-call? .memegoatstx transfer amount tx-sender .memegoat-vault-v1 none)) 
...
(define-public (unstake) 
  (begin
    ...
    (let
      (
        (amount-to-pay (try! (do-withdraw-stake  tx-sender)))
      )
      ;; transfer token from vault
      (as-contract (try! (contract-call? .memegoat-vault-v1 transfer-ft .memegoatstx (decimals-to-fixed amount-to-pay) tx-sender)))
    )
    (ok true)
  )
)
...
(define-public (emergency-withdraw) 
  (begin
    ...
    (let
      (
        (amount-to-pay  (try! (do-emergency-withdraw tx-sender)))
      )
