---
title: "Trait akmekoolbeats-instrumentals-002"
draft: true
---
```
;; akmekoolbeats-instrumentals-002
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token akmekoolbeats-instrumentals-002 uint)
...
(define-data-var artist-address principal 'SP160SEJ6Y5N6S4QE69DKQ7TXQPQXD24GA94NS8GD)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPPBfGtnbv9cbNxs8xeZkRZxJHEaArWBprYeTXtAUN5bk/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? akmekoolbeats-instrumentals-002 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? akmekoolbeats-instrumentals-002 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? akmekoolbeats-instrumentals-002 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? akmekoolbeats-instrumentals-002 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? akmekoolbeats-instrumentals-002 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? akmekoolbeats-instrumentals-002 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? akmekoolbeats-instrumentals-002 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u15805)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u215700000000)
