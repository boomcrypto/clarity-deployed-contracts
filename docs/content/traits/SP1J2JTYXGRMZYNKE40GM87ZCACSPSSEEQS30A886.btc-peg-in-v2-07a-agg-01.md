---
title: "Trait btc-peg-in-v2-07a-agg-01"
draft: true
---
```
...
(define-data-var peg-in-paused bool false)
...
(define-public (finalize-peg-in-agg
	...
	(let (
		...
		(as-contract (try! (contract-call? .cross-peg-out-v2-01-agg-01
			transfer-to-swap
				(get amount-net validation-data)
				'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-abtc
				(get token-out order-details)
				(get min-amount-out order-details)
				(get chain-id order-details)
				{ address: (get to order-details), chain-id: (some u0) })))			
