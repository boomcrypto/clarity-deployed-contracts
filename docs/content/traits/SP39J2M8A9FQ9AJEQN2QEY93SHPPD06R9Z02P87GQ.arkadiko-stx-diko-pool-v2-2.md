---
title: "Trait arkadiko-stx-diko-pool-v2-2"
draft: true
---
```
...
(define-constant UNSTAKE_LOCK_PERIOD u20)
...
(define-read-only (get-lp-token-amount-from-reap (reap-amount uint)) 
  (let 
    (
      (reap-supply (unwrap! (contract-call? .reap-stx-diko-token-v2-2 get-total-supply) ERR-GET-REAP-SUPPLY))
...
(define-public (stake-single-token (amount uint) (is-diko bool))
  (begin 
    ...
    (let
      ...
      (try! (as-contract (contract-call? .reap-stx-diko-token-v2-2 mint reap-tokens-to-mint user)))
...
(define-public (create-unstake-order (reap-amount uint))
  (let 
    (
      (reap-tokens-balance (unwrap! (contract-call? .reap-stx-diko-token-v2-2 get-balance tx-sender) ERR-GET-REAP-BALANCE))
    ...
    (try! (as-contract (contract-call? .reap-stx-diko-token-v2-2 burn reap-amount user)))
