...
;; base did:web uri of the application
(define-data-var app_did_web_uri (optional (string-utf8 256)) (some u"did:web:domains.paradigma.global"))

;; set token URI to new value, only accessible by CONTRACT-OWNER
(define-public (set-app-did-web-uri (newUri (optional (string-utf8 256))))
    (begin
        (asserts! (is-eq tx-sender CONTRACT_OWNER) (err ERR_UNAUTHORIZED))
        (ok (var-set app_did_web_uri newUri))
    )
)

;; Define the type of product considered in CrossCheck Ecosystem (xck.app)

(define-data-var typeOfProduct (string-utf8 50) u"XCK-Inscription-BNS-name") 

(define-read-only (get-type-of-inscription)
     (ok (var-get typeOfProduct))
)

;;TODO add icon or image representing typeOfProduct
;; define initial token URI associated to the product or inscription service

(define-data-var tokenUri (optional (string-utf8 256)) (some u"https://domains.paradigma.global/metadata/xck-inscription-bns-name.json"))

;; set token URI to new value, only accessible by CONTRACT-OWNER
;; The description is used for all IDs in this case
...
(define-read-only (get-domain-service-fee  
                                           (nameSpace (buff 48))
                                           (domainName (buff 48))
                                           (tokenSymbol (string-ascii 32))                                         
                  )
  (default-to u0 (get fee 
...
;; Set reference info for domain service fee for each nameSpace+domainName
    ...
    (if (is-authorized-domain) 
      (if
        ...
        (begin
          ...
          { 
            serviceId: (+ (var-get domainServiceFeeCount) u1)
          }
           {
            nameSpace: nameSpace,
            domainName: domainName,
         ...
         (ok (var-set domainServiceFeeCount (+ (var-get domainServiceFeeCount) u1)))
        )
        (begin
         (ok 
...
;; Stores participants DomainName service sale
...
(define-read-only (get-domain-name-managers-index (id uint))
     (unwrap-panic (map-get? DomainNameManagersIndex
                     {
                      domainNMId: id
                     })
      )
      ...
      (if
        ...
        (begin
          (map-insert DomainNameManagersIndex
           { 
            domainNMId: (+ (var-get domainNameManagerCount) u1)
           }
           {
            nameSpace: nameSpace,
            domainName: domainName
         ...
         (ok (var-set domainNameManagerCount (+ (var-get domainNameManagerCount) u1)))
        )
        (begin
         (ok 
  ...
  (begin
    ...
    (let 
        (
          ...
          (fee (get-domain-service-fee 0x616c6c 0x616c6c symbol))
          (toBurn (- stxToBurn fee))
          (expires (unwrap-panic (contract-call? 'SP000000000000000000002Q6VF78.bns name-preorder hashedSaltedFqn toBurn)))
...
(define-public (order-to-register-domain (sender principal) (fee uint) (nameSpace (buff 48)) (domainName (buff 48)) (subDomain (buff 48)) 
   ...
   (begin
    ...
    (asserts! (> (stx-get-balance tx-sender) u0) (err ERR_INSUFFICIENT_FUNDS))    ;; need of some STX to cover gas fees 
    (if (is-eq (unwrap-panic (contract-call? paymentSIP010Trait get-symbol)) "STX") 
        (asserts! (> (unwrap-panic (contract-call? paymentSIP010Trait get-balance tx-sender)) fee) (err ERR_INSUFFICIENT_FUNDS))   ;;exact fee in STX is not sufficient to cover gas
