...
(define-public (mint-for-protocol (amount uint) (recipient principal))
  (let (
    ...
    (try! (contract-call? .dao check-is-protocol contract-caller))
    (try! (contract-call? .ststxbtc-tracking-data set-total-supply (ft-get-supply ststxbtc)))
    (try! (contract-call? .ststxbtc-tracking refresh-wallet recipient (ft-get-balance ststxbtc recipient)))
    result
  )
)
...
(define-public (burn (amount uint))
  (let (
    ...
    (try! (contract-call? .ststxbtc-tracking-data set-total-supply (ft-get-supply ststxbtc)))
    (try! (contract-call? .ststxbtc-tracking refresh-wallet tx-sender (ft-get-balance ststxbtc tx-sender)))
    result
  )
)
