...
(define-non-fungible-token cyber-girls uint)
...
(define-data-var artist-address principal 'SP3V1XV4CM156ZDH15HZ6V4PGFW46BH5BX1W26K6H)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZPLqkT4uXoM3ft7b4ySJcJaAJCq5PvPhztgHCsCvBk3x/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? cyber-girls next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? cyber-girls token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? cyber-girls token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? cyber-girls token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? cyber-girls id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? cyber-girls id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? cyber-girls id) (err ERR-NOT-FOUND)))
