---
title: "Trait tokenmobiles-flyers"
draft: true
---
```
;; tokenmobiles-flyers
...
(define-non-fungible-token tokenmobiles-flyers uint)
...
(define-data-var artist-address principal 'SP1HC2P2ZCSMHJANT20HYF53GMS7MSDB3Z2VGCDJY)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVpHGamByTBCjViJvCbwsy6PxxAnWDHKggKnHaDdAKwwk/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? tokenmobiles-flyers next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? tokenmobiles-flyers token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? tokenmobiles-flyers token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tokenmobiles-flyers token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? tokenmobiles-flyers id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? tokenmobiles-flyers id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? tokenmobiles-flyers id) (err ERR-NOT-FOUND)))
