...
;; Define LP token
(define-fungible-token STD)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SPGYCP878RYFVT03ZT8TWGPKNYTSQB1578VVXHGE.stdollar"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? STD amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance STD who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply STD)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? STD (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? STD (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (add-liquidity u900000000))
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt transfer u163100000000 tx-sender CONTRACT none)))
```
