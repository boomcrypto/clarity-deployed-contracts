...
(define-non-fungible-token four-garlic-glass uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmNWMngLapAxQb9Zzxi8YwFYhhdbcbeAecthMyUSwKF8jZ/four-garlic-glass/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP1A32MBZDN1B28ZT5DH6MA739VJ0BFH05CJHCM8W )) 
...
(define-data-var public-target-block-height uint u63744)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? four-garlic-glass id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace four-garlic-glass
  (ok (nft-get-owner? four-garlic-glass id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? four-garlic-glass next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? four-garlic-glass id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? four-garlic-glass id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? four-garlic-glass id) ERR-NOT-FOUND))
...
(define-data-var spaghetti-cost-per-mint uint u200000000)
...
;; begin mint 4
(define-public (claim-spaghetti-four)
  (begin
    (try! (claim-spaghetti))
    (try! (claim-spaghetti))
    (try! (claim-spaghetti))
...
;; begin mint 5
(define-public (claim-spaghetti-five)
  (begin
    (try! (claim-spaghetti))
    (try! (claim-spaghetti))
    (try! (claim-spaghetti))
