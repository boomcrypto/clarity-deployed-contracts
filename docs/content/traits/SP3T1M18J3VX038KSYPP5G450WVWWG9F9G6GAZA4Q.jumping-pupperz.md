---
title: "Trait jumping-pupperz"
draft: true
---
```
;; Title: Jumping Pupperz
;; Author: SP3T1M18J3VX038KSYPP5G450WVWWG9F9G6GAZA4Q
;; Created With Charisma
;; https://charisma.rocks

;; Description:
;; A monthly, limited quantity, themed jumping $WELSH NFT.

;; This contract implements the SIP-009 community-standard Non-Fungible Token trait
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

;; Define the NFT's name
(define-non-fungible-token jumping-pupperz uint)

;; Keep track of the last minted token ID
(define-data-var last-token-id uint u0)

;; Define constants
(define-constant COLLECTION_LIMIT u69) ;; Limit to series of 69
(define-constant ENERGY_PER_NFT u69420) ;; 69420 energy per NFT
(define-constant STX_PER_MINT u2000000) ;; 2 STX per MINT for DAO
(define-constant MAX_NFTS_PER_TX u3) ;; Maximum 3 NFTs per transaction
(define-constant OWNER 'SP3T1M18J3VX038KSYPP5G450WVWWG9F9G6GAZA4Q) ;; Collection creator
(define-constant CHA_AMOUNT (* u5 STX_PER_MINT)) ;; 10.000000 CHA per mint to creator
...
(define-data-var base-uri (string-ascii 200) "https://charisma.rocks/api/v0/nfts/SP3T1M18J3VX038KSYPP5G450WVWWG9F9G6GAZA4Q.jumping-pupperz/{id}.json")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? jumping-pupperz token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? jumping-pupperz token-id sender recipient)
  )
)
...
(define-private (mint (recipient principal))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? jumping-pupperz token-id recipient))
...
;; Mint multiple NFTs based on the count (1 to 3)
