---
title: "Trait xbot-melaunch-alex-amm-pool-v2-01"
draft: true
---
```
;; title: xbot-melaunch-alex-amm-pool-v2-01
;; version:
;; summary:
;; description:
...
(define-public (swap-helper-in (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (dx uint) (min-dy (optional uint)))
    ;; #[allow(unchecked_data)]
    (let 
        (
        ...
        (try! (transfer-out-internal token-y-trait sender (try! (swap-helper-internal false token-x-trait token-y-trait amount-in min-dy))))
...
(define-public (swap-helper-out (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (dx uint) (min-dy (optional uint)))
    ;; #[allow(unchecked_data)]
    (let 
        (
            (sender (try! (get-sender)))
            (delta (try! (swap-helper-internal true token-x-trait token-y-trait dx min-dy)))
...
(define-private (swap-helper-internal
    ...
    (let 
        ...
        (if fixed
            ...
            true)
        (try! (as-contract (contract-call? 'SP35G0C2NHXFP1ATJW17YYTFKNEF1DY73R0XB2KG9.melaunch-alex-amm-pool-v2-01 swap-helper token-x-trait token-y-trait dx min-dy)))
