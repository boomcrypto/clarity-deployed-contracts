...
(define-non-fungible-token balls-de-crystal uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcNb3MmFkgTgrtWf1xH22rbe6ZJa3YU1ioysGteWeQ31X/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? balls-de-crystal next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? balls-de-crystal token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? balls-de-crystal token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? balls-de-crystal token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? balls-de-crystal id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? balls-de-crystal id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? balls-de-crystal id) (err ERR-NOT-FOUND)))
