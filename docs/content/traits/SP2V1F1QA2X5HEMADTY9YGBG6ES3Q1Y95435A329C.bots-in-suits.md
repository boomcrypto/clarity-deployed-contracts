...
(define-non-fungible-token bots-in-suits uint)
...
(define-data-var artist-address principal 'SPRPBK860P3EQP1R2AZXR7E7WDX9ETJF38CV5MTQ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVh2bSDihcR5jkhRvZN1gxrdaLpvY9wfua4cYwfCax1k3/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bots-in-suits next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? bots-in-suits token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bots-in-suits token-id)))
