...
(define-fungible-token wturtle)

(define-data-var token-name (string-ascii 32) "Stacking Turtle Wrapper")
(define-data-var token-symbol (string-ascii 32) "wturtle")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wturtle.json"))
...
(define-read-only (get-base-token)
  'SP3YTJRGXXMXDQ1V4T5GTFQC2KJ8G5EMYW93A4Q1G.stacking-turtle-stxcity)

(define-read-only (get-base-decimals)
  (contract-call? 'SP3YTJRGXXMXDQ1V4T5GTFQC2KJ8G5EMYW93A4Q1G.stacking-turtle-stxcity get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP3YTJRGXXMXDQ1V4T5GTFQC2KJ8G5EMYW93A4Q1G.stacking-turtle-stxcity get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP3YTJRGXXMXDQ1V4T5GTFQC2KJ8G5EMYW93A4Q1G.stacking-turtle-stxcity transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
