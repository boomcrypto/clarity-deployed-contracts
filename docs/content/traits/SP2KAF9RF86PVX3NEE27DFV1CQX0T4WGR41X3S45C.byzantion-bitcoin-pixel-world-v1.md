---
title: "Trait byzantion-bitcoin-pixel-world-v1"
draft: true
---
```
...
(define-non-fungible-token bitcoin-pixel-world uint)
...
(define-data-var commission uint u9500)
(define-data-var payout uint u0)
(define-data-var ipfs-full-metadata (string-ascii 125) "ipfs://ipfs/bafybeieeznlfe3ckvqgcynavmulplvwoxc2vxum6ljv5v4eknrehi4rou4/bitcoin_pixel_world/bitcoin_pixel_world_metadata.json")
(define-data-var ipfs-root (string-ascii 112) "ipfs://ipfs/bafybeigfeglyufw26pa7ejgvgorkapy5kxh3niopg2sovylq3yr5p2vyim/bitcoin_pixel_world/bitcoin_pixel_world_")
(define-data-var artist-address principal 'SP27BDVSXB2GY8RS13JJFQ9ZJ0YA8AQV9SBQY86MZ)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? bitcoin-pixel-world next-id new-owner))
...
(define-public (set-root (ipfs (string-ascii 112))) 
  (if (is-eq tx-sender CONTRACT-OWNER)
...
(define-public (set-full (ipfs (string-ascii 125))) 
  (if (is-eq tx-sender CONTRACT-OWNER)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? bitcoin-pixel-world token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-pixel-world token-id)))
