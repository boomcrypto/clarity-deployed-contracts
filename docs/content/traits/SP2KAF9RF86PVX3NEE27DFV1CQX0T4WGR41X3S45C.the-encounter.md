...
(define-non-fungible-token the-encounter uint)
...
(define-data-var cost-per-mint uint u100000000)
(define-data-var commission uint u1200)
(define-data-var payout uint u0)
(define-data-var target-block uint u36787)
(define-data-var ipfs-full (string-ascii 108) "ipfs://placeholder")
(define-data-var ipfs-root (string-ascii 95) "ipfs://placeholder")
(define-data-var artist-address principal 'SP1N00FZNRWZ936RHX93QK0D4XFR300BY7V3XA8XA)
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? the-encounter next-id new-owner))
...
(define-public (set-ipfs-full (new-ipfs-full (string-ascii 84)))
  (begin
    (asserts! (and (var-get ipfs-change-enabled) (is-eq tx-sender CONTRACT-OWNER)) (err ERR-NOT-AUTHORIZED))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? the-encounter token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-encounter token-id)))
