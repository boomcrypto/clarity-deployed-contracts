---
title: "Trait non-fungible-token"
draft: true
---
```
...
(define-non-fungible-token StRewards uint)
...
(define-data-var base-uri (string-ascii 80) "https://ipfs.io/ipfs/QmfMBtp5WURhFbJURf9sKYk9fYzkBqvHiwUrPnVZGLD5a4")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? StRewards token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? StRewards token-id sender recipient)
  )
)
...
(define-public (mint (recipient principal))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? StRewards token-id recipient))
