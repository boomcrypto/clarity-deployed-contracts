---
title: "Trait had-to-secret-rare"
draft: true
---
```
;; had-to-secret-rare
...
(define-non-fungible-token had-to-secret-rare uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qme4qHRim2ZRjc6z1oaSuduiUfcTc2jTSrYwz1jsTkYW92/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? had-to-secret-rare next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? had-to-secret-rare token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? had-to-secret-rare token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? had-to-secret-rare token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? had-to-secret-rare id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? had-to-secret-rare id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? had-to-secret-rare id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u632186)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u443000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u290000000000)
