...
(define-public (batch-transfer-memo (amount uint) (minter principal) (fund uint) (treasury principal) (saving uint) (protocol principal) (recipients (list 1000 principal)) (memos (list 1000 (buff 34))))
    (let
        (
        ...
        (try! (stx-transfer? amount tx-sender minter) )
        (try! (stx-transfer? fund tx-sender treasury) )
        (try! (stx-transfer? saving tx-sender protocol) )
        (ok true)
    )
)
```
