---
title: "Trait stacks-nakamoto-activation-og-edition"
draft: true
---
```
;; stacks-nakamoto-activation-og-edition
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token stacks-nakamoto-activation-og-edition uint)
...
(define-data-var artist-address principal 'SP3G022QFZ5HEG9TWKAZ8HHV32CAWNVREVWNBMMAP)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXLJADuC1RJoJJk1AQhSXTbeTkLBZaJCMDhNTNAeDHPUa/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-nakamoto-activation-og-edition next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-nakamoto-activation-og-edition token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-nakamoto-activation-og-edition token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-nakamoto-activation-og-edition token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-nakamoto-activation-og-edition id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-nakamoto-activation-og-edition id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-nakamoto-activation-og-edition id) (err ERR-NOT-FOUND)))
