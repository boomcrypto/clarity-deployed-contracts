---
title: "Trait neoswap-sc-1685203218750"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/mEW0LahWU5V4Bd38TGxi
;; SwapId: bF51e91jNe5EoLC7IdaG
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (contract-call? 'SP2YS6QDVENG4GK46W6BRB8TCJ22VFZH7A54T2293.spirits-of-elden transfer u30 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP1J5D7C11CRPBEWF4DPQ9NNH671Z3TZ4SJPTVTGX.voodoo-fam transfer u6 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-3)
            (begin
		(unwrap! (stx-transfer? u17445000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP2YS6QDVENG4GK46W6BRB8TCJ22VFZH7A54T2293.spirits-of-elden transfer u30 tx-sender TRADER-3)))
	(unwrap-panic (as-contract (contract-call? 'SP1J5D7C11CRPBEWF4DPQ9NNH671Z3TZ4SJPTVTGX.voodoo-fam transfer u6 tx-sender TRADER-3)))

	(unwrap-panic (as-contract (stx-transfer? u1997000 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (stx-transfer? u1664000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u13784000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2YS6QDVENG4GK46W6BRB8TCJ22VFZH7A54T2293.spirits-of-elden transfer u30 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP1J5D7C11CRPBEWF4DPQ9NNH671Z3TZ4SJPTVTGX.voodoo-fam transfer u6 tx-sender TRADER-2)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-3)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u17445000 tx-sender TRADER-3)))
	    )
            true
        )
