---
title: "Trait test-upload-errors"
draft: true
---
```
;; test-upload-errors
...
(define-non-fungible-token test-upload-errors uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPoZv6wcTEs45Lod1S1nEc7uUj8DDhXLPr1bBtoWyEwED/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? test-upload-errors next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? test-upload-errors token-id sender recipient)))

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? test-upload-errors token-id)))
