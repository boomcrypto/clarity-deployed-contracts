---
title: "Trait dread-pirate-roberts"
draft: true
---
```
...
(define-non-fungible-token dread-pirate-roberts uint)
...
(define-data-var cost-per-mint uint u59000000)
...
(define-private (mint (new-owner principal))
  (let (
        ...
        (payout-one (/ (* (var-get cost-per-mint) u3334) u10000))
        (payout-two (/ (* (var-get cost-per-mint) u3333) u10000))
      )
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? dread-pirate-roberts next-id new-owner))
            (var-set last-id next-id)
            (try! (as-contract (stx-transfer? payout-one (as-contract tx-sender) (var-get artist-address))))
            (try! (as-contract (stx-transfer? payout-two (as-contract tx-sender) 'SP3FSP19YM1A1E5M17H933V8HRCSHFWW97MXMBT09)))
            (try! (as-contract (stx-transfer? payout-two (as-contract tx-sender) 'SP2238S1MPQM31NW09N56BHY053GRZ4N0KKB0WV3C)))
            (ok next-id)
          ) 
          error (err error)
          )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? dread-pirate-roberts token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? dread-pirate-roberts token-id)))
