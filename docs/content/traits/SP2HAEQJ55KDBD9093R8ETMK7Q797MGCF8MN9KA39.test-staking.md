...
(define-read-only (get-usdh-per-susdh) 
  (let (
    (total-usdh-staked (unwrap-panic (contract-call? .test-usdh-token get-balance .test-staking)))
...
(define-public (stake (amount uint))
  (let (
    ...
    (try! (contract-call? .test-usdh-token transfer amount tx-sender .test-staking none))
...
(define-public (unstake (amount uint))
  (let (
    ...
    (try! (contract-call? .test-usdh-token transfer amount-usdh (as-contract tx-sender) .test-staking-silo none))
...
(define-public (init-usdh-per-susdh (ratio uint)) 
  (let (
    ...
    (try! (contract-call? .test-usdh-token mint-for-protocol usdh-base .test-staking))
