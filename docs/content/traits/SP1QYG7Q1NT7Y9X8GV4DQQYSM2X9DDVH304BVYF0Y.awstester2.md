...
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)
  ...
  (define-fungible-token AWSTESTER2 MAX_SUPPLY)
  ...
  (define-public
    ...
    (begin
      ...
      (ft-transfer? AWSTESTER2 amt from to)))


  (define-public (mint (amt uint) (to principal))
    (begin
      (try! (check-owner))
      (ft-mint? AWSTESTER2 amt to) ))

  (define-read-only (get-name)                   (ok "AWSTESTER2"))
  (define-read-only (get-symbol)                 (ok "AWS2"))
  (define-read-only (get-decimals)               (ok u6))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance AWSTESTER2 of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply AWSTESTER2)))
