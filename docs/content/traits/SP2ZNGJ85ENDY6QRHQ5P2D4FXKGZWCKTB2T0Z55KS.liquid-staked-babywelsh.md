---
title: "Trait liquid-staked-babywelsh"
draft: true
---
```
;; Title: Liquid Staked Babywelsh
;; Author: rozar.btc
;; Synopsis:
;; This contract implements a liquid staking solution for Babywelsh.
;; It provides users with liquid tokens (sBABYWELSH) that represent staked Babywelsh. 
...
(define-data-var token-name (string-ascii 32) "Liquid Staked Babywelsh")
(define-data-var token-symbol (string-ascii 10) "sBABYWELSH")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/liquid-staked-babywelsh.json"))
...
(define-public (stake (amount uint))
	(begin
		(let
			...
			(try! (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.tokensoft-token-v4kjvtr37ht transfer amount sender contract none))
...
(define-public (unstake (amount uint))
	(begin
		(let
			...
			(try! (as-contract (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.tokensoft-token-v4kjvtr37ht transfer amount-token contract sender none)))
		)
		(ok true)
	)
)
...
(define-public (deposit (amount uint))
    (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.tokensoft-token-v4kjvtr37ht transfer amount tx-sender contract none)
)
...
(define-read-only (get-total-in-pool)
	(unwrap-panic (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.tokensoft-token-v4kjvtr37ht get-balance contract))
)
