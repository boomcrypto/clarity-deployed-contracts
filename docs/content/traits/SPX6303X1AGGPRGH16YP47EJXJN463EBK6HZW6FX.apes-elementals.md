...
(define-non-fungible-token apes-elementals uint)
...
(define-data-var artist-address principal 'SPX6303X1AGGPRGH16YP47EJXJN463EBK6HZW6FX)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZ2oDanVNz2vxRuQiWCDdn4C4zJS595pDqwUvq4b9spAd/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? apes-elementals next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? apes-elementals token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? apes-elementals token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? apes-elementals token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? apes-elementals id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? apes-elementals id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? apes-elementals id) (err ERR-NOT-FOUND)))
