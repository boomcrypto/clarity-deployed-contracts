---
title: "Trait elon-squirrel-club"
draft: true
---
```
;; elon-squirrel-club
...
(define-non-fungible-token elon-squirrel-club uint)
...
(define-data-var artist-address principal 'SP1BGKYXDQASJWBTAVN0RXFTX1XWYBP8P3PXAEZA9)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmceeiJZD73dN4pGYNPDXN719yi2Zf5cYLZRTYcQbfeFDP/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? elon-squirrel-club next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? elon-squirrel-club token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? elon-squirrel-club token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? elon-squirrel-club token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? elon-squirrel-club id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? elon-squirrel-club id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? elon-squirrel-club id) (err ERR-NOT-FOUND)))
