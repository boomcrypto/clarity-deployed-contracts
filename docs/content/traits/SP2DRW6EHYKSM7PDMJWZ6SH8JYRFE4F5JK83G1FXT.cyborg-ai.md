...
(define-non-fungible-token cyborg-ai uint)
...
(define-data-var total-price uint u180000000)
(define-data-var artist-address principal 'SP2DRW6EHYKSM7PDMJWZ6SH8JYRFE4F5JK83G1FXT)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTLFY6CyiZHZD68ugBHTgfHXGf8mmh9zxZ8EXsXbcxvGi/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? cyborg-ai next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? cyborg-ai token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? cyborg-ai token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? cyborg-ai token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? cyborg-ai id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? cyborg-ai id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? cyborg-ai id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u496649)
