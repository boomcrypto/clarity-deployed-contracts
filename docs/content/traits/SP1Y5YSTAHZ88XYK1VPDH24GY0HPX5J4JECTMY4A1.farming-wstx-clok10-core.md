---
title: "Trait farming-wstx-clok10-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-clok10)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-clok10 transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-clok10 get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-clok10 transfer amt protocol user none)))
