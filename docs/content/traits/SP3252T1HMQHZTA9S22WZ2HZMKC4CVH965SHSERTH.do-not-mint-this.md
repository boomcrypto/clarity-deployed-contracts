...
(define-non-fungible-token do-not-mint-this uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qma8fswdYvQvaXqsPkjHh4vq2EFSRV5ikEAHH5XFMhwW9b/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? do-not-mint-this next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? do-not-mint-this token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? do-not-mint-this token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? do-not-mint-this token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? do-not-mint-this id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? do-not-mint-this id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? do-not-mint-this id) (err ERR-NOT-FOUND)))
