...
(define-public (request-mint
  ...
  (let (
    ...
    (oracle-data (element-at (try! (contract-call? 'SP3R4F6C1J3JQWWCVZ3S7FRRYPMYG6ZW6RZK31FXY.pyth-oracle-v3 decode-price-feeds price-feed-bytes execution-plan)) u0))
    (oracle-price-base (pow u10 (to-uint (* -1 (unwrap-panic (get expo oracle-data))))))
    (oracle-price-conf (unwrap-panic (get conf oracle-data)))
    (oracle-price (to-uint (unwrap-panic (get price oracle-data))))
    (oracle-timestamp (unwrap-panic (get publish-time oracle-data)))
    (oracle-price-feed-id (unwrap-panic (get price-identifier oracle-data)))
    ...
    (state (contract-call? .test-minting-state get-request-mint-state contract-caller))
  )
...
(define-public (request-redeem
  ...
  (let (
    ...
    (scaled-max-price (/ (* max-price price-base) oracle-price-base))
    (scaled-min-price (/ (* min-price price-base) oracle-price-base))
    (state (contract-call? .test-minting-state get-request-redeem-state contract-caller))
  )
    (try! (contract-call? .test-hq check-is-enabled))
    ...
    (try! (contract-call? .test-usdh-token-final transfer amount-usdh contract-caller this-contract none))
...
(define-public (confirm-mint (request-id uint) (price uint) (amount-asset-confirmed uint) (minting-asset <sip-010-trait>) (custody-address principal) (memo (optional (buff 34))))
  (let (
    ...
    (if (> amount-asset-fee u0) (try! (contract-call? minting-asset transfer amount-asset-fee this-contract fee-address memo)) true)
...
(define-public (confirm-redeem (request-id uint) (price uint) (amount-usdh-confirmed uint) (redeeming-asset <sip-010-trait>) (memo (optional (buff 34))))
  (let (
    ...
    (try! (contract-call? .test-redeeming-reserve-v2 transfer amount-asset-confirmed requester redeeming-asset memo))
    (if (> amount-asset-fee u0) (try! (contract-call? .test-redeeming-reserve-v2 transfer amount-asset-fee fee-address redeeming-asset none)) true)
