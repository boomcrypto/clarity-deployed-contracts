  ...
  (begin
    (asserts! 
      (or
        ...
        (or 
          (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "l-stx-reserve")))
          (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "l-sip10-reserve")))
        )
       )
     (err ERR-NOT-AUTHORIZED))
    (asserts! (var-get is-emergency-stop-not-called) (err ERR-EMERGENCY-STOPPED))
...
(define-public (revoke-for-dao (amount uint) (sender principal) (recipient principal))
  (begin
    (asserts! 
      (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "l-lbtc-revoker")))
      (err ERR-NOT-AUTHORIZED))
