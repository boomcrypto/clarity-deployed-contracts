---
title: "Trait sensitive-yellow-bobcat"
draft: true
---
```
...
(define-non-fungible-token auction-nfts uint)
...
(define-data-var target-block uint u41100)
...
(define-public (auction-ended)
    ...
    (try! (nft-mint? auction-nfts next-id buyer))
...
(define-public (burn (value uint))
  ...
  (begin
    (try! (nft-mint? auction-nfts value (as-contract tx-sender)))
    (try! (nft-burn? auction-nfts value (as-contract tx-sender)))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? auction-nfts token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? auction-nfts token-id)))
