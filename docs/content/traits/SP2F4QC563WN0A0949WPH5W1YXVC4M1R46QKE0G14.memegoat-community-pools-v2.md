---
title: "Trait memegoat-community-pools-v2"
draft: true
---
```
;;
...
(define-private (move-p (stake-id uint) (acc uint))
  (let
    (
      ...
      (stake-pool (unwrap-panic (contract-call? .memegoat-community-pools-v1 get-stake-pool stake-id)))
    )
    (map-set stake-pool-map { stake-id: next-stake-id }
      {
        ...
        start-block: (convert-block-time (get start-block stake-pool)),
        end-block: (convert-block-time (get end-block stake-pool)),
        last-update-block: (convert-block-time (get last-update-block stake-pool)),
...
(define-private (move (data {user: principal, old-id: uint}) (stake-id uint))
  (let
    (
      ...
      (u-has-stake (contract-call? .memegoat-community-pools-v1 get-user-stake-has-stake old-id user))
      (user-stake (unwrap-panic (contract-call? .memegoat-community-pools-v1 get-user-staking-data old-id user)))
    )
    (if u-has-stake
      (begin
...
(define-public (create-pool 
  ...
  (let
    ...
    (asserts! (and (> start-block burn-block-height) (> end-block start-block)) ERR-INVALID-DURATION)
...
(define-private (do-stake (stake-id uint) (user-addr principal) (amount uint) (stake-token <ft-trait>))
  (begin
    (let
      ...
      (asserts! (< burn-block-height end-block) ERR-STAKE-ENDED)
...
(define-private (last-block-reward-applicable (end-block uint))
  (if (< burn-block-height end-block)
  burn-block-height
  end-block
  )
)
