...
(define-non-fungible-token billionaire uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcLroiCjyrSQL6fzJ1RqDwYx5uhfN3bNxQWj9Jcfc1Dwd/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? billionaire next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? billionaire token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? billionaire token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? billionaire token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? billionaire id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? billionaire id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? billionaire id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-alex uint u3300000000)
