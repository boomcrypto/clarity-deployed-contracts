...
(define-non-fungible-token oak uint)
...
(define-data-var artist-address principal 'SP1KD2BS98HCAEZQB3A4AXNS2KNAFTXF2CTJBQWF6)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQdyLmU48AiTtvZwQMK6EfR4vB2SMij4g2Kyt3ning9Ee/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? oak next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? oak token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? oak token-id) false)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? oak id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? oak id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? oak id) (err ERR-NOT-FOUND)))
