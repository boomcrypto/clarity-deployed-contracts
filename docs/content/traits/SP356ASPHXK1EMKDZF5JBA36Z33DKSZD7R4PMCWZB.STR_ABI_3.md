...
(define-non-fungible-token STR-ABI-3 uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafyreia6r4y5r63sbgxxtxd6eenwjtji7h7tskjaxkje3hgbuov6k6337u/metadata.json")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? STR-ABI-3 token-id)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? STR-ABI-3 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? STR-ABI-3 token-id) false)))
...
(define-private (iterate-minting (not-to-be-used bool) (next-id uint))
  (if (<= next-id (var-get max-supply))
    (begin
      (unwrap! (nft-mint? STR-ABI-3 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? STR-ABI-3 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? STR-ABI-3 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? STR-ABI-3 id) (err err-not-found)))
...
(define-private (airdrop-iter (collector principal) (next-id uint)) 
  (if (<= next-id (var-get max-supply))
    (let (
      ...
      (unwrap! (nft-mint? STR-ABI-3 next-id collector) next-id)
