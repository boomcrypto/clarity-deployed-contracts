...
(define-non-fungible-token bloopies uint)
...
(define-data-var artist-address principal 'SP2XA59ZFEVYVDS24AD94XRJ60GNXCCE0AD3XG1ZP)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWHnbLujgWyhVCDcHAgyeTA14EUgk5VQcmkCpVkJrC8GS/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bloopies next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bloopies token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bloopies token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bloopies token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bloopies id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bloopies id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bloopies id) (err ERR-NOT-FOUND)))
