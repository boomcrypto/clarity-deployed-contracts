...
;; constants
(define-constant token-name "crashpunks")
(define-constant token-symbol "CPS")
...
(define-read-only (get-owner (nftIndex uint))
  (ok (nft-get-owner? crashpunks nftIndex))
)
...
(define-public (transfer (nftIndex uint) (owner principal) (recipient principal))
  (if (unwrap! (is-approved nftIndex owner) nft-not-owned-err)
    (match (nft-transfer? crashpunks nftIndex owner recipient)
...
(define-public (burn (nftIndex uint) (owner principal))
  (if (unwrap! (is-approved nftIndex owner) nft-not-owned-err)
    (match (nft-burn? crashpunks nftIndex owner)
        success (begin
            (ok success)
        )
        error (nft-transfer-err error))
    ...
    (begin
        ...
        (if (< (len metaDataUrl) u10) (ok (var-get mint-counter))
            (let
                ...
                (unwrap! (nft-mint? crashpunks mintCounter tx-sender) failed-to-mint-err)
...
(define-public (place-bid (nftIndex uint) (nextBidAmount uint))
    (let
        (
            ...
            (currentAmount (unwrap! (get-current-bid-amount nftIndex currentBidIndex) bidding-error))
            (owner (unwrap! (nft-get-owner? crashpunks nftIndex) nft-not-owned-err))
            (seriesOriginal  (unwrap! (get series-original (map-get? nft-data {nft-index: nftIndex})) not-allowed))
        )
        ...
        (if (is-eq currentAmount u0)
            ...
            (if (is-eq closeType u1)
                (begin
                    ...
                    (unwrap! (nft-transfer? crashpunks nftIndex (unwrap! (nft-get-owner? crashpunks nftIndex) nft-not-owned-err) tx-sender) transfer-error)
                )
                (begin
...
(define-private (get-all-data (nftIndex uint))
    (let
        (
            (the-owner                  (unwrap-panic (nft-get-owner? crashpunks nftIndex)))
...
(define-private (internal-payment-split (nftIndex uint) (saleAmount uint) (payer principal) (seriesOriginal uint) (addresses (list 10 principal)) (shares (list 10 uint)) (secondaries (list 10 uint)) (saleCycle uint))
    (let
        ((split u0))
        (+ split (unwrap! (pay-royalty payer saleAmount (unwrap! (nft-get-owner? crashpunks nftIndex) payment-address-error) (unwrap! (if (is-eq saleCycle u1) (element-at shares u0) (element-at secondaries u0)) payment-share-error)) payment-share-error))
