---
title: "Trait bitcoin-memeorials"
draft: true
---
```
;; bitcoin-memeorials
...
(define-non-fungible-token bitcoin-memeorials uint)
...
(define-data-var total-price uint u2100000000)
(define-data-var artist-address principal 'SPC21M3PGDTNY691WAVCJ0TYEVWN3YZZ86EKAJZJ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWDRKVqHxV4QE4BLyqnHBkfpjUMkoEeRJKFHX7c6HSBi9/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-memeorials next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-memeorials token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-memeorials token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-memeorials token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-memeorials id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-memeorials id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-memeorials id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u7876245)
