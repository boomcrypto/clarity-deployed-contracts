...
(define-non-fungible-token stx-punks uint)
...
(define-data-var artist-address principal 'SPKMJBHD0P3DT5QPQ40WY3XPV6FV1PPP747AS7J2)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmP9v66P4NgF4heD9CUbHWoEXqYVZ1XXbp1tjBX4m5dHXG/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stx-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-punks id) (err ERR-NOT-FOUND)))
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? stx-punks (+ last-nft-id u0) 'SPKMJBHD0P3DT5QPQ40WY3XPV6FV1PPP747AS7J2))
      (map-set token-count 'SPKMJBHD0P3DT5QPQ40WY3XPV6FV1PPP747AS7J2 (+ (get-balance 'SPKMJBHD0P3DT5QPQ40WY3XPV6FV1PPP747AS7J2) u1))
      (try! (nft-mint? stx-punks (+ last-nft-id u1) 'SPKMJBHD0P3DT5QPQ40WY3XPV6FV1PPP747AS7J2))
      (map-set token-count 'SPKMJBHD0P3DT5QPQ40WY3XPV6FV1PPP747AS7J2 (+ (get-balance 'SPKMJBHD0P3DT5QPQ40WY3XPV6FV1PPP747AS7J2) u1))
