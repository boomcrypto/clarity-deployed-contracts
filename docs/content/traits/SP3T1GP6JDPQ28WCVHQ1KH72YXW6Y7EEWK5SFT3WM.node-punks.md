...
(define-non-fungible-token node-punks uint)
...
(define-data-var artist-address principal 'SP3T1GP6JDPQ28WCVHQ1KH72YXW6Y7EEWK5SFT3WM)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXbHWDhrr8VtV3ZbUtJXCRbLSaKWMyaZZaaoETGQboFks/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? node-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? node-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? node-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? node-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? node-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? node-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? node-punks id) (err ERR-NOT-FOUND)))
