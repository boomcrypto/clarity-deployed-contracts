...
(define-non-fungible-token mother-satoshi-2 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVf1yhzmZjvXuDD2uWvkSB1oQtbERrQCLvMS3v2UqeZ3i/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? mother-satoshi-2 next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? mother-satoshi-2 token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? mother-satoshi-2 token-id)))
