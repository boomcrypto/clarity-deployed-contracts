...
(define-non-fungible-token square-head uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaefXneFpv1WB1bChoeRWFDk5ETisokNgsaP8NWFeyjnm/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? square-head next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? square-head token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? square-head token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? square-head token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? square-head id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? square-head id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? square-head id) (err ERR-NOT-FOUND)))
