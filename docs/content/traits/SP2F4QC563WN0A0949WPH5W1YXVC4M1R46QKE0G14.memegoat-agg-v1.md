      ...
      (begin
        (let
          (
            ...
            (result1 (try! (contract-call? dex1 swap amount-after-fee (calc-slippage min-amt-4-swap1 slippage true) none route2 factors)))
      ...
      (if to-alex
        (let
          (
            ...
            (min-amt-4-swap1 (get-last-value quote))
            (result1 (try! (contract-call? dex1 swap amount-after-fee (calc-slippage min-amt-4-swap1 slippage false) none route2 factors)))
            (amt-4-swap2 (get-last-value result1))
...
(define-private (calc-slippage (amount uint) (slippage uint) (alex bool))
  (- amount (/ (* amount slippage) (if alex ONE_8 ONE_6)))
)
```
