...
(define-non-fungible-token condompunks uint)
...
;; Internal variables
(define-data-var mint-limit uint u1899)
(define-data-var last-id uint u1)
(define-data-var total-price uint u18000000)
(define-data-var artist-address principal 'SP23HGQPE5EEACMK000YDYAQ1H6NRXPV6V5V4GT9M)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdVJ21zXNZnGyJsuptLYLhL9WJ2rgXENYkwTerj8X8wkY/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? condompunks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? condompunks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? condompunks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? condompunks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? condompunks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? condompunks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? condompunks id) (err ERR-NOT-FOUND)))
