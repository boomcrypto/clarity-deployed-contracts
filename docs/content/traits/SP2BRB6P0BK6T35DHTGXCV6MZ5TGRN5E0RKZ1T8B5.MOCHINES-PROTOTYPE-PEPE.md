---
title: "Trait MOCHINES-PROTOTYPE-PEPE"
draft: true
---
```
;; nft Contract
...
(define-non-fungible-token Prototype_Pepe uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafkreic5xrregka6wne5yezkdsh3dukdw3rxuumte6qxfltmvdp3qypmhi")
...
(define-public (mint (recipient principal))
    (begin 
        ...
        (asserts! (< (var-get last-id) SUPPLY) (err ERR-NO-MORE-NFTS))
            ...
            (try! (nft-mint? Prototype_Pepe u1 tx-sender))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Prototype_Pepe token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Prototype_Pepe token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Prototype_Pepe token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Prototype_Pepe id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Prototype_Pepe id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Prototype_Pepe id) (err ERR-NOT-FOUND)))
