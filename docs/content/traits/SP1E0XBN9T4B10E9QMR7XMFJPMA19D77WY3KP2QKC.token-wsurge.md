...
(define-fungible-token wsurge)

(define-data-var token-name (string-ascii 32) "Surge Wrapper")
(define-data-var token-symbol (string-ascii 32) "wsurge")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wsurge.json"))
...
(define-read-only (get-base-token)
  'SP10X11JHXXGJV2055Z17K5WRW5G42S8AP6CF04S4.surge-stacks)

(define-read-only (get-base-decimals)
  (contract-call? 'SP10X11JHXXGJV2055Z17K5WRW5G42S8AP6CF04S4.surge-stacks get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP10X11JHXXGJV2055Z17K5WRW5G42S8AP6CF04S4.surge-stacks get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP10X11JHXXGJV2055Z17K5WRW5G42S8AP6CF04S4.surge-stacks transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
