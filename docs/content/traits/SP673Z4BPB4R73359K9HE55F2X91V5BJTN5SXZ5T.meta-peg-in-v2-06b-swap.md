---
title: "Trait meta-peg-in-v2-06b-swap"
draft: true
---
```
...
(define-public (finalize-peg-in-cross-swap 
  ...
  (let (
      ...
      (default-fee-tuple (if (is-none (get chain-id order-details)) { peg-out-fee: u0, peg-out-gas-fee: u0 } (try! (get-default-peg-out-fee pair-tuple)))))
    ...
    (and (is-some (get chain-id order-details)) (try! (update-peg-out-fee pair-tuple (var-get peg-out-fee) u0))) ;; % fee charged by relevant peg-out contract, but min/gas-fee charged upfront
        ...
        (and (is-some (get chain-id order-details)) (try! (update-peg-out-fee pair-tuple (get peg-out-fee default-fee-tuple) (get peg-out-gas-fee default-fee-tuple)))) 
...
(define-private (process-fee (tx (buff 32768)) (fee-idx (optional uint)))
  (match fee-idx some-value
    (let (
      ...
      (asserts! (>= (get value fee-output) (+ (get-peg-in-fee) (get-peg-out-gas-fee))) err-invalid-amount)  
    ...
    (begin 
      (asserts! (is-eq u0 (+ (get-peg-in-fee) (get-peg-out-gas-fee))) err-invalid-amount)  
      (ok { fee: u0 }))))
