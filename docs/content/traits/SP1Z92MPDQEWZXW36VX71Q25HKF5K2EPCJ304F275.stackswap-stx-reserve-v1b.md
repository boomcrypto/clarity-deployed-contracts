---
title: "Trait stackswap-stx-reserve-v1b"
draft: true
---
```
...
(define-data-var next-stacker-name (string-ascii 256) "l-stacker")
...
(define-public (add-tokens-to-stack (name (string-ascii 256)) (token-amount uint))
  (let (
    ...
    (asserts!
      (or
        (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "l-mortgager")))
...
(define-public (toggle-stacking (stacker-name (string-ascii 256)) (revoked-stacking bool) (ustx-collateral uint))
  (begin
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "l-mortgager"))) (err ERR-NOT-AUTHORIZED))
  ...
  (begin
    ...
    (let ((max-new-debt (- (unwrap-panic (calculate-lbtc-count token-string ustx-amount collateralization-ratio oracle)) current-debt)))
      (if (>= max-new-debt extra-debt)
        (match (print (as-contract (contract-call? .lbtc-token-v1b mint-for-dao extra-debt vault-owner)))
          success (ok true)
...
(map-set tokens-to-stack { stacker-name: "l-stacker" } { amount: u0 })
(map-set tokens-to-stack { stacker-name: "l-stacker-2" } { amount: u0 })
(map-set tokens-to-stack { stacker-name: "l-stacker-3" } { amount: u0 })
(map-set tokens-to-stack { stacker-name: "l-stacker-4" } { amount: u0 })
