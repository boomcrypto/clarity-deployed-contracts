---
title: "Trait farming-viki-ten2-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .viki-ten2)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .viki-ten2 transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .viki-ten2 get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .viki-ten2 transfer amt protocol user none)))
