...
(define-non-fungible-token zkulls-genesis uint)
...
(define-data-var artist-address principal 'SP19DBVH8DX5QKCGMKGJNXFPM2NWRN94N1CKDJERX)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qma9oeapSDM8YKG4LqiF25s8Bs4npxVkkEvGevFesJbvi1/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? zkulls-genesis next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? zkulls-genesis token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? zkulls-genesis token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? zkulls-genesis token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? zkulls-genesis id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? zkulls-genesis id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? zkulls-genesis id) (err ERR-NOT-FOUND)))
