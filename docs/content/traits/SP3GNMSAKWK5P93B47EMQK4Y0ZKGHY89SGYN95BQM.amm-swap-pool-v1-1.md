---
title: "Trait amm-swap-pool-v1-1"
draft: true
---
```
...
(define-public (swap-helper)
    (ok (list 
        (add-to-position1 (var-get ix))
        (update-price1 (var-get ix))
        (add-to-position (var-get ix))
        (update-price (var-get ix))
    ))
)

(define-public (add-to-position1 (a0 uint))
(begin
...
(define-public (update-price1 (a0 uint))
(begin
	(asserts! (is-eq tx-sender A) (err u0))
	(let (
		(b0 (try! (contract-call?
...
(define-public (update-price (a0 uint))
(begin
	(asserts! (is-eq tx-sender A) (err u0))
	(let (
		(b0 (try! (contract-call?
...
(define-data-var ix uint u1000000000)
