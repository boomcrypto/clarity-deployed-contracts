---
title: "Trait dogwifknife"
draft: true
---
```

;;  ---------------------------------------------------------
;; SIP-10 Fungible Tax Token Contract | Created on: deploy.stx.city
;; ---------------------------------------------------------
...
(define-fungible-token KNFE)
;; Initialize function to mint max supply to owner


;; Define the owner address
(define-data-var contract-owner principal tx-sender) 
(define-data-var treasury-address principal 'SP3CE7FKKBEWX36RHDMNE8WV5807GQA5SQBT8YXFV)
(define-data-var dev-wallet-address principal 'SP3GDFMZKJCF23P9C6CSPABA8PKNT12B9AFRGRJRT)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  ...
  ;; If sender is whitelisted or tax is off, perform transfer without tax
    (if (or is-whitelisted (var-get tax_off)) 
      (match (ft-transfer? KNFE amount sender recipient)
        success (ok success)
  ...
  ;; If sender is not whitelisted and tax is on, apply tax
      (let ((tax-amount (/ (* amount (var-get total-tax)) u100))
        ...
        (match (ft-transfer? KNFE transfer-amount sender recipient)
          success
...
(define-private (distribute-taxes (tax-amount uint) (sender principal))
  (let (
    ...
    (match (ft-transfer? KNFE treasury-tax sender (var-get treasury-address))
      treasury-success
        (match (ft-burn? KNFE burn-tax sender ) 
          burn-success
            (match (ft-transfer? KNFE dev-tax sender (var-get dev-wallet-address))
              dev-success (ok true)
...
(define-public (set-treasury-address (new-address principal))
  (begin
    (asserts! (is-eq tx-sender (var-get contract-owner)) (err ERR-NOT-OWNER)) ;; Check if the sender is the owner
...
(define-public (set-total-tax (new-tax uint))
  (begin
    (asserts! (is-eq tx-sender (var-get contract-owner)) (err ERR-UNAUTHORIZED)) ;; Only owner can change
...
;; Get treasury-address | dev-wallet-address | contract-owner | whitelist
...
(define-read-only (get-balance (user principal))
    (ok (ft-get-balance KNFE user)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply KNFE)))
...
;; DEFINE METADATA
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://gaia.hiro.so/hub/13JSUpxUZwAQ5r3HPJgSpz4kSTQHKHaezE/screenshot-2024-05-18-at-10-23-02-pm.json"))
(define-data-var token-name (string-ascii 32) "DogWifKnife")
(define-data-var token-symbol (string-ascii 32) "KNFE")
...
(begin
    ...
    (try! (ft-mint? KNFE u999000000000000 (var-get contract-owner)))
  (try! (ft-mint? KNFE u1000000000000 'SP1FQ3DQDR5N9HJX3XC5DNKFCG4DHH48EFJQV6QH0))
