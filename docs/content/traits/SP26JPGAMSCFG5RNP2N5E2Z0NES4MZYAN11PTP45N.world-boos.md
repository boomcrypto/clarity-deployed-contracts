...
(define-non-fungible-token world-boos uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmPtSSazQZYTm7aYaa3grQmQ4xTD7koTGNhLFA4rACVoV3/world-boos/")
...
(define-data-var public-target-block-height uint u64977)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u5000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? world-boos id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace world-boos
  (ok (nft-get-owner? world-boos id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? world-boos next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? world-boos id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? world-boos id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? world-boos id) ERR-NOT-FOUND))
