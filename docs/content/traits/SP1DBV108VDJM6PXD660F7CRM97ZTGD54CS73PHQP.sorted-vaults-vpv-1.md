---
title: "Trait sorted-vaults-vpv-1"
draft: true
---
```
;; Vaults Sorted 
...
(impl-trait .sorted-vaults-trait-vpv-1.sorted-vaults-trait)
...
(define-read-only (has-access)
  (begin
    (try! (contract-call? .controller-vpv-1 is-protocol-caller contract-caller))
    (ok true)
  )
)
