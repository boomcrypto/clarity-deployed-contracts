---
title: "Trait btc-peg-in-endpoint-v2-03"
draft: true
---
```
	...
	(let (
		...
		(match (validate-tx-cross-extra validation-data token-out-trait)
			...
			(begin
				...
				(as-contract (try! (contract-call? .cross-router-v2-02 route (get amount-net validation-data) (list 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-abtc) (list ) token-out-trait none { address: (get to order-details), chain-id: (get chain-id order-details) })))
...
(define-private (validate-tx-cross-extra (validation-data { order-details: { from: (buff 128), to: (buff 128), chain-id: (optional uint), token: principal, token-out: principal }, fee: uint, amount-net: uint }) (token-out-trait <ft-trait>))
	(let (
		...
		(try! (contract-call? .cross-router-v2-02 validate-route (get amount-net validation-data) (list 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-abtc) (list ) (get token-out order-details) none { address: (get to order-details), chain-id: (get chain-id order-details) }))
		(ok validation-data)))
