---
title: "Trait the-philosopher-btc-stx-nfts"
draft: true
---
```
;; the-philosopher-btc-stx-nfts
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token the-philosopher-btc-stx-nfts uint)
...
(define-data-var artist-address principal 'SP3T6REQNQGA2ET5J2V3KMPJM344HKT7FGV5Y891H)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTe1avLcZbb6UYtx28YErbiunWgEFvFxJsAtsB7UhmhVA/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-philosopher-btc-stx-nfts next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-philosopher-btc-stx-nfts token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-philosopher-btc-stx-nfts token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-philosopher-btc-stx-nfts token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-philosopher-btc-stx-nfts id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-philosopher-btc-stx-nfts id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-philosopher-btc-stx-nfts id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u63673)
...
;; Alt Minting Default
(define-data-var total-price-mega uint u1000)
