...
(define-constant message-domain-hash (sha256 (unwrap-panic (to-consensus-buff? { name: "BLAZE_PROTOCOL", version: "rc3", chain-id: chain-id }))))
  ...
  (let
    ...
    (match (secp256k1-recover? hash signature)
      recovered-pubkey (begin
          ...
          (principal-of? recovered-pubkey) ;; Return the original signer's principal
        )
