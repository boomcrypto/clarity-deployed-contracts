...
(define-private (swap-alex
    ...
    (let
        (
            (y-amount (unwrap! (as-contract (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.swap-helper-v1-03 swap-helper alex-token-x-trait alex-token-y-trait dx (some u0))) (err ERR-SWAP-ALEX-FAILED)))
        )
        (ok y-amount)
    )
)
    ...
    (let
        ...
        (let
            (
                ...
                (y-amount (unwrap-panic (swap-alex alex-token-x-trait alex-token-y-trait dx)))
    ...
    (let
        ...
        (let
            (
                ...
                (z-amount (unwrap-panic (swap-alex alex-token-x-trait alex-token-y-trait y-amount)))
                (balance-after (stx-get-balance (as-contract tx-sender)))
            )
            (try! (as-contract (stx-transfer? z-amount tx-sender save-tx-sender)))
            (print { message: "swap", balance-before: balance-before, balance-after: balance-after, dx: dx, y-amount: y-amount, z-amount: z-amount })
