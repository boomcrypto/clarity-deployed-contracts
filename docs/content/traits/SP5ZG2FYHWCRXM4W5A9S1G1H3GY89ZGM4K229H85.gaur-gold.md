...
(define-non-fungible-token gaur-gold uint)
...
(define-data-var artist-address principal 'SP5ZG2FYHWCRXM4W5A9S1G1H3GY89ZGM4K229H85)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmaiko5aBuF3hLKeyzc2nmH2jFxtA8DyqEjdDALZmi91xR/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? gaur-gold next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gaur-gold token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gaur-gold token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gaur-gold token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gaur-gold id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gaur-gold id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gaur-gold id) (err ERR-NOT-FOUND)))
