---
title: "Trait stackswap-farming-v1l"
draft: true
---
```
...
(define-read-only (get-farm-info (pool principal))
  (map-get? FarmInfo pool)
)

(define-private (get-farm-info-or-default (pool principal))
  (default-to 
    u0
    (map-get? FarmInfo pool))
)
...
(define-private (stake-tokens-at-cycle (user principal) (amountTokens uint) (pool <liquidity-token>) (startHeight uint) (lockPeriod uint))
  (let
    (
      ...
      (farm-info (get-farm-info-or-default (contract-of pool)))
      (commitment {
        stakerId: user,
