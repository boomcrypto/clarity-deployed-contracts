---
title: "Trait f-claim-liquidation-rewards-xstx-j-v1-11"
draft: true
---
```
...
(define-read-only (check-job)
  (let (
    ...
    (last-id (contract-call? .f-claim-liquidation-rewards-xstx-e-v1-11 get-last-total-reward-ids))
    (next-unlock-block (contract-call? .f-claim-liquidation-rewards-xstx-e-v1-11 get-next-unlock-block))
  )
...
(define-public (run-job)
  (begin
    ...
    (try! (contract-call? .lydian-dao-executor-v1-1 execute-proposal .f-claim-liquidation-rewards-xstx-e-v1-11))
    (ok true)
  )
)
