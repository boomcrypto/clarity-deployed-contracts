---
title: "Trait citycoins-bulls-dao"
draft: true
---
```
;; ***** START MINTER CODE ***** ;;
...
(define-public (mint (nft <nft-trait>) (comm <commission-trait>))
  (begin
    ...
    (try! (add-member tx-sender "citycoins-bulls-member"))
    (ok true)
  )
)
...
;; data maps and vars
(define-data-var dao-name (string-ascii 256) "CityCoins Bulls DAO")
...
(define-public (transfer (nft <nft-trait>) (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (try! (remove-member sender))
    (try! (add-member recipient "citycoins-bulls-member"))
