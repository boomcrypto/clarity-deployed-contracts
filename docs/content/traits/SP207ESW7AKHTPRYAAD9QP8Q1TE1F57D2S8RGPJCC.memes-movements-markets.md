---
title: "Trait memes-movements-markets"
draft: true
---
```
;; memes-movements-markets

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

;; Non Fungible Token, using sip-009
(define-non-fungible-token memes-movements-markets uint)
...
(define-data-var artist-address principal 'SPYTR6C4PXP1Q1ZABP3H9WQF6FM7KME4F00X2V8P)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRgsBweSTj23zPsCAPSKAYcr7NgGoXmvsYMq8xfvY2ZG2/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? memes-movements-markets next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? memes-movements-markets token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? memes-movements-markets token-id)))
