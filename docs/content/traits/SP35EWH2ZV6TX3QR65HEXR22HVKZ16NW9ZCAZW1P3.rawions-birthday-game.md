---
title: "Trait rawions-birthday-game"
draft: true
---
```
...
(define-non-fungible-token rawions-birthday-game uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmXLY9mVMom8ccAdZbG7Jr5jwumN2NxEeh49QQxaRacgKd/rawions-birthday-game/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP35EWH2ZV6TX3QR65HEXR22HVKZ16NW9ZCAZW1P3 )) 

(define-data-var private-sale-active bool true)
(define-data-var sale-active bool true)
(define-data-var private-target-block-height uint u69691)
(define-data-var public-target-block-height uint u70374)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rawions-birthday-game id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace rawions-birthday-game
  (ok (nft-get-owner? rawions-birthday-game id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? rawions-birthday-game next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? rawions-birthday-game id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rawions-birthday-game id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rawions-birthday-game id) ERR-NOT-FOUND))
...
(begin
  (try! (set-private-wl 'SP3SQ0GX8PT4QJV5ZJ70NE5VVV5N6WW5G8F77QXZN u10))
  (ok true)
)
