---
title: "Trait btc-peg-in-endpoint-v2-02"
draft: true
---
```
...
(define-read-only (create-order-cross-or-fail (order { from: (buff 128), to: (buff 128), chain-id: uint }))
	(ok (unwrap! (to-consensus-buff? { f: (get from order), r: (get to order), c: (int-to-ascii (get chain-id order)) }) err-invalid-input)))
(define-read-only (decode-order-cross-or-fail (order-script (buff 128)))
	(let (
			(op-code (unwrap-panic (slice? order-script u1 u2)))
			(offset (if (< op-code 0x4c) u2 u3))
			(raw-order (unwrap! (from-consensus-buff? { f: (buff 128), r: (buff 128), c: (string-ascii 40) } (unwrap-panic (slice? order-script offset (len order-script)))) err-invalid-input)))
		(ok { from: (get f raw-order), to: (get r raw-order), chain-id: (unwrap! (string-to-uint? (get c raw-order)) err-invalid-input) })))
...
(define-public (finalize-peg-in-cross
	...
	(let (
		...
		(as-contract (try! (contract-call? .token-abtc mint-fixed (+ (get fee validation-data) (get amount-net validation-data)) tx-sender)))
		(as-contract (try! (contract-call? .btc-bridge-registry-v2-01 set-peg-in-sent tx output-idx true)))		
		(match (validate-tx-cross-extra validation-data)
			ok-value
			(begin
				(and (> (get fee validation-data) u0) (as-contract (try! (contract-call? .token-abtc transfer-fixed (get fee validation-data) tx-sender (var-get fee-to-address) none))))
				(as-contract (try! (contract-call? .cross-peg-out-endpoint-v2-01 transfer-to-unwrap .token-abtc (get amount-net validation-data) (get chain-id order-details) (get to order-details))))
				(print (merge print-msg { success: true }))
				(ok true))
			err-value
			(begin
				;; TODO refund fee on fail (on peg-out)
				(as-contract (try! (contract-call? .btc-peg-out-endpoint-v2-01 request-peg-out-0 (get from order-details) (+ (get fee validation-data) (get amount-net validation-data)))))
...
(define-private (validate-tx-cross-extra (validation-data { order-details: { from: (buff 128), to: (buff 128), chain-id: uint }, fee: uint, amount-net: uint }))
	(begin
