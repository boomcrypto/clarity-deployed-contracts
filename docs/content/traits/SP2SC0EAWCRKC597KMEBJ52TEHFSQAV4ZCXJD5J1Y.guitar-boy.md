...
(define-non-fungible-token guitar-boy uint)
...
(define-constant MINT-LIMIT u125)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmdUvMBRzoiacnQJYZUV8ueDGrzMbXM7f1tAjcjL4RNcD8/guitar-boy/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP3PN1GRGNNV6KKS47XN34TJZ6MGBGC40G7KFEGMW )) 
...
(define-data-var public-target-block-height uint u71574)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u3000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? guitar-boy id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace guitar-boy
  (ok (nft-get-owner? guitar-boy id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? guitar-boy next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? guitar-boy id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? guitar-boy id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? guitar-boy id) ERR-NOT-FOUND))
