---
title: "Trait byzantion-anime-hybrdz"
draft: true
---
```
...
(define-non-fungible-token anime-hybrdz uint)
...
(define-constant MINT-LIMIT u111)
...
(define-data-var ipfs-full-metadata (string-ascii 80) "ipfs://Qmcq3S3D1n1L4iy4tHQZyr2vM5BSWknYdn5NRxB1NHETuY/anime_hybrdz_metadata.json")
(define-data-var ipfs-root (string-ascii 67) "ipfs://Qmcq3S3D1n1L4iy4tHQZyr2vM5BSWknYdn5NRxB1NHETuY/anime_hybrdz_")
(define-data-var artist-address principal 'SP2RZ0RMBDX454RPA8HGNMR216XD1PRE5VM7ZC0Y4)
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? anime-hybrdz next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? anime-hybrdz token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? anime-hybrdz token-id)))
