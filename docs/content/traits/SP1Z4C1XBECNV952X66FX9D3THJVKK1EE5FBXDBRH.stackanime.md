...
(define-non-fungible-token stackanime uint)
...
(define-data-var artist-address principal 'SP1Z4C1XBECNV952X66FX9D3THJVKK1EE5FBXDBRH)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZd1wMzpChjioQAiKhwWWoKX3bb2Vpmj3st24ZVKXBpsm/json/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stackanime next-id tx-sender) next-id)
      (unwrap! (nft-transfer? stackanime next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stackanime token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stackanime token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stackanime token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stackanime id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stackanime id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stackanime id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u30594)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u1240700000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u1977700000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u32000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u280000000)
