---
title: "Trait farming-wstx-odintkn-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-odintkn)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-odintkn transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-odintkn get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-odintkn transfer amt protocol user none)))
