---
title: "Trait farming-wstx-devcoin-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-devcoin)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-devcoin transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-devcoin get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-devcoin transfer amt protocol user none)))
