...
(define-public (add-swap-data
    ...
    (let
        ...
        (asserts! (is-eq (len decimals) u32) err-invalid-length)
        (asserts! (is-eq (len deposit-id) u32) err-invalid-length)
        ...
        (print {type: "add-swap-data", data-id: data-id, hub-contract: hub-contract, from-chain: from-chain, from-addr: from-addr, to-addr: to-addr, token: token, gov-id: gov-id, tx-hash: tx-hash, amount: amount, decimals: decimals, deposit-id: deposit-id})
...
(define-public (swap (data-id (buff 32)) (sigs (list 100 (buff 65))))
	(let
        ...
        (asserts! (is-eq (is-confirmed sig-hash) false) err-already-confirmed)
        (asserts! (is-eq (is-confirmed swap-hash) false) err-already-confirmed)
        (asserts! (is-eq gov-id (var-get governance-id)) err-invalid-gov-id)
        (asserts! (is-eq token (var-get origin-token)) err-invalid-token-address)
        (asserts! (is-eq (is-valid-chain from-chain) true) err-invalid-chain)
...
(define-public (request-swap (to-chain (buff 256)) (to-addr (buff 20)) (amount uint))
    (let
        (
            ...
            (chain-fee (get-chain-fee to-chain))
            (fee-governance (get-fee-governance))
            (request-amount (pay-tax amount))
        ...
        (asserts! (is-eq (>= (stx-get-balance contract-caller) chain-fee) true) err-invalid-amount)

        (if (is-eq chain-fee u0)
            true
            (try! (stx-transfer? chain-fee tx-sender fee-governance))
        )

        ;;emit SwapRequest(toChain, msg.sender, toAddr, token, tokenAddress, decimal, amount, depositCount, data)
        (print {type: "request-swap", to-chain: to-chain, from-addr: contract-caller, to-addr: to-addr, decimal: decimal, amount: request-amount, deposit-id: deposit-id})
        (var-set deposit-count (+ deposit-id u1))
        (burn amount contract-caller)
    )
)

(define-private (pay-tax (amount uint))
    (let
        (
            (decimal (var-get token-decimals))
            (tax-receiver (get-tax-receiver))
            (tax-rate (get-tax-rate))
            (deposit-id (var-get deposit-count))
        )
        (if (is-eq tax-rate u0)
            amount
            (let
                (
                    (tax-amount (/ (* amount tax-rate) u10000))
                    (remain (- amount tax-amount))
                )
                (if (is-eq tax-amount u0)
                    remain
                    (begin
                        (print {type: "request-swap", to-chain: 0x4f52424954, from-addr: contract-caller, to-addr: tax-receiver, decimal: decimal, amount: tax-amount, deposit-id: deposit-id})
                        (var-set deposit-count (+ deposit-id u1))
                        remain
                    )
                )
...
(define-read-only (is-valid-chain (chain (buff 256)))
    (contract-call? .gov-eth is-valid-chain chain)
)

(define-read-only (is-validator (validator principal))
	(contract-call? .gov-eth is-validator validator)
)

(define-read-only (get-required)
    (contract-call? .gov-eth get-min-confirmation)
)

(define-read-only (get-chain-fee (chain (buff 256)))
    (contract-call? .gov-eth get-chain-fee chain)
)

(define-read-only (get-fee-governance)
    (contract-call? .gov-eth get-fee-governance)
)

(define-read-only (get-tax-receiver)
    (contract-call? .gov-eth get-tax-receiver)
)

(define-read-only (get-tax-rate)
    (contract-call? .gov-eth get-tax-rate)
)
...
;; ================ constructor ================
(var-set name "Orbit Bridge Stacks Tether USD")
(var-set symbol "oUSDT")
(var-set token-decimals u18)
(var-set governance-id 0x50f408f4b0fb17bf4f5143de4bd95802410d00422f008e9deef06fb101a0f060)
(var-set origin-token 0xdac17f958d2ee523a2206206994597c13d831ec7)
