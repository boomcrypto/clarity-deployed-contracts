...
(define-fungible-token wjindo)

(define-data-var token-name (string-ascii 32) "Jindo Wrapper")
(define-data-var token-symbol (string-ascii 32) "wjindo")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wjindo.json"))
...
(define-read-only (get-base-token)
  'SPZXE76HAF3621C4RQGHJ26ENXGEZV2JWAV2Y10Z.jindo)

(define-read-only (get-base-decimals)
  (contract-call? 'SPZXE76HAF3621C4RQGHJ26ENXGEZV2JWAV2Y10Z.jindo get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SPZXE76HAF3621C4RQGHJ26ENXGEZV2JWAV2Y10Z.jindo get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SPZXE76HAF3621C4RQGHJ26ENXGEZV2JWAV2Y10Z.jindo transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
