---
title: "Trait multiple-bids"
draft: true
---
```
(define-private (collection-bid (collection-id (string-ascii 256)) (amount uint))
    (begin
        (try! (contract-call? .byzantion-market-v7 collection-bid collection-id amount))
        (ok true)
    )
)
...
(define-private (multiple-bid (collection-id (string-ascii 256)) (amount uint) (ids (list 5000 uint)) (trait (string-ascii 256)))
    (begin
        (try! (contract-call? .byzantion-market-v7 multiple-bid collection-id amount ids trait))
        (ok true)
    )
)
