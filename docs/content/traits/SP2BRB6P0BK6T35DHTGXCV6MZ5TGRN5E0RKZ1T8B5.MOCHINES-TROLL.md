...
(define-non-fungible-token Troll uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafkreie7b4txdm4vrdan3l5gxtb5dfpl2o4gliy7jzz4oks4nkq537aqxe")
...
(define-public (mint (recipient principal))
    (begin 
        ...
        (asserts! (< (var-get last-id) SUPPLY) (err ERR-NO-MORE-NFTS))
            ...
            (try! (nft-mint? Troll u1 tx-sender))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Troll token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Troll token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Troll token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Troll id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Troll id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Troll id) (err ERR-NOT-FOUND)))
