---
title: "Trait farming-abtc-susdt-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .abtc-susdt)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .abtc-susdt transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .abtc-susdt get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .abtc-susdt transfer amt protocol user none)))
