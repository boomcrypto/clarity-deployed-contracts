---
title: "Trait token-wvliabtc"
draft: true
---
```
(impl-trait 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.trait-sip-010.sip-010-trait)

(define-fungible-token wvliabtc)

(define-data-var token-name (string-ascii 32) "vLiaBTC Wrapper")
(define-data-var token-symbol (string-ascii 32) "wvliabtc")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wvliabtc.json"))
...
(define-read-only (get-base-decimals)
  (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.token-vliabtc get-decimals))
...
(define-read-only (get-balance (who principal))
  (ok (base-to-decimals (unwrap-panic (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.token-vliabtc get-balance who)))))

(define-read-only (get-total-supply)
  (ok (base-to-decimals (unwrap-panic (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.token-vliabtc get-total-supply)))))

(define-read-only (get-share (who principal))
  (ok (base-to-decimals (unwrap-panic (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.token-vliabtc get-share who)))))

(define-read-only (get-shares-to-tokens (amount uint))
  (base-to-decimals (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.token-vliabtc get-shares-to-tokens (decimals-to-base amount))))

(define-read-only (get-tokens-to-shares (amount uint))
  (base-to-decimals (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.token-vliabtc get-tokens-to-shares (decimals-to-base amount))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.token-vliabtc transfer (decimals-to-base amount) sender recipient memo)))

(define-public (mint (amount uint) (recipient principal))
  (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.token-vliabtc mint (decimals-to-base amount) recipient))

(define-public (burn (amount uint) (sender principal))
  (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.token-vliabtc burn (decimals-to-base amount) sender))
