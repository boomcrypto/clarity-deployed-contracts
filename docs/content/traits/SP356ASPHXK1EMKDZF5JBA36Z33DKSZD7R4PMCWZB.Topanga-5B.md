...
(define-non-fungible-token Topanga-5B uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafyreiczxm6g666yelxfy7ahmils3sjb674wyrxshbg2jrv326g7xzowp4/metadata.json")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Topanga-5B token-id)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Topanga-5B token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Topanga-5B token-id) false)))
...
(define-private (iterate-minting (not-to-be-used bool) (next-id uint))
  (if (<= next-id (var-get max-supply))
    (begin
      (unwrap! (nft-mint? Topanga-5B next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Topanga-5B id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Topanga-5B id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Topanga-5B id) (err err-not-found)))
...
(define-private (airdrop-iter (collector principal) (next-id uint)) 
  (if (<= next-id (var-get max-supply))
    (let (
      ...
      (unwrap! (nft-mint? Topanga-5B next-id collector) next-id)
