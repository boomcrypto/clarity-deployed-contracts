...
(define-non-fungible-token serpent-empire uint)
...
;; Internal variables
(define-data-var mint-limit uint u972)
(define-data-var last-id uint u1)
(define-data-var total-price uint u10000000)
(define-data-var artist-address principal 'SP3AM1WVTJXZVJ3N2TSEVMBTF948Y5AY56X1D4BWN)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWqZhvjynVjec3eM9QBBSR7vbatVVdT6UjHB5VUpMEzbs/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? serpent-empire next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? serpent-empire token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? serpent-empire token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? serpent-empire token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? serpent-empire id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? serpent-empire id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? serpent-empire id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u19801)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u25891000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u20361000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u108000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u2575000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u7500000000)
