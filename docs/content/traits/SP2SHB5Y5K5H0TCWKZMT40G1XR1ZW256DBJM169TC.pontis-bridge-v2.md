...
(define-constant ERR-WRONG-STX-PRICE (err u126))
(define-constant ERR-WRONG-RUNE-PRICE (err u126))

(define-constant STX_BUFF 0x535458)
(define-constant BTC_BUFF 0x425443)
...
(define-private (check-utxo-increment (network (buff 12)))
	(ok (if (is-eq network BTC_BUFF) (try! (get-and-increment-utxo)) 0x))
)
...
(define-public (bridge-out-runes (rune (buff 26)) (rune-contract <bridge-ft-trait>) (amount uint) (recipient (buff 64)) (network (buff 12)))
	(let
		(
			...
			(stx-price (default-to u0 (unwrap! (contract-call? .pontis-price-oracle get-price STX_BUFF) ERR-WRONG-STX-PRICE)))
			(rune-price (default-to u0 (unwrap! (contract-call? .pontis-price-oracle get-price rune) ERR-WRONG-STX-PRICE)))
			(key-utxo (try! (check-utxo-increment network)))
			(rune-token-by-id (unwrap! (get-rune-token-by-id rune) ERR-NO-SUCH-RUNE))
			(sender tx-sender)
			(percent-fee-to-pay-in-stx (/ (* percent-fee rune-price) stx-price))
		)
		(asserts! (> stx-price u0) ERR-WRONG-STX-PRICE)
		(asserts! (> rune-price u0) ERR-WRONG-RUNE-PRICE)
		...
		(try! (contract-call? .pontis-bridge-fee-manager-3 pay-stx-fee 
			(+ percent-fee-to-pay-in-stx (unwrap! (contract-call? .pontis-bridge-fee-manager-3 get-runes-base-fee network) ERR-BASE-FEE-UNWRAP))
		))

		(print {operation: "runes", key-utxo: key-utxo, percent-fee: percent-fee, percent-fee-in-stx: percent-fee-to-pay-in-stx, stx-price: stx-price, rune-price: rune-price, rune: rune, amount: amount, recipient: recipient, network: network})
		(as-contract (contract-call? rune-contract burn amount sender))
	)
)
...
(define-public (peg-out-ordinals-batch (ordinals-contract <bridge-nft-trait>) (ids (list 1000 uint)) (recipients (list 1000 (buff 64))))
	(let
		(
			(it (try! (get-iterator (len ids))))
			(key-utxo (try! (check-utxo-increment BTC_BUFF))) ;; Ordinals can be only wrapped and unwrapped
		)
