---
title: "Trait liquid-staked-goat"
draft: true
---
```
;; Title: Liquid Staked Goat
;; Author: rozar.btc
;; Synopsis:
;; This contract implements a liquid staking solution for Goat.
;; It provides users with liquid tokens (sGOAT) that represent staked Goat. 
...
(define-data-var token-name (string-ascii 32) "Liquid Staked Goat")
(define-data-var token-symbol (string-ascii 10) "sGOAT")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/liquid-staked-goat.json"))
...
(define-public (stake (amount uint))
	(begin
		(let
			...
			(try! (contract-call? 'SP2F4QC563WN0A0949WPH5W1YXVC4M1R46QKE0G14.memegoatstx transfer amount sender contract none))
...
(define-public (unstake (amount uint))
	(begin
		(let
			...
			(try! (as-contract (contract-call? 'SP2F4QC563WN0A0949WPH5W1YXVC4M1R46QKE0G14.memegoatstx transfer amount-token contract sender none)))
		)
		(ok true)
	)
)
...
(define-public (deposit (amount uint))
    (contract-call? 'SP2F4QC563WN0A0949WPH5W1YXVC4M1R46QKE0G14.memegoatstx transfer amount tx-sender contract none)
)
...
(define-read-only (get-total-in-pool)
	(unwrap-panic (contract-call? 'SP2F4QC563WN0A0949WPH5W1YXVC4M1R46QKE0G14.memegoatstx get-balance contract))
)
