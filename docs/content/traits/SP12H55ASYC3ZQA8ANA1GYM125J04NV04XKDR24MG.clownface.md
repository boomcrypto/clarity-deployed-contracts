...
(define-non-fungible-token clownface uint)
...
(define-data-var artist-address principal 'SP12H55ASYC3ZQA8ANA1GYM125J04NV04XKDR24MG)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZ9zHnDyWeDg8qTZMdSm3w5VVjbE4he3hGfVhLT3rxFXW/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? clownface next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? clownface token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? clownface token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? clownface token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? clownface id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? clownface id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? clownface id) (err ERR-NOT-FOUND)))
