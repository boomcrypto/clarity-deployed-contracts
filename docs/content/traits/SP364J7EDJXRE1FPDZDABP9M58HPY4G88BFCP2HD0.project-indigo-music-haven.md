---
title: "Trait project-indigo-music-haven"
draft: true
---
```
;; project-indigo-music-haven
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token project-indigo-music-haven uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSVrG3WUE6ZNa3dRnaPhyHJjoyj62Uo6Ve8rDkExJ8N2Q/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? project-indigo-music-haven next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? project-indigo-music-haven token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? project-indigo-music-haven token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? project-indigo-music-haven token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? project-indigo-music-haven id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? project-indigo-music-haven id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? project-indigo-music-haven id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mega uint u900)
