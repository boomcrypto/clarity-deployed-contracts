...
;; punk with rank 2
;; (3-1)/1000=0.002
;; punk has rank 2 so (1000+1)-2 = 999*0.002 = 1,998 + 1 = 2.998
;; punk has rank 1000 so (1000+1)-1000 = 1*0.002 = 0.002 = 1.002 
;; minute value = 1.002/24/60=0.00069
;; minute in micro = 0.00069 * 1000000 = 690 microToken
...
(define-private (get-token-minted (staking-id uint))
  (let (
    ...
    )
    {
      ...
      bonus: nft-bonus,
      minted: nft-minted,
      claimed: (get claimed staking-mapped),
      address-bonus: nft-bonus-address-bonus,
      nft-id: (get nft-id staking-mapped),
      nft-collection: (get nft-collection staking-mapped),
      owner: (get nft-collection staking-mapped),
      rank: rank
    }
  )
)
(define-public (unstake (staking-id uint) (collection <nft-trait>))
  (let (
      (owner tx-sender)
      ...
      (just-claimed (get claimed minting-data))
      (total-to-claim (+ (+ minted-value bonus-value) address-bonus) )
    )
    ...
    (
      if
      (> (- total-to-claim just-claimed) u0)
      (is-ok (as-contract (contract-call? 'SP2A665S3H6FVMZSY4VJ17ESXX21CGS0A32984B1H.romatoken mint owner (- total-to-claim just-claimed) )))
      true
    )
...
(define-public (claim (staking-id uint) (collection <nft-trait>))
  (let (
    ...
    (map-set stacking-map staking-id (merge (unwrap-panic (map-get? stacking-map staking-id)) {claimed: (+ (- total-to-claim just-claimed) just-claimed)})
    )
    (ok true)
  )
)
...
(define-read-only (current-staking-ids (address principal))
  (ok (get nfts (unwrap-panic (map-get? stacking-addresses address) ) ))
  )

(define-read-only (get-staking-status-multiple (ids (list 20 uint) ))
  (begin
    (ok (map get-token-minted ids))
  )
)
