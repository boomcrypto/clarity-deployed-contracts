...
(define-fungible-token metadata-coin)
...
(define-constant TOKEN_URI u"https://gaia.hiro.so/hub/16afad9EURtsEbUb5EQDpLPe1LoUbFbhHc/test_metadata.json") ;; utf-8 string with token metadata host
(define-constant TOKEN_NAME "Metadata Coin")
(define-constant TOKEN_SYMBOL "META")
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance metadata-coin who))
)

;; SIP-010 function: Returns the total supply of fungible token
(define-read-only (get-total-supply)
  (ok (ft-get-supply metadata-coin))
)
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? metadata-coin amount recipient)
  )
)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? metadata-coin amount sender recipient))
