...
(define-non-fungible-token three-card-box uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmanKfzYyYxJHoDvT6z8uKkvkup7ST9cucvG2BdtjZfo1B/three-card-box/")
...
(define-data-var public-target-block-height uint u63620)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? three-card-box id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace three-card-box
  (ok (nft-get-owner? three-card-box id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? three-card-box next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? three-card-box id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? three-card-box id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? three-card-box id) ERR-NOT-FOUND))
...
(define-data-var nyc-cost-per-mint uint u150000000)
