---
title: "Trait farming-velar-aeusdc-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .velar-aeusdc)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .velar-aeusdc transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .velar-aeusdc get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .velar-aeusdc transfer amt protocol user none)))
