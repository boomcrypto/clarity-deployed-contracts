---
title: "Trait googlier-swap-v2-1"
draft: true
---
```
...
(use-trait swap-token .googlier-swap-trait-v1.swap-trait)
...
(define-public (toggle-swap-shutdown)
  (begin
    (asserts! (is-eq contract-caller (contract-call? .googlier-dao get-guardian-address)) (err ERR-NOT-AUTHORIZED))
...
(define-read-only (shutdown-not-activated)
  (and
     (not (unwrap-panic (contract-call? .googlier-dao get-emergency-shutdown-activated)))
...
(define-public (add-to-position (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (swap-token-trait <swap-token>) (x uint) (y uint))
  (let
    ...
    (if (is-eq token-x .wrapped-stx-token)
      (begin
        (try! (contract-call? .googlier-dao mint-token .wrapped-stx-token x tx-sender))
    ...
    (if (is-eq token-y .wrapped-stx-token)
      (begin
        (try! (contract-call? .googlier-dao mint-token .wrapped-stx-token y tx-sender))
...
(define-public (migrate-create-pair (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (swap-token-trait <swap-token>) (pair-name (string-ascii 32)) (shares-total uint))
  ...
  (let (
    ...
    (pair-data {
      ...
      fee-balance-x: u0,
      fee-balance-y: u0,
      fee-to-address: (some (contract-call? .googlier-dao get-payout-address)),
  ...
  (let
    ...
    (if (is-eq token-x .wrapped-stx-token)
      (begin
        ...
        (try! (as-contract (contract-call? .googlier-dao burn-token .wrapped-stx-token withdrawal-x tx-sender)))
      )
    ...
    (if (is-eq token-y .wrapped-stx-token)
      (begin
        ...
        (try! (as-contract (contract-call? .googlier-dao burn-token .wrapped-stx-token withdrawal-y tx-sender)))
      )
  ...
  (let (
    ...
    (if (is-eq token-x .wrapped-stx-token)
      (begin
        ...
        (try! (contract-call? .googlier-dao mint-token .wrapped-stx-token dx tx-sender))
      )
      false
    )
    ...
    (if (is-eq token-y .wrapped-stx-token)
      (begin
        (try! (contract-call? .googlier-dao burn-token .wrapped-stx-token dy tx-sender))
...
(define-public (swap-y-for-x (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (dy uint) (min-dx uint))
  (let (
    ...
    (if (is-eq token-y .wrapped-stx-token)
      (begin
        (try! (contract-call? .googlier-dao mint-token .wrapped-stx-token dy tx-sender))
    ...
    (if (is-eq token-x .wrapped-stx-token)
      (begin
        (try! (contract-call? .googlier-dao burn-token .wrapped-stx-token dx tx-sender))
  ...
  (let (
    ...
    (if (is-eq token-x .wrapped-stx-token)
      (begin
        ...
        (try! (as-contract (contract-call? .googlier-dao burn-token .wrapped-stx-token fee-x tx-sender)))
      )
    ...
    (if (is-eq token-y .wrapped-stx-token)
      (begin
        ...
        (try! (as-contract (contract-call? .googlier-dao burn-token .wrapped-stx-token fee-y tx-sender)))
      )
