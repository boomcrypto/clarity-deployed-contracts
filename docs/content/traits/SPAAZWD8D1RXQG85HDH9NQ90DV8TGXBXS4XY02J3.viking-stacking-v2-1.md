---
title: "Trait viking-stacking-v2-1"
draft: true
---
```
...
(define-read-only (get-total-staked (stake-id uint))
  (match (map-get? stake-pool-map {stake-id: stake-id})
    entry (ok (get total-staked entry))
    (err u0) 
  )
)
...
(define-public (create-pool 
  ...
  (let
    ...
    (try! (contract-call? reward-token transfer reward-amount sender 'SPAAZWD8D1RXQG85HDH9NQ90DV8TGXBXS4XY02J3.viking-stacking-vault-v1 none)) 
...
(define-private (do-withdraw-stake (stake-id uint) (user-addr principal) (amount uint) (stake-token <ft-trait>))
  (begin 
    (let
      ...
      (as-contract (try! (contract-call? 'SPAAZWD8D1RXQG85HDH9NQ90DV8TGXBXS4XY02J3.viking-stacking-vault-v1 transfer-ft stake-token amount user-addr)))   
...
(define-private (do-claim-reward (stake-id uint) (user-addr principal) (reward-token <ft-trait>))
  (begin 
    (let
      ...
      ;; transfer token from vault
      (as-contract (try! (contract-call? 'SPAAZWD8D1RXQG85HDH9NQ90DV8TGXBXS4XY02J3.viking-stacking-vault-v1 transfer-ft reward-token reward user-addr)))   
...
(define-private (move-stake (address {user: principal, stake: uint, stake-token: <ft-trait>}) (stake-id uint))
  (let
    (
      (sender (get user address))
      (amount (get stake address))
      (token (get stake-token address))
    )
    (unwrap-panic (do-stake stake-id sender amount token))
    stake-id
  )
)
