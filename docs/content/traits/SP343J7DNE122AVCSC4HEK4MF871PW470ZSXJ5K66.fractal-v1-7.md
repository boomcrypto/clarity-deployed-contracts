...
(define-public (defractalize-nft (nftContract <sip9>) (originalNftId uint) (fractalNftId uint))
  (let (
    (userId (get-or-create-user-id tx-sender))
    ...
    (begin 
      ...
      (try! (transfer-nft-out-vault nftContract originalNftId))
...
(define-public (transfer 
    ...
    (begin
      ...
      (asserts! (<= amount (unwrap-panic 
        (get-fractal-balance tx-sender nftId))) (err FRACTAL_COUNT_TOO_LOW)
      )
...
(define-constant USER_DOES_NOT_EXIST u201)
(define-constant NFT_DOES_NOT_EXIST u202)
(define-constant USER_OWNED_TOKENS_DOES_NOT_EXIST u203)

(define-constant FRACTAL_COUNT_TOO_LOW u301)

(define-constant UNAUTHORIZED u401)
(define-constant UNAUTHORIZED_CONTRACT u402)
...
;; @desc Retrieves the last Fractal nftId that was fractalized

(define-read-only (get-last-token-id) (ok (- (var-get nftTip) u1)))

;; @desc Retrieves the last userId
