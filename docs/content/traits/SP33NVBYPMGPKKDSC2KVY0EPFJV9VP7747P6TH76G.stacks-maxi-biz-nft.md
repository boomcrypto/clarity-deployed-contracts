---
title: "Trait stacks-maxi-biz-nft"
draft: true
---
```
;; stacks-maxi-biz-nft
...
(define-non-fungible-token stacks-maxi-biz-nft uint)
...
(define-data-var artist-address principal 'SP33NVBYPMGPKKDSC2KVY0EPFJV9VP7747P6TH76G)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZ2qbinwU7Pb6XUtsoveEtbmEmXTHLxCsZbhaen3LbMZc/json/")
(define-data-var mint-paused bool true)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u2300)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-maxi-biz-nft next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-maxi-biz-nft token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-maxi-biz-nft token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-maxi-biz-nft token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-maxi-biz-nft id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-maxi-biz-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-maxi-biz-nft id) (err ERR-NOT-FOUND)))
