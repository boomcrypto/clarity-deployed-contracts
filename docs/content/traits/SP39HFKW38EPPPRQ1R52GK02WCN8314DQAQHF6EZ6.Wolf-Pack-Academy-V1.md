---
title: "Trait Wolf-Pack-Academy-V1"
draft: true
---
```
...
(define-non-fungible-token Wolf-Pack-Academy-V1 uint)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint?  Wolf-Pack-Academy-V1 next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer?  Wolf-Pack-Academy-V1 token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner?  Wolf-Pack-Academy-V1 token-id)))
