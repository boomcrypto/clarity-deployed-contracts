---
title: "Trait vault-upgrade-v2-1"
draft: true
---
```
;; title: vault-vpv-5
...
;; Vault Trait 
(impl-trait .vault-trait-vpv-5.vault-trait)
...
(define-public (new-vault-wrapper (collateral-sbtc uint) (loan-bsd uint) (interest uint) (hint (optional uint)) (bsd <bsd-trait>) (sbtc <sbtc-trait>) (oracle <oracle-trait>) (registry <registry-trait>) (stability <stability-trait>) (sorted-vaults <sorted-vaults-trait>))
    (let 
        (
            ;; validations
            ...
            (valid-stability (try! (contract-call? .controller-vpv-5 check-approved-contract "stability" (contract-of stability))))
...
;; mint loan
(define-public (mint-loan-wrapper (vault-id uint) (borrow-bsd uint) (bsd <bsd-trait>) (sbtc <sbtc-trait>) (oracle <oracle-trait>) (registry <registry-trait>) (stability <stability-trait>))
    (let 
        (
            ;; checks
            (valid-principal (try! (contract-call? .controller-vpv-5 verify-principal contract-caller)))
            ...
            (current-ratio (try! (contract-call? registry calculate-collateral-ratio (+ vault-total-debt borrow-bsd) vault-total-collateral sbtc-price)))

            ;; derived values
            (accrued-interest (unwrap-panic (contract-call? .registry-vpv-5 get-vault-accrued-interest vault-id)))
            (loan-fees (unwrap! (calculate-loan-fee borrow-bsd recovery-mode registry) ERR_CALCULATE_FEE))
            (final-loan-bsd (+ borrow-bsd loan-fees accrued-interest))
...
;; repay-loan
(define-public (repay-loan-wrapper (vault-id uint) (repay-bsd uint) (bsd <bsd-trait>) (sbtc <sbtc-trait>) (oracle <oracle-trait>) (registry <registry-trait>) (stability <stability-trait>) (sorted-vaults <sorted-vaults-trait>))
    (let 
        (
            ;; validations
            (valid-principal (try! (contract-call? .controller-vpv-5 verify-principal contract-caller)))
        ...
        (if (is-eq future-total-debt-amount u0)
            ...
            (begin
                ...
                ;; burn the bsd
                (try! (contract-call? bsd protocol-burn tx-sender adjusted-total-repay-amount))
            ...
            (begin
                ;; burn the bsd
                (try! (contract-call? bsd protocol-burn tx-sender repay-bsd))
...
;; add-collateral
(define-public (add-collateral-wrapper (vault-id uint) (collateral-sbtc uint) (bsd <bsd-trait>) (sbtc <sbtc-trait>) (oracle <oracle-trait>) (registry <registry-trait>) (stability <stability-trait>))
    (let 
        (
            ;; checks
            (valid-principal (try! (contract-call? .controller-vpv-5 verify-principal contract-caller)))
...
;; withdraw-collateral
(define-public (withdraw-collateral-wrapper (vault-id uint) (remove-amount uint) (bsd <bsd-trait>) (sbtc <sbtc-trait>) (oracle <oracle-trait>) (registry <registry-trait>) (stability <stability-trait>) (sorted-vaults <sorted-vaults-trait>))
    (let 
        (
            ;; checks
            (valid-principal (try! (contract-call? .controller-vpv-5 verify-principal contract-caller)))
            ...
            (vault-collateral-minimum-usd (mul-perc vault-loan-minimum PRECISION vault-threshold))
        )
...
(define-public (update-interest-rate-wrapper (vault-id uint) (interest uint) (bsd <bsd-trait>) (sbtc <sbtc-trait>) (oracle <oracle-trait>) (registry <registry-trait>) (stability <stability-trait>))
    (let 
        (
            ;; checks
            (valid-principal (try! (contract-call? .controller-vpv-5 verify-principal contract-caller)))
...
;; protocol-transfer
(define-public (protocol-transfer (to principal) (amount uint) (sbtc <sbtc-trait>) (registry <registry-trait>))
     (let 
        (
            (valid-registry (try! (contract-call? .controller-vpv-5 check-approved-contract "registry" (contract-of registry))))
            (valid-sbtc (try! (contract-call? .controller-vpv-5 check-approved-contract "sbtc" (contract-of sbtc))))
        ...
        (as-contract (contract-call? sbtc transfer amount tx-sender to none))
    )
)
...
(define-public (process-liquidations (vaults (list 10 uint)) (bsd <bsd-trait>) (sbtc <sbtc-trait>) (oracle <oracle-trait>) (registry <registry-trait>) (stability <stability-trait>) (sorted-vaults <sorted-vaults-trait>))
    (let
        (
            (valid-stability (try! (contract-call? .controller-vpv-5 check-approved-contract "stability" (contract-of stability))))
            (valid-registry (try! (contract-call? .controller-vpv-5 check-approved-contract "registry" (contract-of registry))))
    ...
    (match update-response
        helper-tuple
            (let 
                ...
                (ok {index: (+ index u1), updated: (unwrap! (as-max-len? (append updated vault-id) u10) ERR_LIST_OVERFLOW), hints: hints, registry: registry, sorted-vaults: sorted-vaults})    
            )
        err-resp
        (err err-resp)
    )
...
(define-private (accrue-vault (vault-id uint) (bsd <bsd-trait>) (registry <registry-trait>))
    (let 
        (
            ...
            (vault-accrued-bsd (unwrap-panic (contract-call? registry get-vault-accrued-interest vault-id)))
            (vault-data (try! (contract-call? registry get-vault vault-id)))
            (current-epoch-rate (get interest-rate vault-data))
            (last-interest-accrued (unwrap-panic (get last-interest-accrued vault-data)))
            (is-keeper (unwrap-panic (contract-call? .controller-vpv-5 is-keeper tx-sender)))
        )
...
(define-private (attempt-liquidate-vault (vault-id uint) (sbtc-price uint) (bsd <bsd-trait>) (sbtc <sbtc-trait>) (registry <registry-trait>) (stability <stability-trait>) (sorted-vaults <sorted-vaults-trait>))
    (let 
        (
            ...
            (calc-stability-pool-aggregate-sbtc (div-to-fixed-precision stability-pool-aggregate-bsd PRECISION current-sbtc-price))
            ...
            (is-keeper (unwrap! (contract-call? .controller-vpv-5 is-keeper tx-sender) ERR_NO_KEEPER))
            (is-paused (get is-paused protocol-data))
            (stability-pool-aggregate-sbtc (if 
                                                (and 
                                                    (< stability-pool-aggregate-bsd vault-total-debt)
                                                    (> calc-stability-pool-aggregate-sbtc vault-total-collateral) 
                                                )
                                                    vault-total-collateral
                                                    calc-stability-pool-aggregate-sbtc
                                            )
            )
        )
        ...
        (if (> accrued-vault-loan-bsd stability-pool-aggregate-bsd)
            ...
            (let
                (
                    ...
                    (vault-share (- (* u100 one-percent) stability-share))
                ...
                (if (is-eq stability-pool-aggregate-bsd u0)
                    ...
                    (begin
                        ...
                        (try! (contract-call? stability vault-manager-burn stability-pool-aggregate-bsd bsd registry))
            ...
            (begin 
                ...
                (try!  (contract-call? stability vault-manager-burn accrued-vault-loan-bsd bsd registry))
