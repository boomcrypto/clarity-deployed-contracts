---
title: "Trait monkey-d-luffy-rc11"
draft: true
---
```
...
(define-public (swap-a-to-b (amount uint) (recipient principal))
    (let (
        ...
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token-subnet-rc6 transfer amount tx-sender CONTRACT none))
        ;; Transfer token B to sender
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.welsh-token-subnet-rc2 transfer (get dy delta) CONTRACT recipient none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint) (recipient principal))
    (let (
        ...
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.welsh-token-subnet-rc2 transfer amount tx-sender CONTRACT none))
        ;; Transfer token A to sender
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token-subnet-rc6 transfer (get dy delta) CONTRACT recipient none)))
        (ok delta)))

(define-public (add-liquidity (amount uint) (recipient-a principal) (recipient-b principal))
    (let (
        (delta (get-liquidity-quote amount)))
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token-subnet-rc6 transfer (get dx delta) tx-sender CONTRACT none))
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.welsh-token-subnet-rc2 transfer (get dy delta) tx-sender CONTRACT none))
...
(define-public (remove-liquidity (amount uint) (recipient-a principal) (recipient-b principal))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token-subnet-rc6 transfer (get dx delta) CONTRACT recipient-a none)))
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.welsh-token-subnet-rc2 transfer (get dy delta) CONTRACT recipient-b none)))
        (ok delta)))
...
(define-public (x-swap-a-to-b (amount uint) (signature (buff 65)) (uuid (string-ascii 36)) (recipient principal))
    (let ((delta (get-swap-quote amount (some OP_SWAP_A_TO_B))))
...
(define-public (x-swap-b-to-a (amount uint) (signature (buff 65)) (uuid (string-ascii 36)) (recipient principal))
    (let ((delta (get-swap-quote amount (some OP_SWAP_B_TO_A))))
        ;; Transfer tokens to contract from signer
