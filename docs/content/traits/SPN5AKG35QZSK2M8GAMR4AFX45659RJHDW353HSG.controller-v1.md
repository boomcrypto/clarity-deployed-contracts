...
(define-data-var max-reward-per-window uint u3)
(define-data-var update-window uint (* u6 u7))
...
(define-public (log-reward (reward-usdh uint))
  (let (
    (total-usdh-supply (unwrap-panic (contract-call? .usdh-token-v1 get-total-supply)))
    (total-usdh-supply-staked (unwrap-panic (contract-call? .usdh-token-v1 get-balance .staking-v1)))
  )
    ...
    (if (> reward-usdh u0)
      (begin
        ...
        (try! (contract-call? .usdh-token-v1 mint-for-protocol reward-usdh .staking-v1))
      )
      true
    )
