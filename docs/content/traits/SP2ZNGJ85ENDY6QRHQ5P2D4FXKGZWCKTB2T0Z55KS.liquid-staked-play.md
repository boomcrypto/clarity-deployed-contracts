---
title: "Trait liquid-staked-play"
draft: true
---
```
;; Title: Liquid Staked Play
;; Author: rozar.btc
;; Synopsis:
;; This contract implements a liquid staking solution for Play.
;; It provides users with liquid tokens (sPLAY) that represent staked Play. 
...
(define-data-var token-name (string-ascii 32) "Liquid Staked Play")
(define-data-var token-symbol (string-ascii 10) "sPLAY")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/liquid-staked-play.json"))
...
(define-public (stake (amount uint))
	(begin
		(let
			...
			(try! (contract-call? 'SP1PW804599BZ46B4A0FYH86ED26XPJA7SFYNK1XS.play transfer amount sender contract none))
...
(define-public (unstake (amount uint))
	(begin
		(let
			...
			(try! (as-contract (contract-call? 'SP1PW804599BZ46B4A0FYH86ED26XPJA7SFYNK1XS.play transfer amount-token contract sender none)))
		)
		(ok true)
	)
)
...
(define-public (deposit (amount uint))
    (contract-call? 'SP1PW804599BZ46B4A0FYH86ED26XPJA7SFYNK1XS.play transfer amount tx-sender contract none)
)
...
(define-read-only (get-total-in-pool)
	(unwrap-panic (contract-call? 'SP1PW804599BZ46B4A0FYH86ED26XPJA7SFYNK1XS.play get-balance contract))
)
