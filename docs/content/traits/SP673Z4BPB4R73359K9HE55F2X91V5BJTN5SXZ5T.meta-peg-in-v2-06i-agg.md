---
title: "Trait meta-peg-in-v2-06i-agg"
draft: true
---
```
...
(define-read-only (create-order-agg-or-fail (order { from: (buff 128), to: (buff 128), token-out: principal, min-amount-out: (optional uint), chain-id: (optional uint), dest-chain-id: uint, swap-token-in: principal, swap-token-out: principal, expiry: uint }))
	(ok (unwrap! (to-consensus-buff? { f: (get from order), r: (get to order), o: (get token-out order), m: (unwrap-panic (as-max-len? (match (get min-amount-out order) some-value (int-to-ascii some-value) "none") u40)), c: (unwrap-panic (as-max-len? (match (get chain-id order) some-value (int-to-ascii some-value) "none") u40)), d: (unwrap-panic (as-max-len? (int-to-ascii (get dest-chain-id order)) u40)), s: (get swap-token-in order), t: (get swap-token-out order), e: (unwrap-panic (as-max-len? (int-to-ascii (get expiry order)) u40)) }) err-invalid-input)))
...
(define-public (finalize-peg-in-agg 
  ...
  (let (
    ...
    (and (> amt-net u0) (if (get no-burn token-details) 
      (let (
          (token-balance (unwrap-panic (contract-call? token-in-trait get-balance-fixed .meta-peg-out-endpoint-v2-04)))
          (balance-check (asserts! (<= amt-net token-balance) err-invalid-amount))
          (peg-out-balance (- token-balance amt-net)))
        (as-contract (try! (contract-call? .meta-peg-out-endpoint-v2-04 transfer-all-to tx-sender token-in-trait)))
        (and (> peg-out-balance u0) (as-contract (try! (contract-call? token-in-trait transfer-fixed peg-out-balance tx-sender .meta-peg-out-endpoint-v2-04 none)))))
...
(define-private (validate-tx-agg-extra
  (validation-data { 
    ...
    order-details: { from: (buff 128), to: (buff 128), token-out: principal, min-amount-out: (optional uint), chain-id: (optional uint), dest-chain-id: uint, swap-token-in: principal, swap-token-out: principal, expiry: uint }}))  
	(let (
      ...
      (pair-details (get pair-details validation-data))
		...
		(try! (contract-call? .cross-peg-out-v2-01c-agg validate-transfer-to-swap tx-sender amount-in-fixed (get swap-token-in order-details) (get swap-token-out order-details) dest-chain-id success-settle fail-settle))
		(ok { amount-in-fixed: amount-in-fixed, token-in: (get swap-token-in order-details), token-out: (get swap-token-out order-details), min-amount-out: min-amount-out, dest-chain-id: dest-chain-id, expiry: (get expiry order-details), success-settle: success-settle, fail-settle: fail-settle })))
```
