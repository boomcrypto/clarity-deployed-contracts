...
(define-read-only (verify-signature
  ...
  (let ((hash (unwrap! (make-message-hash amount to nonce) false)))
    (verify-hash-signature hash signature sender)
  )
)
...
(define-read-only (verify-hash-signature
    (hash (buff 32))
    (signature (buff 65))
    (signer principal)
  )
