---
title: "Trait crypto-one-hundred"
draft: true
---
```
;; crypto-one-hundred
...
(define-non-fungible-token crypto-one-hundred uint)
...
(define-data-var artist-address principal 'SP1C8C11SD4XAB30SZH0PTRCDK9B52R2CDMDJGXC8)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbyTS7uqwLAGKfBzHTeGxfm2H1mgw8GQ7VALWrkcVhKR1/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? crypto-one-hundred next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? crypto-one-hundred token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? crypto-one-hundred token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? crypto-one-hundred token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? crypto-one-hundred id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? crypto-one-hundred id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? crypto-one-hundred id) (err ERR-NOT-FOUND)))
