...
(define-non-fungible-token public-001 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSwJNnSCG6MzCo9LDMWRKriodZn7kM1fFzSKt1Kg6H7J2/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? public-001 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? public-001 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? public-001 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? public-001 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? public-001 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? public-001 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? public-001 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u15662)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u15183)
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u52700)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u21000000)
