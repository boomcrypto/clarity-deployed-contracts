---
title: "Trait pontis-config-setter"
draft: true
---
```
(use-trait bridge-config-trait-v1 .bridge-config-trait-v1.bridge-config-trait-v1)

(define-constant ERR-WRONG-ARRAY-SIZE (err u10))
(define-constant ERR-UNWRAP-ITERATOR (err u11))
...
(define-private (set-runes-fold (id uint) (state (response { bridge-contract: <bridge-config-trait-v1>, runes: (list 1000 (buff 26)), rune-contracts: (list 1000 principal), is-active-runes: (list 1000 bool) } uint)))
	(let
		(
			(unwrapped-state (try! state))
			(rune (unwrap! (element-at? (get runes unwrapped-state) id) ERR-WRONG-ARRAY-SIZE))
			(rune-contract (unwrap! (element-at? (get rune-contracts unwrapped-state) id) ERR-WRONG-ARRAY-SIZE))
			(is-active (unwrap! (element-at? (get is-active-runes unwrapped-state) id) ERR-WRONG-ARRAY-SIZE))
			(bridge (get bridge-contract unwrapped-state))
		)
		(try! (contract-call? bridge set-rune-token-by-id rune rune-contract is-active))
		state
	)
)

(define-private (set-runes-active-fold (id uint) (state (response { bridge-contract: <bridge-config-trait-v1>, runes: (list 1000 (buff 26)), is-active-runes: (list 1000 bool) } uint)))
	(let
		(
			(unwrapped-state (try! state))
			(rune (unwrap! (element-at? (get runes unwrapped-state) id) ERR-WRONG-ARRAY-SIZE))
		...
		(try! (contract-call? bridge set-rune-token-active rune is-active))
		state
	)
)

(define-public (set-btc-and-runes
	(bridge-contract <bridge-config-trait-v1>)
 	(btc-contract principal)
	(runes (list 1000 (buff 26)))
	(rune-contracts (list 1000 principal))
	(is-active-runes (list 1000 bool))
)
	(let
		(
			(it (try! (get-iterator (len runes))))
		)
		(try! (contract-call? bridge-contract set-btc-token btc-contract))
		(fold set-runes-fold it (ok {bridge-contract: bridge-contract, runes: runes, rune-contracts: rune-contracts, is-active-runes: is-active-runes}))
	)
)

(define-public (set-runes
	(bridge-contract <bridge-config-trait-v1>)
	(runes (list 1000 (buff 26)))
	(rune-contracts (list 1000 principal))
	(is-active-runes (list 1000 bool))
...
(define-public (set-runes-and-btc-active
	(bridge-contract <bridge-config-trait-v1>)
 	(btc-paused bool)
	(runes (list 1000 (buff 26)))
	(is-active-runes (list 1000 bool))
	...
	(let
		...
		(try! (contract-call? bridge-contract set-btc-paused btc-paused))
		(fold set-runes-active-fold it (ok {bridge-contract: bridge-contract, runes: runes, is-active-runes: is-active-runes}))
	)
)
