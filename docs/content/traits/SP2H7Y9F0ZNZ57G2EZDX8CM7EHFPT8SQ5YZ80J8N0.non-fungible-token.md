...
(define-non-fungible-token Hiros uint)
...
(define-constant COLLECTION-LIMIT u100) ;; Limit to series of 1000
(define-constant ERR-OWNER-ONLY (err u100))
(define-constant ERR-NOT-TOKEN-OWNER (err u101))
(define-constant ERR-SOLD-OUT (err u300))

(define-data-var base-uri (string-ascii 80) "https://hiro.so")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Hiros token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? Hiros token-id sender recipient)
  )
)
...
(define-public (mint (recipient principal))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? Hiros token-id recipient))
