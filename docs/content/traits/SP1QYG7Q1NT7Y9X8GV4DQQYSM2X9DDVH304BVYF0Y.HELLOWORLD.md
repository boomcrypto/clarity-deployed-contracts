...
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)
  ...
  (define-fungible-token HELLOWORLD MAX_SUPPLY)
  ...
  (define-public
    ...
    (begin
      ...
      (ft-transfer? HELLOWORLD amt from to)))


  (define-public (mint (amt uint) (to principal))
    (begin
      (try! (check-owner))
      (ft-mint? HELLOWORLD amt to) ))

  (define-read-only (get-name)                   (ok "HELLOWORLD"))
  (define-read-only (get-symbol)                 (ok "HELLOWORLD"))
  (define-read-only (get-decimals)               (ok u6))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance HELLOWORLD of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply HELLOWORLD)))
