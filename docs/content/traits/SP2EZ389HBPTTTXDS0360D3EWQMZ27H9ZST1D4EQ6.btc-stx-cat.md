---
title: "Trait btc-stx-cat"
draft: true
---
```
;; Unlike submarine swaps, catamaran swaps are fully on-chain for maximum transparency.
;; STX senders should never engage in off-contract (non-atomic) transactions that match their swap parameters while their swap is reserved.
...
(define-constant ERR_TX_TOO_OLD (err u29))
...
(define-public (submit-swap 
  ...
  (let ((swap (unwrap! (map-get? swaps id) ERR_INVALID_ID))
      ...
      (asserts! (is-eq tx-sender stx-receiver) ERR_INVALID_STX_RECEIVER)     
      (asserts! (not (get done swap)) ERR_ALREADY_DONE)
      (asserts! (>= height (get when swap)) ERR_TX_TOO_OLD)
