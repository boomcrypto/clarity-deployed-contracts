---
title: "Trait stackswap-leonardoughdavinci-v1a"
draft: true
---
```
...
(define-private (can-mint)
  (or
    (or
      (and (and (> (contract-call? .stackswap-gold-pass-v1 get-balance tx-sender) u0) (>= block-height  (var-get GOLD_PASS_START_BLOCK))) (<= (var-get nft-counter) GOLD_PASS_SUPPLY))
      (and (and (or (> (contract-call? .stackswap-gold-pass-v1 get-balance tx-sender) u0) (> (contract-call? .stackswap-silver-pass-v1 get-balance tx-sender) u0)) (>= block-height  (var-get SILVER_PASS_START_BLOCK))) (<= (var-get nft-counter) SILVER_PASS_SUPPLY))
    )
