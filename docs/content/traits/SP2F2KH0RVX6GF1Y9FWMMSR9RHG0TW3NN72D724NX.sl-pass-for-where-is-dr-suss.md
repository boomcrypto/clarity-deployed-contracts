---
title: "Trait sl-pass-for-where-is-dr-suss"
draft: true
---
```
...
(define-non-fungible-token sl-pass-for-where-is-dr-suss uint)
...
(define-data-var admin (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF 'SP2F2KH0RVX6GF1Y9FWMMSR9RHG0TW3NN72D724NX ))
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? sl-pass-for-where-is-dr-suss item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? sl-pass-for-where-is-dr-suss item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? sl-pass-for-where-is-dr-suss item-id)))
      (begin
        (try! (nft-burn? sl-pass-for-where-is-dr-suss item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sl-pass-for-where-is-dr-suss token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sl-pass-for-where-is-dr-suss id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sl-pass-for-where-is-dr-suss id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sl-pass-for-where-is-dr-suss id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP2F2KH0RVX6GF1Y9FWMMSR9RHG0TW3NN72D724NX u800 "sl pass for where is dr suss " u690000 u64326 u64393 "ipfs://QmfZapZ3Kbh3Ff9bDFBRv6ZZDn4WM9hcVmpf3ZuKzPHEJX/sl-pass-for-where-is-dr-suss/1.json"))
  (ok true)
)
