---
title: "Trait aggregator-core-v-1-1"
draft: true
---
```
...
;; Define is-address-fee-exempt and is-account-fee-exempt maps
(define-map is-address-fee-exempt principal bool)
(define-map is-account-fee-exempt (buff 20) bool)
...
;; Get fee exemption for address and its account
(define-read-only (get-is-fee-exempt (address principal))
  (let (
    (address-data (unwrap! (principal-destruct? address) ERR_INVALID_PRINCIPAL))
    (is-address-exempt (map-get? is-address-fee-exempt address))
    (is-account-exempt (default-to false (map-get? is-account-fee-exempt (get hash-bytes address-data))))
  )
    (if (is-some is-address-exempt)
      (ok (unwrap-panic is-address-exempt))
      (ok is-account-exempt)
    )
  )
)
...
(define-read-only (get-aggregator-fees (contract principal) (provider (optional principal)) (amount uint))
  (let (
    ...
    (is-caller-fee-exempt (try! (get-is-fee-exempt tx-sender)))
...
;; Set fee exemption for address and its account
(define-public (set-is-fee-exempt (address principal) (exempt-address bool) (exempt-account bool))
  (let (
    (address-data (unwrap! (principal-destruct? address) ERR_INVALID_PRINCIPAL))
    (caller tx-sender)
  )
    (begin
      ...
      ;; Set fee exemption for address and its account
      (map-set is-address-fee-exempt address exempt-address)
      (map-set is-account-fee-exempt (get hash-bytes address-data) exempt-account)

      ;; Print function data and return true
      (print {
        action: "set-is-fee-exempt",
        caller: caller,
        data: {
          address: address,
          exempt-address: exempt-address,
          exempt-account: exempt-account
        }
      })
      (ok true)
    )
...
(define-public (transfer-aggregator-fees (token <sip-010-trait>) (contract principal) (provider (optional principal)) (amount uint))
  (let (
    ;; Check if caller is exempt from fees
    (caller tx-sender)
    (is-caller-fee-exempt (try! (get-is-fee-exempt caller)))
