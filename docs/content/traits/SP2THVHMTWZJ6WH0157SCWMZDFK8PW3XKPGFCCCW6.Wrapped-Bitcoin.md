...
(define-constant TOKEN_URI u"https://wrapped.com/xbtc.json") ;; utf-8 string with token metadata host
(define-constant TOKEN_NAME "Wrapped Bitcoin")
(define-constant TOKEN_SYMBOL "xBTC")
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance wrapped-bitcoin who))
)
...
(begin
    ...
    ;; Transfer the tokens
    (let ((result (ft-transfer? wrapped-bitcoin amount sender recipient)))
      (if (is-ok result)
        (begin
...
(begin
  (try! (ft-mint? wrapped-bitcoin u1200000000 CONTRACT_OWNER)) 
  ;; (try! (ft-mint? wrapped-bitcoin u100000000000 'ST18VZNQWK7R44BFN75X3RANYRAT4B9ZA8S47KZ24)) 
) 
```
