...
(define-non-fungible-token rectitude-phunks uint)
...
(define-data-var artist-address principal 'SP1FAYNW2KQVGPJSAW3N9YDZVNDNVQM4WW4XMJE9S)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPGAh7e5GeVcv9sZsacH55BK79bWVZ4DLc4zRrJUeexj2/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? rectitude-phunks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? rectitude-phunks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? rectitude-phunks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? rectitude-phunks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rectitude-phunks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rectitude-phunks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rectitude-phunks id) (err ERR-NOT-FOUND)))
