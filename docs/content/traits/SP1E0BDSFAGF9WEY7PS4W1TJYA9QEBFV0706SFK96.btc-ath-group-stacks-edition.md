---
title: "Trait btc-ath-group-stacks-edition"
draft: true
---
```
;; btc-ath-group-stacks-edition
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token btc-ath-group-stacks-edition uint)
...
(define-data-var artist-address principal 'SP1E0BDSFAGF9WEY7PS4W1TJYA9QEBFV0706SFK96)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbQZWQCGuBGw9H1K15ECVo3DzE2jjHF3ZFspUPc69Z7R3/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? btc-ath-group-stacks-edition next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? btc-ath-group-stacks-edition token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? btc-ath-group-stacks-edition token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? btc-ath-group-stacks-edition token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? btc-ath-group-stacks-edition id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? btc-ath-group-stacks-edition id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? btc-ath-group-stacks-edition id) (err ERR-NOT-FOUND)))
