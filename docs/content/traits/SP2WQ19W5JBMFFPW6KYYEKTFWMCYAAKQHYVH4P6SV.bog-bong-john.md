...
(define-non-fungible-token bog-bong-john uint)
...
(define-data-var admin (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF 'SP2WQ19W5JBMFFPW6KYYEKTFWMCYAAKQHYVH4P6SV ))
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? bog-bong-john item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? bog-bong-john item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? bog-bong-john item-id)))
      (begin
        (try! (nft-burn? bog-bong-john item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bog-bong-john token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bog-bong-john id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bog-bong-john id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bog-bong-john id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP2WQ19W5JBMFFPW6KYYEKTFWMCYAAKQHYVH4P6SV u600 "Bog Bong John" u50000 u64219 u64231 "ipfs://QmaR9Nzcn9pnwNQovuufF2crpTHSEtZhiBPEGFrFyqp1i3/bog-bong-john/1.json"))
  (ok true)
)
