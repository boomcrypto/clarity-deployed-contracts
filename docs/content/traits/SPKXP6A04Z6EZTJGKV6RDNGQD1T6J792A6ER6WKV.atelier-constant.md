...
(define-non-fungible-token atelier-constant uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRVVAaUgE2i4ApEu65N7s38rpZAV9FshR26SWQWUBfxPM/json/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? atelier-constant next-id tx-sender) next-id)
      (unwrap! (nft-transfer? atelier-constant next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? atelier-constant token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? atelier-constant token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? atelier-constant token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? atelier-constant id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? atelier-constant id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? atelier-constant id) (err ERR-NOT-FOUND)))
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? atelier-constant (+ last-nft-id u0) 'SPKXP6A04Z6EZTJGKV6RDNGQD1T6J792A6ER6WKV))
