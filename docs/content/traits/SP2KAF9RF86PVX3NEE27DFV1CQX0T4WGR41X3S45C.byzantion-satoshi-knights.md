---
title: "Trait byzantion-satoshi-knights"
draft: true
---
```
...
(define-non-fungible-token satoshi-knights uint)
...
(define-constant MINT-LIMIT u688)

;; Internal variables
(define-data-var last-id uint u0)
(define-data-var uri-prefix (string-ascii 256) "")
(define-data-var cost-per-mint uint u28000000)
(define-data-var commission-per-mint uint u2240000)
(define-data-var payout uint u0)
(define-data-var ipfs-full-metadata (string-ascii 117) "ipfs://ipfs/bafybeiddhyco55dqaruryz32uq62vdj2gdnqhggqym7e7dtfyrpxqpdcum/satoshi_knights/satoshi_knights_metadata.json")
(define-data-var ipfs-root (string-ascii 104) "ipfs://ipfs/bafybeicnsqt5xjywmmrld65m2pdya3l2mcuvclqtgwe6x26xig3yt4vwtu/satoshi_knights/satoshi_knights_")
(define-data-var artist-address principal 'SPMFK2FBQC79WS2087HT8SD2TTD54YEHFGWCYK5J)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? satoshi-knights next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? satoshi-knights token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? satoshi-knights token-id)))
