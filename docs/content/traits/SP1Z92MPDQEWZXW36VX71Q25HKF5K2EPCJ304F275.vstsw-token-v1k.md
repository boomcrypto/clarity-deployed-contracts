...
(define-public (stake-tokens (amount uint) (month uint))
  (let (
    ...
    (asserts! (or (contract-call? .stackswap-security-list-v1a is-secure-router-or-user contract-caller) (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "staking")))) (err ERR_INVALID_ROUTER))
