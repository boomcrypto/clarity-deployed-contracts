---
title: "Trait nocodeclarity-pass"
draft: true
---
```
;; nocodeclarity-pass
...
(define-non-fungible-token nocodeclarity-pass uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmR2daDDpfvGJsSYmxDoyRYoYhPaS9QdTJZuXbiAdgWy15/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? nocodeclarity-pass next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? nocodeclarity-pass token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? nocodeclarity-pass token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? nocodeclarity-pass token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? nocodeclarity-pass id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? nocodeclarity-pass id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? nocodeclarity-pass id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u469693)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u277000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u68000000000)
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? nocodeclarity-pass (+ last-nft-id u0) 'SP12JCJYJJ31C59MV94SNFFM4687H9A04Q3BHTAJM))
      (map-set token-count 'SP12JCJYJJ31C59MV94SNFFM4687H9A04Q3BHTAJM (+ (get-balance 'SP12JCJYJJ31C59MV94SNFFM4687H9A04Q3BHTAJM) u1))
      (try! (nft-mint? nocodeclarity-pass (+ last-nft-id u1) 'SP2HJVJQS0TRBTQ1WAWWN9H9W2HKGDZ7H5EZCEAQC))
      (map-set token-count 'SP2HJVJQS0TRBTQ1WAWWN9H9W2HKGDZ7H5EZCEAQC (+ (get-balance 'SP2HJVJQS0TRBTQ1WAWWN9H9W2HKGDZ7H5EZCEAQC) u1))
      (try! (nft-mint? nocodeclarity-pass (+ last-nft-id u2) 'SP2HJVJQS0TRBTQ1WAWWN9H9W2HKGDZ7H5EZCEAQC))
      (map-set token-count 'SP2HJVJQS0TRBTQ1WAWWN9H9W2HKGDZ7H5EZCEAQC (+ (get-balance 'SP2HJVJQS0TRBTQ1WAWWN9H9W2HKGDZ7H5EZCEAQC) u1))
