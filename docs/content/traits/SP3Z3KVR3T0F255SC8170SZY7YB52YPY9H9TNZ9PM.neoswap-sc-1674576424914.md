---
title: "Trait neoswap-sc-1674576424914"
draft: true
---
```
...
(define-constant TRADER-2 'SP5G3VY7MZT8BNB6FHXZE9JD4PPF8WRT3H6JSBWW)
(define-constant TRADER-3 'SPH9YG28V53YATFKTJHMDG19RRJT74YFNS2MYTGF)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (contract-call? 'SP3R8APMXYQRQC6JZAE376ZDVKJAQG5KYMTPV3F9E.bitcoin-flowers transfer u124 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u6433000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
		(unwrap! (contract-call? 'SPZZMP4KH0QYW7KX62C41YJEAHYD4Y2GPBWQ374C.bitcoin-kicks-socks transfer u48 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
		(unwrap! (contract-call? 'SP3M2TWQGB7YWRGZW17XSE3FCESR0EZMGB9H3KRDC.coffee-cups transfer u4 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-3)
            (begin
		(unwrap! (stx-transfer? u1078000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP3R8APMXYQRQC6JZAE376ZDVKJAQG5KYMTPV3F9E.bitcoin-flowers transfer u124 tx-sender TRADER-2)))
	(unwrap-panic (as-contract (contract-call? 'SP3M2TWQGB7YWRGZW17XSE3FCESR0EZMGB9H3KRDC.coffee-cups transfer u4 tx-sender TRADER-3)))
	(unwrap-panic (as-contract (contract-call? 'SPZZMP4KH0QYW7KX62C41YJEAHYD4Y2GPBWQ374C.bitcoin-kicks-socks transfer u48 tx-sender TRADER-3)))

	(unwrap-panic (as-contract (stx-transfer? u11000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u7500000 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP3R8APMXYQRQC6JZAE376ZDVKJAQG5KYMTPV3F9E.bitcoin-flowers transfer u124 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u6433000 tx-sender TRADER-2)))
		(unwrap-panic (as-contract (contract-call? 'SPZZMP4KH0QYW7KX62C41YJEAHYD4Y2GPBWQ374C.bitcoin-kicks-socks transfer u48 tx-sender TRADER-2)))
		(unwrap-panic (as-contract (contract-call? 'SP3M2TWQGB7YWRGZW17XSE3FCESR0EZMGB9H3KRDC.coffee-cups transfer u4 tx-sender TRADER-2)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u1078000 tx-sender TRADER-3)))
	    )
            true
        )
