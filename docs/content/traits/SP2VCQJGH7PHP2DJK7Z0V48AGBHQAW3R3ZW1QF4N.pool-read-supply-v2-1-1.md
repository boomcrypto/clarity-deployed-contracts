---
title: "Trait pool-read-supply-v2-1-1"
draft: true
---
```
...
(define-read-only (get-useable-collateral-usd-ststxbtc (who principal))
  (let (
    (asset-balance (unwrap-panic (contract-call? .zststxbtc-v2-0 get-principal-balance who)))
    (reserve-data (get-reserve-data 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststxbtc-token))
    (user-index (unwrap-panic (contract-call? .pool-reserve-data get-user-index-read who 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststxbtc-token)))
    (asset-decimals (get decimals reserve-data))
    (base-ltv-as-collateral (get base-ltv-as-collateral reserve-data))
    (reserve-normalized-income
      (get-normalized-income
...
(define-read-only (get-supplied-balance-user-ststxbtc (who principal))
  (let ((principal (unwrap-panic (contract-call? .zststxbtc-v2-0 get-principal-balance who))))
...
(define-read-only (get-supplied-balance-usd-ststxbtc)
  (token-to-usd
    (unwrap-panic (contract-call? 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststxbtc-token get-balance .pool-vault))
    u6
    (get-ststxbtc-price)
  )
)
