---
title: "Trait neoswap-sc-1682967635742"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/KDYx9JMcDjN5b8ecWvo8
;; SwapId: oa1qA9VlOsey4r7L2xPO

;; traders
(define-constant TRADER-1 'SP2VF3564DKVF2T39NWAT6ZMAQ5X3HHBW0GHN8PWY)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (contract-call? 'SP2VF3564DKVF2T39NWAT6ZMAQ5X3HHBW0GHN8PWY.stacks-animals-lovers transfer u7 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u7009000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP2VF3564DKVF2T39NWAT6ZMAQ5X3HHBW0GHN8PWY.stacks-animals-lovers transfer u7 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u350450 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u6658550 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2VF3564DKVF2T39NWAT6ZMAQ5X3HHBW0GHN8PWY.stacks-animals-lovers transfer u7 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u7009000 tx-sender TRADER-2)))
	    )
            true
        )
