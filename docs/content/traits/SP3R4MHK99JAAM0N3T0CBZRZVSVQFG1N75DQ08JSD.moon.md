...
(define-constant TOKEN_SYMBOL "MOON")
(define-constant TICK_NAME "moon")
...
(define-read-only (get-balance (address principal))
  (ok (ft-get-balance moon address))
)
...
(define-read-only (get_summary (address principal))
  {
    ...
    sip10_balance: (ft-get-balance moon address),
  }
)
...
(define-public (wrap (amount uint))
  (let
    ...
    (ft-mint? moon (* amount ONE_COIN) sender)
  )
)
...
(define-public (unwrap (amount uint))
  (let
    ...
    (try! (ft-burn? moon (* amount ONE_COIN) sender))
    (print {
      type: "unwrap",
      tick: TICK_NAME,
      sender: sender,
