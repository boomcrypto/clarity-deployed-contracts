...
(define-non-fungible-token hexstx-citycoins uint)
...
(define-data-var total-price uint u6000)
(define-data-var artist-address principal 'SP2H4C1ACECDQ9R78AARQM3KGRMBRSGA4GJ455GWD)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qme5DMgcRHKgvzYZ5E6pFYHhRWYwM9KF5ceHjNpUR47MTo/")
  ...
  (let ((next-id (+ u1 (var-get last-id)))  
    ...
    (let
      ((total-commission (/ (* (var-get total-price) (var-get commission)) u10000))
       (total-artist (- (var-get total-price) (* u2 total-commission))))
      (if (is-eq tx-sender (var-get artist-address))
        ...
        (if (is-eq tx-sender commission-address)
          ...
          (begin
            ...
            (try! (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.miamicoin-token transfer total-commission tx-sender 'SM2MARAVW6BEJCD13YV2RHGYHQWT7TDDNMNRB1MVT (some 0x00))) ;; miami wallet
            (mint-helper new-owner next-id))))
    )
  )
)
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? hexstx-citycoins next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? hexstx-citycoins token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? hexstx-citycoins token-id)))
