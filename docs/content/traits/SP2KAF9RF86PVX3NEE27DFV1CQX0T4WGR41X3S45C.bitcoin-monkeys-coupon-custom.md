---
title: "Trait bitcoin-monkeys-coupon-custom"
draft: true
---
```
...
(define-non-fungible-token bitcoin-monkeys-coupon-custom uint)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-monkeys-coupon-custom id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace bitcoin-monkeys-coupon-custom
  (ok (nft-get-owner? bitcoin-monkeys-coupon-custom id)))
...
(define-public (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? bitcoin-monkeys-coupon-custom next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? bitcoin-monkeys-coupon-custom id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-monkeys-coupon-custom id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-monkeys-coupon-custom id) ERR-NOT-FOUND))
