...
;; Define LP token
(define-fungible-token AOE)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.age-of-empires"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? AOE amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance AOE who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply AOE)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 transfer (get dx delta) sender CONTRACT none))
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? AOE (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? AOE (get dk delta) sender))
        (try! (as-contract (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 transfer (get dx delta) CONTRACT sender none)))
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-private (get-reserves)
    { 
      a: (unwrap-panic (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 get-balance CONTRACT)), 
      b: (unwrap-panic (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (add-liquidity u2000000))
    
    ;; Transfer additional token A to achieve desired ratio
    (try! (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 transfer u98000000 tx-sender CONTRACT none)))
```
