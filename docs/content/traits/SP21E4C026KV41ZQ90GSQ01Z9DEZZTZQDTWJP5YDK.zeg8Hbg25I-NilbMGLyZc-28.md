---
title: "Trait zeg8Hbg25I-NilbMGLyZc-28"
draft: true
---
```
...
(define-public (swap-helper-3
    (gmf9MFsEr5uXqn6 <AxSDVMnsiYamlC>)
    (esHhZSmChD5PCRU <AxSDVMnsiYamlC>)
    (token-z-trait <AxSDVMnsiYamlC>)
    (REZaDNbWLaV7mk3 (string-ascii 10))
    (B2ScWCyaYXQhb8x (string-ascii 10))
    (amm-3 (string-ascii 10))
    (dx uint)
    (jnJe1z8sXPa46T bool)
)
    (let
        ...
        (let
            (
                (x-amount (try! (VCQGoSAdvbxUQE REZaDNbWLaV7mk3 KnKttLSP2Bg7cb6 TPksrk9EaNzhmtf dx)))
                (lgOKJUDXNTYsaE (try! (VCQGoSAdvbxUQE B2ScWCyaYXQhb8x TPksrk9EaNzhmtf token-z x-amount)))
                (nmoFJdZDqlSrMz (try! (VCQGoSAdvbxUQE amm-3 token-z KnKttLSP2Bg7cb6 lgOKJUDXNTYsaE)))
            )
            (if (is-eq jnJe1z8sXPa46T true)
                (begin (try! (as-contract (stx-transfer? nmoFJdZDqlSrMz tx-sender z89U2vcrbMgcMz3))))
                (begin (try! (as-contract (contract-call? gmf9MFsEr5uXqn6 transfer nmoFJdZDqlSrMz tx-sender z89U2vcrbMgcMz3 none))))
