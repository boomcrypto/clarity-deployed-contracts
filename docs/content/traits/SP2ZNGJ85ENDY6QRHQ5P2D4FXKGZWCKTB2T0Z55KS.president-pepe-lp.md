---
title: "Trait president-pepe-lp"
draft: true
---
```
;; Title: President Pepe LP
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token CHA-PEPE)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.president-pepe-lp"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? CHA-PEPE amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "President Pepe LP"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance CHA-PEPE who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply CHA-PEPE)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.tokensoft-token-v4k68639zxz transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? CHA-PEPE (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? CHA-PEPE (get dk delta) sender))
...
(begin
    ...
    (try! (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.tokensoft-token-v4k68639zxz transfer u2050000000 tx-sender CONTRACT none)))
```
