...
(define-non-fungible-token ghost-rock uint)
...
(define-constant ITEM-COUNT u365)

;; variables
(define-data-var metadata-frozen bool false)
(define-data-var ghost-counter uint u0)
(define-data-var ghost-index uint u0)
(define-data-var token-uri (string-ascii 256) "ipfs://QmXZ19V8PSyJ4qGUqza3NBX3bDYdUnUVzodhEMGag5KVSv/{id}.json")
(define-data-var cost-per-mint uint u52000000)
(define-data-var marketplace-commission uint u7000000)
...
(define-private (mint-next)
  (let (
    ...
    (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
      success (begin
        (try! (nft-mint? ghost-rock random-ghost-id tx-sender))
        (var-set ghost-counter (+ count u1))
        (if (is-eq u0 (var-get rotation))
          (begin
            (var-set rotation u1)
            (var-set ghost-index (+ u1 (var-get ghost-index)))
          )
          (var-set rotation u0)
        )
        (try!
...
(define-public (burn (index uint))
  (let (
    ...
    (match (nft-burn? ghost-rock index tx-sender)
      success (begin
        (map-set token-count tx-sender (- current-balance u1))
        (ok true)
      )
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ghost-rock id sender recipient)
    success
      (let (
...
(define-read-only (get-owner (index uint))
  (ok (nft-get-owner? ghost-rock index))
)
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let (
    (owner (unwrap! (nft-get-owner? ghost-rock id) (err ERR-NOT-FOUND)))
...
(define-private (is-sender-owner (id uint))
  (let (
    (owner (unwrap! (nft-get-owner? ghost-rock id) false))
  )
