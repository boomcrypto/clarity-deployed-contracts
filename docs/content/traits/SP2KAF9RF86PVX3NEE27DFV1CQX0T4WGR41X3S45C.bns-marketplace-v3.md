---
title: "Trait bns-marketplace-v3"
draft: true
---
```
...
(define-public (list-domain (namespace (buff 20))
    ...
    (try! (stx-transfer? (var-get list-cost) tx-sender CONTRACT-OWNER))
    (ok true)
    )
)
)
...
(define-public (admin-bid (namespace (buff 20))
                            (domain (buff 48))
                            (amount uint)
                            (bidder principal))
    (let (
    ...
    (if (is-none prev-offer)
      (match (stx-transfer? amount tx-sender (as-contract tx-sender))
        ...
        (asserts! (var-get active) ERR-NOT-ACTIVE)
        (asserts! (> amount u0) ERR-BID-NOT-HIGH-ENOUGH)
      (map-set bids { namespace: namespace, name: domain } { buyer: bidder, offer: amount })
      (ok true)
      )
      error (err error)
      )
