---
title: "Trait stacks-golden-runestone"
draft: true
---
```
;; stacks-golden-runestone
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token stacks-golden-runestone uint)
...
(define-data-var artist-address principal 'SP1EK9QDXECSNVWWXZFDZTZKWVJX1K4T7NNKSP6J3)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQVsEtVj8f5tMzZCR2MUpfHxCCZ3SRnF9qVLdgj2Qxm1W/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? stacks-golden-runestone next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-golden-runestone token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-golden-runestone token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-golden-runestone token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-golden-runestone id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-golden-runestone id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-golden-runestone id) (err ERR-NOT-FOUND)))
