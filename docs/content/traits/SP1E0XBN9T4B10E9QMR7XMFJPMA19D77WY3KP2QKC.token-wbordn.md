...
(define-fungible-token wbordn)

(define-data-var token-name (string-ascii 32) "Bordn Wrapper")
(define-data-var token-symbol (string-ascii 32) "wbordn")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wbordn.json"))
...
(define-read-only (get-base-decimals)
  (contract-call? 'SP32P7T7VSRKN8D1B9S6WYT8ZS4XAD6ZZJGBVSMRF.Boreden-Retriever-Token get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP32P7T7VSRKN8D1B9S6WYT8ZS4XAD6ZZJGBVSMRF.Boreden-Retriever-Token get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP32P7T7VSRKN8D1B9S6WYT8ZS4XAD6ZZJGBVSMRF.Boreden-Retriever-Token transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
