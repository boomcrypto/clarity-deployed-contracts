---
title: "Trait byzantion-stacksboi"
draft: true
---
```
...
(define-non-fungible-token stacksboi uint)
...
(define-data-var cost-per-mint uint u70000000)
(define-data-var commission uint u1500)
(define-data-var payout uint u0)
(define-data-var ipfs-full-metadata (string-ascii 105) "ipfs://ipfs/bafybeiabeewc2dkzqudhlnulfgok5knfwg6plwec7kiyhlblbtmy5gd3sa/stacksboi/stacksboi_metadata.json")
(define-data-var ipfs-root (string-ascii 92) "ipfs://ipfs/bafybeiabeewc2dkzqudhlnulfgok5knfwg6plwec7kiyhlblbtmy5gd3sa/stacksboi/stacksboi_")
(define-data-var artist-address principal 'SP3MC9CB9A226S1XZBN4F2HEE8NPKNK9BRS3XRMKX)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? stacksboi next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? stacksboi token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacksboi token-id)))
