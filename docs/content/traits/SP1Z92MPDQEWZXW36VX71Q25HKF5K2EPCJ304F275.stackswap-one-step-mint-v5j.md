---
title: "Trait stackswap-one-step-mint-v5j"
draft: true
---
```
...
(define-public (create-pair-new-liquidity-token (token-x-trait <sip-010-token>) (token-y-trait <sip-010-token>)  (token-liquidity <liquidity-token>) (pair-name (string-ascii 32)) (x uint) (y uint) (token-liquidity-soft <initable-liquidity>) (swap-contract <stackswap-swap>) )
  (begin
    ...
    (asserts! (is-eq (contract-of swap-contract) (unwrap-panic (contract-call? .stackswap-dao-v5j get-qualified-name-by-name "swap"))) ERR_DAO_ACCESS)
...
(define-private (is-valid-caller (caller principal))
  (begin
    (asserts! (is-eq caller (unwrap-panic (contract-call? .stackswap-dao-v5j get-qualified-name-by-name "lp-deployer"))) ERR_DAO_ACCESS)
    (ok true)
  )
)
