---
title: "Trait upgraded-shark"
draft: true
---
```
;; Title: Upgraded Shark
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token upS)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SPGYCP878RYFVT03ZT8TWGPKNYTSQB1578VVXHGE.upgraded-shark"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? upS amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance upS who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply upS)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.shark-coin-stxcity transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.shark-coin-stxcity transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.shark-coin-stxcity transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? upS (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? upS (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.shark-coin-stxcity get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (add-liquidity u44000000))
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SP1KMAA7TPZ5AZZ4W67X74MJNFKMN576604CWNBQS.shark-coin-stxcity transfer u99956000000 tx-sender CONTRACT none)))
```
