...
(define-non-fungible-token genser-pixel uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRCegcWNeCtpFwXcD42F5aezSqjD8ECrJMeSL2oe8hwYQ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? genser-pixel next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? genser-pixel token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? genser-pixel token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? genser-pixel token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? genser-pixel id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? genser-pixel id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? genser-pixel id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u14435)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u516100000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u1171200000000)
