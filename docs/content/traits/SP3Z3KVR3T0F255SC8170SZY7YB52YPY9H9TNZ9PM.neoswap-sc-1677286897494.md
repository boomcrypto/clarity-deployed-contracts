---
title: "Trait neoswap-sc-1677286897494"
draft: true
---
```
;; multiway-swap
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u3102000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP09XKHN2MJNF6TWFEFGYFZHVVKEKX16PYKDKB8B.lucrative-lizards transfer u23 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-3)
            (begin
		(unwrap! (stx-transfer? u3044000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (contract-call? 'SP09XKHN2MJNF6TWFEFGYFZHVVKEKX16PYKDKB8B.lucrative-lizards transfer u23 tx-sender TRADER-3)))

	(unwrap-panic (as-contract (stx-transfer? u744000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u5402000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u3102000 tx-sender TRADER-1)))
		(unwrap-panic (as-contract (contract-call? 'SPJSCH3DDEJ8GQPGYZBHSB4F3HX5Q222CG89PSAB.guest-wins transfer u390 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP09XKHN2MJNF6TWFEFGYFZHVVKEKX16PYKDKB8B.lucrative-lizards transfer u23 tx-sender TRADER-2)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-3)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u3044000 tx-sender TRADER-3)))
		(unwrap-panic (as-contract (contract-call? 'SPP28QCMZ51GVN9933VFH9TXKQ3XCT4WE5A07533.warrior-pigeons transfer u39 tx-sender TRADER-3)))
		(unwrap-panic (as-contract (contract-call? 'SP2QDMH88MEZ8FFAYHW4B0BTXJRTHX8XBD54FE7HJ.were-mint-to-be transfer u346 tx-sender TRADER-3)))
		(unwrap-panic (as-contract (contract-call? 'SPP28QCMZ51GVN9933VFH9TXKQ3XCT4WE5A07533.warrior-pigeons transfer u37 tx-sender TRADER-3)))
	    )
            true
        )
