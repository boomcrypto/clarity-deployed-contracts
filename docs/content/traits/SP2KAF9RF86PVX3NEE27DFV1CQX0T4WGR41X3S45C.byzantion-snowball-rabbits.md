---
title: "Trait byzantion-snowball-rabbits"
draft: true
---
```
...
(define-non-fungible-token snowball-rabbits uint)
...
(define-constant MINT-LIMIT u50)
...
(define-data-var ipfs-full-metadata (string-ascii 119) "ipfs://ipfs/bafybeige7tqhevobjz2o3d7rufkg3wztfnatspuct6ailxhmxnvqyqx3fa/snowball_rabbits/snowball_rabbits_metadata.json")
(define-data-var ipfs-root (string-ascii 106) "ipfs://ipfs/bafybeihxy6zsoae4ern5ovian7xhz36yqeqtu7bkfzwes62gjtf337nd34/snowball_rabbits/snowball_rabbits_")
(define-data-var artist-address principal 'SP33QYJHG988R7XD10B5MPKG7NCWQJ77ETKMA2HK4)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? snowball-rabbits next-id new-owner))
...
(define-public (set-root (ipfs (string-ascii 106))) 
  (if (is-eq tx-sender CONTRACT-OWNER)
...
(define-public (set-full (ipfs (string-ascii 119))) 
  (if (is-eq tx-sender CONTRACT-OWNER)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? snowball-rabbits token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? snowball-rabbits token-id)))
