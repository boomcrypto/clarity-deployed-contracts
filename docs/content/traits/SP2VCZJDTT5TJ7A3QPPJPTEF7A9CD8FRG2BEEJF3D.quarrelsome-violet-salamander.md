---
title: "Trait quarrelsome-violet-salamander"
draft: true
---
```
...
(define-non-fungible-token OrdBunnies uint)
...
(define-data-var base-uri (string-ascii 99) "https://ordinals.com/inscription/61855b8db75c3d00f693232bdeed0ef37cad09f2863d501184d96ca03b91f3bei0")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? OrdBunnies token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? OrdBunnies token-id sender recipient)
  )
)
...
(define-public (mint (recipient principal))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? OrdBunnies token-id recipient))
