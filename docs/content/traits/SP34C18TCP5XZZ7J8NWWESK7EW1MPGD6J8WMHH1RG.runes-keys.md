...
(define-non-fungible-token runes-keys uint)
...
(define-data-var artist-address principal 'SP34C18TCP5XZZ7J8NWWESK7EW1MPGD6J8WMHH1RG)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcEWC8JeRayudE8iMSNfhnxExxkqud3q3pAvgJZ6fEgx9/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? runes-keys next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? runes-keys token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? runes-keys token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? runes-keys token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? runes-keys id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? runes-keys id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? runes-keys id) (err ERR-NOT-FOUND)))
