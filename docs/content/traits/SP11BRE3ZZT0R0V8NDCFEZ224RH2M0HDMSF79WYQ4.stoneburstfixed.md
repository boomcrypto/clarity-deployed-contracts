...
;; The slippage tolerance (e.g. 3% => user won't accept final < 97% of expected)
(define-data-var slippage-percent uint u3)
    ...
    (let ((my-pool-id (var-get stx-stone-pool-id)))
      (let ((maybe-pool
              (contract-call?
                ...
                get-pool
                my-pool-id)))
        (asserts! (is-some maybe-pool) (err u600))  ;; pool not found
        (let (
          ...
          (let ((out-result
                  (contract-call?
                    ...
                    r0
                    r1
                    (tuple (num u997) (den u1000))  ;; 0.3% swap fee
                  )))
    ...
    (let (
      ...
      (let ((expected-stone (unwrap! maybe-expected ERR-SWAP-FAILED)))
        (let ((slip (var-get slippage-percent)))
          ;; e.g. 3% => keep at least 97% of expected
          (let ((min-out (/ (* expected-stone (- u100 slip)) u100)))
            (let (
              (router-call
                (contract-call?
...
(define-public (set-percentage-progressive (new-perc uint))
  (begin
    (try! (check-is-owner))
    (asserts! (< new-perc HUNDRED-PERCENT) (err u601)) ;; must be < 100%
...
(define-public (set-percentage-burn (new-perc uint))
  (begin
    (try! (check-is-owner))
    (asserts! (> new-perc u0) (err u602)) ;; must be > 0
...
(define-public (set-slippage-percent (new-slippage uint))
  (begin
    (try! (check-is-owner))
    ;; e.g. limit slippage to <= 50% 
    (asserts! (<= new-slippage u50) (err u603)) 
