...
(define-non-fungible-token mr-bitcoin uint)
...
;; Internal variables
(define-data-var mint-limit uint u1010)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1500000)
(define-data-var artist-address principal 'SP2E1XY41CTKD1FMGFRS3T3ZD1Y1ERR1Z9VR82AM2)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcafKae2Hy2mv7kRqNo5LvNkmg9R5exAhjD3C7qyEPxbL/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? mr-bitcoin next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? mr-bitcoin token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? mr-bitcoin token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? mr-bitcoin token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? mr-bitcoin id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? mr-bitcoin id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? mr-bitcoin id) (err ERR-NOT-FOUND)))
