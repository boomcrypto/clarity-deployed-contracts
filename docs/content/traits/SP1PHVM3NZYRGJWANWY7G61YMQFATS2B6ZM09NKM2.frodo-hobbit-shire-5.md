---
title: "Trait frodo-hobbit-shire-5"
draft: true
---
```
...
(define-non-fungible-token joker-nft uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmbeieV5x1AfQTWRypt5j9BUEJ335sxEBHJsjQ7ab2tPpt/")
(define-data-var contract-uri (string-ascii 100) "ipfs://placeholder")

(define-data-var commission-percentage uint u10000) 
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? joker-nft id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace joker-nft
  (ok (nft-get-owner? joker-nft id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? joker-nft next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? joker-nft id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? joker-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? joker-nft id) ERR-NOT-FOUND))
