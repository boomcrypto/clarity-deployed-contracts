...
(define-fungible-token zsusdt)

(define-data-var token-uri (string-utf8 256) u"https://token-meta.s3.eu-central-1.amazonaws.com/zsUSDT.json")
(define-data-var token-name (string-ascii 32) "Zest sUSDT")
(define-data-var token-symbol (string-ascii 32) "zsUSDT")

(define-constant asset-addr 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt)
(define-constant decimals u8)

(define-read-only (get-total-supply)
  (ok (ft-get-supply zsusdt)))
...
(define-private (transfer-internal (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (match (ft-transfer? zsusdt amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-read-only (get-principal-balance (account principal))
  (ok (ft-get-balance zsusdt account))
)
