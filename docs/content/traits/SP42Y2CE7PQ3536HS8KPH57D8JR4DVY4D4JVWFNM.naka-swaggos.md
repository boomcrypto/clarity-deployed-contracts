...
(define-non-fungible-token naka-swaggos uint)
...
(define-data-var artist-address principal 'SP42Y2CE7PQ3536HS8KPH57D8JR4DVY4D4JVWFNM)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRpWvCYd1NFizJ9t8e5u27b2Kc5mn5VcdXg4keaGDUL2y/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? naka-swaggos next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? naka-swaggos token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? naka-swaggos token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? naka-swaggos token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? naka-swaggos id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? naka-swaggos id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? naka-swaggos id) (err ERR-NOT-FOUND)))
