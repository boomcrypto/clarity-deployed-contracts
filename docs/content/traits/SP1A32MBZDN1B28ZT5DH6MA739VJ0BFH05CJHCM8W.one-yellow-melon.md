...
(define-non-fungible-token one-yellow-melon uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmWWwtUtJ3FWg5BNPSUoLHzMDi6WFmZTtReHcTUMDFj1fS/one-yellow-melon/")
...
(define-data-var public-target-block-height uint u63632)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? one-yellow-melon id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace one-yellow-melon
  (ok (nft-get-owner? one-yellow-melon id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? one-yellow-melon next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? one-yellow-melon id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? one-yellow-melon id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? one-yellow-melon id) ERR-NOT-FOUND))
