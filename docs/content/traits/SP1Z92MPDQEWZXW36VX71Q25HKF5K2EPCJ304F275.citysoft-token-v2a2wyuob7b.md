---
title: "Trait citysoft-token-v2a2wyuob7b"
draft: true
---
```
(impl-trait .sip-010-v1a.sip-010-trait)
(impl-trait .initializable-city-token-trait-v1a.initializable-city-token-trait)
...
(define-constant ERR_INVALID_RESERVED_AMOUNT u4246)
...
(define-data-var reserve-wallet-addr principal tx-sender)

(define-data-var ratio-base uint u100)
(define-data-var ratio-reserve uint u30)

(define-data-var minimum-mining-amount uint u0)
...
(define-public (mine-many (amounts (list 200 uint)))
  (begin
    ...
    (match (fold mine-single amounts (ok { user: tx-sender, toStackers: u0, toFounder: u0, toReserve: u0, stacksHeight: block-height }))
      okReturn 
      (begin
        (asserts! (>= (stx-get-balance tx-sender) (+ (get toStackers okReturn) (+ (get toFounder okReturn) (get toReserve okReturn)))) (err ERR_INSUFFICIENT_BALANCE))
        (if (> (get toStackers okReturn ) u0)
          (try! (stx-transfer? (get toStackers okReturn ) tx-sender (as-contract tx-sender)))
          false
        )
        (if (> (get toFounder okReturn ) u0)
          (try! (stx-transfer? (get toFounder okReturn ) tx-sender (var-get contract-owner)))
          false
        )
        (if (> (get toReserve okReturn ) u0)
          (try! (stx-transfer? (get toReserve okReturn ) tx-sender (var-get reserve-wallet-addr)))
          false
        )
        ;; (print okReturn)
        (ok true)
  ...
  (match return okReturn
    (let
      (
        ...
        (toReserve
          (if stackingActive
            (/ (* (var-get ratio-reserve) amountUstx) u100)
            (- amountUstx toFounder)
          )
        )
        (toStackers (- amountUstx (+ toFounder toReserve)))
      )
      (asserts! (not (has-mined-at-block stacksHeight (get user okReturn))) (err ERR_USER_ALREADY_MINED))
      (asserts! (> amountUstx (var-get minimum-mining-amount)) (err ERR_INSUFFICIENT_COMMITMENT))
      (try! (set-tokens-mined (get user okReturn) stacksHeight amountUstx toStackers toFounder toReserve))
      (ok (merge okReturn 
        {
          toStackers: (+ (get toStackers okReturn) toStackers),
          toReserve: (+ (get toReserve okReturn) toReserve),
...
(define-private (set-tokens-mined (user principal) (stacksHeight uint) (amountUstx uint) (toStackers uint) (toFounder uint) (toReserve uint))
  (let
    (
    ...
    (map-set MiningStatsAtBlock
      stacksHeight
      {
        ...
        amountToReserve: (+ (get amountToReserve blockStats) toFounder),
...
(define-public (initialize (name-to-set (string-ascii 32)) (symbol-to-set (string-ascii 32)) (decimals-to-set uint) (uri-to-set (string-utf8 256))
    (website-to-set (string-utf8 256)) (initial-mint-amount uint) (first-stacking-block-to-set uint) (reward-cycle-lengh-to-set uint) (token-reward-maturity-to-set uint) (coinbase-reward-to-set uint) (reserve-ratio uint) (minimum-mining-amount-to-set uint))
  (begin
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "one-step-mint-city"))) (err ERR_PERMISSION_DENIED))
    ...
    (asserts! (< reserve-ratio u97) (err ERR_INVALID_RESERVED_AMOUNT))
    ...
    (try! (ft-mint? stackables (contract-call? .stackswap-one-step-mint-fee-v1a get-owner-amount initial-mint-amount) tx-sender))
    (try! (ft-mint? stackables (contract-call? .stackswap-one-step-mint-fee-v1a get-stackswap-amount initial-mint-amount) (contract-call? .stackswap-dao-v5k get-payout-address)))
    ...
    (var-set ratio-reserve reserve-ratio)
    (var-set minimum-mining-amount minimum-mining-amount-to-set)
    (ok u0)
))
