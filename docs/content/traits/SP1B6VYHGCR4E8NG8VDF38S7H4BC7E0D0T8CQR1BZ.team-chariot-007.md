...
(define-non-fungible-token team-chariot-007 uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmbVGQMWzQvJ7P8YnFs9yxFvS4Mi58hXZewzcLuAYDrALi/team-chariot/")
...
(define-data-var royalty (list 1000 uint) (list u7200 u1800 ))
(define-data-var artist-address (list 1000 principal) (list 'SP1B6VYHGCR4E8NG8VDF38S7H4BC7E0D0T8CQR1BZ 'SPCE90YFF2C24DYTEQDVJ2Y8TEM5HAXV7JCYGE9K )) 

(define-data-var private-sale-active bool true)
(define-data-var sale-active bool true)
(define-data-var private-target-block-height uint u63899)
(define-data-var public-target-block-height uint u64043)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? team-chariot-007 id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace team-chariot-007
  (ok (nft-get-owner? team-chariot-007 id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? team-chariot-007 next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? team-chariot-007 id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? team-chariot-007 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? team-chariot-007 id) ERR-NOT-FOUND))
...
(define-data-var mia-cost-per-mint uint u20000)
...
;; ###### BEGIN: fungible token mint template code ##################################################
(define-data-var nyc-cost-per-mint uint u30000)
...
;; ###### BEGIN: fungible token mint template code ##################################################
(define-data-var banana-cost-per-mint uint u40000)
...
;; ###### BEGIN: fungible token mint template code ##################################################
(define-data-var slime-cost-per-mint uint u50000)
...
;; ###### BEGIN: fungible token mint template code ##################################################
(define-data-var spaghetti-cost-per-mint uint u60000)
...
;; begin mint 3
(define-public (claim-spaghetti-three)
  (begin
    (try! (claim-spaghetti))
    (try! (claim-spaghetti))
    (try! (claim-spaghetti))
...
(begin
  (try! (set-private-wl 'SP3Z0B10A7K4CRQQFYRNGRCB6F259N176MS0VQ2D3 u10))
  (try! (set-private-wl 'SP2WQ19W5JBMFFPW6KYYEKTFWMCYAAKQHYVH4P6SV u15))
  (ok true)
)
