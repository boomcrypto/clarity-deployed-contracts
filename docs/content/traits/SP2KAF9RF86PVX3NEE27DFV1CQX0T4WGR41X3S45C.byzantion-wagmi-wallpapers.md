---
title: "Trait byzantion-wagmi-wallpapers"
draft: true
---
```
...
(define-non-fungible-token wagmi-wallpapers uint)
...
(define-data-var ipfs-full-metadata (string-ascii 84) "ipfs://QmWAutGW9SXy4YiKdbwr45mBjmHyzriQRt6DoE6WrzjPPo/WAGMI_wallpapers_metadata.json")
(define-data-var ipfs-root (string-ascii 71) "ipfs://QmWAutGW9SXy4YiKdbwr45mBjmHyzriQRt6DoE6WrzjPPo/WAGMI_wallpapers_")
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? wagmi-wallpapers next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? wagmi-wallpapers token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? wagmi-wallpapers token-id)))
