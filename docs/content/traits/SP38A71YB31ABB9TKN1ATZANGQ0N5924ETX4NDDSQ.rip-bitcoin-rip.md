...
(define-non-fungible-token rip-bitcoin-rip uint)
...
;; Internal variables
(define-data-var mint-limit uint u255)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmaqb3rvJ3vkqREwRrdbzPjUqeBwrDKw9kjvECZk1KTLxx/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? rip-bitcoin-rip next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? rip-bitcoin-rip token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? rip-bitcoin-rip token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? rip-bitcoin-rip token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rip-bitcoin-rip id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rip-bitcoin-rip id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rip-bitcoin-rip id) (err ERR-NOT-FOUND)))
