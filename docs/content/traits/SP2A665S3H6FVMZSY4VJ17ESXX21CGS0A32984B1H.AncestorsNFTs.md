---
title: "Trait AncestorsNFTs"
draft: true
---
```
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token PunksArmyAncestors uint)
...
(define-private (mint_punk (punk-id uint) (new-owner principal))
	(begin
		...
		(nft-mint? PunksArmyAncestors punk-id new-owner)
	)
)
...
(define-public 
	...
	(begin     
		...
		(nft-transfer? PunksArmyAncestors token-id sender recipient))
)

(
	define-read-only (get-owner (punk-id uint))  
	(ok (nft-get-owner? PunksArmyAncestors punk-id))
)
