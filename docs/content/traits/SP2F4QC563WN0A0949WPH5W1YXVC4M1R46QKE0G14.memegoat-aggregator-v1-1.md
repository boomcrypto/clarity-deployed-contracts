---
title: "Trait memegoat-aggregator-v1-1"
draft: true
---
```
...
(define-constant ERR-INVALID-SLIPPAGE (err u700))

(define-data-var  transfer-fee-percent uint u2000) ;; 0.2%
...
(define-public (register-dex (dex principal) (status bool))
  (begin
    (try! (check-is-owner)) 
    (ok (map-set registered-dexes dex status))
  )
)
...
(define-public (update-fee (amount uint))
  (begin
    (try! (check-is-owner)) 
    (ok (var-set transfer-fee-percent amount))
  )
)
...
(define-public (cross-dex-swap 
  ...
  (let 
    ...
    (asserts! (> (/ slippage u10000) u0) ERR-INVALID-SLIPPAGE)
      ...
      (begin
        (let
          (
            ...
            (min-amt-4-swap1 (get-last-value quote))
            (result1 (try! (contract-call? dex1 swap amount-after-fee (calc-slippage min-amt-4-swap1 slippage) none route2 factors)))
      ...
      (if to-alex
        (let
          (
            ...
            (min-amt-4-swap1 (get-last-value quote))
            (result1 (try! (contract-call? dex1 swap amount-after-fee (calc-slippage min-amt-4-swap1 slippage) route1 none none)))
            (amt-4-swap2 (get-last-value result1))
...
(define-private (calc-slippage (amount uint) (slippage uint))
  (- amount (/ (* amount slippage) ONE_6))
)
```
