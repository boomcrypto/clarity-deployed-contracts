...
(define-non-fungible-token green-tree-house uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmPwdi8X3N88zaY9ZaKjyHAzXtmJt8QUsxHDa8bRBvxYQs/green-tree-house/")
...
(define-data-var public-target-block-height uint u66808)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? green-tree-house id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace green-tree-house
  (ok (nft-get-owner? green-tree-house id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? green-tree-house next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? green-tree-house id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? green-tree-house id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? green-tree-house id) ERR-NOT-FOUND))
...
(define-data-var alex-cost-per-mint uint u10000)
