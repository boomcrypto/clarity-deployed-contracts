...
(define-map multiple-bids { collection-id: (string-ascii 256), order: uint } { order: uint, ids: (list 5000 uint), buyer: principal, offer: uint })
...
(define-public (multiple-bid (collection-id (string-ascii 256)) (amount uint) (ids (list 5000 uint)))
  (let (
    (order (var-get order-number))
    ...
    (match (stx-transfer? amount tx-sender (as-contract tx-sender))
      success (begin
        ...
        (print (+ u1 order))
        (map-set multiple-bids { collection-id: collection-id, order: (+ order u1) } { order: (+ order u1), ids: ids, buyer: tx-sender, offer: amount })
        (map-set bidder { buyer: tx-sender } { balance: (+ balance amount) })
        (var-set order-number (+ u1 order))
...
(define-read-only (get-multiple-bid (collection-id (string-ascii 256)) (order uint))
  (default-to
    { order: u0, ids: (list ), buyer: CONTRACT-OWNER, offer: u0 }
