...
(define-fungible-token t-alex-v1)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance t-alex-v1 account))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? t-alex-v1 amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? t-alex-v1 amount recipient)
  )
)
...
(define-public (burn (amount uint) (sender principal))
  (begin
    (try! (check-is-approved contract-caller))
    (ft-burn? t-alex-v1 amount sender)
  )
)
...
(define-read-only (get-total-supply-fixed)
  (ok (decimals-to-fixed (ft-get-supply t-alex-v1)))
)
...
(define-read-only (get-balance-fixed (account principal))
  (ok (decimals-to-fixed (ft-get-balance t-alex-v1 account)))
)
