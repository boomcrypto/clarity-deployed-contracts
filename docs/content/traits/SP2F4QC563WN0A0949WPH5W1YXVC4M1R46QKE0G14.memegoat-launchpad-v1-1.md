---
title: "Trait memegoat-launchpad-v1-1"
draft: true
---
```
...
(define-data-var launchpad-fee uint u1) ;; 1% of raised stx
...
;; @desc update-token-launch-status: updates launch state to listed
;; @requirement only callable by current owner
;; @params token-launch-id
;; @returns (response boolean)
(define-public (update-token-launch-status (token-launch-id uint))
  (begin
    (try! (check-is-owner))
    (let
      (
...
(define-public 
  ...
  (begin
    ...
    (let
      ...
      (try! (contract-call? token transfer total-to-send tx-sender .memegoat-launchpad-vault-v1 none))
...
(define-public (deposit-stx (amount uint) (token-launch-id uint) (goat-token-trait <ft-trait-ext>))
  (begin
    ...
    (let
      ...
      (try! (stx-transfer? amount tx-sender .memegoat-launchpad-vault-v1))
...
(define-public (claim-token (token-launch-id uint) (token-trait <ft-trait>))
  (begin
    ...
    (let
      ...
      (as-contract (try! (contract-call? .memegoat-launchpad-vault-v1 transfer-ft token-trait user-allocation sender)))      
...
(define-private (transfer-many-iter (recipient {addr: principal, points: uint}) (factor {total-points: uint, campaign-allocation: uint, rewards-sent: uint, token: <ft-trait>}))
    ...
    ;; transfer token from vault
    (unwrap-panic (as-contract (contract-call? .memegoat-launchpad-vault-v1 transfer-ft token reward addr)))    
    factor-updated
  )
)
