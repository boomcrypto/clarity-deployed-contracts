---
title: "Trait think-build-achieve"
draft: true
---
```
;; think-build-achieve
...
(define-non-fungible-token think-build-achieve uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcS5iPkJcMEuqo8YJS3vzkUF6LmjncsVgpDEw7SRnemgp/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? think-build-achieve next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? think-build-achieve token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? think-build-achieve token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? think-build-achieve token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? think-build-achieve id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? think-build-achieve id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? think-build-achieve id) (err ERR-NOT-FOUND)))
