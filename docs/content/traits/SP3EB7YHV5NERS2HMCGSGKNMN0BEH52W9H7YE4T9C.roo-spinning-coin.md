...
(define-non-fungible-token roo-spinning-coin uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdEZ4U5k2e45r2Q8Y9ND6oKksiKc5xMMpxs76VfrLEABi/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? roo-spinning-coin next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? roo-spinning-coin token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? roo-spinning-coin token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? roo-spinning-coin token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? roo-spinning-coin id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? roo-spinning-coin id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? roo-spinning-coin id) (err ERR-NOT-FOUND)))
