...
(define-non-fungible-token blockcity-girl-v1 uint)
...
(define-data-var total-price uint u29000000)
(define-data-var artist-address principal 'SP2HV5QB20QKNJ2PAN662PXM822EVB2E5P8ZXNTBN)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVZ67cx4jiiPgHzzNWr7HyXSMtjnyqUmc5CMPDdxvq3K3/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? blockcity-girl-v1 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? blockcity-girl-v1 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? blockcity-girl-v1 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? blockcity-girl-v1 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? blockcity-girl-v1 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? blockcity-girl-v1 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? blockcity-girl-v1 id) (err ERR-NOT-FOUND)))
