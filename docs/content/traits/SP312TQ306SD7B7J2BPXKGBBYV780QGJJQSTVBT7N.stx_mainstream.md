...
;; Variables
(define-fungible-token STXM MAXSUPPLY)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://example.com/image.jpg"))
(define-data-var token-name (string-ascii 32) "stx_mainstream")
(define-data-var token-symbol (string-ascii 32) "STXM")
(define-data-var token-decimals uint u6)

;; Read-Only Functions
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance STXM owner))
)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance STXM from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? STXM amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
;; Mint Tokens
(begin
  (let ((result (ft-mint? STXM MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
