...
(define-fungible-token bridged-ubtc)

(define-data-var token-name (string-ascii 32) "uBTC")
(define-data-var token-symbol (string-ascii 10) "uBTC")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-ubtc.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(try! (ft-transfer? bridged-ubtc amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance bridged-ubtc who)))

(define-read-only (get-total-supply)
	(ok (ft-get-supply bridged-ubtc)))
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? bridged-ubtc amount recipient)))
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? bridged-ubtc amount sender)))
