...
(define-fungible-token kangaroo)
;; Define errors
...
(define-constant TOKEN_URI u"http://to-the-moon.s3.ap-southeast-2.amazonaws.com/asset.json") ;; utf-8 string with token metadata host
(define-constant TOKEN_NAME "Kangaroo the Jumping Coin")
(define-constant TOKEN_SYMBOL "$ROO")
(define-constant TOKEN_DECIMALS u6) ;; 8 units displayed past decimal, e.g. 1.00_000_000 = 1 token
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance kangaroo who))
)
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT_OWNER) ERR_OWNER_ONLY)
    (ft-mint? kangaroo amount recipient)
  )
)
...
(begin
    ...
    ;; Transfer the tokens
    (let ((result (ft-transfer? kangaroo amount sender recipient)))
      (if (is-ok result)
        (begin
...
(begin
  (try! (ft-mint? kangaroo u1200000000000 CONTRACT_OWNER)) 
  ;; (try! (ft-mint? kangaroo u100000000000 'ST18VZNQWK7R44BFN75X3RANYRAT4B9ZA8S47KZ24)) 
) 
```
