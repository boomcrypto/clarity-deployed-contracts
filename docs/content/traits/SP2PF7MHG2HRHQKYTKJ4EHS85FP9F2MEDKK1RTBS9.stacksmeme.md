...
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)
  ...
  (define-fungible-token stacksmeme MAX_SUPPLY)
  ...
  (define-public
    ...
    (begin
      ...
      (ft-transfer? stacksmeme amt from to)))


  (define-public (mint (amt uint) (to principal))
    (begin
      (try! (check-owner))
      (ft-mint? stacksmeme amt to) ))

  (define-read-only (get-name)                   (ok "stacksmeme"))
  (define-read-only (get-symbol)                 (ok "STACKSMEME"))
  (define-read-only (get-decimals)               (ok u6))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance stacksmeme of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply stacksmeme)))
