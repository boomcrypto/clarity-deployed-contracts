...
(define-non-fungible-token test-five uint)
...
(define-constant COMM_ADDR 'SP3GDVJ93E47GXZVV963BY1S8GR7HNGQN316SPTF5)

;; Internal variables
(define-data-var mint-limit uint u10)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1000000)
(define-data-var artist-address principal 'SP3GDVJ93E47GXZVV963BY1S8GR7HNGQN316SPTF5)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmfJvo8zxL6FiTPewAzJnLxGqKvbXZ34BHxDkFEyduAoQw/bitcoin_badger_")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? test-five next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? test-five token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? test-five token-id)))
