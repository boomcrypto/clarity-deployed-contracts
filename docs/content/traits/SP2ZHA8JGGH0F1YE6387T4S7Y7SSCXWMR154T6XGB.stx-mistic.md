...
(define-non-fungible-token stx-mistic uint)
...
(define-data-var artist-address principal 'SP2ZHA8JGGH0F1YE6387T4S7Y7SSCXWMR154T6XGB)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWfUAbjWoGQGwqHPFLoUnrmNtpcrBJrpCbUPP3ZKYNN6q/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? stx-mistic next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-mistic token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-mistic token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-mistic token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-mistic id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-mistic id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-mistic id) (err ERR-NOT-FOUND)))
