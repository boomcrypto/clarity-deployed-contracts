---
title: "Trait syvita-mining-mia-v1"
draft: true
---
```
;;                __   __
...
(define-private (contract-claim-mining-reward (block uint))
    (let
      (
        ...
        (coinbaseAmount (as-contract (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.miamicoin-core-v1 get-coinbase-amount block)))
        (isWinner (as-contract (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.miamicoin-core-v1 can-claim-mining-reward POOL_CONTRACT_ADDRESS block)))
      )
      (if isWinner
        (begin
          (try! (as-contract (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.miamicoin-core-v1 claim-mining-reward block)))
...
(define-private (payout-fee (mineManyId uint))
    (let
      ...
      (try! (as-contract (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.miamicoin-token transfer (* (/ coinsWon u100) fee) POOL_CONTRACT_ADDRESS (var-get feeAddress) none)))
      (ok true)
    )
)
...
(define-public (contributor-claim-single-reward-from-pool (mineManyId uint))
   (let
      ...
      (try! (as-contract (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.miamicoin-token transfer (/ (* (* (/ coinsWon u100) (- u100 fee)) currentContribution) totalContributed) POOL_CONTRACT_ADDRESS callerAddress none)))
...
(define-public (contract-mine-many (poolId uint) (amounts (list 200 uint)))
    (let
      ...
      (try! (as-contract (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.miamicoin-core-v1 mine-many amounts)))
