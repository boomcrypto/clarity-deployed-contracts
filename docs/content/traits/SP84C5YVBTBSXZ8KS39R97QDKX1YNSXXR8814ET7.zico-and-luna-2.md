...
(define-non-fungible-token zico-and-luna-2 uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmWmMuBUF5SaHFLXp2BDZ4y1DqSkoVsW8p41END9GfaaR5/zico-and-luna/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP84C5YVBTBSXZ8KS39R97QDKX1YNSXXR8814ET7 )) 
...
(define-data-var public-target-block-height uint u67844)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? zico-and-luna-2 id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace zico-and-luna-2
  (ok (nft-get-owner? zico-and-luna-2 id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? zico-and-luna-2 next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? zico-and-luna-2 id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? zico-and-luna-2 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? zico-and-luna-2 id) ERR-NOT-FOUND))
