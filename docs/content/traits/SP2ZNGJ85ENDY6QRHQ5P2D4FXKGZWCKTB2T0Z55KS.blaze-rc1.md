...
(define-constant message-domain-hash (sha256 (unwrap-panic (to-consensus-buff? { name: "BLAZE_PROTOCOL", version: "rc1", chain-id: chain-id }))))
  ...
  (let
    ...
    (match (secp256k1-recover? hash signature)
      recovered-pubkey (let
          ...
          (ok signer-principal) ;; Return the original signer's principal
        )
...
(define-read-only (verify
  ...
  (let
    ...
    ;; Verify the signature against the hash. Does not check or modify consumption map.
    (match (secp256k1-recover? hash signature)
      recovered-pubkey (ok (principal-of? recovered-pubkey)) ;; Return signer principal on success
      error ERR_INVALID_SIGNATURE ;; Return error on failure
    )
  )
)
...
(define-read-only (get-signer (hash (buff 32)) (signature (buff 65)))
  (match (secp256k1-recover? hash signature)
    public-key (ok (principal-of? public-key))
    error ERR_INVALID_SIGNATURE
  )
)
```
