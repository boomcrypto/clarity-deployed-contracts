---
title: "Trait mega-chill-meteors"
draft: true
---
```
;; mega-chill-meteors
...
(define-non-fungible-token mega-chill-meteors uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQQZt9yMkMgLdgqe8aqMeVM9EZ4CRoSXThXERzAgnjs7N/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? mega-chill-meteors next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? mega-chill-meteors token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? mega-chill-meteors token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? mega-chill-meteors token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? mega-chill-meteors id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? mega-chill-meteors id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? mega-chill-meteors id) (err ERR-NOT-FOUND)))
