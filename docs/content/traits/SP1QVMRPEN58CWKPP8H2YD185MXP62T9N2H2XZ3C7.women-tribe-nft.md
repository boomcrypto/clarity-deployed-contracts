...
(define-non-fungible-token women-tribe-nft uint)
...
(define-data-var artist-address principal 'SP1QVMRPEN58CWKPP8H2YD185MXP62T9N2H2XZ3C7)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQR5i1GEK1RTSV5Hgf8BtUEXK8a311cXNfPRFKmgwtDMP/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? women-tribe-nft next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? women-tribe-nft token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? women-tribe-nft token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? women-tribe-nft token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? women-tribe-nft id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? women-tribe-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? women-tribe-nft id) (err ERR-NOT-FOUND)))
