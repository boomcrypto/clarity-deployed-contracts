...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://example-memetoken.com"))
(define-data-var token-name (string-ascii 32) "MemeFunToken")
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance MEME from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? MEME amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
;; Mint Tokens
(begin
  (let ((result (ft-mint? MEME MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
