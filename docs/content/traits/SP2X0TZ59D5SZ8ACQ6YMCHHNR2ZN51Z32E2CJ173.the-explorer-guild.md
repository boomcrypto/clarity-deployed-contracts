---
title: "Trait the-explorer-guild"
draft: true
---
```
...
(define-constant err-all-minted (err u12))
...
(define-data-var base-token-uri (string-ascii 210) "https://www.explorerguild.io/api/metadata/")
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (match (nft-transfer? The-Explorer-Guild token-id sender recipient)
      success (ok success)
      error (err error))))

;; SIP009: Get the owner of the specified token ID
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? The-Explorer-Guild token-id)))
...
(define-public (set-starting-index)
  (begin
    (asserts! (is-eq tx-sender contract-owner) err-not-owner)
    (asserts! (is-eq (var-get starting-index) u0) err-starting-index-already-set)

    (match (cycle-random-id max-mint-number)
     random-id (begin
        (if (is-eq random-id u0)
            ;; Prevent default sequence by starting at 1 if random id returned is 0
            (ok (var-set starting-index u1))
...
;; Code to generate random number - END
