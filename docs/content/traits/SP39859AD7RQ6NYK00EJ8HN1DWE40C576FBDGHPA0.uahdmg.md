...
;; Define LP token
(define-fungible-token DMG-UAH)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.uahdmg"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? DMG-UAH amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance DMG-UAH who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply DMG-UAH)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? DMG-UAH (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? DMG-UAH (get dk delta) sender))
...
(begin
    ...
    (try! (add-liquidity u161092473))
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.ukrainian-hryvnia-stxcity transfer u4908838907527 tx-sender CONTRACT none)))
```
