...
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)
  ...
  (define-constant MAX_SUPPLY (* u1000000000 (pow u10 u7)))

  (define-fungible-token hell MAX_SUPPLY)
  ...
  (define-read-only (get-name)                   (ok "hell"))
  (define-read-only (get-symbol)                 (ok "HELL"))
  (define-read-only (get-decimals)               (ok u7))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance hell of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply hell)))
