...
(define-non-fungible-token forever-punks-2 uint)
...
(define-data-var artist-address principal 'SP1C370RX4STZWAKBEDCK2DSRWA1GNDYT453570NB)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmU1J3bFaQpDjvtg5rvbymjaB2xicta17U3i4SnUh35pPJ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? forever-punks-2 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? forever-punks-2 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? forever-punks-2 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? forever-punks-2 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? forever-punks-2 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? forever-punks-2 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? forever-punks-2 id) (err ERR-NOT-FOUND)))
