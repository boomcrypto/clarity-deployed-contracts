---
title: "Trait farming-velar-usdc-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .velar-usdc)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .velar-usdc transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .velar-usdc get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .velar-usdc transfer amt protocol user none)))
