---
title: "Trait chabtz-lp-token"
draft: true
---
```
;; Title: CHA-BTZ LP Token
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token CHA-BTZ)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.chabtz-lp-token"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? CHA-BTZ amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance CHA-BTZ who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply CHA-BTZ)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.betrezen-legion-of-the-damned transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.betrezen-legion-of-the-damned transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.betrezen-legion-of-the-damned transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? CHA-BTZ (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? CHA-BTZ (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.betrezen-legion-of-the-damned get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token transfer u900000 tx-sender CONTRACT none)))
```
