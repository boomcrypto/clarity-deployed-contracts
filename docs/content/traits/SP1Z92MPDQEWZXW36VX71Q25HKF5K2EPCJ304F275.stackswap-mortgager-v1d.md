---
title: "Trait stackswap-mortgager-v1d"
draft: true
---
```
...
(define-data-var liquidation-block-length uint u2)

(define-read-only (get-liquidation-block-length)
  (var-get liquidation-block-length)
)

(define-public (set-liquidation-block-length (height uint))
  (begin
    (asserts! (is-eq contract-caller (contract-call? .stackswap-dao-v5k get-dao-owner)) (err ERR-NOT-AUTHORIZED))
    (ok (var-set liquidation-block-length height))
  )
)
...
(define-public (burn-to-unliquidate
  ...
  (let (
    ...
    (asserts! (< block-height (+ (get updated-at-block-height vault) (var-get liquidation-block-length))) (err ERR-VAULT-LIQUIDATION-ENDED))
    (try! (contract-call? .stackswap-vault-data-v1c update-vault vault-id updated-vault))
    (try! (accrue-stability-fee vault-id coll-type))
...
(define-public (finalize-liquidation
  ...
  (let (
    ...
    (asserts! (> block-height (+ (get updated-at-block-height vault) (var-get liquidation-block-length))) (err ERR-VAULT-LIQUIDATION-NOT-ENDED))
...
;; initialization
(map-set stacking-unlock-burn-height { stacker-name: "stacker-l" } { height: u718550 })
