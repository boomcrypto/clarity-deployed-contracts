...
(define-non-fungible-token duka-shape uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmP4WRHEU937FMtnrvoQgAqLZLBx9ZmxPVysZ4GA8wXj6H/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? duka-shape next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? duka-shape token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? duka-shape token-id)))
