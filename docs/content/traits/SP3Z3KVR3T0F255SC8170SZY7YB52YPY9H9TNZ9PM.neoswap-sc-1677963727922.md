---
title: "Trait neoswap-sc-1677963727922"
draft: true
---
```
;; multiway-swap
...
;; traders
(define-constant TRADER-1 'SP2NS78688FKS2G8ENV9B4HC4QR3Z3DNX21CRDFDQ)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (contract-call? 'SP2NS78688FKS2G8ENV9B4HC4QR3Z3DNX21CRDFDQ.loobles transfer u290 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u52000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP2NS78688FKS2G8ENV9B4HC4QR3Z3DNX21CRDFDQ.loobles transfer u290 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u4000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u48000 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2NS78688FKS2G8ENV9B4HC4QR3Z3DNX21CRDFDQ.loobles transfer u290 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u52000 tx-sender TRADER-2)))
	    )
            true
        )
