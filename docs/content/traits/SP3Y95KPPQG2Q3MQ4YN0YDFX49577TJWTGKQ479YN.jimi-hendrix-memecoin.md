---
title: "Trait jimi-hendrix-memecoin"
draft: true
---
```
...
;; Constants
(define-constant MAXSUPPLY u5896297000000)

;; Variables
(define-fungible-token JIMI MAXSUPPLY)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.midjourney.com/f61496b1-955c-4b1a-b2e0-83ef5107f490/0_0.jpeg"))
(define-data-var token-name (string-ascii 32) "Jimi Hendrix Memecoin")
(define-data-var token-symbol (string-ascii 32) "JIMI")
(define-data-var token-decimals uint u6)

;; Read-Only Functions
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance JIMI owner))
)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance JIMI from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? JIMI amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
;; Mint Tokens
(begin
  (let ((result (ft-mint? JIMI MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
