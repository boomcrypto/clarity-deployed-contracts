...
(define-non-fungible-token manuelita uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRuSpi3nUMVxbsDubdMKo2vSxCuGsLKAUdRqEfk8SpcD6/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? manuelita next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? manuelita token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? manuelita token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? manuelita token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? manuelita id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? manuelita id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? manuelita id) (err ERR-NOT-FOUND)))
...
(map-set mint-passes 'SP2KQYDWF51AJKE5G6FRW95TWW8QQTXVT006QG2CT u950)
...
(map-set mint-passes 'SP3H3XD5FM5DQ9ZYQS1A92KFPPCNRTEZBS395QYDE u1)
(map-set mint-passes 'SP10EWQGQ1A944F6J1QW270KQBSHGJ58YK2EZCV4E u1)
(map-set mint-passes 'SP2XT0HS6GPGMWT412HQSFTP6AJXEAXXHMK1SYKME u1)
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? manuelita (+ last-nft-id u0) 'SP383GHGJV8JRJ1K6M723P9SK7V41PN042NWWJBBJ))
      (map-set token-count 'SP383GHGJV8JRJ1K6M723P9SK7V41PN042NWWJBBJ (+ (get-balance 'SP383GHGJV8JRJ1K6M723P9SK7V41PN042NWWJBBJ) u1))
      (try! (nft-mint? manuelita (+ last-nft-id u1) 'SP2NEMS3667D8E6PEA7BXP943TQS6FRHQ51RYZ0PZ))
      (map-set token-count 'SP2NEMS3667D8E6PEA7BXP943TQS6FRHQ51RYZ0PZ (+ (get-balance 'SP2NEMS3667D8E6PEA7BXP943TQS6FRHQ51RYZ0PZ) u1))
      (try! (nft-mint? manuelita (+ last-nft-id u2) 'SP2CJR5N97PT7RT0TDS6YXHSV63GB4XFQD4MHYH4J))
