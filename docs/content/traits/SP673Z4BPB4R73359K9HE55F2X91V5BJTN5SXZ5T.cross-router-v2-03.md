---
title: "Trait cross-router-v2-03"
draft: true
---
```
...
(define-read-only (validate-route (amount uint) (routing-tokens (list 5 principal)) (routing-factors (list 4 uint)) (token-out principal) (min-amount-out (optional uint)) (recipient { address: (buff 128), chain-id: (optional uint) }))
	(let (
		...
		(match (get chain-id recipient)
			some-value
			(if (> some-value u1000)
				(begin (try! (contract-call? .meta-peg-out-endpoint-v2-04 validate-peg-out (get amount route-details) { token: token-out, chain-id: some-value })) (ok true))
...
(define-public (route (amount uint) (routing-tokens (list 5 <ft-trait>)) (routing-factors (list 4 uint)) (token-out-trait <ft-trait>) (min-amount-out (optional uint)) (recipient { address: (buff 128), chain-id: (optional uint) }))
	(let (
		...
		(match (get chain-id recipient) some-value
			(if (> some-value u1000)
				(try! (contract-call? .meta-peg-out-endpoint-v2-04 request-peg-out amount-out (get address recipient) token-out-trait some-value))
				(if (is-eq some-value u0)
			...
			(try! (contract-call? token-out-trait transfer-fixed amount sender (unwrap! (construct-principal (unwrap! (as-max-len? (get address recipient) u20) err-principal-construct)) err-principal-construct) none)))
		(ok true)))
