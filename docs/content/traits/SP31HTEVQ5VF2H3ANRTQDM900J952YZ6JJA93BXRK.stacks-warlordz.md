...
(define-non-fungible-token stacks-warlordz uint)
...
(define-data-var artist-address principal 'SP31HTEVQ5VF2H3ANRTQDM900J952YZ6JJA93BXRK)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcKzCYvE7sYsGGHobN5Bb2GvQM25Kiv464a839PN4StL5/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-warlordz next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-warlordz token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-warlordz token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-warlordz token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-warlordz id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-warlordz id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-warlordz id) (err ERR-NOT-FOUND)))
