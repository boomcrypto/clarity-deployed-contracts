---
title: "Trait mlbct-old-fart"
draft: true
---
```
(impl-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)

(define-fungible-token old-fart)
...
(define-constant dev-principal 'SPEMB0KQRD7PWKY2W2J2Y1Y6Q9YBJ702DWQADE7V)
(define-constant token-supply u690000000)
...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://bafybeiayqqgvfga5vrq6r2xduyukutdo4qb3eoxq4wa5lgg5wzicjh5bjq.ipfs.dweb.link/")) 
...
(define-read-only (get-name) (ok "Old Fart"))
(define-read-only (get-symbol) (ok "FART"))
(define-read-only (get-decimals) (ok u0))
(define-read-only (get-balance (user principal)) (ok (ft-get-balance old-fart user)))
(define-read-only (get-total-supply) (ok (ft-get-supply old-fart)))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? old-fart amount from to)
    )
)
...
(define-public (swap-stx-for-token (stx-amount uint) (min-tokens-out uint)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? old-fart tokens-out (as-contract tx-sender) tx-sender))
      (if (>= (/ (* (+ (var-get real-stx-reserve) (var-get virtual-stx-reserve)) token-supply) (var-get token-reserve)) market-cap-threshold)
        (begin
          (let (
            ...
            (try! (ft-transfer? old-fart burn-amount (as-contract tx-sender) burn-address))
            (try! (ft-transfer? old-fart remaining-tokens (as-contract tx-sender) liquidity-builder-address))
...
(define-public (swap-token-for-stx (token-amount uint) (min-stx-out uint) (recipient principal)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? old-fart token-amount tx-sender (as-contract tx-sender)))
...
(begin
  (try! (ft-mint? old-fart token-supply (as-contract tx-sender)))
