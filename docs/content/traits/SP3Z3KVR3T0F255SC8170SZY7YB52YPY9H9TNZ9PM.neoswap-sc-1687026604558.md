---
title: "Trait neoswap-sc-1687026604558"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/E7wZ91vXIbsQKx2ZnEbg
;; SwapId: JUbHggFjSm3vpal8Jp5e
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u2083458 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
		(unwrap! (contract-call? 'SP3BRRCHKMPBR60V8ES9J5YF40VXWMABWXK4SEB9G.loop-isle transfer u100 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (contract-call? 'SP3BRRCHKMPBR60V8ES9J5YF40VXWMABWXK4SEB9G.loop-isle transfer u100 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u2083458 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u2083458 tx-sender TRADER-1)))
		(unwrap-panic (as-contract (contract-call? 'SP3BRRCHKMPBR60V8ES9J5YF40VXWMABWXK4SEB9G.loop-isle transfer u100 tx-sender TRADER-1)))
	    )
            true
        )
