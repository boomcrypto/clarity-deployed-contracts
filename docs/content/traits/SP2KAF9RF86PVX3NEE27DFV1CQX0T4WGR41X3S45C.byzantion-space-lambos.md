---
title: "Trait byzantion-space-lambos"
draft: true
---
```
...
(define-non-fungible-token space-lambos uint)
...
(define-data-var ipfs-full-metadata (string-ascii 106) "ipfs://bafybeialemp4pnemeo53rw47l7wm6brugmetyrhm6giqy2xrmbcz75a22y/space_lambos/space_lambos_metadata.json")
(define-data-var ipfs-root (string-ascii 93) "ipfs://bafybeialemp4pnemeo53rw47l7wm6brugmetyrhm6giqy2xrmbcz75a22y/space_lambos/space_lambos_")
(define-data-var artist-address principal 'SP3B4QF04MKC7ANSK4SX8244609PNDQGVTDF0W8KE)
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? space-lambos next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? space-lambos token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? space-lambos token-id)))
