...
(define-non-fungible-token skinny-melons uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaUzPqe858g9yqe5gnwvPto7NmGfaUyWdCMPpfafnVDZn/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? skinny-melons next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? skinny-melons token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? skinny-melons token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? skinny-melons token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? skinny-melons id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? skinny-melons id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? skinny-melons id) (err ERR-NOT-FOUND)))
