...
(define-non-fungible-token warm-squares uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmb8S6nTH9xisRMpNw6gCzkze69UXv7wC6cvUNXrPXtJVJ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? warm-squares next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? warm-squares token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? warm-squares token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? warm-squares token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? warm-squares id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? warm-squares id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? warm-squares id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u283)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u1104)
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u5388)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u2)
