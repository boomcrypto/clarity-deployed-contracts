...
(define-non-fungible-token crypto-mayors uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQZ5nEZEtJNkhj53EERoQKLY4YSpGmYkMpx6HWNZVyoni/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? crypto-mayors next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? crypto-mayors token-id sender recipient)))

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? crypto-mayors token-id)))
