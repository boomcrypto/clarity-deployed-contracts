...
;; Define LP token
(define-fungible-token iouWELSHiouROOLP)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SPGYCP878RYFVT03ZT8TWGPKNYTSQB1578VVXHGE.iwr"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? iouWELSHiouROOLP amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance iouWELSHiouROOLP who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply iouWELSHiouROOLP)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-roo transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-roo transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-roo transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? iouWELSHiouROOLP (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? iouWELSHiouROOLP (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-roo get-balance CONTRACT))
    })
...
(define-read-only (get-liquidity-quote (amount uint))
    (let (
        (k (ft-get-supply iouWELSHiouROOLP))
        (reserves (get-reserves)))
        {
...
(define-read-only (get-reserves-quote)
    (let (
        (reserves (get-reserves))
        (supply (ft-get-supply iouWELSHiouROOLP)))
        {
          dx: (get a reserves),
          dy: (get b reserves),
          dk: supply
...
(begin
    ...
    (try! (add-liquidity u150000000000))
    
    ;; Transfer additional token A to achieve desired ratio
    (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-welsh transfer u4850000000000 tx-sender CONTRACT none)))
```
