...
(define-fungible-token wchillguy)

(define-data-var token-name (string-ascii 32) "Just a Chill Guy Wrapper")
(define-data-var token-symbol (string-ascii 32) "wchillguy")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wchillguy.json"))
...
(define-read-only (get-base-token)
  'SP3CTAW6MTJJJ741V19PH9RVZHK2QXT7E4EXSBVDN.just-a-chill-guy)

(define-read-only (get-base-decimals)
  (contract-call? 'SP3CTAW6MTJJJ741V19PH9RVZHK2QXT7E4EXSBVDN.just-a-chill-guy get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP3CTAW6MTJJJ741V19PH9RVZHK2QXT7E4EXSBVDN.just-a-chill-guy get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP3CTAW6MTJJJ741V19PH9RVZHK2QXT7E4EXSBVDN.just-a-chill-guy transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
