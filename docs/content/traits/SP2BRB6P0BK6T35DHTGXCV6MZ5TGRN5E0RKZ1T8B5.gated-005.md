...
;; Add these at the top of your contract
(define-data-var base-url (string-ascii 80) "https://fuchsia-tropical-alligator-381.mypinata.cloud/")
(define-data-var ipfs-cid (string-ascii 80) "QmYourIPFSHash")
(define-data-var metadata-path (string-ascii 80) "metadata/")
(define-data-var contract-uri (string-ascii 256) "ipfs://YOUR_CONTRACT_METADATA_CID")
...
(define-private (pay-royalty (price uint) (royalty uint))
 ...
 ;; Update functions for base URL, IPFS CID and metadata path
(define-public (set-base-url (new-url (string-ascii 80)))
    (begin
        (asserts! (is-eq tx-sender DEPLOYER) ERR-NOT-AUTHORIZED)
        (asserts! (not (var-get metadata-frozen)) ERR-METADATA-FROZEN)
        (var-set base-url new-url)
        (ok true)))

(define-public (set-ipfs-cid (new-cid (string-ascii 80)))
    (begin
        (asserts! (is-eq tx-sender DEPLOYER) ERR-NOT-AUTHORIZED)
        (asserts! (not (var-get metadata-frozen)) ERR-METADATA-FROZEN)
        (var-set ipfs-cid new-cid)
        (ok true)))

(define-public (set-metadata-path (new-path (string-ascii 80)))
    (begin
        (asserts! (is-eq tx-sender DEPLOYER) ERR-NOT-AUTHORIZED)
        (asserts! (not (var-get metadata-frozen)) ERR-METADATA-FROZEN)
        (var-set metadata-path new-path)
        (ok true)))
...
;; Get full URI for token metadata
(define-read-only (get-full-uri (id uint))
    (ok (some (concat (concat (concat (var-get base-url) "ipfs/") (var-get ipfs-cid)) (var-get metadata-path)))))
```
