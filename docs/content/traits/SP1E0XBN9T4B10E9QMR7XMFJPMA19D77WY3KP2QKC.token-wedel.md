...
(define-fungible-token wedel)

(define-data-var token-name (string-ascii 32) "EDLC Wrapper")
(define-data-var token-symbol (string-ascii 32) "wedel")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wedel.json"))
...
(define-read-only (get-base-token)
  'SP26PZG61DH667XCX51TZNBHXM4HG4M6B2HWVM47V.edelcoin)

(define-read-only (get-base-decimals)
  (contract-call? 'SP26PZG61DH667XCX51TZNBHXM4HG4M6B2HWVM47V.edelcoin get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP26PZG61DH667XCX51TZNBHXM4HG4M6B2HWVM47V.edelcoin get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP26PZG61DH667XCX51TZNBHXM4HG4M6B2HWVM47V.edelcoin transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
