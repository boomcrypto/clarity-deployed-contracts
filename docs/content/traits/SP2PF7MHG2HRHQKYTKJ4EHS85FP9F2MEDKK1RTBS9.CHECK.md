...
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)
  ...
  (define-fungible-token CHECK MAX_SUPPLY)
  ...
  (define-public
    ...
    (begin
      ...
      (ft-transfer? CHECK amt from to)))


  (define-public (mint (amt uint) (to principal))
    (begin
      (try! (check-owner))
      (ft-mint? CHECK amt to) ))

  (define-read-only (get-name)                   (ok "CHECK"))
  (define-read-only (get-symbol)                 (ok "CHECK"))
  (define-read-only (get-decimals)               (ok u6))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance CHECK of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply CHECK)))
