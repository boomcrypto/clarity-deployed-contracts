...
(define-non-fungible-token stack-vip-pi-card uint)
...
(define-data-var total-price uint u7572000)
(define-data-var artist-address principal 'SP243SRW37VNCWM3ZRYN71MSACN8BKPVXPX5NPHJF)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYVM7YPBRW4DLksYTnx3XbGeDQtbaonqiQLCaHtAVCL7R/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? stack-vip-pi-card next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stack-vip-pi-card token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stack-vip-pi-card token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stack-vip-pi-card token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stack-vip-pi-card id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stack-vip-pi-card id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stack-vip-pi-card id) (err ERR-NOT-FOUND)))
