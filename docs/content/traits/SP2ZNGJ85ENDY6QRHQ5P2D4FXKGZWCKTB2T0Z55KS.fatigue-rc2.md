...
;; 3. Default Burn Amount: Defaults to 10 energy (10,000,000).
...
;; Data Variables
(define-data-var contract-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/explore/fatigue-rc2"))
...
(define-private (burn-energy-action (sender principal))
  (begin
    (match (contract-call? .dungeon-keeper-rc2 exhaust (/ (var-get energy-burn-amount) u10) sender) success true error false)
    (match (contract-call? .dungeon-keeper-rc2 exhaust (/ (var-get energy-burn-amount) u10) sender) success true error false)
    (match (contract-call? .dungeon-keeper-rc2 exhaust (/ (var-get energy-burn-amount) u10) sender) success true error false)
    (match (contract-call? .dungeon-keeper-rc2 exhaust (/ (var-get energy-burn-amount) u10) sender) success true error false)
    (match (contract-call? .dungeon-keeper-rc2 exhaust (/ (var-get energy-burn-amount) u10) sender) success true error false)
