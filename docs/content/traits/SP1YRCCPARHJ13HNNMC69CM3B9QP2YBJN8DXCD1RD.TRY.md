...
(define-constant token-first-buy (* u9 DECI))  
...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://memefun.s3.ap-northeast-1.amazonaws.com/metadata/uk8ww0cro80urhy0yhc3h2ghbtvpxcc2.json?"))
(define-data-var is-minted bool false)


(define-fungible-token TRY token-max-supply)
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance TRY who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply TRY)))
...
(define-public (mint (amount uint) (recipient principal)) 
    (begin 
        ...
        (try! (ft-mint? TRY amount recipient))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34)))) 
    (begin
        ...
        (try! (ft-transfer? TRY amount sender recipient))
