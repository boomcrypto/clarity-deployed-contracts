---
title: "Trait happy-international-corgi-day"
draft: true
---
```
...
(define-non-fungible-token happy-international-corgi-day uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeCwdUgppDMKtq27bpJErEt9QPyF5Xu1MNaiy5HVBnUwY/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? happy-international-corgi-day next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? happy-international-corgi-day token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? happy-international-corgi-day token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? happy-international-corgi-day token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? happy-international-corgi-day id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? happy-international-corgi-day id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? happy-international-corgi-day id) (err ERR-NOT-FOUND)))
