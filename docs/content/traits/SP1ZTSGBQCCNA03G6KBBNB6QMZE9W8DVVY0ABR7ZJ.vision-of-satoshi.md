...
(define-non-fungible-token vision-of-satoshi uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRC1CJgeuepUzXNfVx4h8ZAtpupG5Y84FYFzcECVfyUAE/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? vision-of-satoshi next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? vision-of-satoshi token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? vision-of-satoshi token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? vision-of-satoshi token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? vision-of-satoshi id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? vision-of-satoshi id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? vision-of-satoshi id) (err ERR-NOT-FOUND)))
