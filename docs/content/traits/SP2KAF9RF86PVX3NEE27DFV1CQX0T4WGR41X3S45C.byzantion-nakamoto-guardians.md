---
title: "Trait byzantion-nakamoto-guardians"
draft: true
---
```
...
(define-non-fungible-token nakamoto-guardians uint)
...
(define-constant MINT-LIMIT u40)

;; Internal variables
(define-data-var last-id uint u0)
(define-data-var uri-prefix (string-ascii 256) "")
(define-data-var cost-per-mint uint u200000000)
(define-data-var commission uint u1500)
(define-data-var payout uint u0)
(define-data-var set-limit uint u20)
(define-data-var set-length uint u20)
(define-data-var artist-address principal 'SP35FG6SHERAY92DN14BBE42BFNFBVEHTN9DCG7EE)
(define-data-var minting-enabled bool true)
(define-map sets uint { ipfs-full-metadata: (string-ascii 123), ipfs-root: (string-ascii 110) })
  ...
  (let (
      ...
      (asserts! (< count (var-get set-limit)) (err ERR-SET-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? nakamoto-guardians next-id new-owner))
...
(define-public (add-set
  (ipfs-full-metadata (string-ascii 123))
  (ipfs-root (string-ascii 110))
)
  (begin
    (asserts!
      (or
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? nakamoto-guardians token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? nakamoto-guardians token-id)))
