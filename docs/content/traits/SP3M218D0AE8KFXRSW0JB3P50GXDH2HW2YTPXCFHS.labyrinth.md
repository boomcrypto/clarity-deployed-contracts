...
(define-non-fungible-token labyrinth uint)
...
(define-data-var artist-address principal 'SP3M218D0AE8KFXRSW0JB3P50GXDH2HW2YTPXCFHS)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPKbxCuUZGQmftQJU7mT81rvSLe6PNMP8428bAMAPEJAh/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? labyrinth next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? labyrinth token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? labyrinth token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? labyrinth token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? labyrinth id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? labyrinth id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? labyrinth id) (err ERR-NOT-FOUND)))
