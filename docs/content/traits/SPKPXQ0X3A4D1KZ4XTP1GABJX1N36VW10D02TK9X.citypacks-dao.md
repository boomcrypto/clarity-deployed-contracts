...
(define-public (mint (nft <nft-trait>) (comm <commission-trait>))
  (begin
    ...
    (try! (add-member tx-sender "citypacks-member"))
    (ok true)
  )
)
...
;; data maps and vars
(define-data-var dao-name (string-ascii 256) "CityPacksDAO")
...
(define-public (transfer (nft <nft-trait>) (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (try! (add-member recipient "citypacks-member"))
