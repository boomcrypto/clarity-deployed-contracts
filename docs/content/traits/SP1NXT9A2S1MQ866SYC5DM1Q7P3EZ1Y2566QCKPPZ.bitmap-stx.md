...
(define-non-fungible-token bitmap-stx uint)
...
;; Internal variables
(define-data-var mint-limit uint u50000)
(define-data-var last-id uint u1)
(define-data-var total-price uint u100000)
(define-data-var artist-address principal 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcSaurkRW1pcuRCmmFskQpMbKtQ7pMm5teM28Xrqj1hGG/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitmap-stx next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitmap-stx token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitmap-stx token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitmap-stx token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitmap-stx id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitmap-stx id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitmap-stx id) (err ERR-NOT-FOUND)))
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? bitmap-stx (+ last-nft-id u0) 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD))
      (map-set token-count 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD (+ (get-balance 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD) u1))
      (try! (nft-mint? bitmap-stx (+ last-nft-id u1) 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD))
      (map-set token-count 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD (+ (get-balance 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD) u1))
      (try! (nft-mint? bitmap-stx (+ last-nft-id u2) 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD))
      (map-set token-count 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD (+ (get-balance 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD) u1))
      (try! (nft-mint? bitmap-stx (+ last-nft-id u3) 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD))
      (map-set token-count 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD (+ (get-balance 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD) u1))
