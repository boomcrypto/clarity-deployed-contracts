---
title: "Trait lydian-airdrop-v1-2"
draft: true
---
```
...
(define-read-only (get-ldn-for-stdiko-pool (user principal) (block uint))
  (let (
    ...
    (stdiko-governance (get amount (at-block block-hash (contract-call? .arkadiko-governance-v2-1 get-tokens-by-member-by-id u6 user .stdiko-token))))
    (stdiko-total (+ stdiko-wallet stdiko-governance))
    (stdiko-supply (unwrap-panic (at-block block-hash (contract-call? .stdiko-token get-total-supply))))

    ;; Pool percentage
    (pool-percentage (/ (* stdiko-total u1000000000000) stdiko-supply))
