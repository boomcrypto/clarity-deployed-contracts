---
title: "Trait kind-lavender-vole-28"
draft: true
---
```
...
(define-read-only (get-apy
    ...
    (let (
        (reward-program-income-state (unwrap-panic (get-reward-program-income supplied-asset reward-asset)))
        (liquidity-rate (get liquidity-rate reward-program-income-state))
    )
    (ok {
        apy: (unwrap-panic (convert-to supplied-asset reward-asset liquidity-rate))
        })
    ) 
)
