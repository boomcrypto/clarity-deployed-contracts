---
title: "Trait MOCHINES-WOJAK-MASK"
draft: true
---
```
;; nft Contract
...
(define-non-fungible-token Wojak_Mask uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafkreidcrw3wverx44hrzhdjuufysueqbxljbjbrryn4dbigsfwtbxvqg4")
...
(define-public (mint (recipient principal))
    (begin 
        ...
        (asserts! (< (var-get last-id) SUPPLY) (err ERR-NO-MORE-NFTS))
            ...
            (try! (nft-mint? Wojak_Mask u1 tx-sender))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Wojak_Mask token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Wojak_Mask token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Wojak_Mask token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Wojak_Mask id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Wojak_Mask id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Wojak_Mask id) (err ERR-NOT-FOUND)))
