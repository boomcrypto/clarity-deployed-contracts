---
title: "Trait stacks-little-cats"
draft: true
---
```
;; stacks-little-cats
...
(define-non-fungible-token stacks-little-cats uint)
...
(define-data-var artist-address principal 'SP3N7SDEMYGVDNSEJBY2YWQ5Q1EF9BG1CCVR44HMC)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmfP742QxuLFC7HqB14SBJ6B8N5yYqkPqSnuFBT8V1vX3b/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-little-cats next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-little-cats token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-little-cats token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-little-cats token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-little-cats id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-little-cats id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-little-cats id) (err ERR-NOT-FOUND)))
