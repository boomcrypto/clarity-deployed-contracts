...
(define-non-fungible-token runestone-punks uint)
...
;; Internal variables
(define-data-var mint-limit uint u3971)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1000000)
(define-data-var artist-address principal 'SP2KYTW30AB40H686SWEHGFRXA0JM6VPFM45HRD2Y)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcZpbcyGFG5U2bW4hpuJBqQQfCSrHz8eCb8TEh1XZpJbM/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? runestone-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? runestone-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? runestone-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? runestone-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? runestone-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? runestone-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? runestone-punks id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u5466)
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? runestone-punks (+ last-nft-id u0) 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD))
      (map-set token-count 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD (+ (get-balance 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD) u1))
      (try! (nft-mint? runestone-punks (+ last-nft-id u1) 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD))
      (map-set token-count 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD (+ (get-balance 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD) u1))
      (try! (nft-mint? runestone-punks (+ last-nft-id u2) 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD))
      (map-set token-count 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD (+ (get-balance 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD) u1))
      (try! (nft-mint? runestone-punks (+ last-nft-id u3) 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD))
      (map-set token-count 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD (+ (get-balance 'SP38C4PCSDNSXD50PE28BM9NNTQ0J2CZ3D190M1XD) u1))
