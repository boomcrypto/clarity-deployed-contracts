...
(define-non-fungible-token ordinals-adipose uint)
...
(define-data-var artist-address principal 'SP2403GWHVWHM00NV8HDKJC6EG756WGF8SG46T74J)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSNK8a9qgSaotC6SKxWGo4rFnrCXWcZtyKYuuyxmu64yX/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinals-adipose next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinals-adipose token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinals-adipose token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinals-adipose token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinals-adipose id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinals-adipose id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinals-adipose id) (err ERR-NOT-FOUND)))
