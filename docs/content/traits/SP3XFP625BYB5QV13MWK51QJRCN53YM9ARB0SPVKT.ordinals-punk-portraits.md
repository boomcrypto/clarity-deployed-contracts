---
title: "Trait ordinals-punk-portraits"
draft: true
---
```
;; ordinals-punk-portraits
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token ordinals-punk-portraits uint)
...
(define-data-var artist-address principal 'SP3XFP625BYB5QV13MWK51QJRCN53YM9ARB0SPVKT)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmen81DdXSKXZTAbdbjzFpJy4ReRcSs5iMMtbmRgoQA2NU/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinals-punk-portraits next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinals-punk-portraits token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinals-punk-portraits token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinals-punk-portraits token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinals-punk-portraits id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinals-punk-portraits id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinals-punk-portraits id) (err ERR-NOT-FOUND)))
