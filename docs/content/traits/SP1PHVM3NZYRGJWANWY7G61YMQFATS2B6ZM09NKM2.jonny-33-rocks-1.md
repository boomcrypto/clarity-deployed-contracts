...
(define-non-fungible-token jonny-33-rocks-1 uint)
...
(define-constant MINT-LIMIT u500)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://SAMPLE_META_HASH/")
...
(define-data-var private-target-block-height uint u64215)
(define-data-var public-target-block-height uint u64503)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? jonny-33-rocks-1 id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace jonny-33-rocks-1
  (ok (nft-get-owner? jonny-33-rocks-1 id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? jonny-33-rocks-1 next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? jonny-33-rocks-1 id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? jonny-33-rocks-1 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? jonny-33-rocks-1 id) ERR-NOT-FOUND))
...
(define-public (pay (id uint) (price uint))
    (if (> (var-get secondary-commission) u0)
        (begin  
                ...
                ;; total of 5% = u500
                  (try! (stx-transfer? (/ (* price u550) u10000) tx-sender 'SP2XQ2Y69BS0C6TYEVEEVY734QWCHFZ1Q4SM0WD47))
...
(define-data-var alex-cost-per-mint uint u85000000)
