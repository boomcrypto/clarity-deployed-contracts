---
title: "Trait BabyMice"
draft: true
---
```
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)

  (define-constant MAX_SUPPLY (* u8000000 (pow u10 u4)))

  (define-fungible-token BabyMice MAX_SUPPLY)
  ...
  (define-public
    ...
    (begin
      ...
      (ft-transfer? BabyMice amt from to)))


  (define-public (mint (amt uint) (to principal))
    (begin
      (try! (check-owner))
      (ft-mint? BabyMice amt to) ))

  (define-read-only (get-name)                   (ok "BabyMice"))
  (define-read-only (get-symbol)                 (ok "BabyMice"))
  (define-read-only (get-decimals)               (ok u4))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance BabyMice of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply BabyMice)))
