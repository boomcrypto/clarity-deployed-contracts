---
title: "Trait logical-aquamarine-wallaby"
draft: true
---
```
;; bridge-stx-psbtc-btc-v-1-1
...
(define-public (get-helper-a (amount uint))
  (let (
    ...
    ;; Get quote for STX to psBTC swap via XYK Core
    (quote-stx-to-psbtc (try! (contract-call?
    ...
    (pontis-ft-fee (contract-call? 'SP14NS8MVBRHXMM96BQY0727AJ59SWPV7RMHC0NCG.pontis-bridge-fee-manager calculate-btc-percent-fee quote-stx-to-psbtc))

    ;; Calculate amount bridged out after subtracting the percent fee
    (amount-after-pontis-ft-fee (- quote-stx-to-psbtc pontis-ft-fee))
  )
    ;; Return action data
    (ok {
      amount: amount,
      ...
      quote-stx-to-psbtc: quote-stx-to-psbtc,
...
;; Swap STX to psBTC and then bridge out psBTC to specified Bitcoin address
(define-public (bridge-helper-a
  ...
  (let (
    ...
    ;; Perform STX to psBTC swap via XYK Core
    (swap-stx-to-psbtc (try! (contract-call?
    ...
    (bridge-btc-out (try! (pontis-bridge-btc-out swap-stx-to-psbtc btc-address pontis-btc-contract)))
  )
    (begin
      ...
      (print {
        ...
        data: {
          ...
          bridge-fee-amount: bridge-fee-amount,
          pontis-stx-fee: pontis-stx-fee,
          swap-stx-to-psbtc: swap-stx-to-psbtc,
