---
title: "Trait the-capuchins-monkeys"
draft: true
---
```
;; the-capuchins-monkeys
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token the-capuchins-monkeys uint)
...
(define-data-var artist-address principal 'SP3SJAVVKKNFCPC9B1DQCPZWEG1SX1C9DNFJBC7S2)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZ33PNonbUH7ALqv8ofYY9dt4HKHaKwcXJfaD98vHL16L/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-capuchins-monkeys next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-capuchins-monkeys token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-capuchins-monkeys token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-capuchins-monkeys token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-capuchins-monkeys id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-capuchins-monkeys id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-capuchins-monkeys id) (err ERR-NOT-FOUND)))
