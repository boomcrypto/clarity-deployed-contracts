---
title: "Trait x-lp-rc1"
draft: true
---
```
;; Title: Extreme Liquidity Provisions
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token x-lp)
;; (define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.extreme-liquidity-provisions"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? x-lp amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "Extreme Liquidity Provisions"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance x-lp who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply x-lp)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? x-lp (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? x-lp (get dk delta) sender))
...
(define-public (x-add-liquidity (signature-a (buff 65)) (signature-b (buff 65)) (amount uint) (uuid (string-ascii 36)))
    (let (
        (delta (get-liquidity-quote amount))
        ...
        (signer-a (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.blaze-rc10 recover signature-a
            'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token-subnet-rc6 "TRANSFER_TOKENS" none (some amount-a) (some CONTRACT) uuid)))

        (signer-b (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.blaze-rc10 recover signature-b
            'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.welsh-token-subnet-rc2 "TRANSFER_TOKENS" none (some amount-b) (some CONTRACT) uuid))))

        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.charisma-token-subnet-rc6 x-transfer signature-a amount-a uuid CONTRACT))
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.welsh-token-subnet-rc2 x-transfer signature-b amount-b uuid CONTRACT))
        (try! (ft-mint? x-lp (/ (get dk delta) u2) signer-a))
        (try! (ft-mint? x-lp (/ (get dk delta) u2) signer-b))
        (ok delta)))
