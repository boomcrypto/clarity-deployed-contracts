---
title: "Trait byzantion-bitcoin-bulls"
draft: true
---
```
...
(define-non-fungible-token bitcoin-bulls uint)
...
(define-data-var target-block uint u36787)
(define-data-var ipfs-full-metadata (string-ascii 108) "ipfs://bafybeihbuprqzqi5jml73mogx6lty53fw4h47brqso32a4r7goifiq6pdi/bitcoin_bulls/bitcoin_bulls_metadata.json")
(define-data-var ipfs-root (string-ascii 95) "ipfs://bafybeihbuprqzqi5jml73mogx6lty53fw4h47brqso32a4r7goifiq6pdi/bitcoin_bulls/bitcoin_bulls_")
...
(define-data-var artist-address-two principal 'SP217FZ8AZYTGPKMERWZ6FYRAK4ZZ6YHMJ7XQXGEV)
(define-data-var artist-payout-two uint u3300)
(define-data-var artist-address-three principal 'SP3Y5WK0G9GMXS4YRNW9SSVEET0WFJM37X2SBEW99)
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? bitcoin-bulls next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? bitcoin-bulls token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-bulls token-id)))
