...
(define-fungible-token token-link)

(define-data-var token-name (string-ascii 32) "LINK")
(define-data-var token-symbol (string-ascii 10) "LINK")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-link.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(try! (ft-transfer? token-link amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance token-link who)))

(define-read-only (get-total-supply)
	(ok (ft-get-supply token-link)))
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? token-link amount recipient)))
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? token-link amount sender)))
