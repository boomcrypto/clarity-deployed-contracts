---
title: "Trait welsh-credits-rc1"
draft: true
---
```
;; title: welsh-credits-rc1
;; author: rozar.btc
...
(define-constant ERR_TRANSFER_FAILED       (err u105)) ;; Error during deposit/withdraw token transfer
...
(define-constant ERR_SIGNER_CALL_FAILED    (err u107)) ;; Error calling the blaze-signer contract
...
;;       (balance-key { owner: effective-recipient }) ;; Key for the effective recipient's credit balance
;;       (current-recipient-balance (default-to { amount: u0 } (map-get? balances balance-key)))
;;     )
...
;;       { amount: (+ (get amount current-recipient-balance) amount) }
;;     )
...
;;       (balance-key { owner: owner }) ;; Balance key is always the owner (tx-sender)
;;       (current-owner-balance (default-to { amount: u0 } (map-get? balances balance-key)))
;;     )
;;     ;; 1. Check sufficient internal credit balance of the owner (tx-sender)
;;     (asserts! (>= (get amount current-owner-balance) amount) ERR_INSUFFICIENT_BALANCE)

;;     ;; 2. First reduce the internal credit balance of the owner
;;     (map-set balances balance-key
;;       { amount: (- (get amount current-owner-balance) amount) }
;;     )

;;     ;; 3. Then transfer actual tokens from this contract TO the effective recipient
;;     (try! (as-contract (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer amount tx-sender effective-recipient none)) ERR_TRANSFER_FAILED)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (let
    ...
    (map-set balances from-balance-key { amount: (- (get amount current-from-balance) amount) })
    (map-set balances to-balance-key { amount: (+ (get amount current-to-balance) amount) })
...
;;       (amount-str (concat "TRANSFER_" (int-to-ascii amount)))
;;       (signer-principal (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.blaze-rc5 submit signature amount-str uuid) ERR_SIGNER_CALL_FAILED))
;;     )
...
(define-read-only (get-balance (owner principal))
  (get amount (default-to { amount: u0 } (map-get? balances { owner: owner })))
)
...
(define-private (do-internal-transfer (from principal) (to principal) (amount uint))
  (let
    ...
    (asserts! (>= (get amount current-from-balance) amount) ERR_OPCODE_ACTION_FAILED) ;; Check 'from' balance
    (map-set balances from-balance-key { amount: (- (get amount current-from-balance) amount) })
    (map-set balances to-balance-key { amount: (+ (get amount current-to-balance) amount) })
    (ok true)
  )
