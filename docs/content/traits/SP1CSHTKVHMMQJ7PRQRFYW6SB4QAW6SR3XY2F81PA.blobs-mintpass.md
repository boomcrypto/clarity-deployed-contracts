...
(define-non-fungible-token blobs-mintpass uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmadTcig8Wq5fSeqTBxZ4aPM89cM6MfNvSc5W9Yb9jcR2m/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? blobs-mintpass next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? blobs-mintpass token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? blobs-mintpass token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? blobs-mintpass token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? blobs-mintpass id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? blobs-mintpass id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? blobs-mintpass id) (err ERR-NOT-FOUND)))
