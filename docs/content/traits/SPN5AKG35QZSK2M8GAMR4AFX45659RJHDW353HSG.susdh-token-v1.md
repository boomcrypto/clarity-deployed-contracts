...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (if (var-get only-protocol) 
      (asserts! (or (contract-call? .hq-v1 get-contract-active sender) (contract-call? .hq-v1 get-contract-active recipient)) ERR_ONLY_PROTOCOL)
      true
    )

    (if (var-get blacklist-enabled)
      (try! (contract-call? .blacklist-susdh-v1 check-is-not-full-blacklist-two sender recipient))
      true
    )
