---
title: "Trait burton-snowboards-collection"
draft: true
---
```
;; Burton Snowboards
...
(define-non-fungible-token Burton uint)
...
(define-data-var second-artist-address principal 'SP1KHNCZ5XRPSKA7ZZ0Q7WNS5P069DE2Z6ZPD2VSS) ;; ST (2.5%)
(define-data-var third-artist-address principal 'SP1KHNCZ5XRPSKA7ZZ0Q7WNS5P069DE2Z6ZPD2VSS) ;; SV (10%)
(define-data-var artist-address principal 'SP1KHNCZ5XRPSKA7ZZ0Q7WNS5P069DE2Z6ZPD2VSS) ;; OAS (rest)
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? Burton next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? Burton token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Burton token-id)))
