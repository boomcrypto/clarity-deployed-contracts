---
title: "Trait brc20-bridge-endpoint-dev-preview-8"
draft: true
---
```
...
(define-public (finalize-peg-in (tx (buff 4096)) (output-idx uint) (offset-idx uint) (order-idx uint) (token-trait <sip010-trait>))
	(let (
		...
		(if (is-eq dest u0) ;; map cannot hold traits, so these have to be hard-coded
			...
			(begin 
				...
				(and (> amt-net u0) (as-contract (try! (contract-call? stxdx-wallet-zero transfer-in 
					...
					(unwrap! (contract-call? stxdx-registry get-asset-id token) err-asset-not-supported-on-b20) ;; asset-id
					token-trait))))			
			)
		)
		(print (merge tx-idxed { type: "peg-in", order-address: order-address, fee: fee, amt-net: amt-net, dest: dest }))
		(ok true)))
