...
(define-public (confirm-mint (request-id uint) (price uint) (amount-asset-confirmed uint) (minting-asset <sip-010-trait>) (custody-address principal) (memo (optional (buff 34))))
  (let (
    ...
    (asserts! (<= amount-asset-confirmed amount-requested) ERR_AMOUNT_NOT_ALLOWED)
    ...
    (if (not (is-eq amount-requested amount-asset-confirmed))
      (try! (as-contract (contract-call? minting-asset transfer (- amount-requested amount-asset-confirmed) tx-sender requester none)))
      true
    )
...
(define-public (confirm-redeem (request-id uint) (price uint) (amount-usdh-confirmed uint) (redeeming-asset <sip-010-trait>))
  (let (
    ...
    (try! (contract-call? .test-usdh-token burn-for-protocol (- amount-usdh-confirmed amount-usdh-fee) this-contract))
    (if (> amount-usdh-fee u0) (try! (contract-call? .test-usdh-token transfer amount-usdh-fee this-contract fee-address none)) true)
    (if (not (is-eq amount-usdh-requested amount-usdh-confirmed))
      (try! (contract-call? .test-usdh-token transfer (- amount-usdh-requested amount-usdh-confirmed) this-contract requester none))
      true
    )
