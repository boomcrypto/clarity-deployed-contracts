---
title: "Trait god-did-acappella-nft"
draft: true
---
```
;; god-did-acappella-nft
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token god-did-acappella-nft uint)
...
(define-data-var total-price uint u23000000)
(define-data-var artist-address principal 'SPGGAEQWA7Y9HRZY5T0XJCEYEZ28J6RKCCC1HP9M)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVTnVe9tbBq1DRtck54gLxL4CTUcmbc3g7wbvjVHoiBB8/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? god-did-acappella-nft next-id tx-sender) next-id)
      (unwrap! (nft-transfer? god-did-acappella-nft next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? god-did-acappella-nft token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? god-did-acappella-nft token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? god-did-acappella-nft token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? god-did-acappella-nft id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? god-did-acappella-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? god-did-acappella-nft id) (err ERR-NOT-FOUND)))
