---
title: "Trait Cerulean-SIP10-Bounties-v1"
draft: true
---
```
...
;; Define commission
(define-data-var commission-rate uint u30) ;; 3%
...
(define-public (create-bounty (amount uint) (bounty-title (string-ascii 200)) (bounty-id (string-ascii 36)) (token-name <token>))
  (let (
        (commission (/ (* amount (var-get commission-rate)) u1000))
    ...
    (try! (contract-call? token-name transfer bounty-amount tx-sender (as-contract tx-sender) none))
...
;; #[allow(unchecked_data)]
(define-public (accept-submission (bounty-id (string-ascii 36)) (submitter principal) (used-token <token>))
  (let (
        ...
        (as-contract (try! (contract-call? used-token transfer (get bounty-amount bounty) tx-sender submitter none)))
        (map-set bounties
        bounty-id
...
(begin
  ...
  (ok (var-set commission-rate amount))
  )
)
...
;; Cerulean Bounties for SIP10 tokens by Zero Authority DAO
```
