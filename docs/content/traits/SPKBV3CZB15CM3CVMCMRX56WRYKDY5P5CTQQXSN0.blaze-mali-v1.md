...
;; version: mali-v1
;; summary: Subnet token for Belgian Malinois with fast off-chain transactions

;; Platform constants
(define-constant fee-recipient-address 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS)
(define-constant deployment-fee u50000000)
...
(define-public (deposit (amount uint))
  (let
    ...
    (try! (contract-call? 'SPKBV3CZB15CM3CVMCMRX56WRYKDY5P5CTQQXSN0.belgian-malinois transfer amount sender (as-contract tx-sender) none))
...
(define-public (withdraw (amount uint))
  (let
    ...
    (try! (as-contract (contract-call? 'SPKBV3CZB15CM3CVMCMRX56WRYKDY5P5CTQQXSN0.belgian-malinois transfer amount tx-sender sender none)))
...
(define-read-only (get-name)
  (contract-call? 'SPKBV3CZB15CM3CVMCMRX56WRYKDY5P5CTQQXSN0.belgian-malinois get-name)
)

(define-read-only (get-symbol)
  (contract-call? 'SPKBV3CZB15CM3CVMCMRX56WRYKDY5P5CTQQXSN0.belgian-malinois get-symbol)
)

(define-read-only (get-decimals)
  (contract-call? 'SPKBV3CZB15CM3CVMCMRX56WRYKDY5P5CTQQXSN0.belgian-malinois get-decimals)
)

(define-read-only (get-token-uri)
  (contract-call? 'SPKBV3CZB15CM3CVMCMRX56WRYKDY5P5CTQQXSN0.belgian-malinois get-token-uri)
)

(define-read-only (get-total-supply)
  (contract-call? 'SPKBV3CZB15CM3CVMCMRX56WRYKDY5P5CTQQXSN0.belgian-malinois get-total-supply)
)
