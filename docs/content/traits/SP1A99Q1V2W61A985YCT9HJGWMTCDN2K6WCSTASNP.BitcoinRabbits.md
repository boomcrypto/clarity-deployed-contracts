...
(define-non-fungible-token BitcoinRabbits uint)
...
(define-data-var base-uri (string-ascii 80) "bafybeifihw5h3lziwysyexe5fqmkdgbmtt6tzl7ijxngjmqkiii5cadzr4/")
(define-data-var mint-price uint u10000000)
;; Percent u100 = 1%
(define-data-var royalty-percent uint u250)
(define-data-var mint-limit uint u500)
(define-data-var max-supply uint u1336)
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? BitcoinRabbits next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? BitcoinRabbits next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? BitcoinRabbits id sender recipient)
        success
          (let
            (
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? BitcoinRabbits id))
)
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? BitcoinRabbits id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? BitcoinRabbits id) ERR_NO_OWNER))
