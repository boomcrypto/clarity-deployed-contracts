---
title: "Trait liquid-staked-pepe"
draft: true
---
```
;; Title: Liquid Staked Pepe
;; Author: rozar.btc
;; Synopsis:
;; This contract implements a liquid staking solution for Pepe.
;; It provides users with liquid tokens (sPEPE) that represent staked Pepe. 
...
(define-data-var token-name (string-ascii 32) "Liquid Staked Pepe")
(define-data-var token-symbol (string-ascii 10) "sPEPE")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/liquid-staked-pepe.json"))
(define-data-var token-decimals uint u3)
...
(define-public (unstake (amount uint))
	(begin
		(let
			...
			(try! (as-contract (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.tokensoft-token-v4k68639zxz transfer amount-token contract sender none)))
		)
		(ok true)
	)
)
...
(define-public (deposit (amount uint))
    (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.tokensoft-token-v4k68639zxz transfer amount tx-sender contract none)
)
...
(define-read-only (get-total-in-pool)
	(unwrap-panic (contract-call? 'SP1Z92MPDQEWZXW36VX71Q25HKF5K2EPCJ304F275.tokensoft-token-v4k68639zxz get-balance contract))
)
