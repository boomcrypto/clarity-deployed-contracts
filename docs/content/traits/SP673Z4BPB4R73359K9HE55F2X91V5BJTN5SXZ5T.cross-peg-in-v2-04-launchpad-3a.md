---
title: "Trait cross-peg-in-v2-04-launchpad-3a"
draft: true
---
```
...
(define-read-only (validate-launchpad-order (order { address: (buff 128), chain-id: uint, launch-id: uint, dest: (buff 128), token: principal, amount-in-fixed: uint, salt: (buff 256) } ))
  (contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.alex-launchpad-v2-03a validate-register { address: (get address order), chain-id: (some (get chain-id order)) } (get dest order) (get launch-id order) (get amount-in-fixed order) (get token order)))
    ...
    (let (
      ...
      (match (validate-launchpad-order order)
        ...
        (begin
          ...
          (as-contract (contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.alex-launchpad-v2-03a register { address: (get address order), chain-id: (some (get chain-id order)) } (get dest order) (get launch-id order) (get amount-in-fixed order) token-trait)))
        err-value
        (begin
