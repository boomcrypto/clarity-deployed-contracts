---
title: "Trait memobot-mint-helper"
draft: true
---
```
...
(define-public (mint (land-id uint) (nfts-to-mint uint))
    (let
        (
            (tapped-out (unwrap-panic (contract-call? .edk-v1 tap land-id .land-helper-v3 (some (* (/ STX_PER_NFT ENERGY_DISCOUNT_RATE) nfts-to-mint)))))
