---
title: "Trait memegoat-lottery-rng"
draft: true
---
```
;; MEMEGOAT LOTTERY RANDOM NUMBER GENERATOR

(define-read-only (get-final-number)
  (let
    (
      (vrf-seed (unwrap-panic (get-block-info? vrf-seed (- block-height u1))))
      (final-number (buff-to-uint64 vrf-seed) )
    )
    (ok (+ u1000000 (mod final-number u1000000)))
  )
)
