...
(define-constant ERR-STAKED u601)
...
(define-public (stake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (asserts! (not (is-some (index-of (contract-call? .spaghettipunk-staking get-staked-nfts tx-sender (contract-of collection)) item))) (err ERR-STAKED))
    (if (not custodial) 
...
(define-public (unstake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (asserts! (is-eq (contract-of lookup-table) multiplier) (err ERR-NOT-AUTHORIZED))
    (asserts! (is-some (index-of (contract-call? .spaghettipunk-staking get-staked-nfts tx-sender (contract-of collection)) item)) (err ERR-NOT-STAKED))
    (if (not custodial) 
...
(define-private (stake-non-custodial (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (if (is-eq (contract-of collection) .the-cavalry)
        ...
        (try! (contract-call? .spaghettipunk-staking-counter set-staked (contract-of collection) item true)))
        true     
    )      
    (try! (contract-call? .spaghettipunk-staking-bonuses check-bonuses tx-sender))
    (ok true)))
...
(define-private (unstake-non-custodial (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (if (is-eq (contract-of collection) .the-cavalry)
       (begin 
        ...
        (try! (contract-call? .spaghettipunk-staking-counter set-staked (contract-of collection) item false)))
        true     
    )   
    (try! (contract-call? .spaghettipunk-staking-bonuses check-bonuses tx-sender))
    (ok true)))
...
(define-public (admin-check-bonuses (staker principal))
    (begin 
        (asserts! (is-eq (var-get shutoff-valve) false) (err ERR-NOT-AUTHORIZED))
        (asserts! (is-eq tx-sender (var-get admin)) (err ERR-NOT-AUTHORIZED))
        (try! (contract-call? .spaghettipunk-staking-bonuses check-bonuses staker))
        (ok true)))
...
(try! (contract-call? .spaghettipunk-staking-counter approve-contract (as-contract tx-sender)))
```
