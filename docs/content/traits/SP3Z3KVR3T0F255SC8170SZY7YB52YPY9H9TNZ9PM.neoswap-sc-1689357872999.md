---
title: "Trait neoswap-sc-1689357872999"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/fS0dcKBNjLayteGzpyc8
;; SwapId: HlAH4vEtVRqOMbFg0c5M
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u4824000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP26GZCVY8FYHNZ6C73W68TCFJHS8F8C9E772XX7X.iso-friends transfer u64 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP26GZCVY8FYHNZ6C73W68TCFJHS8F8C9E772XX7X.iso-friends transfer u64 tx-sender TRADER-1)))
	...
	(unwrap-panic (as-contract (stx-transfer? u4824000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u4824000 tx-sender TRADER-1)))
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP26GZCVY8FYHNZ6C73W68TCFJHS8F8C9E772XX7X.iso-friends transfer u64 tx-sender TRADER-2)))
	    )
            true
        )
