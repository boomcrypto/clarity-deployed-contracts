...
;; Define LP token
(define-fungible-token TNA)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.techno-anarchism"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? TNA amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance TNA who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply TNA)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? TNA (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? TNA (get dk delta) sender))
...
(begin
    ...
    (try! (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token transfer u184999000 tx-sender CONTRACT none)))
```
