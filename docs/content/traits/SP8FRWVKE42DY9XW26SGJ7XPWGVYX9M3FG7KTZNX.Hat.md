...
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)
  ...
  (define-fungible-token Hat MAX_SUPPLY)
  ...
  (define-read-only (get-name)                   (ok "Hat"))
  (define-read-only (get-symbol)                 (ok "Hat"))
  (define-read-only (get-decimals)               (ok u6))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance Hat of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply Hat)))
  (define-read-only (get-max-supply)             (ok MAX_SUPPLY))
  (define-read-only (get-token-uri)              (ok (some u"https://www.hatonstacks.com/hat.json")))
