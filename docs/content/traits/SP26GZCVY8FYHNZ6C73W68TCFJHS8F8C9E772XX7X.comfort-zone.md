...
(define-non-fungible-token comfort-zone uint)
...
(define-data-var admin (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF 'SP26GZCVY8FYHNZ6C73W68TCFJHS8F8C9E772XX7X ))
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? comfort-zone item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? comfort-zone item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? comfort-zone item-id)))
      (begin
        (try! (nft-burn? comfort-zone item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? comfort-zone token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? comfort-zone id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? comfort-zone id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? comfort-zone id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP26GZCVY8FYHNZ6C73W68TCFJHS8F8C9E772XX7X u1000 "Comfort Zone" u200000000 u64549 u64771 "ipfs://QmSqTVP9zwgRTkGsYhRrV3CQaJKQgS53v8fxhaAjXT7mbi/comfort-zone/1.json"))
  (ok true)
)
