...
(define-non-fungible-token bull-run uint)
...
(define-data-var artist-address principal 'SP3WGVNQDVHNQRBR6NTX84PEM0467PF7YG7ZFBNS4)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUokprSBRiqC1xwY6yRDLsDvpfZGJ3Pi9aniJVb31Cn7u/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bull-run next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bull-run token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bull-run token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bull-run token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bull-run id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bull-run id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bull-run id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u232890)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u8649000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u49400000000)
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? bull-run (+ last-nft-id u0) 'SP3WGVNQDVHNQRBR6NTX84PEM0467PF7YG7ZFBNS4))
      (map-set token-count 'SP3WGVNQDVHNQRBR6NTX84PEM0467PF7YG7ZFBNS4 (+ (get-balance 'SP3WGVNQDVHNQRBR6NTX84PEM0467PF7YG7ZFBNS4) u1))
