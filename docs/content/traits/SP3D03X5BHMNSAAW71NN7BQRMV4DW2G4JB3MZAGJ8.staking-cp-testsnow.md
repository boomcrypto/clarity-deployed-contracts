---
title: "Trait staking-cp-testsnow"
draft: true
---
```
;; Crash Punk NFT(s) -> $TEST-SNOW Staking Contract
  ...
  (let
    ...
    (if
      (is-some (index-of custodial-list (contract-of collection))) 
        (unwrap! (contract-call? collection transfer id tx-sender .staking-cp-testsnow) (err u401))
    false
    )
...
(define-public (claim-item-stake (collection-collective <nft-trait>) (staked-id uint))
  (let
    ...
    (unwrap! (contract-call? .cp-testsnow mint (* this-collection-multiplier-normalized blocks-staked) tx-sender) ERR-UNWRAP)
...
(define-public (claim-all-stake)
  (let
    ...
    ;; contract call to mint for X amount
    (unwrap! (contract-call? .cp-testsnow mint unclaimed-balance-total tx-sender) ERR-UNWRAP)
...
(define-public (unstake-item (collection <nft-trait>) (staked-id uint))
  (let
    ...
    (if
        ...
        (as-contract (unwrap! (contract-call? collection transfer staked-id .staking-cp-testsnow (var-get function-caller-helper-to-unstake)) (err u401)))
...
(define-public (admin-emergency-unstake (collection <nft-trait>) (staked-id uint) (original-owner principal))
  (let
    ...
    (if (> blocks-staked u0)
      ...
      (unwrap! (contract-call? .cp-testsnow mint (* this-collection-multiplier-normalized blocks-staked) original-owner) ERR-UNWRAP)
    ...
    (if
        ...
        (as-contract (unwrap! (contract-call? collection transfer staked-id .staking-cp-testsnow original-owner) (err u401))) 
...
(define-private (get-token-max-supply)
  (match (var-get token-max-supply)
    ...
    (let
      (
        (new-token-max-supply (unwrap! (contract-call? .cp-testsnow get-max-supply) u0))
      )
