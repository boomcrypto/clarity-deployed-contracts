---
title: "Trait router-xyk-arkadiko-v-1-2"
draft: true
---
```

;; router-xyk-arkadiko-v-1-2
...
(define-public (swap-helper-b
  ...
  (let (
    (aggregator-fee-token (if (is-eq diko-reversed false) token-x-trait token-y-trait))
    (amount-after-aggregator-fees (try! (transfer-aggregator-fees aggregator-fee-token provider amount)))
    (swap-a (if (is-eq diko-reversed false)
                (unwrap! (arkadiko-a token-x-trait token-y-trait amount-after-aggregator-fees) ERR_SWAP_A)
                (unwrap! (arkadiko-b token-x-trait token-y-trait amount-after-aggregator-fees) ERR_SWAP_A)))
