...
(define-non-fungible-token ntest2 uint)
...
(define-data-var base-uri (string-ascii 80) "http://ipfs.io/ipfs/QmaPGJYDUBGZSyM85meh2QikgkzppkGbTpwDaevEUqdZ6G/")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ntest2 token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? ntest2 token-id sender recipient)
  )
)
...
(define-public (mint (recipient principal))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? ntest2 token-id recipient))
