---
title: "Trait many-multiples-v4"
draft: true
---
```
(define-private (collection-bid (collection-id (string-ascii 256)) (amount uint))
    (begin
        (try! (contract-call? .multiples-v3 collection-bid collection-id amount))
        (ok true)
    )
)
...
(define-private (multiple-bid (collection-id (string-ascii 256)) (amount uint) (ids (list 5000 uint)) (trait (string-ascii 256)))
    (begin
        (try! (contract-call? .multiples-v3 multiple-bid collection-id amount ids trait))
        (ok true)
    )
)
