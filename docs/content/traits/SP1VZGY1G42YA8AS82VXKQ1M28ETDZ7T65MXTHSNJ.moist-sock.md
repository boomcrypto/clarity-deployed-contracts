...
(define-non-fungible-token moist-sock uint)
...
(define-data-var artist-address principal 'SP1VZGY1G42YA8AS82VXKQ1M28ETDZ7T65MXTHSNJ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWz7mfYRhEk5RRGrmC3grYamciuT7Z5xXgVBfE175FTAN/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? moist-sock next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? moist-sock token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? moist-sock token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? moist-sock token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? moist-sock id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? moist-sock id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? moist-sock id) (err ERR-NOT-FOUND)))
