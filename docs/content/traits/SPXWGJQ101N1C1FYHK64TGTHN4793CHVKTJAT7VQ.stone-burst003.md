...
(define-public (required-stone (asset-count uint) (asset-name (string-ascii 10)))
  (let ((required-stx
          (if (is-eq asset-name "gems")
            (if (is-eq asset-count u5) u200000
              (if (is-eq asset-count u25) u400000
                (if (is-eq asset-count u50) u700000
                  (if (is-eq asset-count u100) u1000000 u0))))
            (if (is-eq asset-name "hearts")
              (if (is-eq asset-count u3) u1000000
                (if (is-eq asset-count u10) u2000000
                  (if (is-eq asset-count u25) u3000000
                    (if (is-eq asset-count u50) u4000000 u0))))
              u0))))
    (asserts! (>= (stx-get-balance tx-sender) required-stx) INVALID-STX-BALANCE)
    (let (
        (share-to-progressive (/ (* required-stx (var-get percentage-progressive)) u100))
        (remaining-stx (- required-stx share-to-progressive))
        (share-to-burn (/ (* remaining-stx (var-get percentage-burn)) total-percentage))
        (swapped-stone (try! (swap share-to-burn)))
    )
    (ok swapped-stone))
