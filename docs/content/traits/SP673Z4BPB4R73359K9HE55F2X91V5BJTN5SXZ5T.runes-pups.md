...
(define-fungible-token runes-pups)

(define-data-var token-name (string-ascii 32) "PUPS WORLD PEACE")
(define-data-var token-symbol (string-ascii 10) "PUPS")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/runes-pups.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(try! (ft-transfer? runes-pups amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance runes-pups who)))

(define-read-only (get-total-supply)
	(ok (ft-get-supply runes-pups)))
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? runes-pups amount recipient)))
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? runes-pups amount sender)))
