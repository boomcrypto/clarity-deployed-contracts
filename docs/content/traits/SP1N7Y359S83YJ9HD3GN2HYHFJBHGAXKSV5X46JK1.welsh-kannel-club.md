...
(define-non-fungible-token welsh-kannel-club uint)
...
(define-data-var artist-address principal 'SP1N7Y359S83YJ9HD3GN2HYHFJBHGAXKSV5X46JK1)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPpAv88988sfqvhmkFRA2sMNHpfRFeuRFqHoXgSnUhz4M/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? welsh-kannel-club next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? welsh-kannel-club token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? welsh-kannel-club token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? welsh-kannel-club token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? welsh-kannel-club id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? welsh-kannel-club id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? welsh-kannel-club id) (err ERR-NOT-FOUND)))
