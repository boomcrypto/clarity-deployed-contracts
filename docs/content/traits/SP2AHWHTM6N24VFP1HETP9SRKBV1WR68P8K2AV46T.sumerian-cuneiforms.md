---
title: "Trait sumerian-cuneiforms"
draft: true
---
```
;; sumerian-cuneiforms
...
(define-non-fungible-token sumerian-cuneiforms uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmd8YvdDf76gyB8GzU4ve2K27mCMUf4TqUgMqH8DMFnHFr/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? sumerian-cuneiforms next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? sumerian-cuneiforms token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? sumerian-cuneiforms token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sumerian-cuneiforms token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sumerian-cuneiforms id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sumerian-cuneiforms id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sumerian-cuneiforms id) (err ERR-NOT-FOUND)))
