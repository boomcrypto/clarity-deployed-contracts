...
(define-read-only (hash-tx (tx { bitcoin-tx: (buff 8192), output: uint, offset: uint, tick: (string-utf8 4), amt: uint, from: (buff 128), to: (buff 128), from-bal: uint, to-bal: uint, decimals: uint } ))
...
(define-read-only (get-user-balance-or-default (user (buff 128)) (tick (string-utf8 4)))
	(contract-call? .oracle-registry-v1-02 get-user-balance-or-default user tick))
(define-read-only (get-tick-decimals-or-default (tick (string-utf8 4)))
	(contract-call? .oracle-registry-v1-02 get-tick-decimals-or-default tick))
		...
		(let (
			...
			(and (is-err (get-bitcoin-tx-mined-or-fail (get bitcoin-tx tx))) 
				(begin 
					...
					(as-contract (try! (contract-call? .oracle-registry-v1-02 set-tx-mined (get bitcoin-tx tx) (get height (get block signed-tx)))))
				)
			)
			(var-set tx-hash-to-iter tx-hash)
			(try! (fold validate-signature-iter signature-packs (ok true)))
			(as-contract (try! (contract-call? .oracle-registry-v1-02 set-tx-indexed { tx-hash: (get bitcoin-tx tx), output: (get output tx), offset: (get offset tx) } { tick: (get tick tx), amt: (get amt tx), from: (get from tx), to: (get to tx) })))
			(and (> height (get up-to-block from-bal)) (as-contract (try! (contract-call? .oracle-registry-v1-02 set-user-balance { user: (get from tx), tick: (get tick tx) } { balance: (get from-bal tx), up-to-block: height }))))
			(and (> height (get up-to-block to-bal)) (as-contract (try! (contract-call? .oracle-registry-v1-02 set-user-balance { user: (get to tx), tick: (get tick tx) } { balance: (get to-bal tx), up-to-block: height }))))
			(as-contract (try! (contract-call? .oracle-registry-v1-02 set-tick-decimals (get tick tx) (get decimals tx))))
			(ok true))
		prev-err
		previous-response))
