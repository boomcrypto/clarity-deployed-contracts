---
title: "Trait farming-abtc-aeusdc2-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .abtc-aeusdc2)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .abtc-aeusdc2 transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .abtc-aeusdc2 get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .abtc-aeusdc2 transfer amt protocol user none)))
