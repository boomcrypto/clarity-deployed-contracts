---
title: "Trait memegoat-launchpad-v1-4-ext"
draft: true
---
```
...
(define-data-var launchpad-fee uint u4)
...
(define-public (transfer-campaign-info (token-launch-id uint))
    (begin
    (try! (check-is-owner))
    (let
      (
        (token-launch (try! (contract-call? .memegoat-launchpad-v1-4 get-token-launch-by-id token-launch-id)))
      ...
      (map-set launchpad-map {token-launch-id: next-launchpad-id} {
        ...
        pool-amount: (get pool-amount token-launch),
        hardcap: (get hardcap token-launch),
        softcap: (get softcap token-launch),
        total-stx-deposited: (get total-stx-deposited token-launch),
        no-of-participants: (get no-of-participants token-launch),
        min-stx-deposit: (get min-stx-deposit token-launch),
        max-stx-deposit: (get max-stx-deposit token-launch),
        duration: (+ (get duration token-launch) u144),
        start-block: (get start-block token-launch),
        end-block: (+ (get end-block token-launch) u144),
        owner: (get owner token-launch),
        is-vested: (get is-vested token-launch),
        is-listed: (get is-listed token-launch),
        listing-allocation: (get listing-allocation token-launch),
        campaign-allocation: (get campaign-allocation token-launch),
        campaign-rewards-set: (get campaign-rewards-set token-launch),
      })
      (map-set launchpad-map-by-token-addr {token-addr: (get token token-launch)} next-launchpad-id)
...
(define-public (transfer-user-records (token-launch-id uint) (addrs (list 200 principal)))
  (begin
    (try! (check-is-owner))
    (fold transfer-record-iter addrs token-launch-id)
    (ok true)
  )
)
  ...
  (begin
    ...
    (let
      (
        ...
        (min-campaign-allocation (/ (* total-supply u2) u100))
...
(define-private (transfer-record-iter (addr principal) (token-launch-id uint))
	(let
    (
      (user-deposits (contract-call? .memegoat-launchpad-v1-4 get-user-deposits addr token-launch-id))
    )
    (map-set users-deposits {user-addr: addr, token-launch-id: token-launch-id} user-deposits)
    token-launch-id
  )
)
```
