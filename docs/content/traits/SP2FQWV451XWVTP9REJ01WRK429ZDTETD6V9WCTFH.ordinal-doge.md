...
(define-non-fungible-token ordinal-doge uint)
...
;; Internal variables
(define-data-var mint-limit uint u9972)
(define-data-var last-id uint u1)
(define-data-var total-price uint u2000000)
(define-data-var artist-address principal 'SP2FQWV451XWVTP9REJ01WRK429ZDTETD6V9WCTFH)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qme2pVRHAMtZmtL9PBai1L7MpRiM7PRW6uYY4hBmww5HKC/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinal-doge next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinal-doge token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinal-doge token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinal-doge token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinal-doge id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinal-doge id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinal-doge id) (err ERR-NOT-FOUND)))
