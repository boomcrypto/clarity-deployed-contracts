...
(define-data-var admin (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF 'SP3YSG653BZZNTJVFHFMBSQCTP3GK6NAQEHC82TNK ))
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? be-not-afraid item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? be-not-afraid item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? be-not-afraid item-id)))
      (begin
        (try! (nft-burn? be-not-afraid item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? be-not-afraid token-id)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? be-not-afraid id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP3YSG653BZZNTJVFHFMBSQCTP3GK6NAQEHC82TNK u500 "Be Not Afraid" u170000000 u64395 u64539 "ipfs://QmQrV77wh2FvJZrknmJvCDpYmEfDqwJoSZStyy8RnbVfaG/be-not-afraid/1.json"))
  (ok true)
)
