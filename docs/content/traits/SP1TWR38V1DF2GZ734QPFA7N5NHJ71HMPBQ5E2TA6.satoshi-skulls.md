...
(define-non-fungible-token satoshi-skulls uint)
...
(define-data-var artist-address principal 'SP1TWR38V1DF2GZ734QPFA7N5NHJ71HMPBQ5E2TA6)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbUeA1YZyStd3ttAVhtoB8AEfo8wY9RriWNUnv7MYcShJ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? satoshi-skulls next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? satoshi-skulls token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? satoshi-skulls token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? satoshi-skulls token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? satoshi-skulls id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? satoshi-skulls id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? satoshi-skulls id) (err ERR-NOT-FOUND)))
