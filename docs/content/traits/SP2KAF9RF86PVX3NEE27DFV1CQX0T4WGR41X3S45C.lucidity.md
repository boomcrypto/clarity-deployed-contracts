...
(define-non-fungible-token lucidity uint)
...
(define-constant MINT-LIMIT u750)
...
(define-data-var ipfs-full-metadata (string-ascii 106) "ipfs://QmWEUMNbi5SPs3hEMHTTG5Z86huUr9TSTynrCXwTfuWpdp/lucidity.json")
(define-data-var ipfs-root (string-ascii 93) "ipfs://QmWEUMNbi5SPs3hEMHTTG5Z86huUr9TSTynrCXwTfuWpdp/")
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? lucidity next-id new-owner))
...
(define-public (activate-contract)
  (if (is-eq tx-sender (var-get artist-address))
    (begin
        (try! (stx-transfer? u150000000 (var-get artist-address) CONTRACT-OWNER))
        (ok (var-set minting-enabled true))
    )
    (err ERR-NOT-AUTHORIZED)
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? lucidity token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? lucidity token-id)))
