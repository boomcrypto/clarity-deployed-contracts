---
title: "Trait stx-prime-apes-society"
draft: true
---
```
;; stx-prime-apes-society
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token stx-prime-apes-society uint)
...
;; Internal variables
(define-data-var mint-limit uint u9784)
(define-data-var last-id uint u1)
(define-data-var total-price uint u850000)
(define-data-var artist-address principal 'SP1Z91NHWMWHA6GHRAVRNEJFA3SE9SJ6X0WNPEYCR)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPGgD4My1Han39ikQ8iM7uEXaG5aDo7CjiWwRX32z9TgM/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stx-prime-apes-society next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-prime-apes-society token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-prime-apes-society token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-prime-apes-society token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-prime-apes-society id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-prime-apes-society id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-prime-apes-society id) (err ERR-NOT-FOUND)))
