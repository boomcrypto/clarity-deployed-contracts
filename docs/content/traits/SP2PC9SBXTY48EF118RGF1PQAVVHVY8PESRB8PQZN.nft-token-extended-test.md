---
title: "Trait nft-token-extended-test"
draft: true
---
```
...
(define-non-fungible-token NFTTOKENNAME uint)
...
(define-private (mint_punk (punk-id uint) (new-owner principal))
  (begin
    ...
    (nft-mint? NFTTOKENNAME punk-id new-owner)
  )
)
...
(define-public 
  ...
  (begin     
    ...
    (nft-transfer? NFTTOKENNAME token-id sender recipient))
)
;; BURN A TOKEN
(define-public 
  (burn-token 
  ...
  (begin     
    (asserts! (is-eq (some tx-sender) (nft-get-owner? NFTTOKENNAME token-id) ) (err u403)) 
    (asserts! (is-none (map-get? delegated-nft token-id)) (err u401))    
    (nft-burn? NFTTOKENNAME token-id tx-sender))
)

(
  define-read-only (get-owner (punk-id uint))  
  (ok (nft-get-owner? NFTTOKENNAME punk-id))
)
...
(define-public (delegate-nft (token-id uint) (sender principal) (recipient principal) )
  (begin
    (asserts! (is-eq tx-sender sender) (err u403))  
    (asserts! (is-eq tx-sender (unwrap-panic (nft-get-owner? NFTTOKENNAME token-id)) ) (err u401))  
