...
(define-non-fungible-token blue-hobbit-cloud-3 uint)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? blue-hobbit-cloud-3 id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace blue-hobbit-cloud-3
  (ok (nft-get-owner? blue-hobbit-cloud-3 id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? blue-hobbit-cloud-3 next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? blue-hobbit-cloud-3 id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? blue-hobbit-cloud-3 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? blue-hobbit-cloud-3 id) ERR-NOT-FOUND))
...
(define-private (private-mintpass-mint (new-owner principal))
  (let ((mint-count (get-private-presale-balance new-owner)))
    (asserts! (var-get private-sale-active) ERR-SALE-NOT-ACTIVE)
    (asserts! (>= (var-get private-target-block-height) block-height) ERR-BEFORE-MINT-TIME)
...
(define-private (public-mint (new-owner principal))
  (let ((mint-count (get-public-balance new-owner)))
    (asserts! (var-get sale-active) ERR-SALE-NOT-ACTIVE)
    (asserts! (>= (var-get public-target-block-height) block-height) ERR-BEFORE-MINT-TIME)
    (mint new-owner)
  )
)
