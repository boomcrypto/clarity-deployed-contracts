...
(define-data-var payout-addresses (list 1000 principal) (list 'SP2H3TTG3MQK9CEF59S7VQ86H4FX9CH596ZXSE2EK 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C))
...
(define-public (collect (fungible <token-trait>) (addresses (list 1000 principal)))
    ...
    (begin 
        ...
        (if (bonus-check-all addresses)
          (map-set pool { staker: tx-sender } { stake-time: block, lifetime-points: lifetime, points-balance: u0, total-multiplier: total-multiplier })
          (map-set pool { staker: tx-sender } { stake-time: block, lifetime-points: lifetime, points-balance: u0, total-multiplier: total-multiplier })
        )
        (print (map-get? pool { staker: tx-sender }))
        (ok true)
...
(define-read-only (check-collect (staker principal) (addresses (list 1000 principal)))
    (let (
        (block block-height)
        (owner staker)
        (balance (if (is-some (map-get? pool { staker: owner })) (unwrap-panic (get points-balance (map-get? pool { staker: owner }))) u0))
