---
title: "Trait fake-token-2"
draft: true
---
```
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)

  (define-constant MAX_SUPPLY (* u10000000 (pow u10 u8)))

  (define-fungible-token fake-token-2 MAX_SUPPLY)
  ...
  (define-public
    ...
    (begin
      ...
      (ft-transfer? fake-token-2 amt from to)))


  (define-public (mint (amt uint) (to principal))
    (begin
      (try! (check-owner))
      (ft-mint? fake-token-2 amt to) ))

  (define-read-only (get-name)                   (ok "fake-token-2"))
  (define-read-only (get-symbol)                 (ok "FTK2"))
  (define-read-only (get-decimals)               (ok u8))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance fake-token-2 of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply fake-token-2)))
