...
(define-non-fungible-token trashies uint)
...
(define-data-var artist-address principal 'SP36BVQV8BKJ06DB7Y3X39K4YKY97KV4G5EAZR787)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmcv123Y44JxNsLXQNDc24D7g8MvKzT1Ls8K2F3JDWu1Mu/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? trashies next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? trashies token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? trashies token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? trashies token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? trashies id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? trashies id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? trashies id) (err ERR-NOT-FOUND)))
