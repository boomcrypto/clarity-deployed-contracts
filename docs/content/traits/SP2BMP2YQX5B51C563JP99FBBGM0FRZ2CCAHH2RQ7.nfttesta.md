...
(define-non-fungible-token nfttesta uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPyxdEhSzjSjTTi7sNiErdKHj5qHWA1NoU6MxrAfjigHL/")
...
(define-read-only (get-owner (token-id uint)) (ok (nft-get-owner? nfttesta token-id)))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
    (begin
        ...
        (nft-transfer? nfttesta token-id sender recipient)))
...
(define-public (mint (recipient principal))
    (let
        ...
        (try! (nft-mint? nfttesta token-id recipient))
