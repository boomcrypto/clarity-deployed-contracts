...
(define-non-fungible-token rat-rebells uint)
...
;; Internal variables
(define-data-var mint-limit uint u220)
(define-data-var last-id uint u1)
(define-data-var total-price uint u5000000)
(define-data-var artist-address principal 'SPED63AHT9050SAFQEK4YQRJVDW8Y1HDPBN3D910)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbLppUKJRXiLbsdGzzU8tKGX36FsSnQtrKpb4VS4kVTTV/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? rat-rebells next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? rat-rebells token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? rat-rebells token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? rat-rebells token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rat-rebells id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rat-rebells id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rat-rebells id) (err ERR-NOT-FOUND)))
