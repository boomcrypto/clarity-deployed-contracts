...
(define-data-var last-discord (string-ascii 30) "none")
(define-data-var bet-active bool false)

;; Storage
(define-map bets { bet-id: uint } { wallet: principal, price: (string-ascii 20), discord: (string-ascii 30) })
...
;; Make bet in STX
(define-public (bet-stx (price (string-ascii 20)) (discord (string-ascii 30)))
  (begin
    ...
    (var-set last-discord discord)
    (map-set bets { bet-id: (var-get last-bet-id) } { wallet: tx-sender, price: (var-get last-price), discord: (var-get last-discord) })
...
(define-public (pay-prize (winner principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (var-set prize-pool (/ (* (var-get last-bet-id) (var-get bet-price) u95) u100))
...
(define-read-only (get-prize-pool-stx)
  (ok (* (var-get last-bet-id) (var-get bet-price))))
```
