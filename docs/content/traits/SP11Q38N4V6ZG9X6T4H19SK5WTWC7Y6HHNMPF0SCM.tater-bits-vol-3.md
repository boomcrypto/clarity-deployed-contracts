...
(define-non-fungible-token tater-bits-vol-3 uint)
...
(define-data-var artist-address principal 'SP11Q38N4V6ZG9X6T4H19SK5WTWC7Y6HHNMPF0SCM)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZuXiP9Ca2TPv1ZNxUtWmU5T5s6y44gQQ2WXpZFmMbc38/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? tater-bits-vol-3 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? tater-bits-vol-3 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? tater-bits-vol-3 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tater-bits-vol-3 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? tater-bits-vol-3 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? tater-bits-vol-3 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? tater-bits-vol-3 id) (err ERR-NOT-FOUND)))
