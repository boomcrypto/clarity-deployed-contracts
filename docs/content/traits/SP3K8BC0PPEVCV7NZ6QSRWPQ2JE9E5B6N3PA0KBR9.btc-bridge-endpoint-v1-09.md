---
title: "Trait btc-bridge-endpoint-v1-09"
draft: true
---
```
...
(define-read-only (get-peg-in-sent-or-default (tx (buff 8192)) (output uint))
	(contract-call? .btc-bridge-registry-v1-02 get-peg-in-sent-or-default tx output))
...
(define-read-only (get-txid (tx (buff 8192)))
	(if (try! (contract-call? .clarity-bitcoin-v1-05 is-segwit-tx tx))
		(ok (contract-call? .clarity-bitcoin-v1-05 get-segwit-txid tx))
		(ok (contract-call? .clarity-bitcoin-v1-05 get-txid tx))
	))
...
(define-read-only (validate-tx-0 (tx (buff 8192)) (output-idx uint) (order-idx uint))
	(let (
...
(define-read-only (validate-tx-1 (tx (buff 8192)) (output-idx uint) (order-idx uint) (token principal))
...
(define-read-only (validate-tx-2 (tx (buff 8192)) (output-idx uint) (order-idx uint) (token1 principal) (token2 principal))
...
(define-read-only (validate-tx-3 (tx (buff 8192)) (output-idx uint) (order-idx uint) (token1 principal) (token2 principal) (token3 principal))
...
(define-read-only (validate-tx-launchpad (tx (buff 8192)) (output-idx uint) (order-idx uint) (owner-idx uint))
...
(define-public (finalize-peg-in-0
	...
	(let (
		...
		(as-contract (try! (contract-call? .btc-bridge-registry-v1-02 set-peg-in-sent tx output-idx true)))
...
(define-public (request-peg-out-0 (peg-out-address (buff 128)) (amount uint))
	(let (
			...
			(request-id (as-contract (try! (contract-call? .btc-bridge-registry-v1-02 set-request u0 request-details)))))
...
(define-public (claim-peg-out (request-id uint) (fulfilled-by (buff 128)))
	(let (
		...
		(as-contract (try! (contract-call? .btc-bridge-registry-v1-02 set-request request-id (merge request-details { claimed: (+ block-height (get-request-claim-grace-period)), claimed-by: claimer, fulfilled-by: fulfilled-by }))))
...
(define-public (finalize-peg-out
	...
	(let (
		...
		(asserts! (not (get finalized request-details)) err-request-already-finalized)
		(as-contract (try! (contract-call? .btc-bridge-registry-v1-02 set-peg-in-sent tx output-idx true)))
		(as-contract (try! (contract-call? .btc-bridge-registry-v1-02 set-request request-id (merge request-details { finalized: true }))))
...
(define-public (revoke-peg-out (request-id uint))
	(let (
		...
		(asserts! (not (get revoked request-details)) err-request-already-revoked)
		(asserts! (not (get finalized request-details)) err-request-already-finalized)
		(as-contract (try! (contract-call? .btc-bridge-registry-v1-02 set-request request-id (merge request-details { revoked: true }))))
		...
		(ok true)))
(define-private (validate-tx-1-base (tx (buff 8192)) (output-idx uint) (order-idx uint))
	(let (
			(validation-data (try! (validate-tx-common tx output-idx order-idx))))
...
(define-private (validate-tx-2-base (tx (buff 8192)) (output-idx uint) (order-idx uint))
	(let (
			(validation-data (try! (validate-tx-common tx output-idx order-idx))))
...
(define-private (validate-tx-3-base (tx (buff 8192)) (output-idx uint) (order-idx uint))
	(let (
			(validation-data (try! (validate-tx-common tx output-idx order-idx))))
...
(define-private (validate-tx-launchpad-base (tx (buff 8192)) (output-idx uint) (order-idx uint) (owner-idx uint))
	(let (
			(validation-data (try! (validate-tx-common tx output-idx order-idx)))
...
(define-private (validate-tx-launchpad-extra (validation-data { owner-script: (buff 128), order-details: { user: principal, launch-id: uint }, fee: uint, amount-net: uint }))
	(let (
		...
		(ok validation-data)))
(define-private (validate-tx-common (tx (buff 8192)) (output-idx uint) (order-idx uint))
	(let (
			(parsed-tx (try! (extract-tx-ins-outs tx)))
			(output (unwrap! (element-at (get outs parsed-tx) output-idx) err-invalid-tx))
			(amount (get value output))
