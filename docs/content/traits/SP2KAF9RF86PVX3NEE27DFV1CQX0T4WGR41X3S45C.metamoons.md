...
(define-non-fungible-token metamoons uint)
...
(define-constant MINT-LIMIT u151)

;; Internal variables
(define-data-var last-id uint u0)
(define-data-var uri-prefix (string-ascii 256) "")
(define-data-var cost-per-mint uint u29500000)
(define-data-var payout uint u0)
(define-data-var target-block uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var ipfs-full-metadata (string-ascii 106) "ipfs://QmTCgRupB1f5ezf5qUL937aYVtd4TDbb5QBWCN9zWeWkDH/meta_moons_metadata.json")
(define-data-var ipfs-root (string-ascii 93) "ipfs://QmTCgRupB1f5ezf5qUL937aYVtd4TDbb5QBWCN9zWeWkDH/")
(define-data-var artist-address principal 'SP1E645A6361NNMSXCGKCFASB936CD2ZWM2J4D0T6)
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? metamoons next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? metamoons token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? metamoons token-id)))
