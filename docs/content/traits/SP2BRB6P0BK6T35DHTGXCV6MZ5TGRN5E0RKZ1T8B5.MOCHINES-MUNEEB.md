...
(define-non-fungible-token Muneeb uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafkreia52kowctfop6w4z3bzhmggabppfp37lwcjv4k5kvwdjexa6amtn4")
...
(define-public (mint (recipient principal))
    (begin 
        ...
        (asserts! (< (var-get last-id) SUPPLY) (err ERR-NO-MORE-NFTS))
            ...
            (try! (nft-mint? Muneeb u1 tx-sender))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Muneeb token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Muneeb token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Muneeb token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Muneeb id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Muneeb id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Muneeb id) (err ERR-NOT-FOUND)))
