...
(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)
  ...
  (define-fungible-token SAD MAX_SUPPLY)
  ...
  (define-read-only (get-name)                   (ok "SAD"))
  (define-read-only (get-symbol)                 (ok "SAD"))
  (define-read-only (get-decimals)               (ok u5))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance SAD of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply SAD)))
