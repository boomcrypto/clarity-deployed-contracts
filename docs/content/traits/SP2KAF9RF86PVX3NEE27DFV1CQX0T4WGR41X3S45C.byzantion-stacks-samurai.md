---
title: "Trait byzantion-stacks-samurai"
draft: true
---
```
...
(define-non-fungible-token stacks-samurai uint)
...
(define-constant MINT-LIMIT u75)
...
(define-data-var ipfs-full-metadata (string-ascii 115) "ipfs://ipfs/bafybeih5y6m3chr2qmixtej6uotqb6wjpaf6cxbdz2itzb2s46wtckctum/stacks_samurai/stacks_samurai_metadata.json")
(define-data-var ipfs-root (string-ascii 102) "ipfs://ipfs/bafybeih5y6m3chr2qmixtej6uotqb6wjpaf6cxbdz2itzb2s46wtckctum/stacks_samurai/stacks_samurai_")
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? stacks-samurai next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? stacks-samurai token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-samurai token-id)))
