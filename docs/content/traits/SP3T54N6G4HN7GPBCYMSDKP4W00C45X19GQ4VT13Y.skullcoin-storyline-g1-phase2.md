---
title: "Trait skullcoin-storyline-g1-phase2"
draft: true
---
```
;; Traits
...
;; Define NFT token
(define-non-fungible-token skullcoin_storyline_g1_p2 uint)
...
(define-data-var base-uri (string-ascii 80) "ipfs://bafybeih6hqw5xcivxp3p3n6l3pvizwktqqnn2a57vhlkrloaa2eftng5ji/")
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? skullcoin_storyline_g1_p2 id)))
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (match (nft-mint? skullcoin_storyline_g1_p2 next-id new-owner)
        success
        (let
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? skullcoin_storyline_g1_p2 id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? skullcoin_storyline_g1_p2 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? skullcoin_storyline_g1_p2 id) ERR-NOT-FOUND))
