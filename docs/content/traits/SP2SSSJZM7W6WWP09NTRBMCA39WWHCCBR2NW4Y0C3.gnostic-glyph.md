...
(define-non-fungible-token gnostic-glyph uint)
...
(define-data-var artist-address principal 'SP2SSSJZM7W6WWP09NTRBMCA39WWHCCBR2NW4Y0C3)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmbctpu6NZC7n76Jpyk9LDgZMkd6MnAjqV9EeHCbW1CBad/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? gnostic-glyph next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gnostic-glyph token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gnostic-glyph token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gnostic-glyph token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gnostic-glyph id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gnostic-glyph id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gnostic-glyph id) (err ERR-NOT-FOUND)))
