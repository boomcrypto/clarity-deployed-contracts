...
(define-non-fungible-token early-crows uint)
...
(define-data-var artist-address principal 'SP2J6Y09JMFWWZCT4VJX0BA5W7A9HZP5EX96Y6VZY)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWtvb6MrbE4PwNQHdaqsepk7bpiMPnNP9mZzqj7PbzSmV/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? early-crows next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? early-crows token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? early-crows token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? early-crows token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? early-crows id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? early-crows id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? early-crows id) (err ERR-NOT-FOUND)))
