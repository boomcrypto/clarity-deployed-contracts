...
(define-non-fungible-token aurora-space uint)
...
(define-data-var artist-address principal 'SP92GXFN6X3722GDW740BGR9NFZ0KAA37MW75P68)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcDNJxZfFfBEyShZ4CmpntpUn8RwKBUJEP6aqQ4GRvLMB/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? aurora-space next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? aurora-space token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? aurora-space token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? aurora-space token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? aurora-space id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? aurora-space id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? aurora-space id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mega uint u10000)
