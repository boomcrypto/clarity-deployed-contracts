...
(define-data-var artist-address principal 'SP19H1D5DV317DR2196ADZNN4P83H9KP5GA1PPEW5)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbgCioWjvyJZSAPowDapoPF2t3SKh4Mxa671ihD2MjVdP/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? spacewhales next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? spacewhales token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? spacewhales token-id)))
