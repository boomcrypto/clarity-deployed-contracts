---
title: "Trait farming-wstx-nakamoto-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-nakamoto)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-nakamoto transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-nakamoto get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-nakamoto transfer amt protocol user none)))
