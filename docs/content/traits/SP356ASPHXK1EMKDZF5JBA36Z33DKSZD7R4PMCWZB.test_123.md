...
```
    ...
    (define-non-fungible-token test_123 uint)
    ...
    (define-read-only (get-owner (token-id uint))
        (ok (nft-get-owner? test_123 token-id)))
    ...
    (define-public (burn (token-id uint))
       (begin 
        ...
        (nft-burn? test_123 token-id tx-sender)))
    
    (define-private (is-owner (token-id uint) (user principal))
        (is-eq user (unwrap! (nft-get-owner? test_123 token-id) false)))
    ...
    (define-private (iterate-minting (not-to-be-used bool) (next-id uint))
        ...
        (begin
            (unwrap! (nft-mint? test_123 next-id tx-sender) next-id)
            (+ next-id u1)    
        )
        next-id))
    ...
    (define-private (trnsfr (id uint) (sender principal) (recipient principal))
        (match (nft-transfer? test_123 id sender recipient)
        success
            (let
    ...
    (define-private (is-sender-owner (id uint))
        (let ((owner (unwrap! (nft-get-owner? test_123 id) false)))
    ...
    (define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
        (let ((owner (unwrap! (nft-get-owner? test_123 id) (err err-not-found)))
