...
(define-non-fungible-token stxstone-ordinal uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZEuUMe8B7DQyeAZs9nym4yXR8mEU3DVNJSbcpuURU5iP/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stxstone-ordinal next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stxstone-ordinal token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stxstone-ordinal token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stxstone-ordinal token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stxstone-ordinal id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stxstone-ordinal id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stxstone-ordinal id) (err ERR-NOT-FOUND)))
