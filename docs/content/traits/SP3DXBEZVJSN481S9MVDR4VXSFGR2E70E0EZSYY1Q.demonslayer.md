...
(define-non-fungible-token demonslayer uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUWpY9ZS8Hw4kmdgAy563KywU977AGJU9PBohE6e9MKhe/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? demonslayer next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? demonslayer token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? demonslayer token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? demonslayer token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? demonslayer id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? demonslayer id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? demonslayer id) (err ERR-NOT-FOUND)))
