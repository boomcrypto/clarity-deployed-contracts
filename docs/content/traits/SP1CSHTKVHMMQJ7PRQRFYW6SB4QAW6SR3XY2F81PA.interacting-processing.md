---
title: "Trait interacting-processing"
draft: true
---
```
...
(define-non-fungible-token interacting-processing uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTNBqDFLkVdSArxW5kJUmrB9aWyK1NvEQR4rx45TLz7dg/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? interacting-processing next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? interacting-processing token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? interacting-processing token-id)))
