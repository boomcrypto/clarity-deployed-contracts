...
(define-fungible-token runes-mineticket)

(define-data-var token-name (string-ascii 32) "RUNEMINE ON ALEX")
(define-data-var token-symbol (string-ascii 10) "mineticket")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/runes-mineticket.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(try! (ft-transfer? runes-mineticket amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance runes-mineticket who)))

(define-read-only (get-total-supply)
	(ok (ft-get-supply runes-mineticket)))
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? runes-mineticket amount recipient)))
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? runes-mineticket amount sender)))
