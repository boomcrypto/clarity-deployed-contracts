...
(define-non-fungible-token indigo-act1-music uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWD6XoPZ68f7nkpTcGeMoar8pezFnHE6GT9qCAGwmZLuu/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? indigo-act1-music next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? indigo-act1-music token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? indigo-act1-music token-id)))
