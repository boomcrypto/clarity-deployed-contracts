---
title: "Trait tomatoshi-and-cucumoto"
draft: true
---
```
;; tomatoshi-and-cucumoto
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token tomatoshi-and-cucumoto uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNReQQFXa1RewQjpfmj3fruC14w438NnR3DqzRQn5p1cS/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? tomatoshi-and-cucumoto next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? tomatoshi-and-cucumoto token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? tomatoshi-and-cucumoto token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tomatoshi-and-cucumoto token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? tomatoshi-and-cucumoto id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? tomatoshi-and-cucumoto id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? tomatoshi-and-cucumoto id) (err ERR-NOT-FOUND)))
