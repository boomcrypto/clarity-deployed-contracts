---
title: "Trait stacks-itoshimototsu"
draft: true
---
```
;; stacks-itoshimototsu
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token stacks-itoshimototsu uint)
...
(define-data-var artist-address principal 'SP8FFZ0X5Q4QR377F6RWTNAYJSCPFDRM0C2F1KBW)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmajdnzLrS9BpbvApfZuSUbjidA4N9V4uh9AiwGEjSApFo/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-itoshimototsu next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-itoshimototsu token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-itoshimototsu token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-itoshimototsu token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-itoshimototsu id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-itoshimototsu id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-itoshimototsu id) (err ERR-NOT-FOUND)))
