...
(define-fungible-token TAX)
;; Initialize function to mint max supply to owner


;; Define the owner address
(define-data-var contract-owner principal tx-sender) 
(define-data-var treasury-address principal 'SP2470N2A31DGDHX541MK2FKJSRHSCW907S5KKYTR)
(define-data-var dev-wallet-address principal 'SP2470N2A31DGDHX541MK2FKJSRHSCW907S5KKYTR)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  ...
  ;; If sender is whitelisted or tax is off, perform transfer without tax
    (if (or is-whitelisted (var-get tax_off)) 
      (match (ft-transfer? TAX amount sender recipient)
        success (ok success)
  ...
  ;; If sender is not whitelisted and tax is on, apply tax
      (let ((tax-amount (/ (* amount (var-get total-tax)) u100))
        ...
        (match (ft-transfer? TAX transfer-amount sender recipient)
          success
...
(define-private (distribute-taxes (tax-amount uint) (sender principal))
  (let (
    ...
    (match (ft-transfer? TAX treasury-tax sender (var-get treasury-address))
      treasury-success
        (match (ft-burn? TAX burn-tax sender ) 
          burn-success
            (match (ft-transfer? TAX dev-tax sender (var-get dev-wallet-address))
              dev-success (ok true)
...
(define-read-only (get-balance (user principal))
    (ok (ft-get-balance TAX user)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply TAX)))
...
;; DEFINE METADATA
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://gaia.hiro.so/hub/13iqwgRT8yh3catMgzhHLXLFmx1ZUCgTGg/tax.json"))
(define-data-var token-name (string-ascii 32) "Taxation as Theft")
(define-data-var token-symbol (string-ascii 32) "TAX")
...
(begin
    ...
    (try! (ft-mint? TAX u9990000000 (var-get contract-owner)))
  (try! (ft-mint? TAX u10000000 'SP1FQ3DQDR5N9HJX3XC5DNKFCG4DHH48EFJQV6QH0))
