---
title: "Trait ordinary-senior-citizen"
draft: true
---
```
...
(define-non-fungible-token ordinary-senior-citizen uint)
...
(define-data-var artist-address principal 'SP2FY2X9M1Q223MCKNPESHRQFJ9FK52F7AD7WZVE7)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYgC3rkmfTKkUpVdFandXnkNH7T9965Ajz1VLzLsALsKY/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinary-senior-citizen next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinary-senior-citizen token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinary-senior-citizen token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinary-senior-citizen token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinary-senior-citizen id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinary-senior-citizen id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinary-senior-citizen id) (err ERR-NOT-FOUND)))
