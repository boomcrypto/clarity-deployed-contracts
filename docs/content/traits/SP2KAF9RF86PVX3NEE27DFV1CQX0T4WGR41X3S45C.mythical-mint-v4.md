...
;; Define Variables
(define-data-var stx-private-cost-per-mint uint u10000)
(define-data-var stx-public-cost-per-mint uint u20000)
...
(define-private (private-mintpass-mint (new-owner principal))
  (let ((mint-count (get-private-presale-balance new-owner)))
    ...
    (map-set private-presale-count
              new-owner
              (- mint-count u1))
    (contract-call? .mythical-nft-v4 mint new-owner)  
  )
)
...
(define-public (flip-private-sale)
  (begin
    ...
    (try! (contract-call? .mythical-nft-v4 set-stx-cost (var-get stx-private-cost-per-mint)))
...
(define-public (flip-sale)
  (begin
    ...
    (try! (contract-call? .mythical-nft-v4 set-stx-cost (var-get stx-public-cost-per-mint)))
...
(as-contract (contract-call? .mythical-nft-v4 set-mint-address))
...
(define-private (banana-public-mint (new-owner principal))
  (let ((mint-count (get-public-balance new-owner)))
    (contract-call? .mythical-nft-v4 banana-mint new-owner)
  )
)
...
(define-private (slime-public-mint (new-owner principal))
  (let ((mint-count (get-public-balance new-owner)))
    (contract-call? .mythical-nft-v4 slime-mint new-owner)
  )
)
