---
title: "Trait stackswap-governance-v5g"
draft: true
---
```
...
(define-public (add-exit-controller (user principal))
  (begin
    (asserts! (is-eq tx-sender (contract-call? .stackswap-dao-v5g get-dao-owner)) (err ERR_NOT_AUTHORIZED))
...
(define-public (propose
  ...
  (let (
    (proposer-balance (unwrap-panic (contract-call? .vstsw-token-v1g get-balance tx-sender)))
    ...
    (asserts! (is-eq (unwrap! (contract-call? .stackswap-dao-v5g get-qualified-name-by-name "governance") (err ERR_DAO_GET)) (as-contract tx-sender)) (err ERR_NOT_AUTHORIZED))
...
(define-public (vote-for (proposal-id uint) (amount uint))
  (let (
    ...
    (unwrap! (contract-call? .vstsw-token-v1g transfer amount tx-sender (as-contract tx-sender) none) (err ERR_TRANSFER_FAIL))
    (map-set proposals
      proposal-id 
...
(define-public (return-votes-to-member (proposal-id uint) (member principal))
  (let (
    ...
    (as-contract (contract-call? .vstsw-token-v1g transfer vote-count (as-contract tx-sender) member none))
  )
)
...
(define-private (execute-proposal (proposal-id uint))
  (let (
    ...
    (asserts! (> (len contract-changes) u0) (err ERR_NO_CONTRACT_CHANGES))
    (unwrap! (as-contract (contract-call? .stackswap-dao-v5g execute-proposals contract-changes)) (err ERR_NO_CONTRACT_CHANGES))
    (ok true)
  )
)
...
(define-public (add-contract-address (name (string-ascii 256)) (address principal) (qualified-name principal))
  (begin
    ...
    (if (is-some (contract-call? .stackswap-dao-v5g get-contract-address-by-name name))
      (ok false)
      (begin
        (try! (contract-call? .stackswap-dao-v5g set-contract-address name address qualified-name))
        (ok true)
      )
    )
  )
