---
title: "Trait welshups-lp-token"
draft: true
---
```
;; Title: WELSH-upS LP Token
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token WUPS)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SPGYCP878RYFVT03ZT8TWGPKNYTSQB1578VVXHGE.welshups-lp-token"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? WUPS amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "WELSH-upS LP Token"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance WUPS who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply WUPS)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SPGYCP878RYFVT03ZT8TWGPKNYTSQB1578VVXHGE.upgraded-shark transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SPGYCP878RYFVT03ZT8TWGPKNYTSQB1578VVXHGE.upgraded-shark transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer (get dx delta) sender CONTRACT none))
        (try! (contract-call? 'SPGYCP878RYFVT03ZT8TWGPKNYTSQB1578VVXHGE.upgraded-shark transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? WUPS (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? WUPS (get dk delta) sender))
        (try! (as-contract (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer (get dx delta) CONTRACT sender none)))
        (try! (as-contract (contract-call? 'SPGYCP878RYFVT03ZT8TWGPKNYTSQB1578VVXHGE.upgraded-shark transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-private (get-reserves)
    { 
      a: (unwrap-panic (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token get-balance CONTRACT)), 
      b: (unwrap-panic (contract-call? 'SPGYCP878RYFVT03ZT8TWGPKNYTSQB1578VVXHGE.upgraded-shark get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (add-liquidity u1500000000))
    
    ;; Transfer additional token A to achieve desired ratio
    (try! (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer u3998500000000 tx-sender CONTRACT none)))
```
