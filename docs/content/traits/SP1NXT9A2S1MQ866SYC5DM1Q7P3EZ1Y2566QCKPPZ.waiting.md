...
(define-non-fungible-token waiting uint)
...
;; Internal variables
(define-data-var mint-limit uint u153)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWGSTpfhypK7F5onD1Vu7ebrbgudiuNFFZrqy5ABnngXR/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? waiting next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? waiting token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? waiting token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? waiting token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? waiting id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? waiting id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? waiting id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u6259)
