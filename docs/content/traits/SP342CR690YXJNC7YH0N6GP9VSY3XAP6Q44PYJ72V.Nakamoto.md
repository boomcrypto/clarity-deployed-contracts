...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://memefun.s3.ap-northeast-1.amazonaws.com/metadata/c4vux512eqa765job14q051pmsq6co4x.json?"))
(define-data-var is-minted bool false)


(define-fungible-token Nakamoto token-max-supply)
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance Nakamoto who)))
...
(define-public (mint (amount uint) (recipient principal)) 
    (begin 
        ...
        (try! (ft-mint? Nakamoto amount recipient))
...
(define-public (burn (amount uint))
    (ft-burn? Nakamoto amount tx-sender))
