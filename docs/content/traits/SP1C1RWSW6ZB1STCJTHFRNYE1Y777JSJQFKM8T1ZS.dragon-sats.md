...
(define-non-fungible-token dragon-sats uint)
...
(define-data-var artist-address principal 'SP1C1RWSW6ZB1STCJTHFRNYE1Y777JSJQFKM8T1ZS)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmaa39ATj19EXbq24p54WL7dVQQrJReUj8ArkEvyjZRmJi/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? dragon-sats next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? dragon-sats token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? dragon-sats token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? dragon-sats token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? dragon-sats id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? dragon-sats id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? dragon-sats id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u72295)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u37000000)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u67014000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u60175000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u1459000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u9200000000)
