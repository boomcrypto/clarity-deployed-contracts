---
title: "Trait neoswap-sc-1682417517870"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/Cc6r3LkYmE8F8eLo21gv
;; SwapId: nk2kxozJ1SpioIiXXsvS
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u7600000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u7600000 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u7600000 tx-sender TRADER-2)))
	    )
            true
        )
