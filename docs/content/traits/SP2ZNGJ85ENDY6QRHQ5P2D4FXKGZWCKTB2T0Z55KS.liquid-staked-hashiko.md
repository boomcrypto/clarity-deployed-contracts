---
title: "Trait liquid-staked-hashiko"
draft: true
---
```
;; Title: Liquid Staked Hashiko
;; Author: rozar.btc
;; Synopsis:
;; This contract implements a liquid staking solution for Hashiko.
;; It provides users with liquid tokens (sHASHIKO) that represent staked Hashiko. 
...
(define-data-var token-name (string-ascii 32) "Liquid Staked Hashiko")
(define-data-var token-symbol (string-ascii 10) "sHASHIKO")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/liquid-staked-hashiko.json"))
...
(define-public (stake (amount uint))
	(begin
		(let
			...
			(try! (contract-call? 'SP28NB976TJHHGF4218KT194NPWP9N1X3WY516Z1P.Hashiko transfer amount sender contract none))
...
(define-public (unstake (amount uint))
	(begin
		(let
			...
			(try! (as-contract (contract-call? 'SP28NB976TJHHGF4218KT194NPWP9N1X3WY516Z1P.Hashiko transfer amount-token contract sender none)))
		)
		(ok true)
	)
)
...
(define-public (deposit (amount uint))
    (contract-call? 'SP28NB976TJHHGF4218KT194NPWP9N1X3WY516Z1P.Hashiko transfer amount tx-sender contract none)
)
...
(define-read-only (get-total-in-pool)
	(unwrap-panic (contract-call? 'SP28NB976TJHHGF4218KT194NPWP9N1X3WY516Z1P.Hashiko get-balance contract))
)
