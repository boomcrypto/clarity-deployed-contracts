...
(define-non-fungible-token TT-NFT uint)
...
(define-data-var artist-address principal 'SP2P6KSAJ4JVV8PFSNKJ9BNG5PEPR4RT71VXZHWBK)
(define-data-var ipfs-root (string-ascii 80) "ipfs://QmcY6Tgypo9xacU/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? TT-NFT next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? TT-NFT token-id tx-sender)))


(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? TT-NFT token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? TT-NFT token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? TT-NFT id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? TT-NFT id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? TT-NFT id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u352)
  ...
  (let 
    ...
    (if (or (is-eq tx-sender art-addr) (is-eq tx-sender DEPLOYER) (is-eq (var-get total-price-mia) u0000000))
      ...
      (begin
        (var-set last-id id-reached)
        (map-set token-count tx-sender (+ current-balance (- id-reached last-nft-id)))
        (try! (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.miamicoin-token transfer price tx-sender (var-get artist-address) (some 0x00)))
      )    
    )
    (ok id-reached)))
...
;; nyc
(define-data-var total-price-nyc uint u632)
  ...
  (let 
    ...
    (if (or (is-eq tx-sender art-addr) (is-eq tx-sender DEPLOYER) (is-eq (var-get total-price-nyc) u0000000))
      ...
      (begin
        (var-set last-id id-reached)
        (map-set token-count tx-sender (+ current-balance (- id-reached last-nft-id)))
        (try! (contract-call? 'SP2H8PY27SEZ03MWRKS5XABZYQN17ETGQS3527SA5.newyorkcitycoin-token transfer price tx-sender (var-get artist-address) (some 0x00)))
      )    
    )
    (ok id-reached)))
...
(map-set mint-passes 'SP28QGZJQ1FKQ8F0V40AEPM8VTNGP2T6MQD2ZVJ8S u5)
;; Airdrop
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? TT-NFT (+ last-nft-id u0) 'SP28QGZJQ1FKQ8F0V40AEPM8VTNGP2T6MQD2ZVJ8S))
