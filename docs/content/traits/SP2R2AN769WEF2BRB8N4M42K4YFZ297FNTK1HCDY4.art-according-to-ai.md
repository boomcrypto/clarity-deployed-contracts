---
title: "Trait art-according-to-ai"
draft: true
---
```
;; art-according-to-ai
...
(define-non-fungible-token art-according-to-ai uint)
...
(define-data-var artist-address principal 'SP2R2AN769WEF2BRB8N4M42K4YFZ297FNTK1HCDY4)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmd4ewM92ED5shm2G17G6aw1WGg4npDdUovRQXr5SiQCD6/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? art-according-to-ai next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? art-according-to-ai token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? art-according-to-ai token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? art-according-to-ai token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? art-according-to-ai id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? art-according-to-ai id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? art-according-to-ai id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u13000)
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u97000)
