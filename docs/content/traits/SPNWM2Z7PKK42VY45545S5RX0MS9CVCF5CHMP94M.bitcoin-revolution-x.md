---
title: "Trait bitcoin-revolution-x"
draft: true
---
```
;; bitcoin-revolution-x
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token bitcoin-revolution-x uint)
...
;; Internal variables
(define-data-var mint-limit uint u41)
(define-data-var last-id uint u1)
(define-data-var total-price uint u2000000)
(define-data-var artist-address principal 'SPNWM2Z7PKK42VY45545S5RX0MS9CVCF5CHMP94M)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmfMXa16yNzG3Sm59Ur4reKNQb81PNJ33mgyYBFocwBdiA/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-revolution-x next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-revolution-x token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-revolution-x token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-revolution-x token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-revolution-x id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-revolution-x id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-revolution-x id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u8641)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u10082000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u7726000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u20000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u249000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u1500000000)
