---
title: "Trait charisma-client-rc4"
draft: true
---
```
...
(define-public (standard-action (generator <generator-trait>) (interaction <interaction-trait>) (action (string-ascii 32)))
    (let
        (
            ...
            (initial-energy (unwrap-panic (contract-call? generator tap)))
        )
        ...
        (let
            ((action-result (free-action interaction action)))
            (try! (handle-energy-overflow))
            action-result
        )
    )
)
...
;; Private functions
(define-private (handle-energy-overflow)
    (contract-call? .energy-overflow handle-overflow)
)
```
