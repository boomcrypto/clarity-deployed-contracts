---
title: "Trait mint-test-v2"
draft: true
---
```
;; (impl-trait 'ST283G030DPDZE00M98SQT3YYC9FGB7ER3TJZFBZ8.numerous-scarlet-muskox.nft-trait) ;; testnet
;; (impl-trait .nft-trait.nft-trait);; local
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait) ;; mainnet

(define-non-fungible-token token-big-qwe-pzs uint)

;; Constants
(define-constant DEPLOYER tx-sender)
(define-constant COMM u1000)
;; (define-constant COMM-ADDR 'ST1XR5G30X40AFDC234FJYVCF7BJ5ASH90Z1N9QSP)
(define-constant USER-MINT-LIMIT u5)
(define-constant TOKEN-PRICE u1)
;; (define-constant WALLET_1 'ST1TFC15DR4XNBC0A1MC5M4WHVSM1MFEJCZVJCYBS)
...
(define-private (mint-many-iter (ignore bool) (next-id uint)) 
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? token-big-qwe-pzs next-id tx-sender) next-id)
      (+ next-id u1) 
    )
    next-id)) 
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? token-big-qwe-pzs token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? token-big-qwe-pzs id sender recipient)
    success
      (let (
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? token-big-qwe-pzs id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? token-big-qwe-pzs id) (err ERR-NOT-FOUND)))
