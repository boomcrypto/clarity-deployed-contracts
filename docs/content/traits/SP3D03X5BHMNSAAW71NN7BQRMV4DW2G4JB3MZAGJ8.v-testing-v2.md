...
(define-public (create-vault (collateral-amount uint) (debt-amount uint))
    (let 
        ...
        (unwrap! (contract-call? .s-testing transfer collateral-amount user .v-testing-v2 none) (err "err-transfer"))
        ;; Mint USDB to the user
        (unwrap! (as-contract (contract-call? .u-testing-v2 mint-usdb user debt-amount)) (err "err-mint"))
...
(define-public (mint-usdb-loan (debt-amount uint))
    (let 
        ...
        ;; Mint USDB to the user
        (unwrap! (contract-call? .u-testing-v2 mint-usdb user debt-amount) (err "err-mint"))
        (ok true)
    )
)
...
(define-public (withdraw-collateral (collateral-amount uint))
    (let 
        ...
        (unwrap! (contract-call? .s-testing transfer collateral-amount .v-testing-v2 user none) (err "err-transfer"))
        (ok true)
    )
)
...
(define-public (repay-usdb-loan (repay-amount uint))
    (let 
        ...
        (unwrap! (contract-call? .u-testing-v2 burn-usdb user repay-amount) (err "err-burn"))
        ...
        (if is-full-repayment
            (begin
                ...
                (unwrap! (contract-call? .s-testing transfer current-collateral .v-testing-v2 user none) (err "err-transfer"))
...
(define-public (liquidate-vault (vault-id principal))
    (let
        ...
        (unwrap! (contract-call? .sp-testing-v2 burn-usdb current-debt) (err "err-burn-usdb"))
        ...
        (map distribute-collateral (contract-call? .sp-testing-v2 get-all-current-users-share))
...
(define-private (distribute-collateral (user-share {user: principal, share: uint}))
    (let 
        ...
        (unwrap! (contract-call? .sp-testing-v2 update-users-map collateral-to-distribute user) (err "err-update-maps"))

        ;; Transfer collateral to the user
        (ok (unwrap! (contract-call? .s-testing transfer collateral-to-distribute .v-testing-v2 user none) (err "err-transfer-collateral")))
...
(define-public (start-redemption (usdb-amount uint) (vault-owner principal))
    (let 
        ...
        (unwrap! (contract-call? .u-testing-v2 burn-usdb tx-sender usdb-amount) (err "err-burn"))

        ;; Transfer the net sBTC amount to the user initiating the redemption
        (unwrap! (contract-call? .s-testing transfer net-sBTC-amount .v-testing-v2 tx-sender none) (err "err-transfer"))

        ;; To whom are we sending the redemption fee?
        (unwrap! (contract-call? .s-testing transfer new-redemption-fee .v-testing-v2 'SP3D03X5BHMNSAAW71NN7BQRMV4DW2G4JB3MZAGJ8 none) (err "err-transfer"))
