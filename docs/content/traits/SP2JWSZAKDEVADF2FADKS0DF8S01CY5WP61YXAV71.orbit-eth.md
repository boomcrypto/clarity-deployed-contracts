...
(define-read-only (is-valid-chain (chain (buff 256)))
    (contract-call? .gov-klaytn is-valid-chain chain)
)

(define-read-only (is-validator (validator principal))
	(contract-call? .gov-klaytn is-validator validator)
)

(define-read-only (get-required)
    (contract-call? .gov-klaytn get-min-confirmation)
)

(define-read-only (get-chain-fee (chain (buff 256)))
    (contract-call? .gov-klaytn get-chain-fee chain)
)

(define-read-only (get-fee-governance)
    (contract-call? .gov-klaytn get-fee-governance)
)

(define-read-only (get-tax-receiver)
    (contract-call? .gov-klaytn get-tax-receiver)
)

(define-read-only (get-tax-rate)
    (contract-call? .gov-klaytn get-tax-rate)
)
