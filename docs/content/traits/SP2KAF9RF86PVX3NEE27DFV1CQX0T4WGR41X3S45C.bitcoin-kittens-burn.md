---
title: "Trait bitcoin-kittens-burn"
draft: true
---
```
(define-public (burn)
...
(define-private (claim-and-burn (new-owner principal))
        (let (
            (token (unwrap-panic (contract-call? .byzantion-bitcoin-kittens claim)))
        )
        (contract-call? .byzantion-bitcoin-kittens transfer token new-owner 'SP000000000000000000002Q6VF78)
)
)
```
