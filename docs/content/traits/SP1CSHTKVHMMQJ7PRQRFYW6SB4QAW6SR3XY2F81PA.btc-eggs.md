...
(define-non-fungible-token btc-eggs uint)
...
(define-constant ERR-MINT-PAUSED (err u106))
(define-constant ERR-NOT-AUTHORIZED (err u107))

;; CONSTANTS
(define-constant CONTRACT-OWNER tx-sender)
(define-constant TOKEN-LIMIT u70)
(define-constant URI-CID "QmUo7ugjNewwWczT1Sk1PDEEiKJvHN8wueFjfJ8e4NwV4e/")

;; VARS
(define-data-var last-token-id uint u0)
(define-data-var mint-price uint u25000000)
(define-data-var is-paused bool false)

;; testnet
;; (define-data-var artist-address principal 'ST1CSHTKVHMMQJ7PRQRFYW6SB4QAW6SR3XZ54PKG7)
;; mainnet
(define-data-var artist-address principal 'SP1FBDV7183BMRESDSKYAA712WVGZA9M95H06VG1W)
...
(define-read-only (get-owner (token-id uint)) 
    (ok (nft-get-owner? btc-eggs token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
    (begin 
        ...
        (nft-transfer? btc-eggs token-id sender recipient)
    )
)
...
(define-public (burn (token-id uint))
    (begin 
        ...
        (nft-burn? btc-eggs token-id tx-sender)
    )
)
...
(define-public (pause-mint)
    (begin 
        (asserts! (or (is-eq tx-sender (var-get artist-address)) (is-eq tx-sender CONTRACT-OWNER)) ERR-NOT-AUTHORIZED)
...
(define-private (mint (recipient principal))
    (let 
        ...
        (asserts! (not paused) (err ERR-MINT-PAUSED))
        (unwrap! (stx-transfer? u23750000 tx-sender (var-get artist-address)) (err ERR-STX-TRANSFER))
        (unwrap! (stx-transfer? u1250000 tx-sender (var-get helper-address)) (err ERR-STX-TRANSFER))
        (unwrap! (nft-mint? btc-eggs token-id recipient) (err ERR-NFT-MINT))
...
(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? btc-eggs token-id) false))
)
