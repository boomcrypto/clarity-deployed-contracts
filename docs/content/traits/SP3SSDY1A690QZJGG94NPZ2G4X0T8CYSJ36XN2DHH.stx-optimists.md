...
(define-non-fungible-token stx-optimists uint)
...
(define-data-var artist-address principal 'SP3SSDY1A690QZJGG94NPZ2G4X0T8CYSJ36XN2DHH)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRhjZnxwBBMGJ5VoYdXcCudminAUseFX43iiXxHeNbeUp/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stx-optimists next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-optimists token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-optimists token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-optimists token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-optimists id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-optimists id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-optimists id) (err ERR-NOT-FOUND)))
