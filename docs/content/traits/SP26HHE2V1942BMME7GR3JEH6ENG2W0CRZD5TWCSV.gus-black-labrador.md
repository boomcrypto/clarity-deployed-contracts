---
title: "Trait gus-black-labrador"
draft: true
---
```
;; gus-black-labrador
...
(define-non-fungible-token gus-black-labrador uint)
...
(define-data-var artist-address principal 'SP26HHE2V1942BMME7GR3JEH6ENG2W0CRZD5TWCSV)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaB9MvJvdcbPFMKJrwkSMymLzpES9pabJqjyewf9okaNB/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? gus-black-labrador next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gus-black-labrador token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gus-black-labrador token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gus-black-labrador token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gus-black-labrador id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gus-black-labrador id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gus-black-labrador id) (err ERR-NOT-FOUND)))
