...
(define-non-fungible-token bitcoin-crows uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmQRLCzEvVQ3PC32TfoDkdeJQGZx56SVBazEbg9Q89mpRc/bitcoin-crows/")
...
(define-data-var artist-address (list 1000 principal) (list 'SPJERZTSMVWW2FWA2H10AHJZXEJF2YBY99DGKZV1 )) 
...
(define-data-var public-target-block-height uint u69042)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u18000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-crows id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace bitcoin-crows
  (ok (nft-get-owner? bitcoin-crows id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? bitcoin-crows next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? bitcoin-crows id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-crows id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-crows id) ERR-NOT-FOUND))
