...
(define-fungible-token wsleo)

(define-data-var token-name (string-ascii 32) "Samurai Leo Wrapper")
(define-data-var token-symbol (string-ascii 32) "wsleo")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wsleo.json"))
...
(define-read-only (get-base-token)
  'SPQPSQ8MAEX8198N8D8QWPCX0XQ6AC2YSGGAAKX0.samurai-leo)

(define-read-only (get-base-decimals)
  (contract-call? 'SPQPSQ8MAEX8198N8D8QWPCX0XQ6AC2YSGGAAKX0.samurai-leo get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SPQPSQ8MAEX8198N8D8QWPCX0XQ6AC2YSGGAAKX0.samurai-leo get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SPQPSQ8MAEX8198N8D8QWPCX0XQ6AC2YSGGAAKX0.samurai-leo transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
