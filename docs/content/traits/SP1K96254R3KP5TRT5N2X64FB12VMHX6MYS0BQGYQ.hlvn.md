...
(define-non-fungible-token hlvn uint)
...
(define-data-var total-price uint u3125000)
(define-data-var artist-address principal 'SP1K96254R3KP5TRT5N2X64FB12VMHX6MYS0BQGYQ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZHrX8Qw4q2aSU8soMjyaqDBuWUj9ypx7PpHLCjiFn4Mm/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? hlvn next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? hlvn token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? hlvn token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? hlvn token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? hlvn id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? hlvn id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? hlvn id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-alex uint u2400000000)
