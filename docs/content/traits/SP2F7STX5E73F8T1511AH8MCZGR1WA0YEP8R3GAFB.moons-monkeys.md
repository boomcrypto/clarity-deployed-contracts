...
(define-non-fungible-token moons-monkeys uint)
...
;; Internal variables
(define-data-var mint-limit uint u599)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1500000)
(define-data-var artist-address principal 'SP2F7STX5E73F8T1511AH8MCZGR1WA0YEP8R3GAFB)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWFGoUX8mSztiRTUyA4gYC9uHpc3H5X56ppSddvcjroY9/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? moons-monkeys next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? moons-monkeys token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? moons-monkeys token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? moons-monkeys token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? moons-monkeys id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? moons-monkeys id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? moons-monkeys id) (err ERR-NOT-FOUND)))
