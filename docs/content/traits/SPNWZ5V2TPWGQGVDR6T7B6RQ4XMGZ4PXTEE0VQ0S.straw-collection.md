...
(define-non-fungible-token straw-collection uint)
...
(define-data-var artist-address principal 'SP18BFREGEWVGNRG40MFA9Z9N3H2QQH0Q7TMC085Q)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTAcf2ssxVM97U9JFVyQR4nzsJ3fAfMPoYn356MZYWday/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? straw-collection next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? straw-collection token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? straw-collection token-id)))
