...
(define-non-fungible-token trevor-test-1 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSQSAbJMMkRPzF4yveDwpsuypnxHpyhfEcmUG3Mn1vv2V/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? trevor-test-1 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? trevor-test-1 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? trevor-test-1 id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? trevor-test-1 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? trevor-test-1 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u1000)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u1000)
