---
title: "Trait boomboxes-cycle-42"
draft: true
---
```
;; Boombox 42
...
(define-non-fungible-token b-42 uint)
...
(define-read-only (is-approved (id uint) (operator principal))
  (let ((owner (unwrap! (nft-get-owner? b-42 id) err-not-found)))
...
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (let ((owner (unwrap! (nft-get-owner? b-42 id) err-not-found)))
    ...
    (nft-transfer? b-42 id sender recipient)))
...
(define-read-only (get-token-uri (id uint))
  (ok (some "ipfs://bafkreia2nf5j5ltowmxj2eepuv5j3rymtljdrsr6y3kaln4cqsbt3vs6xu")))
...
(define-public (mint (bb-id uint) (stacker principal) (amount-ustx uint) (pox-addr {version: (buff 1), hashbytes: (buff 20)}) (locking-period uint))
  (let ((next-id (+ u1 (var-get last-id))))
    ...
    (try! (nft-mint? b-42 next-id stacker))
    (ok next-id)))
