...
(define-non-fungible-token tlylum-33 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRzLWCauihs4rDCbcDhpXh9EPZ3wz4mt2eP4ZmPjS8z4g/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? tlylum-33 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? tlylum-33 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? tlylum-33 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tlylum-33 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? tlylum-33 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? tlylum-33 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? tlylum-33 id) (err ERR-NOT-FOUND)))
