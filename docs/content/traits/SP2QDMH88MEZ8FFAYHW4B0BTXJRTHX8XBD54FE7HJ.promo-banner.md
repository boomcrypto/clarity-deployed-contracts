...
(define-non-fungible-token promo-banner uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUcWnKSj7zTTRGXiTDJiVrXFnAMFM2aq6JWfx9jvufN98/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? promo-banner next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? promo-banner token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? promo-banner token-id)))
