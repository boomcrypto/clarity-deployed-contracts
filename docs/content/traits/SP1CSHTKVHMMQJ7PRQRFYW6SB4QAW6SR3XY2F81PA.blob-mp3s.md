...
(define-non-fungible-token blob-mp3s uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaYveMH4f8PdmhgVR7q5w65HYQt8wZt8u4fHExDa6ULJe/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? blob-mp3s next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? blob-mp3s token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? blob-mp3s token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? blob-mp3s token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? blob-mp3s id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? blob-mp3s id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? blob-mp3s id) (err ERR-NOT-FOUND)))
