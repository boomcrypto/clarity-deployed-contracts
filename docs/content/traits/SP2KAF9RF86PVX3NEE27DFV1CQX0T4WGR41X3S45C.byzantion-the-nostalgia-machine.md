---
title: "Trait byzantion-the-nostalgia-machine"
draft: true
---
```
...
(define-non-fungible-token the-nostalgia-machine uint)
...
(define-data-var cost-per-mint uint u125000000)
(define-data-var commission uint u800)
(define-data-var payout uint u0)
(define-data-var target-block uint u36658)
(define-data-var ipfs-full-metadata (string-ascii 124) "ipfs://bafybeie7qcvpbcwvm2n3gd6hdmf4ydtf6irwnrqir64md3mdpib7ubur3q/the_nostalgia_machine/the_nostalgia_machine_metadata.json")
(define-data-var ipfs-root (string-ascii 111) "ipfs://bafybeie7qcvpbcwvm2n3gd6hdmf4ydtf6irwnrqir64md3mdpib7ubur3q/the_nostalgia_machine/the_nostalgia_machine_")
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? the-nostalgia-machine next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? the-nostalgia-machine token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-nostalgia-machine token-id)))
