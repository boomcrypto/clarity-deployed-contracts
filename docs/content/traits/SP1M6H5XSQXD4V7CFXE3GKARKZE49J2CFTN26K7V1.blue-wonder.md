...
(define-non-fungible-token blue-wonder uint)
...
;; Internal variables
(define-data-var mint-limit uint u212)
(define-data-var last-id uint u1)
(define-data-var total-price uint u4000000)
(define-data-var artist-address principal 'SP1M6H5XSQXD4V7CFXE3GKARKZE49J2CFTN26K7V1)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRBwTT44WcVFM7kXU28WTHQML7nf2dH763BEpqkWEejA2/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? blue-wonder next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? blue-wonder token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? blue-wonder token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? blue-wonder token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? blue-wonder id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? blue-wonder id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? blue-wonder id) (err ERR-NOT-FOUND)))
