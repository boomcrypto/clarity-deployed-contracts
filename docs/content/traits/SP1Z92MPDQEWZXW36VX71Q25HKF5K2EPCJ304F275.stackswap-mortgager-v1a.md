---
title: "Trait stackswap-mortgager-v1a"
draft: true
---
```
(impl-trait .stackswap-manager-trait-v1a.vault-manager-trait)
(use-trait vault-trait .stackswap-vault-trait-v1a.vault-trait)
(use-trait ft-trait .sip-010-v1a.sip-010-trait)
(use-trait vault-manager-trait .stackswap-manager-trait-v1a.vault-manager-trait)
(use-trait collateral-types-trait .stackswap-collateral-types-trait-v1a.collateral-types-trait)
(use-trait oracle-trait .stackwap-oracle-trait-v1a.oracle-trait)
...
(define-public (set-stacking-unlock-burn-height (name (string-ascii 256)) (burn-height uint))
  (begin
    (asserts! 
      (or
        (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "stacker")))
        (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "stacker-2")))
        (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "stacker-3")))
        (is-eq contract-caller (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "stacker-4")))
...
(define-read-only (get-vault-by-id (vault-id uint))
  (contract-call? .stackswap-vault-data-v1a get-vault-by-id vault-id)
)

(define-read-only (get-vault-entries (user principal))
  (contract-call? .stackswap-vault-data-v1a get-vault-entries user)
)
...
(define-public (calculate-current-collateral-to-debt-ratio
  ...
  (let ((vault (get-vault-by-id vault-id)))
    (asserts! (is-eq (contract-of oracle) (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "oracle"))) (err ERR-NOT-AUTHORIZED))
...
(define-public (toggle-stacking (vault-id uint))
  (let (
    ...
    (try! (contract-call? .stackswap-stx-reserve-v1a toggle-stacking (get stacker-name vault) (not (get revoked-stacking vault)) (get collateral vault)))

    (try!
      (contract-call? .stackswap-vault-data-v1a update-vault vault-id (merge vault {
...
(define-public (stack-collateral (vault-id uint))
  (let (
    ...
    (try! (contract-call? .stackswap-stx-reserve-v1a add-tokens-to-stack (get stacker-name vault) (get collateral vault)))
    (try!
      (contract-call? .stackswap-vault-data-v1a update-vault vault-id (merge vault {
...
(define-public (collateralize-and-mint
  ...
  (let (
    ...
    (asserts! (is-eq (contract-of coll-type) (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "collateral-types"))) (err ERR-NOT-AUTHORIZED))
    (asserts! (is-eq (contract-of oracle) (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "oracle"))) (err ERR-NOT-AUTHORIZED))
    (asserts!
      (or
        (is-eq (contract-of reserve) (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "stx-reserve")))
        (is-eq (contract-of reserve) (unwrap-panic (contract-call? .stackswap-dao-v5k get-qualified-name-by-name "sip10-reserve")))
      )
      (err ERR-NOT-AUTHORIZED)
    )
    (asserts!
    ...
    (try! (as-contract (contract-call? .lbtc-token-v1a mint-for-dao debt sender)))
    (let (
      (vault-id (+ (contract-call? .stackswap-vault-data-v1a get-last-vault-id) u1))
      (vault {
        id: vault-id,
        owner: sender,
      ...
      (try! (contract-call? .stackswap-vault-data-v1a update-vault-entries sender vault-id))
      (try! (contract-call? .stackswap-vault-data-v1a update-vault vault-id vault))
      (try! (contract-call? .stackswap-vault-data-v1a set-last-vault-id vault-id))
...
(define-public (deposit
  ...
  (let (
    ...
    (try! (contract-call? .stackswap-vault-data-v1a update-vault vault-id updated-vault))
...
(define-public (close-vault
  ...
  (let ((vault (get-vault-by-id vault-id))
    ...
    (if (is-eq (get debt vault) u0)
      true
      (try! (contract-call? .lbtc-token-v1a burn-for-dao (get debt vault) (get owner vault)))
    )
    ...
    (try! (contract-call? .stackswap-vault-data-v1a update-vault vault-id updated-vault))
    ;; (try! (contract-call? .stackswap-vault-rewards-v1-1 remove-collateral (get collateral vault) (get owner vault)))
    (print { type: "vault", action: "close", data: updated-vault })
    (try! (contract-call? .stackswap-vault-data-v1a close-vault vault-id))
    (ok true)
  )
)
...
(define-private (burn-partial-debt
  ...
  (let ((vault (get-vault-by-id vault-id)))
    (try! (contract-call? .lbtc-token-v1a burn-for-dao debt tx-sender))
    (try! (contract-call? .stackswap-vault-data-v1a update-vault vault-id (merge vault {
...
(define-public (pay-stability-fee
  ...
  (let (
    ...
    (if (> fee u0)
      (begin
        (try! (contract-call? .lbtc-token-v1a transfer fee tx-sender (as-contract tx-sender) none))
        (try! (contract-call? .stackswap-vault-data-v1a update-vault vault-id (merge vault {
            updated-at-block-height: block-height,
            stability-fee-accrued: u0,
            stability-fee-last-accrued: block-height
...
(define-public (notify-risky-vault
  ...
  (let (
    ...
    (collateral-to-debt-ratio (unwrap-panic (calculate-current-collateral-to-debt-ratio vault-id coll-type oracle true)))
...
(define-public (burn-to-unliquidate
  ...
  (let (
    ...
    (asserts! (>= ratio (get collateral-to-debt-ratio collateral-type)) (err ERR-INSUFFICIENT-LBTC))
...
(define-public (finalize-liquidation
  ...
  (let (
    ...
    (if (and (not (get revoked-stacking vault)) (is-eq "STX" (get collateral-token vault)))
      (try! (contract-call? .stackswap-stx-reserve-v1a toggle-stacking (get stacker-name vault) true (get collateral vault)))
      u0
    )
    (try! (contract-call? .stackswap-vault-data-v1a finalize-liquidate-vault vault-id (merge vault {
...
(define-read-only (get-lbtc-balance)
  (contract-call? .lbtc-token-v1a get-balance (as-contract tx-sender))
)
  ...
  (begin
    ...
    (if (and (> lbtc-amount u0) (> stsw-amount u0))
      (begin
        ...
        (as-contract (contract-call? .lbtc-token-v1a transfer lbtc-amount tx-sender (contract-call? .stackswap-dao-v5k get-payout-address) none))
      )
      (if (> lbtc-amount u0)
        (as-contract (contract-call? .lbtc-token-v1a transfer lbtc-amount tx-sender (contract-call? .stackswap-dao-v5k get-payout-address) none))
...
(define-public (close-vault-liquidated
  ...
  (let ((vault (get-vault-by-id vault-id))
    ...
    (try! (contract-call? .stackswap-vault-data-v1a close-vault-liquidated vault-id))
    (ok true)
  )
)
