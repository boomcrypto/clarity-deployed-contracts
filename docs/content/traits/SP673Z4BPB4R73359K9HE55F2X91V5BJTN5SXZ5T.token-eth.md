...
(define-fungible-token token-eth)

(define-data-var token-name (string-ascii 32) "ETH")
(define-data-var token-symbol (string-ascii 10) "ETH")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-eth.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(try! (ft-transfer? token-eth amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance token-eth who)))

(define-read-only (get-total-supply)
	(ok (ft-get-supply token-eth)))
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? token-eth amount recipient)))
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? token-eth amount sender)))
