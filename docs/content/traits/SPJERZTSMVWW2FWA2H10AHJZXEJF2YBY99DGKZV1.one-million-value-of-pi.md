---
title: "Trait one-million-value-of-pi"
draft: true
---
```
;; one-million-value-of-pi
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token one-million-value-of-pi uint)
...
;; Internal variables
(define-data-var mint-limit uint u254)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNf2Aea7WgMo2CWdnsipeSJR5fs9Fyi6iyzNfGzeQ4hY8/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? one-million-value-of-pi next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? one-million-value-of-pi token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? one-million-value-of-pi token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? one-million-value-of-pi token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? one-million-value-of-pi id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? one-million-value-of-pi id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? one-million-value-of-pi id) (err ERR-NOT-FOUND)))
