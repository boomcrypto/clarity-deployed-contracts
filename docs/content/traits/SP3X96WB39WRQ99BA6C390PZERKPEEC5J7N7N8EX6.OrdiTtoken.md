---
title: "Trait OrdiTtoken"
draft: true
---
```
(impl-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)
(define-constant ERR-UNAUTHORIZED (err u401))
(define-fungible-token OrdiToken)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? OrdiToken amount from to)
    )
)
...
(define-read-only (get-balance (user principal))
    (ok (ft-get-balance OrdiToken user)
    )
)

(define-read-only (get-total-supply)
    (ok (ft-get-supply OrdiToken)
    )
)
...
(begin
  (try! (ft-mint? OrdiToken u1000000000000000 contract-creator))
)
```
