---
title: "Trait neoswap-sc-1688230886154"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/oqc7Bvj9GkTKdPlCzTvM
;; SwapId: Qagt3otMAPWBhanvAAr7

;; traders
(define-constant TRADER-1 'SPJ1NDKNTN2WDXJ7MZZB8QR0TZ24VPRG2J017DJY)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u3847000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.wasteland-apes-nft transfer u4805 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.wasteland-apes-nft transfer u4805 tx-sender TRADER-1)))

	(unwrap-panic (as-contract (stx-transfer? u192350 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u3654650 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u3847000 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.wasteland-apes-nft transfer u4805 tx-sender TRADER-2)))
	    )
            true
        )
