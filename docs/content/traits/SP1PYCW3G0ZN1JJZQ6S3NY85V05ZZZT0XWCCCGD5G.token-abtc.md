...
(define-constant TOKEN_URI u"https://cdn.alexlab.co/metadata/token-abtc.json") ;; utf-8 string with token metadata host
(define-constant TOKEN_NAME "aBTC")
...
(begin
    ...
    ;; Transfer the tokens
    (let ((result (ft-transfer? bridged-btc amount sender recipient)))
      (if (is-ok result)
        (begin
...
;;   (let ((transferOk (try! (transferToken amount tx-sender to (some 0x00)))))
;;     (ok transferOk)
;;   )
;; )
...
(begin
  (try! (ft-mint? bridged-btc u1200000000 CONTRACT_OWNER)) 
  ;; (try! (ft-mint? bridged-btc u100000000000 'ST18VZNQWK7R44BFN75X3RANYRAT4B9ZA8S47KZ24)) 
) 
