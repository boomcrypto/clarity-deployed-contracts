---
title: "Trait cross-peg-out-endpoint-v2"
draft: true
---
```
...
(define-public (transfer-to-unwrap (token-trait <ft-trait>) (amount-in-fixed uint) (dest-chain-id uint) (settle-address (buff 256)))
  (let (
    ...
    (print { object: "cross-bridge-endpoint", action: "transfer-to-unwrap", user: sender, chain: (get name chain-details), net-amount: net-amount, fee-amount: fee, settle-address: settle-address, token: token })
    (ok true)))
