...
(define-non-fungible-token bitcoin-whales-3d uint)
...
(define-data-var artist-address principal 'SP2WF8C5AVTB3P1AESCDJHHPCSDG3CGES97XSNP72)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeQVmq4SkWnhC8xmgbtry5fAcnMQvYn5gUVGZ8n4obHcs/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-whales-3d next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? bitcoin-whales-3d token-id sender recipient)))

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-whales-3d token-id)))
