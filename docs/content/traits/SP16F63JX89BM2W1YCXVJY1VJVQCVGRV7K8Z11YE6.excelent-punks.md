...
(define-non-fungible-token excelent-punks uint)
...
(define-data-var artist-address principal 'SP16F63JX89BM2W1YCXVJY1VJVQCVGRV7K8Z11YE6)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmfAKUQSLGCsQSA2DFz9hcuNKoEFsqneK9183vWVT37iKW/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? excelent-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? excelent-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? excelent-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? excelent-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? excelent-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? excelent-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? excelent-punks id) (err ERR-NOT-FOUND)))
