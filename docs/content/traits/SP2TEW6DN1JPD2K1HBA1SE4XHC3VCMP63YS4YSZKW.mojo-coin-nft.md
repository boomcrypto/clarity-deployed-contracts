...
(define-non-fungible-token mojo-coin-nft uint)
...
(define-data-var total-price uint u2069000)
(define-data-var artist-address principal 'SP2TEW6DN1JPD2K1HBA1SE4XHC3VCMP63YS4YSZKW)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmT8C6CF29CxvD2mEvWwZfu6SbrBnU8Pd2vpX8DRXRoFdN/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? mojo-coin-nft next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? mojo-coin-nft token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? mojo-coin-nft token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? mojo-coin-nft token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? mojo-coin-nft id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? mojo-coin-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? mojo-coin-nft id) (err ERR-NOT-FOUND)))
