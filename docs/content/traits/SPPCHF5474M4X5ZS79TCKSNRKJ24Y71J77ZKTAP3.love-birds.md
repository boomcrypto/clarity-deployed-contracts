...
(define-non-fungible-token love-birds uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQhAiyvVpz6CAnZ3HTGw1Hw9tEF8zNZnp3LXJix9Yyq6K/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? love-birds next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? love-birds token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? love-birds token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? love-birds token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? love-birds id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? love-birds id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? love-birds id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u7042)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u12645)
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u63357)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u24)
