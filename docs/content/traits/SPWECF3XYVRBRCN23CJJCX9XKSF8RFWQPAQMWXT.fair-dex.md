...
(define-constant allow-token 'SPWECF3XYVRBRCN23CJJCX9XKSF8RFWQPAQMWXT.fair-bonding-curve)
      ...
      (if (>= new-stx-balance  STX_TARGET_AMOUNT)
        (begin
          (let (
            ...
            (remain-stx-alex-format (/ remain-stx (pow u10 ONE_8)))
            (remain-tokens-alex-format (/ remain-tokens (pow u10 ONE_8)))
            
          )
            ;; burn tokens
            (try! (as-contract (contract-call? token-trait transfer burn-amount tx-sender BURN_ADDRESS none)))
            ;; Send to self-service
            ...
            ;; (try! (as-contract (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.self-listing-helper-v2-02 request-create-and-fund 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-wstx-v2 (contract-of token-trait) 
            ;; u100000000 remain-stx-alex-format remain-tokens-alex-format u500000 u500000 u60000000 u60000000 u0 u0 true u99000000 u0 none)) )
