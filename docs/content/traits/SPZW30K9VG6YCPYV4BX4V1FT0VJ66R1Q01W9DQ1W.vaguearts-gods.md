...
(define-non-fungible-token vaguearts-gods uint)
...
(define-data-var artist-address principal 'SP1E08C7JNNVSVP9CT8CKPZV2VRGA33NFST3KR7Z6)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeRGiHiJHsNVbB8F1PsDG5pLKV6wSR8KpXvqKk6w59TZg/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? vaguearts-gods next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? vaguearts-gods token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? vaguearts-gods token-id)))
