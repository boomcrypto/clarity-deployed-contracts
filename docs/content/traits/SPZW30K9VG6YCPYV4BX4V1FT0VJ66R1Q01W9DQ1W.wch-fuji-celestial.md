---
title: "Trait wch-fuji-celestial"
draft: true
---
```
;; wch-fuji-celestial
...
(define-non-fungible-token wch-fuji-celestial uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVueHMVqoRpe7H4CBGSrZbpcVLiUXsKo1SPBEZhvLhgaT/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? wch-fuji-celestial next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? wch-fuji-celestial token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? wch-fuji-celestial token-id)))
