...
(define-non-fungible-token Boogeyman-6A uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafyreicwyj3rhzg5jswcdf5ln2u6myuwuwdgp6dsczkcqicnh3cdshevpa/metadata.json")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Boogeyman-6A token-id)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Boogeyman-6A token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Boogeyman-6A token-id) false)))
...
(define-private (iterate-minting (not-to-be-used bool) (next-id uint))
  (if (<= next-id (var-get max-supply))
    (begin
      (unwrap! (nft-mint? Boogeyman-6A next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Boogeyman-6A id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Boogeyman-6A id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Boogeyman-6A id) (err err-not-found)))
...
(define-private (airdrop-iter (collector principal) (next-id uint)) 
  (if (<= next-id (var-get max-supply))
    (let (
      ...
      (unwrap! (nft-mint? Boogeyman-6A next-id collector) next-id)
