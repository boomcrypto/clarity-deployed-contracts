...
(define-non-fungible-token recetas-nft uint)
...
(define-data-var artist-address principal 'SP39FZ0AJAEFNVDEXH3WNGVYNG9HNCYCY2XW92GG4)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmdy7BSqpnnKNv2G3GmpJQSyGS5Cz2WTc8xEsCeN6s4e6X/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? recetas-nft next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? recetas-nft token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? recetas-nft token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? recetas-nft token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? recetas-nft id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? recetas-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? recetas-nft id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u126358)
