...
(define-private (verify-price-with-oracle 
  (price uint)
  (conf-tolerance-bps uint)
...
(define-public (request-mint
  ...
  (let (
    ...
    (try! (verify-price-with-oracle price conf-tolerance-bps oracle-data minting-asset-contract))
...
(define-public (request-redeem
  ...
  (let (
    ...
    (try! (verify-price-with-oracle price conf-tolerance-bps oracle-data redeeming-asset-contract))
    (asserts! (> oracle-timestamp block-timestamp) ERR_STALE_DATA)
...
(define-public (confirm-mint (request-id uint) (price uint) (amount-asset-confirmed uint) (minting-asset <sip-010-trait>) (custody-address principal) (memo (optional (buff 34))))
  (let (
    ...
    (try! (contract-call? minting-asset transfer (- amount-asset-confirmed amount-asset-fee) this-contract custody-address none))
    ...
    (if (not (is-eq amount-asset-requested amount-asset-confirmed))
      (try! (contract-call? minting-asset transfer (- amount-asset-requested amount-asset-confirmed) this-contract requester memo))
      true
    )
