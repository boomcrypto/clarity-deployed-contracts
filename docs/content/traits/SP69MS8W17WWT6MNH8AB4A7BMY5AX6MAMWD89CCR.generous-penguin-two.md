---
title: "Trait generous-penguin-two"
draft: true
---
```
...
(define-non-fungible-token generous-penguin-two uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmRW5QNsG3S8kEnmjsRVRb1G7jXcNeeC2Am8nLi7oJFjBD/generous-penguin-two/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP69MS8W17WWT6MNH8AB4A7BMY5AX6MAMWD89CCR )) 
...
(define-data-var public-target-block-height uint u64284)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? generous-penguin-two id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace generous-penguin-two
  (ok (nft-get-owner? generous-penguin-two id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? generous-penguin-two next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? generous-penguin-two id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? generous-penguin-two id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? generous-penguin-two id) ERR-NOT-FOUND))
...
(define-data-var alex-cost-per-mint uint u3000000000)
