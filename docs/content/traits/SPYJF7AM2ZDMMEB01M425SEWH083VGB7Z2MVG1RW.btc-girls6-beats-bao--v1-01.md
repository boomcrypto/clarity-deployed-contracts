---
title: "Trait btc-girls6-beats-bao--v1-01"
draft: true
---
```
;; btc-girls6-beats-bao--v1-01
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token btc-girls6-beats-bao--v1-01 uint)
...
;; Internal variables
(define-data-var mint-limit uint u306)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYViVxXbFQWrESTjM1zxchfHrj2aHcQsf7rryx7VyWqTi/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? btc-girls6-beats-bao--v1-01 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? btc-girls6-beats-bao--v1-01 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? btc-girls6-beats-bao--v1-01 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? btc-girls6-beats-bao--v1-01 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? btc-girls6-beats-bao--v1-01 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? btc-girls6-beats-bao--v1-01 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? btc-girls6-beats-bao--v1-01 id) (err ERR-NOT-FOUND)))
