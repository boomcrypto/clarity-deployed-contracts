---
title: "Trait MOCHINES-ANONYMOUS_"
draft: true
---
```
;; nft Contract
...
(define-non-fungible-token Anonymous_ uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafkreih45mwhqlzha3w5r657aumnhxhyatmechyragumwuhdbhr7g2pxxq")
...
(define-public (mint (recipient principal))
    (begin 
        ...
        (asserts! (< (var-get last-id) SUPPLY) (err ERR-NO-MORE-NFTS))
            ...
            (try! (nft-mint? Anonymous_ u1 tx-sender))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Anonymous_ token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Anonymous_ token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Anonymous_ token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Anonymous_ id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Anonymous_ id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Anonymous_ id) (err ERR-NOT-FOUND)))
