...
(define-non-fungible-token bitcoin-invaders uint)
...
(define-data-var total-price uint u3333000)
(define-data-var artist-address principal 'SP5A7DMHH279TT4Q92007TSVNCRD35N7EREBVRTH)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQzp5eFS56yg7Y2QhpRo5qqvTZ2BfJsAYkiycppZmh3Tt/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-invaders next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-invaders token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-invaders token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-invaders token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-invaders id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-invaders id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-invaders id) (err ERR-NOT-FOUND)))
