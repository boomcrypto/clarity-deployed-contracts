---
title: "Trait gen-stacks-wallpapers-iphone-pro-max"
draft: true
---
```
...
(define-non-fungible-token gen-stacks-wallpapers-iphone-pro-max uint)
...
(define-constant MINT-LIMIT u1261)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmNPdAbrx9rRMFGkFVRzpiJvh5XG7DEjKA2bVkGuHZtmBw/gen-stacks-wallpapers-iphone-pro-max/")
...
(define-data-var public-target-block-height uint u70583)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u100000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gen-stacks-wallpapers-iphone-pro-max id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace gen-stacks-wallpapers-iphone-pro-max
  (ok (nft-get-owner? gen-stacks-wallpapers-iphone-pro-max id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? gen-stacks-wallpapers-iphone-pro-max next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? gen-stacks-wallpapers-iphone-pro-max id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gen-stacks-wallpapers-iphone-pro-max id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gen-stacks-wallpapers-iphone-pro-max id) ERR-NOT-FOUND))
