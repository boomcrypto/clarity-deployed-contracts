...
(define-non-fungible-token stacks-ape-gang uint)
...
(define-data-var artist-address principal 'SP2FEBCJW8WZN4CDAW4F8GZV199VE2RRMCQWZAC0Y)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcnBa9uyTBuN857hBNG4JiHdGviY17nBwQZc4QvMg2B4H/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-ape-gang next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-ape-gang token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-ape-gang token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-ape-gang token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-ape-gang id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-ape-gang id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-ape-gang id) (err ERR-NOT-FOUND)))
