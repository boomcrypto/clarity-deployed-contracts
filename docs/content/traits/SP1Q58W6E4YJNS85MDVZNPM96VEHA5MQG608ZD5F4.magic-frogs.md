...
(define-non-fungible-token magic-frogs uint)
...
(define-data-var artist-address principal 'SP1Q58W6E4YJNS85MDVZNPM96VEHA5MQG608ZD5F4)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPgjn2vwGA7y1koFAAGFHf263vMa4etEUaip33X7cvug1/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? magic-frogs next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? magic-frogs token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? magic-frogs token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? magic-frogs token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? magic-frogs id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? magic-frogs id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? magic-frogs id) (err ERR-NOT-FOUND)))
