---
title: "Trait into-the-light-next-level"
draft: true
---
```
;; into-the-light-next-level
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token into-the-light-next-level uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTsTsRbAVk7w36eMpPuAVVM6qoLL13bS1hBN25eiqdxCk/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? into-the-light-next-level next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? into-the-light-next-level token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? into-the-light-next-level token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? into-the-light-next-level token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? into-the-light-next-level id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? into-the-light-next-level id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? into-the-light-next-level id) (err ERR-NOT-FOUND)))
