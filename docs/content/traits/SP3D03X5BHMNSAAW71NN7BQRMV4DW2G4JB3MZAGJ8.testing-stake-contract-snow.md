---
title: "Trait testing-stake-contract-snow"
draft: true
---
```
;; $SNOW FT Contract
...
(define-public (mint (amount uint) (recipient principal))
  (let
    ...
    (if (var-get mint-active) 
      (asserts! (or (is-eq tx-sender contract-owner) (is-eq contract-caller .testing-stake-contract)) ERR_NOT_AUTH) 
      (asserts! (is-eq contract-caller .testing-stake-contract) ERR_NOT_AUTH) 
    )

    ;; asserts that recipient is not staking contract
    (asserts! (not (is-eq recipient .testing-stake-contract)) ERR_RECIPIENT_NOT_VALID)
