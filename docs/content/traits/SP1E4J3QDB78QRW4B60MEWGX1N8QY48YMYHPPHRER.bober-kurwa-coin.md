...
(define-non-fungible-token bober-kurwa-coin uint)
...
;; Internal variables
(define-data-var mint-limit uint u21000000)
(define-data-var last-id uint u1)
(define-data-var total-price uint u555000)
(define-data-var artist-address principal 'SP1E4J3QDB78QRW4B60MEWGX1N8QY48YMYHPPHRER)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZ9g8yvGnt8JCrmCbZwj1Wh7wfQNn66aXQfpgsvYpwc5H/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u100000)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? bober-kurwa-coin next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bober-kurwa-coin token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bober-kurwa-coin token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bober-kurwa-coin token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bober-kurwa-coin id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bober-kurwa-coin id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bober-kurwa-coin id) (err ERR-NOT-FOUND)))
