...
(define-non-fungible-token stacks-gang uint)
...
(define-data-var artist-address principal 'SP1A81R70X8CT86Q0FC2JDP8AV56Y07Z1EKHRNH0R)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaYHK89T818hWJzZ4n3ujFoaSsfk6KTF7nK4omHm9SkJT/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-gang next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-gang token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-gang token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-gang token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-gang id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-gang id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-gang id) (err ERR-NOT-FOUND)))
