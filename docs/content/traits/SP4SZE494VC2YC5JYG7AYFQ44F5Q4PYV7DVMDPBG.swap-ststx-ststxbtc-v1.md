---
title: "Trait swap-ststx-ststxbtc-v1"
draft: true
---
```
...
(define-public (swap-ststx-for-ststxbtc (ststx-amount uint) (reserve <reserve-trait>) )
  (let (
    (stx-ststx (try! (contract-call? .data-core-v2 get-stx-per-ststx reserve)))
    (stx-amount (/ (* ststx-amount stx-ststx) DENOMINATOR_6))
  )
    (try! (contract-call? .dao check-is-protocol (contract-of reserve)))

    (try! (contract-call? .ststxbtc-token mint-for-protocol stx-amount tx-sender))
    ...
    (asserts! (>= (unwrap-panic (contract-call? .ststxbtc-token get-total-supply)) u1000000) (err ERR_LOW_SUPPLY))
...
(define-public (swap-ststxbtc-for-ststx (ststxbtc-amount uint) (reserve <reserve-trait>) )
  (let (
    ...
    (try! (contract-call? .ststxbtc-token burn-for-protocol ststxbtc-amount tx-sender))
