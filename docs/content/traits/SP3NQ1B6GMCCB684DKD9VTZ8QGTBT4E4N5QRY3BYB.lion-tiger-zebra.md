...
(define-constant ONE_EIGHT u100000000)
...
(if is-in-mainnet 
  (begin 
    ;; TODO: update for mainnet when known
    (var-set current-cycle-tenure-height u177795) 
    (var-set nr-snapshots-cycle u3)
    (var-set nr-blocks-snapshot u60)
  )
  ...
  ;; ;; testnet
  ;; ;; https://api.testnet.hiro.so/v2/pox get reward btc block
  ;; ;; https://api.testnet.hiro.so/v2/info get tenure height
  ;; (begin
  ;;   ;; deployment configs primary testnet
  ;;   ;; "current_burnchain_block_height": 75350, -> 250 in the future
  ;;   ;; "reward_phase_start_block_height": 75600,
  ;;   ;; "tenure_height": 62628, -> u62878

  ;;   (var-set current-cycle-tenure-height u62878) 
  ;;   (var-set nr-snapshots-cycle u10)
  ;;   (var-set nr-blocks-snapshot u100)
  ;; )
)
...
;; APR - annual percentage rate scaled by 10^6. This represents the maximum
  ...
  (let
    (
      (balance (at-block
        ...
        (unwrap-panic (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token get-balance address))))
...
;; add print for backend as that address is opted-out (if enrolled)
...
(define-public (set-can-distribute-rewards)
  (begin 
    ...
    (let ((pool-rewards (at-block 
                  ...
                  (unwrap-panic (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token get-balance (as-contract contract-caller)))))
          ...
          (local-rewards-to-distribute 
            (if 
              (< (* ONE_EIGHT pool-rewards) max-cap-rewards) 
              (* ONE_EIGHT pool-rewards)
              max-cap-rewards)))
...
;; Cycle percentage rate scaled by 10^6. This represents the maximum
;; percentage of the sBTC yield contributors are ready to give away as rewards
;; in a cycle.
