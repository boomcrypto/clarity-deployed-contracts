---
title: "Trait brc20-trio"
draft: true
---
```
(define-constant ERR-NOT-AUTHORIZED (err u1000))
(define-constant ONE_8 u100000000)
(define-fungible-token brc20-trio)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-name (string-ascii 32) "TRIO (BRC20)")
(define-data-var token-symbol (string-ascii 32) "TRIO")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/brc20-trio.json"))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance brc20-trio who))
)
(define-read-only (get-total-supply)
	(ok (ft-get-supply brc20-trio))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? brc20-trio amount recipient)
	)
)
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? brc20-trio amount sender)
	)
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? brc20-trio amount sender recipient))
...
(contract-call? .alex-vault-v1-1 set-approved-token .brc20-trio true)
```
