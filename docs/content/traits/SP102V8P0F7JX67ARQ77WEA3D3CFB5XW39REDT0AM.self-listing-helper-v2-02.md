---
title: "Trait self-listing-helper-v2-02"
draft: true
---
```
...
(define-public (request-create
    ...
    (let (
            ...
            (updated-request-details (merge request-details { requested-by: tx-sender, requested-at: block-height, status: PENDING, status-memo: none, bal-y-sent: false })))
...
(define-public (request-create-and-fund
    (request-details {
        ...
        oracle-enabled: bool, oracle-average: uint,
        start-block: uint,
        memo: (optional (buff 256)) }) (token-x-trait <ft-trait>) (token-y-trait <ft-trait-standard>))
	(let (
...
(define-public (request-create-and-finalize
    (request-details {
        ...
        oracle-enabled: bool, oracle-average: uint,
        start-block: uint,
        memo: (optional (buff 256)) }) (token-x-trait <ft-trait>) (token-y-trait <ft-trait>))
	(let (
...
(define-public (finalize-request (request-id uint) (token-x-trait <ft-trait>) (token-y-trait <ft-trait>))
    (let (
        ...
        (asserts! (or (is-eq tx-sender (get requested-by request-details)) (try! (is-dao-or-extension))) err-not-authorised)
        (asserts! (is-eq (get status request-details) APPROVED) err-request-not-approved)
        (asserts! (is-eq (get token-x request-details) (contract-of token-x-trait)) err-token-mismatch)
        (asserts! (is-eq (get token-y request-details) (contract-of token-y-trait)) err-token-mismatch)
        (and (not (is-eq sender (as-contract tx-sender))) (as-contract (try! (contract-call? token-x-trait transfer-fixed (get bal-x request-details) tx-sender sender none))))
				(and (not (is-eq sender (as-contract tx-sender))) (get bal-y-sent request-details) (as-contract (try! (contract-call? token-y-trait transfer-fixed (get bal-y request-details) tx-sender sender none))))
...
(define-public (execute (sender principal))
	(begin
		...
		(try! (approve-token-x .token-alex true u1000000000000))
		(try! (approve-token-x .token-wstx-v2 true u180000000000))		
		(ok true)))
```
