---
title: "Trait neoswap-sc-1682625662910"
draft: true
---
```
;; multiway-swap
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u1500000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u28500000 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u30000000 tx-sender TRADER-2)))
	    )
            true
        )
