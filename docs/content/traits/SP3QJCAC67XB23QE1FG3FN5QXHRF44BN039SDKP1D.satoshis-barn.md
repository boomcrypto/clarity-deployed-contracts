...
(define-non-fungible-token satoshis-barn uint)
...
(define-data-var artist-address principal 'SP3QJCAC67XB23QE1FG3FN5QXHRF44BN039SDKP1D)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcEyhZC1sGqADiZKqZxy3HUPFp4NJHY4TinQeH7uE6gty/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? satoshis-barn next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? satoshis-barn token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? satoshis-barn token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? satoshis-barn token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? satoshis-barn id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? satoshis-barn id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? satoshis-barn id) (err ERR-NOT-FOUND)))
