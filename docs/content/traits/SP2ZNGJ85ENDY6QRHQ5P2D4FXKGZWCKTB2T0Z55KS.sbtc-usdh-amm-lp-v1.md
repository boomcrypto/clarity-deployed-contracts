---
title: "Trait sbtc-usdh-amm-lp-v1"
draft: true
---
```

;; Title: sBTC-USDh Liquidity Pool
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token sBTC-USDh-LP)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? sBTC-USDh-LP amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "sBTC-USDh Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance sBTC-USDh-LP who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply sBTC-USDh-LP)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? sBTC-USDh-LP (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? sBTC-USDh-LP (get dk delta) sender))
...
(define-read-only (get-reserves-quote)
    (let (
        (reserves (get-reserves))
        (supply (ft-get-supply sBTC-USDh-LP)))
        {
          dx: (get a reserves),
          dy: (get b reserves),
          dk: supply
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://charisma-metadata.vercel.app/api/v1/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.sbtc-usdh-amm-lp-v1"))
    ...
    (try! (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 transfer u99999000 tx-sender CONTRACT none))
)
```
