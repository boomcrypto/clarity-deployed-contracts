---
title: "Trait bitcoin-origns-nft"
draft: true
---
```
;; bitcoin-origns-nft
...
(define-non-fungible-token bitcoin-origns-nft uint)
...
(define-data-var artist-address principal 'SP3CNXCVHZB01FXTM1KJTAX7PTHNQDDEDMY7HJ8G0)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZycDyJBb9VLj5equ9HRyKFvi8fqhmVcBENoYLq18jfjs/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-origns-nft next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-origns-nft token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-origns-nft token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-origns-nft token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-origns-nft id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-origns-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-origns-nft id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mega uint u1000000)
