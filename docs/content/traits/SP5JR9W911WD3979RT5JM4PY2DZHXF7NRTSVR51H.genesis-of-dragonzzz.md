---
title: "Trait genesis-of-dragonzzz"
draft: true
---
```
;; genesis-of-dragonzzz
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token genesis-of-dragonzzz uint)
...
(define-data-var artist-address principal 'SP5JR9W911WD3979RT5JM4PY2DZHXF7NRTSVR51H)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTf6o28ZcXNNici1d7V7FFe6nwXtFXeeur12bowfiqWnS/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? genesis-of-dragonzzz next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? genesis-of-dragonzzz token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? genesis-of-dragonzzz token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? genesis-of-dragonzzz token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? genesis-of-dragonzzz id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? genesis-of-dragonzzz id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? genesis-of-dragonzzz id) (err ERR-NOT-FOUND)))
