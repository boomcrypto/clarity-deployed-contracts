---
title: "Trait miamicoin-art-week"
draft: true
---
```
;; miamicoin-art-week
...
(define-non-fungible-token miamicoin-art-week uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaEGt1xAd3z8R3PrWVexrMPjEBvKWvPo4Do1TdqcmqjDw/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? miamicoin-art-week next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? miamicoin-art-week token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? miamicoin-art-week token-id)))
