...
(define-non-fungible-token stx-bayc uint)
...
(define-data-var artist-address principal 'SP368GZ5BZPEM9ZQHH392C4HSJCFF9M8S6Y9EKJ2Z)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYkQ7mvJgFYSM7eJKcwrrRHkpVRDkxgjQy96hM7eNfwZ2/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stx-bayc next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-bayc token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-bayc token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-bayc token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-bayc id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-bayc id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-bayc id) (err ERR-NOT-FOUND)))
