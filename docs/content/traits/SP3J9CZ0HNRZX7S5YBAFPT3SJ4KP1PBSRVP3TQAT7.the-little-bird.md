...
(define-non-fungible-token the-little-bird uint)
...
(define-data-var total-price uint u130000)
(define-data-var artist-address principal 'SP3J9CZ0HNRZX7S5YBAFPT3SJ4KP1PBSRVP3TQAT7)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWzKCdRcxTLWNeZ5arKXh1bneoJoeYyNDGfjTYjFeZeAE/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? the-little-bird next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-little-bird token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-little-bird token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-little-bird token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-little-bird id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-little-bird id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-little-bird id) (err ERR-NOT-FOUND)))
