...
(define-non-fungible-token welshiebits uint)
...
(define-data-var artist-address principal 'SP2BXK29DV08ZFG86ZFT2JXPHC326A29BNTJ8PD5G)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRCYwuNQms1SXXmsnnsstTd4A9BeM1UPAaBAgDpnUCqaB/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? welshiebits next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? welshiebits token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? welshiebits token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? welshiebits token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? welshiebits id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? welshiebits id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? welshiebits id) (err ERR-NOT-FOUND)))
