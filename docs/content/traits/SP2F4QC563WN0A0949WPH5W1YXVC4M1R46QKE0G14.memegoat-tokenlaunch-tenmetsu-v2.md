---
title: "Trait memegoat-tokenlaunch-tenmetsu-v2"
draft: true
---
```
...
(define-data-var no-of-participants uint u86)
...
(define-read-only (get-user-deposits (user-addr principal)) 
  (let
    ...
    (+ old-deposit (default-to u0 (map-get? users-deposits {user-addr: user-addr})))
  )
)
