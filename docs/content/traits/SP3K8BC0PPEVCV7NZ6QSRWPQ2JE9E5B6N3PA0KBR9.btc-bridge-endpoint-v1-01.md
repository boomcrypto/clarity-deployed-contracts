---
title: "Trait btc-bridge-endpoint-v1-01"
draft: true
---
```
...
(define-read-only (decode-order-0-or-fail (order-script (buff 128)))
	(ok (unwrap! (from-consensus-buff? principal (unwrap-panic (slice? order-script u2 (len order-script)))) err-invalid-input)))
...
(define-read-only (create-order-1-or-fail (order { user: principal, token: (string-ascii 40), factor: uint, min-dy: uint }))
	(ok (unwrap! (to-consensus-buff? { u: (get user order), t: (get token order), f: (int-to-ascii (get factor order)), y: (int-to-ascii (get min-dy order)) }) err-invalid-input)))
(define-read-only (decode-order-1-or-fail (order-script (buff 128)))
	(let (
			(raw-order (unwrap! (from-consensus-buff? { u: principal, t: (string-ascii 40), f: (string-ascii 40), y: (string-ascii 40) } (unwrap-panic (slice? order-script u2 (len order-script)))) err-invalid-input))
			(factor (unwrap! (string-to-uint? (get f raw-order)) err-invalid-input))
			(min-dy (unwrap! (string-to-uint? (get y raw-order)) err-invalid-input)))
		(ok { user: (get u raw-order), token: (get t raw-order), factor: factor, min-dy: min-dy })))
...
(define-read-only (create-order-2-or-fail (order { user: principal, token1: (string-ascii 40), token2: (string-ascii 40), factor1: uint, factor2: uint, min-dz: uint }))
	(ok (unwrap! (to-consensus-buff? { u: (get user order), t1: (get token1 order), t2: (get token2 order), f1: (int-to-ascii (get factor1 order)), f2: (int-to-ascii (get factor2 order)), z: (int-to-ascii (get min-dz order)) }) err-invalid-input)))
(define-read-only (decode-order-2-or-fail (order-script (buff 128)))
	(let (
			(raw-order (unwrap! (from-consensus-buff? { u: principal, t1: (string-ascii 40), t2: (string-ascii 40), f1: (string-ascii 40), f2: (string-ascii 40), z: (string-ascii 40) } (unwrap-panic (slice? order-script u2 (len order-script)))) err-invalid-input))
			(factor1 (unwrap! (string-to-uint? (get f1 raw-order)) err-invalid-input))
			(factor2 (unwrap! (string-to-uint? (get f1 raw-order)) err-invalid-input))
			(min-dz (unwrap! (string-to-uint? (get z raw-order)) err-invalid-input)))
		(ok { user: (get u raw-order), token1: (get t1 raw-order), token2: (get t2 raw-order), factor1: factor1, factor2: factor2, min-dz: min-dz })))
...
(define-read-only (create-order-3-or-fail (order { user: principal, launch-id: uint }))
...
(define-read-only (decode-order-3-or-fail (order-script (buff 128)))
	(let (
			(raw-order (unwrap! (from-consensus-buff? { u: principal, l: (string-ascii 40) } (unwrap-panic (slice? order-script u2 (len order-script)))) err-invalid-input))
			(launch-id (unwrap! (string-to-uint? (get l raw-order)) err-invalid-input)))
		(ok { user: (get u raw-order), launch-id: launch-id })))
(define-read-only (validate-tx-3 (tx (buff 4096)) (output-idx uint) (order-idx uint))
	(validate-tx-3-extra (try! (validate-tx-3-base tx output-idx order-idx))))
...
(define-public (finalize-peg-in-1
	...
	(let (
		...
		(and (> fee u0) (as-contract (try! (contract-call? .token-abtc mint-fixed fee (var-get fee-address)))))
		(print { type: "peg-in", tx-id: (try! (get-txid tx)), output: output-idx, order-details: order-details, fee: fee, amount-net: amount-net })
		(if (is-ok (validate-tx-1-extra validation-data (contract-of token-trait)))
			(let (
					(swapped (as-contract (try! (contract-call? .amm-swap-pool-v1-1 swap-helper .token-abtc token-trait (get factor order-details) amount-net (some (get min-dy order-details)))))))
...
(define-public (finalize-peg-in-2
	...
	(let (
		...
		(and (> fee u0) (as-contract (try! (contract-call? .token-abtc mint-fixed fee (var-get fee-address)))))
		(print { type: "peg-in", tx-id: (try! (get-txid tx)), output: output-idx, order-details: order-details, fee: fee, amount-net: amount-net })
		(if (is-ok (validate-tx-2-extra validation-data (contract-of token1-trait) (contract-of token2-trait)))
			(let (
					(swapped (as-contract (try! (contract-call? .amm-swap-pool-v1-1 swap-helper-a .token-abtc token1-trait token2-trait (get factor1 order-details) (get factor2 order-details) amount-net (some (get min-dz order-details)))))))
...
(define-public (finalize-peg-in-3
	...
	(let (
		...
		(print { type: "peg-in", tx-id: (try! (get-txid tx)), output: output-idx, order-details: order-details, fee: fee, amount-net: amount-net })
		(if (is-ok (validate-tx-3-extra validation-data))
			(begin
				(as-contract (try! (contract-call? .alex-launchpad-v1-4 register-on-behalf (get user order-details) (get launch-id order-details) amount-net .token-abtc)))
				(ok true)
			)		
			(begin 
				(as-contract (try! (contract-call? .token-abtc transfer-fixed amount-net tx-sender (get user order-details) none)))
...
(define-public (request-peg-out-0 (amount uint) (peg-out-address (buff 128)))
	(let (
		...
		(ok true)))
(define-public (request-peg-out-1 (token-trait <sip010-trait>) (factor uint) (dx uint) (min-dy (optional uint)) (peg-out-address (buff 128)))
	(let (
			(swapped (try! (contract-call? .amm-swap-pool-v1-1 swap-helper token-trait .token-abtc factor dx min-dy))))
		(request-peg-out-0 swapped peg-out-address)))
(define-public (request-peg-out-2 (token1-trait <sip010-trait>) (token2-trait <sip010-trait>) (factor1 uint) (factor2 uint) (dx uint) (min-dz (optional uint)) (peg-out-address (buff 128)))
	(let (
...
(define-private (validate-tx-1-extra (validation-data { order-details: { user: principal, token: (string-ascii 40), factor: uint, min-dy: uint }, fee: uint, amount-net: uint }) (token principal))
	(let (
			(order-details (get order-details validation-data))
			(dy (try! (contract-call? .amm-swap-pool-v1-1 get-helper .token-abtc token (get factor order-details) (get amount-net validation-data)))))
		(asserts! (is-eq (get token order-details) (unwrap-panic (get name (unwrap-panic (principal-destruct? token))))) err-token-mismatch)
...
(define-private (validate-tx-2-extra (validation-data { order-details: { user: principal, token1: (string-ascii 40), token2: (string-ascii 40), factor1: uint, factor2: uint, min-dz: uint }, fee: uint, amount-net: uint }) (token1 principal) (token2 principal))
	(let (
			(order-details (get order-details validation-data))
			(dz (try! (contract-call? .amm-swap-pool-v1-1 get-helper-a .token-abtc token1 token2 (get factor1 order-details) (get factor2 order-details) (get amount-net validation-data)))))
		(asserts! (is-eq (get token1 order-details) (unwrap-panic (get name (unwrap-panic (principal-destruct? token1))))) err-token-mismatch)
		(asserts! (is-eq (get token2 order-details) (unwrap-panic (get name (unwrap-panic (principal-destruct? token2))))) err-token-mismatch)
...
(define-private (validate-tx-3-extra (validation-data { order-details: { user: principal, launch-id: uint }, fee: uint, amount-net: uint }))
	(let (
			(order-details (get order-details validation-data)))
		(try! (contract-call? .alex-launchpad-v1-4 validate-register (get user order-details) (get launch-id order-details) (get amount-net validation-data) .token-abtc))
		(ok validation-data)))		
