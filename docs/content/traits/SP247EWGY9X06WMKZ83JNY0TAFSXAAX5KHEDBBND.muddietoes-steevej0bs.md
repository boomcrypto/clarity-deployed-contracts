---
title: "Trait muddietoes-steevej0bs"
draft: true
---
```
;; muddietoes-steevej0bs
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token muddietoes-steevej0bs uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSKSYzaSVp14LMAQbrH67rjYdNZKaXekeZ6aiHsrUrPrw/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? muddietoes-steevej0bs next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? muddietoes-steevej0bs token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? muddietoes-steevej0bs token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? muddietoes-steevej0bs token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? muddietoes-steevej0bs id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? muddietoes-steevej0bs id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? muddietoes-steevej0bs id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mega uint u8000)
