...
(define-non-fungible-token metaraffes uint)
...
(define-data-var artist-address principal 'SPP930EBFW6DVDRZ2Y84660J2R28T0HBB8S1ST5C)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPSdWCbFXS5BYwfQaTKhV5xbK18jH4QPgfqTykYvFDjyo/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? metaraffes next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? metaraffes token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? metaraffes token-id)))
