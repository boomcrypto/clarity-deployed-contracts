---
title: "Trait purple-smoke-session-finale--zvimba"
draft: true
---
```
;; purple-smoke-session-finale--zvimba
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token purple-smoke-session-finale--zvimba uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmf3a8sKvZrPe1UjNh88Pt6wnt2Mk82v5QZW8RZY518rm6/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? purple-smoke-session-finale--zvimba next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? purple-smoke-session-finale--zvimba token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? purple-smoke-session-finale--zvimba token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? purple-smoke-session-finale--zvimba token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? purple-smoke-session-finale--zvimba id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? purple-smoke-session-finale--zvimba id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? purple-smoke-session-finale--zvimba id) (err ERR-NOT-FOUND)))
