---
title: "Trait mlbct-ustacks"
draft: true
---
```
(impl-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)

(define-fungible-token ustacks)
...
(define-constant dev-principal 'SP1ASYMXKPM76TPTRHX2XA0DSM48VMX2SRWBB3HA2)
(define-constant token-supply u21000000000000)
...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://bafybeihzzusshnqzoijxh5sqq2qvt4wiqpfn7d7lfrfzb7cfcm4564epuu.ipfs.dweb.link/")) 
...
(define-read-only (get-name) (ok "USTACKS"))
(define-read-only (get-symbol) (ok "USTx"))
(define-read-only (get-decimals) (ok u0))
(define-read-only (get-balance (user principal)) (ok (ft-get-balance ustacks user)))
(define-read-only (get-total-supply) (ok (ft-get-supply ustacks)))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? ustacks amount from to)
    )
)
...
(define-public (swap-stx-for-token (stx-amount uint) (min-tokens-out uint)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? ustacks tokens-out (as-contract tx-sender) tx-sender))
      (if (>= (/ (* (+ (var-get real-stx-reserve) (var-get virtual-stx-reserve)) token-supply) (var-get token-reserve)) market-cap-threshold)
        (begin
          (let (
            ...
            (try! (ft-transfer? ustacks burn-amount (as-contract tx-sender) burn-address))
            (try! (ft-transfer? ustacks remaining-tokens (as-contract tx-sender) liquidity-builder-address))
...
(define-public (swap-token-for-stx (token-amount uint) (min-stx-out uint) (recipient principal)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? ustacks token-amount tx-sender (as-contract tx-sender)))
...
(begin
  (try! (ft-mint? ustacks token-supply (as-contract tx-sender)))
