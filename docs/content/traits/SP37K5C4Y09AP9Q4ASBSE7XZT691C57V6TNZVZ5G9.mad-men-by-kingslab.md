---
title: "Trait mad-men-by-kingslab"
draft: true
---
```
;; mad-men-by-kingslab
...
(define-non-fungible-token mad-men-by-kingslab uint)
...
(define-data-var artist-address principal 'SP37K5C4Y09AP9Q4ASBSE7XZT691C57V6TNZVZ5G9)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQNsCNXuYoUvJCCbnw3m2KWvRNZyAC3Lk8wsomisnjDmf/json/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? mad-men-by-kingslab next-id tx-sender) next-id)
      (unwrap! (nft-transfer? mad-men-by-kingslab next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? mad-men-by-kingslab token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? mad-men-by-kingslab token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? mad-men-by-kingslab token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? mad-men-by-kingslab id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? mad-men-by-kingslab id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? mad-men-by-kingslab id) (err ERR-NOT-FOUND)))
