---
title: "Trait memegoat-staking-v1e"
draft: true
---
```
;;
;;  MEMEGOAT STAKING CONTRACT
;;
...
;; @returns (response uint)
(define-read-only (get-active-staked) 
  (ok (var-get staked-balance))
)

;; @desc get-total-rewards
;; @returns (response uint)
(define-read-only (get-remaining-rewards)
  (ok (var-get reward-balance))
)
...
(define-read-only (convert-block-time (block-time uint))
  (let
    (
      (ref-block u171572)
      (ref-burn-block u867567)
    )
    (if (> ref-block block-time)
      (- ref-burn-block (- ref-block block-time))
      (+ ref-burn-block (- block-time ref-block))
    )
  )
)
...
(define-public (set-stake-record (stake-index uint) (interest-rate uint) (duration-in-blocks uint))
  (begin
    ...
    (map-set stake-rates {stake-index: stake-index} { interest-rate: interest-rate, duration-in-blocks: duration-in-blocks, block-stamp: burn-block-height})
    (ok true)
  )
)
...
;; @desc update records data
(define-public (move-records (total-staked uint) (total-rewards uint) (reward-balance_ uint))
  (begin
    (try! (is-dao-or-extension))
    (var-set staked-total total-staked)
    (var-set total-reward total-rewards)
    (var-set reward-balance reward-balance_)
    (ok true) 
  )
)

;; @desc move-stakes: move stakes from old stake contract
(define-public (move-user-records (users (list 500 principal)))
  (begin
    (try! (is-dao-or-extension))
    (let
      (
        (active-stake-balance (var-get staked-balance))
        (active-stakes (fold move-iter users u0))
      )
      (var-set staked-balance (+ active-stake-balance active-stakes))
    )
    (ok true) 
  )
)

(define-private (move-iter (user principal) (active-stakes uint))
  (let
    (
      (stake-data (unwrap-panic (contract-call? .memegoat-staking-v1 get-user-staking-data user)))
      (has-stake (contract-call? .memegoat-staking-v1 get-user-stake-has-staked user))
    )

    (if has-stake
      (map-set has-staked { user-addr: user } true)
      (map-set has-staked { user-addr: user } false)
    )

    (map-set deposit-map 
      { user-addr: user } 
      {
        deposit-amount: (get deposit-amount stake-data), 
        deposit-block: (convert-block-time (get deposit-block stake-data)), 
        end-block: (convert-block-time (get end-block stake-data)), 
        lock-rewards: (get lock-rewards stake-data), 
        stake-index: (get stake-index stake-data), 
        paid: (get paid stake-data) 
      }
    )
    (+ active-stakes (get deposit-amount stake-data))
  )
)
    ...
    (let
      ...
      (if has-stake
        ...
        (begin
          ...
          (map-set deposit-map 
            { user-addr: user-addr } 
            {deposit-amount: amount, deposit-block: burn-block-height, end-block: (+ burn-block-height duration-in-blocks), lock-rewards: u0, stake-index: stake-index, paid: false }
          )
...
(define-private (do-withdraw-stake (user-addr principal))
  (begin 
    (let
      ...
      ;; run checks 
      (asserts! (> burn-block-height end-block) ERR-STAKE-LOCK-NOT-EXPIRED)
