...
(define-map user-tokens { user: principal } { token-ids: (list 500 uint) })
...
(define-private (add-token-to-user-list (user principal) (token-id uint))
  (let (
    ...
    (map-set user-tokens { user: user } { token-ids: (unwrap-panic (as-max-len? (append current-user-tokens token-id) u500)) })
  )
)
...
(define-public (stake (token-id uint))
  (begin
    ...
    (let (
      ;; Token info
      (token-points (contract-call? .board-tiles-data get-token-points token-id))
      ...
      (try! (contract-call? .board-tiles transfer token-id tx-sender (as-contract tx-sender)))
...
(define-public (unstake (token-id uint))
  (begin
    ...
    (let (
      ...
      (try! (as-contract (contract-call? .board-tiles transfer token-id tx-sender staker)))
  ...
  (begin
    ...
    (let (
      ...
      (if (>= pending-rewards u1)
        (begin
          (try! (contract-call? .board-main mint-ft-token .board-points pending-rewards tx-sender))
...
(define-public (increase-cumm-reward-per-stake)
  (if (>= block-height (var-get last-reward-increase-block))
    (let (
...
(begin
  (var-set last-reward-increase-block u37420)
)
