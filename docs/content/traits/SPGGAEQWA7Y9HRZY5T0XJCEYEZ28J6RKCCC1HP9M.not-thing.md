...
(define-non-fungible-token not-thing uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWyy9YTZjZt2Yjxw7GPLHY78z6C3J2cHKoR7aW6dZiT2m/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? not-thing next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? not-thing token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? not-thing token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? not-thing token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? not-thing id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? not-thing id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? not-thing id) (err ERR-NOT-FOUND)))
