---
title: "Trait neoswap-sc-1690655060564"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/8fyHT0mBO5iljCjq9zCc
;; SwapId: vynuOWmGnaYJadsWxGHb
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u2621294 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
		(unwrap! (contract-call? 'SP2N3BAG4GBF8NHRPH6AY4YYH1SP6NK5TGCY7RDFA.stacks-mfers transfer u523 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (contract-call? 'SP2N3BAG4GBF8NHRPH6AY4YYH1SP6NK5TGCY7RDFA.stacks-mfers transfer u523 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u2621294 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u2621294 tx-sender TRADER-1)))
		(unwrap-panic (as-contract (contract-call? 'SP2N3BAG4GBF8NHRPH6AY4YYH1SP6NK5TGCY7RDFA.stacks-mfers transfer u523 tx-sender TRADER-1)))
	    )
            true
        )
