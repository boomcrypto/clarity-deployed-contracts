---
title: "Trait test-emergency-recover"
draft: true
---
```
;; @contract Recover
...
(define-public (recover-usdh (address principal) (recipient principal))
  (let (
    ...
    (try! (contract-call? .test-usdh-token burn-for-protocol balance address))
    (ok (try! (contract-call? .test-usdh-token mint-for-protocol balance recipient)))
  )
)
...
(define-public (recover-susdh (address principal) (recipient principal))
  (let (
    ...
    (try! (contract-call? .test-susdh-token burn-for-protocol balance address))
    (ok (try! (contract-call? .test-susdh-token mint-for-protocol balance recipient)))
  )
)
```
