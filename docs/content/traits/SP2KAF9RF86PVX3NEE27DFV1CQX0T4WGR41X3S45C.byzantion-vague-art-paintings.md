---
title: "Trait byzantion-vague-art-paintings"
draft: true
---
```
...
(define-non-fungible-token vague-art-paintings uint)
...
(define-data-var ipfs-full-metadata (string-ascii 125) "ipfs://ipfs/bafybeiaq7wdsr6mwsvpc2liuqoler7rrtmwqu7rjbrjoeuuymj6lav4ip4/vague_art_paintings/vague_art_paintings_metadata.json")
(define-data-var ipfs-root (string-ascii 112) "ipfs://ipfs/bafybeifwoqwdhs5djtx6vopvuwfcdrqeuecayp5wzpzjylxycejnhtrhgu/vague_art_paintings/vague_art_paintings_")
(define-data-var artist-address principal 'SP225F26PGZ86QX8GN11Z5S8YS6A6V66EY0W64V51)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? vague-art-paintings next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? vague-art-paintings token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? vague-art-paintings token-id)))
