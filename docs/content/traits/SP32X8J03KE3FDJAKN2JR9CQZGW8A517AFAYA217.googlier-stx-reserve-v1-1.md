---
title: "Trait googlier-stx-reserve-v1-1"
draft: true
---
```
(impl-trait .googlier-vault-trait-v1.vault-trait)
...
(define-public (add-tokens-to-stack (name (string-ascii 256)) (token-amount uint))
  (let (
    ...
    (asserts!
      (or
        (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "hettich")))
        ...
        (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "stacker-payer")))
      )
...
(define-public (toggle-stacking (stacker-name (string-ascii 256)) (revoked-stacking bool) (ustx-collateral uint))
  (begin
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "hettich"))) (err ERR-NOT-AUTHORIZED))
...
(define-public (request-stx-to-auto-payoff (requested-ustx uint))
  (begin
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "stacker-payer"))) (err ERR-NOT-AUTHORIZED))

    (as-contract
      (stx-transfer? requested-ustx tx-sender (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "stacker-payer")))
    )
  )
)
...
(define-public (request-stx-to-stack (name (string-ascii 256)) (requested-ustx uint))
  (let (
    ...
    (as-contract
      (stx-transfer? requested-ustx tx-sender (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name name)))
    )
  )
)
...
(define-public (set-next-stacker-name (stacker-name (string-ascii 256)))
  (begin
    (if
      (or
        (is-eq tx-sender (contract-call? .googlier-dao get-dao-owner))
        (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "stacker")))
        (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "stacker-2")))
        (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "stacker-3")))
        (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "stacker-4")))
...
(define-public (calculate-usd-count
...
(define-public (mint
  ...
  (begin
    ...
    (let ((max-new-debt (- (unwrap-panic (calculate-usd-count token-string ustx-amount collateralization-ratio oracle)) current-debt)))
      (if (>= max-new-debt extra-debt)
        (match (print (as-contract (contract-call? .googlier-dao mint-token .usd-token extra-debt vault-owner)))
          success (ok true)
