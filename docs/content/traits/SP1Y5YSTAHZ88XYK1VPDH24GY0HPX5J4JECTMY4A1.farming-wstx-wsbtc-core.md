---
title: "Trait farming-wstx-wsbtc-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-wsbtc)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-wsbtc transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-wsbtc get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-wsbtc transfer amt protocol user none)))
