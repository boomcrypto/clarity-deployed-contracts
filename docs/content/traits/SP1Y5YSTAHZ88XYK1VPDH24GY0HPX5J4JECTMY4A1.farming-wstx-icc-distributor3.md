---
title: "Trait farming-wstx-icc-distributor3"
draft: true
---
```
...
(define-public
  ...
  (let ((epoch (contract-call? .farming-wstx-icc-core current-epoch))
...
(define-read-only
  ...
  (let ((share   (try! (contract-call? .farming-wstx-icc-core get-share-at user epoch)))
...
(define-read-only (mkepochs (start-epoch uint))
  (let ((current (contract-call? .farming-wstx-icc-core current-epoch))
