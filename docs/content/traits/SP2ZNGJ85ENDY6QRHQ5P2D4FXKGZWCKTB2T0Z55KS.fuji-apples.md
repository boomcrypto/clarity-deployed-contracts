...
;; Description:
;; Fuji apples grow in the apple orchard. Farmers specialize in collecting them.
;; Unwrap Link:
...
(define-constant unlock-block u157400)
(define-constant token-a-ratio u1)
(define-constant token-b-ratio u1)
(define-constant index-token-ratio u2000000)

(define-fungible-token index-token)

(define-data-var token-name (string-ascii 32) "Fuji Apples")
(define-data-var token-symbol (string-ascii 10) "FUJI")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.fuji-apples.json"))
...
(define-public (add-liquidity (amount uint))
    (let
        ...
        (try! (is-unlocked))
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.liquid-staked-charisma transfer amount-a tx-sender contract none))
...
(define-public (remove-liquidity (amount uint))
    (let
        ...
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.liquid-staked-charisma transfer amount-a contract sender none)))
