---
title: "Trait bitgear-genesis-fund"
draft: true
---
```
;; Title: Bitgear Genesis Fund
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token BGF)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.bitgear-genesis-fund"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? BGF amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "Bitgear Genesis Fund"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance BGF who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply BGF)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? BGF (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? BGF (get dk delta) sender))
...
(begin
    ...
    (try! (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token transfer u287000000 tx-sender CONTRACT none)))
```
