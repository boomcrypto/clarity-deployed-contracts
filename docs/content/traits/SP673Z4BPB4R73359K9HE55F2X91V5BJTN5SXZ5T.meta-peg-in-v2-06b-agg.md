---
title: "Trait meta-peg-in-v2-06b-agg"
draft: true
---
```
...
(define-read-only (validate-tx-agg (commit-tx { tx: (buff 32768), output-idx: uint, fee-idx: (optional uint) }) (reveal-tx { tx: (buff 32768), order-idx: uint }))
    ...
    (asserts! (is-eq (contract-call? .clarity-bitcoin-v1-08 get-segwit-txid (get tx commit-tx)) (get commit-txid reveal-tx-data)) err-commit-tx-mismatch)
...
(define-private (decode-from-reveal-tx-or-fail (tx (buff 32768)) (order-idx uint))
  (let (
      (parsed-tx (unwrap! (contract-call? .clarity-bitcoin-v1-08 parse-wtx tx) err-invalid-tx))
...
(define-private (extract-tx-ins-outs (tx (buff 32768)))
  (if (try! (contract-call? .clarity-bitcoin-v1-08 is-segwit-tx tx))
    (let (
        (parsed-tx (unwrap! (contract-call? .clarity-bitcoin-v1-08 parse-wtx tx) err-invalid-tx)))
      (ok { ins: (get ins parsed-tx), outs: (get outs parsed-tx) }))
    (let (
        (parsed-tx (unwrap! (contract-call? .clarity-bitcoin-v1-08 parse-tx tx) err-invalid-tx)))
      (ok { ins: (get ins parsed-tx), outs: (get outs parsed-tx) }))))

(define-private (get-txid (tx (buff 32768)))
	(if (try! (contract-call? .clarity-bitcoin-v1-08 is-segwit-tx tx))
		(ok (contract-call? .clarity-bitcoin-v1-08 get-segwit-txid tx))
		(ok (contract-call? .clarity-bitcoin-v1-08 get-txid tx))))
...
(define-private (verify-mined (tx (buff 32768)) (block { header: (buff 80), height: uint }) (proof { tx-index: uint, hashes: (list 14 (buff 32)), tree-depth: uint }))
	(if (is-eq chain-id u1)
		(let (
				(response (if (try! (contract-call? .clarity-bitcoin-v1-08 is-segwit-tx tx)) (contract-call? .clarity-bitcoin-v1-08 was-segwit-tx-mined? block tx proof) (contract-call? .clarity-bitcoin-v1-08 was-tx-mined? block tx proof))))
