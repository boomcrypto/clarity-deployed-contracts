...
(define-private
  ...
  (let ((p (contract-call? .curve-registry_v1_1_0 get-pool id)))
    (match p
     pool (do-insert
           (if (is-usdh (get contract pool)) USDH CURVE)
           (get contract pool) id (get token0 pool) (get token1 pool))
     (ok true)) ))


(define-read-only
 (is-usdh (contract principal))
 (is-eq contract .curve-pool-v1_1_0-0001))
...
(define-private
 ...
 (let ((p (contract-call? .univ2-registry_v1_0_0 get-pool id)))
   (match p
    pool (do-insert UNIV2V2 (get contract pool) id (get token0 pool) (get token1 pool))
    (ok true)) ))
