---
title: "Trait memobots-mint-helper"
draft: true
---
```
...
(define-constant ENERGY_DISCOUNT_RATE u100000) ;; 100,000 energy = 1 STX discount
(define-constant MIN_STX_PRICE u1) ;; Minimum price of 1 STX

(define-public (mint (land-id uint) (nfts-to-mint uint))
    (let
        (
            (tapped-out (unwrap-panic (contract-call? .edk-v0 tap land-id .land-helper-v3 (some (* ENERGY_DISCOUNT_RATE nfts-to-mint)))))
            ...
            (stx-cost-per-nft (max (- STX_PER_MINT stx-discount-per-nft) MIN_STX_PRICE))
        )
