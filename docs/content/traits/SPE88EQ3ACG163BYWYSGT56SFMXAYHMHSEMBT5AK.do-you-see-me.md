...
(define-non-fungible-token do-you-see-me uint)
...
(define-data-var total-price uint u125000000)
(define-data-var artist-address principal 'SPE88EQ3ACG163BYWYSGT56SFMXAYHMHSEMBT5AK)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmdg8iX9sea7UoLsNKakdcxUir8gvRYQVMXqpH5yp3k192/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? do-you-see-me next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? do-you-see-me token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? do-you-see-me token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? do-you-see-me token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? do-you-see-me id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? do-you-see-me id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? do-you-see-me id) (err ERR-NOT-FOUND)))
