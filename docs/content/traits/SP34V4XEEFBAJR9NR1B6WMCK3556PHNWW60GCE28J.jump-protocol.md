...
;; DESCRIPTION: The Jump Protocol is an early attempt at bridging NFTs from various blockchains to Stacks.
...
(define-map nfts uint
  {
    ...
    originalUri: (string-ascii 2048),
    originalAddress: (string-ascii 2048),
    minter: principal
  }
)
...
(define-public (mint-nft (name (string-ascii 2048)) (description (string-ascii 2048)) (chain (string-ascii 2048)) (uri (string-ascii 2048)) (originalUri (string-ascii 2048)) (originalAddress (string-ascii 2048)) (minter principal))
  (begin
    ...
    (let
      ...
      (map-insert nfts id {id: id, name: name, description: description, chain: chain, uri: uri, originalUri: originalUri, originalAddress: originalAddress, minter: minter})
      (map-set token-count minter (+ (get-balance minter) u1))
      (match (nft-mint? jump-protocol id minter)
        success (ok true)
        error (err ERR-MINT)
      )
    )
...
(define-public (update-nft (id uint) (name (string-ascii 2048)) (description (string-ascii 2048)) (chain (string-ascii 2048)) (uri (string-ascii 2048)) (originalUri (string-ascii 2048)) (originalAddress (string-ascii 2048)) (minter principal))
  (begin
    (asserts! (is-eq (var-get contract-owner) tx-sender) (err ERR-NOT-AUTHORIZED))
    (map-set nfts id {id: id, name: name, description: description, chain: chain, uri: uri, originalUri: originalUri, originalAddress: originalAddress, minter: minter})
    (ok true)
  )
)
...
(define-public (buy-in-ustx (id uint))
  (let ((owner (unwrap! (nft-get-owner? jump-protocol id) (err ERR-NOT-FOUND)))
...
(define-private (is-owner (id uint) (user principal))
  (is-eq user (unwrap! (nft-get-owner? jump-protocol id) false)))
