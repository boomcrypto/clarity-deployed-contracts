...
;; Constants
(define-constant MAXSUPPLY u90000000000)

(define-fungible-token SAFEMOON)
;; Initialize function to mint max supply to owner

(begin
  (try! (ft-mint? SAFEMOON MAXSUPPLY tx-sender)) 
)
;; Define the owner address
(define-data-var owner-address principal tx-sender) 
(define-data-var treasury-address principal 'SP1EY26NDX3FFDS868DVVYR018RK4462BM8BNTWE3)
(define-data-var dev-wallet-address principal 'SPHNNSBV7Q29CJ0KCEGVGT5N3G9S7Z99KGGSMFW2)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  ...
  ;; If sender is whitelisted or tax is off, perform transfer without tax
    (if (or is-whitelisted (var-get tax_off)) 
      (match (ft-transfer? SAFEMOON amount sender recipient)
        success (ok success)
  ...
  ;; If sender is not whitelisted and tax is on, apply tax
      (let ((tax-amount (/ (* amount (var-get total-tax)) u100))
        ...
        (match (ft-transfer? SAFEMOON transfer-amount sender recipient)
          success
...
(define-private (distribute-taxes (tax-amount uint) (sender principal))
  (let (
    ...
    (match (ft-transfer? SAFEMOON treasury-tax sender (var-get treasury-address))
      treasury-success
        (match (ft-burn? SAFEMOON burn-tax sender ) 
          burn-success
            (match (ft-transfer? SAFEMOON dev-tax sender (var-get dev-wallet-address))
              dev-success (ok true)
...
(define-read-only (get-balance (user principal))
    (ok (ft-get-balance SAFEMOON user)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply SAFEMOON)))
...
;; DEFINE METADATA
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://gaia.hiro.so/hub/1PAXThAjEmVAuGHHVCcHmsqtiwEpYrD3QQ/safemoon.json"))
(define-data-var token-name (string-ascii 32) "SAFEMOON DEMO")
(define-data-var token-symbol (string-ascii 32) "SAFEMOON")
