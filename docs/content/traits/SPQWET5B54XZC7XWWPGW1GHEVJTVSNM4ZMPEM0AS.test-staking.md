...
(define-read-only (get-usdh-per-susdh) 
  (let (
    (total-usdh-staked (unwrap-panic (contract-call? .test-usdh-token get-balance .test-staking-reserve)))
...
(define-public (stake (amount uint))
  (let (
    ...
    (try! (contract-call? .test-usdh-token transfer amount tx-sender .test-staking-reserve none))
    (ok (try! (contract-call? .test-susdh-token mint-for-protocol amount-susdh tx-sender)))
  )
)
...
(define-public (unstake (amount uint))
  (let (
    ...
    (try! (contract-call? .test-staking-silo create-claim amount-usdh tx-sender))
    (try! (contract-call? .test-staking-reserve transfer amount-usdh))
...
(define-public (init-usdh-per-susdh (ratio uint)) 
  (let (
    ...
    (try! (contract-call? .test-usdh-token mint-for-protocol usdh-base .test-staking-reserve))
    (try! (contract-call? .test-susdh-token mint-for-protocol susdh-amount .test-staking))
