...
(define-non-fungible-token tater-bits uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbfZ2qvRqvUinqkSeZa3y1xrz8M8EQXrSUpXdzieW8pqM/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? tater-bits next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? tater-bits token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? tater-bits token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tater-bits token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? tater-bits id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? tater-bits id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? tater-bits id) (err ERR-NOT-FOUND)))
