...
(define-non-fungible-token cyborg uint)
...
(define-data-var set-limit uint u10)
(define-data-var set-length uint u10)
(define-data-var artist-address principal 'SP2TKGQ8V47CKXN3P2AZBT0K93FMD69KJTPW4B54K)
(define-data-var minting-enabled bool true)
(define-map sets uint { ipfs-full-metadata: (string-ascii 99), ipfs-root: (string-ascii 86) })
  ...
  (let (
      ...
      (asserts! (< count (var-get set-limit)) (err ERR-SET-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? cyborg next-id new-owner))
...
(define-public (add-set
  (ipfs-full-metadata (string-ascii 99))
  (ipfs-root (string-ascii 86))
)
  (begin
    (asserts!
      (or
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? cyborg token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? cyborg token-id)))
