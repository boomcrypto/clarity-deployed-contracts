---
title: "Trait Wolf-Pack-Academy-Mint"
draft: true
---
```
...
(define-non-fungible-token Wolf-Pack-Academy-Mint uint)
...
(define-data-var ipfs-full-metadata (string-ascii 500) "ipfs://ipfs/QmRaR35ksPhUddCfkzbS1qQnKAKFKrATyWgTjjMRtzWgfq")
(define-data-var ipfs-root (string-ascii 200) "ipfs://ipfs/QmR6qVjY5wTjtXRZSaDUuM5kbCqzn4max8TouwFrvrGtwa/wolf_pack_")
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint?  Wolf-Pack-Academy-Mint next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer?  Wolf-Pack-Academy-Mint token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner?  Wolf-Pack-Academy-Mint token-id)))
