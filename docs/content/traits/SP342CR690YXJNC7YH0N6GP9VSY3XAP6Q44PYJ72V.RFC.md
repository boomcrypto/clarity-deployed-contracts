...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://memefun.s3.ap-northeast-1.amazonaws.com/metadata/9qyag7eacw0npqslqqocn3dgrgxh0vwm.json?"))
(define-data-var is-minted bool false)


(define-fungible-token RFC token-max-supply)
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance RFC who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply RFC)))
...
(define-public (mint (amount uint) (recipient principal)) 
    (begin 
        ...
        (try! (ft-mint? RFC amount recipient))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34)))) 
    (begin
        ...
        (try! (ft-transfer? RFC amount sender recipient))
