---
title: "Trait neoswap-sc-1654254973605"
draft: true
---
```
...
(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u20970000 tx-sender agent-1)))
	)
	...
	(unwrap-panic (as-contract (contract-call? 'SP1CSHTKVHMMQJ7PRQRFYW6SB4QAW6SR3XY2F81PA.stxplates transfer u362 tx-sender agent-3)))
	(unwrap-panic (begin
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u2300000 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u7680000 tx-sender agent-3)))
	)
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SP1CSHTKVHMMQJ7PRQRFYW6SB4QAW6SR3XY2F81PA.stxplates transfer u362 tx-sender agent-4)))
	(var-set agent-4-status false))
	true)
	(if (is-eq (var-get agent-5-status) true)
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u3290000 tx-sender agent-5)))
	)
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u7680000 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
		...
		(asserts! (is-ok (stx-transfer? u3290000 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
