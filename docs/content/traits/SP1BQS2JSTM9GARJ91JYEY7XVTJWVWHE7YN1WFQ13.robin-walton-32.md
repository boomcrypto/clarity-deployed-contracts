...
(define-private (lIrZANWJdVyFpL
    ...
    (let
        (
            ...
            (lgOKJUDXNTYsaE (unwrap! (as-contract (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.swap-helper-v1-03 swap-helper gSdwYJmyTzoWDD WwVyVrEOHQxVXy (gqyGnZJgEuDTGv dx compare-token-decimals alex-token-decimal) none)) (err ERR-SWAP-ALEX-FAILED)))
            (lgOKJUDXNTYsaE-converted (gqyGnZJgEuDTGv lgOKJUDXNTYsaE alex-token-decimal compare-token-decimals))
        )
...
(define-public (jCNlDTWjRAKdEN
    ...
    (let
        ...
        (let
            (
                (zywleRsXXeoJku (stx-get-balance (as-contract tx-sender)))
                ...
                (KFdiwjTaoFMCUG (stx-get-balance (as-contract tx-sender)))
            )
            (try! (as-contract (stx-transfer? nmoFJdZDqlSrMz tx-sender save-tx-sender)))
            (print { message: "swap", zywleRsXXeoJku: zywleRsXXeoJku, KFdiwjTaoFMCUG: KFdiwjTaoFMCUG, dx: dx, lgOKJUDXNTYsaE: lgOKJUDXNTYsaE, nmoFJdZDqlSrMz: nmoFJdZDqlSrMz })
            (asserts! (< zywleRsXXeoJku KFdiwjTaoFMCUG) (err ERR-BALANCE-LOWER))
            (ok nmoFJdZDqlSrMz)
        )
    )
)
