---
title: "Trait mint-test-v26"
draft: true
---
```
(impl-trait 'SP3XSYPNX5W7F9SFT5K9T95XCN725JH9WJYF90CSG.nft-trait-main.nft-trait)

(define-non-fungible-token test-token-name-v26 uint)

;; Constants
(define-constant DEPLOYER tx-sender)
(define-constant USER-MINT-LIMIT u5)
(define-constant WALLET_1 'SP3XSYPNX5W7F9SFT5K9T95XCN725JH9WJYF90CSG)
...
(define-data-var special-mint-limit uint u36)
(define-data-var last-id uint u1)
(define-data-var special-last-id uint u1)
(define-data-var stsw-price uint u720000000)
...
(define-private (mint-many-iter (ignore bool) (next-id uint)) 
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? test-token-name-v26 next-id tx-sender) next-id)
      (+ next-id u1) 
    )
    next-id)) 
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? test-token-name-v26 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? test-token-name-v26 id sender recipient)
    success
      (let (
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? test-token-name-v26 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? test-token-name-v26 id) (err ERR-NOT-FOUND)))
...
(define-read-only (get-stsw-price)
  (ok (var-get stsw-price)))

(define-public (claim-special)
  (mint-special (list true))) 

(define-public (claim-special-five)
  (mint-special (list true true true true true)))

(define-private (mint-special (orders (list 5 bool)))
  (begin
    (asserts! (is-eq tx-sender DEPLOYER) (err ERR-NOT-AUTHORIZED))
    (mint-many-special orders)
  )
)

(define-private (mint-many-special (orders (list 5 bool ))) 
  (let ((last-nft-id (var-get special-last-id)) 
      (enabled (asserts! (<= last-nft-id (var-get special-mint-limit)) (err ERR-NO-MORE-NFTS)))
      (id-reached (fold mint-special-many-iter orders last-nft-id)) 
      (current-balance (get-balance tx-sender)) 
    )
    (begin (var-set special-last-id id-reached)
      (map-set users-tokens tx-sender (+ current-balance (- id-reached last-nft-id)))
    )
    (ok id-reached)))

(define-private (mint-special-many-iter (ignore bool) (next-id uint)) 
  (if (<= next-id (var-get special-mint-limit))
    (begin
      (unwrap! (nft-mint? test-token-name-v26 next-id tx-sender) next-id)
      (+ next-id u1) 
    )
...
(define-read-only (get-special-last-token-id)
  (ok (- (var-get special-last-id) u1)))
```
