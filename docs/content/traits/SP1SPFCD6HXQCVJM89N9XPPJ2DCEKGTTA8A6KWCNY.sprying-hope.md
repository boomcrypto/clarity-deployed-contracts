...
(define-non-fungible-token sprying-hope uint)
...
(define-data-var artist-address principal 'SP1SPFCD6HXQCVJM89N9XPPJ2DCEKGTTA8A6KWCNY)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmemg5Z6EJ4PdRDxg8FZtmsR3LmgAVr3bvkugZxqQSKZZG/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? sprying-hope next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? sprying-hope token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? sprying-hope token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sprying-hope token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sprying-hope id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sprying-hope id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sprying-hope id) (err ERR-NOT-FOUND)))
