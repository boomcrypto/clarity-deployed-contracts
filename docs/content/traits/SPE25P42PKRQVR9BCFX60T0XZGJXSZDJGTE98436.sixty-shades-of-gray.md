---
title: "Trait sixty-shades-of-gray"
draft: true
---
```
...
(define-non-fungible-token sixty-shades-of-gray uint)
...
(define-data-var artist-address principal 'SPE25P42PKRQVR9BCFX60T0XZGJXSZDJGTE98436)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmR3hKWNNm3MFhPQ8EGGvQdKUR94bu6Hq869L6PNR22CDN/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? sixty-shades-of-gray next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? sixty-shades-of-gray token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? sixty-shades-of-gray token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sixty-shades-of-gray token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sixty-shades-of-gray id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sixty-shades-of-gray id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sixty-shades-of-gray id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-usda uint u13000000)
