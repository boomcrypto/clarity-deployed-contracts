...
(define-non-fungible-token public-004 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWaWVnX64Lt1Xf4fZb3GACXjwsQTrNvaHY1WEbuFQESNJ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? public-004 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? public-004 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? public-004 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? public-004 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? public-004 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? public-004 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? public-004 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u19323)
...
;; Alt Minting Default
(define-data-var total-price-mia uint u546600000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u969800000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u15000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u68000000)
