...
(define-non-fungible-token storm uint)
...
(define-data-var cost-per-mint uint u20000000000)
(define-data-var cost-reducer uint u500000000)
(define-data-var cost-floor uint u500000000)
(define-data-var commission uint u800)
(define-data-var payout uint u0)
(define-data-var first-block uint block-height)
(define-data-var block-difference uint u12)
(define-data-var ipfs-full-metadata (string-ascii 97) "ipfs://ipfs/bafybeiaqfegigln4x3ksl4gs6gzotvocen2bhcvljhevnbw7vn4whj3h5m/storm/storm_metadata.json")
(define-data-var ipfs-root (string-ascii 97) "ipfs://ipfs/bafybeiaqfegigln4x3ksl4gs6gzotvocen2bhcvljhevnbw7vn4whj3h5m/storm/storm_metadata.json")
  ...
  (let (
      ...
      (if (< block-dif u468)
        (match (stx-transfer? new-cost tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? storm next-id new-owner))
            (var-set last-id next-id)
            (var-set payout (- new-cost (/ (* new-cost (var-get commission)) u10000)))
        ...
        (match (stx-transfer? new-cost tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? storm next-id new-owner))
            (var-set last-id next-id)
            (var-set payout (- (var-get cost-floor) (/ (* (var-get cost-floor) (var-get commission)) u10000)))
            (try! (as-contract (stx-transfer? (var-get payout) (as-contract tx-sender) (var-get artist-address))))
            (ok next-id)
          ) 
          error (err error)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? storm token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? storm token-id)))
...
(define-read-only (get-commission)
  (let (
    ...
    (if (< block-dif u468)
  (ok (/ (* new-cost (var-get commission)) u10000))
  (ok (/ (* (var-get cost-floor) (var-get commission)) u10000))
    )
  )
  )
