---
title: "Trait farming-viki-ten-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .viki-ten)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .viki-ten transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .viki-ten get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .viki-ten transfer amt protocol user none)))
