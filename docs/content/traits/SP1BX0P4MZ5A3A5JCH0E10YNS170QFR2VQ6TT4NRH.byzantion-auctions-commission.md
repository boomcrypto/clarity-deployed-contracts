---
title: "Trait byzantion-auctions-commission"
draft: true
---
```
...
(define-public (pay (id uint) (price uint))
    (let (
        (artist (unwrap-panic (contract-call? .byzantion-auctions get-artist id)))
        (royalty (unwrap-panic (contract-call? .byzantion-auctions get-royalty id)))
    )
        (if (> (var-get commission) u0)
            (begin  
                (try! (stx-transfer? (/ (* price (var-get commission)) u10000) tx-sender (var-get commission-address)))
                (try! (stx-transfer? (/ (* price royalty) u10000) tx-sender artist))
                (ok true)
            )
            (ok true)
        )
