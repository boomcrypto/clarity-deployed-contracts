---
title: "Trait rainbow-pixel-boys"
draft: true
---
```
;; rainbow-pixel-boys
...
(define-non-fungible-token rainbow-pixel-boys uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWSx5DeBqS4GueQ5QNSKGeR3Wd99N5wcbQZtWgv2m6cJh/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? rainbow-pixel-boys next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? rainbow-pixel-boys token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? rainbow-pixel-boys token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? rainbow-pixel-boys token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rainbow-pixel-boys id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rainbow-pixel-boys id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rainbow-pixel-boys id) (err ERR-NOT-FOUND)))
