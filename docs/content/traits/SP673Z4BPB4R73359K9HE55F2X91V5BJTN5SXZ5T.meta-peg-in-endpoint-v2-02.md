---
title: "Trait meta-peg-in-endpoint-v2-02"
draft: true
---
```
...
(define-public (finalize-peg-in-cross-on-index
  ...
  (fee-idx (optional uint)) (token-trait <ft-trait>) (token-out-trait <ft-trait>))
  (begin
    (try! (index-tx tx block proof signature-packs))
    (finalize-peg-in-cross { tx: (get bitcoin-tx tx), output-idx: (get output tx), fee-idx: fee-idx } reveal-tx token-trait token-out-trait)))

(define-public (finalize-peg-in-cross (commit-tx { tx: (buff 32768), output-idx: uint, fee-idx: (optional uint) }) (reveal-tx { tx: (buff 32768), order-idx: uint }) (token-trait <ft-trait>) (token-out-trait <ft-trait>))
  (let (
    ...
    (and (not (get no-burn token-details)) (> amt-net u0) (try! (contract-call? token-trait mint-fixed amt-net tx-sender)))
...
(define-private (validate-drop-common
  ...
  (match fee-idx some-value
    (let (
      ...
      (asserts! (>= (get value fee-output) (get-peg-in-fee)) err-invalid-amount)  
		  (asserts! (contract-call? .btc-peg-in-endpoint-v2-03 is-peg-in-address-approved (get scriptPubKey fee-output)) err-peg-in-address-not-found)
      (asserts! (not (contract-call? .btc-peg-in-endpoint-v2-03 get-peg-in-sent-or-default tx some-value)) err-already-sent)    
