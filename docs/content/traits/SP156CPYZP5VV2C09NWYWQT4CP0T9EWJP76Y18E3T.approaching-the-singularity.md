---
title: "Trait approaching-the-singularity"
draft: true
---
```
...
(define-non-fungible-token approaching-the-singularity uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmW4fsLDGjFH55vF5HHhnZQMk5RQCmKHDZFoWbZ3pLT8z9/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? approaching-the-singularity next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? approaching-the-singularity token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? approaching-the-singularity token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? approaching-the-singularity token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? approaching-the-singularity id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? approaching-the-singularity id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? approaching-the-singularity id) (err ERR-NOT-FOUND)))
