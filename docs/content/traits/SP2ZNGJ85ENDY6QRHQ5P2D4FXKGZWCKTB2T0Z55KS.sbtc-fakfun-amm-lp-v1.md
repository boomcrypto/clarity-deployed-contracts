---
title: "Trait sbtc-fakfun-amm-lp-v1"
draft: true
---
```

;; Title: sBTC-FAKFUN Liquidity Pool
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token sBTC-FAKFUN-LP)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? sBTC-FAKFUN-LP amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "sBTC-FAKFUN Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance sBTC-FAKFUN-LP who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply sBTC-FAKFUN-LP)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-faktory transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-faktory transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-faktory transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? sBTC-FAKFUN-LP (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? sBTC-FAKFUN-LP (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-faktory get-balance CONTRACT))
    })
...
(define-read-only (get-liquidity-quote (amount uint))
    (let (
        (k (ft-get-supply sBTC-FAKFUN-LP))
        (reserves (get-reserves)))
        {
...
(define-read-only (get-reserves-quote)
    (let (
        (reserves (get-reserves))
        (supply (ft-get-supply sBTC-FAKFUN-LP)))
        {
          dx: (get a reserves),
          dy: (get b reserves),
          dk: supply
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://charisma-metadata.vercel.app/api/v1/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.sbtc-fakfun-amm-lp-v1"))
    ;; Add initial balanced liquidity (handles both token transfers at 1:1)
    (try! (add-liquidity u20610182))
    
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.fakfun-faktory transfer u12794791550342032 tx-sender CONTRACT none))
)
```
