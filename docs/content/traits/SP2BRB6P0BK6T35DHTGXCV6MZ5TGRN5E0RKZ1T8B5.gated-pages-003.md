...
;; Defines Token URI 
(define-data-var token-uri (string-ascii 256) "ipfs://")
...
;; Read-only function to get complete token URI
(define-read-only (get-metadata-uri (token-id uint))
      (let 
        (
            (token-data (map-get? token-metadata token-id))
            (base-uri (var-get token-uri-base))  ;; Changed from token-uri to token-uri-base
        )
        (match token-data
            data (some (concat base-uri (get uri-suffix data)))
            none
        )
    )
)

;; Data variable for token URI with metadata handling
(define-data-var token-uri-base (string-ascii 256) "ipfs://")
...
(define-public (set-token-metadata (token-id uint) (uri-suffix (string-ascii 256)))
    (let 
        (
            (current-version (default-to u0 (get version (map-get? token-metadata token-id))))
        )
        (begin
            ;; Verify the token exists
            (asserts! (is-some (nft-get-owner? gated-page token-id)) ERR_INVALID_TOKEN)
            
            ;; Only token owner can set metadata
            (asserts! (or 
                (is-eq (some tx-sender) (nft-get-owner? gated-page token-id))
            ) ERR_UNAUTHORIZED)
              
            ;; Update the metadata map with just the suffix
...
;; Contract owner function to update base URI
(define-public (update-base-uri (new-base-uri (string-ascii 256)))
    (begin
        (asserts! (is-eq tx-sender deployer) ERR_UNAUTHORIZED)
        (var-set token-uri-base new-base-uri)
        (print {event: "base-uri-updated", new-uri: new-base-uri})
        (ok true)
    )
)
...
;; Add a new page
(define-public (create-page (title (string-ascii 64)) (description (string-ascii 256))) 
    (let
        (
...
(define-public (update-page (id uint) (title (string-ascii 64)) (description (string-ascii 256)) (active bool))
    (begin
        ...
        (match (map-get? pages-by-id { id: id })
            ...
            ;; If title is different from current, check it's not already used
                ...
                ;; Emit event
                (print {event: "page successfully updated", owner: tx-sender, active: active, title: title, description: description})
...
;; Contract owner functions
(define-public (update-uri (new-uri (string-ascii 246)))
    (begin
        (asserts! (is-eq tx-sender deployer) ERR_UNAUTHORIZED)
        (var-set token-uri new-uri)
        (print {event: "uri-updated", new-uri: new-uri})
        (ok true)
    )
)
...
(define-public (airdrop-pages 
    ...
    (begin
        ...
        (map airdrop-single-page recipients titles descriptions)
