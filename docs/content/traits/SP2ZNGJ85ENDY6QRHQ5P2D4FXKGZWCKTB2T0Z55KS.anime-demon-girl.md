---
title: "Trait anime-demon-girl"
draft: true
---
```
;; Title: Anime Demon Girl
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token THICC)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.anime-demon-girl"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? THICC amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance THICC who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply THICC)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? THICC (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? THICC (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve get-balance CONTRACT))
    })
...
(begin
    ...
    (try! (contract-call? 'SP739VRRCMXY223XPR28BWEBTJMA0B27DY8GTKCH.gyatt-bonding-curve transfer u999999990000 tx-sender CONTRACT none)))
```
