---
title: "Trait pox3-info-helper-v2"
draft: true
---
```
...
(define-read-only (pox-info-for-cycle (cycle uint))
  (let ((block-in-cycle (unwrap! (map-get? blocks-in-cycle cycle) err-cycle-data-missing)))
    (at-block (unwrap! (get-block-info? id-header-hash block-in-cycle) err-invalid-block) 
      (contract-call? 'SP000000000000000000002Q6VF78.pox-3 get-pox-info))))

(define-read-only (total-ustx-stacked-for-cycle (cycle uint))
  (let ((block-in-cycle (unwrap! (map-get? blocks-in-cycle cycle) err-cycle-data-missing)))
    (at-block (unwrap! (get-block-info? id-header-hash block-in-cycle) err-invalid-block) 
        (ok (contract-call? 'SP000000000000000000002Q6VF78.pox-3 get-total-ustx-stacked cycle)))))
