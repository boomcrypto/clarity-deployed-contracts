...
(define-non-fungible-token destiny-ceb uint)
...
(define-data-var artist-address principal 'SP24PD1MQ3SAFDN1F2MYX5AAP7T7564ETYCWVQ9CR)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRk5njdght5f2E5WoKPXEfuPnnWDu4AsaLZ3ztRXva38L/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? destiny-ceb next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? destiny-ceb token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? destiny-ceb token-id)))
