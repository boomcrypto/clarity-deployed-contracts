---
title: "Trait token-wcatstacks"
draft: true
---
```
(impl-trait 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.trait-sip-010.sip-010-trait)

(define-fungible-token wcatstacks)

(define-data-var token-name (string-ascii 32) "catstacks Wrapper")
(define-data-var token-symbol (string-ascii 32) "wcatstacks")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wcatstacks.json"))
...
(define-read-only (get-base-token)
  'SP3KRBFBCSM7SDA09N57BXRFEDGSA7PC6A1K4FVA9.catstacks)

(define-read-only (get-base-decimals)
  (contract-call? 'SP3KRBFBCSM7SDA09N57BXRFEDGSA7PC6A1K4FVA9.catstacks get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP3KRBFBCSM7SDA09N57BXRFEDGSA7PC6A1K4FVA9.catstacks get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP3KRBFBCSM7SDA09N57BXRFEDGSA7PC6A1K4FVA9.catstacks transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
