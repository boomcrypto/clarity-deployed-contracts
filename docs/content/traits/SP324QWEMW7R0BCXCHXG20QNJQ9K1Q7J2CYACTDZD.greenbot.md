...
(define-non-fungible-token greenbot uint)
...
;; Internal variables
(define-data-var mint-limit uint u114)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmbv3MfNXUrL27gekResYHv5qjcgTzAmQeusW3tWMyF23B/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? greenbot next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? greenbot token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? greenbot token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? greenbot token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? greenbot id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? greenbot id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? greenbot id) (err ERR-NOT-FOUND)))
