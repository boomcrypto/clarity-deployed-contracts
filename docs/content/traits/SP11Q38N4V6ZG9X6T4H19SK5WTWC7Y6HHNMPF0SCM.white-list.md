...
(define-non-fungible-token white-list uint)
...
(define-data-var total-price uint u17380000)
(define-data-var artist-address principal 'SP11Q38N4V6ZG9X6T4H19SK5WTWC7Y6HHNMPF0SCM)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdkkKWDUy4zQeC2hTphyiGo9QhLMP7hoWFnMZEd2GbtYc/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? white-list next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? white-list token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? white-list token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? white-list token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? white-list id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? white-list id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? white-list id) (err ERR-NOT-FOUND)))
