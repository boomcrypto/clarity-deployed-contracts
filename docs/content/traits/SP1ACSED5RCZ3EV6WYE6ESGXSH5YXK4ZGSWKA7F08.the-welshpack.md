...
(define-non-fungible-token the-welshpack uint)
...
(define-data-var artist-address principal 'SP3MF31YZ7WFZ2PB3HWZ8DA50Y0AD1XPN5JEE3BXD)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUy5XNdkwBoKFg2Uhoqbnn1jfhUEiAoxfXbP4xewWFxYY/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-welshpack next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-welshpack token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-welshpack token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-welshpack token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-welshpack id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-welshpack id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-welshpack id) (err ERR-NOT-FOUND)))
