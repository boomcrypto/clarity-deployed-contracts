...
(define-non-fungible-token hedgehog uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVpp1WMZguw51MweQ6KfnKsFq44zs65sycHyRWzkCiVLc/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? hedgehog next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? hedgehog token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? hedgehog token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? hedgehog token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? hedgehog id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? hedgehog id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? hedgehog id) (err ERR-NOT-FOUND)))
