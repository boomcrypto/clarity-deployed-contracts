---
title: "Trait neoswap-sc-1685998915374"
draft: true
---
```
;; multiway-swap
...
;; SwapId: 26dvq5pUWuQW1yhQhRjE

;; traders
(define-constant TRADER-1 'SP2HZZBSESM3BKSETDPE0742M0PPMTRP2P2VTMKXY)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.stacks-punks-v3 transfer u6753 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.stacks-punks-v3 transfer u6753 tx-sender TRADER-1)))

	(unwrap-panic (as-contract (stx-transfer? u1102500 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u20947500 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u22050000 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.stacks-punks-v3 transfer u6753 tx-sender TRADER-2)))
	    )
            true
        )
