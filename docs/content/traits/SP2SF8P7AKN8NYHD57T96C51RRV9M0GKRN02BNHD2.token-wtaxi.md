...
(define-fungible-token wtaxi)

(define-data-var token-name (string-ascii 32) "stxtaxi Wrapper")
(define-data-var token-symbol (string-ascii 32) "wtaxi")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wtaxi.json"))
...
(define-read-only (get-base-token)
  'SP327J3J6RK88KGRQSMSWDRCP53EDHGJKHSNNGR4P.stacktaxi)

(define-read-only (get-base-decimals)
  (contract-call? 'SP327J3J6RK88KGRQSMSWDRCP53EDHGJKHSNNGR4P.stacktaxi get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP327J3J6RK88KGRQSMSWDRCP53EDHGJKHSNNGR4P.stacktaxi get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP327J3J6RK88KGRQSMSWDRCP53EDHGJKHSNNGR4P.stacktaxi transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
