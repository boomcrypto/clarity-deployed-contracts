...
(define-non-fungible-token stone-burst-badge uint)
...
(define-data-var artist-address principal 'SP22121YEW0ETCP2E29J9D7PFBX870EJCXA81PN08)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeqxAUrHttKQecZ1UyqtoXXwS5AWSNXuAxZbuV8jiPBnU/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stone-burst-badge next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stone-burst-badge token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stone-burst-badge token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stone-burst-badge token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stone-burst-badge id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stone-burst-badge id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stone-burst-badge id) (err ERR-NOT-FOUND)))
