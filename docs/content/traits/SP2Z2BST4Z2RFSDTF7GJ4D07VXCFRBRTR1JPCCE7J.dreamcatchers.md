...
(define-non-fungible-token dreamcatchers uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPiFXpaZQLNTL7iRkjesQBnSbwHys6oEYKht3CguKuLgm/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? dreamcatchers next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? dreamcatchers token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? dreamcatchers token-id)))
