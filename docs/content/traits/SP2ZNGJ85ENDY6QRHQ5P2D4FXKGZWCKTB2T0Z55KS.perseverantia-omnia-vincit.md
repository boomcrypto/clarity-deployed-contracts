---
title: "Trait perseverantia-omnia-vincit"
draft: true
---
```
;; Title: Perseverantia Omnia Vincit
;; Version: 1.0.0
;; Description: 
...
(define-constant LP_REBATE u40000)
...
;; Define LP token
(define-fungible-token POV)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.perseverantia-omnia-vincit"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? POV amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "Perseverantia Omnia Vincit"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance POV who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply POV)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? POV (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? POV (get dk delta) sender))
...
(begin
    ...
    (try! (add-liquidity u1200000))
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.hooter-the-owl transfer u9998800000 tx-sender CONTRACT none)))
```
