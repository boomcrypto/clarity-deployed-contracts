---
title: "Trait trevor-newnoncust3"
draft: true
---
```
;; trevor-newnoncust3
...
(define-non-fungible-token trevor-newnoncust3 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQQyrNm1uoze9Tgo4SLCoBkBQfX98Mp31mcCWcyDtUZ3L/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? trevor-newnoncust3 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? trevor-newnoncust3 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? trevor-newnoncust3 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? trevor-newnoncust3 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? trevor-newnoncust3 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? trevor-newnoncust3 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? trevor-newnoncust3 id) (err ERR-NOT-FOUND)))
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (var-set last-id (+ last-nft-id u0))
