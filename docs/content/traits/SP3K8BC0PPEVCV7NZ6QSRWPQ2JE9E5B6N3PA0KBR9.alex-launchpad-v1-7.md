---
title: "Trait alex-launchpad-v1-7"
draft: true
---
```
...
(define-map offering-ticket-bounds {launch-id: uint, owner: principal} {start: uint, end: uint})
(define-map offering-ticket-amounts {launch-id: uint, owner: principal} uint)
(define-map total-tickets-won uint uint)
(define-map tickets-won {launch-id: uint, owner: principal} uint)
(define-map claim-walk-positions uint uint)
(define-map use-whitelist uint bool)
(define-map whitelisted {launch-id: uint, owner: principal} bool)
...
(define-read-only (validate-register (owner principal) (launch-id uint) (payment-amount uint) (payment-token principal))
	(let (
			...
			(apower-bal (unwrap! (contract-call? .token-apower get-balance-fixed owner) err-apower-not-enough)))
...
(define-read-only (get-whitelisted-or-default (launch-id uint) (owner principal))
...
(define-public (set-whitelisted (launch-id uint) (whitelisted-users (list 200 {owner: principal, whitelisted: bool})))
	(let (
			(offering (try! (get-launch-or-fail launch-id))))
		(asserts! 
			(or 
...
(define-public (register-on-behalf (owner principal) (launch-id uint) (payment-amount uint) (payment-token-trait <ft-trait>))
	(let
		...
		(asserts! (get-whitelisted-or-default launch-id tx-sender) err-not-in-whitelist)
		(try! (contract-call? payment-token-trait transfer-fixed payment-amount sender (as-contract tx-sender) none))		
		(and (> apower-to-burn u0) (as-contract (try! (contract-call? .token-apower burn-fixed apower-to-burn owner))))
...
(define-private (set-whitelisted-iter (e {owner: principal, whitelisted: bool}) (launch-id uint))
	(begin 
...
(define-private (buff-to-uint64 (bytes (buff 32)))
	(+
