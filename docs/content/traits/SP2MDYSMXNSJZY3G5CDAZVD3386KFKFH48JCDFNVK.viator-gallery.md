...
(define-non-fungible-token viator-gallery uint)
...
(define-data-var artist-address principal 'SP2MDYSMXNSJZY3G5CDAZVD3386KFKFH48JCDFNVK)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdXVn1fHbrvVrnyqaeiVqtYBagoBVPMCvVr8cA1XZY1h3/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? viator-gallery next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? viator-gallery token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? viator-gallery token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? viator-gallery token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? viator-gallery id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? viator-gallery id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? viator-gallery id) (err ERR-NOT-FOUND)))
