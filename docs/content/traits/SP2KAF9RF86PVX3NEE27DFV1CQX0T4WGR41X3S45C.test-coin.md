...
(define-constant ERR-NO-MORE-TESTS u400)
(define-constant CONTRACT-OWNER tx-sender)

(define-data-var approved-principals (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C))
(define-data-var shutoff-valve bool false)

;; Define TEST with a maximum of 1,000,000 tokens / 1T microtokens.
(define-constant TEST-CAP u21000000000000)
(define-fungible-token TEST TEST-CAP)
...
(define-public (collect (sender principal) (amount uint))
    (let (
        ...
        (begin
            (try! (ft-mint? TEST amount sender))
            (ok true)
        )
    )
)
...
(define-read-only (get-token-uri)
  (ok (some u"https://test.io")))

(define-public (burn (burn-amount uint))
    (begin
        (try! (ft-burn? TEST burn-amount tx-sender))
        (ok true)
    )
)
...
(define-public (admin-airdrop (address principal) (amount uint))
    (let (
        ...
        (asserts! (is-some (index-of (var-get approved-principals) contract-caller)) (err ERR-NOT-AUTHORIZED))
        (begin
            (try! (ft-mint? TEST amount address))
            (ok true)
        )
    )
)
