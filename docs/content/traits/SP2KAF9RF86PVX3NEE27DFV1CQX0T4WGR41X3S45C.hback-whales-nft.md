...
(define-non-fungible-token hback-whales-nft uint)
...
(define-data-var banana-cost-per-mint uint u88000000)
(define-data-var commission-1 uint u1000)
(define-data-var base-uri (string-ascii 100) "ipfs://QmP5CKP4rm2Ynp9172tZaXDL3MVVX3HRspnpTDojazAfju/")
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? hback-whales-nft id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace hback-whales-nft
  (ok (nft-get-owner? hback-whales-nft id)))
...
(define-public (banana-mint (new-owner principal))
    ...
    (begin
      ...
      (asserts! (< (var-get last-id) (- STX-MINT-LIMIT u2)) ERR-SOLD-OUT)
      (match (nft-mint? hback-whales-nft next-id new-owner)
        success
        (let
        ((current-balance (get-balance new-owner)))
          (begin
            (print "mint in banana")
            (try! (contract-call? .btc-monkeys-bananas transfer commission-one tx-sender (var-get commission-one-address) (some 0x00)))
            (try! (contract-call? .btc-monkeys-bananas transfer payout tx-sender (var-get artist-address) (some 0x00)))
...
(define-public (admin-mint (new-owner principal))
    (let (
      ...
      (asserts! (is-eq tx-sender (var-get artist-address)) ERR-NOT-AUTHORIZED)
      (asserts! (>= (var-get last-id) (- STX-MINT-LIMIT u2)) ERR-NOT-AUTHORIZED)
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? hback-whales-nft id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? hback-whales-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? hback-whales-nft id) ERR-NOT-FOUND))
