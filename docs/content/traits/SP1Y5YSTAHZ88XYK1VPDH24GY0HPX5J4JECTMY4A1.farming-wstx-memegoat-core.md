---
title: "Trait farming-wstx-memegoat-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-memegoat)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-memegoat transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-memegoat get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-memegoat transfer amt protocol user none)))
