---
title: "Trait arkadiko-stx-diko-pool-v1-0"
draft: true
---
```
...
(define-private (swap-stx-for-diko (amount uint))
  (let 
    (
      (diko-amount (element-at? (try! (as-contract (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-swap-v2-1 swap-x-for-y 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.wrapped-stx-token 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-token amount u0))) u1))
    )
...
(define-private (swap-diko-for-stx (amount uint)) 
  (let 
    (
      (stx-amount (element-at? (try! (as-contract (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-swap-v2-1 swap-y-for-x 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.wrapped-stx-token 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-token amount u0))) u0))
    )
    ...
    (let
      (
        ...
        (stx-amount (if is-diko
          (try! (swap-diko-for-stx swap-amount))
          (- amount swap-amount)
        ))
        (diko-amount (if is-diko
          (- amount swap-amount)
          (try! (swap-stx-for-diko swap-amount))
        ))
  ...
  (begin
    ...
    (let
      (
        ...
        (swap-result (swap-diko-for-stx (/ diko-initial-balance u2)))
      ...
      (let
        ...
        (if (is-eq unstake-mode UNSTAKE_PAIR)
          ...
          (if (is-eq unstake-mode UNSTAKE_STX)
            (let 
              (
                (swapped-stx-amount (try! (swap-diko-for-stx withdrawn-diko)))
            ...
            (let 
              (
                (swapped-diko-amount (try! (swap-stx-for-diko withdrawn-stx)))
