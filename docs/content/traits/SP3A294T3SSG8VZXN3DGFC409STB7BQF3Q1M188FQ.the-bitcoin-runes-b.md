---
title: "Trait the-bitcoin-runes-b"
draft: true
---
```
;; the-bitcoin-runes-b
...
(define-non-fungible-token the-bitcoin-runes-b uint)
...
(define-data-var total-price uint u220000)
(define-data-var artist-address principal 'SP3A294T3SSG8VZXN3DGFC409STB7BQF3Q1M188FQ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXTLj86jvkSnFhCUGYiGJxG2izXKMqtDpE7sB9BwM1esp/json/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u22)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-bitcoin-runes-b next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-bitcoin-runes-b token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-bitcoin-runes-b token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-bitcoin-runes-b token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-bitcoin-runes-b id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-bitcoin-runes-b id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-bitcoin-runes-b id) (err ERR-NOT-FOUND)))
