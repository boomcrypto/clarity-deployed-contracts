...
(define-non-fungible-token diddy-oil uint)
...
(define-data-var artist-address principal 'SP2HNR813RVPNSSA4P0DJ0D6GP0ET5J53HKED5BMW)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSCxGEYPHd3Esmsd4ojWxQNMnGSUWR6KwQq9KxmMtPwo6/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? diddy-oil next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? diddy-oil token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? diddy-oil token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? diddy-oil token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? diddy-oil id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? diddy-oil id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? diddy-oil id) (err ERR-NOT-FOUND)))
