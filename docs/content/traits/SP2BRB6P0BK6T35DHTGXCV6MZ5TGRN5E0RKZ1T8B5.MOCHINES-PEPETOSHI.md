---
title: "Trait MOCHINES-PEPETOSHI"
draft: true
---
```
;; nft Contract
...
(define-non-fungible-token Pepetoshi uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafkreidm4z75higqufxg5miojok3q3srqyjfkkelmtwab2epbilxwrsjkm")
...
(define-public (mint (recipient principal))
    (begin 
        ...
        (asserts! (< (var-get last-id) SUPPLY) (err ERR-NO-MORE-NFTS))
            ...
            (try! (nft-mint? Pepetoshi u1 tx-sender))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Pepetoshi token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Pepetoshi token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Pepetoshi token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Pepetoshi id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Pepetoshi id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Pepetoshi id) (err ERR-NOT-FOUND)))
