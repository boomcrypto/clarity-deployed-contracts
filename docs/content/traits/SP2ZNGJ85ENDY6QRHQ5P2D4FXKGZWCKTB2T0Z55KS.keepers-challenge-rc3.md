---
title: "Trait keepers-challenge-rc3"
draft: true
---
```
...
(define-data-var exp-boost-factor uint u10000000) ;; 1% boost per 1000 EXP
...
(define-private (petition)
  (let
    ...
    (try! (contract-call? .dungeon-keeper-rc2 exhaust energy-to-spend tx-sender))
    (try! (contract-call? .dungeon-keeper-rc2 reward u500000 tx-sender)) ;; 0.5 EXP
    (try! (contract-call? .dungeon-keeper-rc2 transfer base-reward (var-get contract-owner) tx-sender))
    (ok true)
  )
)
...
(define-private (challenge)
  (let
    ...
    (try! (contract-call? .dungeon-keeper-rc2 exhaust energy-to-spend tx-sender))
    (try! (contract-call? .dungeon-keeper-rc2 reward u2000000 tx-sender)) ;; 2 EXP
    ...
    (let
      ...
      (try! (contract-call? .dungeon-keeper-rc2 transfer final-reward (var-get contract-owner) tx-sender))
...
(define-private (heist)
  (let
    ...
    (if (< success-rate u50) ;; 50% chance of success
      (begin
        (try! (contract-call? .dungeon-keeper-rc2 exhaust u1000000 tx-sender)) ;; Burn 1 EXP on failure
        (try! (contract-call? .dungeon-keeper-rc2 transfer boosted-reward tx-sender (var-get contract-owner)))
        (ok false)
      )
      (begin
        (try! (contract-call? .dungeon-keeper-rc2 reward u5000000 tx-sender)) ;; 5 EXP on success
        (try! (contract-call? .dungeon-keeper-rc2 transfer boosted-reward (var-get contract-owner) tx-sender))
        (ok true)
      )
    )
  )
