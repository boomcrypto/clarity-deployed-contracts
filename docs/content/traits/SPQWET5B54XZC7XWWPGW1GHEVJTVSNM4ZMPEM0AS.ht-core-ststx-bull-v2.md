---
title: "Trait ht-core-ststx-bull-v2"
draft: true
---
```
;; @contract Core
...
;; @desc Triggers methods that settle the current epoch
(define-private (settle) 
  (begin 
    (try! (charge-fees))
    (try! (settle-epoch-info))
    (try! (activate-pending-claims))
    (try! (as-contract (contract-call? .ht-hq-ststx-bull-v2 update-fees-and-settings)))
    (unwrap-panic (reset-data))
    (ok true)))

;; @desc Charges performance and management fees
(define-private (charge-fees)
  (let (
    (current-performance-fee (get current (try! (contract-call? .ht-hq-ststx-bull-v2 get-fees "performance"))))
    (current-management-fee (get current (try! (contract-call? .ht-hq-ststx-bull-v2 get-fees "management"))))
    (fee-address (contract-call? .ht-hq-ststx-bull-v2 get-fee-address))
    (no-profit-management-fee (contract-call? .ht-hq-ststx-bull-v2 get-no-profit-management-fee))
    ...
    (if (and strike-set (not (var-get delinquent-epoch)))
      (if (> (* unit-pnl units-transacted) total-premium)
        (begin
          (try! 
            (as-contract (contract-call? .ht-vault-ststx-bull-v2 payout-funds
...
;; @desc Updates the epoch-info map in core and vault with the settled values
(define-private (settle-epoch-info)
  (let (
    (underlying-per-token (contract-call? .ht-vault-ststx-bull-v2 get-underlying-per-token))
    (current-epoch-info (unwrap-panic (get-current-epoch-info)))) 
    ...
    (as-contract (contract-call? .ht-vault-ststx-bull-v2 update-epoch-info-for-claims))))

;; @desc Updates vault variables that processs deposits and withdrawals
(define-private (activate-pending-claims) 
  (begin
    (try! (as-contract (contract-call? .ht-vault-ststx-bull-v2 activate-pending-deposit-claims)))
    (try! (as-contract (contract-call? .ht-vault-ststx-bull-v2 activate-pending-withdrawal-claims)))
    (ok true)))

;; @desc Initiates trading-funds-available for the next epoch
(define-private (init-trading-funds-available)
  (begin
    (var-set trading-funds-available 
    (/ 
      (* 
        (contract-call? .ht-vault-ststx-bull-v2 get-total-underlying-active)
        (contract-call? .ht-hq-ststx-bull-v2 get-epoch-risk)) 
      bps-base))
    (ok true)))

;; @desc Resets all variables and maps
...
;; @desc Creates a payment-request for every entry in counterparty-info
;; @param counterparty: principal that has sold option(s) to the contract
...
;; @desc Provides pricing data to determine the option pnl and either settles the epoch or requests payment from counterparties
;; @param pnl-calculator-contract: contract address of active pnl-calculator
;; @param use-oracle-price: if false, use the settlement-rate argument as the settlment price
;; @param settlement-rate: usd-per-underlying rate at which the option contract settled (oracle format: 10**8)
;; @param option-asset-per-underlying: option asset to underlying rate i.e. BTCsBTC or STXstSTX (oracle format: 10**8)
;; @price-feed-bytes: pyth price feed data
;; @execution-plan: pyth contracts to be called by oracle
(define-public (determine-pnl
  ...
  (let (
    ...
    (option-type (contract-call? .ht-hq-ststx-bull-v2 get-option-type))
    (strategy-type (contract-call? .ht-hq-ststx-bull-v2 get-strategy-type))
    (unit-size (contract-call? .ht-hq-ststx-bull-v2 get-unit-size))
    ...
    (total-premium (get total-premium current-epoch-info)))
    (try! (contract-call? .ht-hq-ststx-bull-v2 check-is-updater tx-sender))
    (try! (contract-call? .ht-hq-ststx-bull-v2 check-is-pnl-calculator-active (contract-of pnl-calculator-contract)))
    ...
    (asserts! (<= timestamp (+ current-epoch-expiry (contract-call? .ht-hq-ststx-bull-v2 get-pnl-data-window))) ERR_SETTLEMENT_WINDOW_CLOSED)
...
;; @desc Allows admin to start a new epoch and set the expiry, strike and barrier prices
;; @param pnl-calculator-contract: contract address of active pnl-calculator
;; @param expiry: unix timestamp in ms 
;; @param strike-call: strike price in USD for call option (oracle format: 10**8) 
;; @param strike-put: strike price in USD for put option (oracle format: 10**8) 
;; @param barrier-up: upwards barrier in USD (oracle format: 10**8)
;; @param barrier-down: donwards barrier in USD (oracle format: 10**8)
(define-public (start-new-epoch
  ...
  (let (
    ...
    (try! (contract-call? .ht-hq-ststx-bull-v2 check-is-trader tx-sender))
    (try! (contract-call? .ht-hq-ststx-bull-v2 check-is-pnl-calculator-active (contract-of pnl-calculator-contract)))
    (asserts! (contract-call? .ht-hq-ststx-bull-v2 get-trading-allowed) ERR_TRADING_NOT_ALLOWED)
    ...
    (if (and (is-eq current-epoch-expiry expiry) (not current-epoch-settled))
      ...
      (begin 
        ...
        (asserts! (>= expiry (+ current-epoch-expiry (contract-call? .ht-hq-ststx-bull-v2 get-min-epoch-duration))) ERR_VIOLATES_MIN_EPOCH_DURATION)
        (asserts! (<= expiry (+ current-epoch-expiry (contract-call? .ht-hq-ststx-bull-v2 get-max-epoch-duration))) ERR_VIOLATES_MAX_EPOCH_DURATION)
        (map-set epoch-info { epoch-id: new-epoch-id } 
          {
            ...
            total-premium: u0,
            total-underlying-active: (contract-call? .ht-vault-ststx-bull-v2 get-total-underlying-active), 
            underlying-per-token-settled: none,
            epoch-risk: (contract-call? .ht-hq-ststx-bull-v2 get-epoch-risk),
            unit-size: (contract-call? .ht-hq-ststx-bull-v2 get-next-unit-size)
          }
        )
        (try! (as-contract (contract-call? .ht-vault-ststx-bull-v2 create-epoch-info-for-claims)))
        (var-set current-epoch-id new-epoch-id)
      )
    )
    (ok true)))

;; @desc Registers a trade by declaring the counterparty, amount and price
;; @param amount: amount of units to be transacted 
;; @param option-price-per-underlying: price of an option represeting one base unit of underlying (i.e. 1 sBTC / 100,000,000 sats or 1 stSTX)
;; @param counterparty: principal of the counterparty
(define-public (register-trade
  ...
  (let (
    (current-epoch-info (unwrap-panic (get-current-epoch-info)))
    (funds-outgoing (* amount (/ (* option-price-per-underlying (contract-call? .ht-hq-ststx-bull-v2 get-unit-size)) underlying-base))))
    (try! (contract-call? .ht-hq-ststx-bull-v2 check-is-trader tx-sender))
    ...
    (asserts! (<= burn-block-height (+
      ...
      (contract-call? .ht-hq-ststx-bull-v2 get-registration-window))) 
...
;; @desc Updates a trade registration
;; @param new-amount: amount of units to be transacted
;; @param option-price-per-underlying: price of an option represeting one unit of underlying (for example 1 BTC)
;; @param counterparty: principal of the counterparty
...
;; @desc Allows a counterparty to confirm a trade and transfers the premium 
;; @param amount: amount of units to be transacted 
;; @param option-price-per-underlying: price of an option represeting one unit of underlying (for example 1 BTC)
(define-public (confirm-trade
  ...
  (let (
    ...
    (funds-outgoing (/ (* (* option-price-per-underlying amount) (contract-call? .ht-hq-ststx-bull-v2 get-unit-size)) underlying-base))
    (current-epoch-info (unwrap-panic (get-current-epoch-info))))
    ...
    (asserts! (<= burn-block-height (+ block-height-registered (contract-call? .ht-hq-ststx-bull-v2 get-confirmation-window))) ERR_CONFIRMATION_WINDOW_CLOSED)
    (try! (as-contract (contract-call? .ht-vault-ststx-bull-v2 payout-funds funds-outgoing counterparty)))
...
;; @desc Allows a counterparty to make a payment for an ITM option
;; @param amount: amount to be paid in underlying
(define-public (make-payment
  ...
  (let (
    ...
    (payment-window (contract-call? .ht-hq-ststx-bull-v2 get-payment-window)))
    ...
    (try! (contract-call? .ht-vault-ststx-bull-v2 deposit-funds amount tx-sender))
...
;; @desc Allows a counterparty to make a payment for an ITM option after the payment-window closed
;; @param amount: amount to be paid in underlying
...
;; @desc Deletes counterparty entry from counterparty-info
;; @param counterparty: principal that has sold option(s) to the contract
(define-private (delete-counterparty
  (counterparty-address principal))
  (ok (map-delete counterparty-info { address: counterparty-address })))

;; @desc Moves counterparty from counter-party-ledger to delinquent-counterparty-info
;; @param counterparty: principal that has sold option(s) to the contract
...
;; @desc Removes a principal from a list
;; @param list-item: principal that is checked against helper-principal
(define-private (remove-principal
  (list-item principal))
  (not (is-eq (var-get helper-principal) list-item)))

;; @desc Converts usd value to option-asset value (i.e. BTC)
;; @param usd: usd value
;; @usd-per-option-asset: usd price of option-asset (i.e. BTCUSD)
...
;; @desc Allows admin to initialize the contract; can only be called once
;; @param first-expiry: unix timestamp in milliseconds of the first expiry
...
;; @desc Forces the epoch to settle if the payment-window has closed
(define-public (force-settle)
  (let (
    (current-epoch-info (unwrap-panic (get-current-epoch-info)))
    (block-height-pnl (unwrap! (get block-height-pnl current-epoch-info) ERR_PNL_UNDETERMINED)))
    (asserts! (> burn-block-height (+ block-height-pnl (contract-call? .ht-hq-ststx-bull-v2 get-payment-window))) ERR_PAYMENT_WINDOW_STILL_OPEN)
...
;; @desc Forces sets pnl if the pnl-calculation-window has closed
;; @param use-block-height: if true, use block-height to determine pnl-calculation-window instead of oracle timestamp
;; @price-feed-bytes: pyth price feed data
;; @execution-plan: pyth contracts to be called by oracle
(define-public (force-determine-pnl
  ...
  (let (
    ...
    (current-epoch-info (unwrap-panic (get-current-epoch-info)))
    (current-epoch-expiry (get epoch-expiry current-epoch-info))
    (pnl-calculation-window (contract-call? .ht-hq-ststx-bull-v2 get-pnl-calculation-window)))
...
;; @desc Forces all queued-deposits to be processed
(define-public (force-activate-pending-deposit-claims)
  (begin
    (try! (contract-call? .ht-hq-ststx-bull-v2 check-is-admin tx-sender))
    (as-contract (contract-call? .ht-vault-ststx-bull-v2 activate-pending-deposit-claims))))

;; @desc Forces all queued-withdrawals to be processed
(define-public (force-activate-pending-withdrawal-claims)
  (begin
    (try! (contract-call? .ht-hq-ststx-bull-v2 check-is-admin tx-sender))
    (as-contract (contract-call? .ht-vault-ststx-bull-v2 activate-pending-withdrawal-claims))))
```
