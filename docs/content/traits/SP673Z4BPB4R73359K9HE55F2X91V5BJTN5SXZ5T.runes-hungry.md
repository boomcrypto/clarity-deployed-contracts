...
(define-fungible-token runes-hungry)

(define-data-var token-name (string-ascii 32) "TONIGHT I AM HUNGRY")
(define-data-var token-symbol (string-ascii 10) "HUNGRY")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/runes-hungry.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(try! (ft-transfer? runes-hungry amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance runes-hungry who)))

(define-read-only (get-total-supply)
	(ok (ft-get-supply runes-hungry)))
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? runes-hungry amount recipient)))
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? runes-hungry amount sender)))
