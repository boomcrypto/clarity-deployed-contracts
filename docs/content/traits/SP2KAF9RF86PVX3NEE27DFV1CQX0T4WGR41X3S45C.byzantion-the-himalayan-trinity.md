---
title: "Trait byzantion-the-himalayan-trinity"
draft: true
---
```
...
(define-non-fungible-token the-himalayan-trinity uint)
...
(define-data-var cost-per-mint uint u300000000)
(define-data-var commission-per-mint uint u24000000)
(define-data-var payout uint u0)
(define-data-var ipfs-full-metadata (string-ascii 129) "ipfs://ipfs/bafybeialgku6lgq3vh6qhhfjy7jmbr3dfir7duqm4sgprs56bqf2zva7kq/the_himalayan_trinity/the_himalayan_trinity_metadata.json")
(define-data-var ipfs-root (string-ascii 116) "ipfs://ipfs/bafybeibunnfz44hiqc7lc5zo2z4g3wlsmboxhax6hj5ajpry7vthsukyra/the_himalayan_trinity/the_himalayan_trinity_")
(define-data-var artist-address principal 'SP1HYPJ0P6VQ02AAFZFSME4KCYGWM590A82Z4YK1P)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? the-himalayan-trinity next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? the-himalayan-trinity token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-himalayan-trinity token-id)))
