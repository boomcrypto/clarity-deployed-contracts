---
title: "Trait city-collection-nyc"
draft: true
---
```
;; city-collection-nyc
...
(define-non-fungible-token city-collection-nyc uint)
...
(define-data-var coin-price uint u12500)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmR9CXwXGqabNFaMxNgefmL5YcJRng7vc3vhcFNss1sUL1/")
...
(define-private (transfer-funds (is-stx bool) (price uint))
   (let (
    ...
    (if is-stx
        ...
        (begin
            (try! (contract-call? 'SP2H8PY27SEZ03MWRKS5XABZYQN17ETGQS3527SA5.newyorkcitycoin-token transfer total-artist tx-sender art-addr none))
            (try! (contract-call? 'SP2H8PY27SEZ03MWRKS5XABZYQN17ETGQS3527SA5.newyorkcitycoin-token transfer total-commission1 tx-sender COMM_ADDR1 none))
            (try! (contract-call? 'SP2H8PY27SEZ03MWRKS5XABZYQN17ETGQS3527SA5.newyorkcitycoin-token transfer total-commission2 tx-sender COMM_ADDR2 none)))
    )
    (ok true)
  )
)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? city-collection-nyc next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? city-collection-nyc token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? city-collection-nyc token-id)))
