---
title: "Trait stacks-spanish-bootcamp"
draft: true
---
```
...
(define-non-fungible-token Stacks-Spanish-Bootcamp uint)
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Stacks-Spanish-Bootcamp token-id))
)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? Stacks-Spanish-Bootcamp token-id sender recipient)
  )
)
...
(define-public (mint (recipient principal) (token-uri (string-ascii 150)))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? Stacks-Spanish-Bootcamp token-id recipient))
