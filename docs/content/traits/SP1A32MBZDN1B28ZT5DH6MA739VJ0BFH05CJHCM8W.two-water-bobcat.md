...
(define-non-fungible-token two-water-bobcat uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://Qmf26yS3GiqRxX36WTvEymVNV2iWEZmvrZ57ny7Se3odR1/two-water-bobcat/")
...
(define-data-var public-target-block-height uint u63619)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? two-water-bobcat id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace two-water-bobcat
  (ok (nft-get-owner? two-water-bobcat id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? two-water-bobcat next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? two-water-bobcat id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? two-water-bobcat id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? two-water-bobcat id) ERR-NOT-FOUND))
...
(define-data-var mia-cost-per-mint uint u180000000)
