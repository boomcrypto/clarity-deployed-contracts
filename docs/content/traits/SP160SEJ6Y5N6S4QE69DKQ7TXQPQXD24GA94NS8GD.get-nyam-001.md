...
(define-non-fungible-token get-nyam-001 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVbms3fqSoHydEFe4cvd41FxHXWNcZFijX4VgHoyrLP11/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? get-nyam-001 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? get-nyam-001 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? get-nyam-001 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? get-nyam-001 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? get-nyam-001 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? get-nyam-001 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? get-nyam-001 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u129400000000)
