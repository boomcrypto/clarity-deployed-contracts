---
title: "Trait iouwelshwelsh-lp-token"
draft: true
---
```
;; Title: iouWELSH-WELSH LP Token
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token iouWELSH-WELSH)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/api/v0/metadata/SP3T1M18J3VX038KSYPP5G450WVWWG9F9G6GAZA4Q.iouwelshwelsh-lp-token"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? iouWELSH-WELSH amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "iouWELSH-WELSH LP Token"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance iouWELSH-WELSH who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply iouWELSH-WELSH)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-welsh transfer amount sender CONTRACT none))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-welsh transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-welsh transfer (get dx delta) sender CONTRACT none))
        ...
        (try! (ft-mint? iouWELSH-WELSH (get dk delta) sender))
        (ok delta)))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? iouWELSH-WELSH (get dk delta) sender))
        (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-welsh transfer (get dx delta) CONTRACT sender none)))
        (try! (as-contract (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer (get dy delta) CONTRACT sender none)))
        (ok delta)))
...
(define-private (get-reserves)
    { 
      a: (unwrap-panic (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-welsh get-balance CONTRACT)), 
...
(define-read-only (get-liquidity-quote (amount uint))
    (let (
        (k (ft-get-supply iouWELSH-WELSH))
        (reserves (get-reserves)))
        {
...
(define-read-only (get-reserves-quote)
    (let (
        (reserves (get-reserves))
        (supply (ft-get-supply iouWELSH-WELSH)))
        {
          dx: (get a reserves),
          dy: (get b reserves),
          dk: supply
...
(begin
    ...
    (try! (add-liquidity u92000000000))
    
    ;; Transfer additional token A to achieve desired ratio
    (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.synthetic-welsh transfer u908000000000 tx-sender CONTRACT none)))
```
