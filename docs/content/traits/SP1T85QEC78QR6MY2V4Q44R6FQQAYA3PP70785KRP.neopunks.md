...
(define-non-fungible-token neopunks uint)
...
(define-data-var artist-address principal 'SP1T85QEC78QR6MY2V4Q44R6FQQAYA3PP70785KRP)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeUbJ8mPs7tUbM4UVS7YMro3iWLTWZxEfKVTeBaTbWnDp/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? neopunks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? neopunks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? neopunks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? neopunks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? neopunks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? neopunks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? neopunks id) (err ERR-NOT-FOUND)))
