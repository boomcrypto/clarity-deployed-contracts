---
title: "Trait stacks-pops-roll-dice-v2"
draft: true
---
```
;; Define Constants
...
;; Deposit $ICE in the contract to fund the machine
(define-public (deposit-ice (amount uint))
  (begin
    (try! (contract-call? .stacks-pops-ice-v2 transfer amount tx-sender .stacks-pops-roll-dice-v2 (some 0x6465706F736974)))
    (ok true)))
...
(define-private (execute-bet (player principal) (random-value uint) (bet-dice-value uint) (bet-ice-value uint) (prize-value uint))
  (let ((draw-dice-value (+ (mod random-value u6) u1))
    ...
    (if
      ...
      (contract-call? .stacks-pops-ice-v2 transfer bet-ice-value tx-sender .stacks-pops-roll-dice-v2 (some 0x4C4F4F5345))
    )
  )
)
