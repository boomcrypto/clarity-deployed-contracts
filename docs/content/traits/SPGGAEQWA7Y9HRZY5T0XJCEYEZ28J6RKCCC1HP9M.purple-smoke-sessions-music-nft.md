---
title: "Trait purple-smoke-sessions-music-nft"
draft: true
---
```
;; purple-smoke-sessions-music-nft
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token purple-smoke-sessions-music-nft uint)
...
;; Internal variables
(define-data-var mint-limit uint u160)
(define-data-var last-id uint u1)
(define-data-var total-price uint u15000000)
(define-data-var artist-address principal 'SPGGAEQWA7Y9HRZY5T0XJCEYEZ28J6RKCCC1HP9M)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmckuMRvr9rueCBcn4v5TX9uRUCb2Mob9GGrEnh8GpSDDc/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? purple-smoke-sessions-music-nft next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? purple-smoke-sessions-music-nft token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? purple-smoke-sessions-music-nft token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? purple-smoke-sessions-music-nft token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? purple-smoke-sessions-music-nft id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? purple-smoke-sessions-music-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? purple-smoke-sessions-music-nft id) (err ERR-NOT-FOUND)))
