...
(define-public (confirm-redeem (request-id uint) (price uint) (amount-usdh-confirmed uint) (redeeming-asset <sip-010-trait>))
  (let (
    ...
    (try! (as-contract (contract-call? .test-usdh-token burn-for-protocol (- amount-usdh-confirmed amount-usdh-fee) tx-sender)))
    ...
    (if (not (is-eq amount-usdh-requested amount-usdh-confirmed))
      (try! (as-contract (contract-call? .test-usdh-token transfer (- amount-usdh-requested amount-usdh-confirmed) tx-sender requester none)))
      true
    )
