...
(define-non-fungible-token tenmetsu uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSnH81zrDB2Gz9kX4j3Hf3koGNjbFjUta119GUn7hyH4h/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? tenmetsu next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? tenmetsu token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? tenmetsu token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tenmetsu token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? tenmetsu id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? tenmetsu id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? tenmetsu id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u11109)
