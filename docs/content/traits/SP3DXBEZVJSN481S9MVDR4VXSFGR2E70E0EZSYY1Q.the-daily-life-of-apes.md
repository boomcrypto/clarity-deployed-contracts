---
title: "Trait the-daily-life-of-apes"
draft: true
---
```
;; the-daily-life-of-apes
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token the-daily-life-of-apes uint)
...
;; Internal variables
(define-data-var mint-limit uint u155)
(define-data-var last-id uint u1)
(define-data-var total-price uint u7000000)
(define-data-var artist-address principal 'SP3DXBEZVJSN481S9MVDR4VXSFGR2E70E0EZSYY1Q)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNyf5snySKADqzFxU4RARmkgjxU2ku5HthvA6toQeDKCQ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-daily-life-of-apes next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-daily-life-of-apes token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-daily-life-of-apes token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-daily-life-of-apes token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-daily-life-of-apes id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-daily-life-of-apes id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-daily-life-of-apes id) (err ERR-NOT-FOUND)))
