...
(define-non-fungible-token doodle-friends uint)
...
;; Internal variables
(define-data-var mint-limit uint u4924)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1088000)
(define-data-var artist-address principal 'SP375QX2YY3WNW6J4XP2YPGXRZAQVJFERSXP6SY1P)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQXAgErHvsHGivY8X8jXzPsdBv9j4Lx4Arqc1N91B1rSc/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? doodle-friends next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? doodle-friends token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? doodle-friends token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? doodle-friends token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? doodle-friends id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? doodle-friends id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? doodle-friends id) (err ERR-NOT-FOUND)))
