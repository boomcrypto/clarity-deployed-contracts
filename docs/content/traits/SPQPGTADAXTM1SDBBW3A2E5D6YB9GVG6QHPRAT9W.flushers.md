...
(define-non-fungible-token flushers uint)
...
(define-data-var artist-address principal 'SPQPGTADAXTM1SDBBW3A2E5D6YB9GVG6QHPRAT9W)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmW5YUZdfCGW9HQGJcsk6MLJmXXaScHVvSVXUgTY2NXMdt/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? flushers next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? flushers token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? flushers token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? flushers token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? flushers id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? flushers id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? flushers id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u8084)
