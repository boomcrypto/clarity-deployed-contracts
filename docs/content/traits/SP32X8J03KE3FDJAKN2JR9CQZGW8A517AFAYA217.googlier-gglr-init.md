---
title: "Trait googlier-gglr-init"
draft: true
---
```
;; @contract GGLR Init - Tokens to be claimed by foundation and team
;; @version 1
...
(define-public (foundation-claim-tokens (amount uint))
  (let (
    ...
    (as-contract (contract-call? .googlier-token transfer amount .googlier-gglr-init wallet none))
  )
)
...
(begin
  (contract-call? .googlier-dao mint-token .googlier-token (+ TOTAL-FOUNDATION TOTAL-FOUNDERS) .googlier-gglr-init)
)
