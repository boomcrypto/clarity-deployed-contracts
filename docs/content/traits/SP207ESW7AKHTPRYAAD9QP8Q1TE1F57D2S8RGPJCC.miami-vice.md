...
(define-non-fungible-token miami-vice uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmW3LSzVQTsCfbyCnD7LAngXCTHNAoTyLvnofoDou5u8gX/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? miami-vice next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? miami-vice token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? miami-vice token-id)))
