...
(define-non-fungible-token apunkalypse uint)
...
;; Internal variables
(define-data-var mint-limit uint u681)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmcc7Va56Z86s21M5reJnDuGNyq3sScbZqLXZZBz3nSpKW/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? apunkalypse next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? apunkalypse token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? apunkalypse token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? apunkalypse token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? apunkalypse id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? apunkalypse id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? apunkalypse id) (err ERR-NOT-FOUND)))
