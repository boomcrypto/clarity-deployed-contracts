---
title: "Trait neoswap-sc-1693832525764"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/KX5MZCGLX7w5pxc5GO3u
;; SwapId: goLfD5QaPG71ceMIFoVP
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u6028708 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u6028708 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u6028708 tx-sender TRADER-1)))
	    )
            true
        )
