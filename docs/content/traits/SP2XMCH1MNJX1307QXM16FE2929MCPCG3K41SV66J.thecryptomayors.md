...
(define-non-fungible-token thecryptomayors uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmP2LX45pmtUSZ9s8n462aVxwyJPMqbc2qiMKa7KCKELTF/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? thecryptomayors next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? thecryptomayors token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? thecryptomayors token-id) false)))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? thecryptomayors token-id sender recipient)))

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? thecryptomayors token-id)))
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u60000)
