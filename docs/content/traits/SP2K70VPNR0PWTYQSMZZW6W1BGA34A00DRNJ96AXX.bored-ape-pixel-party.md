---
title: "Trait bored-ape-pixel-party"
draft: true
---
```
;; bored-ape-pixel-party
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token bored-ape-pixel-party uint)
...
(define-data-var artist-address principal 'SP2K70VPNR0PWTYQSMZZW6W1BGA34A00DRNJ96AXX)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeAJzLSJ24kuEqHgr2PDcFScS6z3LEBvjUJkC2MZudaKG/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bored-ape-pixel-party next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bored-ape-pixel-party token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bored-ape-pixel-party token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bored-ape-pixel-party token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bored-ape-pixel-party id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bored-ape-pixel-party id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bored-ape-pixel-party id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u40347)
