...
(define-public (deposit-usdb (usdb-amount uint))
    (let 
        ...
        (asserts! (>= (unwrap! (contract-call? .u-testing get-balance user) (err "err-insufficient-balance")) usdb-amount) (err "err-insufficient-balance"))

        ;; Transfer USDB from the user's account to the Stability Pool's contract
        (unwrap! (contract-call? .u-testing transfer usdb-amount user (as-contract tx-sender) none) (err "err-transfer"))
...
(define-public (withdraw-usdb (usdb-amount uint))
    (let 
        ...
        (unwrap! (contract-call? .u-testing transfer usdb-amount (as-contract tx-sender) user none) (err "err-transfer"))
...
(define-public (burn-usdb (amount-to-burn uint))
    (let 
        ...
        (ok (unwrap! (contract-call? .u-testing burn-usdb .sp-testing amount-to-burn) (err "err-burn-usdb-stability")))
    )
)
...
(define-public (update-users-map (collateral-to-distribute uint) (user principal))
    (let 
        ...
        (asserts! (is-eq contract-caller .v-testing) (err "err-not-authorized"))
