...
(define-fungible-token wgecko)

(define-data-var token-name (string-ascii 32) "Stacking Gecko Wrapper")
(define-data-var token-symbol (string-ascii 32) "wgecko")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wgecko.json"))
...
(define-read-only (get-base-token)
  'SP1A7A633VW9RRJK06XX9VRPWT7N20JXHJ947Q5XJ.stacking-gecko)

(define-read-only (get-base-decimals)
  (contract-call? 'SP1A7A633VW9RRJK06XX9VRPWT7N20JXHJ947Q5XJ.stacking-gecko get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP1A7A633VW9RRJK06XX9VRPWT7N20JXHJ947Q5XJ.stacking-gecko get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP1A7A633VW9RRJK06XX9VRPWT7N20JXHJ947Q5XJ.stacking-gecko transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
