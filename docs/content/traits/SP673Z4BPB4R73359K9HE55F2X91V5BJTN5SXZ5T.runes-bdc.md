...
(define-fungible-token runes-bdc)

(define-data-var token-name (string-ascii 32) "BILLION DOLLAR CAT")
(define-data-var token-symbol (string-ascii 10) "BDC")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/runes-bdc.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(try! (ft-transfer? runes-bdc amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance runes-bdc who)))

(define-read-only (get-total-supply)
	(ok (ft-get-supply runes-bdc)))
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? runes-bdc amount recipient)))
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? runes-bdc amount sender)))
