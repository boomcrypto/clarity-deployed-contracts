---
title: "Trait byzantion-stacks-pixels"
draft: true
---
```
...
(define-data-var ipfs-full-metadata (string-ascii 108) "ipfs://bafybeiczsuexkoavazj2v3hb4zd2e27grqe2xbelz5m32pfemdy2kitrum/stacks_pixels/stacks_pixels_metadata.json")
(define-data-var ipfs-root (string-ascii 95) "ipfs://bafybeiczsuexkoavazj2v3hb4zd2e27grqe2xbelz5m32pfemdy2kitrum/stacks_pixels/stacks_pixels_")
(define-data-var artist-address principal 'SP264D4RFM6TG2DAGK7XX26C3KFXAKSHPTSB8458R)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? stacks-pixels next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? stacks-pixels token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
