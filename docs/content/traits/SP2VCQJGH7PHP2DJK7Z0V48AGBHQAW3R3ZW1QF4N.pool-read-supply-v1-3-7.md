---
title: "Trait pool-read-supply-v1-3-7"
draft: true
---
```
...
(define-read-only (get-useable-collateral-usd-usdh (who principal))
  (let (
    (asset-balance (unwrap-panic (contract-call? .zusdh-v1-2 get-principal-balance who)))
...
(define-read-only (get-useable-collateral-usd-susdt (who principal))
  (let (
    (asset-balance (unwrap-panic (contract-call? .zsusdt-v1-2 get-principal-balance who)))
...
(define-read-only (get-supplied-balance-user-usdh (who principal))
  (let ((principal (unwrap-panic (contract-call? .zusdh-v1-2 get-principal-balance who))))
