...
(define-non-fungible-token duck-inu uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWZ82MMp3BE4XyF55GWGVK31C4jGYeb7AjGPvaLi6tDvf/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? duck-inu next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? duck-inu token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? duck-inu token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? duck-inu token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? duck-inu id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? duck-inu id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? duck-inu id) (err ERR-NOT-FOUND)))
