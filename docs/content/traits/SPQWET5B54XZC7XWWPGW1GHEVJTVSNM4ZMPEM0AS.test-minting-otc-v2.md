...
(define-public (confirm-redeem (request-id (string-ascii 36)) (price  uint) (amount-usdh-confirmed uint))
  (let (
    ...
    (try! (as-contract (contract-call? .test-usdh-token burn-for-protocol amount-usdh-confirmed-minus-fee tx-sender)))
    ...
    (if (not (is-eq amount-usdh-requested amount-usdh-confirmed))
      (try! (as-contract (contract-call? .test-usdh-token transfer (- amount-usdh-requested amount-usdh-confirmed) tx-sender (get requester redeem-request) none)))
      true
    )
