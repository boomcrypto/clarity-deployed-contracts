...
(define-data-var proposal-count uint u15)
...
(define-public (propose-public
  ...
  (let (
    (supply (unwrap-panic (contract-call? .wrapped-lydian-token get-total-supply)))
...
(define-public (end-proposal (proposal-id uint) (proposal-trait <lydian-dao-proposal-trait>))
  (let (
    ...
    (if (> (get yes-votes proposal) (get no-votes proposal))
      (begin
        (try! (as-contract (contract-call? .lydian-dao-executor-v1-1 execute-proposal proposal-trait)))
        true
      )
      false
    )
