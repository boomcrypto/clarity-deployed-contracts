...
(define-non-fungible-token blobnames3 uint)

;; Constants
(define-constant DEPLOYER tx-sender)
(define-constant COMM u1000)
(define-constant COMM-ADDR 'STK9FCNG823TEH0JD64RKQXMQMAZ0K69TCP9WXMD)
...
;; Internal variables
(define-data-var mint-limit uint u51)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1000000)
(define-data-var artist-address principal 'ST3BVZ44BGYS0VM277KET4M5T8EDKN5PN02D9MPXJ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUzKMPLAzr4MYkavdbQgjmZ3QPgmYo3nWVkY7bruSFXXi/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? blobnames3 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? blobnames3 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? blobnames3 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? blobnames3 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? blobnames3 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? blobnames3 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? blobnames3 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u2000)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u3000)
