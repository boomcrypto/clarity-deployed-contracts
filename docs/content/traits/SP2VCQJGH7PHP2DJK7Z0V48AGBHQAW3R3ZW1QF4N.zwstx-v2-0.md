...
(define-constant asset-addr 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.wstx)
(define-constant decimals u6)

(define-read-only (get-total-supply)
  (contract-call? .zwstx-token get-total-supply))

(define-read-only (get-name)
  (contract-call? .zwstx-token get-name))

(define-read-only (get-symbol)
  (contract-call? .zwstx-token get-symbol))

(define-read-only (get-decimals)
  (contract-call? .zwstx-token get-decimals))

(define-read-only (get-token-uri)
  (contract-call? .zwstx-token get-token-uri))
...
(define-private (transfer-internal (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (contract-call? .zwstx-token transfer amount sender recipient memo)
)

(define-private (mint-internal (amount uint) (owner principal))
  (contract-call? .zwstx-token mint amount owner)
)

(define-private (burn-internal (amount uint) (owner principal))
  (contract-call? .zwstx-token burn amount owner)
)
...
(define-read-only (get-principal-balance (account principal))
  (ok
    (contract-call? .zwstx-token get-balance account)
  )
)
