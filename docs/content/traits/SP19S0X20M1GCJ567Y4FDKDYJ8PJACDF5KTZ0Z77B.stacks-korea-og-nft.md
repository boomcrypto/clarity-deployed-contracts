---
title: "Trait stacks-korea-og-nft"
draft: true
---
```
(impl-trait 'SP19S0X20M1GCJ567Y4FDKDYJ8PJACDF5KTZ0Z77B.nft-trait.nft-trait)
(use-trait commission-trait 'SP19S0X20M1GCJ567Y4FDKDYJ8PJACDF5KTZ0Z77B.commission-trait.commission)

(define-non-fungible-token stacks-korea-og-nft uint)
...
(define-constant TOKEN-LIMIT u1000)
...
(define-data-var base-token-uri (string-ascii 210) "ipfs://QmaxRaKyM6BoYUtWq9dBRWR7AfZNcG2Eh9g59959HA4SQ2/")
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-korea-og-nft id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace stacks-korea-og-nft
  (ok (nft-get-owner? stacks-korea-og-nft id)))
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (match (nft-mint? stacks-korea-og-nft next-id new-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-korea-og-nft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-korea-og-nft id) ERR-NOT-FOUND))
...
(define-public (burn (id uint))
    (let ((owner (unwrap! (nft-get-owner? stacks-korea-og-nft id) ERR-NOT-AUTHORIZED)))
        (asserts! (is-eq owner contract-caller) ERR-NOT-OWNER)
        (map-delete market id)
        (nft-burn? stacks-korea-og-nft id contract-caller)
    )
)
