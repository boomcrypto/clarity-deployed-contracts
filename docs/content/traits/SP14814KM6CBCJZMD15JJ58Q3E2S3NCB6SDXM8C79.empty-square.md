...
(define-non-fungible-token empty-square uint)
...
(define-data-var total-price uint u500000000)
(define-data-var artist-address principal 'SP14814KM6CBCJZMD15JJ58Q3E2S3NCB6SDXM8C79)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQEKkpXpaW7d8iSPoc21e3UG1BjgDmM7cQ5gpxVDwxd2E/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? empty-square next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? empty-square token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? empty-square token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? empty-square token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? empty-square id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? empty-square id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? empty-square id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u969233)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u210000000)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u29874900000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u50522600000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u810000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u3847000000)
