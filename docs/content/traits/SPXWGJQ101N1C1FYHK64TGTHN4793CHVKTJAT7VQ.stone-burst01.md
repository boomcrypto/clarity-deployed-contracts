...
(define-constant ERR-STX-LIMIT-EXCEEDED (err u606))
(define-constant ERR-STX-LIMIT-HEARTS (err u607))

(define-constant MAX-STX-HEARTS u4000000)
(define-constant MAX-STX-DIAMONDS u1000000)
...
(define-data-var KRYPTOMIND principal 'SP1EB05PBRZKX1FTKT61NZQAW27KMKTTFJ47FQWQ1)
...
(define-public (buy-diamonds (diamond-count uint))
  (begin
    ...
    (let ((required-stx
      ...
      (asserts! (<= required-stx MAX-STX-DIAMONDS) ERR-STX-LIMIT-EXCEEDED)
...
(define-public (buy-hearts (heart-count uint))
  (begin
    ...
    (let ((required-stx (if (is-eq heart-count u3) u1000000
      ...
      (asserts! (>= (stx-get-balance tx-sender) required-stx) INVALID-STX-BALANCE)
      (asserts! (<= required-stx MAX-STX-HEARTS) ERR-STX-LIMIT-HEARTS)
      (let ((share-to-progressive (/ (* required-stx (var-get percentage-progressive)) u100))
...
(define-public (enter-contest)
  (begin
    (asserts! (>= (stx-get-balance tx-sender) u100000) INVALID-STX-BALANCE)
    (try! (stx-transfer? u100000 tx-sender (var-get PROGRESSIVE_WALLET)))
    (var-set progressive-pool (+ (var-get progressive-pool) u100000))
    (map-set user-contest-inventory tx-sender {hearts: u3, diamonds: u10})
    (map-set user-diamonds {user: tx-sender} {diamonds: u10})
