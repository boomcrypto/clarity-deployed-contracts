---
title: "Trait neoswap-sc-1691943241657"
draft: true
---
```
;; multiway-swap
...
;; Room: https://neoswap.xyz/rooms/RlZPFCowNS5NglkF0MsV
;; SwapId: bXVfsv4X5D1y5TBDPK6K
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u1600000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u1600000 tx-sender TRADER-1)))
