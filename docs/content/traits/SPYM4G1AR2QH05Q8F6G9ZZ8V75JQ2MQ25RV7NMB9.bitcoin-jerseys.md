...
(define-non-fungible-token bitcoin-jerseys uint)
...
(define-data-var artist-address principal 'SPYM4G1AR2QH05Q8F6G9ZZ8V75JQ2MQ25RV7NMB9)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmX6daSahNX83nhf59qPwbPD31cottMbKJKbpiBmiFDreP/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-jerseys next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-jerseys token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-jerseys id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-jerseys id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-jerseys id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u2500)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u2500)
