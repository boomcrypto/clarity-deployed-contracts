...
(define-fungible-token wtothemoon)

(define-data-var token-name (string-ascii 32) "To The Moon Wrapper")
(define-data-var token-symbol (string-ascii 32) "wtothemoon")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wtothemoon.json"))
...
(define-read-only (get-base-token)
  'SP2HH1X2KVS1M0BS15ZE42JXJFWNBVPH7C672XEKP.to-the-moon)

(define-read-only (get-base-decimals)
  (contract-call? 'SP2HH1X2KVS1M0BS15ZE42JXJFWNBVPH7C672XEKP.to-the-moon get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP2HH1X2KVS1M0BS15ZE42JXJFWNBVPH7C672XEKP.to-the-moon get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP2HH1X2KVS1M0BS15ZE42JXJFWNBVPH7C672XEKP.to-the-moon transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
