...
(define-fungible-token bsc-ghiblicz)

(define-data-var token-name (string-ascii 32) "GhibliCZ")
(define-data-var token-symbol (string-ascii 10) "Ghibli")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/bsc-ghiblicz.json"))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(try! (ft-transfer? bsc-ghiblicz amount sender recipient))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance bsc-ghiblicz who)))

(define-read-only (get-total-supply)
	(ok (ft-get-supply bsc-ghiblicz)))
...
(define-public (mint (amount uint) (recipient principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? bsc-ghiblicz amount recipient)))
...
(define-public (burn (amount uint) (sender principal))
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? bsc-ghiblicz amount sender)))
