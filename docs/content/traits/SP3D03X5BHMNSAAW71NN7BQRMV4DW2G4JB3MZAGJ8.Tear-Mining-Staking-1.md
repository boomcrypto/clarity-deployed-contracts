---
title: "Trait Tear-Mining-Staking-1"
draft: true
---
```
...
(define-constant STAKING_AND_TEAR_FUNDING_FIRST_BLOCK u91981)
...
(define-constant MIN_BLOCK_CONFIRMS_TO_CLAIM_REWARD u20)
...
(define-constant BLOCKS_PER_CYCLE u216)
...
(define-public (claim-reward-block (block uint))
  (let
    ...
    (ok (match (as-contract (contract-call? .Tear-Token-1 mint reward miner))
      returnOk reward
      returnErr u0
    ))
  )
...
(define-public (stake-many-cycles (uTEARAmount uint) (lockCycles uint))
  (let
    ...
    (try! (contract-call? .Tear-Token-1 transfer uTEARAmount tx-sender STAKING_POOL))
...
(define-public (claim-reward-cycle (cycle uint))
  (let
    ...
    (if (> releaseUTEAR u0)
      (try! (as-contract (contract-call? .Tear-Token-1 transfer releaseUTEAR STAKING_POOL claimer)))
      true
    )
...
(define-private (get-token-max-supply)
  (match (var-get tokenMaxSupply)
    ...
    (let
      ((newTokenMaxSupply (unwrap! (contract-call? .Tear-Token-1 get-max-supply) u0)))
