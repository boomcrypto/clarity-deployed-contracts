...
(define-public (swap-helper-in (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dx uint) (min-dy (optional uint)) (pool <pool-trait>))
    ...
    (let 
        ...
        (try! (transfer-out-internal token-y-trait sender (try! (swap-helper-internal false token-x-trait token-y-trait factor amount-in min-dy pool))))
...
(define-public (swap-helper-out (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dx uint) (min-dy (optional uint)) (pool <pool-trait>))
    ...
    (let 
        (
            (sender tx-sender)
            (delta (try! (swap-helper-internal true token-x-trait token-y-trait factor dx min-dy pool)))
...
(define-public (swap-helper-a-in (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (token-z-trait <ft-trait>) (factor-x uint) (factor-y uint) (dx uint) (min-dz (optional uint)) (pool <pool-trait>))
    ...
    (let 
        ...
        (try! (transfer-in-internal token-x-trait dx))
        (try! (transfer-out-internal token-z-trait sender (try! (swap-helper-a-internal false token-x-trait token-y-trait token-z-trait factor-x factor-y amount-in min-dz pool))))
        (print {extra-fee: (try! (balance-of-this token-x-trait)), receiver: (var-get fee-receiver)})
        (ok (try! (transfer-out-internal token-x-trait (var-get fee-receiver) (try! (balance-of-this token-x-trait)))))
    )
)
...
(define-public (swap-helper-a-out (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (token-z-trait <ft-trait>) (factor-x uint) (factor-y uint) (dx uint) (min-dz (optional uint)) (pool <pool-trait>))
    ...
    (let 
        (
            (sender tx-sender)
            (delta (try! (swap-helper-a-internal true token-x-trait token-y-trait token-z-trait factor-x factor-y dx min-dz pool)))
            (amount-out (/ (* delta (- u10000 (var-get fee-point))) u10000))
        )
...
(define-public (swap-helper-b-in
        ...
        (dx uint) (min-dw (optional uint)) (pool <pool-trait>))
    ;; #[allow(unchecked_data)]
    (let 
        (
            (sender tx-sender)
        ...
        (try! (transfer-in-internal token-x-trait dx))
        (try! (transfer-out-internal token-w-trait sender (try! (swap-helper-b-internal false token-x-trait token-y-trait token-z-trait token-w-trait factor-x factor-y factor-z amount-in min-dw pool))))
        (print {extra-fee: (try! (balance-of-this token-x-trait)), receiver: (var-get fee-receiver)})
        (ok (try! (transfer-out-internal token-x-trait (var-get fee-receiver) (try! (balance-of-this token-x-trait)))))
    )
)
...
(define-public (swap-helper-b-out
    ...
    (let 
        (
            (sender tx-sender)
            (delta (try! (swap-helper-b-internal true token-x-trait token-y-trait token-z-trait token-w-trait factor-x factor-y factor-z dx min-dw pool)))
            (amount-out (/ (* delta (- u10000 (var-get fee-point))) u10000))
        )
...
(define-public (swap-helper-c-in
        ...
        (dx uint) (min-dv (optional uint)) (pool <pool-trait>))
    ;; #[allow(unchecked_data)]
    (let 
        (
            (sender tx-sender)
        ...
        (try! (transfer-in-internal token-x-trait dx))
        (try! (transfer-out-internal token-v-trait sender (try! (swap-helper-c-internal false token-x-trait token-y-trait token-z-trait token-w-trait token-v-trait factor-x factor-y factor-z factor-w amount-in min-dv pool))))
        (print {extra-fee: (try! (balance-of-this token-x-trait)), receiver: (var-get fee-receiver)})
        (ok (try! (transfer-out-internal token-x-trait (var-get fee-receiver) (try! (balance-of-this token-x-trait)))))
    )
)
...
(define-public (swap-helper-c-out
    ...
    (let 
        (
            (sender tx-sender)
            (delta (try! (swap-helper-c-internal true token-x-trait token-y-trait token-z-trait token-w-trait token-v-trait factor-x factor-y factor-z factor-w dx min-dv pool)))
            (amount-out (/ (* delta (- u10000 (var-get fee-point))) u10000))
        )
...
(define-private (swap-helper-internal
        ...
        (factor uint)
        (dx uint) (min-dy (optional uint)) (pool <pool-trait>))
    ;; #[allow(unchecked_data)]
    (let 
        (
            (sender tx-sender)
...
(define-private (swap-helper-a-internal
        ...
        (dx uint) (min-dz (optional uint)) (pool <pool-trait>))
    ;; #[allow(unchecked_data)]
    (let 
        (
            (sender tx-sender)
...
(define-private (swap-helper-b-internal
    ...
    (let 
        ...
        (if fixed
            (try! (contract-call? token-x-trait transfer-fixed dx sender (as-contract tx-sender) none))
            true)
        (try! (as-contract (contract-call? pool swap-helper-b token-x-trait token-y-trait token-z-trait token-w-trait factor-x factor-y factor-z dx min-dw)))
...
(define-private (swap-helper-c-internal
    ...
    (let 
        ...
        (if fixed
            (try! (contract-call? token-x-trait transfer-fixed dx sender (as-contract tx-sender) none))
            true)
        (try! (as-contract (contract-call? pool swap-helper-c token-x-trait token-y-trait token-z-trait token-w-trait token-v-trait factor-x factor-y factor-z factor-w dx min-dv)))
