---
title: "Trait pontis-price-oracle"
draft: true
---
```
(impl-trait .pontis-price-oracle-trait.pontis-price-oracle-trait)

;; For calculation convenience all prices are multiplied by 10^36, price is represented for 1 absolute value without decimals, for example:
;; STX price will be for 0.000001 ~ 2.7 * 10^30
...
(define-map prices-map (buff 26) uint)

(define-data-var last-update-block uint u0)

(define-private (set-price-fold (id uint) (state (response { asset-names: (list 1000 (buff 26)), asset-prices: (list 1000 uint) } uint)))
	(let
		(
			(unwrapped-state (try! state))
			(asset-name (unwrap! (element-at? (get asset-names unwrapped-state) id) ERR-WRONG-ARRAY-SIZE))
			(asset-price (unwrap! (element-at? (get asset-prices unwrapped-state) id) ERR-WRONG-ARRAY-SIZE))
		)
		(map-set prices-map asset-name asset-price)
		state
	)
)

(define-public (set-prices (asset-names (list 1000 (buff 26))) (asset-prices (list 1000 uint)))
	(let
		(
			(it (try! (get-iterator (len asset-names))))
		)
    (asserts! (is-eq (len asset-names) (len asset-prices)) ERR-WRONG-ARRAY-SIZE)
		(try! (contract-call? .pontis-bridge-controller authorize-bridge-owner))
    (var-set last-update-block stacks-block-height)

		(fold set-price-fold it (ok { asset-names: asset-names, asset-prices: asset-prices }))
	)
)

(define-read-only (get-price (asset-name (buff 26)))
	(ok (map-get? prices-map asset-name))
)

(define-read-only (get-last-update-block-height)
