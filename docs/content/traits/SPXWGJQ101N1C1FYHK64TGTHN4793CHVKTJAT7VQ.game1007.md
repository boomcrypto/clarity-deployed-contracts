...
(define-private (swap (stx-amount uint))
  (begin 
    ;; Ensure the stx-amount is greater than 0
    (asserts! (> stx-amount u0) ERR-INSUFFICIENT-FUNDS)

    ;; Call the public swap function and capture the result
    (let ((swap-result
           (contract-call? 
      ...
      ;; Extract the amt-out from the swap result
