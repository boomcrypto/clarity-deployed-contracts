---
title: "Trait farming-wstx-lsodin-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .wstx-lsodin)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .wstx-lsodin transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .wstx-lsodin get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .wstx-lsodin transfer amt protocol user none)))
