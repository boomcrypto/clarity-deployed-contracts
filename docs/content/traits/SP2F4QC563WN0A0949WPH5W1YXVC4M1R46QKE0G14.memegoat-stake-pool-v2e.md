---
title: "Trait memegoat-stake-pool-v2e"
draft: true
---
```
;;
...
(define-private (move (data {user: principal, old-id: uint}) (stake-id uint))
  (let
    (
      ...
      (u-has-stake (contract-call? .memegoat-stake-pool-v2 get-user-stake-has-stake old-id user))
      (user-stake (unwrap-panic (contract-call? .memegoat-stake-pool-v2 get-user-staking-data old-id user)))
    )
    (if u-has-stake
      (begin
