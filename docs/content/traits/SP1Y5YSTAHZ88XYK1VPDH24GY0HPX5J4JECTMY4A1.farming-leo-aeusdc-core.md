---
title: "Trait farming-leo-aeusdc-core"
draft: true
---
```
...
(define-constant STAKING-TOKEN .leo-aeusdc)
...
(define-public (stake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (contract-call? .leo-aeusdc transfer amt user protocol none))
    ...
    ;; Postconditions
    (asserts!
      (>= (unwrap-panic (contract-call? .leo-aeusdc get-balance protocol))
...
(define-public (unstake (amt uint))
  (let ((user      tx-sender)
    ...
    ;; Update global state
    (try! (as-contract (contract-call? .leo-aeusdc transfer amt protocol user none)))
