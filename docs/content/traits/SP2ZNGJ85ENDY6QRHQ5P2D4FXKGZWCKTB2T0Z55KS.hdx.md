...
;; Protocol fee
(define-private (tap-and-pay)
  (begin
    (match (tap) success true error false)
    (match (contract-call? .charisma-rulebook-v0 pay-to-play) success true error false)))
...
(define-public (swap (forward bool) (amt-in uint))
  (let (
    ...
    (paid-energy (tap-and-pay))
    ;; Calculate effective input amount
    (effective-in (if paid-energy amt-in (/ (* amt-in (- FEE_DENOMINATION (var-get swap-fee))) 
                        FEE_DENOMINATION)))
