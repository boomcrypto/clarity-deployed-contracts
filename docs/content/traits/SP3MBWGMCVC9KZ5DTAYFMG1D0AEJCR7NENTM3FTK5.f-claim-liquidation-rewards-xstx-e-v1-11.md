---
title: "Trait f-claim-liquidation-rewards-xstx-e-v1-11"
draft: true
---
```
...
(define-public (execute)
  (let (
    ...
    (print { new-next-unlock-block: (var-get next-unlock-block) })
...
(define-private (claim-liquidation-rewards (reward-id uint) (total-reward-ids uint))
  (let (
    ...
    (if (and is-xstx (not reward-claimed) (< reward-id total-reward-ids))
      ...
      (if (< (get unlock-block reward-data) burn-block-height)
        (begin
          (try! (claim-reward-xstx reward-id))
          (print { reward-id: reward-id, check: "claim now" })
          (ok true)
        )
        ...
        (if (or (is-eq (var-get next-unlock-block) u0) (< (get unlock-block reward-data) (var-get next-unlock-block)))
          (begin
            ...
            (print { reward-id: reward-id, check: "update next block", value: (get unlock-block reward-data), next-block: (var-get next-unlock-block) })
            (ok false)
          )

          (begin
            (print { reward-id: reward-id, check: "not update next block", value: (get unlock-block reward-data), next-block: (var-get next-unlock-block) })
            (ok false)
          )
        )
      )
      ...
      (begin
        (print { reward-id: reward-id, check: "not stx, claimed or not existing" })
        (ok false)
      )
    )
  )
...
(define-private (claim-reward-xstx (reward-id uint))
  (begin
    ...
    ;; Redeem STX
    (try! (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-stacker-payer-v3-3 redeem-stx-helper 
