---
title: "Trait four-seasons-the-fall"
draft: true
---
```
;; four-seasons-the-fall
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token four-seasons-the-fall uint)
...
(define-data-var total-price uint u619000000)
(define-data-var artist-address principal 'SP35K3WCA9GCJV2XC7X021MR2D9D2PKF855CVCKB0)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeWbQNwEzsAvP9qee6B4nZfLn1bUVxD7oAfH6mxX9bvLa/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? four-seasons-the-fall next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? four-seasons-the-fall token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? four-seasons-the-fall token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? four-seasons-the-fall token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? four-seasons-the-fall id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? four-seasons-the-fall id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? four-seasons-the-fall id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u1117854)
