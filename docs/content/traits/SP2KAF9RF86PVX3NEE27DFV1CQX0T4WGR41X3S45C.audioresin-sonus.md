...
(define-non-fungible-token SONUS uint)
...
(define-constant ERR-METADATA-FROZEN (err u510))
(define-constant MINT-LIMIT u100)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmZFtSZesG71Zib7bonAQGh9FmvG5mZvkPH424uXQSb6MG/")
...
(define-data-var payout-one uint u4500)
(define-data-var payout-two uint u200)
(define-data-var payout-three uint u4500)
...
(define-data-var wallet-1 principal 'SP3J05S9NZBXTEA6F5R398TV6ZE104QSV39DFYWKE)
(define-data-var wallet-2 principal 'SP3J05S9NZBXTEA6F5R398TV6ZE104QSV39DFYWKE)
(define-data-var wallet-3 principal 'SP21C4YMBBN87AWZVRRC1G2VRM5J3PXKHZ4N4XDR4)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? SONUS id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace SONUS
  (ok (nft-get-owner? SONUS id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? SONUS next-id new-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? SONUS id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? SONUS id) ERR-NOT-FOUND))
