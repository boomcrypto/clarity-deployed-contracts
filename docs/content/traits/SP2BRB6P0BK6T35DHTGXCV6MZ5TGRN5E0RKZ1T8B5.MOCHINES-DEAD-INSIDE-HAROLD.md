---
title: "Trait MOCHINES-DEAD-INSIDE-HAROLD"
draft: true
---
```
;; nft Contract
...
(define-non-fungible-token Dead_Inside_Harold uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafkreiagvghthl3jo4ba2ple5a7dgtzfhlxaygpynekyh3syjrxcbypcvm")
...
(define-public (mint (recipient principal))
    (begin 
        ...
        (asserts! (< (var-get last-id) SUPPLY) (err ERR-NO-MORE-NFTS))
            ...
            (try! (nft-mint? Dead_Inside_Harold u1 tx-sender))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Dead_Inside_Harold token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Dead_Inside_Harold token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Dead_Inside_Harold token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Dead_Inside_Harold id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Dead_Inside_Harold id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Dead_Inside_Harold id) (err ERR-NOT-FOUND)))
