---
title: "Trait staking-migration-v2"
draft: true
---
```
...
(define-constant ERR-NOT-STAKED u602)

(define-data-var admin principal tx-sender)
(define-data-var migration-allowed bool false)
(define-data-var to-check (list 2500 uint) (list ))
...
(define-private (migrate-bull (item uint))
    (begin
        ...
        (try! (contract-call? .staking-helper set-stake-pass 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-bitcoin-bulls item true))
        (try! (contract-call? .staking-helper stake 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-bitcoin-bulls 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.bull-multipliers item))
        (ok true)))
...
(define-private (migrate-bear (item uint))
    (begin
        ...
        (try! (contract-call? .staking-helper set-stake-pass 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-bitcoin-bears item true))
        (try! (contract-call? .staking-helper stake 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-bitcoin-bears 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.bear-multipliers item))
        (ok true)))
...
(define-private (migrate-whale (item uint))
    (begin
        ...
        (try! (contract-call? .staking-helper set-stake-pass 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.bitcoin-whales item true))
        (try! (contract-call? .staking-helper stake 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.bitcoin-whales 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.whale-multipliers item))
        (ok true)))
...
(define-private (migrate-goat (item uint))
    (begin
        ...
        (try! (contract-call? .staking-helper set-stake-pass .bitcoin-goats item true))
        (try! (contract-call? .staking-helper stake .bitcoin-goats 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.goat-multipliers item))
        (ok true)))
...
(define-private (migrate-cavalry)
    (let (
        ...
        (asserts! (> (len staked-nfts) u0) (err ERR-NOT-STAKED))
        (if staking 
            (begin 
...
(define-public (migrate-redeem-list (ids (list 100 uint)))
    (let (
        (staked-nfts (contract-call? .spaghettipunk-x-minotauri-staking get-staked-nfts tx-sender))
    )
    (var-set to-check staked-nfts)
    (asserts! (var-get migration-allowed) (err ERR-MIGRATION-NOT-ALLOWED))
    (asserts! (is-eq (len (filter not-staked ids)) u0) (err ERR-NOT-STAKED))
    (asserts! (> (len ids) u0) (err ERR-NOT-STAKED))
    (try! (unwrap! (some (migrate-cavalry-list ids)) (err u202)))
    (ok true)))

(define-private (not-staked (id uint))
    (not (is-some (index-of (var-get to-check) id))))

(define-private (migrate-cavalry-list (ids (list 100 uint)))
    (let (
        (staking (> (len ids) u0))
        ) 
        (if staking 
            (begin 
                (map unstake-item ids)
                (try! (contract-call? .the-cavalry redeem ids))
                (map stake-item ids)
                true
            )
            true)
...
(define-private (stake-item (item uint))
    (let (
        ...
        (try! (contract-call? .staking-helper set-stake-pass .the-cavalry id true))
        (try! (contract-call? .staking-helper stake .the-cavalry .the-cavalry-multipliers id))
        (ok true)))

(try! (contract-call? .spaghettipunk-staking migration-change (as-contract tx-sender)))
(try! (allow-migration true))
(try! (contract-call? .staking-helper migration-change (as-contract tx-sender)))
```
