---
title: "Trait meta-bridge-endpoint-v2-03"
draft: true
---
```
...
(define-constant err-reveal-peg-in-address (err u1014))
...
(define-read-only (decode-order-from-reveal-tx-or-fail (tx (buff 8192)) (order-idx uint))
	(let (
			...
			(peg-in-script (get scriptPubKey (unwrap-panic (element-at? (get outs parsed-tx) u0))))
			...
			(ok { peg-in-script: peg-in-script, order-details: order-details })))
...
(define-read-only (validate-drop-tx
	...
	(let (
		...
		(asserts! (is-eq (get to commit-tx-idxed) (get peg-in-script reveal-tx-data)) err-reveal-peg-in-address)
