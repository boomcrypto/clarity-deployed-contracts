---
title: "Trait byzantion-fruitmen"
draft: true
---
```
...
(define-non-fungible-token fruitmen uint)
...
(define-constant MINT-LIMIT u555)

;; Internal variables
(define-data-var last-id uint u0)
(define-data-var uri-prefix (string-ascii 256) "")
(define-data-var cost-per-mint uint u25000000)
(define-data-var commission-per-mint uint u2000000)
(define-data-var payout uint u0)
(define-data-var ipfs-full-metadata (string-ascii 103) "ipfs://ipfs/bafybeibu47qfcxlsfrtg46edjmjbdmq6pomrf3b6chv2takp3if37x5j4e/fruitmen/fruitmen_metadata.json")
(define-data-var ipfs-root (string-ascii 90) "ipfs://ipfs/bafybeicg34abjutatr4ck3jausv3mdl4tnrhvwylz4xambsndmhx2s3fwa/fruitmen/fruitmen_")
(define-data-var artist-address principal 'SPA6FR2P1TJ9C5KDSXDXY30HHRXSFKNKCDWWCDN6)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? fruitmen next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? fruitmen token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? fruitmen token-id)))
