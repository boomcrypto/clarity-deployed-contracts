---
title: "Trait lottery-episode-1"
draft: true
---
```
;; Bitcoin decides whether you can win 6.btc with only 0.1 STX and 0.1 $ORDI.
;; https://lasereye.vip/#/lottery/bet
...
(define-constant ORDI_PRICE_PER_BET u10000000)        ;; 0.1 $ORDI/bet
...
(define-constant CALL_END_BET_REWARD u50000)          ;; Reward 0.05 STX to whom firstly clicks End-bet button
(define-constant CALL_DRAW_REWARD u100000)            ;; Reward 0.1 STX to whom firstly clicks Draw button
...
(define-constant RETURN_NAME_ADDRESS 'SP12H8VF8X3B8J51BDQ4S8HZB8RNM21CSWZ22G2N2)
(define-constant FOUNDER 'SP12H8VF8X3B8J51BDQ4S8HZB8RNM21CSWZ22G2N2)
...
(define-public (bet (bet_list (list 200 (string-ascii 5))))
  (let
    ...
    (if (is-eq round_bet_count u0)
      (begin
        (unwrap! (stx-transfer? (+ CALL_END_BET_REWARD CALL_DRAW_REWARD) caller (as-contract tx-sender)) (err ERR_TRANSFER_STX)) ;; one for end_bet, one for draw
        (unwrap! (stx-transfer? (- stx_cost CALL_END_BET_REWARD CALL_DRAW_REWARD) caller FOUNDER) (err ERR_TRANSFER_STX))
      )
...
(define-public (end_bet (btc_height uint))
  (let
    ...
    (as-contract (unwrap! (stx-transfer? CALL_END_BET_REWARD tx-sender caller) (err ERR_TRANSFER_STX)))
    (ok true)
  )
)
...
(define-public (draw)
  (let
    ...
    (if (> (len winners) u0)
      (begin
        (try! (contract-call? .lottery-episode-1-prize set_winners winners))
    ...
    (if (is-eq (var-get m_state) STATE_IDLE)
      (if (>= block-height RETURN_NAME_MIN_BLOCK)
        (begin
          ...
          (contract-call? .lottery-episode-1-prize return_name RETURN_NAME_ADDRESS)
        )
        (err ERROR_RETURN_NAME)
      )
...
(contract-call? .lottery-episode-1-prize set_lottery_contract (as-contract tx-sender))
