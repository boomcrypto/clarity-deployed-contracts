...
(define-non-fungible-token the-explorers uint)
...
(define-data-var artist-address principal 'SP2823PXH844YXS3Z6HZ1GTTWT79NB13GDSFY78FR)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNh7arRingCKzBzM2pfUg71CbkiKBnL78u7WiVGAWep7Q/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-explorers next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-explorers token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-explorers token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-explorers token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-explorers id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-explorers id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-explorers id) (err ERR-NOT-FOUND)))
