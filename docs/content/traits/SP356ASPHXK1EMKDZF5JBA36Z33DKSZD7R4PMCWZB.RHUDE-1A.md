...
(define-non-fungible-token RHUDE-1A uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafyreibts3pq5zqhga6clxf3dzcb4odmmvstpv77uq7k4xmzjvpwc2lyky/metadata.json")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? RHUDE-1A token-id)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? RHUDE-1A token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? RHUDE-1A token-id) false)))
...
(define-private (iterate-minting (not-to-be-used bool) (next-id uint))
  (if (<= next-id (var-get max-supply))
    (begin
      (unwrap! (nft-mint? RHUDE-1A next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? RHUDE-1A id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? RHUDE-1A id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? RHUDE-1A id) (err err-not-found)))
...
(define-private (airdrop-iter (collector principal) (next-id uint)) 
  (if (<= next-id (var-get max-supply))
    (let (
      ...
      (unwrap! (nft-mint? RHUDE-1A next-id collector) next-id)
