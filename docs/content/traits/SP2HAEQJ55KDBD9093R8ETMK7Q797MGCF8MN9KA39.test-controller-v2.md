---
title: "Trait test-controller-v2"
draft: true
---
```
;; @contract Controller
...
(define-public (log-reward (reward-usdh uint))
  (let (
    (total-usdh-supply (unwrap-panic (contract-call? .test-usdh-token-final get-total-supply)))
    (total-usdh-supply-staked (unwrap-panic (contract-call? .test-usdh-token-final get-balance .test-staking-v2)))
  )
    ...
    (if (> reward-usdh u0)
      (begin
        ...
        (try! (contract-call? .test-usdh-token-final mint-for-protocol reward-usdh .test-staking-v2))
      )
      true
    )
