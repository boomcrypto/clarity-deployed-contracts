---
title: "Trait byzantion-hero-of-miami"
draft: true
---
```
...
(define-non-fungible-token hero-of-miami uint)
...
(define-data-var ipfs-full-metadata (string-ascii 108) "ipfs://bafybeiaarb2hzik3f6ctlkhb3xs5vjvieeyn5xdauo62cxby4ml3fnecim/hero_of_miami/hero_of_miami_metadata.json")
(define-data-var ipfs-root (string-ascii 95) "ipfs://bafybeiaarb2hzik3f6ctlkhb3xs5vjvieeyn5xdauo62cxby4ml3fnecim/hero_of_miami/hero_of_miami_")
(define-data-var artist-address principal 'SP031H4KJD6RSF5YZT3DN9H5VYKMMZGR0N1C9M1N)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? hero-of-miami next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? hero-of-miami token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? hero-of-miami token-id)))
