---
title: "Trait multisig-crp-alex-autoalex"
draft: true
---
```
...
(define-private (execute-proposal (proposal-id uint))
  (let 
    ...
    (as-contract (try! (contract-call? .collateral-rebalancing-pool set-fee-rate-x .age000-governance-token .auto-alex expiry new-fee-rate-x)))
    (as-contract (try! (contract-call? .collateral-rebalancing-pool set-fee-rate-y .age000-governance-token .auto-alex expiry new-fee-rate-y)))
