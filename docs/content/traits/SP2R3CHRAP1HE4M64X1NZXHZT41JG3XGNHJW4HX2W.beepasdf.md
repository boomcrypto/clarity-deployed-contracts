...
(define-non-fungible-token beepasdf uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmUDzxa9FzsZQhrcKrEGZWzMqhYgwuxfLnv9k7i5vB2bBm/beepasdf/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP2R3CHRAP1HE4M64X1NZXHZT41JG3XGNHJW4HX2W )) 
...
(define-data-var public-target-block-height uint u64145)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u1000000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? beepasdf id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace beepasdf
  (ok (nft-get-owner? beepasdf id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? beepasdf next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? beepasdf id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? beepasdf id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? beepasdf id) ERR-NOT-FOUND))
