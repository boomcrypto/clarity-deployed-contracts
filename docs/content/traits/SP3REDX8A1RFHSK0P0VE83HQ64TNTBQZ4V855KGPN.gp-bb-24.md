---
title: "Trait gp-bb-24"
draft: true
---
```
;; Stacks Holiday Grandprize Boombox 

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)
(impl-trait 'SP1QK1AZ24R132C0D84EEQ8Y2JDHARDR58R72E1ZW.boombox-trait.boombox-trait)
(define-non-fungible-token grandprize-bb-24 uint)
...
(define-read-only (is-approved (id uint) (operator principal))
  (let ((owner (unwrap! (nft-get-owner? grandprize-bb-24 id) err-not-found)))
...
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (let ((owner (unwrap! (nft-get-owner? grandprize-bb-24 id) err-not-found)))
    ...
    (nft-transfer? grandprize-bb-24 id sender recipient)))
...
(define-read-only (get-token-uri (id uint))
  (ok (some "ipfs://QmXbDmopiDzinQA4NkMX7ThLFgQDLYzYGKc3RQgXTGnhSu/metadata/{id}.json")))
...
(define-public (mint (bb-id uint) (stacker principal) (amount-ustx uint) (pox-addr {version: (buff 1), hashbytes: (buff 20)}) (locking-period uint))
  (let ((next-id (+ u1 (var-get last-id))))
    ...
    (try! (nft-mint? grandprize-bb-24 next-id stacker))
    (ok next-id)))
