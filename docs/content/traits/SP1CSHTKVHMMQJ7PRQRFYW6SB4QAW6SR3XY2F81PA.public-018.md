...
(define-non-fungible-token public-018 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRu4aE3kQR1GUnEPDR3F96G2nLDMQpAmoQ1mpgkP4m8Ea/metadata/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? public-018 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? public-018 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? public-018 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? public-018 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? public-018 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? public-018 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? public-018 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia uint u2955000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u4095000000)
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u6290)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u40000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u505000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u10500000000)
