...
(define-non-fungible-token takish-nakamoto uint)
...
(define-data-var artist-address principal 'SP3PADVMHTYHWYN5EQTAPBKHFGZPZYTC6YT4DZD60)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVhFsvMwQBgyinWjZy8YHUrZspihGRPvzKVJsqZ8YdPxc/json/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u45)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? takish-nakamoto next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? takish-nakamoto token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? takish-nakamoto token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? takish-nakamoto token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? takish-nakamoto id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? takish-nakamoto id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? takish-nakamoto id) (err ERR-NOT-FOUND)))
