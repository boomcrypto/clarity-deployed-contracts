...
(define-non-fungible-token goedess uint)
...
(define-data-var ipfs-full-metadata (string-ascii 75) "ipfs://QmUAHj4WTqyTBXfCRCQvgWDh7HnXqKB8AGGqu8iYJUp46Q/goedess_metadata.json")
(define-data-var ipfs-root (string-ascii 62) "ipfs://QmUAHj4WTqyTBXfCRCQvgWDh7HnXqKB8AGGqu8iYJUp46Q/goedess_")
(define-data-var artist-address principal 'SPB6THCME050EBX2C991ZCT7R8W1CSA77QFK8NZ4)
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? goedess next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? goedess token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? goedess token-id)))
