...
(define-non-fungible-token trump-on-stacks uint)
...
;; Internal variables
(define-data-var mint-limit uint u2024)
(define-data-var last-id uint u1)
(define-data-var total-price uint u250000000)
(define-data-var artist-address principal 'SP2ZGCAHJ2GFECRG754NP98N4F18CP39HSP8PBNXA)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQBQJdHAcsX41jxxtwgX2Rx4ZffPpwtvaWN1rUk3hqRJg/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? trump-on-stacks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? trump-on-stacks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? trump-on-stacks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? trump-on-stacks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? trump-on-stacks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? trump-on-stacks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? trump-on-stacks id) (err ERR-NOT-FOUND)))
