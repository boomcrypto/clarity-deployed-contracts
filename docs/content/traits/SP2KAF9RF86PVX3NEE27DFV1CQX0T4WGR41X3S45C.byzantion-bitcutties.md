---
title: "Trait byzantion-bitcutties"
draft: true
---
```
...
(define-non-fungible-token bitcutties uint)
...
(define-constant MINT-LIMIT u340)
...
(define-data-var ipfs-full-metadata (string-ascii 107) "ipfs://ipfs/bafybeienktxve7rmc7awx26qzhpkxojim2eynfezisawttsgh7pcsrsg6y/bitcutties/bitcutties_metadata.json")
(define-data-var ipfs-root (string-ascii 94) "ipfs://ipfs/bafybeibnocelzyzd6cghren65lgfsk2w7s5wjuyjwzbm5rro5vuzctngdu/bitcutties/bitcutties_")
(define-data-var artist-address principal 'SP3PJBFHVSKYP33ZAEKQW8GQXWJYZ53S7AT0KYD4W)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? bitcutties next-id new-owner))
...
(define-public (set-root (ipfs (string-ascii 94))) 
  (if (is-eq tx-sender CONTRACT-OWNER)
...
(define-public (set-full (ipfs (string-ascii 107))) 
  (if (is-eq tx-sender CONTRACT-OWNER)
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? bitcutties token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcutties token-id)))
