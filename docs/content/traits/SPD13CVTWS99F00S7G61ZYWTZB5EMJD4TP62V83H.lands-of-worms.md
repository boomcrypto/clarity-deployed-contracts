...
(define-non-fungible-token lands-of-worms uint)
...
(define-data-var total-price uint u24000000)
(define-data-var artist-address principal 'SPD13CVTWS99F00S7G61ZYWTZB5EMJD4TP62V83H)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNWzk4pBy7vLyTGsju5X6aF56L6BAcQTje3VubNUuCALz/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? lands-of-worms next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? lands-of-worms token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? lands-of-worms token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? lands-of-worms token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? lands-of-worms id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? lands-of-worms id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? lands-of-worms id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u115419)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u71000000)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u121837000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u90842000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u486000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u6335000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u17200000000)
