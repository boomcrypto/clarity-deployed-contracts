---
title: "Trait bobba-fetts-lost-helmet"
draft: true
---
```
...
(define-non-fungible-token bobba-fetts-lost-helmet uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmRdh41LbgXHegFMByFcSZLEKJvwtFM35WcjD6ohK2jFjp/bobba-fetts-lost-helmet/")
...
(define-data-var public-target-block-height uint u63999)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u400000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bobba-fetts-lost-helmet id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace bobba-fetts-lost-helmet
  (ok (nft-get-owner? bobba-fetts-lost-helmet id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? bobba-fetts-lost-helmet next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? bobba-fetts-lost-helmet id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bobba-fetts-lost-helmet id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bobba-fetts-lost-helmet id) ERR-NOT-FOUND))
...
(define-data-var alex-cost-per-mint uint u500000)
