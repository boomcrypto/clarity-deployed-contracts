...
(define-non-fungible-token wealthy-apes-klub uint)
...
(define-data-var total-price uint u1350000)
(define-data-var artist-address principal 'SP1E7YK523YD7N3VRC7NDZS7HTYNBFKPXEWZ5DCTX)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdHnxTcvWEmUd6ZifKdgFh8Ym4aKdqrPpUvPa5U1xbguV/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? wealthy-apes-klub next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? wealthy-apes-klub token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? wealthy-apes-klub token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? wealthy-apes-klub token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? wealthy-apes-klub id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? wealthy-apes-klub id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? wealthy-apes-klub id) (err ERR-NOT-FOUND)))
