...
(define-private (transfer (amount uint) (recipient principal))
    (contract-call? .megacoin transfer amount (as-contract tx-sender) recipient none))
...
(define-public (withdraw)
  (begin
    (asserts! (is-eq tx-sender contract-owner) error-not-authorised)
    (try! (contract-call? .megacoin transfer (unwrap-panic (contract-call? .megacoin get-balance (as-contract tx-sender))) (as-contract tx-sender) contract-owner none))
    (ok true)))
