...
(define-non-fungible-token MinoTauri uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmPmS6CBmC11SKRLBTNjUTg5iHejxdRquGdcY4vjmPDubs/")
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? MinoTauri id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace MinoTauri
  (ok (nft-get-owner? MinoTauri id)))
...
(define-public (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? MinoTauri next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? MinoTauri id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? MinoTauri id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? MinoTauri id) ERR-NOT-FOUND))
...
(define-public (set-artist-address (addresses (list 1000 principal)))
  (begin
    (asserts! (or (is-some (index-of (var-get artist-address) tx-sender)) (is-eq tx-sender (var-get admin))) ERR-NOT-AUTHORIZED)
