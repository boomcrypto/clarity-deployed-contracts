...
(define-non-fungible-token TEST-K uint)
...
(define-data-var ipfs-full-metadata (string-ascii 85) "ipfs://QmZ7vvRqSinDTTUka1MdKBEMMyiDfm3Jx4pWaWDhBk/wolf_pack_academy_metadata.json")
(define-data-var ipfs-root (string-ascii 72) "ipfs://QmZ7vvRqSinDTTUka1MdKBEMMyiDfm3Jx4pWaWDhBkGZ/")
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? TEST-K  next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? TEST-K token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? TEST-K token-id)))
