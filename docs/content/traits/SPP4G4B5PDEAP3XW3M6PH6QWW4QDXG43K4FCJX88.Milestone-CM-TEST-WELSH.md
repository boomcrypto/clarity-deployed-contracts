---
title: "Trait Milestone-CM-TEST-WELSH"
draft: true
---
```
;; title: milestone
...
(define-constant ERR_WRONG_TOKEN (err u414))
...
(define-public (create-milestone-gig (artist principal) (period uint) (job-title (string-ascii 30))
 ...
 (fourth-milestone-amount uint) (fifth-milestone-amount uint) (pay-on <token>))
    ...
    (try! (contract-call? pay-on transfer total-amount tx-sender (as-contract tx-sender) none))
...
(define-public (accept-gig (gig-id uint) (token-name <token>))
(begin 
    ...
    (let ((gig-info (unwrap-panic (map-get? gig gig-id)))
      ...
      (as-contract (try! (contract-call? token-name transfer ( - amount fee) tx-sender (get to gig-info) none)))
      (as-contract (try! (contract-call? token-name transfer fee tx-sender (var-get contract-owner) none)))
...
(define-public (decline-gig (gig-id uint) (token-name <token>)) 
  (begin 
    ...
    (let ((gig-info (unwrap-panic (map-get? gig gig-id)))
      ...
      (asserts! (is-eq get-stage u0) ERR_ALREADY_ACCEPTED)
      (asserts! (<= block-height (+ (get block-created gig-info) (* u144 days))) ERR_EXPIRED)
      (as-contract (try! (contract-call? token-name transfer (get remaining-amount gig-info) tx-sender (get from gig-info) none)))
...
(define-public (complete-milestone (gig-id uint) (token-name <token>))
...
(define-public (redeem-fund (gig-id uint) (token-name <token>)) 
  (begin 
    (asserts! (is-eq (contract-of token-name) welshcorgicoin-address) ERR_WRONG_TOKEN)
    ...
    (let ((gig-info (unwrap-panic (map-get? gig gig-id)))) 
      ...
      (if (is-eq tx-sender (get from gig-info))
          (begin 
            ...
            (try! (as-contract (contract-call? token-name transfer (get remaining-amount gig-info)  tx-sender (get from gig-info) none)))
          )
          (let ((total-amount (get remaining-amount gig-info))
            ...
            (try! (as-contract (contract-call? token-name transfer (- (get remaining-amount gig-info) fee)  tx-sender (get to gig-info) none)))
            (try! (as-contract (contract-call? token-name transfer fee  tx-sender (var-get contract-owner) none)))
          )
      )
