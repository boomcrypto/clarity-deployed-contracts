...
(define-public (claim (claim-id uint))
  (let (
    ...
    (try! (contract-call? .test-usdh-token transfer (get amount current-claim) (as-contract tx-sender) (get recipient current-claim) none))
...
(define-public (create-claim (amount uint) (recipient principal))
  (let (
    ...
    (asserts! (is-eq contract-caller .test-staking) ERR_ONLY_STAKING_CONTRACT)
