...
(define-fungible-token wswoody)

(define-data-var token-name (string-ascii 32) "Swoody Wrapper")
(define-data-var token-symbol (string-ascii 32) "wswoody")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/token-wswoody.json"))
...
(define-read-only (get-base-token)
  'SP3SEJEBV9V6ZVS8AVSSGZ3X1ZEK163HX3HPHYDDP.swoody-stacks)

(define-read-only (get-base-decimals)
  (contract-call? 'SP3SEJEBV9V6ZVS8AVSSGZ3X1ZEK163HX3HPHYDDP.swoody-stacks get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP3SEJEBV9V6ZVS8AVSSGZ3X1ZEK163HX3HPHYDDP.swoody-stacks get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP3SEJEBV9V6ZVS8AVSSGZ3X1ZEK163HX3HPHYDDP.swoody-stacks transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))
;; ------
