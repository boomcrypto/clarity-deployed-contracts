---
title: "Trait meta-peg-in-v2-06e-agg"
draft: true
---
```
...
(define-read-only (create-order-agg-or-fail (order { from: (buff 128), to: (buff 128), token-in: principal, token-out: principal, min-amount-out: (optional uint), chain-id: (optional uint), dest-chain-id: uint, swap-token-in: principal, swap-token-out: principal }))
	(ok (unwrap! (to-consensus-buff? { f: (get from order), r: (get to order), i: (get token-in order), o: (get token-out order), m: (unwrap-panic (as-max-len? (match (get min-amount-out order) some-value (int-to-ascii some-value) "none") u40)), c: (unwrap-panic (as-max-len? (match (get chain-id order) some-value (int-to-ascii some-value) "none") u40)), d: (unwrap-panic (as-max-len? (int-to-ascii (get dest-chain-id order)) u40)), s: (get swap-token-in order), t: (get swap-token-out order) }) err-invalid-input)))

(define-read-only (decode-order-agg-or-fail (order-script (buff 512)) (offset uint))
	(let (
			(raw-order (unwrap! (from-consensus-buff? { f: (buff 128), r: (buff 128), i: principal, o: principal, m: (string-ascii 40), c: (string-ascii 40), d: (string-ascii 40), s: principal, t: principal } (unwrap-panic (slice? order-script offset (len order-script)))) err-invalid-input)))
...
(define-public (finalize-peg-in-agg 
  ...
  (let (
    ...
    (match (validate-tx-agg-extra validation-data)
      ...
      err-value
      (begin 
        (as-contract (try! (refund fee amt-net (get from order-details) token-in-trait (get chain-id (get pair-details validation-data)))))
...
(define-private (validate-tx-agg-extra
  (validation-data { 
    ...
    order-details: { from: (buff 128), to: (buff 128), token-in: principal, token-out: principal, min-amount-out: (optional uint), chain-id: (optional uint), dest-chain-id: uint, swap-token-in: principal, swap-token-out: principal }}))  
	(let (
      ...
      (pair-details (get pair-details validation-data))
			...
			(fail-settle { address: (get from order-details), chain-id: (some (get chain-id pair-details)), token: (get token-in order-details) }))      
		(try! (contract-call? .cross-peg-out-v2-01b-agg validate-transfer-to-swap tx-sender amount-in-fixed token-in token-out dest-chain-id success-settle fail-settle))
		(ok { amount-in-fixed: amount-in-fixed, token-in: token-in, token-out: token-out, min-amount-out: min-amount-out, dest-chain-id: dest-chain-id, success-settle: success-settle, fail-settle: fail-settle })))
```
