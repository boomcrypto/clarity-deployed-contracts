...
(define-non-fungible-token the-jumbo-punks uint)
...
(define-data-var artist-address principal 'SP34W1Y3D1ZF9Y62EK6509Q9BH8QDV1P9860MZWEC)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRWZXktmD6mDeQiSiiqyBwNH7VfjYEzG7bUSYt3gqNV2c/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-jumbo-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-jumbo-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-jumbo-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-jumbo-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-jumbo-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-jumbo-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-jumbo-punks id) (err ERR-NOT-FOUND)))
