---
title: "Trait neo-donk-classic-v3"
draft: true
---
```
...
(define-public (donk 
  ...
  (v { token-in: <ft-trait-b>, token-out: <ft-trait-b>, token-0: <ft-trait-b>, token-1: <ft-trait-b>, pool-id: uint })
  (direction uint)
  (amt-in uint)
  (amount-out-min uint)
)
      ...
      (let (
        ...
        (if (is-eq direction u0)
          ...
          (let (
            ...
            (v-amt-in (unwrap! (moon-math a-amt-out (contract-of a-token-y) (contract-of v-token-in)) (err u1)))
          ...
          (let (
            ...
            (a-amt-in (unwrap! (moon-math v-amt-out (contract-of v-token-out) (contract-of a-token-x)) (err u1)))
      ...
      (let (
        ...
        (if (is-eq direction u0)
          ...
          (let (
            ...
            (v-amt-in (unwrap! (moon-math a-amt-out (contract-of a-token-z) (contract-of v-token-in)) (err u1)))
            (v-amt-out (swap-exact-tokens-for-tokens 
              v-pool-id 
              v-token-0 
              v-token-1
...
(define-public (add-donks (l (list 100 principal)) (v (list 100 uint)))
  (begin
    (asserts! (is-owner) (err u2))
    (ok (map add-donk l v))
  )
)
