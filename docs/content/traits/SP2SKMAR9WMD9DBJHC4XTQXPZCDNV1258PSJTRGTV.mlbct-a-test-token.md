---
title: "Trait mlbct-a-test-token"
draft: true
---
```
(impl-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)

(define-fungible-token a-test-token)
...
(define-constant dev-principal 'SP2SKMAR9WMD9DBJHC4XTQXPZCDNV1258PSJTRGTV)
...
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://bafybeiaprm57cbl4sagsot3t6cnq4vnre5cnhmz3ocbtdjmkjgwewr5pa4.ipfs.dweb.link/")) 
...
(define-read-only (get-name) (ok "A Test Token"))
(define-read-only (get-symbol) (ok "tTEST"))
(define-read-only (get-decimals) (ok u0))
(define-read-only (get-balance (user principal)) (ok (ft-get-balance a-test-token user)))
(define-read-only (get-total-supply) (ok (ft-get-supply a-test-token)))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? a-test-token amount from to)
    )
)
...
(define-public (swap-stx-for-token (stx-amount uint) (min-tokens-out uint)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? a-test-token tokens-out (as-contract tx-sender) tx-sender))
      (if (>= (/ (* (+ (var-get real-stx-reserve) (var-get virtual-stx-reserve)) token-supply) (var-get token-reserve)) market-cap-threshold)
        (begin
          (let (
            ...
            (try! (ft-transfer? a-test-token burn-amount (as-contract tx-sender) burn-address))
            (try! (ft-transfer? a-test-token remaining-tokens (as-contract tx-sender) liquidity-builder-address))
...
(define-public (swap-token-for-stx (token-amount uint) (min-stx-out uint) (recipient principal)) 
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? a-test-token token-amount tx-sender (as-contract tx-sender)))
...
(begin
  (try! (ft-mint? a-test-token token-supply (as-contract tx-sender)))
