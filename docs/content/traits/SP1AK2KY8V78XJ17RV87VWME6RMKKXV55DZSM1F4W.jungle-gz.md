...
(define-non-fungible-token jungle-gz uint)
...
(define-data-var artist-address principal 'SP1AK2KY8V78XJ17RV87VWME6RMKKXV55DZSM1F4W)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRhwDgby49re2mDec612FCXpGEJN2yCGM9HPv8nyHWZFg/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? jungle-gz next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? jungle-gz token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? jungle-gz token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? jungle-gz token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? jungle-gz id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? jungle-gz id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? jungle-gz id) (err ERR-NOT-FOUND)))
