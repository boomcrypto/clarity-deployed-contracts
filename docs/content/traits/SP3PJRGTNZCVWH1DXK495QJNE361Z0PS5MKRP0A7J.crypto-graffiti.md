...
(define-constant artist-address 'SP8Q8QHDCZFG661H8431T2B1WX1FBD7PH76ZY1EF)
...
(define-constant total-nfts u188)

;; Set map metadata, price, and claimed
(map-set nft-data u1 {
  metadata: "ipfs://QmetnMZnroiSGpFBZTWubDkhtUPh2FgvXwP65db87Pw6Xi",
  price: u25000000,
  claimed: false
})
...
(map-set nft-data u7 {
  metadata: "ipfs://QmSf8vKEBQX1ApmyiyqyyWVFmxNCB2asFbrGcGVJyuhaJE",
  price: u25000000,
  claimed: false
})
...
(map-set nft-data u188 {
  metadata: "ipfs://QmZQ9dAdN65KUQWMewNxaJtQj8215nFPXDhgrTrStdHW1L",
  price: u1000000000,
  claimed: false
})

;; Define commission (basis points 1.00% = u100)
...
(define-public (set-commission (preferred-commission uint))
  (if (is-eq tx-sender artist-address)
...
(define-private (mint (new-owner principal) (nft-id uint))
  (match (map-get? nft-data nft-id)
    ...
    (let
      ...
      (if (and (is-eq tx-sender artist-address)
          (not (get claimed next-nft)))
        (mint-helper new-owner nft-id next-nft)
        (if (is-eq tx-sender commission-address)
          (begin
            (try! (stx-transfer? total-price tx-sender artist-address))
            (mint-helper new-owner nft-id next-nft))
          (begin
            (try! (stx-transfer? total-commission tx-sender commission-address))
            (try! (stx-transfer? total-artist tx-sender artist-address))
            (mint-helper new-owner nft-id next-nft)))))
