...
(define-non-fungible-token ueberkatzen uint)
...
(define-data-var artist-address principal 'SP21R1S8QBZKW9VGKHEECVN6AJKDC8PCS50X34BXN)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmToTgG692euv9Gq6qCvuuA9Bk46K23kapEPDykaMh3rY6/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? ueberkatzen next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? ueberkatzen token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ueberkatzen token-id)))
