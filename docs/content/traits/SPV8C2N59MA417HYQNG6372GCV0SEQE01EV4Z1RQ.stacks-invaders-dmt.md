---
title: "Trait stacks-invaders-dmt"
draft: true
---
```
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token stacks-invaders-dmt uint)
...
(define-private (mint-many (orders (list 25 bool )))  
  (let 
    ...
    (map-set blocks-gen { block-id: block-height } { token-id: last-nft-id , miner: tx-sender })
    (map-set generated-dmt { token-id: last-nft-id } { block-id: block-height , miner: tx-sender })
    ;; get whitelist
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-invaders-dmt next-id tx-sender) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-invaders-dmt token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-invaders-dmt token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-invaders-dmt token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-invaders-dmt id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-invaders-dmt id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-invaders-dmt id) (err ERR-NOT-FOUND)))
