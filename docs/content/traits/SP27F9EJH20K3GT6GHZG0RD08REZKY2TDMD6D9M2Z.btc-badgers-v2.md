---
title: "Trait btc-badgers-v2"
draft: true
---
```
;; bitcoin-badgers-nft-v2.5
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)
(define-non-fungible-token btc-badgers-nft-v2 uint)

;; This contract has two main parts to it
;; 1. v1 -> v2 "Burn"
;; Each Badger holder will call either burn one, burn two or burn ten to "burn" their v1 badger(s) & in exchange receive v2 badger(s)
;; The word "burn" is in quotes because technically we are only transferring from one address to this one*** which will later send to Stacks burn address
;; 1.5 Admin Mint / Airdrop
;; To rescue any edge-case Badger(s) stuck in listing
;; 2. 2nd / Final Mint
;; Last public mint (same time - block 40776) & same prices (69 stx & 5460 mia)
...
(define-data-var badgers-burned uint u0)
(define-data-var ipfs-root (string-ascii 102) "ipfs://ipfs/QmfJvo8zxL6FiTPewAzJnLxGqKvbXZ34BHxDkFEyduAoQw/bitcoin_badger_")
...
(define-constant ERR-V1-TRANSFER-POSSIBLY-STUCK (err u107))
...
;; v1 Burned
(define-read-only (burned)
  (ok (var-get badgers-burned))
)
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? btc-badgers-nft-v2 id))
)
...
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? btc-badgers-nft-v2 id sender recipient)
  )
)

;; 1. v1 -> v2 "Burn"
;; Each Badger holder will call either burn one, burn two or burn ten to "burn" their v1 badger(s) & in exchange receive v2 badger(s)
;; general burn function (transfer v1, transfers v2)
(define-public (burn-one (id uint))
  (begin
    ;; asserts that tx-sender is owner of associated v1 Badger
    (asserts! (is-eq tx-sender (unwrap-panic (unwrap-panic (contract-call? 'SP38FN88VZ97GWV0E8THXRM6Z5VMFPHFY4J1JEC5S.btc-badgers get-owner id)))) ERR-NOT-AUTHORIZED)
    ;; transfers v1 Badger from tx-sender to us
    (unwrap! (contract-call? 'SP38FN88VZ97GWV0E8THXRM6Z5VMFPHFY4J1JEC5S.btc-badgers transfer id tx-sender contract-owner) ERR-V1-TRANSFER-POSSIBLY-STUCK)
    ;; mints v2 Badger with same id to tx-sender
    (try! (nft-mint? btc-badgers-nft-v2 id tx-sender))
    (var-set badgers-burned (+ (var-get badgers-burned) u1))
    (ok u1)
  )
)

;; 1.5 Admin Mint/Aidrop For v1
;; Justttt in case we have some trouble unlisting from current marketplaces
;; This function accepts two inputs (principal - for recipient / original v1 owner & id - to specify which Badger)
(define-public (admin-mint-drop (recipient principal) (id uint))
  (begin
    ;; check tx-sender is contract owner
    (asserts! (is-eq tx-sender contract-owner) ERR-NOT-AUTHORIZED)
    ;; mint stuck badger (by id) & send to original recipient( by principal)
    (try! (nft-mint? btc-badgers-nft-v2 id recipient))
    (var-set badgers-burned (+ (var-get badgers-burned) u1))
    (ok u1)
  )
)
...
;; 2. 2nd / Final Mint
;; Last public mint (same time - block 40776) & same prices (69 stx & 5460 mia)
;; General claim function. Byz *always* sent mint-in-mia bool to FALSE; STX *always* set to TRUE
(define-public (claim-one (mint-in-mia bool))
  (let (
    ...
    (and
      ...
      (asserts! (< block-height first-public-mint-over-block-height) (err u102))
    )
    (begin
      (if mint-in-mia
        (begin
          ;; allows for minting in MIA, sends price to us
          ...
          ;; allows for minting in MIA, sends commission to Jamil
          (unwrap! (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.miamicoin-token transfer mint-commission-mia tx-sender commission-address-stxnft (some 0x00)) (err u102))
        )
        (unwrap! (stx-transfer? mint-price-stx tx-sender contract-owner) (err u104))
      )
      (try! (nft-mint? btc-badgers-nft-v2 next-id tx-sender))
...
;; Burn 2
(define-public (burn-two (id1 uint) (id2 uint))
  (begin
    (try! (burn-one id1))
    (try! (burn-one id2))
    (ok true)
...
;; Burn 10
(define-public (burn-ten (id1 uint) (id2 uint) (id3 uint) (id4 uint) (id5 uint) (id6 uint) (id7 uint) (id8 uint) (id9 uint) (id10 uint))
  (begin
    (try! (burn-one id1))
    (try! (burn-one id2))
    (try! (burn-one id3))
