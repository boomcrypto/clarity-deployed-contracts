---
title: "Trait rewards-sbtc-alex-v2"
draft: true
---
```
...
(define-public (initialize-contract)
  (let
    (
      (current-cycle-stx-block-height
...
(define-private (reset-state-for-cycle
    (stx-block-height uint)
    (stx-block-header-hash (buff 32))
  ) 
  (begin
    ...
    (var-set nr-snapshots-cycle
      (at-block stx-block-header-hash (yield-contract-nr-snapshots-cycle)))
    (var-set nr-blocks-snapshot
      (at-block stx-block-header-hash (yield-contract-nr-blocks-snapshot))
    )
  )
)
  ...
  (let
    (
      ...
      (next-snapshot-stx-block-height
        (unwrap!
          (contract-call?
              'SPQVH3QWEJYRFN9H4XRK1M55EWM5YMNYMVJ65EHJ.yield-rewards-sbtc-incentive-test-v3
            get-stacks-block-height-for-cycle-snapshot
        ...
        (print
          {
            ...
            reward-address: our-sbtc-address,
...
(define-public (head-to-next-cycle) 
  (let
    ...
    (reset-state-for-cycle next-cycle-stx-block-height stx-block-header-hash)
...
(define-read-only (get-is-yield-active) 
  (contract-call?   'SPQVH3QWEJYRFN9H4XRK1M55EWM5YMNYMVJ65EHJ.yield-rewards-sbtc-incentive-test-v3 get-is-contract-active)
)
...
(define-read-only (yield-contract-current-cycle-stacks-block-height)
  (get current-cycle-stacks-block-height (contract-call?   'SPQVH3QWEJYRFN9H4XRK1M55EWM5YMNYMVJ65EHJ.yield-rewards-sbtc-incentive-test-v3 cycle-data))
)

(define-read-only (yield-contract-cycle-id)
  (get cycle-id (contract-call?   'SPQVH3QWEJYRFN9H4XRK1M55EWM5YMNYMVJ65EHJ.yield-rewards-sbtc-incentive-test-v3 cycle-data))
)

(define-read-only (yield-contract-snapshot-index)
  (get current-snapshot-index (contract-call?   'SPQVH3QWEJYRFN9H4XRK1M55EWM5YMNYMVJ65EHJ.yield-rewards-sbtc-incentive-test-v3 cycle-data))
)

(define-read-only (yield-contract-nr-snapshots-cycle)
  (get nr-snapshots-cycle (contract-call?   'SPQVH3QWEJYRFN9H4XRK1M55EWM5YMNYMVJ65EHJ.yield-rewards-sbtc-incentive-test-v3 cycle-data))
)

(define-read-only (yield-contract-nr-blocks-snapshot)
  (get nr-blocks-snapshot (contract-call?   'SPQVH3QWEJYRFN9H4XRK1M55EWM5YMNYMVJ65EHJ.yield-rewards-sbtc-incentive-test-v3 cycle-data))
)
...
(define-read-only (can-head-to-next-cycle-or-snapshot)
  (let 
    (
      (yield-data (contract-call?   'SPQVH3QWEJYRFN9H4XRK1M55EWM5YMNYMVJ65EHJ.yield-rewards-sbtc-incentive-test-v3 current-overview-data))
...
;; Pool_1 (sBTC/aBTC)
(define-read-only (get-total-sBTC-balance-aBTC)
	(let (
			(pool-map (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01 get-pool-details-by-id u124))))
		(get balance-y (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01 get-pool-details (get token-x pool-map) (get token-y pool-map) (get factor pool-map)))))
)

(define-read-only (get-user-sBTC-balance-aBTC (address principal))
	(let (
			(total-balance (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-amm-pool-v2-01 get-total-supply u124)))
			(user-balance (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-amm-pool-v2-01 get-balance u124 address)))
			(surge-balance (get amount (contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.farming-campaign-v2-02 get-campaign-staker-or-default u2 u124 address))))
		(/ (* (+ user-balance surge-balance) (get-total-sBTC-balance-aBTC)) total-balance))
)

;; Pool_2 (sBTC/ALEX)
(define-read-only (get-total-sBTC-balance-ALEX)
	(let (
			(pool-map (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01 get-pool-details-by-id u125))))
		(get balance-y (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01 get-pool-details (get token-x pool-map) (get token-y pool-map) (get factor pool-map)))))
)

(define-read-only (get-user-sBTC-balance-ALEX (address principal))
	(let (
			(total-balance (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-amm-pool-v2-01 get-total-supply u125)))
			(user-balance (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-amm-pool-v2-01 get-balance u125 address)))
			(surge-balance (get amount (contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.farming-campaign-v2-02 get-campaign-staker-or-default u2 u125 address))))
		(/ (* (+ user-balance surge-balance) (get-total-sBTC-balance-ALEX)) total-balance))
)

;; Pool_3 (sBTC/STX)
(define-read-only (get-total-sBTC-balance-STX)
	(let (
			(pool-map (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01 get-pool-details-by-id u126))))
		(get balance-y (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.amm-pool-v2-01 get-pool-details (get token-x pool-map) (get token-y pool-map) (get factor pool-map)))))
)

(define-read-only (get-user-sBTC-balance-STX (address principal))
	(let (
			(total-balance (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-amm-pool-v2-01 get-total-supply u126)))
			(user-balance (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-amm-pool-v2-01 get-balance u126 address)))
			(surge-balance (get amount (contract-call? 'SP1E0XBN9T4B10E9QMR7XMFJPMA19D77WY3KP2QKC.farming-campaign-v2-02 get-campaign-staker-or-default u2 u126 address))))
		(/ (* (+ user-balance surge-balance) (get-total-sBTC-balance-STX)) total-balance))
)

;; User total sBTC from all pools
(define-read-only (get-user-total-sBTC-balance (address principal))
  (let (
    (balance-aBTC (get-user-sBTC-balance-aBTC address))
    (balance-ALEX (get-user-sBTC-balance-ALEX address))
    (balance-STX (get-user-sBTC-balance-STX address))
  )
    (+ balance-aBTC balance-ALEX balance-STX)
  )
)

(define-public (user-balances (address principal)) 
  (begin 
    (print
      {
        pool-abtc: (get-user-sBTC-balance-aBTC address),
        pool-stx: (get-user-sBTC-balance-STX address),
				pool-alex: (get-user-sBTC-balance-ALEX address),
        pool-total: (get-user-total-sBTC-balance address),
      }
    )
    (ok true)
  )
