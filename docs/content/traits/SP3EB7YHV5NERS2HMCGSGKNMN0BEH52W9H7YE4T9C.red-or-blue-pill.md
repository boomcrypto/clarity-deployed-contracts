...
(define-non-fungible-token red-or-blue-pill uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmR7LshzQjLAdcoZqrTe5GdkUkV3BUgH4Cdc146RkB6oUL/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? red-or-blue-pill next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? red-or-blue-pill token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? red-or-blue-pill token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? red-or-blue-pill token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? red-or-blue-pill id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? red-or-blue-pill id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? red-or-blue-pill id) (err ERR-NOT-FOUND)))
