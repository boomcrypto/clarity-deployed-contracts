...
(define-non-fungible-token satspace uint)
...
(define-data-var artist-address principal 'SP17H86VEV5VB5QKSP2Z5RE18FKM8DV9VMQXZM4DC)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZ1YdnYKiGWBQAnh6WehffqHYDroxYV4vVkAS8hY4ezBz/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? satspace next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? satspace token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? satspace token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? satspace token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? satspace id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? satspace id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? satspace id) (err ERR-NOT-FOUND)))
