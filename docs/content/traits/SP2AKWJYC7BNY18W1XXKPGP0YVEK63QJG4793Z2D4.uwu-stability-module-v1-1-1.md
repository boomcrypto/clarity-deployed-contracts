---
title: "Trait uwu-stability-module-v1-1-1"
draft: true
---
```
;; UWU STABILITY MODULE CONTRACT Version 1.1.1
...
(define-public (withdraw-token (token-trait <ft-trait>) (amount uint) (recipient principal))
  (let (
    (token-contract (contract-of token-trait))
    (token-data (unwrap! (map-get? tokens token-contract) ERR_INVALID_TOKEN))
    (sender tx-sender)
    ...
    (begin
      ...
      (print {action: "withdraw-token", sender: sender, token: token-contract, amount: amount, recipient: recipient})
      (ok true)
    )
  )
)
