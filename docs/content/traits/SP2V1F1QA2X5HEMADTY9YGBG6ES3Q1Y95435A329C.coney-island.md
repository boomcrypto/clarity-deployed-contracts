...
(define-non-fungible-token coney-island uint)
...
(define-data-var total-price uint u624000000)
(define-data-var artist-address principal 'SPJZ54R81XER8AACQQ44T770Z8HYSDHD07CKXVX0)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmfEF5JycMFd4BVk2QWacm1Kt7iuJw83mbbFZaQnuDvAzL/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? coney-island next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? coney-island token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? coney-island token-id)))
