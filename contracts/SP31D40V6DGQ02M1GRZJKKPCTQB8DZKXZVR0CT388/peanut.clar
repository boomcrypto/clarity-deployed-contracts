...
(define-non-fungible-token peanut uint)
...
(define-data-var artist-address principal 'SP3YC3HEMQKM9PJPX770D589938233M0VVGGS43DZ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSE8j6r5KHtvviH3X74YUwvZ33dkmyaWVvMDnk44eaYoF/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? peanut next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? peanut token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? peanut token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? peanut token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? peanut id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? peanut id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? peanut id) (err ERR-NOT-FOUND)))
