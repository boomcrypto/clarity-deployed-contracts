...
(define-constant ERR-UNCLAIMED-NFT u404)
(define-constant ERR-DUPLICATE-CLAIM u405)
(define-constant ERR-MINT-FAILED u100)
(define-constant ERR_TRX_FEES u102)
(define-constant ERR_STX_TRANSFER u101)
(define-constant CONTRACT-OWNER tx-sender)
(define-constant walt3r 'SP26RS42R5ZH10VWWG4HFYPRJRC3JJ3FKWY4V58CW)
(define-constant traklist 'SP3B08K0WR6H3SYW6HPERXV4TRMGJDWWBMNR51JXN)
(define-map nft-whitelist (string-ascii 36) bool)
...
(define-public (user-purchase-whitelist (price uint))
     (stx-transfer? price tx-sender traklist))

(define-public (bernard-claim-whitelist (nft-id (string-ascii 36)) (user-address principal))
  (begin
    (asserts! (is-eq tx-sender walt3r) (err ERR-NOT-AUTHORIZED))
    (map-set nft-whitelist nft-id true)
    (asserts! (is-eq (map-insert user-whitelist user-address true) true) (err ERR-DUPLICATE-CLAIM))
    
    (ok true)))

(define-public (user-claim-nft (nft-id (string-ascii 36)))
    (begin 
...
(define-public (traklist-pay (price uint) (recipient principal)) 
    (begin 
    (unwrap! (stx-transfer? u100000 tx-sender traklist)  (err ERR_TRX_FEES))
    (unwrap! (stx-transfer? price tx-sender recipient) (err ERR_STX_TRANSFER))
    (ok true)))