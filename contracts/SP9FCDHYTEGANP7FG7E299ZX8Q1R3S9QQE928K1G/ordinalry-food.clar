...
(define-non-fungible-token ordinalry-food uint)
...
(define-data-var artist-address principal 'SP9FCDHYTEGANP7FG7E299ZX8Q1R3S9QQE928K1G)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSaatH3QyN9oBrYhrb6ZHLRKMvuBrDp6L84Kx8djR8buk/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinalry-food next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinalry-food token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinalry-food token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinalry-food token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinalry-food id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinalry-food id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinalry-food id) (err ERR-NOT-FOUND)))
