...
(define-non-fungible-token goto uint)
...
(define-constant MINT-LIMIT u118)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmNhkunDDLg17AE6FBFv1nA6CeTQy9XqK9LCbRVdFq61da/goto/")
...
(define-data-var public-target-block-height uint u65554)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u100000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? goto id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace goto
  (ok (nft-get-owner? goto id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? goto next-id new-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? goto id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? goto id) ERR-NOT-FOUND))
