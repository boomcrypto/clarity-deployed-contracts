...
(define-non-fungible-token a-p-e uint)
...
(define-constant MINT-LIMIT u178)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmRoRsBwdFDARLTvSa4ReFsoGXVUwFTzRqmrjt1EXnEvNz/a-p-e/")
...
(define-data-var public-target-block-height uint u65937)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? a-p-e id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace a-p-e
  (ok (nft-get-owner? a-p-e id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? a-p-e next-id new-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? a-p-e id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? a-p-e id) ERR-NOT-FOUND))
