...
(define-non-fungible-token oil-dodo uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmW71gwP9DiVz3JyXc1i6EcYVjHhP1Q9ocCWvu5JSpoq5g/oil-dodo/")
...
(define-data-var public-target-block-height uint u65182)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? oil-dodo id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace oil-dodo
  (ok (nft-get-owner? oil-dodo id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? oil-dodo next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? oil-dodo id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? oil-dodo id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? oil-dodo id) ERR-NOT-FOUND))
