...
(define-non-fungible-token mars-warrior uint)
...
(define-constant MINT-LIMIT u245)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmQWj9sAPsMsow58kNSTmKjer7GfhDfAfDBba5vgoLTnZA/mars-warrior/")
...
(define-data-var public-target-block-height uint u65690)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? mars-warrior id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace mars-warrior
  (ok (nft-get-owner? mars-warrior id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? mars-warrior next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? mars-warrior id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? mars-warrior id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? mars-warrior id) ERR-NOT-FOUND))
