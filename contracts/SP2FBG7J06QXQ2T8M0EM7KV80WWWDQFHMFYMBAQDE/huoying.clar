...
(define-non-fungible-token huoying uint)
...
(define-constant MINT-LIMIT u438)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmQLaUdQDfj8zFxQv9TfBJEpZ4jqH7mZdAQ2hWFLkZroY1/huoying/")
...
(define-data-var artist-address (list 1000 principal) (list 'SPWGSV28BN5QMQ618CZAPDD18XPSN6EK776CDS84 )) 
...
(define-data-var public-target-block-height uint u65429)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? huoying id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace huoying
  (ok (nft-get-owner? huoying id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? huoying next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? huoying id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? huoying id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? huoying id) ERR-NOT-FOUND))
