...
(define-non-fungible-token test-collection-1 uint)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? test-collection-1 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
     (begin 
        ...
        (nft-burn? test-collection-1 token-id tx-sender))
)


(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? test-collection-1 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? test-collection-1 token-id)))
