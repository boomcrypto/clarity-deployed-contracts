...
(define-non-fungible-token lost-shipments-series-2 uint)
...
(define-data-var lost-shipments-series-2-snapshot-index uint u1)

;; Purchase Price - starting at 30 stx but will be able to be modified by admin
(define-data-var purchase-price uint u1)
...
(define-read-only (get-purchase-remaining) 
    (- (var-get collection-limit-purchase) (var-get lost-shipments-series-2-purchased-index))
)

;; Get Earn Remaining
(define-read-only (get-earn-remaining) 
    (- (var-get collection-limit-earn) (var-get lost-shipments-series-2-earned-index))
)

;; Get Airdrop Remaining
(define-read-only (get-snapshot-remaining) 
    (- (var-get collection-limit-snapshot) (var-get lost-shipments-series-2-snapshot-index))
)

;; Get Mints Remaining
(define-read-only (get-mints-remaining) 
    (- (var-get collection-limit-total) (var-get lost-shipments-series-2-index))
)
...
(define-read-only (get-last-token-id)
  (ok (var-get lost-shipments-series-2-index))
)

(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? lost-shipments-series-2 id))
)
...
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? lost-shipments-series-2 id sender recipient)
  )
)
...
(define-private (is-sender-owner (id uint))
  (let
    (
      (owner (unwrap! (nft-get-owner? lost-shipments-series-2 id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let
    (
      (owner (unwrap! (nft-get-owner? lost-shipments-series-2 id) ERR-NOT-AUTH))
    ...
    (try! (nft-transfer? lost-shipments-series-2 id owner tx-sender))
    (map-delete market id)
...
(define-public (mint-lost-shipment-purchase)
  (let
    (
      (current-id (var-get lost-shipments-series-2-index))
      (next-id (+ current-id u1))
      (current-purchase-id (var-get lost-shipments-series-2-purchased-index))
    ...
    ;; PI Fund (37%)
    (try! (stx-transfer? (/ (* (var-get purchase-price) u37) u100) tx-sender 'SP2DADKD5KK22MHMVN3DCSKS10T17CM7PDTC6WQV8))
    ;; Jon (26%) 
    (try! (stx-transfer? (/ (* (var-get purchase-price) u26) u100) tx-sender 'SP18J677R5GRD7EKK0S096WVQW19SDPWTC0TCBTGV))
    ;; Karel (25%)
    (try! (stx-transfer? (/ (* (var-get purchase-price) u25) u100) tx-sender 'SP1AD4C22XFTYTV12G0MCGSPGC1B6KP2H1FBJKHWE))
    ;; Jamie (12%)
    (try! (stx-transfer? (/ (* (var-get purchase-price) u12) u100) tx-sender 'SPZE89TY5HZPMHQGT0WGQ2HJHEJPHYF17YH825H6))

    ;; mint lost-shipment to tx-sender
    (try! (nft-mint? lost-shipments-series-2 current-id tx-sender))

    ;; update purchase index
    (var-set lost-shipments-series-2-purchased-index next-purchased)

    ;; update lost-shipments-index
    (ok (var-set lost-shipments-series-2-index next-id))
...
(define-public (mint-lost-shipment-earn (user principal))
  (let
    (
      (current-id (var-get lost-shipments-series-2-index))
      (next-id (+ current-id u1))
      (current-earn-id (var-get lost-shipments-series-2-earned-index))
    ...
    (try! (nft-mint? lost-shipments-series-2 current-id user))

    ;; update lost-shipments-index
    (var-set lost-shipments-series-2-index next-id)

    ;; update lost-shipments-earned
    (ok (var-set lost-shipments-series-2-earned-index next-earned))
  )
)
...
;; Reassign-allocation ;;
;;;;;;;;;;;;;;;;;;;;;;;;;
;; Public function for any eligible admin to change the collection allocation for purchase, earn and airdrop
...
(define-public (reassign-allocation (new-purchase uint) (new-snapshot uint) (new-earn uint)) 
  (let 
    (
      ...
      (mints-remaining (- collection-total (var-get lost-shipments-series-2-index)))
      (current-earn (var-get collection-limit-earn))
      (current-earn-minted (var-get lost-shipments-series-2-earned-index))
      (next-earn new-earn)
      (current-snapshot (var-get collection-limit-snapshot))
      (current-snapshot-minted (var-get lost-shipments-series-2-snapshot-index))
      (next-snapshot new-snapshot)
      (current-purchase (var-get collection-limit-purchase))
      (current-purchase-minted (var-get lost-shipments-series-2-purchased-index))
    ...
    (asserts! (is-eq total-sum (- collection-total u1)) ERR-INVALID-TOTAL)

    ;;asserts that the new-purchase limit is equal or bigger than current purchase index
    (asserts! (>= new-purchase (- current-purchase-minted u1)) ERR-INVALID-PARAMS)

    ;;asserts that the new-snapshot limit is equal or bigger than current snapshot index
    (asserts! (>= new-snapshot (- current-snapshot-minted u1)) ERR-INVALID-PARAMS)

    ;;asserts that the new-earn limit is equal or bigger than current earn index
    (asserts! (>= new-earn (- current-earn-minted u1)) ERR-INVALID-PARAMS)

    ;; set new snapshot
    (var-set collection-limit-snapshot (+ next-snapshot u1))

    ;; set new earn
    (var-set collection-limit-earn (+ next-earn u1))

    ;; set new purchase
    (ok (var-set collection-limit-purchase (+ next-purchase u1)))
...
(define-public (snapshot-airdrops (recipients (list 250 principal)))
  (let 
    (
      ...
      (current-snapshot-index (var-get lost-shipments-series-2-snapshot-index))
...
(define-private (mass-airdrop (recipient principal)) 
  (let 
    ...
    (try! (nft-mint? lost-shipments-series-2 current-id recipient))

    ;; increment snapshot index
    (var-set lost-shipments-series-2-snapshot-index new-snapshot-index) 
...
(define-public (burn-lost-shipment (lost-shipment-id uint))
  (begin 
    ...
    ;; assert that contract-caller is project-indigo-equipment-2.clar 
    (asserts! (is-eq contract-caller 'SP3D03X5BHMNSAAW71NN7BQRMV4DW2G4JB3MZAGJ8.p-i-e-test) ERR-NOT-AUTH)

    ;; burn lost-shipment
    (nft-burn? lost-shipments-series-2 lost-shipment-id tx-sender)
  )
)
