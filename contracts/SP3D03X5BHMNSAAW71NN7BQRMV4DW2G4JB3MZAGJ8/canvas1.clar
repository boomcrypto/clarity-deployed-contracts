...
(define-public (place-tile (position uint) (color (string-ascii 6)) (collection <nft-trait>) (collection-id (optional uint))) 
  (let
    (
      ...
      (current-user-tile-balance (unwrap! (contract-call? .tiles1 get-total-balance) (err u0)))
    ...
    (ok (unwrap! (contract-call? .tiles1 spend-tile) ERR-SPEND-TILE))
  )
)
...
;; Admin place-many tiles
;; @desc - Admin function for placing many tiles on the canvas at once
;; @param - Tile-List (list { position: uint, color: (string-ascii 6) })
(define-public (admin-place-tile-many (tiles (list 100 {position: uint, color: (string-ascii 6)}))) 
  (let 
    (
      (current-canvas-index (var-get canvas-index))
      (current-canvas-meta (unwrap! (map-get? canvas-meta current-canvas-index) ERR-UNWRAP-META))
      (current-placed-tiles (get placed-tiles current-canvas-meta))
      (current-placed-tiles-len (len current-placed-tiles))
      (current-end-height (get end-height current-canvas-meta))
      (current-minted-status (get minted current-canvas-meta))
    )
    ...
    ;; Assert len of tiles placed is less than current tiles reminaing (5000 - current-placed-tiles-len)
    (asserts! (< (len tiles) (- tiles-in-canvas current-placed-tiles-len)) ERR-TILE-LIST-OVERFLOW)
    
    (ok (map place-many tiles))

  )
)

;; Private helper function for placing many tiles on the canvas at once
;; THIS DOES NOT INCLUDE COLLECTIONS - ONLY FOR ADMIN TESTING
(define-private (place-many (tile-in-list {position: uint, color: (string-ascii 6)}))
  (let 
    (
      (current-canvas-index (var-get canvas-index))
      (current-canvas-meta (unwrap! (map-get? canvas-meta current-canvas-index) ERR-UNWRAP-META))
    ...
    ;; Assert that tile position is empty
    (asserts! (is-none (map-get? tile { canvas: current-canvas-index, tile: (get position tile-in-list)})) ERR-POSITION-NOT-EMPTY)

    ;; Assert color is valid
    (asserts! (is-eq (len (get color tile-in-list)) u6) ERR-NOT-ENOUGH-COLORS)

    ;; Update tile by adding new item to list
    (map-set tile {canvas: current-canvas-index, tile: (get position tile-in-list)} 
      (list {
        color: (get color tile-in-list),
        collection: none,
        owner: tx-sender
      })
    )
    ...
    (ok (map-set canvas-meta current-canvas-index 
      (merge 
        current-canvas-meta 
        { placed-tiles: (unwrap! (as-max-len? (append current-placed-tiles (get position tile-in-list)) u5000) ERR-TILE-LIST-OVERFLOW) }
      )
    ))
