...
;;define badger nft
(define-non-fungible-token baby-badger uint)

;; Badger(s) NFT collection limit (1k)
(define-constant baby-badger-limit u1000)
...
(define-data-var baby-badger-index uint u1)
...
(define-read-only (get-last-token-id)
  (ok (var-get baby-badger-index))
)

(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? baby-badger id))
)
...
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? baby-badger id sender recipient)
  )
)
...
;; @param none
(define-public (mint-baby-badger)
  (let
    (
      (current-id (var-get baby-badger-index))
      (next-id (+ current-id u1))
    )

    ;; mint container to tx-sender
    (try! (nft-mint? baby-badger current-id tx-sender))

    ;; update containers-index
    (ok (var-set baby-badger-index next-id))
