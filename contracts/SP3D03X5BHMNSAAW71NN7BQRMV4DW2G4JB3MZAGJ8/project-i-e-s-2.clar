...
(define-public (mint-equipment (lost-shipment-id uint))
  (let
    ...
    (asserts! (is-eq (some tx-sender) (unwrap! (contract-call? .project-i-l-s-s-2 get-owner lost-shipment-id) ERR-UNWRAP)) ERR-GET-OWNER-FAILED)

    ;; burn lost-shipments NFTs
    ;; (try! (contract-call? .project-indigo-lost-shipments-series-2 burn-lost-shipment lost-shipment-id))
    (try! (contract-call? .project-i-l-s-s-2 burn-lost-shipment lost-shipment-id))
