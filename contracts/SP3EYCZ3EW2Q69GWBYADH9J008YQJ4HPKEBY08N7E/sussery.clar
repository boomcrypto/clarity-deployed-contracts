...
(define-non-fungible-token sussery uint)
...
(define-data-var admin (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF 'SP3EYCZ3EW2Q69GWBYADH9J008YQJ4HPKEBY08N7E ))
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? sussery item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? sussery item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? sussery item-id)))
      (begin
        (try! (nft-burn? sussery item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sussery token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sussery id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sussery id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sussery id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP3EYCZ3EW2Q69GWBYADH9J008YQJ4HPKEBY08N7E u500 "sussery" u1000000 u65405 u65460 "ipfs://QmcgPUBdUzbBjRi3LJFVFTgpze1UZ6tpKQ2hASWPLGx4Kh/sussery/1.json"))
  (ok true)
)
