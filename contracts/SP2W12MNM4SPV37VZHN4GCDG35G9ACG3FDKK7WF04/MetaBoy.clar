...
;; Defining NFT
(define-non-fungible-token MetaBoy uint)
...
(define-data-var ipfs-root (string-ascii 100) "ipfs://ipfs/QmZSzf8FyEANymXwKKQQCbPijuG9cGCk9uFQn15DfAKkxx/json/")
;; Team royalty percentage per sale
...
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (begin
    ;; Asserting that sender is not recipient
    (asserts! (not (is-eq sender recipient)) ERR-NOT-AUTHORIZED) ;; Check if sender is not the recipient
    ;; Asserting that sender owns token -> not needed as nft-transfer? implicitly checks this but nice to have anyways
    (asserts! (is-eq (unwrap! (unwrap! (get-owner id) ERR-CANT-GET-OWNER) ERR-CANT-GET-OWNER) sender) ERR-NOT-AUTHORIZED) ;; Check if sender owns token
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? MetaBoy id sender recipient) ;;Transfer NFT from sender to recipient
    success
      (begin
...
(define-public (mint (id uint))
  (begin
    (asserts! (is-eq (unwrap! (unwrap! (contract-call? 'SP30TV0HVAAXY0H4K3JQ0YXC465X6G4GQCE018RSH.metaboy-cartridge-first-odyssey get-owner id) ERR-INVALID-USER) ERR-INVALID-USER) tx-sender) ERR-NOT-AUTHORIZED)
    (match (nft-mint? MetaBoy id tx-sender) ;; MINT
      success
        (begin
          (unwrap! (contract-call? 'SP30TV0HVAAXY0H4K3JQ0YXC465X6G4GQCE018RSH.metaboy-cartridge-first-odyssey burn id) ERR-CANT-BURN)
...
(define-private (mint-many-iter (id uint) )
    (begin
      (asserts! (is-eq (unwrap! (unwrap! (contract-call? 'SP30TV0HVAAXY0H4K3JQ0YXC465X6G4GQCE018RSH.metaboy-cartridge-first-odyssey get-owner id) ERR-INVALID-USER) ERR-INVALID-USER) tx-sender) ERR-NOT-AUTHORIZED)
      (match (nft-mint? MetaBoy id tx-sender)
        success
            (begin
              (unwrap! (contract-call? 'SP30TV0HVAAXY0H4K3JQ0YXC465X6G4GQCE018RSH.metaboy-cartridge-first-odyssey burn id) ERR-CANT-BURN)
              (var-set last-id (+ u1 (var-get last-id)))
...
(define-public (burn (id uint))
    ...
    (asserts! (is-none (map-get? market id)) ERR-LISTING)
    (match (nft-burn? MetaBoy id token-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? MetaBoy id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? MetaBoy id) ERR-NOT-FOUND))
...
;; Mint many NFTs, through array of ids
(define-public (claim-many (ids (list 500 uint)))
  (begin   
    (print (map claim-iter ids)) 
    (ok true)
  ))

(define-private (claim-iter (id uint) )
  (begin
    (match (nft-mint? MetaBoy id tx-sender) ;; MINT
      success
        (begin          
          (unwrap! (contract-call? 'SP2JTQV4N1HZJV0YRXTBPW7Q4C3148MC63WA3MD7Y.metaboy burn id) ERR-CANT-BURN)
          (var-set last-id (+ u1 (var-get last-id)))
          (map-set token-count ;; Increase the new-owner NFT count
            tx-sender
            (+ (get-balance tx-sender) u1)
