...
(define-non-fungible-token pizza-bb uint)
...
(define-constant MINT-LIMIT u188)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmdKrjRL98DmnrQtmMDya7XjJBt7wrR3nis5Lu8oRCJ38j/pizza-bb/")
...
(define-data-var public-target-block-height uint u70835)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u2000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pizza-bb id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace pizza-bb
  (ok (nft-get-owner? pizza-bb id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? pizza-bb next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? pizza-bb id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pizza-bb id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pizza-bb id) ERR-NOT-FOUND))
