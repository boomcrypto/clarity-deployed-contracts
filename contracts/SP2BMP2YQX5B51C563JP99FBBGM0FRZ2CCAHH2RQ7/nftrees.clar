...
(define-non-fungible-token nftrees uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZHMaaiZnjVNCfwAV5haD5h7Uxs8Ucv2tefj8pLQs8Rkb/")
...
(define-read-only (get-owner (token-id uint)) (ok (nft-get-owner? nftrees token-id)))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
    (begin
        ...
        (nft-transfer? nftrees token-id sender recipient)))
...
(define-public (mint (recipient principal))
    (let
        ...
        (try! (nft-mint? nftrees token-id recipient))
