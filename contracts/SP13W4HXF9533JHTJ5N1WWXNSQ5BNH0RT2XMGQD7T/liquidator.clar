...
(use-trait flash-loan-trait 'SP2ZS72524XRW1GWYSK8NHQ1QK83ESMD40G2HQ2AT.trait-flash-loan-v1.flash-loan)
(impl-trait 'SP2ZS72524XRW1GWYSK8NHQ1QK83ESMD40G2HQ2AT.trait-flash-loan-v1.flash-loan)
...
(define-public (on-granite-flash-loan (amount uint) (fee uint) (data (optional (buff 1024))))
  (let (
    ...
    (cdata (unwrap! (from-consensus-buff? {dex: uint} (unwrap-panic data)) ERR-INVALID-CALLBACK-DATA))
  )
    (try! (liqudate-and-swap-inner pyth-price-feed-data batch (get dex cdata)))
...
(define-public (liquidate-with-swap 
  ...
  (let (
    ...
    (if (> amount-to-flash-loan u0)
      (let 
        ((callback-data (to-consensus-buff? {dex: dex})))
        (var-set liquidator-data (some {
          pyth-price-feed-data: pyth-price-feed-data,
          batch: batch
        }))
        (try! (contract-call? 'SP2ZS72524XRW1GWYSK8NHQ1QK83ESMD40G2HQ2AT.flash-loan-v1 flash-loan amount-to-flash-loan callback callback-data))
      )
