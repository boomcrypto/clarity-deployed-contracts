;; the-curative-guild-passport
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token the-curative-guild-passport uint)
...
(define-data-var artist-address principal 'SPXQX6XD05CQT1092CER4VR4Y1D1FBZ680WGN7C3)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTPmotfB2rCzJpdHBsWcjVGZNX55vymgvbx5W6FAY7Ho4/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? the-curative-guild-passport next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-curative-guild-passport token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-curative-guild-passport token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-curative-guild-passport token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-curative-guild-passport id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-curative-guild-passport id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-curative-guild-passport id) (err ERR-NOT-FOUND)))
