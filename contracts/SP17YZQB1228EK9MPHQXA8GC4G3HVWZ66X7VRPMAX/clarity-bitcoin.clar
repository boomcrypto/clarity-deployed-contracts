;; Clarity code for parsing Bitcoin transactions and block headers,
;; and verifying that Bitcoin transactions were sent on the Bitcoin chain.
;;
;; This code is provided for educational purposes and has not been audited.
;;
;; Source: https://github.com/jcnelson/clarity-bitcoin

;; Error codes
(define-constant ERR-OUT-OF-BOUNDS u1)
(define-constant ERR-TOO-MANY-TXINS u2)
(define-constant ERR-TOO-MANY-TXOUTS u3)
(define-constant ERR-VARSLICE-TOO-LONG (err u4))
(define-constant ERR-BAD-HEADER u5)
(define-constant ERR-PROOF-TOO-SHORT (err u6))
...
(define-read-only (inner-read-slice-1024
  (ignored bool)
  (input { acc: (buff 1024), data: (buff 1024), index: uint })
)
  (let (
    (acc (get acc input))
    (data (get data input))
...
(define-read-only (read-slice-512 (input { data: (buff 1024), index: uint }))
  (get acc (fold
    ...
    LIST_512
    { acc: 0x, data: (get data input), index: (get index input) }
  ))
)
...
(define-read-only (inner-read-slice (chunk_size uint) (input {
  acc: (buff 1024),
  buffer: (buff 1024),
  index: uint,
  remaining: uint
}))
  (let ((ctr (get index input)) (remaining (get remaining input)))
  (if (is-eq u0 remaining)
    ;; done reading
    input
    (let ((acc (get acc input)) (databuff (get buffer input)))
...
(define-read-only (read-slice (data (buff 1024)) (offset uint) (size uint))
  (if (or (>= offset (len data)) (> (+ offset size) (len data)))
    ...
    (begin
      ...
      (ok (get acc (fold
        inner-read-slice
        (list u512 u256 u128 u64 u32 u16 u8 u4 u2 u1)
        { acc: 0x, buffer: data, index: offset, remaining: size }
      )))
    )
  )
)
...
(define-read-only (read-uint16 (ctx { txbuff: (buff 1024), index: uint }))
  (let (
    ...
    (print "read uint16")
    (print ret)
    (ok { uint16: ret, ctx: { txbuff: data, index: (+ u2 base) } })
  )
)
...
(define-read-only (read-uint32 (ctx { txbuff: (buff 1024), index: uint }))
  (let (
    ...
    (print "read uint32")
    (print ret)
    (ok { uint32: ret, ctx: { txbuff: data, index: (+ u4 base) } })
  )
)
...
(define-read-only (read-uint64 (ctx { txbuff: (buff 1024), index: uint }))
  (let (
    ...
    (print "read uint64")
    (print ret)
    (ok { uint64: ret, ctx: { txbuff: data, index: (+ u8 base) } })
  )
)
...
(define-read-only (read-varint (ctx { txbuff: (buff 1024), index: uint }))
  (let (
    (ptr (get index ctx))
    (tx (get txbuff ctx))
    (byte (buff-to-u8 (unwrap! (element-at tx ptr) (err ERR-OUT-OF-BOUNDS))))
  )
    (if (<= byte u252)
      (begin
        (print "varint 1")
      ...
      (if (is-eq byte u253)
        ...
        (let ((parsed-u16 (try! (read-uint16 { txbuff: tx, index: (+ u1 ptr) }))))
          (print "varint 2")
...
(define-read-only (inner-buff32-permutation (target-index uint) (state {
  hash-input: (buff 32),
  hash-output: (buff 32)
}))
  {
    hash-input: (get hash-input state),
    hash-output: (unwrap-panic (as-max-len?
      (concat
        (get hash-output state)
        (unwrap-panic (as-max-len? (unwrap-panic
...
(define-read-only (read-hashslice (old-ctx { txbuff: (buff 1024), index: uint }))
  (let (
    (hash-le (unwrap-panic
      (as-max-len? (try! (read-slice (get txbuff old-ctx) (get index old-ctx) u32)) u32)
    ))
  )
  (ok {
        ...
        (ok {
          ...
          txins: (unwrap!
            (as-max-len? (append
              (get txins state)
              {
                outpoint: { hash: (get hashslice parsed-hash), index: (get uint32 parsed-index) },
                scriptSig: (unwrap!
                  (as-max-len? (get varslice parsed-scriptSig) u256)
...
(define-read-only (read-txins (ctx { txbuff: (buff 1024), index: uint }))
  ...
  (if (> num-txins u8)
    ...
    (fold
      read-next-txin
      (list true true true true true true true true)
      (ok { ctx: new-ctx, remaining: num-txins, txins: (list ) })
    )
  ))
)
          ...
          txouts: (unwrap!
            (as-max-len?
              (append (get txouts state) {
                ...
                scriptPubKey: (unwrap! (as-max-len?
                  (get varslice parsed-script)
                  u128
                ) ERR-VARSLICE-TOO-LONG)
              })
              u8
  ...
  (let (
    ...
    (if (> num-txouts u8)
      ...
      (fold
        read-next-txout
        (list true true true true true true true true)
        (ok { ctx: new-ctx, remaining: num-txouts, txouts: (list ) })
      )
    )
  )
)
...
(define-read-only (inner-merkle-proof-verify
  (ctr uint)
  (state {
    path: uint,
    root-hash: (buff 32),
    proof-hashes: (list 12 (buff 32)),
    tree-depth: uint,
    cur-hash: (buff 32),
    verified: bool
  })
...
(define-read-only (was-tx-mined?
  ...
  (if (verify-block-header (get header block) (get height block))
    (verify-merkle-proof
      (get-reversed-txid tx)
      (get merkle-root (try! (parse-block-header (get header block))))
      proof
    )
    (ok false)
  )
