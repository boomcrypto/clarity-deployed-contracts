...
(define-non-fungible-token abstract-ordinals uint)
...
(define-data-var total-price uint u41000000)
(define-data-var artist-address principal 'SP1Y46BMZF5QH5CZ4KV1ZE0KAYN65GTR1CFAXHXZG)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qme6gASz2qhUdVCmbAmg3QSW9yWM9z3UdRhCgGSHvN6urh/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? abstract-ordinals next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? abstract-ordinals token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? abstract-ordinals token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? abstract-ordinals token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? abstract-ordinals id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? abstract-ordinals id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? abstract-ordinals id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mega uint u300)
