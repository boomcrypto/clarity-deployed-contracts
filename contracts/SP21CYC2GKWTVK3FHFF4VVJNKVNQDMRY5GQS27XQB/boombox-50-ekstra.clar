...
(define-non-fungible-token boombox-50-ekstra uint)
...
;; Internal variables
(define-data-var mint-limit uint u85)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWQXdBJcMizCmFM5Tdi3WHwU1zeZ54Q8F2ChBdsjtuSjU/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? boombox-50-ekstra next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? boombox-50-ekstra token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? boombox-50-ekstra token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? boombox-50-ekstra token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? boombox-50-ekstra id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? boombox-50-ekstra id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? boombox-50-ekstra id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u32944)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u242000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u58800000000)
