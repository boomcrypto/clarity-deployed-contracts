...
(define-non-fungible-token getBREAD uint)
...
(define-data-var base-uri (string-ascii 80) "bafybeicvn75p3nfc2y5gibnhc6ph6npogqhrmnetfnfgueb64vdq6tg7fq/")
(define-data-var mint-price uint u20000000)
;; Percent u100 = 1%
(define-data-var royalty-percent uint u750)
(define-data-var mint-limit uint u5)
(define-data-var max-supply uint u5555)
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? getBREAD next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? getBREAD next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? getBREAD id sender recipient)
        success
          (let
            (
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? getBREAD id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? getBREAD id) ERR_NO_OWNER))
