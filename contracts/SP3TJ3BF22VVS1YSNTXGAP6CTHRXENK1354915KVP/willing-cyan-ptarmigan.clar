...
(define-public (is-dao-or-extension)
  (ok (asserts!
    (or
      (is-eq tx-sender 'SP3TJ3BF22VVS1YSNTXGAP6CTHRXENK1354915KVP.awful-olive-fowl)
      (contract-call? 'SP3TJ3BF22VVS1YSNTXGAP6CTHRXENK1354915KVP.awful-olive-fowl is-extension contract-caller))
    ERR_UNAUTHORIZED
  ))
)
...
(define-public (direct-execute (proposal <proposal-trait>))
  (let
    ...
    (and (>= signals (var-get signalsRequired))
      (try! (contract-call? 'SP3TJ3BF22VVS1YSNTXGAP6CTHRXENK1354915KVP.awful-olive-fowl execute proposal tx-sender))
    )
