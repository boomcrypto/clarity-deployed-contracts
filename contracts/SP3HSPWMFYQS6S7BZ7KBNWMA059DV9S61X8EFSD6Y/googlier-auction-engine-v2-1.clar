;; @contract Auction Engine - Sells off vault collateral to raise USD
;; @version 1

(impl-trait .googlier-auction-engine-trait-v1.auction-engine-trait)
...
(use-trait auction-engine-trait .googlier-auction-engine-trait-v1.auction-engine-trait)
...
(define-data-var lot-size uint u1000000000) ;; 1000 USD
  ...
  (default-to
    {
      ...
      collateral-token: "",
      collateral-address: (contract-call? .googlier-dao get-dao-owner),
      debt-to-raise: u0,
      discount: u0,
      vault-id: u0,
      lot-size: u0,
  ...
  (default-to
    {
      ...
      collateral-token: "",
      collateral-address: (contract-call? .googlier-dao get-dao-owner),
      owner: (contract-call? .googlier-dao get-dao-owner),
      redeemed: false
    }
...
;; @desc ;; 1. Create auction object in map per 100 USD
...
(define-public (start-auction
  ...
  (let ((vault (contract-call? .googlier-vault-data-v1-1 get-vault-by-id vault-id)))
...
;; @desc start an auction to sell off gglr governance tokens
...
;; @param vault-id; ID of the vault for which not enough USD could be raised
;; @param debt-to-raise; the amount of debt left to raise after running the initial collateral auction
;; @param discount; the discount on the gglr collateral to the keepers
;; @post auction; a new auction will be created that runs for one day
  ...
  (let ((vault (contract-call? .googlier-vault-data-v1-1 get-vault-by-id vault-id)))
    ...
    (let (
      ...
      (auction {
        ...
        collateral-token: "gglr",
        collateral-address: .googlier-token,
...
;; e.g. if we need to cover 10 USD debt, and we have 20 STX at $1/STX,
...
;; @desc bid a certain USD amount on a lot in an auction
...
;; @param USD; the amount of USD that is bid. If an amount equal to the lot size (usually 1000 USD) is bid, the lot is automatically won
;; @post USD; the USD amount will be transferred from the tx-sender to this contract
;; @post bid; the lot will be assigned to the current bidder
;; @post stacker-payout; if the STX are being stacked, the yield will go to the keeper pro-rata to the amount of USD the keeper bought
(define-public (bid
  ...
  (let ((auction (get-auction-by-id auction-id)))
    ...
    (asserts! (is-eq (contract-of vault-manager) (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "freddie"))) (err ERR-NOT-AUTHORIZED))
    (asserts! (is-eq (contract-of oracle) (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "oracle"))) (err ERR-NOT-AUTHORIZED))
    ...
    (register-bid vault-manager oracle coll-type auction-id lot-index USD)
  )
)
...
(define-public (redeem-lot-collateral
  ...
  (let (
    ...
    (if (is-eq (get auction-type auction) "debt")
      ...
      (begin
        (try! (contract-call? .googlier-dao request-gglr-tokens (get collateral-amount last-bid)))
    ...
    (if (< (get USD last-bid) (var-get lot-size))
      (begin
        (map-set auctions
          { id: auction-id }
          (merge auction {
...
;; 3. check if vault debt is covered (sum of USD in lots >= debt-to-raise)
...
;; 6. if not all vault debt is covered and no collateral is left: cover USD with gov token
...
(define-public (close-auction
  ...
  (let ((auction (get-auction-by-id auction-id)))
    ...
    (let (
      (vault (contract-call? .googlier-vault-data-v1-1 get-vault-by-id (get vault-id auction)))
    )
      (if (> (get debt vault) (get total-debt-burned auction))
        (let (
          ...
          (if (> amount-to-burn u0)
            (try! (contract-call? .googlier-dao burn-token .USD-token amount-to-burn (as-contract tx-sender)))
            true
          )
          (map-set auctions
            { id: auction-id }
    ...
    (try!
      (if (>= (get total-debt-raised auction) (get debt-to-raise auction))
        ...
        (if (< (get total-collateral-sold auction) (get collateral-amount auction)) ;; we have some collateral left to auction
          ...
          (begin
            ;; no collateral left. Need to sell governance token to raise more USD
            (unwrap-panic (remove-auction auction-id))
            (start-debt-auction
              (get vault-id auction)
...
;; auction engine should only contain USD from bids
;; @param auction-engine; the contract of the new auction engine
;; @param token; the token contract that specifies the FT to transfer, usually will be USD-token
...
;; @desc redeem USD to burn gglr gov token from open market
;; taken from auctions, paid by liquidation penalty on vaults
;; @param USD-amount; the amount of USD to be redeemed from the contract
;; @post USD; all USD tokens will have been transferred to DAO's payout address
(define-public (redeem-USD (USD-amount uint))
  (begin
    (asserts!
      (and
        (is-eq (unwrap-panic (contract-call? .googlier-dao get-emergency-shutdown-activated)) false)
...
(define-private (register-bid
  ...
  (let (
    ...
    (lot-got-sold (if (>= USD (var-get lot-size))
        u1
        u0
      )
    )
    ...
    (asserts! (< (get USD last-bid) (var-get lot-size)) (err ERR-LOT-SOLD))
    ;; Need a better bid than previously already accepted
    (asserts! (> USD (get USD last-bid)) (err ERR-POOR-BID)) 

    ;; Return USD of last bid to (now lost) bidder
    (if (> (get USD last-bid) u0)
      (try! (return-USD (get owner last-bid) (get USD last-bid) auction-id lot-index))
      true
    )
    ;; Transfer USD from liquidator to this contract
    (try! (contract-call? .USD-token transfer USD tx-sender (as-contract tx-sender) none))
    ...
    (map-set auctions
      ...
      (merge auction {
        ...
        total-debt-raised: (- (+ USD (get total-debt-raised auction)) (get USD last-bid))
      })
    )
    ;; Update bids
    (map-set bids
    ...
    ;; Set stacker payout
    (try! (contract-call? .googlier-vault-data-v1-1 set-stacker-payout (get vault-id auction) lot-index collateral-amount tx-sender))
    (print { type: "bid", action: "registered", data: { auction-id: auction-id, lot-index: lot-index, USD: USD } })

    ;; End auction if needed
    (if
      (or
        (>= block-height (get ends-at auction))
        (>= (- (+ USD (get total-debt-raised auction)) (get USD last-bid)) (get debt-to-raise auction))
      )
...
(define-private (return-USD (owner principal) (USD uint) (auction-id uint) (lot-index uint))
  (if (> USD u0)
    (let ((lots (get-winning-lots owner)))
      ...
      (try! (as-contract (contract-call? .USD-token transfer USD (as-contract tx-sender) owner none)))
