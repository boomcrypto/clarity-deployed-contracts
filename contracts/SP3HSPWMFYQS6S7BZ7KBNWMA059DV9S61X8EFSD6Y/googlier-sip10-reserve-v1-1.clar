...
(define-public (collateralize-and-mint
  ...
  (let (
    ...
    ;; token should be a trait e.g. 'SP3GWX3NE58KXHESRYE4DYQ1S31PQJTCRXB3PE9SB.googlier-token
...
(define-public (mint
  ...
  (begin
    ...
    (let ((max-new-debt (- (unwrap-panic (calculate-USD-count token-string ucollateral-amount collateralization-ratio oracle)) current-debt)))
      (if (>= max-new-debt extra-debt)
        (match (as-contract (contract-call? .googlier-dao mint-token .USD-token extra-debt vault-owner))
          success (ok true)
...
(define-public (mint-xstx (collateral uint))
  (begin
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "freddie"))) (err ERR-NOT-AUTHORIZED))
    (contract-call? .googlier-dao mint-token .xstx-token collateral (as-contract tx-sender))
  )
)
...
(define-public (burn-xstx (ustx-amount uint) (sender principal))
  (begin
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "freddie"))) (err ERR-NOT-AUTHORIZED))
    (try! (contract-call? .googlier-dao burn-token .xstx-token  ustx-amount sender))
    (ok true)
  )
)
