...
(define-public (add-tokens-to-stack (name (string-ascii 256)) (token-amount uint))
  (let (
    ...
    (asserts!
      (or
        (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "freddie")))
...
(define-public (toggle-stacking (stacker-name (string-ascii 256)) (revoked-stacking bool) (ustx-collateral uint))
  (begin
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "freddie"))) (err ERR-NOT-AUTHORIZED))
