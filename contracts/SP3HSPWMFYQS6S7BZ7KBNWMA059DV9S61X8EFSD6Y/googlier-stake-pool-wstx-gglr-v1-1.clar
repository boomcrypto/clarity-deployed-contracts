;; @contract Stake Pool - Stake wSTX-USD LP tokens
...
;; Constants
(define-constant POOL-TOKEN .googlier-swap-token-wstx-gglr)
...
(define-map stakes 
   { staker: principal } 
   {
      uamount: uint,  ;; micro gglr amount staked
...
(define-public (stake (registry-trait <stake-registry-trait>) (token <ft-trait>) (staker principal) (amount uint))
  (begin
    ...
    (let (
      ...
      (try! (contract-call? .googlier-swap-token-wstx-gglr transfer amount staker (as-contract tx-sender) none))
...
(define-public (unstake (registry-trait <stake-registry-trait>) (token <ft-trait>) (staker principal) (amount uint))
  (let (
    ...
    (let (
      ...
      (try! (as-contract (contract-call? .googlier-swap-token-wstx-gglr transfer amount tx-sender staker none)))
...
(define-public (emergency-withdraw (registry-trait <stake-registry-trait>))
  (begin
    ...
    (let (
      ...
      ;; Transfer LP token back from this contract to the user
      (try! (as-contract (contract-call? .googlier-swap-token-wstx-gglr transfer stake-amount tx-sender staker none)))
  ...
  (begin
    ...
    (let (
      ...
      (if (>= pending-rewards u1)
        (begin
          ;; Mint gglr rewards for staker
          (try! (contract-call? .googlier-dao mint-token .googlier-token pending-rewards staker))
...
(define-public (calculate-cumm-reward-per-stake (registry-trait <stake-registry-trait>))
  (let (
    (rewards-per-block (unwrap-panic (contract-call? registry-trait get-rewards-per-block-for-pool .googlier-stake-pool-wstx-gglr-v1-1)))
...
(define-private (get-last-block-height (registry-trait <stake-registry-trait>))
  (let (
    (deactivated-block (unwrap-panic (contract-call? registry-trait get-pool-deactivated-block .googlier-stake-pool-wstx-gglr-v1-1)))
