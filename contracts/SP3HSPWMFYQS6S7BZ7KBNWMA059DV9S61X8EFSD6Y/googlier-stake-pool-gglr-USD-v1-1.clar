;; @contract Stake Pool - Stake gglr-USD LP tokens
...
;; Constants
(define-constant POOL-TOKEN .googlier-swap-token-gglr-USD)
...
(define-public (stake (registry-trait <stake-registry-trait>) (token <ft-trait>) (staker principal) (amount uint))
  (begin
    ...
    (let (
      ...
      (try! (contract-call? .googlier-swap-token-gglr-USD transfer amount staker (as-contract tx-sender) none))
...
(define-public (unstake (registry-trait <stake-registry-trait>) (token <ft-trait>) (staker principal) (amount uint))
  (let (
    ...
    (let (
      ...
      (try! (as-contract (contract-call? .googlier-swap-token-gglr-USD transfer amount tx-sender staker none)))
...
(define-public (emergency-withdraw (registry-trait <stake-registry-trait>))
  (let (
    ...
    (let (
      ...
      ;; Transfer LP token back from this contract to the user
      (try! (as-contract (contract-call? .googlier-swap-token-gglr-USD transfer stake-amount tx-sender sender none)))
...
(define-public (calculate-cumm-reward-per-stake (registry-trait <stake-registry-trait>))
  (let (
    (rewards-per-block (unwrap-panic (contract-call? registry-trait get-rewards-per-block-for-pool .googlier-stake-pool-gglr-USD-v1-1)))
...
(define-private (get-last-block-height (registry-trait <stake-registry-trait>))
  (let (
    (deactivated-block (unwrap-panic (contract-call? registry-trait get-pool-deactivated-block .googlier-stake-pool-gglr-USD-v1-1)))
