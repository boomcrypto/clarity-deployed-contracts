;; @contract googlier governance
;; Can see, vote and submit a new proposal
;; A proposal will just update the DAO with new contracts.
;; @version 1.1

(use-trait ft-trait .sip-010-trait-ft-standard.sip-010-trait)
(use-trait stake-pool-gglr-trait .googlier-stake-pool-gglr-trait-v1.stake-pool-gglr-trait)
...
(define-read-only (is-token-accepted (token <ft-trait>))
  (let (
    (is-gglr (is-eq (contract-of token) .googlier-token))
    (is-stgglr (is-eq (contract-of token) .stgglr-token))
  )
    (or is-gglr is-stgglr)
  )
)
...
(define-public (propose-dao
  (stake-pool-gglr <stake-pool-gglr-trait>)
  (start-block-height uint)
  (title (string-utf8 256))
  (url (string-utf8 256))
  (contract-changes (list 10 (tuple (name (string-ascii 256)) (address principal) (qualified-name principal) (can-mint bool) (can-burn bool))))
...
;; @param stake-pool-gglr; gglr pool to get stgglr/gglr ratio from
...
(define-public (propose
  ...
  (let (
    (proposer-gglr-balance (unwrap-panic (contract-call? .googlier-token get-balance tx-sender)))
    (proposer-stgglr-balance (unwrap-panic (contract-call? .stgglr-token get-balance tx-sender)))
    
    (proposer-gglr-votes (unwrap-panic (token-amount-to-votes stake-pool-gglr .googlier-token proposer-gglr-balance)))
    (proposer-stgglr-votes (unwrap-panic (token-amount-to-votes stake-pool-gglr .stgglr-token proposer-stgglr-balance)))
    (proposer-total-balance (+ proposer-gglr-votes proposer-stgglr-votes))
    (end-block-height
    ...
    (gglr-init-balance (unwrap-panic (contract-call? .googlier-token get-balance .googlier-gglr-init)))
    (supply (- (unwrap-panic (contract-call? .googlier-token get-total-supply)) gglr-init-balance))
    ...
    (asserts!
      (is-eq
        (contract-of stake-pool-gglr)
        (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "stake-pool-gglr"))
      )
...
;; @desc translate tokens to amount of votes - stgglr to gglr to votes
;; @param stake-pool-gglr; gglr stake pool to get gglr/stgglr ratio from
;; @param token; proposal to vote for
;; @param amount; amount of tokens
;; @post uint; amount of votes
(define-public (token-amount-to-votes (stake-pool-gglr <stake-pool-gglr-trait>) (token <ft-trait>) (amount uint))
  (begin
    (asserts! (is-eq (contract-of stake-pool-gglr) (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "stake-pool-gglr"))) (err ERR-NOT-AUTHORIZED))
    (if (is-eq (contract-of token) .googlier-token)
      (ok amount)
      (let (
        (gglr-stgglr (unwrap-panic (contract-call? stake-pool-gglr gglr-stgglr-ratio)))
      )
        (ok (/ (* amount gglr-stgglr) u1000000))
      )
    )
  )
)
...
;; @param stake-pool-gglr; gglr stake pool to get gglr/stgglr ratio from
;; @param token; token used to vote (gglr or stgglr)
;; @param proposal-id; proposal to vote for
;; @param amount; amount of votes (tokens)
;; @post uint; returns 3200 when votes accepted
(define-public (vote-for (stake-pool-gglr <stake-pool-gglr-trait>) (token <ft-trait>) (proposal-id uint) (amount uint))
  (let (
    ...
    (add-vote-count (unwrap-panic (token-amount-to-votes stake-pool-gglr token amount)))
  )
    (asserts!
      (and
        (is-eq (unwrap-panic (contract-call? .googlier-dao get-emergency-shutdown-activated)) false)
...
;; @param amount; amount of votes (tokens)
;; @post uint; returns 3200 when votes accepted
(define-public (vote-against (stake-pool-gglr <stake-pool-gglr-trait>) (token <ft-trait>) (proposal-id uint) (amount uint))
  (let (
    (proposal (get-proposal-by-id proposal-id))
    (vote-count (get vote-count (get-votes-by-member-by-id proposal-id tx-sender)))
    (token-count (get amount (get-tokens-by-member-by-id proposal-id tx-sender token)))
...
;; @desc  Return votes (gglr or stgglr tokens) to voter
;; @param token; token to return (gglr or stgglr)
...
(define-private (execute-proposal-change-contract (change (tuple (name (string-ascii 256)) (address principal) (qualified-name principal) (can-mint bool) (can-burn bool))))
  (let (
    ...
    (if (not (is-eq name ""))
      (begin
        (try! (contract-call? .googlier-dao set-contract-address name address qualified-name can-mint can-burn))
        (ok true)
      )
      (ok false)
    )
...
(define-public (add-contract-address (name (string-ascii 256)) (address principal) (qualified-name principal) (can-mint bool) (can-burn bool))
  (begin
    ...
    (if (is-some (contract-call? .googlier-dao get-contract-address-by-name name))
      (ok false)
      (begin
        (try! (contract-call? .googlier-dao set-contract-address name address qualified-name can-mint can-burn))
        (ok true)
