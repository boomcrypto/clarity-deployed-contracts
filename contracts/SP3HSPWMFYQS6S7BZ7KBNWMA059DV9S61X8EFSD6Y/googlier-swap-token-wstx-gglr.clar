...
(define-fungible-token wstx-gglr)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? wstx-gglr amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-read-only (get-name)
  (ok "googlier V1 wSTX gglr LP Token")
)
...
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance wstx-gglr owner))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply wstx-gglr))
)

(define-read-only (get-token-uri)
  (ok (some u"https://googlier.finance/tokens/wstx-gglr-token.json"))
)
;; {
;;   "name":"wSTX-gglr",
;;   "description":"wSTX-gglr googlier LP token",
;;   "image":"url",
;;   "vector":"url"
;; }
...
(define-public (mint (recipient principal) (amount uint))
  (begin
    ...
    (ft-mint? wstx-gglr amount recipient)
  )
)
...
(define-public (burn (recipient principal) (amount uint))
  (begin
    ...
    (asserts! (is-eq contract-caller (unwrap-panic (contract-call? .googlier-dao get-qualified-name-by-name "swap"))) (err ERR-NOT-AUTHORIZED))
    (ft-burn? wstx-gglr amount recipient)
  )
)
