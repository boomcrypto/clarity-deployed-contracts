...
(define-constant APP-VERSION "0.1.1")
...
(define-map used-nonces (string-ascii 36) bool)
...
(define-public 
    ...
    (let  (
        ...
        ;; assert nonce is uuid length
        (asserts! (is-eq (len nonce) u36) (err "err-nonce-invalid"))

        ;; assert nonce is not used
        (asserts! 
            (is-none (map-get? used-nonces nonce))
            (err "err-nonce-used"))
        ...
        (unwrap! 
            (nft-mint? endorsement token-id recipient) 
            (err "err-mint-failed"))

        (map-insert used-nonces nonce true)
        
        ;; Update largest-nft-id
        ;; Map set the endorsement submission
        (map-insert endorsement-submission token-id {
            sender: sender,
            recipient: recipient,
