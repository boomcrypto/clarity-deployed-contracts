...
(define-data-var admins (list 1000 principal) (list 'SP158WX3ZNRDG4SR56TD1398NZK1945QTTKB8B60X 'SPH9R4KTPQS6AWXX9WG0KHC1K9393M83J7KP5NNG 'SPEK28QPHDX8X1WH54T60JADW6G0MD9SP8J0EZ8G))
...
(define-public (register-nft-name (nft <nft-trait>) (id uint) (name (string-utf8 256)))
  (let ((owner (unwrap! (unwrap! (contract-call? nft get-owner id) err-not-found) err-not-found)))
    ...
    (try! (contract-call? .comm1 pay nft id name))
    ;; register name
...
(define-public (transfer-nft-name (nft <nft-trait>) (id uint) (name (string-utf8 256)) (receiver principal) (nft2name <nft-trait>) (id2 uint))
  ...
  (let (
    ...
    (try! (contract-call? .transfer-fee pay nft2name id2 name))
...
;; Admin Functions are hopefully never used, these are fail-safes for The Unexpected;;)

;;Admin Delete to remove any name violating the  naming policy
...
;;Admin Transfer for user support only
