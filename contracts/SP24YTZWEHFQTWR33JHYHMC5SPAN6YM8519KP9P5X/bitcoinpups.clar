...
(define-non-fungible-token bitcoinpups uint)
...
(define-data-var artist-address principal 'SP24YTZWEHFQTWR33JHYHMC5SPAN6YM8519KP9P5X)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQjhYqSP2JfZxxQkrtT9XvJNEbftCGToDtCfg96wEcC5N/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoinpups next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoinpups token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoinpups token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoinpups token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoinpups id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoinpups id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoinpups id) (err ERR-NOT-FOUND)))
