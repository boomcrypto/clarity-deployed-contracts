...
(define-non-fungible-token Celebrate_TOKYO uint)
...
(define-constant IPFS-ROOT "ipfs://ipfs/bafybeibygsv263yv3zppgre6drnob47woj6rckqggee7gk6ftcwyqniwke/{id}.json")
...
(define-public (mint)
    (let ((token-id (+ u1 (var-get last-token-id))))
        ...
        (nft-mint? Celebrate_TOKYO token-id tx-sender)
    )
)
...
(define-public (burn (id uint))
    (let ((owner (unwrap! (nft-get-owner? Celebrate_TOKYO id) (err ERROR-UNAUTHORIZED))))
        (asserts! (is-eq owner tx-sender) (err ERROR-UNAUTHORIZED))
        (nft-burn? Celebrate_TOKYO id owner)
    )
)
...
(define-read-only (get-owner (id uint))
    (ok (nft-get-owner? Celebrate_TOKYO id))
)
