...
(define-non-fungible-token zomapes uint)
...
(define-data-var artist-address principal 'SPDP4KZYP9CAH58C26V05E41ZVA83DWZ08MTQP8Q)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbR2nQobY2dkSNBqCGMztvebPDVJ41wzxYHb1zh9h1hkq/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? zomapes next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? zomapes token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? zomapes token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? zomapes token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? zomapes id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? zomapes id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? zomapes id) (err ERR-NOT-FOUND)))
