...
(define-non-fungible-token Bounty_Test uint)
...
(define-constant IPFS-ROOT "ipfs://ipfs/bafybeicyytyxbzon2v7ep6tmkuy3bj6snepknh2mxr2omxboub2g3mlgl4/{id}.json")
...
(define-public (mint)
    (let ((token-id (+ u1 (var-get last-token-id))))
        ...
        (nft-mint? Bounty_Test token-id tx-sender)
    )
)
...
(define-public (burn (id uint))
    (let ((owner (unwrap! (nft-get-owner? Bounty_Test id) (err ERROR-UNAUTHORIZED))))
        (asserts! (is-eq owner tx-sender) (err ERROR-UNAUTHORIZED))
        (nft-burn? Bounty_Test id owner)
    )
)
...
(define-read-only (get-owner (id uint))
    (ok (nft-get-owner? Bounty_Test id))
)
