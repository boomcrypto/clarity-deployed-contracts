...
(define-non-fungible-token ordinal-mutants uint)
...
;; Internal variables
(define-data-var mint-limit uint u29)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1000000000)
(define-data-var artist-address principal 'SP21139DFR52583TKBY23QRV77XMK62R8JR39VDRV)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTo4TJjWDdRXFXA9te9iJq5egEk8Ba5rxwjB1fiwG5neo/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinal-mutants next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinal-mutants token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinal-mutants token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinal-mutants token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinal-mutants id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinal-mutants id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinal-mutants id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u3174943)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u751000000)
