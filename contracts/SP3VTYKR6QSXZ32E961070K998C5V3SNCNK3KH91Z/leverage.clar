...
;; Errors
(define-constant ERR-UNAUTHORIZED (err u30000))
(define-constant ERR-INSUFFICIENT-BALANCE (err u30001))
(define-constant ERR-INVALID-CALLBACK-DATA (err u30002))
(define-constant ERR-TIMEOUT (err u30003))
...
(define-public (on-granite-flash-loan (amount uint) (fee uint) (data (optional (buff 20480))))
  (let (
    ...
    ;; Send aeUSDC to this contract
    ;;(try! (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer amount sender SELF none))
    ;; Swap from the contract holding in aeUSDC to sBTC
    (let 
      (
        (sbtc-out (try! (swap-aeusdc-to-sbtc-bitflow amount min-sbtc-out)))
      )
        ;; Transfer sBTC to the user
        ;;(try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer sbtc-out SELF user none))
        ;; deposit margin + obtained sBTC
        (try! (contract-call? 'SP2Q5KYRHR7C07D5H5Y0QMN6K9F9410X1CS0J7YS0.borrower-v1 add-collateral 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token (+ sbtc-out margin) (some user)))
        ;; borrow
