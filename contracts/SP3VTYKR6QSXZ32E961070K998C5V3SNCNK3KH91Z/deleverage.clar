...
(define-constant ERR-INSUFFICIENT-BALANCE (err u40001))
(define-constant ERR-INVALID-CALLBACK-DATA (err u40002))
(define-constant ERR-TIMEOUT (err u40003))
...
(define-public (on-granite-flash-loan (amount uint) (fee uint) (data (optional (buff 20480))))
  (let (
    (cdata (unwrap! (from-consensus-buff? {
      ...
      (user (get user cdata))
      (min-aeusdc-out (get min-aeusdc-out cdata))
      (sender tx-sender)
  )
    ...
    ;; Repay debt
    (try! (contract-call? 'SP2Q5KYRHR7C07D5H5Y0QMN6K9F9410X1CS0J7YS0.borrower-v1 repay amount (some user)))
    ;; Withdraw sBTC
    (try! (contract-call? 'SP2Q5KYRHR7C07D5H5Y0QMN6K9F9410X1CS0J7YS0.borrower-v1 remove-collateral pyth-price-feed-data 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token sbtc-to-withdraw (some user)))
    ;; Swap from the user holding in sBTC to aeUSDC    
    (let ((aeusdc-out (try! (swap-sbtc-to-aeusdc-bitflow sbtc-to-withdraw min-aeusdc-out))))
      (print {
        action: "sbtc deleverage",
        user: user,
        repaid: amount,
