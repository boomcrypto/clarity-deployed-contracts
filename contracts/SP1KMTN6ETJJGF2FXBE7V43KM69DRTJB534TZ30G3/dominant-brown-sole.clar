  ...
  (let (
    ...
    (as-contract
      (let (
        (b1 (try! (swap-wstx-alex-alex amountIn)))
        (b2 (try! (swap-alex-usda-alex b1)))
        (b3 (try! (swap-usda-usdx-alex b2)))
        (b4 (try! (swap-usdx-wstx-alex b3)))
      )
        (print { b1: b1, b2: b2, b3: b3, b4: b4 })
        (asserts! (> b4 amountIn) (err ERR-COOLDOWN))
        (try! (stx-transfer? b4 tx-sender sender))
  ...
  (let (
    ...
    (as-contract
      (let (
        (b1 (try! (swap-wstx-usdx-alex amountIn)))
        (b2 (try! (swap-usdx-usda-alex b1)))
        (b3 (try! (swap-usda-alex-alex b2)))
        (b4 (try! (swap-alex-wstx-alex b3)))
      )
        (print { b1: b1, b2: b2, b3: b3, b4: b4 })
        (asserts! (> b4 amountIn) (err ERR-COOLDOWN))
        (try! (stx-transfer? b4 tx-sender sender))
...
(define-private (swap-usdx-wstx-alex (dx uint))
  (let ((r (try! (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.fixed-weight-pool-v1-01 swap-y-for-x 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.token-wstx 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.token-wxusd u50000000 u50000000 (* dx u1) (some u0)))))
  (ok (/ (get dx r) u100)))
)

(define-private (swap-usdx-usda-alex (dx uint))
  (let ((r (try! (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.amm-swap-pool swap-x-for-y 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.token-wxusd 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.token-wusda u500000 (* dx u1) (some u0)))))
  (ok (/ (get dy r) u100)))
)

(define-private (swap-usda-usdx-alex (dx uint))
  (let ((r (try! (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.amm-swap-pool swap-y-for-x 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.token-wxusd 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.token-wusda u500000 (* dx u100) (some u0)))))
  (ok (/ (get dx r) u1)))
)
