...
(define-non-fungible-token animjp uint)
...
;; Internal variables
(define-data-var mint-limit uint u314)
(define-data-var last-id uint u1)
(define-data-var total-price uint u105000000)
(define-data-var artist-address principal 'SPJEP02F55VAN2K81FNBZBF1Y7TH0V4QPX68PX8)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmX8XuHWt9G14jhPvovywJmWgca6CwBxJE5qPbknHDH34F/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? animjp next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? animjp token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? animjp token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? animjp token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? animjp id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? animjp id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? animjp id) (err ERR-NOT-FOUND)))
