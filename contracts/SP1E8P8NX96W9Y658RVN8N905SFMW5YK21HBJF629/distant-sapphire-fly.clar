...
(define-public (npNiEKpZqMiWpysLu
    ...
    (CXWwAaHSjGJpyOHHl <TIOaKkHYJRWXepw>) 
    (lFWXIWCZHyyQxjwTS <TIOaKkHYJRWXepw>) 
    ...
    (let 
        (
            (fWWAQpaODedaTLiAf (qaNyBCOIKDqQHcnTT qRXbRiatsWCtdRTfy CXWwAaHSjGJpyOHHl))
            (HyiLEyhXJyyTLTdez (qaNyBCOIKDqQHcnTT jwragaWAMRRZvKUrN lFWXIWCZHyyQxjwTS))
        ...
        (if (is-eq qRXbRiatsWCtdRTfy true)
            (begin
                ...
                (var-set SQOQESUEmkmWIoTBZ (- (qaNyBCOIKDqQHcnTT qRXbRiatsWCtdRTfy CXWwAaHSjGJpyOHHl) fWWAQpaODedaTLiAf))
            )
            (begin 
                (try! (contract-call? CXWwAaHSjGJpyOHHl transfer AgIJzBsylJwSnXjfW tx-sender (as-contract tx-sender) none))
                (var-set SQOQESUEmkmWIoTBZ (- (unwrap-panic (contract-call? CXWwAaHSjGJpyOHHl get-balance (as-contract tx-sender))) fWWAQpaODedaTLiAf))
            )
        )
        ...
        ;; (let 
        ;;     (  
        ;;         (mJyWYdafcxnFpCxPC (qaNyBCOIKDqQHcnTT jwragaWAMRRZvKUrN lFWXIWCZHyyQxjwTS))
