...
(define-non-fungible-token please-the-master uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmcDj6bKTGWfWXw1Rk6UhyFSg729uzWyeBqkyZxmDRV6jj/please-the-master/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP26JPGAMSCFG5RNP2N5E2Z0NES4MZYAN11PTP45N )) 
...
(define-data-var public-target-block-height uint u64471)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? please-the-master id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace please-the-master
  (ok (nft-get-owner? please-the-master id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? please-the-master next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? please-the-master id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? please-the-master id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? please-the-master id) ERR-NOT-FOUND))
