...
(define-non-fungible-token discoloration uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmREeLYk36K7Zi1RW5CxBC69xZV6TxQXvTq2egNBgKok4k/discoloration/")
...
(define-data-var public-target-block-height uint u68272)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? discoloration id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace discoloration
  (ok (nft-get-owner? discoloration id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? discoloration next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? discoloration id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? discoloration id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? discoloration id) ERR-NOT-FOUND))
