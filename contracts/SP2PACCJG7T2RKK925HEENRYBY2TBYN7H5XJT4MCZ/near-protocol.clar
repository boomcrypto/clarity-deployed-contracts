...
(define-non-fungible-token near-protocol uint)
...
(define-data-var artist-address principal 'SP2PACCJG7T2RKK925HEENRYBY2TBYN7H5XJT4MCZ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbFcEQjrPFUjCndgDhqUSgTBk63F6ikcnntSA8aTQLboC/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? near-protocol next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? near-protocol token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? near-protocol token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? near-protocol token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? near-protocol id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? near-protocol id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? near-protocol id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mega uint u50000)
