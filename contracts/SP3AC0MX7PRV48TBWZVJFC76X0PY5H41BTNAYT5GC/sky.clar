...
(define-non-fungible-token sky uint)
...
(define-data-var artist-address principal 'SP3AC0MX7PRV48TBWZVJFC76X0PY5H41BTNAYT5GC)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmayfJtLvm5eyiXzfAbPbcc4mvKkiwwHUZrgnw7MJLiaXy/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? sky next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? sky token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? sky token-id) false)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sky id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sky id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sky id) (err ERR-NOT-FOUND)))
