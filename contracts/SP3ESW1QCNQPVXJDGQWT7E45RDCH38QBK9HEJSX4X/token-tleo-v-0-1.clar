...
(define-constant USER_MINT_AMOUNT u1000000000000000)
(define-constant DEPLOY_MINT_AMOUNT u100000000000000000)
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance tLEO account))
)

(define-read-only (get-balance-simple (account principal))
  (ft-get-balance tLEO account)
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply tLEO))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? tLEO amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint)
  (begin
    ...
    (ft-mint? tLEO USER_MINT_AMOUNT tx-sender)
  )
)
...
(define-public (admin-mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? tLEO amount recipient)
  )
)

(define-public (burn (amount uint))
  (begin
    (ft-burn? tLEO amount tx-sender)
  )
)

(ft-mint? tLEO DEPLOY_MINT_AMOUNT CONTRACT_DEPLOYER)