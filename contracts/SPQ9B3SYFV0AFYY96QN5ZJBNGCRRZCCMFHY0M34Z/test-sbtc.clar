...
(define-constant ERR-TRANSFER-FAILED (err u8)) ;; sBTC transfer failed

;; Constants
(define-constant PRICE u10000) ;; Price in satoshis (sBTC base unit)
...
(define-public (get-contract-balance)
  (ok (contract-call? SBTC-CONTRACT get-balance (as-contract tx-sender)))
)
...
(define-public (get-customer-sbtc-balance (customer principal))
  (ok (contract-call? SBTC-CONTRACT get-balance customer))
)
