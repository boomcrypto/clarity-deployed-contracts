...
(define-non-fungible-token ordinals-neon-bayc uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWddbYgz7naFkN1YTQY1msPTFBFKPafYaSVNZqdhPCzyD/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinals-neon-bayc next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinals-neon-bayc token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinals-neon-bayc token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinals-neon-bayc token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinals-neon-bayc id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinals-neon-bayc id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinals-neon-bayc id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u20020)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u5000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u502000000)
