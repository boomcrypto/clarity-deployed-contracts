    ...
    (begin
        ...
        (match admin-optional-signed-data
            ...
            (begin
                ...
                (try! (verify-admin-signature "Add"  user-id (unwrap-panic admin-auth-optional) signed-data))
    ...
    (begin
        ...
        (match admin-optional-signed-data
            ...
            (begin
                ...
                ;; Verify signatures
                ;; Check that all signatures are valid & all signers are in the policy
                (try! (verify-admin-signature "Remove"  user-id (unwrap-panic admin-auth-optional) signed-data))
    ...
    (begin
        ...
        (match admin-optional-signed-data
            ...
            (begin
                ...
                ;; Verify signatures
                ;; Check that all signatures are valid & all signers are in the policy
                (try! (verify-admin-signature "Rotate"  user-id (unwrap-panic admin-auth-optional) signed-data))
                ;; Check that signing threshold is met (2)
                (asserts! (>= (len signed-data) u2) ERR_THRESHOLD_NOT_MET)
...
(define-public (verify-admin-signature (type (string-ascii 32))
                                    (admin-id (string-ascii 64))
                                    (auth-id (string-ascii 64))
                                    (signed-data (list 35 {signer: (buff 33), signature: (buff 65)})))
    (begin
        ;; Check that all signatures are valid & all signers are in the policy
        (try! (fold verify-admin-signature-helper signed-data (ok {type: type, admin-id: admin-id, auth-id: auth-id})))
        (ok true)
    )
)
...
(define-private (verify-admin-signature-helper (signed-data {signer: (buff 33), signature: (buff 65)}) 
                                  (signed-response (response {type: (string-ascii 32), admin-id: (string-ascii 64), auth-id: (string-ascii 64)} uint)))
