...
(define-non-fungible-token munuub uint)
...
(define-data-var artist-address principal 'SP386T3KHA99D7T2WYW4P046Q1RFAZDJ95AEKAWF9)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdiDp12iRZCJNz6Zs6SKAQJgVf8Mmf2jx28E2EZhGRKNj/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? munuub next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? munuub token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? munuub token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? munuub token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? munuub id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? munuub id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? munuub id) (err ERR-NOT-FOUND)))
