...
(define-non-fungible-token dexblock uint)
...
;; Internal variables
(define-data-var mint-limit uint u2009)
(define-data-var last-id uint u1)
(define-data-var total-price uint u256000000)
(define-data-var artist-address principal 'SP26ACNW5NPNDY6ZS485MB67J3FCFMCPMSDJZAZ9F)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmcr8gngxkBXzCtjBU8jiQZd4rbfbxfs9CM31dNgTcQzsH/json/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? dexblock next-id tx-sender) next-id)
      (unwrap! (nft-transfer? dexblock next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? dexblock token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? dexblock token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? dexblock token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? dexblock id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? dexblock id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? dexblock id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u451722)
