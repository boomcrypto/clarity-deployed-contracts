...
(define-non-fungible-token perfect-solitude uint)
...
(define-data-var artist-address principal 'SP1PN96CD5E3V15TNWD2DQ9NJ3S81CNB9VP2HDT6Y)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmY3NvcuqwYMQMhcHqdPd1Cv7DQ3CBwhxvsyYATsY89nsb/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? perfect-solitude next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? perfect-solitude token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? perfect-solitude token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? perfect-solitude token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? perfect-solitude id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? perfect-solitude id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? perfect-solitude id) (err ERR-NOT-FOUND)))
