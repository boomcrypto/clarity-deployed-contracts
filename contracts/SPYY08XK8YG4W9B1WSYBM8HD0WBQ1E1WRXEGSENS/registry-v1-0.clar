(impl-trait .registry-trait-v1-0.registry-trait)
...
(define-data-var protocol-fee-destination principal 'SP3XX4465B4V5729HA6W01B1ENB7ECHTZH9WV3XT3)
...
(define-public (set-processed-epoch (new-epoch uint))
  (let ((keeper (unwrap-panic (contract-call? .controller-v1-0 is-keeper tx-sender))))
...
(define-public (set-protocol-state (new-state uint))
  (begin
    (try! (contract-call? .controller-v1-0 is-admin tx-sender))
    (asserts!
      (or
...
(define-public (hot-pause)
  (begin
    (try! (contract-call? .controller-v1-0 is-hot-pause-caller tx-sender))
...
(define-public (set-decay-parameters
  ...
  (begin
    (try! (contract-call? .controller-v1-0 is-protocol-caller contract-caller))
    (asserts!
