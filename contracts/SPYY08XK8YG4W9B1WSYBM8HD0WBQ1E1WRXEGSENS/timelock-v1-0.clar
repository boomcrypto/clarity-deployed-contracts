...
(use-trait stability-trait .stability-trait-v1-0.stability-trait)
...
(define-public (propose-vault-protocol-transfer-sbtc 
    ...
    (let 
        (
          ...
          (delay (unwrap! (contract-call? .registry-v1-0 get-timelock-delay) ERR_INVALID_DELAY))
...
(define-private (execute-redeem-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-v1-0 set-redeem-parameters 
...
(define-private (execute-decay-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-v1-0 set-decay-parameters 
            new-max-hours-decay 
...
(define-private (execute-vault-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-v1-0 set-vault-parameters 
...
(define-private (execute-borrow-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-v1-0 set-borrow-parameters 
...
(define-private (execute-global-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-v1-0 set-global-parameters 
...
(define-private (execute-add-hot-pause-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-v1-0 add-hot-pause-principal add-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-hot-pause-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-v1-0 remove-hot-pause-principal remove-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-add-supported-contract-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-v1-0 add-supported-contract contract-key contract-address qualified-contract)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-supported-contract-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-v1-0 remove-supported-contract contract-key)
            ok-result
            (begin
                (print {
...
(define-private (execute-set-keeper-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-v1-0 set-keeper new-keeper)
            ok-result
            (begin
                (print {
...
(define-private (execute-add-controller-privileged-protocol-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-v1-0 add-privileged-protocol-principal new-protocol-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-controller-privileged-protocol-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-v1-0 remove-privileged-protocol-principal remove-protocol-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-propose-admin-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-v1-0 propose-admin new-admin)
            ok-result
            (begin
                (print {
...
(define-private (execute-set-contracts-allowed-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-v1-0 set-contracts-allowed allowed)
            ok-result
            (begin
                (print {
...
(define-private (execute-keeper-allow-all-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-v1-0 keeper-allow-all allowed)
            ok-result
            (begin
                (print {
