...
(define-non-fungible-token expansion-punks uint)
...
(define-data-var artist-address principal 'SP37TX92THNKDNAXG092JNFPGGQ30604E58C2GK9A)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPMbQw5GYM365zMzNrjEkL8x5B9JmRgy6S8eDiAawt7Ya/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? expansion-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? expansion-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? expansion-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? expansion-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? expansion-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? expansion-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? expansion-punks id) (err ERR-NOT-FOUND)))
