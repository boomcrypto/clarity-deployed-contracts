...
(define-non-fungible-token vv-owl uint)
...
(define-data-var artist-address principal 'SP2BQBGVH73WP6PZNZ5JA1MMPZQ8DWT7E1FZWKQQB)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaP7wtDDEXwJX3FHzb8NsJ6m3usZBm1R4oHnXyFS3iQQg/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? vv-owl next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? vv-owl token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? vv-owl token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? vv-owl token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? vv-owl id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? vv-owl id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? vv-owl id) (err ERR-NOT-FOUND)))
