;; sunrise-n-sunsets-unlimited
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token sunrise-n-sunsets-unlimited uint)
...
(define-data-var artist-address principal 'SP2A15MGAYDK5JYDF4BTZQZN7MQR29AZ14C2968BH)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVyk1ZpsQD29bNbPZVB6EPsK1ArCrAJtjHm7HcoMsaBfg/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? sunrise-n-sunsets-unlimited next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? sunrise-n-sunsets-unlimited token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? sunrise-n-sunsets-unlimited token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sunrise-n-sunsets-unlimited token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sunrise-n-sunsets-unlimited id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sunrise-n-sunsets-unlimited id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sunrise-n-sunsets-unlimited id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u8613)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u5435000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u5035000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u275000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u2800000000)
