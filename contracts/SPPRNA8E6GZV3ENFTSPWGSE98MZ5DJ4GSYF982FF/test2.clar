...
;; data vars
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://bafybeicxrqs27d4xqniresn22jbygepca7uaw2a3jbzklkxlvckao7jhli.ipfs.dweb.link/")) 
...
(define-read-only (get-name) (ok "test2"))
(define-read-only (get-symbol) (ok "TEST2"))
(define-read-only (get-decimals) (ok u0))
(define-read-only (get-balance (user principal)) (ok (ft-get-balance test2 user)))
(define-read-only (get-total-supply) (ok (ft-get-supply test2)))
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
    (begin
        ...
        (ft-transfer? test2 amount from to)
    )
)
...
(define-public (swap-stx-for-token (stx-amount uint) (min-tokens-out uint)) ;; swap in for virtual trading
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? test2 tokens-out (as-contract tx-sender) tx-sender))
      (if (>= (/ (* (+ (var-get real-stx-reserve) (var-get virtual-stx-reserve)) token-supply) (var-get token-reserve)) market-cap-threshold)
        (begin
          (let (
            ...
            (try! (ft-transfer? test2 burn-amount (as-contract tx-sender) burn-address))
            (try! (ft-transfer? test2 remaining-tokens (as-contract tx-sender) liquidity-builder-address))
...
(define-public (swap-token-for-stx (token-amount uint) (min-stx-out uint) (recipient principal)) ;; swap out for virtual trading
  (begin
    ...
    (let (
      ...
      (try! (ft-transfer? test2 token-amount tx-sender (as-contract tx-sender)))
...
(begin
  (try! (ft-mint? test2 token-supply (as-contract tx-sender)))
