...
(define-non-fungible-token harmony-of-joy uint)
...
(define-data-var artist-address principal 'SP63VFR7W3HF60XCA350XSHTWVHR57Z5FW8QQSN9)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmV9nC599fgxyRWScz71GojDbcWukbgRbcHXVYi7Zq6MXY/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? harmony-of-joy next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? harmony-of-joy token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? harmony-of-joy token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? harmony-of-joy token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? harmony-of-joy id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? harmony-of-joy id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? harmony-of-joy id) (err ERR-NOT-FOUND)))
