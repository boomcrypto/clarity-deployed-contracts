...
(define-public (re-price (id uint) (ft <fungible-token>) (fees <fees-trait>) (ustx uint) (expiry (optional uint)))
  (let ((swap (unwrap! (map-get? swaps id) ERR_INVALID_ID))
      ...
      (match expiry
            some-expiry (begin 
                        (asserts! (map-set swaps id (merge swap {ustx: ustx, expired-height: (some (+ burn-block-height some-expiry))})) ERR_NATIVE_FAILURE)
                        (print 
                        {
                            type: "re-price",
                            swap-id: id, 
            ...
            (begin 
             (asserts! (map-set swaps id (merge swap {ustx: ustx, expired-height: none})) ERR_NATIVE_FAILURE)
                        (print 
                        {
                            type: "re-price",
                            swap-id: id, 
