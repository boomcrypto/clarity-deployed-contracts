...
;;transfer
(define-public (transfer-nft-name (nft <nft-trait>) (id uint) (name (string-utf8 256)) (nft2name <nft-trait>) (id2 uint))
  (let ((owner (unwrap! (unwrap! (contract-call? nft get-owner id) err-not-found) err-not-found)))
   (begin 
    (asserts! (is-eq tx-sender owner) err-not-authorized)
    (try! (stx-transfer? (var-get registration-fee ) tx-sender (var-get topay)))
...
(define-public (admintransfer (nft <nft-trait>) (id uint) (name (string-utf8 256)) (nft2name <nft-trait>) (id2 uint))
  (begin 
   (asserts! (is-administrative tx-sender) err-not-authorized)
   (trnsfer nft id name nft2name id2) 
  )
...
;;functions
(define-private (del (nft <nft-trait>) (id uint) (name (string-utf8 256)))
  (let ((owner (unwrap! (unwrap! (contract-call? nft get-owner id) err-not-found) err-not-found)))
   (begin 
...
(define-private (trnsfer (nft <nft-trait>) (id uint) (name (string-utf8 256)) (nft2name <nft-trait>) (id2 uint))
   (let (
