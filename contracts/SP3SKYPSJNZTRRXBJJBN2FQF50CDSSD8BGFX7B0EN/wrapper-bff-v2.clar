...
(define-public (router-wrapper (name (string-ascii 128)) (instructions (buff 4096)))
    (begin
        (if (is-eq name "swap-stx-usdh")
            ;; #[filter(instructions)]
            (swap-stx-usdh instructions)
            (err u404)
        )
    )
)
...
(define-private (swap-stx-usdh (params (buff 4096)))
    (let
        (
            (params-deserialized (unwrap! (from-consensus-buff? {
                amount: uint,
                min-receive: uint
            } params) ERR_INVALID_INSTRUCTIONS))
            (params-amount (get amount params-deserialized))
            (params-min-receive (get min-receive params-deserialized))
        )
        (begin
            (try! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.router-stableswap-xyk-v-1-3 swap-helper-a
             params-amount
             params-min-receive
             none
             true
