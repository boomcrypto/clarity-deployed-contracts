...
(define-non-fungible-token color uint)
...
(define-data-var total-price uint u30000000000)
(define-data-var artist-address principal 'SP2AAMRF8P81SA1BZNH9M64YC6G9F9QG0B26VSRE4)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSAojtYDbh4sD6x9rLTpBy5hWbxopBaPwBBFc4aegobis/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? color next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? color token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? color token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? color token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? color id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? color id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? color id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u99526027)
