...
(define-constant ERR_EXCEED_MAX_PLAYERS_COUNT 1004)
...
(define-constant MAX_BETS_EACH_SIDE u400)
(define-constant DRAW_BET_NUM_THRESHOLD u2)     ;;TODO:u10
(define-constant HIT_SHARES u1000000)
(define-constant ONE_ROUND_DURATION_BLOCKS u1) ;;TODO u36 6 hours
(define-constant DRAW_CD_BLOCKS u1)            ;;TODO u3 ;; 0.5 hour
(define-constant CANCEL_CD_BLOCKS u1)         ;;TODO u48 ;; 8 hours
...
(define-constant LIST_400 (list
    ...
    u351 u352 u353 u354 u355 u356 u357 u358 u359 u360 u361 u362 u363 u364 u365 u366 u367 u368 u369 u370 u371 u372 u373 u374 u375 
    u376 u377 u378 u379 u380 u381 u382 u383 u384 u385 u386 u387 u388 u389 u390 u391 u392 u393 u394 u395 u396 u397 u398 u399 u400
))
...
(define-data-var price-list (list 3 uint) (list u1000000 u10000000 u100000000))
...
(define-data-var draw-award-per-share uint u0)
...
(define-public (bet (bet-type uint) (bet-round uint) (bet-v uint) (bet-s uint) (stx-cost uint))
  (let
    ...
    (asserts! (and (<= v-num MAX_BETS_EACH_SIDE) (<= s-num MAX_BETS_EACH_SIDE)) (err ERR_EXCEED_MAX_PLAYERS_COUNT))
    (asserts! (or (is-eq bet-v u0) (and (>= bet-v u1) (<= bet-v u50))) (err ERR_INVALID_BET_VALUE))
    (asserts! (or (is-eq bet-s u0) (and (>= bet-s u51) (<= bet-s u100))) (err ERR_INVALID_BET_VALUE))
    ...
    (let
      (
        (bet-value (+ (* u100 bet-v) bet-s))
        (start-time (if (is-eq (get start-time bet-summary-tuple) u0)
                      (default-to u0 (get-block-info? time start-at))
                      (get start-time bet-summary-tuple)))
    ...
    ;; if (last-round has not draw)
    ;;    if end-time hasn't be marked, try mark it.
    ;;    if (has passed more than 0.5 hour since last-round-end-time) then (draw last round)
    (let
      (
        (last-round (- cur-round u1))
        (last-bet-summary-tuple-opt (map-get? bet-summary-map { bet-type: bet-type, round: last-round }))
      )
      (match last-bet-summary-tuple-opt last-bet-summary-tuple
        (if (is-eq (get rand-num last-bet-summary-tuple) u0)
          (begin
            ;; try mark end-time
            (if
              (and
                (is-eq (get end-time last-bet-summary-tuple) u0)
                (> block-height (get end-at last-bet-summary-tuple))
              )
              (map-set bet-summary-map
                { bet-type: bet-type, round: last-round }
                (merge last-bet-summary-tuple {
                  end-time: (default-to u0 (get-block-info? time (get end-at last-bet-summary-tuple)))
                })
              )
              true          
            )
            ;; try draw
            (if (> block-height (+ (get end-at last-bet-summary-tuple) DRAW_CD_BLOCKS))
              (draw bet-type last-round)
              true
            )
          )
          true
    ...
    ;; if (has passed 6 hours since round start) and (total bets >= 10) and (small-side-bet-count*2 > big-side-bet-count) then (end cur round, start next round)
    (if
      (and
        (> block-height (+ start-at ONE_ROUND_DURATION_BLOCKS))
        (>= (+ v-num s-num) DRAW_BET_NUM_THRESHOLD)
        (> (* v-num u2) s-num)
        (> (* s-num u2) v-num)
      )
      (let
        (
          (cur-bet-summary-tuple (unwrap-panic (map-get? bet-summary-map { bet-type: bet-type, round: cur-round })))
        )
        (map-set bet-summary-map
          { bet-type: bet-type, round: cur-round }
...
(define-private (draw (bet-type uint) (round uint))
  (let
    (
      ...
      (rand-num (calc-round-rand-num bet-type round))
      (is-v (< rand-num u51))
    )
    ;;
    ...
    (filter calc-shares-loop LIST_400)

    ;; calc each award per share
    (var-set draw-award-per-share (/ (var-get draw-award-pool-num) (var-get draw-total-shares)))
    ...
    (match (map-get? bet-record-map { bet-type: bet-type, round: round, index: index }) bet-record-tuple
      (let
        (
          ...
          (bet-v (/ bet-value u100))
          (bet-s (mod bet-value u100))
          (shares (
            if (<= rand-num u50)
              (if (is-eq bet-v u0)
                u0
    ...
    (match (map-get? bet-record-map { bet-type: bet-type, round: round, index: index }) bet-record-tuple
      (let
        (
          ...
          (origin-award (* (get shares bet-record-tuple) (var-get draw-award-per-share)))
          (award
            (if (> origin-award bet-price)
              (/ (* origin-award u98) u100)
              origin-award
            )
          )
          (player (get player bet-record-tuple))
          (bet-value (get bet-value bet-record-tuple))
          (is-bet-both-side (and (> bet-value u100) (> (mod bet-value u100) u0)))
          (win-num
            (if is-bet-both-side
              (- (to-int award) (to-int bet-price))
              (if (> origin-award u0) (to-int award) (- 0 (to-int bet-price)))
            )
          )
  ...
  (let
    ...
    (match (map-get? bet-record-map { bet-type: bet-type, round: round, index: index }) bet-record-tuple
      (let 
        (
          (bet-value (get bet-value bet-record-tuple))
          (bet-v-flag (> bet-value u100))
          (bet-s-flag (> (mod bet-value u100) u0))
          (player (get player bet-record-tuple))
...
(define-read-only (get-player-bet-info (bet-type uint) (player principal) (index uint))
  (map-get? player-record-map { player: player, bet-type: bet-type, index: index })
)

(define-read-only (get-bet-records (key-list (list 50 { bet-type: uint, round: uint, index: uint })))
