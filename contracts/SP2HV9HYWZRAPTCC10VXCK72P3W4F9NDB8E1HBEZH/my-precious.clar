...
(define-non-fungible-token my-precious uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmWS82zX3DVmJQtMWpEf4X2EiZ61J4qZX9r7dHXTQAvkVE/my-precious/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP2HV9HYWZRAPTCC10VXCK72P3W4F9NDB8E1HBEZH )) 
...
(define-data-var public-target-block-height uint u64117)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? my-precious id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace my-precious
  (ok (nft-get-owner? my-precious id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? my-precious next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? my-precious id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? my-precious id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? my-precious id) ERR-NOT-FOUND))
