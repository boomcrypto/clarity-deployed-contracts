...
(define-public (on-granite-flash-loan (amount uint) (fee uint) (data (optional (buff 20480))))
  (let (
    ...
    (asserts! (or (is-eq dex u1) (is-eq dex u2) (is-eq dex u999)) ERR-INVALID-DEX)
    (try! (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer amount sender SELF none))
    (if (is-eq dex u999)
      (try! (liqudate-and-swap-inner-usdh (get pyth-price-feed-data cdata) (get batch cdata) (get btc-price cdata) (get price-slippage-tolerance cdata)))
...
(define-public (liquidate-with-swap-usdh
  ...
  (begin 
    (asserts! (or (is-owner) (is-operator)) ERR-UNAUTHORIZED)
    (asserts! (> deadline (default-to u0 (get-stacks-block-info? time (- stacks-block-height u1)))) ERR-TIMEOUT)
    (try! (liqudate-and-swap-inner-usdh pyth-price-feed-data batch btc-price price-slippage-tolerance))
    SUCCESS
  )
)
...
(define-read-only (compute-mintable-usdh (btc-amount uint) (btc-price uint))
  (let (
    ...
    (bps-base   u10000)
    (token-base u100000000) ;; 8 decimals same for: btc, pyth btc price, usdh
    (slippage-amount (/ (* btc-price price-slippage-bps) bps-base))
    (effective-price (- btc-price slippage-amount))
    (numerator (* btc-amount effective-price))
    (denominator (* token-base token-base))
    (raw-result (/ (* numerator token-base) denominator))
  )
    raw-result
  )
)
