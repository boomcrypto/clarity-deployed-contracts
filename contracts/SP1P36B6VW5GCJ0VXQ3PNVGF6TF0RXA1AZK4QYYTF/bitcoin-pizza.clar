...
(define-non-fungible-token bitcoin-pizza uint)
...
(define-data-var artist-address principal 'SP1P36B6VW5GCJ0VXQ3PNVGF6TF0RXA1AZK4QYYTF)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUDaA8iZF6d2HyBGapmu2WMjirq5URWL4vL2odM2fFxx9/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-pizza next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-pizza token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-pizza token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-pizza token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-pizza id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-pizza id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-pizza id) (err ERR-NOT-FOUND)))
