...
(define-non-fungible-token bitcoin-rabbits uint)
...
;; Internal variables
(define-data-var mint-limit uint u1332)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQYE5WGGNjUnePkfK7fCh1naTNeMbPt7fn6ttfSfQKQon/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-rabbits next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-rabbits token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-rabbits token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-rabbits token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-rabbits id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-rabbits id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-rabbits id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u15056)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u13726000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u10492000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u58000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u618000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u4900000000)
