...
(define-non-fungible-token btc-rabbits uint)
...
;; Internal variables
(define-data-var mint-limit uint u331)
(define-data-var last-id uint u1)
(define-data-var total-price uint u30000000)
(define-data-var artist-address principal 'SP1A99Q1V2W61A985YCT9HJGWMTCDN2K6WCSTASNP)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYBmhUucwstV4deXtsKh16vicLy27mNrZMx6D11Ts4Dpz/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? btc-rabbits next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? btc-rabbits token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? btc-rabbits token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? btc-rabbits token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? btc-rabbits id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? btc-rabbits id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? btc-rabbits id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u86621)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u33000000)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u80601000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u58697000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u340000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u3653000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u28800000000)
