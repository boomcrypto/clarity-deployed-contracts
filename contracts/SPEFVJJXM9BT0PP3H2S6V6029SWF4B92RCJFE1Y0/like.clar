...
(define-public (like (tid uint))
  (let
    (
      (sender_tid (unwrap! (contract-call? .laser_eyes get_id_by_player tx-sender) (err ERR_NO_AUTHORITY)))
      (liked_owner (unwrap! (contract-call? .laser_eyes get_player_by_id tid) (err ERR_TOKEN_NOT_EXIST)))
    ...
    (try! (contract-call? .laser_eyes deduct (- like_price liked_award)))
    ...
    (contract-call? .laser_eyes set_ud tid u6 u4 liked_cnt)
  )
)
