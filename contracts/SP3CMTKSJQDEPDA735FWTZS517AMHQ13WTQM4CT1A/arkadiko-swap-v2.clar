...
(define-public (swap-x-for-y (amountIn uint) (amountIn2 uint))
  (let (
    (b1 (unwrap-panic (swap-welsh-wstx-arkadiko amountIn)))
    (b2 (unwrap-panic (swap-wstx-welsh-alex amountIn2)))
  )
    (begin
      (asserts! (> b2 amountIn) (err ERR-COOLDOWN))
      (asserts! (is-eq CONTRACT-OWNER tx-sender) (err ERR-ALL-MINTED))
...
(define-public (swap-y-for-x (amountIn uint) (amountIn2 uint))
  (let (
    (b1 (unwrap-panic (swap-welsh-wstx-alex amountIn)))
    (b2 (unwrap-panic (swap-wstx-welsh-arkadiko amountIn2)))
  )
    (begin
      (asserts! (> b2 amountIn) (err ERR-COOLDOWN))
      (asserts! (is-eq CONTRACT-OWNER tx-sender) (err ERR-ALL-MINTED))
