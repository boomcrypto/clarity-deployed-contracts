;; hipster-collection-animals
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token hipster-collection-animals uint)
...
;; Internal variables
(define-data-var mint-limit uint u94)
(define-data-var last-id uint u1)
(define-data-var total-price uint u750000)
(define-data-var artist-address principal 'SP5BHGWQ5MYY8JGGPW0K74Y6JZT6S4NTYRDN6BNJ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmf6mq2knXccNPrLQeC2LC9htiZTYGjf7wwd2hh2gW54Cf/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? hipster-collection-animals next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? hipster-collection-animals token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? hipster-collection-animals token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? hipster-collection-animals token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? hipster-collection-animals id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? hipster-collection-animals id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? hipster-collection-animals id) (err ERR-NOT-FOUND)))
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? hipster-collection-animals (+ last-nft-id u0) 'SP5BHGWQ5MYY8JGGPW0K74Y6JZT6S4NTYRDN6BNJ))
      (map-set token-count 'SP5BHGWQ5MYY8JGGPW0K74Y6JZT6S4NTYRDN6BNJ (+ (get-balance 'SP5BHGWQ5MYY8JGGPW0K74Y6JZT6S4NTYRDN6BNJ) u1))
      (try! (nft-mint? hipster-collection-animals (+ last-nft-id u1) 'SP3CF4JWY44MWYFCC6BMV6K9NTEVCQ9CGE9EF1WZT))
