;; baby-monkey-collection
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token baby-monkey-collection uint)
...
(define-data-var artist-address principal 'SP1NJYY6VKMQCR41KRDTHKYTH51QM2RFGBY9X5QT3)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qma75UJWfLfs9eb5rDXVcKnWG7EtaXQqfb3K7QsyBFMkLF/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? baby-monkey-collection next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? baby-monkey-collection token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? baby-monkey-collection token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? baby-monkey-collection token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? baby-monkey-collection id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? baby-monkey-collection id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? baby-monkey-collection id) (err ERR-NOT-FOUND)))
