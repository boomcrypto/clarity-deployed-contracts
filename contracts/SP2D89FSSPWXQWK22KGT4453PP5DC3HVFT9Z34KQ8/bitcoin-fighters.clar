...
(define-non-fungible-token bitcoin-fighters uint)
...
(define-data-var artist-address principal 'SP2D89FSSPWXQWK22KGT4453PP5DC3HVFT9Z34KQ8)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRJ8tiTpsNgzgh6ZrnVae9Mq3XbdvT726UCymMqvu8Mq7/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-fighters next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-fighters token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-fighters token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-fighters token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-fighters id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-fighters id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-fighters id) (err ERR-NOT-FOUND)))
