...
(define-public (supply
  ...
  (let ((asset-principal (contract-of asset)))
    ...
    (try! (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-borrow-v2-2 supply lp pool-reserve asset amount owner))
...
(define-public (borrow
  ...
  (let ((asset-principal (contract-of asset-to-borrow)))
    ...
    (try! (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-borrow-v2-2 borrow pool-reserve oracle asset-to-borrow lp assets amount-to-be-borrowed fee-calculator interest-rate-mode owner))
...
(define-public (repay
  ...
  (let (
    ...
    (payback-amount (try! (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-borrow-v2-2 repay asset amount-to-repay on-behalf-of payer)))
    )
...
(define-public (set-user-use-reserve-as-collateral
  ...
  (let (
    ...
    (asserts! (is-eq tx-sender contract-caller) ERR_UNAUTHORIZED)
    (try! (write-feed price-feed-bytes))
    (try! (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-borrow-v2-2 set-user-use-reserve-as-collateral who lp-token asset enable-as-collateral oracle assets-to-calculate))
...
(define-public (withdraw
  ...
  (let (
    ...
    (withdraw-res (try! (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-borrow-v2-2 withdraw pool-reserve asset lp oracle assets amount owner)))
    )
...
(define-public (liquidation-call
  ...
  (let (
    ...
    (try! (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-borrow-v2-2 liquidation-call
      assets
      collateral-lp
...
(define-public (set-e-mode
  ...
  (begin
    (asserts! (is-eq tx-sender contract-caller) ERR_UNAUTHORIZED)
    (try! (write-feed price-feed-bytes))
    (try! (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-borrow-v2-2 set-e-mode user assets new-e-mode-type))
