...
(define-read-only (is-isolated-asset (asset principal))
  (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-0-reserve-v2-0 is-isolated-type asset)
)
...
(define-read-only (is-borroweable-in-isolation (asset principal))
  (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-0-reserve-v2-0 is-borroweable-isolated asset)
)
...
(define-read-only (get-max-borroweable
  ...
  (let ((price (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.aeusdc-oracle-v1-0 get-price)))
    (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.math-v2-0 mul-to-fixed-precision (- useable-collateral borrowed-collateral) decimals price)
  )
)
...
(define-read-only (get-borrowed-balance-user-usd-sbtc (who principal))
  (let (
    ...
    (unit-price (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.sbtc-oracle-v1-0 get-price))
  )
    (token-to-usd (get compounded-balance balance) u8 unit-price)
  )
)

(define-read-only (get-borrowed-balance-user-usd-ststxbtc-v2 (who principal))
  (let (
    (balance (get-borrowed-balance-user-ststxbtc-v2 who))
...
(define-read-only (get-borrowed-balance-user-usd-alex (who principal))
  (let (
    (balance (get-borrowed-balance-user-alex who))
    (unit-price (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.alex-oracle-v1-0 get-price))
  )
    (token-to-usd (get compounded-balance balance) u8 unit-price)
  )
)
...
(define-read-only (get-borrowed-balance-user-stx (who principal))
  (get-user-borrow-balance who 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.wstx)
)
...
(define-read-only (get-borrowed-balance-user-sbtc (who principal))
  (get-user-borrow-balance who 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token)
)

(define-read-only (get-borrowed-balance-user-ststxbtc-v2 (who principal))
  (get-user-borrow-balance who 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststxbtc-token-v2)
)

(define-read-only (get-borrowed-balance-user-alex (who principal))
  (get-user-borrow-balance who 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex)
)
...
(define-read-only (get-user-borrow-balance (who principal) (reserve principal))
  (let (
    (user-data (unwrap-panic (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-reserve-data get-user-reserve-data-read who reserve)))
    (reserve-data (unwrap-panic (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-reserve-data get-reserve-state-read reserve)))
