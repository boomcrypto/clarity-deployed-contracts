...
(define-constant asset-addr 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststxbtc-token-v2)
(define-constant decimals u6)

(define-read-only (get-total-supply)
  (contract-call? .zststxbtc-v2-token get-total-supply))

(define-read-only (get-name)
  (contract-call? .zststxbtc-v2-token get-name))

(define-read-only (get-symbol)
  (contract-call? .zststxbtc-v2-token get-symbol))

(define-read-only (get-decimals)
  (contract-call? .zststxbtc-v2-token get-decimals))

(define-read-only (get-token-uri)
  (contract-call? .zststxbtc-v2-token get-token-uri))
...
(define-private (transfer-internal (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (contract-call? .zststxbtc-v2-token transfer amount sender recipient memo)
)

(define-private (mint-internal (amount uint) (owner principal))
  (contract-call? .zststxbtc-v2-token mint amount owner)
)

(define-private (burn-internal (amount uint) (owner principal))
  (contract-call? .zststxbtc-v2-token burn amount owner)
)
...
(define-read-only (get-principal-balance (account principal))
  (ok
    (contract-call? .zststxbtc-v2-token get-balance account)
  )
)
...
(define-public (set-contract-owner (owner principal))
  (begin
    ...
    (print { type: "set-contract-owner-zststxbtc-v2", payload: owner })
