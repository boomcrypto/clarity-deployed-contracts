...
(define-constant ststxbtc-contract-v2 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststxbtc-token-v2)
...
(define-read-only (get-price (token principal))
  (let (
    ...
    (price-feed-id (get price-feed-id price-feed))
    (sigma-mu (get sigma-mu price-feed))
    (oracle-data (try! (contract-call? 'SP1CGXWEAMG6P6FT04W66NVGJ7PQWMDAC19R7PJ0Y.pyth-storage-v4 get-price price-feed-id)))
    (expo (get expo oracle-data))
...
(define-private (convert-stx-to-ststx (stx-price uint))
  (let (
    (ratio
      (try!
        (contract-call? 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.data-core-v3
          get-stx-per-ststx
...
(define-read-only (convert-stx-to-ststx-read (stx-price uint))
  (let (
    ...
    (ststxbtc-supply-v2 (unwrap-panic (contract-call? 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.ststxbtc-token-v2 get-total-supply)))
    (stx-for-ststx (- total-stx-amount ststxbtc-supply ststxbtc-supply-v2))
    (ratio (contract-call? 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.data-core-v3 get-stx-per-ststx-helper stx-for-ststx))
  )
    (ok (/ (* stx-price ratio) stx-den))
  )
)
...
;; sigma/mu of 0.5%
(map-set assets sbtc-contract { price-feed-id: btc-price-feed-id, sigma-mu: u1000000 })
;; 0.5%
(map-set assets wstx-contract { price-feed-id: stx-price-feed-id, sigma-mu: u1000000 })
(map-set assets ststxbtc-contract { price-feed-id: stx-price-feed-id, sigma-mu: u1000000 }) ;; same price as STX
(map-set assets ststxbtc-contract-v2 { price-feed-id: stx-price-feed-id, sigma-mu: u1000000 }) ;; same price as STX
(map-set assets ststx-contract { price-feed-id: stx-price-feed-id, sigma-mu: u1000000 })