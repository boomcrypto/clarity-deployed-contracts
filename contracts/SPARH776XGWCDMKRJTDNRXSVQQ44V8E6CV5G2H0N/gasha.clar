...
(define-non-fungible-token gasha uint)
...
(define-data-var artist-address principal 'SPARH776XGWCDMKRJTDNRXSVQQ44V8E6CV5G2H0N)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSYMcW2Gd7xzy4odiSMPhT6xSirn5LNBQiw4fPqfmDQ37/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? gasha next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gasha token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gasha token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gasha token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gasha id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gasha id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gasha id) (err ERR-NOT-FOUND)))
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? gasha (+ last-nft-id u0) 'SPARH776XGWCDMKRJTDNRXSVQQ44V8E6CV5G2H0N))
      (map-set token-count 'SPARH776XGWCDMKRJTDNRXSVQQ44V8E6CV5G2H0N (+ (get-balance 'SPARH776XGWCDMKRJTDNRXSVQQ44V8E6CV5G2H0N) u1))
