...
(define-non-fungible-token qpwnice uint)
...
(define-data-var total-price uint u100000000000)
(define-data-var artist-address principal 'SP1S4H94H1BS25PJDD7HD360VEZRMPF8SZSWDAQ31)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXSh8V5p6kTiJHMJTFLWHKTUNE1QaHqdhCNLERUWuyuUp/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? qpwnice next-id tx-sender) next-id)
      (unwrap! (nft-transfer? qpwnice next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? qpwnice token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? qpwnice token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? qpwnice token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? qpwnice id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? qpwnice id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? qpwnice id) (err ERR-NOT-FOUND)))
