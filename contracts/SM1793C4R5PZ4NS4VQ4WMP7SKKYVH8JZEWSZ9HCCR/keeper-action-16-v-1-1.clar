...
(define-public (execute-action 
  ...
  (let (
    ...
    (keeper-fee-amount (unwrap! (contract-call? .keeper-4-helper-v-1-1 get-keeper-fee-amount lp-to-restake) ERR_INVALID_HELPER_DATA))
...
(define-private (xyk-unstake-lp
  ...
  (let (
    ...
    ;; Calculate updated lp-to-transfer
    (updated-lp-to-transfer (if (is-eq lp-to-transfer u1) u0 lp-to-transfer))
    ...
    (lp-to-restake (if transfer-to-owner (- total-lp-unstaked updated-lp-to-transfer) total-lp-unstaked))
  )
    ;; Transfer updated-lp-to-transfer LP tokens from the contract to owner-address if transfer-to-owner is true
    (if (and transfer-to-owner (> updated-lp-to-transfer u0)) (try! (contract-call? token-trait-a transfer updated-lp-to-transfer tx-sender owner-address none)) false)
