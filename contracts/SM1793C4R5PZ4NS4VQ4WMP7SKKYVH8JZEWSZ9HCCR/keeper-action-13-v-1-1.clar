...
(define-private (stableswap-withdraw-liquidity
  ...
  (let (
    ...
    (withdraw-liquidity-result (try! (contract-call? .stableswap-core-v-1-4 withdraw-proportional-liquidity .stableswap-pool-stx-ststx-v-1-4 token-trait-a token-trait-b amount-after-keeper-fee u1 u1)))
    (x-amount (get x-amount withdraw-liquidity-result))
    (y-amount (get y-amount withdraw-liquidity-result))
  )
