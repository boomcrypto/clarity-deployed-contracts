...
(define-non-fungible-token beardominant uint)
...
;; Internal variables
(define-data-var mint-limit uint u117)
(define-data-var last-id uint u1)
(define-data-var total-price uint u20000000)
(define-data-var artist-address principal 'SP1Q6XETSRPX64Y9PDQKQRJJQKPCX2QQFMQZZSNNC)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTbURMMy56KnNB9bj8x4EKVRT96ZSD6BBtWREBwann66p/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? beardominant next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? beardominant token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? beardominant token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? beardominant token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? beardominant id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? beardominant id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? beardominant id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mega uint u1800)
