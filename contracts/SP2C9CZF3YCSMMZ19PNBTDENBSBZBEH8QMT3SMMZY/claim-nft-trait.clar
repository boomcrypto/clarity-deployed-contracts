...
{
  a: "list-in-ustx",
  commission: 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait-commission-v1,
  id: u912,
  price: u4672897196262000000,
  royalty: u500
}

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token sbtc-maps uint)
...
;; Internal variables
(define-data-var mint-limit uint u100000000)
(define-data-var last-id uint u5000000000000)
(define-data-var total-price uint u50000000000000)
(define-data-var artist-address principal 'SPCH3FRRNCZXZEMKFNYPTF0PEM8SJXV2SC93SNY4)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbURxQ1wz5VqYmDKjQi2ASfytTcte3Cmnf6tyPnfbxLw3/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? sbtc-maps next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? sbtc-maps token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? sbtc-maps token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sbtc-maps token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sbtc-maps id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sbtc-maps id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sbtc-maps id) (err ERR-NOT-FOUND)))
