...
;; Room: https://neoswap.xyz/rooms/JfCHcQo4as3ZuYAtNe7U
;; SwapId: 8kZkaTDibo6bbbboN3Sz
...
(define-constant TRADER-3 'SP33ME1B0RGW30BC0HKDK4EYR9B5W2G6JJBW8MHC2)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (contract-call? 'SP1XJ6GNTDVF6HR1VHPQDMFZJY87D9W6TGMH3QCP.meta-structures transfer u9 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	...
	(if (is-eq tx-sender TRADER-3)
            (begin
		(unwrap! (stx-transfer? u6644000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP1XJ6GNTDVF6HR1VHPQDMFZJY87D9W6TGMH3QCP.meta-structures transfer u9 tx-sender TRADER-2)))
	...
	(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-robot-component-nft transfer u8045 tx-sender TRADER-3)))

	(unwrap-panic (as-contract (stx-transfer? u474000 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (stx-transfer? u672000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u5498000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP1XJ6GNTDVF6HR1VHPQDMFZJY87D9W6TGMH3QCP.meta-structures transfer u9 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-robot-component-nft transfer u8045 tx-sender TRADER-2)))
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-3)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u6644000 tx-sender TRADER-3)))
	    )
            true
        )
