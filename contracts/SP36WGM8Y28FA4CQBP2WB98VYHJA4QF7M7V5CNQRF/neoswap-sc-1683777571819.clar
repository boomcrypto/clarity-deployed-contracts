...
;; Room: https://neoswap.xyz/rooms/OBkxLL7zeHAz0ua5jp18
;; SwapId: PM2EhOUc3j5b7Lmqin1N

;; traders
(define-constant TRADER-1 'SP1DFYHBWDW2YG4YD2HKWDWNKFFQVEAJS5XWGSRPK)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP2BE8TZATXEVPGZ8HAFZYE5GKZ02X0YDKAN7ZTGW.okcoin-airdrop transfer u5923 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
		(unwrap! (contract-call? 'SPYMQGWH0SXVZQY2R46CYWVX1CEVHHV34H76DC49.in-bitcoin-we-trust transfer u216 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP2BE8TZATXEVPGZ8HAFZYE5GKZ02X0YDKAN7ZTGW.okcoin-airdrop transfer u5923 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (contract-call? 'SPYMQGWH0SXVZQY2R46CYWVX1CEVHHV34H76DC49.in-bitcoin-we-trust transfer u216 tx-sender TRADER-1)))

	(unwrap-panic (as-contract (stx-transfer? u50000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u950000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2BE8TZATXEVPGZ8HAFZYE5GKZ02X0YDKAN7ZTGW.okcoin-airdrop transfer u5923 tx-sender TRADER-2)))
		(unwrap-panic (as-contract (contract-call? 'SPYMQGWH0SXVZQY2R46CYWVX1CEVHHV34H76DC49.in-bitcoin-we-trust transfer u216 tx-sender TRADER-2)))
	    )
            true
        )
