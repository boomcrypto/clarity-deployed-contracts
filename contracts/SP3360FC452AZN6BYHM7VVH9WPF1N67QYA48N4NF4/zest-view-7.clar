...
(define-private (acc-supply
  (asset principal)
  (ctx { user: principal, res: (list 100 { asset: principal, a-token-address: principal, balance: uint }) }))
  (match (contract-call? 'SP2VCQJGH7PHP2DJK7Z0V48AGBHQAW3R3ZW1QF4N.pool-reserve-data get-reserve-state-read asset)
    reserve-data
      (let (
        ...
        (balance (match (get-a-token-balance a-token-addr (get user ctx))
                   ok-val ok-val
                   err-val u0))
      )
        { user: (get user ctx),
