...
(define-read-only (get-name) (contract-call? 'SPXPN2CHPX5VWVA8MYAZVYYN0D95CTQCP36YB7SN.scihub get-name))
(define-read-only (get-symbol) (contract-call? 'SPXPN2CHPX5VWVA8MYAZVYYN0D95CTQCP36YB7SN.scihub get-symbol))
(define-read-only (get-base-decimals) (contract-call? 'SPXPN2CHPX5VWVA8MYAZVYYN0D95CTQCP36YB7SN.scihub get-decimals))
(define-read-only (get-token-uri) (contract-call? 'SPXPN2CHPX5VWVA8MYAZVYYN0D95CTQCP36YB7SN.scihub get-token-uri))
(define-read-only (get-balance (who principal)) (if (is-eq who VAULT) (ok (ft-get-balance wrapped-token who)) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SPXPN2CHPX5VWVA8MYAZVYYN0D95CTQCP36YB7SN.scihub get-balance who))))))
(define-read-only (get-total-supply) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SPXPN2CHPX5VWVA8MYAZVYYN0D95CTQCP36YB7SN.scihub get-total-supply)))))
...
(define-read-only (get-base-token)
  'SPXPN2CHPX5VWVA8MYAZVYYN0D95CTQCP36YB7SN.scihub)
  ...
  (begin
    ...
    (if (is-eq sender VAULT)
			(begin
				...
				(as-contract (contract-call? 'SPXPN2CHPX5VWVA8MYAZVYYN0D95CTQCP36YB7SN.scihub transfer (fixed-to-decimals amount) tx-sender recipient memo)))
			(begin
				(and (is-eq recipient VAULT) (try! (ft-mint? wrapped-token amount recipient)))
				(contract-call? 'SPXPN2CHPX5VWVA8MYAZVYYN0D95CTQCP36YB7SN.scihub transfer (fixed-to-decimals amount) tx-sender (if (is-eq recipient VAULT) (as-contract tx-sender) recipient) memo)))))
