...
(define-public (register-user (user principal))
  (begin
    ...
    (contract-call? .cross-bridge-registry-v1-01 register-user user)))
  ...
  (let
    ...
    (if (get burnable token-details)
      (begin
        ...
        (and (> fee u0) (try! (contract-call? token-trait transfer-fixed fee tx-sender .cross-bridge-registry-v1-01 none)))
      )
      (try! (contract-call? token-trait transfer-fixed amount-in-fixed tx-sender .cross-bridge-registry-v1-01 none))
    )
    ;; (as-contract (try! (contract-call? .cross-bridge-registry-v1-01 set-token-registry token-id (merge token-details { accrued-fee: (+ (get accrued-fee token-details) fee) }))))
    (as-contract (try! (contract-call? .cross-bridge-registry-v1-01 add-accrued-fee token-id fee)))
    (as-contract (try! (contract-call? .cross-bridge-registry-v1-01 set-token-reserve { token-id: token-id, chain-id: the-chain-id } (- (get-token-reserve-or-default token-id the-chain-id) amount-in-fixed))))
    (print {
...
(define-read-only (is-approved-operator-or-default (operator principal))
  (contract-call? .cross-bridge-registry-v1-01 is-approved-operator-or-default operator))
(define-read-only (is-approved-relayer-or-default (relayer principal))
  (contract-call? .cross-bridge-registry-v1-01 is-approved-relayer-or-default relayer))
(define-read-only (get-user-id (user principal))
  (contract-call? .cross-bridge-registry-v1-01 get-user-id user))
(define-read-only (get-user-id-or-fail (user principal))
  (contract-call? .cross-bridge-registry-v1-01 get-user-id-or-fail user))
(define-read-only (user-from-id (id uint))
  (contract-call? .cross-bridge-registry-v1-01 user-from-id id))
(define-read-only (user-from-id-or-fail (id uint))
  (contract-call? .cross-bridge-registry-v1-01 user-from-id-or-fail id))
(define-read-only (get-validator-id (validator principal))
	(contract-call? .cross-bridge-registry-v1-01 get-validator-id validator))
(define-read-only (get-validator-id-or-fail (validator principal))
	(contract-call? .cross-bridge-registry-v1-01 get-validator-id-or-fail validator))
(define-read-only (validator-from-id (id uint))
	(contract-call? .cross-bridge-registry-v1-01 validator-from-id id))
(define-read-only (validator-from-id-or-fail (id uint))
	(contract-call? .cross-bridge-registry-v1-01 validator-from-id-or-fail id))
(define-read-only (get-required-validators)
  (contract-call? .cross-bridge-registry-v1-01 get-required-validators))
(define-read-only (get-approved-chain-or-fail (the-chain-id uint))
  (contract-call? .cross-bridge-registry-v1-01 get-approved-chain-or-fail the-chain-id))
(define-read-only (get-token-reserve-or-default (the-token-id uint) (the-chain-id uint))
  (contract-call? .cross-bridge-registry-v1-01 get-token-reserve-or-default the-token-id the-chain-id))
(define-read-only (get-min-fee-or-default (the-token-id uint) (the-chain-id uint))
  (contract-call? .cross-bridge-registry-v1-01 get-min-fee-or-default the-token-id the-chain-id))
(define-read-only (get-approved-token-id-or-fail (token principal))
  (contract-call? .cross-bridge-registry-v1-01 get-approved-token-id-or-fail token))
(define-read-only (get-approved-token-by-id-or-fail (token-id uint))
  (contract-call? .cross-bridge-registry-v1-01 get-approved-token-by-id-or-fail token-id))
(define-read-only (get-approved-token-or-fail (token principal))
  (contract-call? .cross-bridge-registry-v1-01 get-approved-token-or-fail token))
(define-read-only (check-is-approved-token (token principal))
  (contract-call? .cross-bridge-registry-v1-01 check-is-approved-token token))
(define-read-only (is-order-sent-or-default (order-hash (buff 32)))
  (contract-call? .cross-bridge-registry-v1-01 is-order-sent-or-default order-hash))
(define-read-only (is-order-validated-by-or-default (order-hash (buff 32)) (validator principal))
  (contract-call? .cross-bridge-registry-v1-01 is-order-validated-by-or-default order-hash validator))
...
(define-public (transfer-to-wrap
    ...
    (let (
      ...
      (as-contract (try! (contract-call? .cross-bridge-registry-v1-01 set-token-reserve { token-id: (get token order), chain-id: (get chain-id order) } (+ (get-token-reserve-or-default (get token order) (get chain-id order)) (get amount-in-fixed order)))))
      (print {
        object: "cross-bridge-endpoint",
      ...
      (as-contract (contract-call? .cross-bridge-registry-v1-01 set-order-sent order-hash true))))
(define-public (transfer-to-launchpad
    ...
    (let (
      ...
      (asserts! (is-eq (try! (get-approved-token-id-or-fail token)) (get token order)) ERR-TOKEN-NOT-AUTHORIZED)
      (if (get burnable token-details)
        (as-contract (try! (contract-call? token-trait mint-fixed amount (as-contract tx-sender))))
        (as-contract (try! (contract-call? token-trait transfer-fixed amount tx-sender (as-contract tx-sender) none))))
      (as-contract (try! (contract-call? .cross-bridge-registry-v1-01 set-token-reserve { token-id: (get token order), chain-id: (get chain-id order) } (+ (get-token-reserve-or-default (get token order) (get chain-id order)) (get amount-in-fixed order)))))
      (print {
        object: "cross-bridge-endpoint",
      ...
      (as-contract (try! (contract-call? .cross-bridge-registry-v1-01 set-order-sent order-hash true)))
...
(define-private (validate-order (order-hash (buff 32)) (signature-pack { signer: principal, order-hash: (buff 32), signature: (buff 65)}))
  (let (
    ...
    (as-contract (contract-call? .cross-bridge-registry-v1-01 set-order-validated-by { order-hash: order-hash, validator: (get signer signature-pack) } true))))
