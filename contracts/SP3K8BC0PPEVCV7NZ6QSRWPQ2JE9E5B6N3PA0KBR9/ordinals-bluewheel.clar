...
(define-fungible-token ordinals-bluewheel)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-name (string-ascii 32) "Bluewheel OrdEx (Ordinals)")
(define-data-var token-symbol (string-ascii 32) "BORDEX")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/ordinals-bluewheel.json"))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance ordinals-bluewheel who))
)
(define-read-only (get-total-supply)
	(ok (ft-get-supply ordinals-bluewheel))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? ordinals-bluewheel amount recipient)
	)
)
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? ordinals-bluewheel amount sender)
	)
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? ordinals-bluewheel amount sender recipient))
