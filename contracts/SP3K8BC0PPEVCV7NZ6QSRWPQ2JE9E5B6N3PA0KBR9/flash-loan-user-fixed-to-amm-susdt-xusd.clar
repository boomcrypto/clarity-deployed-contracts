...
(define-public (execute (token <ft-trait>) (amount uint) (memo (optional (buff 16))))
    (let
        (   
            (swapped (try! (contract-call? .swap-helper-bridged-v1-1 swap-helper-from-amm .token-susdt .token-wstx .token-wxusd ONE_8 amount none)))
            (swapped-back (try! (contract-call? .amm-swap-pool-v1-1 swap-helper .token-wxusd .token-susdt u5000000 swapped none)))            
