...
(define-fungible-token brc20-ords)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-name (string-ascii 32) "ords (BRC20)")
(define-data-var token-symbol (string-ascii 32) "ords")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.alexlab.co/metadata/brc20-ords.json"))
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance brc20-ords who))
)
(define-read-only (get-total-supply)
	(ok (ft-get-supply brc20-ords))
)
...
(define-public (mint (amount uint) (recipient principal))
	(begin		
		...
		(ft-mint? brc20-ords amount recipient)
	)
)
(define-public (burn (amount uint) (sender principal))
	(begin
		(asserts! (or (is-ok (check-is-approved)) (is-ok (check-is-owner))) ERR-NOT-AUTHORIZED)
		(ft-burn? brc20-ords amount sender)
	)
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? brc20-ords amount sender recipient))
