		...
		(let (
			...
			(and (not (get-bitcoin-tx-mined-or-default (get bitcoin-tx tx))) 
				(begin 
					...
					(try! (contract-call? .indexer-registry-dev-preview-1 set-tx-mined (get bitcoin-tx tx) true))
				)
			)
			(var-set tx-hash-to-iter tx-hash)
			(try! (fold validate-signature-iter signature-packs (ok true)))
			(try! (contract-call? .indexer-registry-dev-preview-1 set-tx-indexed { tx-hash: (get bitcoin-tx tx), output: (get output tx), offset: (get offset tx) } { tick: (get tick tx), amt: (get amt tx), from: (get from tx), to: (get to tx) }))
			(and (> height (get up-to-block from-bal)) (try! (contract-call? .indexer-registry-dev-preview-1 set-user-balance { user: (get from tx), tick: (get tick tx) } { balance: (get from-bal tx), up-to-block: height })))
			(and (> height (get up-to-block to-bal)) (try! (contract-call? .indexer-registry-dev-preview-1 set-user-balance { user: (get to tx), tick: (get tick tx) } { balance: (get to-bal tx), up-to-block: height })))
			(ok true))
		prev-err
		previous-response))
