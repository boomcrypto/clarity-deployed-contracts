...
(define-read-only (extract-tx-ins-outs (tx (buff 4096)))
	(if (try! (contract-call? .clarity-bitcoin-v1-01 is-segwit-tx tx))
		(let
			(
				(parsed-tx (unwrap! (contract-call? .clarity-bitcoin-v1-01 parse-wtx tx) err-invalid-tx))
			)
			(ok { ins: (get ins parsed-tx), outs: (get outs parsed-tx) })
		)
		(let
			(
				(parsed-tx (unwrap! (contract-call? .clarity-bitcoin-v1-01 parse-tx tx) err-invalid-tx))
			)
			(ok { ins: (get ins parsed-tx), outs: (get outs parsed-tx) })
		)
	)
...
(define-read-only (validate-tx (tx (buff 4096)) (output-idx uint) (offset-idx uint) (order-idx uint) (token principal))
	(let (
			(tx-idxed (try! (contract-call? .oracle-v1-01 get-bitcoin-tx-indexed-or-fail tx output-idx offset-idx)))
			...
			(amt-in-fixed (decimals-to-fixed (get amt tx-idxed) (contract-call? .oracle-v1-01 get-tick-decimals-or-default (get tick tx-idxed))))
...
(define-public (finalize-peg-out (request-id uint) (tx (buff 4096)) (output-idx uint) (offset-idx uint) (token-trait <sip010-trait>))
	(let (
			...
			(token-details (try! (get-token-details-or-fail (get tick request-details))))
			(tx-idxed (try! (contract-call? .oracle-v1-01 get-bitcoin-tx-indexed-or-fail tx output-idx offset-idx)))
			(tx-mined-height (try! (contract-call? .oracle-v1-01 get-bitcoin-tx-mined-or-fail tx)))
			(amount-in-fixed (decimals-to-fixed (get amt tx-idxed) (contract-call? .oracle-v1-01 get-tick-decimals-or-default (get tick tx-idxed))))
