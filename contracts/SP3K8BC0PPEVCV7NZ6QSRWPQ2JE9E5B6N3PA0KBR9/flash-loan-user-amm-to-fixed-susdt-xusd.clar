...
(define-public (execute (token <ft-trait>) (amount uint) (memo (optional (buff 16))))
    (let
        (   
            (swapped (try! (contract-call? .amm-swap-pool-v1-1 swap-helper .token-susdt .token-wxusd u5000000 amount none)))   
            (swapped-back (try! (contract-call? .swap-helper-bridged-v1-1 swap-helper-to-amm .token-wxusd .token-wstx .token-susdt ONE_8 swapped none)))                                                
