...
(define-public (execute (token <ft-trait>) (amount uint) (memo (optional (buff 16))))
    (let
        (   
            (swapped (try! (contract-call? .amm-swap-pool-v1-1 swap-helper .token-wstx .token-wbtc ONE_8 amount none)))   
            (swapped-back (try! (contract-call? .swap-helper-v1-03 swap-helper .token-wbtc .token-wstx swapped none)))                                                
