...
(define-non-fungible-token gpt-girls uint)
...
;; Internal variables
(define-data-var mint-limit uint u230)
(define-data-var last-id uint u1)
(define-data-var total-price uint u2000000)
(define-data-var artist-address principal 'SPAZQCMNAWMM91XV77Q0FMQEEK84CC11JTXBZZE0)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNegQc2QvNfpRmfPPbT1Q17EbwqMBWVH13CqocDGj6VCn/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? gpt-girls next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gpt-girls token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gpt-girls token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gpt-girls token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gpt-girls id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gpt-girls id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gpt-girls id) (err ERR-NOT-FOUND)))
