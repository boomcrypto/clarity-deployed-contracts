...
(define-public (initiate-new-snapshot (details {
  ...
  (let ((snapshot-id (var-get next-snapshot-id)))
    ...
    (map-set snapshot-details snapshot-id {
  ...
  (let ((block-timestamp (unwrap-panic (get-stacks-block-info? time (- stacks-block-height u1)))))
    ...
    (if (is-eq (var-get next-snapshot-id) u0)
      ...
      (let (
          ...
          (current-snapshot (unwrap! (map-get? snapshot-details current-snapshot-id) ERR-MISSING-SNAPSHOT))
          (snapshot-data-uploaded (get snapshot-uploaded current-snapshot))
        )
        (asserts! snapshot-data-uploaded ERR-SNAPSHOT-INCOMPLETE)
        SUCCESS  
      )
    )
))
...
(define-private (ensure-current-snapshot-completed (new-snapshot-time uint))
  (if (is-eq (var-get next-snapshot-id) u0)
    ...
    (let (
        (current-snapshot-id (- (var-get next-snapshot-id) u1))
        (current-snapshot (unwrap! (map-get? snapshot-details current-snapshot-id) ERR-MISSING-SNAPSHOT))
        (snapshot-time (get snapshot-time current-snapshot))
        (snapshot-data-uploaded (get snapshot-uploaded current-snapshot))
...
(define-private (mark-snapshot-uploaded)
  (let (
        (current-snapshot-id (- (var-get next-snapshot-id) u1))
        (current-snapshot (unwrap! (map-get? snapshot-details current-snapshot-id) ERR-MISSING-SNAPSHOT))
      )
      (map-set snapshot-details current-snapshot-id {
        snapshot-time: (get snapshot-time current-snapshot),
        total-lp-shares: (get total-lp-shares current-snapshot),
