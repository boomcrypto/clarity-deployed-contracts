...
(define-non-fungible-token botp uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmchRS224pYHv19ZzshrFB6Jt9CKiFB3SG43dNyCbS8CjH/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? botp next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? botp token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? botp token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? botp token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? botp id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? botp id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? botp id) (err ERR-NOT-FOUND)))
