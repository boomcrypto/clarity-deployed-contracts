...
(define-data-var base-uri (string-ascii 80) "bafybeia7bhzvaualn2itihvxhvcjphrrdwa4qm7wrr4i4n7tx6dalb7qce/")
(define-data-var mint-price uint u33330000)
;; Percent u100 = 1%
(define-data-var royalty-percent uint u1000)
(define-data-var mint-limit uint u10)
(define-data-var max-supply uint u9)
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? Degen next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? Degen next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Degen id sender recipient)
        success
          (let
            (
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? Degen id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? Degen id) ERR_NO_OWNER))
