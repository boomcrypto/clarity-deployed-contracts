...
(define-non-fungible-token hussens uint)
...
(define-data-var artist-address principal 'SP5B14ABMWXZEWWXDN16MJEQWGGPQ6S7K8CK7BY4)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaXhsD6mkssUCgfAcQVzr6VXXLJzdr3ygpvzzvMPrgCJE/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? hussens next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? hussens token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? hussens token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? hussens token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? hussens id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? hussens id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? hussens id) (err ERR-NOT-FOUND)))
