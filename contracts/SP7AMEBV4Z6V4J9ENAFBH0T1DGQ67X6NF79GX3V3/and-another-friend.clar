...
(define-non-fungible-token And-Another-Friend uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://QmNxjnprDU7J1tbaZVtGAZN6pyQWpXjus96wsDqfPMt3cs/")
...
(define-private (mint-many-iter (user principal) (next-id uint))
  (begin
    (unwrap! (nft-mint? And-Another-Friend next-id user) next-id)
    (+ next-id u1)
  )
)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? And-Another-Friend id sender recipient)
        success
          (let
            ...
            (recipient-balance (get-balance recipient)))
              ...
              (try! (nft-mint? And-Another-Friend (var-get last-id) sender))
              (var-set last-id (+ (var-get last-id) u1))
              (try! (contract-call? .besties-forever claim tx-sender))
              (print "You must be doing this on purpose now, right?")
              (ok success))
        error (err error)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? And-Another-Friend token-id)))

(define-read-only (get-last-token-id)
  (ok (- (var-get last-id) u1)))

(define-read-only (get-token-uri (token-id uint))
    (ok (some (concat (concat (var-get ipfs-root) "and-another-friend") ".json"))))
