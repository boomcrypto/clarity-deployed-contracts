...
(define-non-fungible-token Friends-Are-Forever uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://QmUCSNZ674fgKfbzZobXu3gYpKu7fW9zdNXBUvfjqNjnFN/")
...
(define-private (mint-many-iter (user principal) (next-id uint))
  (begin
    (unwrap! (nft-mint? Friends-Are-Forever next-id user) next-id)
    (+ next-id u1)
  )
)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Friends-Are-Forever id sender recipient)
        success
          (let
            ...
            (recipient-balance (get-balance recipient)))
              ...
              (try! (nft-mint? Friends-Are-Forever (var-get last-id) sender))
              (var-set last-id (+ (var-get last-id) u1))
              (try! (contract-call? .another-friend claim tx-sender))
              (print "I thought you would have learned your lesson the first time...")
              (ok success))
        error (err error)))

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? Friends-Are-Forever token-id)))

(define-read-only (get-last-token-id)
  (ok (- (var-get last-id) u1)))

(define-read-only (get-token-uri (token-id uint))
    (ok (some (concat (concat (var-get ipfs-root) "Friends-Are-Forever") ".json"))))
