...
(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait) ;; sip-009 
...
(define-data-var mint-remainings uint u2500)  
...
(define-data-var base-uri (string-ascii 80) "ipfs://QmV6ryVzx9nBUS66fxusfF6qmCNzfvzHMNse4Jb3evKR4s/")
(define-data-var mint-principal principal tx-sender)
(define-data-var token-compare uint u0)
(define-data-var b-idx uint u0)
(define-data-var is-citymint-active bool false)
...
(define-public (init-mint (BAN uint))  
    (begin               
...
(define-public (change-token-mint-prices (BAN uint))
  (begin    
    (asserts! (is-authorized) ERR-NOT-AUTHORIZED)   
    (set-token-mint-price BAN)
    (ok true)))
...
(define-public (mint-with-token (target-wallet principal) (send-token <sip010-token>))
    (begin
        ...
        (asserts! (called-from-mint) ERR-NOT-AUTHORIZED)
        (asserts! (< (var-get last-id) MINT-LIMIT) ERR-SOLD-OUT)
        (asserts! (is-eq (contract-of send-token) BAN-COIN-CONTRACT) ERR-INVALID-TOKEN)
        
        (let ((next-id (+ u1 (var-get last-id)))
            (target-mint-id (unwrap-panic (cycle-random-id (var-get mint-remainings)))))              
            (match (nft-mint? Marbling target-mint-id target-wallet)
                success
                    (let ((mint-price (default-to u100000000 (map-get? token-mint-price (contract-of send-token))))
                    (new-total-balance (get-token-balance target-wallet))
                    (new-token-id-list (get-token-id-list target-wallet)))
                    (begin                        
...
(define-private (set-token-mint-price (BAN uint))
  (begin    
