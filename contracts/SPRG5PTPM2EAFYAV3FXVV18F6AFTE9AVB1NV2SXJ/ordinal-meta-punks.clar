...
(define-non-fungible-token ordinal-meta-punks uint)
...
(define-data-var artist-address principal 'SPRG5PTPM2EAFYAV3FXVV18F6AFTE9AVB1NV2SXJ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQ9YSSgUvpbP6XDqEKwpDudrBtjkzMFKU2dyUfvkTKQUe/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinal-meta-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinal-meta-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinal-meta-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinal-meta-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinal-meta-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinal-meta-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinal-meta-punks id) (err ERR-NOT-FOUND)))
