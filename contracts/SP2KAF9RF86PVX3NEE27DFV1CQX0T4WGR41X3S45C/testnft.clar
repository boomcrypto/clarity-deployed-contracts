...
(define-public (banana-mint (new-owner principal))
    ...
    (begin
      ...
      (match (nft-mint? testnft next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? testnft id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? testnft id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? testnft id) ERR-NOT-FOUND))
