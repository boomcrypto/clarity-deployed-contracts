...
(define-non-fungible-token openxis uint)
...
(define-constant MINT-LIMIT u9891)

;; Internal variables
(define-data-var last-id uint u0)
(define-data-var uri-prefix (string-ascii 256) "")
(define-data-var cost-per-mint uint u6040000)
(define-data-var commission uint u800)
(define-data-var payout uint u0)
(define-data-var target-block uint u37055)
(define-data-var ipfs-full-metadata (string-ascii 96) "ipfs://bafybeiamfyjbfebb5h7ud7ovmemtwqjcp24psyzej5xi6txyx5c56w2y74/openxis/openxis_metadata.json")
(define-data-var ipfs-root (string-ascii 83) "ipfs://bafybeiamfyjbfebb5h7ud7ovmemtwqjcp24psyzej5xi6txyx5c56w2y74/openxis/openxis_")
(define-data-var artist-address principal 'SP30WP3V6Z1QPG0F819NCW15TD79SXYVFVWHMAK5M)
  ...
  (let (
      ...
      (asserts! (>= block-height target) ERR-NOT-MINT-TIME)
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? openxis next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? openxis token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? openxis token-id)))
