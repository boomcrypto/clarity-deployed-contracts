...
(define-non-fungible-token Chocoloate-Moose uint)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Chocoloate-Moose id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace Chocoloate-Moose
  (ok (nft-get-owner? Chocoloate-Moose id)))
...
(define-public (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? Chocoloate-Moose next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? Chocoloate-Moose id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Chocoloate-Moose id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Chocoloate-Moose id) ERR-NOT-FOUND))
...
(define-public (set-mint-address)
  (let ((the-mint (map-get? mint-address true)))
    (asserts! (is-eq tx-sender .chocolate-moose-mint) ERR-NOT-AUTHORIZED)
...
(define-public (admin-mint-and-burn)
    (let (
      ...
      (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
      (asserts! (< (var-get last-id) MOOSE-LIMIT) ERR-SOLD-OUT)
      (match (nft-mint? Chocoloate-Moose next-id (as-contract tx-sender))
        success
        (let
        ((current-balance (get-balance (as-contract tx-sender))))
          (begin
            (var-set last-id next-id)
            (try! (nft-burn? Chocoloate-Moose next-id (as-contract tx-sender)))
            (ok true)))
        error (err (* error u10000)))
    )
)
