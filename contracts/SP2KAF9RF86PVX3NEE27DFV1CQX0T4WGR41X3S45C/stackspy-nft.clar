...
(define-non-fungible-token stackspy uint)
...
(define-constant MINT-LIMIT u3000)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var stx-cost-per-mint uint u50000000)
(define-data-var commission-1 uint u1000) 
(define-data-var royalty-1 uint u4950) 
(define-data-var royalty-2 uint u1350) 
(define-data-var royalty-3 uint u1350) 
(define-data-var royalty-4 uint u1350) 
...
(define-data-var commission-1-address principal 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C)
(define-data-var artist-1-address principal 'SPRTXZF3GKXJKQ877P56HHG9J1M8GAP2A91FC06B)
(define-data-var artist-2-address principal 'SP2MJ98NVBY9MZQ0BNQBBD32BRDCNDFT7QDDETAVQ)
(define-data-var artist-3-address principal 'SP37X3WKT41YDB66MRW86WYS0VYZ615STVTW8VBHM)
(define-data-var artist-4-address principal 'SPSN5FVTJJFWT6F5H403CPW26CD6TWJD7W4N0DQR)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stackspy id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace stackspy
  (ok (nft-get-owner? stackspy id)))
...
(define-public (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? stackspy next-id new-owner)
        success
        (let
        ((current-balance (get-balance new-owner)))
          (begin
            ;; (print payout)
            (try! (stx-transfer? commission-one tx-sender (var-get commission-1-address)))
            (try! (stx-transfer? royalty-one tx-sender (var-get artist-1-address)))
            (try! (stx-transfer? royalty-two tx-sender (var-get artist-2-address)))
            (try! (stx-transfer? royalty-three tx-sender (var-get artist-3-address)))
            (try! (stx-transfer? royalty-four tx-sender (var-get artist-4-address)))
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? stackspy id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stackspy id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stackspy id) ERR-NOT-FOUND))
...
(define-public (set-artist-1-address (address principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (var-set artist-1-address address)
    (ok true)))

(define-public (set-artist-2-address (address principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (var-set artist-2-address address)
    (ok true)))

(define-public (set-artist-3-address (address principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (var-set artist-3-address address)
    (ok true)))

(define-public (set-artist-4-address (address principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (var-set artist-4-address address)
    (ok true)))