...
(define-non-fungible-token alaska uint)
...
(define-constant ERR-AIRDROP-CALLED (err u511))
(define-constant STX-MINT-LIMIT u22)
...
(define-data-var stx-cost-per-mint uint u75000000)
(define-data-var commission-1 uint u800)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? alaska id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace alaska
  (ok (nft-get-owner? alaska id)))
...
(define-public (stx-mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? alaska next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? alaska id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? alaska id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? alaska id) ERR-NOT-FOUND))
...
(define-public (admin-airdrop)
  (let
    (
      (last-nft-id (+ u1 (var-get last-id)))
    )
    (begin
      ...
      (try! (nft-mint? alaska (+ last-nft-id u0) 'SP1GR33848GSTMFR955Z77DAB835XYE9FZG19Y7NX))
      (map-set token-count 'SP1GR33848GSTMFR955Z77DAB835XYE9FZG19Y7NX (+ (get-balance 'SP1GR33848GSTMFR955Z77DAB835XYE9FZG19Y7NX) u1))
      (try! (nft-mint? alaska (+ last-nft-id u1) 'SP1GR33848GSTMFR955Z77DAB835XYE9FZG19Y7NX))
      (map-set token-count 'SP1GR33848GSTMFR955Z77DAB835XYE9FZG19Y7NX (+ (get-balance 'SP1GR33848GSTMFR955Z77DAB835XYE9FZG19Y7NX) u1))
