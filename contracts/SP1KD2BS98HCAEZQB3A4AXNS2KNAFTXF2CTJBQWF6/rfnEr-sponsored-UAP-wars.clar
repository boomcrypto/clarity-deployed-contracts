...
(define-constant TOKEN_NAME "DRONES")
...
(define-constant DEPLOYER 'SP1KD2BS98HCAEZQB3A4AXNS2KNAFTXF2CTJBQWF6)
(define-constant POOL_SIZE u18857000000)
    ...
    (begin
        ...
        (if (is-eq tx-sender DEPLOYER)
            ...
            (begin
                ...
                (match (contract-call? 'SP1JBV7TE0490KNVM1VAM19KHZG0CPC9426YCY3ZF.drones-stxcity transfer POOL_SIZE tx-sender (as-contract tx-sender) none)
                    success
                    (begin
            ...
            (if (get is-sponsor player-data)
                (begin
                    ...
                    (let (
                        ...
                        (balance (unwrap-panic (contract-call? 'SP1JBV7TE0490KNVM1VAM19KHZG0CPC9426YCY3ZF.drones-stxcity get-balance (as-contract tx-sender))))
                    )
                        (asserts! (secp256k1-verify msg-hash signature TRUSTED_PUBLIC_KEY) (err ERR_INVALID_SIGNATURE))

                        (match (as-contract (contract-call? 'SP1JBV7TE0490KNVM1VAM19KHZG0CPC9426YCY3ZF.drones-stxcity transfer balance tx-sender DEPLOYER none))
                            success
                            (begin
...
(define-public (claim-reward (amount uint) (signature (buff 65)))
    (begin
        ...
        (let (
            ...
            (current-balance (unwrap-panic (contract-call? 'SP1JBV7TE0490KNVM1VAM19KHZG0CPC9426YCY3ZF.drones-stxcity get-balance (as-contract tx-sender))))
        )
            (asserts! (secp256k1-verify msg-hash signature TRUSTED_PUBLIC_KEY) (err ERR_INVALID_SIGNATURE))
            ...
            (let ((fee-result
                (if (not has-paid-fee)
                    (match (as-contract (contract-call? 'SP1JBV7TE0490KNVM1VAM19KHZG0CPC9426YCY3ZF.drones-stxcity transfer fee tx-sender STACKS_WARS_FEE_WALLET none))
                        fee-success
                        (begin
                ...
                (match (as-contract (contract-call? 'SP1JBV7TE0490KNVM1VAM19KHZG0CPC9426YCY3ZF.drones-stxcity transfer net-amount tx-sender recipient none))
                    reward-success
                    (begin
