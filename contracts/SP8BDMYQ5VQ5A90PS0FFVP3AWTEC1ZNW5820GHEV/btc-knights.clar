...
(define-non-fungible-token btc-knights uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmauDTNoVqvmG8UjJS1S5YAZq6dKYzV4NyEAL73HTvBjvb/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? btc-knights next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? btc-knights token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? btc-knights token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? btc-knights token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? btc-knights id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? btc-knights id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? btc-knights id) (err ERR-NOT-FOUND)))
