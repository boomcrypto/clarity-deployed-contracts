;; bitcoin-fashion-week-gold-pass
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token bitcoin-fashion-week-gold-pass uint)
...
(define-data-var artist-address principal 'SP1M91XD1D9GHBMBXYK4YNJQ8W6XZTDZH4Y0QAYND)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTn2QzJqs8vGbSB1GazwU7eNzvFn1dP3LtZrjp2RNUGoH/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? bitcoin-fashion-week-gold-pass next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-fashion-week-gold-pass token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-fashion-week-gold-pass token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-fashion-week-gold-pass token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-fashion-week-gold-pass id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-fashion-week-gold-pass id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-fashion-week-gold-pass id) (err ERR-NOT-FOUND)))
