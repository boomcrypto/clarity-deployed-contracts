...
(define-public (on-granite-flash-loan (amount uint) (fee uint) (data (optional (buff 20480))))
  (let (
    ...
    ;; Repay debt
    (try! (contract-call? 'SP3Y6GFKWN50HPA8RKRXMY0EXAJR9VXPY899P88JN.borrower-v1 repay amount (some user)))
    ;; Withdraw sBTC
    (try! (contract-call? 'SP3Y6GFKWN50HPA8RKRXMY0EXAJR9VXPY899P88JN.borrower-v1 remove-collateral pyth-price-feed-data 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token sbtc-to-withdraw (some user)))
    ;; Swap from the user holding in sBTC to aeUSDC    
    (let (
      (aeusdc-out (try! (swap-sbtc-to-aeusdc-bitflow sbtc-to-withdraw min-aeusdc-out)))
      (to-repay (+ amount fee))
      )
      (print {
