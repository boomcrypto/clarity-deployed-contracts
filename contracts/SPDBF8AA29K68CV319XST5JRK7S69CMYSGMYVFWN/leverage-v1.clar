...
(define-constant ERR-INVALID-CALLBACK-DATA (err u30001))
(define-constant ERR-TIMEOUT (err u30002))
...
(define-public (on-granite-flash-loan (amount uint) (fee uint) (data (optional (buff 20480))))
  (let (
    ...
    (let
      (
        ;; swap flash loan amount to sbtc
        (sbtc-out (try! (swap-aeusdc-to-sbtc-bitflow amount min-sbtc-out)))
      )
        ;; deposit obtained sBTC
        (try! (contract-call? 'SP3Y6GFKWN50HPA8RKRXMY0EXAJR9VXPY899P88JN.borrower-v1 add-collateral 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token sbtc-out (some user)))
        ;; borrow aeUSDC to repay the flash loan + fee
        (try! (contract-call? 'SP3Y6GFKWN50HPA8RKRXMY0EXAJR9VXPY899P88JN.borrower-v1 borrow pyth-price-feed-data borrow-amount (some user)))
        (print {
