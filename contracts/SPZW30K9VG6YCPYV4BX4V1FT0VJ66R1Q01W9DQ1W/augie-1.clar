...
(define-non-fungible-token augie uint)
...
;; Internal variables
(define-data-var mint-limit uint u87)
(define-data-var last-id uint u1)
(define-data-var total-price uint u44000000)
(define-data-var artist-address principal 'SP346MGAJ4V8HR2HWPC7Q5NTY2E83MF9SAPMG2GM3)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXebmitnwgkx23sekehM1y6zMGi1BNcFmcQ3cFjxA8JAa/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? augie next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? augie token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? augie token-id)))
