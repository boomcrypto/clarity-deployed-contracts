...
(define-non-fungible-token nebula uint)
...
(define-data-var artist-address principal 'SP3G73V7XPDHVDJ584NWVKJQPH2TZ9DVP6NZSYD77)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmf2htJVDph9TXGn4Nspc5LTfLjcoWNvWCkK3TXnbnfVto/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? nebula next-id tx-sender) next-id)
      (+ next-id u1)
    )
    next-id
  )
...
(define-public (set-ipfs-root (new-ipfs-root (string-ascii 80)))
  (begin
    (asserts! (and (var-get ipfs-change-enabled) (is-eq tx-sender COMM_ADDR1)) (err err-invalid-user))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? nebula token-id sender recipient)
  )
)

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? nebula token-id)))
