...
(define-non-fungible-token squijack uint)
...
(define-data-var artist-address principal 'SP20QGYAVXEVX24710H2FKPSR4RD8F25Q12DNPX8S)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPExuon2FMEAxRDHstnP9tSVyzvbyf5VZ6jt697ZxfAvy/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? squijack next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? squijack token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? squijack token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? squijack token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? squijack id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? squijack id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? squijack id) (err ERR-NOT-FOUND)))
