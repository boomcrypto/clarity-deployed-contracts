...
(define-non-fungible-token candy-flip-360 uint)
...
(define-data-var artist-address principal 'SP3NWVVRWYTB3NV73ZCM4TG2AJ560ARJ5E5P4M575)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYkJRsL9gQb5YeXp55PQ85NNcKQgPY6buxWmLDTpkqe2u/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? candy-flip-360 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? candy-flip-360 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? candy-flip-360 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? candy-flip-360 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? candy-flip-360 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? candy-flip-360 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? candy-flip-360 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mega uint u3800)
