...
(define-non-fungible-token blonde uint)
...
;; Internal variables
(define-data-var mint-limit uint u303)
(define-data-var last-id uint u1)
(define-data-var total-price uint u250000000)
(define-data-var artist-address principal 'SP1MW49K0S1V9NPGERN2NE92W21KA9EM903CK29FW)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXwQptv26C5Pa3ar8ag784riRxcJb2hVhgERmtCiradte/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? blonde next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? blonde token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? blonde token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? blonde token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? blonde id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? blonde id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? blonde id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u633894)
