...
(define-non-fungible-token thousandtest uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTEQgHzDq4r3886X5XoFwhm21JaL8oZtYqbdudY1twwmM/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? thousandtest next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? thousandtest token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? thousandtest token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? thousandtest token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? thousandtest id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? thousandtest id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? thousandtest id) (err ERR-NOT-FOUND)))
...
(map-set mint-passes 'SP2D0PHF9ZRAARRQXNPNJH0JDBS4PDDGTD628QXZN u400)
