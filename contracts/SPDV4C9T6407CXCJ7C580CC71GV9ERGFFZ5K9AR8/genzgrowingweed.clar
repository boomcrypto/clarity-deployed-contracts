...
(define-non-fungible-token genzgrowingweed uint)
...
(define-data-var artist-address principal 'SPDV4C9T6407CXCJ7C580CC71GV9ERGFFZ5K9AR8)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmU33M3tb661K97mtSYRf8eR3zW4bnoL87dmzsgwQARZhM/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? genzgrowingweed next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? genzgrowingweed token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? genzgrowingweed token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? genzgrowingweed token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? genzgrowingweed id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? genzgrowingweed id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? genzgrowingweed id) (err ERR-NOT-FOUND)))
