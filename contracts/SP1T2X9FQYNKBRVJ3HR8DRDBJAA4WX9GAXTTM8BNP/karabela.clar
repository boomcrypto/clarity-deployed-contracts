...
(define-non-fungible-token karabela uint)
...
(define-data-var artist-address principal 'SP1T2X9FQYNKBRVJ3HR8DRDBJAA4WX9GAXTTM8BNP)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmckLmD3xhrz8okStVXx83EUdVPir8EMWuHh1JNqG3fXoW/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? karabela next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? karabela token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? karabela token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? karabela token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? karabela id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? karabela id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? karabela id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mega uint u100000)
