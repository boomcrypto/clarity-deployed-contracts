...
(define-non-fungible-token funforge-chronicles uint)
...
(define-data-var artist-address principal 'SP349S4ZER0DH7GSDC4QZ6E0R5977BC7CNC8XJXK8)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmf2PuTtYtFm2kCHN5zMp6XqFhZsfohSTpQLtZd9xpT8Xd/json/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u43)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? funforge-chronicles next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? funforge-chronicles token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? funforge-chronicles token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? funforge-chronicles token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? funforge-chronicles id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? funforge-chronicles id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? funforge-chronicles id) (err ERR-NOT-FOUND)))
