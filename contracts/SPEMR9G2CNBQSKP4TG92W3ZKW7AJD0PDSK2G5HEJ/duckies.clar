...
(define-non-fungible-token duckies uint)
...
(define-data-var artist-address principal 'SPEMR9G2CNBQSKP4TG92W3ZKW7AJD0PDSK2G5HEJ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQuBb1gnp3aryymKvzb9RwXVUdcaj1nR4kmvPoVtGn5W2/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? duckies next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? duckies token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? duckies token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? duckies token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? duckies id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? duckies id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? duckies id) (err ERR-NOT-FOUND)))
