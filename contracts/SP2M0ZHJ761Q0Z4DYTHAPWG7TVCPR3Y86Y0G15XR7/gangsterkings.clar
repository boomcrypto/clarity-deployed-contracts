...
(define-non-fungible-token gangsterkings uint)
...
;; Internal variables
(define-data-var mint-limit uint u472)
(define-data-var last-id uint u1)
(define-data-var total-price uint u5000000)
(define-data-var artist-address principal 'SP2M0ZHJ761Q0Z4DYTHAPWG7TVCPR3Y86Y0G15XR7)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTk5kfdcWiZF9SEavdi1JoCGmFhQT36kXKqfHJETMWEXa/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? gangsterkings next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gangsterkings token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gangsterkings token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gangsterkings token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gangsterkings id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gangsterkings id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gangsterkings id) (err ERR-NOT-FOUND)))
