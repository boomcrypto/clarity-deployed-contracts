...
(define-non-fungible-token koongle uint)
...
(define-data-var artist-address principal 'SP2TCWSB36ES3SHD1WX1QR436J0J9BQN9KY4YCQHE)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUjhLfZMhG4krYXPs5c6GqMD8xCJoe3X3Hb8KbirZ2ecD/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? koongle next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? koongle token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? koongle token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? koongle token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? koongle id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? koongle id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? koongle id) (err ERR-NOT-FOUND)))
