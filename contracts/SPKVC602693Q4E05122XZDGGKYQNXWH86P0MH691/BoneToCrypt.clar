...
(define-data-var base-uri (string-ascii 80) "bafybeien2yk2ufbtm5txpxgglgssbrjvg7cjzune44w6y4kuosot3qsj2q/")
(define-data-var mint-price uint u10000000)
;; Percent u100 = 1%
(define-data-var royalty-percent uint u1000)
(define-data-var mint-limit uint u100)
(define-data-var max-supply uint u2100)
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? BoneToCrypt next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? BoneToCrypt next-id recipient))
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? BoneToCrypt id))
)
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? BoneToCrypt id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? BoneToCrypt id) ERR_NO_OWNER))
