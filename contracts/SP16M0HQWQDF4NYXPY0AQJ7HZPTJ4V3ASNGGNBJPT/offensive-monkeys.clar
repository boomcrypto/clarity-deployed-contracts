...
(define-non-fungible-token offensive-monkeys uint)
...
(define-data-var artist-address principal 'SP16M0HQWQDF4NYXPY0AQJ7HZPTJ4V3ASNGGNBJPT)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeAREZrdvRqiWXZv1cwyGS97z1M7CKcGHci2NRMJTzB6r/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? offensive-monkeys next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? offensive-monkeys token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? offensive-monkeys token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? offensive-monkeys token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? offensive-monkeys id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? offensive-monkeys id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? offensive-monkeys id) (err ERR-NOT-FOUND)))
