    ...
    (define-non-fungible-token wax___ uint)
    ...
    (define-read-only (get-owner (token-id uint))
        (ok (nft-get-owner? wax___ token-id)))
    ...
    (define-public (burn (token-id uint))
       (begin 
        ...
        (nft-burn? wax___ token-id tx-sender)))
    
    (define-private (is-owner (token-id uint) (user principal))
        (is-eq user (unwrap! (nft-get-owner? wax___ token-id) false)))
    ...
    (define-private (iterate-minting (not-to-be-used bool) (next-id uint))
        ...
        (begin
            (unwrap! (nft-mint? wax___ next-id tx-sender) next-id)
            (+ next-id u1)    
        )
        next-id))
    ...
    (define-private (trnsfr (id uint) (sender principal) (recipient principal))
        (match (nft-transfer? wax___ id sender recipient)
        success
            (let
    ...
    (define-private (is-sender-owner (id uint))
        (let ((owner (unwrap! (nft-get-owner? wax___ id) false)))
    ...
    (define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
        (let ((owner (unwrap! (nft-get-owner? wax___ id) (err err-not-found)))
