    ...
    (define-non-fungible-token test-200 uint)
    ...
    (define-data-var ipfs (string-ascii 80) "ipfs://bafyreigzfaivwnysfllvputo2k4ow7tlvdf2e7nuady7rooux2zu2rto5a/metadata.json")
    ...
    (define-read-only (get-owner (token-id uint))
        (ok (nft-get-owner? test-200 token-id)))
    ...
    (define-public (burn (token-id uint))
       (begin 
        ...
        (nft-burn? test-200 token-id tx-sender)))
    
    (define-private (is-owner (token-id uint) (user principal))
        (is-eq user (unwrap! (nft-get-owner? test-200 token-id) false)))
    ...
    (define-private (iterate-minting (not-to-be-used bool) (next-id uint))
        ...
        (begin
            (unwrap! (nft-mint? test-200 next-id tx-sender) next-id)
            (+ next-id u1)    
        )
        next-id))
    ...
    (define-private (trnsfr (id uint) (sender principal) (recipient principal))
        (match (nft-transfer? test-200 id sender recipient)
        success
            (let
    ...
    (define-private (is-sender-owner (id uint))
        (let ((owner (unwrap! (nft-get-owner? test-200 id) false)))
    ...
    (define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
        (let ((owner (unwrap! (nft-get-owner? test-200 id) (err err-not-found)))
