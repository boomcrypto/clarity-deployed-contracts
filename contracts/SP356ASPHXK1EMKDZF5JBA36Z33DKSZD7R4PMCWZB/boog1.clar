    ...
    (define-non-fungible-token boog1 uint)
    ...
    (define-data-var total-price uint u200000)
    (define-data-var artiste-address principal tx-sender)
    (define-data-var nonce uint u1)
    (define-data-var ipfs (string-ascii 80) "https://radio-vault.vercel.app/api/metadata/645edc897c64e34f2ebb70be")
    ...
    (define-read-only (get-owner (token-id uint))
        (ok (nft-get-owner? boog1 token-id)))
    ...
    (define-public (burn (token-id uint))
       (begin 
        ...
        (nft-burn? boog1 token-id tx-sender)))
    
    (define-private (is-owner (token-id uint) (user principal))
        (is-eq user (unwrap! (nft-get-owner? boog1 token-id) false)))
    ...
    (define-public (transfer (id uint) (sender principal) (recipient principal))
        ...
        (asserts! (not (is-eq id u0)) (err err-not-authorized))
    ...
    (define-private (iterate-minting (not-to-be-used bool) (next-id uint))
        ...
        (begin
            (unwrap! (nft-mint? boog1 next-id tx-sender) next-id)
            (+ next-id u1)    
        )
        next-id))
    ...
    (define-private (trnsfr (id uint) (sender principal) (recipient principal))
        (match (nft-transfer? boog1 id sender recipient)
        success
            (let
    ...
    (define-private (is-sender-owner (id uint))
        (let ((owner (unwrap! (nft-get-owner? boog1 id) false)))
    ...
    (define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
        (let ((owner (unwrap! (nft-get-owner? boog1 id) (err err-not-found)))
    ...
    (unwrap! (nft-mint? boog1 u0 tx-sender) (err u200))
