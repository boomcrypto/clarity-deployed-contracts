    ...
    (define-non-fungible-token test_1234 uint)
    ...
    (define-data-var ipfs (string-ascii 80) "https://radio-vault.vercel.app/api/metadata/63e663f76ca8032f281fda3a")
    ...
    (define-read-only (get-owner (token-id uint))
        (ok (nft-get-owner? test_1234 token-id)))
    ...
    (define-public (burn (token-id uint))
       (begin 
        ...
        (nft-burn? test_1234 token-id tx-sender)))
    
    (define-private (is-owner (token-id uint) (user principal))
        (is-eq user (unwrap! (nft-get-owner? test_1234 token-id) false)))
    ...
    (define-private (iterate-minting (not-to-be-used bool) (next-id uint))
        ...
        (begin
            (unwrap! (nft-mint? test_1234 next-id tx-sender) next-id)
            (+ next-id u1)    
        )
        next-id))
    ...
    (define-private (trnsfr (id uint) (sender principal) (recipient principal))
        (match (nft-transfer? test_1234 id sender recipient)
        success
            (let
    ...
    (define-private (is-sender-owner (id uint))
        (let ((owner (unwrap! (nft-get-owner? test_1234 id) false)))
    ...
    (define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
        (let ((owner (unwrap! (nft-get-owner? test_1234 id) (err err-not-found)))
    ...
    (unwrap! (nft-mint? test_1234 u0 tx-sender) (err u200))
