    ...
    (define-non-fungible-token test200 uint)
    ...
    (define-data-var max-supply uint u12)
    ...
    (define-data-var ipfs (string-ascii 80) "ipfs://bafyreig4qtscvvynv3z4pjogksb26wdsc7vww3sj5z44acmmslorthzaau/metadata.json")
    ...
    (define-read-only (get-owner (token-id uint))
        (ok (nft-get-owner? test200 token-id)))
    ...
    (define-public (burn (token-id uint))
       (begin 
        ...
        (nft-burn? test200 token-id tx-sender)))
    
    (define-private (is-owner (token-id uint) (user principal))
        (is-eq user (unwrap! (nft-get-owner? test200 token-id) false)))
    ...
    (define-private (iterate-minting (not-to-be-used bool) (next-id uint))
        ...
        (begin
            (unwrap! (nft-mint? test200 next-id tx-sender) next-id)
            (+ next-id u1)    
        )
        next-id))
    ...
    (define-private (trnsfr (id uint) (sender principal) (recipient principal))
        (match (nft-transfer? test200 id sender recipient)
        success
            (let
    ...
    (define-private (is-sender-owner (id uint))
        (let ((owner (unwrap! (nft-get-owner? test200 id) false)))
    ...
    (define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
        (let ((owner (unwrap! (nft-get-owner? test200 id) (err err-not-found)))
