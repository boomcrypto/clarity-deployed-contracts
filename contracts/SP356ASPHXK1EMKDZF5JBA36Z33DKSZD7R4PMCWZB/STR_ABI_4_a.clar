...
(define-non-fungible-token STR_ABI_4_a uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafyreibctmifsy2v6tmi646iie24tjcp4qqmp6vacsgkwj62ur734xfhju/metadata.json")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? STR_ABI_4_a token-id)))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? STR_ABI_4_a token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? STR_ABI_4_a token-id) false)))
...
(define-private (iterate-minting (not-to-be-used bool) (next-id uint))
  (if (<= next-id (var-get max-supply))
    (begin
      (unwrap! (nft-mint? STR_ABI_4_a next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? STR_ABI_4_a id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? STR_ABI_4_a id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? STR_ABI_4_a id) (err err-not-found)))
...
(define-private (airdrop-iter (collector principal) (next-id uint)) 
  (if (<= next-id (var-get max-supply))
    (let (
      ...
      (unwrap! (nft-mint? STR_ABI_4_a next-id collector) next-id)
