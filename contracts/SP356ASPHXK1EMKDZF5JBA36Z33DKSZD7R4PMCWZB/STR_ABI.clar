...
(define-non-fungible-token STR-ABI uint)
...
(define-data-var ipfs (string-ascii 80) "ipfs://bafyreif3sdd5xgvloti3g4x6blzqxvte6n5eiwnpgiwuaxwwiazahxv2fq/metadata.json")
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? STR-ABI token-id)))
...
(define-read-only (get-token-uri (token-id uint))
  (ok (some (concat (concat (var-get ipfs) "{id}") ".json"))))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? STR-ABI token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? STR-ABI token-id) false)))
...
(define-private (iterate-minting (not-to-be-used bool) (next-id uint))
  (if (<= next-id (var-get max-supply))
    (begin
      (unwrap! (nft-mint? STR-ABI next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? STR-ABI id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? STR-ABI id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? STR-ABI id) (err err-not-found)))
...
(define-private (airdrop-iter (collector principal) (next-id uint)) 
  (if (<= next-id (var-get max-supply))
    (let (
      ...
      (unwrap! (nft-mint? STR-ABI next-id collector) next-id)
...
(define-public (airdrop (collectors (list 10 principal))) 
  (let (
    (last-nft-id (var-get nonce))
...
(unwrap! (airdrop (list 'SP3TY55JJ0Z72M6RMN1SYQC0XYM3Z1S33J6KA78GW)) (err u500))