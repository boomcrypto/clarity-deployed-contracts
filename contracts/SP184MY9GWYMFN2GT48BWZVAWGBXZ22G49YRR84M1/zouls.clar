...
(define-non-fungible-token zouls uint)
...
(define-data-var artist-address principal 'SP184MY9GWYMFN2GT48BWZVAWGBXZ22G49YRR84M1)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPhGBSnEXQFqFWs8XjUASct8dkGCqHwURt6vZGP7jq3Wj/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? zouls next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? zouls token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? zouls token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? zouls token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? zouls id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? zouls id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? zouls id) (err ERR-NOT-FOUND)))
