...
(define-constant DUMMY_BITCOIN_BLOCK_NOT_REACHED u1000000)
...
(define-data-var last-operation-done uint u0) ;; snapshotting
...
(define-private (reset-state-for-cycle (yield-cycle-id-wanted uint)) 
  (let 
    (
      (yield-data 
        (contract-call? 
      ...
      (updated-blocks-per-snapshot (unwrap! (get blocks-per-snapshot yield-data) ERR_CYCLE_DATA_NOT_SAVED))
      (updated-snapshots-per-cycle (unwrap! (get snapshots-per-cycle yield-data) ERR_CYCLE_DATA_NOT_SAVED))
    )
    (asserts! (is-eq contract-caller (var-get admin)) ERR_NOT_ADMIN)
...
(define-public (set-is-distribution-enabled)
  (begin 
    ...
    (let
      ...
      (var-set is-distribution-enabled true)
      ...
      (var-set last-operation-done u2) ;; set-can-distribute
      (ok pool-rewards)
    )
  )
)
...
(define-public (distribute-rewards (principals (list 900 principal))) 
  (begin
    (asserts! (var-get is-distribution-enabled) ERR_CANNOT_DISTRIBUTE_REWARDS)
    (if 
...
(define-read-only (get-is-yield-active) 
  (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 get-is-contract-active)
)
...
(define-read-only (yield-contract-current-cycle-stacks-block-height)
  (get current-cycle-stacks-block-height (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 cycle-data))
)

(define-read-only (yield-contract-cycle-id)
  (get cycle-id (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 cycle-data))
)

(define-read-only (yield-contract-snapshot-index)
  (get current-snapshot-index (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 cycle-data))
)

(define-read-only (yield-contract-snapshots-per-cycle)
  (get snapshots-per-cycle (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 cycle-data))
)

(define-read-only (yield-contract-blocks-per-snapshot)
  (get blocks-per-snapshot (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 cycle-data))
)
...
(define-read-only (can-call-set-is-distribution-enabled)
...
(define-read-only (can-call-set-snapshot-participants-count
    (offset-bitcoin-blocks uint)
  ) 
  (begin
    ;; True if current bitcoin-block-height > bitcoin-block-height for operation on yield + offset and
    ...
    (and 
      ...
      (>
        burn-block-height
        (+
          offset-bitcoin-blocks
          (default-to DUMMY_BITCOIN_BLOCK_NOT_REACHED
            (get-bitcoin-block-height-for-cycle-snapshot
              (var-get last-cycle-id)
...
(define-read-only (can-head-to-next-cycle-or-snapshot)
  (let 
    (
      (yield-data (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 current-overview-data))
...
(define-read-only (state-last-operation) 
  (if 
    (and 
      (is-eq (var-get last-operation-done) u2) 
      (is-some (contract-call? 'SP1HFCRKEJ8BYW4D0E3FAWHFDX8A25PPAA83HWWZ9.dual-stacking-v1 get-yield-cycle-data (+ (var-get last-cycle-id) u1)))
      (is-eq (var-get current-cycle-participants-count) (var-get rewarded-count))
    )
    (+ (var-get last-operation-done) u1) ;; can-head-to-next-cycle
...
(define-read-only (get-total-sBTC-balance)
  (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token get-balance 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-vaults-pool-active-v1-1)
)
...
(define-read-only (get-total-balance-at-stacks-block
  ...
  (ok 
    (at-block
      ...
      (get-user-total-sBTC-balance user)
    )
  )
)
