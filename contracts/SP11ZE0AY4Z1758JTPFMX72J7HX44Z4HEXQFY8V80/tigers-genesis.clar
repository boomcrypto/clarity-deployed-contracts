...
(define-non-fungible-token tigers-genesis uint)
...
(define-data-var artist-address principal 'SP11ZE0AY4Z1758JTPFMX72J7HX44Z4HEXQFY8V80)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmR56Wk9sKozmPU9nKSuLoMyqGn1uALN49Gx4kgJ9SN8Vc/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? tigers-genesis next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? tigers-genesis token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? tigers-genesis token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tigers-genesis token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? tigers-genesis id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? tigers-genesis id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? tigers-genesis id) (err ERR-NOT-FOUND)))
