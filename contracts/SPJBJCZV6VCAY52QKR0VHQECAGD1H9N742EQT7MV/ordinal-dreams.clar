...
(define-non-fungible-token ordinal-dreams uint)
...
;; Internal variables
(define-data-var mint-limit uint u158)
(define-data-var last-id uint u1)
(define-data-var total-price uint u7500000)
(define-data-var artist-address principal 'SPJBJCZV6VCAY52QKR0VHQECAGD1H9N742EQT7MV)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmabGwXyAjdKe6REHjWEy3GjggxvnSrp9ejkgumXRc51ve/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinal-dreams next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinal-dreams token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinal-dreams token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinal-dreams token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinal-dreams id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinal-dreams id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinal-dreams id) (err ERR-NOT-FOUND)))
