...
(use-trait silo-trait .test-silo-trait3-v1.silo-trait)
...
(define-public (init-withdraw (amount uint) (silo <silo-trait>))
  (let (
    ...
    (try! (contract-call? .test-state-hbtc-v1-1 check-is-silo silo-contract))

    (try! (contract-call? .test-token-hbtc-v1-1 burn-for-protocol amount contract-caller))
