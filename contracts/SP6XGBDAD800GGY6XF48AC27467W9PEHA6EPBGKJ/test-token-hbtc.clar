...
(define-fungible-token test-hBTC)
...
(define-data-var token-name (string-ascii 32) "Test Hermetica hBTC")
(define-data-var token-symbol (string-ascii 32) "test-hBTC")
...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance test-hBTC account))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? test-hBTC amount sender recipient)
      response (begin
        (print memo)
...
(define-public (set-token-uri (value (string-utf8 256)))
  (begin
    (try! (contract-call? .test-hq-vaults-v1 check-is-owner tx-sender))
...
(define-public (mint-for-protocol (amount uint) (recipient principal))
  (begin
    (try! (contract-call? .test-hq-vaults-v1 check-is-vault contract-caller))
    (ft-mint? test-hBTC amount recipient)
  )
)
...
(define-public (burn-for-protocol (amount uint) (sender principal))
  (begin
    (try! (contract-call? .test-hq-vaults-v1 check-is-vault contract-caller))
    (ft-burn? test-hBTC amount sender)
  )
)

;; Burn external
(define-public (burn (amount uint))
  (ft-burn? test-hBTC amount tx-sender)
)
