...
(define-public (log-reward (reward-usdh uint))
  (let (
    ...
    (total-usdh-supply-staked (unwrap-panic (contract-call? .test-usdh-token-v1 get-balance .test-staking-reserve-v1)))
  )
    ...
    (if (> reward-usdh u0)
      (begin
        ...
        (try! (contract-call? .test-usdh-token-v1 mint-for-protocol reward-usdh .test-staking-reserve-v1))
      )
      true
    )
