...
(define-public (transfer (asset <ft>) (amount uint) (recipient principal))
  (let (
    ...
    (try! (contract-call? .test-state-hbtc2-v1 check-is-trading-asset (contract-of asset)))
    (asserts! (>= balance amount) ERR_INSUFFICIENT_BALANCE)
    (print { action: "transfer", user: contract-caller, contract: .test-reserve-hbtc2-v1, data: { asset: asset, amount: amount, recipient: recipient, balance: balance }})
