...
(define-public (transfer (asset <ft>) (amount uint) (recipient principal))
  (let (
    ...
    (try! (contract-call? .test-hq-vaults-v1-1 check-is-protocol recipient))
    (try! (contract-call? .test-state-hbtc-v1-1 check-is-transfer-enabled))
    (try! (contract-call? .test-state-hbtc-v1-1 check-is-trading-asset (contract-of asset)))
    (asserts! (>= balance amount) ERR_INSUFFICIENT_BALANCE)
    (print { action: "transfer", user: contract-caller, contract: .test-reserve-hbtc-v1-1, data: { asset: asset, amount: amount, recipient: recipient, balance: balance }})
