...
(use-trait silo-trait .test-silo-trait-v1.silo-trait)
...
(define-public (deposit (amount uint) (affiliate (optional (buff 64))))
  (let (
    (price (contract-call? .test-state-hbtc-v1 get-token-price))
    ...
    (try! (contract-call? .test-state-hbtc-v1 check-is-deposit-enabled))
    (asserts! (<= (+ vault-balance amount) (contract-call? .test-state-hbtc-v1 get-deposit-cap)) ERR_DEPOSIT_CAP_EXCEEDED)

    (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer amount contract-caller .test-reserve-hbtc-v1 none))
...
(define-public (init-withdraw (amount uint) (silo <silo-trait>))
  (let (
    ...
    (exit-fee (contract-call? .test-state-hbtc-v1 get-custom-exit-fee contract-caller))
    ...
    (asserts! (> amount u0) ERR_INVALID_AMOUNT)
    (try! (contract-call? .test-blacklist-vaults-v1 check-is-not-soft-blacklist contract-caller))
    (try! (contract-call? .test-state-hbtc-v1 check-is-withdraw-enabled))
    (try! (contract-call? .test-state-hbtc-v1 check-is-silo silo-contract))
    ...
    (ok (try! (contract-call? .test-silo-hbtc-v1 create-claim amount-after-fee fee-amount contract-caller)))
  )
)