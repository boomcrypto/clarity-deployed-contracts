...
(define-constant zest-interface .test-zest-interface-hbtc-v1-1)
...
(define-public (open-position-zest
  ...
  (begin
    ...
    (if (> sbtc-supply-amount u0)
      (try! (contract-call? .test-zest-interface-hbtc-v1-1 zest-supply
        borrow-helper-trait
        lp-sbtc-trait
        pool-reserve-data
        sbtc-token-trait
    ...
    (try! (contract-call? .test-zest-interface-hbtc-v1-1 zest-borrow
      borrow-helper-trait
      pool-reserve-data
      oracle-trait
    ...
    (try! (contract-call? .test-hermetica-interface-hbtc-v1-2 hermetica-stake
      usdh-borrow-amount
      staking-trait))
    ...
    (print {
      ...
      user: tx-sender,
      data: { sbtc-supplied: sbtc-supply-amount, usdh-borrowed-and-staked: usdh-borrow-amount, interest-rate-mode: interest-rate-mode }
    })
    (ok true)
  )
)
...
(define-public (close-position-zest
  ...
  (begin
    ...
    (try! (contract-call? .test-hermetica-interface-hbtc-v1-2 hermetica-unstake-and-withdraw
    ...
    (try! (contract-call? .test-zest-interface-hbtc-v1-1 zest-repay
      borrow-helper-trait
      borrowed-asset-trait
      usdh-repay-amount
      zest-interface)) ;; Payer is the zest-interface itself

    (print {
      action: "close-position-zest",
      user: tx-sender,
      data: { susdh-unstaked: susdh-unstake-amount, usdh-repaid: usdh-repay-amount }
    })
    (ok true)
  )
)