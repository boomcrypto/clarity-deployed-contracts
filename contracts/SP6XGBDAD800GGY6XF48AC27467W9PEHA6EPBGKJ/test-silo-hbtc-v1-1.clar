...
(impl-trait .test-silo-trait3-v1.silo-trait)
...
(define-public (create-claim (amount uint) (fee uint) (recipient principal))
  (let (
    ...
    (claim-ts (+ (get-current-ts) (contract-call? .test-state-hbtc-v1-1 get-custom-cooldown recipient)))
  )
...
(define-public (fund-claim (claim-id uint))
  (let (
    ...
    (try! (contract-call? .test-reserve-hbtc-v1-1 transfer sbtc-token total-amount this-contract))
