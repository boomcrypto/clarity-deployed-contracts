...
(define-data-var token-name (string-ascii 32) "Hermetica Staked USDh")
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (if (var-get only-protocol) 
      (asserts! (or (contract-call? .test-hq-v1 get-contract-active sender) (contract-call? .test-hq-v1 get-contract-active recipient)) ERR_ONLY_PROTOCOL)
      true
    )

    (if (var-get blacklist-enabled)
      (try! (contract-call? .test-blacklist-susdh-v1 check-is-not-full-blacklist-two sender recipient))
      true
    )
...
(define-public (start-migration (manager <token-migration-trait>))
	(begin
		(try! (is-not-migrated))
		(try! (contract-call? .test-hq-v1 check-is-owner contract-caller))
