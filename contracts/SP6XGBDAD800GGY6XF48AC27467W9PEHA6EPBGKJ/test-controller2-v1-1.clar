...
(define-constant min-update-window (* u1 u1))
...
(define-data-var update-window uint (* u1 u1))
...
(define-public (log-reward (reward-usdh uint))
  (let (
    (total-usdh-supply (unwrap-panic (contract-call? .test-usdh-token-v1 get-total-supply)))
    (total-usdh-supply-staked (unwrap-panic (contract-call? .test-usdh-token-v1 get-balance .test-staking-reserve2-v1)))
  )
    ...
    (if (> reward-usdh u0)
      (begin
        ...
        (try! (contract-call? .test-usdh-token-v1 mint-for-protocol reward-usdh .test-staking-reserve2-v1))
      )
      true
    )
