...
(impl-trait .test-minting-auto-trait3-v1.minting-auto-trait)
...
(define-constant min-mint-limit-reset-window u10)
...
(define-public (mint
  ...
  (let (
    ...
    (state (try! (contract-call? .test-minting-auto-state-v1 get-state contract-caller minting-asset-contract )))
    ...
    (asserts! (>= amount-usdh-requested (contract-call? .test-minting-state-v1 get-min-amount-usdh-requested)) ERR_BELOW_MIN)
    ...
    (try! (contract-call? .test-usdh-token-v1 mint-for-protocol amount-usdh-requested contract-caller))
...
(define-public (redeem
  ...
  (let (
    ...
    (state (try! (contract-call? .test-minting-auto-state-v1 get-state contract-caller redeeming-asset-contract )))
    (token-base (get token-base state))
    (conf-tolerance-bps (get conf-tolerance-bps state))
    (price-slippage-bps (get price-slippage-bps state))
    (block-timestamp (unwrap-panic (get-stacks-block-info? time (- stacks-block-height (get block-delay state)))))
    ...
    (try! (contract-call? .test-usdh-token-v1 burn-for-protocol amount-usdh-requested contract-caller))
    (try! (contract-call? .test-redeeming-reserve-v1-2 transfer (- amount-asset-required fee-amount) contract-caller redeeming-asset memo))

    (if (> fee-amount u0)
      (try! (contract-call? .test-redeeming-reserve-v1-2 transfer fee-amount (get fee-address state) redeeming-asset none))
      true
    )
