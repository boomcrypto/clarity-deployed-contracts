...
(define-public (withdraw (claim-id uint))
  (let (
    ...
    (if (> fee u0)
      (try! (as-contract (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer fee this-contract fee-address none)))
      true
    )
    (print { action: "withdraw", user: contract-caller, data: { claim-id: claim-id, amount: amount, fee: fee, recipient: recipient, fee-address: fee-address } })
...
(define-public (create-claim (amount uint) (fee uint) (recipient principal))
  (let (
    ...
    (claim-ts (+ (get-current-ts) (contract-call? .test-state-hbtc-v1 get-custom-cooldown recipient)))
  )
...
(define-public (fund-claim (claim-id uint))
  (let (
    ...
    (try! (contract-call? .test-reserve-hbtc-v1 transfer 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token total-amount this-contract))
    (print { action: "fund-claim", user: contract-caller, is-manager: is-manager, data: { claim: (try! (get-claim claim-id)) } })
    (ok (map-set claims { claim-id: claim-id } (merge claim { funded: true })))
  )
)