...
(define-constant rf .test-reserve-fund-hbtc-v1-1)
...
(define-public (log-reward (reward uint) (is-positive bool))
  (let (
    (token-price (contract-call? .test-state-hbtc-v1-1 get-token-price))
    (vault-balance (/ (* token-price (unwrap-panic (contract-call? .test-token-hbtc-v1-1 get-total-supply))) hbtc-base))
    (fees (contract-call? .test-state-hbtc-v1-1 get-fees))
    ...
    (try! (contract-call? .test-hq-vaults-v1-1 check-is-protocol-enabled))
    (try! (contract-call? .test-hq-vaults-v1-1 check-is-rewarder contract-caller))
    (try! (contract-call? .test-state-hbtc-v1-1 check-is-update-window-open))
    (try! (contract-call? .test-state-hbtc-v1-1 check-max-reward reward vault-balance))
    ...
    (ok (try! (contract-call? .test-state-hbtc-v1-1 update-last-log-ts)))
  )
)
...
(define-public (transfer-pending)
  (let (
    ...
    (if (> fees u0) (try! (contract-call? .test-reserve-hbtc-v1-1 transfer sbtc-token fees fee-collector)) true)
    ;; Process accumulated rf transfer
    (if (> pending-rf-balance u0) (try! (contract-call? .test-reserve-hbtc-v1-1 transfer sbtc-token pending-rf-balance rf)) true)
...
(define-private (handle-profit 
  ...
  (let (
    (hbtc-supply (unwrap-panic (contract-call? .test-token-hbtc-v1-1 get-total-supply)))
    (reward-after-fees (- reward (+ perf-fee mgmt-fee)))
    (rf-change (/ (* reward-after-fees (contract-call? .test-state-hbtc-v1-1 get-reserve-rate)) bps-base))
    ...
    (print {
      ...
      data: { reward: { gross: reward, token: reward-token, is-positive: true }, fees: { perf: perf-fee, mgmt: mgmt-fee }, rf-balance: { old: rf-balance, new: (+ rf-balance rf-change)  }, token-price: { old: token-price, new: new-token-price }, vault-balance: vault-balance, return-percent-of-bps: (/ (* reward-token bps-base pct-base) vault-balance) }
    })
    
    (var-set pending-fees (+ (var-get pending-fees) perf-fee mgmt-fee))
    (var-set pending-rf (+ pending-rf-balance rf-change))
    (ok (try! (contract-call? .test-state-hbtc-v1-1 set-token-price new-token-price)))
  )
)
...
(define-private (handle-loss-covered 
  ...
  (let (
    ...
    (print {
      ...
      case: "loss-covered",
      user: contract-caller,
      data: { reward: { gross: reward, token: u0, is-positive: is-positive }, fees: { perf: u0, mgmt: mgmt-fee }, rf-balance: { old: rf-balance, new: (- rf-balance rf-req) }, token-price: { old: token-price, new: token-price }, vault-balance: vault-balance, return-percent-of-bps: u0 }
    })
    (var-set pending-fees (+ (var-get pending-fees) mgmt-fee))
    (if (> transfer-amount u0)
      (begin 
        (var-set pending-rf u0)
        (try! (contract-call? .test-reserve-fund-hbtc-v1-1 transfer sbtc-token transfer-amount reserve none))) 
...
(define-private (handle-loss-exceeds 
  ...
  (let (
    ...
    (print {
      ...
      case: "loss-exceeds",
      user: contract-caller,
      data: { reward: { gross: reward, token: loss, is-positive: is-positive }, fees: { perf: u0, mgmt: mgmt-fee }, rf-balance: { old: rf-balance, new: u0 }, token-price: { old: token-price, new: new-token-price }, vault-balance: vault-balance, return-percent-of-bps: (/ (* loss bps-base pct-base) vault-balance) }
    })
    (var-set pending-fees (+ (var-get pending-fees) mgmt-fee))
    (var-set pending-rf u0)
    (if (> rf-balance pending-rf-balance)
      (try! (contract-call? .test-reserve-fund-hbtc-v1-1 transfer sbtc-token (- rf-balance pending-rf-balance) reserve none))
      true
    )
    (ok (try! (contract-call? .test-state-hbtc-v1-1 set-token-price new-token-price)))
  )
