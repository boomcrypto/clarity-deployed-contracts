...
(impl-trait 'SP2PMZQNSRY91ZEEFHQPM0KMFE8KKTRYCM6F3870E.trait-flash-loan-v1.flash-loan)
...
(define-public (on-granite-flash-loan (amount uint) (fee uint) (data (optional (buff 10240))))
  (let (
    ...
    (try! (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer amount caller SELF none))
    (try! (liqudate-and-swap-inner (get pyth-price-feed-data cdata) (get batch cdata) dex))
    (try! (as-contract (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer (+ amount fee) SELF caller none)))
    SUCCESS
  )
)
