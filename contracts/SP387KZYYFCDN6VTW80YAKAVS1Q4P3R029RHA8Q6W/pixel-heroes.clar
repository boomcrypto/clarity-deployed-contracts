...
(define-non-fungible-token pixel-heroes uint)
...
(define-data-var artist-address principal 'SP387KZYYFCDN6VTW80YAKAVS1Q4P3R029RHA8Q6W)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYLp91gw12UR5C8zqWLNhKMRphFUtEqipHvzoArLaCPxq/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? pixel-heroes next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? pixel-heroes token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? pixel-heroes token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? pixel-heroes token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pixel-heroes id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pixel-heroes id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pixel-heroes id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u37657)
