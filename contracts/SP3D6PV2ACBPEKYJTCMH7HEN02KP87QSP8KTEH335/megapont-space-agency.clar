...
(define-constant ERR-NO-OWNER (err u411))
...
(define-public (airdrop (old-msa-id uint))
    (let
        (
            (new-owner (unwrap! (unwrap! (contract-call? .msa-nft get-owner old-msa-id) ERR-NO-OWNER) ERR-NO-OWNER))
            (next-id (+ u1 (var-get last-id)))
        )
        (begin
            (asserts! (called-from-mint) ERR-NOT-AUTHORIZED)
            (asserts! (is-eq (default-to false (map-get? airdrop-claimed old-msa-id)) false) ERR-AIRDROP-CLAIMED)
            (match (nft-mint? Megapont-Space-Agency next-id new-owner)
                success
                    (begin
                        (map-set token-count new-owner (+ (get-balance new-owner) u1))
                        (map-set airdrop-claimed old-msa-id true)
