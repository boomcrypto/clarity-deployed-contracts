...
(define-non-fungible-token ghostack uint)
...
(define-data-var artist-address principal 'SP2WZFQD8BKMSKVYVTHQ1JCXHFZVHZV8ZEESFAM0N)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNybbBLgvoUDQXyXg2SH9WMhnTb2sDSRQKvRntZTcFMes/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ghostack next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ghostack token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ghostack token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ghostack token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ghostack id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ghostack id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ghostack id) (err ERR-NOT-FOUND)))
