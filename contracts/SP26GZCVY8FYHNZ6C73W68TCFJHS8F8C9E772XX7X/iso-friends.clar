...
(define-non-fungible-token iso-friends uint)
...
;; Internal variables
(define-data-var mint-limit uint u170)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNYnVWL9m7kpiu8n8GHKZGmXfcWRzqQ6vt8n8mQApoHyq/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? iso-friends next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? iso-friends token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? iso-friends token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? iso-friends token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? iso-friends id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? iso-friends id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? iso-friends id) (err ERR-NOT-FOUND)))
