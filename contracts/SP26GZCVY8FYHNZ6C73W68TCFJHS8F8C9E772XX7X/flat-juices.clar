...
(define-non-fungible-token flat-juices uint)
...
(define-data-var total-price uint u110000000)
(define-data-var artist-address principal 'SP26GZCVY8FYHNZ6C73W68TCFJHS8F8C9E772XX7X)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPPPHC5PRY6g1Ujkn4BJ7Hs1AfK11a6LqVhLCN4vwpYFP/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? flat-juices next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? flat-juices token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? flat-juices token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? flat-juices token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? flat-juices id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? flat-juices id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? flat-juices id) (err ERR-NOT-FOUND)))
