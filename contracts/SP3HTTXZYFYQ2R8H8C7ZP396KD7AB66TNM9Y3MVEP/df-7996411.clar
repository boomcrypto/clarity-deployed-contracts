;;
;; cvx (c) TRUBIT.TECH 2022
;;

(define-non-fungible-token df uint)

(define-constant CONTRACT-CREATOR tx-sender)

(define-constant ERR-NOT-AUTHORIZED (err u0))

(define-data-var token-uri (string-ascii 256) "https://sspblsjcforgiuatmnbp.nhost.run/v1/functions/rewards/stacks/metadata?id=7996411")
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? df token-id sender recipient)
  )
)

(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? df token-id))
)
...
(define-public (mint (token-id uint) (recipient principal))
  (begin
    ...
    (try! (nft-mint? df token-id recipient))
