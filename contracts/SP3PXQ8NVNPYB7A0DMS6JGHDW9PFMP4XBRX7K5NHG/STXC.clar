...
(define-non-fungible-token STXC uint)
...
(define-data-var base-uri (string-ascii 80) "bafybeiex6ptpn7ha5gu4qjud523eyb7uehbaxivc2ddax564c3iiy5izqq/")
(define-data-var mint-price uint u50000000)
;; Percent u100 = 1%
(define-data-var royalty-percent uint u1000)
(define-data-var mint-limit uint u10)
(define-data-var max-supply uint u34)
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? STXC next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? STXC next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? STXC id sender recipient)
        success
          (let
            (
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? STXC id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? STXC id) ERR_NO_OWNER))
