...
(define-non-fungible-token sdsd uint)
...
(define-data-var total-price uint u400)
(define-data-var artist-address principal 'SP2Q6B7ASMQ30KEA1JJ9F4RW5S0BGEGBX20TMWP4J)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbQtEshmh4ePt2ESciu3rAvn65zTzpoW2bBqFNLpVuibb/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? sdsd next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? sdsd token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? sdsd token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sdsd token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sdsd id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sdsd id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sdsd id) (err ERR-NOT-FOUND)))
