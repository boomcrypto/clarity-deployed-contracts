...
(define-public (claim-rewards (on-behalf-of (optional principal)))
  (let (
    ...
    (if (> reward-amount u0) 
      (as-contract (try! (contract-call? 'SP2R995XG7W1KHYSTGC6Q8AT0C79379W7C6133AQZ.state-v1 transfer reward-amount (as-contract contract-caller) user none)))
      true
    )
...
(define-public (transfer-remaining-lp-tokens (recipient principal))
  (let ((balance (unwrap! (contract-call? 'SP2R995XG7W1KHYSTGC6Q8AT0C79379W7C6133AQZ.state-v1 get-balance (as-contract contract-caller)) ERR-FAILED-TO-GET-LP-BALANCE)))
    (try! (ensure-snapshot-uploader))
    (try! (ensure-epoch-closed))
    (asserts! (is-eq (var-get unclaimed-user-reward-count) u0) ERR-REWARDS-NOT-CLAIMED)
    (asserts! (> balance u0) ERR-ZERO-REWARDS)
    (as-contract (try! (contract-call? 'SP2R995XG7W1KHYSTGC6Q8AT0C79379W7C6133AQZ.state-v1 transfer balance (as-contract contract-caller) recipient none)))
    (print {
