...
(define-non-fungible-token prod-by-mattrick uint)
...
;; Internal variables
(define-data-var mint-limit uint u808)
(define-data-var last-id uint u1)
(define-data-var total-price uint u40000000)
(define-data-var artist-address principal 'SP3Y0CDV54SRTHWP610E4YNHA3YZ7BY1F9VDAYXVF)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQfp6D8xgVLd97jcZGNqFHZSbaTbs1sdHgcitwS7sGkZG/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? prod-by-mattrick next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? prod-by-mattrick token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? prod-by-mattrick token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? prod-by-mattrick token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? prod-by-mattrick id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? prod-by-mattrick id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? prod-by-mattrick id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u52134)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u14000000)
