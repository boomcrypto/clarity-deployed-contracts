...
(define-non-fungible-token Zero_Authority uint)
...
(define-constant IPFS-ROOT "ipfs://ipfs/bafybeib475ul4e55uiindmmdw4jvh6gpudmgkrmfgo6nnrpujeuq4gbade/{id}.json")
...
(define-public (mint)
    (let ((token-id (+ u1 (var-get last-token-id))))
        ...
        (nft-mint? Zero_Authority token-id tx-sender)
    )
)
...
(define-public (burn (id uint))
    (let ((owner (unwrap! (nft-get-owner? Zero_Authority id) (err ERROR-UNAUTHORIZED))))
        (asserts! (is-eq owner tx-sender) (err ERROR-UNAUTHORIZED))
        (nft-burn? Zero_Authority id owner)
    )
)
...
(define-read-only (get-owner (id uint))
    (ok (nft-get-owner? Zero_Authority id))
)
