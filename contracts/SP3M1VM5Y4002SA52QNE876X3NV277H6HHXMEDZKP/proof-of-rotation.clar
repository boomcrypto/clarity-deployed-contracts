...
(define-non-fungible-token proof-of-rotation uint)
...
(define-data-var artist-address principal 'SP3M1VM5Y4002SA52QNE876X3NV277H6HHXMEDZKP)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbvFL23b8ZJtMJtTkWA3Yi36U3yQQqYhqrmbmdUtcszh1/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? proof-of-rotation next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? proof-of-rotation token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? proof-of-rotation token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? proof-of-rotation token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? proof-of-rotation id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? proof-of-rotation id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? proof-of-rotation id) (err ERR-NOT-FOUND)))
