...
(define-public (bitcoin-bears-claim-one-with-referral (ref-code (string-ascii 30)))
  (let (
    (some-ref-addr (unwrap! (contract-call? .spaghettipunk-referral-codes get-referral-address ref-code) (err ERR_NOT_VALID_REFERRAL)))
    (total-reward (contract-call? .spaghettipunk-referrals get-total-reward 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.bitcoin-bears))
    (mints u1)
    (reward (* mints total-reward))
  )
    (asserts! (is-eq (validate-referral ref-code) (ok true)) (err ERR_NOT_VALID_REFERRAL))
    (try! (contract-call? 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.bitcoin-bears claim))
    (try! (contract-call? .spaghettipunk-referrals handout-referral-reward ref-code 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.bitcoin-bears mints none))
