...
(define-data-var migration-contract principal .staking-migration)
(define-data-var bonuses (list 100 uint) (list ))
...
(define-read-only (bonus-check-all (addresses (list 100 principal)) (id uint))
  (let (
...
(define-read-only (check-collect-bonus (staker principal))
    (let (
        (all-bonus (var-get bonuses))
        (bonus-indexer (if (> (len all-bonus) u0) (- (len all-bonus) u1) u0))
        (bonus-addresses (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.principal-lists lookup staker bonus-indexer))
        
        ) 
        (if (> (len bonus-addresses) u0)
          (fold + (map check-collect-bonus-one all-bonus bonus-addresses ) u0)
          u0
        )        
    )
)
...
(define-read-only (check-collect-bonus-one (bonus-id uint) (staker principal))
    (let (
...
(define-public (stake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (begin 
        ...
        (begin
          (map-set staker-info { staker: tx-sender} (merge (get-staker-info tx-sender) {stake-time: block, points-balance: (+ (get points-balance (get-staker-info tx-sender)) bonus-points)}))
...
(define-public (admin-unstake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (begin 
        (if (is-eq custodial true) (try! (as-contract (contract-call? collection transfer item (as-contract tx-sender) owner ))) true)
        (begin
          (map-set staker-info { staker: owner} (merge (get-staker-info owner) {stake-time: block, points-balance: (+ (get points-balance (get-staker-info owner)) bonus-points)}))
...
(define-public (bonus-add (addresses (list 10 principal)) ( blocks-x-token-bonus uint) (start-block uint) (bonus-period uint))
  (let (
    ...
    (var-set bonuses (unwrap-panic (as-max-len? (append (var-get bonuses) bonus-id) u100)))
...
(print (bonus-add (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-bitcoin-bulls 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-bitcoin-bears 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.bitcoin-whales) u288 block-height u4224))
