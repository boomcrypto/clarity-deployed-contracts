...
(define-map staker-info { staker: principal } { collections: (list 100 principal), stake-time: uint, lifetime-points: uint, points-balance: uint })
...
(define-data-var bonus { blocks-per-token: uint, bonus-check: (list 10 principal),  bonus-start-block: uint, bonus-end-block: uint } { blocks-per-token: u0, bonus-check: (list ),  bonus-start-block: u0, bonus-end-block: u0 })
...
(define-read-only (check-staker (staker principal))
    (default-to 
      {collections: (list ), stake-time: block-height, lifetime-points: u0,  points-balance: u0} 
...
(define-read-only (bonus-check-all (addresses (list 100 principal)))
  (let (
    (the-bonus (get-bonus))
...
(define-read-only (check-collect (staker principal))
  (let (
      ...
      (bonus-balance (get points-balance info))
      (staker-indexer (if (> (len staker-collections) u0) (- (len staker-collections) u1) u0))
      (staker-addresses (if (> (len staker-collections) u0) (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.principal-lists lookup staker staker-indexer) (list )))
      (to-collect (if (> (len staker-collections) u0) (fold + (map check-collect-one staker-collections staker-addresses ) u0) u0))
      (bonus-points (check-collect-bonus staker))
    )
    (+ (+ to-collect bonus-points) bonus-balance)
  )
)

(define-read-only (check-collect-one (collection principal) (staker principal))
    (let (
...
(define-read-only (check-collect-bonus (staker principal))
    (let (
        (the-bonus (get-bonus))
        (bonus-checks (get bonus-check the-bonus))
        ...
        (check (if (> (len addresses) u0) (bonus-check-all addresses) false))
        (info (check-staker staker))
        (prev-time (get stake-time info))
        (active-bonus (and (> block-height block-start) (>  block-height prev-time)))
        ) 
...
(define-public (stake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    (let (
        ...
        (bonus-points (check-collect-bonus tx-sender))
    ...
    (begin 
        (if (and (> fees u0) (not (is-eq contract-caller (var-get migration-contract))))
          (begin
        ...
        (begin
          (map-set staker-info { staker: tx-sender} (merge (check-staker tx-sender) {stake-time: block, points-balance: (+ (get points-balance (check-staker tx-sender)) bonus-points)}))
...
(define-public (unstake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    (let (
        ...
        (bonus-points (check-collect-bonus owner))
...
(define-public (collect (fungible <token-trait>))
  (let (
    ...
    (begin
      ...
      (map-set staker-info { staker: tx-sender} (merge (check-staker tx-sender) {stake-time: block, lifetime-points: (+ (get lifetime-points info) to-collect) , points-balance: u0}))
      (map collect-one staker-collection)
    )
    (print {
      action: "collect",
      staker: (check-staker tx-sender)
...
(define-public (admin-unstake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (begin 
        (if (is-eq custodial true) (try! (as-contract (contract-call? collection transfer item (as-contract tx-sender) owner ))) true)
        (begin
          (map-set staker-info { staker: owner} (merge (check-staker owner) {stake-time: block, points-balance: (+ (get points-balance (check-staker owner)) bonus-points)}))
...
(define-public (bonus-change (addresses (list 10 principal)) ( blocks-x-token-bonus uint) (start-block uint) (bonus-period uint))
  (begin
    (asserts! (is-eq tx-sender (var-get admin)) (err ERR-NOT-AUTHORIZED))
    (asserts! (>= blocks-x-token-bonus u288) (err ERR-EMISSION-TOO-HIGH))
    (var-set bonus {blocks-per-token: blocks-x-token-bonus, bonus-check: addresses,  bonus-start-block: start-block, bonus-end-block: (+ start-block bonus-period)})
    (ok (print {bonus: (get-bonus)}))
  )
)
...
;;migration functions
(define-public (migrate-lifetime (staker principal) (lifetime uint))
(let (
  (info (check-staker staker))
)
  (asserts! (is-eq contract-caller (var-get migration-contract)) (err ERR-MIGRATION-NOT-AUTHORIZED))
  (map-set staker-info {staker: staker} (merge info { lifetime-points: (+ (get lifetime-points info) lifetime)}))
  (ok (print {
        action: "migrate-lifetime",
        staker: (check-staker staker)}))
))
...
(print (bonus-change (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-bitcoin-bulls 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-bitcoin-bears 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.bitcoin-whales) u288 block-height u3925))