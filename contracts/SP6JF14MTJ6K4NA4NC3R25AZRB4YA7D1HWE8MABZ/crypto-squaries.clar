...
(define-non-fungible-token crypto-squaries uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPjKwkwDGA1fftHdNFFuxoHTZ4Bk9iodUz6iCnj3LJPyc/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? crypto-squaries next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? crypto-squaries token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? crypto-squaries token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? crypto-squaries token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? crypto-squaries id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? crypto-squaries id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? crypto-squaries id) (err ERR-NOT-FOUND)))
