...
(define-non-fungible-token moonclaws uint)
...
(define-data-var artist-address principal 'SP30GQV146XYB9Z3EXJ8FWGNVGX36QW48JPWV03N3)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmP84xJNrmADxfvFsjz3KukAfkEGWRCGFdWRCUdqXBaDba/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? moonclaws next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? moonclaws token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? moonclaws token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? moonclaws token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? moonclaws id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? moonclaws id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? moonclaws id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u34677)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u22900000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u23182000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u273000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u4079000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u59800000000)
