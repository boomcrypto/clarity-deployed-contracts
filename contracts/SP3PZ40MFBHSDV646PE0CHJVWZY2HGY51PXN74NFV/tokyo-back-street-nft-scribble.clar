;; tokyo-back-street-nft-scribble
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token tokyo-back-street-nft-scribble uint)
...
(define-data-var total-price uint u50000000000)
(define-data-var artist-address principal 'SP3PZ40MFBHSDV646PE0CHJVWZY2HGY51PXN74NFV)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeNruxtQYHBVhyEm1ZNrwbCyGmtiQDY2SqPyQgMqXqERx/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? tokyo-back-street-nft-scribble next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? tokyo-back-street-nft-scribble token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? tokyo-back-street-nft-scribble token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tokyo-back-street-nft-scribble token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? tokyo-back-street-nft-scribble id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? tokyo-back-street-nft-scribble id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? tokyo-back-street-nft-scribble id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u111787244)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u26400000000)
