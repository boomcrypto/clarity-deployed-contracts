...
(define-public (execute-action 
  ...
  (let (
    ...
    ;; Get x-amount to withdraw from Stableswap pool
    (x-amount (default-to u0 (element-at? unwrapped-uint-list u0)))

    ;; Get y-amount to withdraw from Stableswap pool
    (y-amount (default-to u0 (element-at? unwrapped-uint-list u1)))
    ...
    ;; Withdraw imbalanced liquidity from Stableswap pool
    (withdraw-liquidity-result (try! (stableswap-withdraw-liquidity x-amount y-amount amount owner-address fee-recipient unwrapped-token-list transfer-to-owner)))
    (x-amount-withdrawn (get x-amount-withdrawn withdraw-liquidity-result))
    (y-amount-withdrawn (get y-amount-withdrawn withdraw-liquidity-result))
    (dlp (get dlp withdraw-liquidity-result))

    ;; Get keeper fee and calculate updated dlp amount
    (keeper-fee-amount (unwrap! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.keeper-4-helper-v-1-1 get-keeper-fee-amount dlp) ERR_INVALID_HELPER_DATA))
    (dlp-after-keeper-fee (+ dlp keeper-fee-amount))
    ...
    (begin
      ;; Assert x-amount-withdrawn is greater than or equal to x-amount
      (asserts! (>= x-amount-withdrawn x-amount) ERR_MINIMUM_RECEIVED)

      ;; Assert y-amount-withdrawn is greater than or equal to y-amount
      (asserts! (>= y-amount-withdrawn y-amount) ERR_MINIMUM_RECEIVED)
      
      ;; Assert dlp-after-keeper-fee is less than or equal to amount
      (asserts! (<= dlp-after-keeper-fee amount) ERR_MINIMUM_RECEIVED)
      ...
      (print {
        ...
        data: {
          ...
          withdraw-liquidity-result: withdraw-liquidity-result
        }
      })
...
;; Withdraw imbalanced liquidity from STX-stSTX Stableswap pool
(define-private (stableswap-withdraw-liquidity
  ...
  (let (
    ...
    ;; Perform withdraw-imbalanced-liquidity
    (withdraw-liquidity-result (try! (contract-call? 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.stableswap-core-v-1-4 withdraw-imbalanced-liquidity 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.stableswap-pool-stx-ststx-v-1-4 token-trait-a token-trait-b x-amount y-amount max-dlp)))
    (x-amount-withdrawn (get x-amount withdraw-liquidity-result))
    (y-amount-withdrawn (get y-amount withdraw-liquidity-result))
    (dlp (get dlp withdraw-liquidity-result))
  )
    ...
    (ok {
      x-amount-withdrawn: x-amount-withdrawn,
      y-amount-withdrawn: y-amount-withdrawn,
      dlp: dlp
    })
  )
)
