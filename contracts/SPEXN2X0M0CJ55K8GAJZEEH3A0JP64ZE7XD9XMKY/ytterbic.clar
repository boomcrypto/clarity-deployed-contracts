...
;; Midpoint constant
(define-constant POOL_CONTRACT 'SM1793C4R5PZ4NS4VQ4WMP7SKKYVH8JZEWSZ9HCCR.stableswap-pool-stx-ststx-v-1-4)
(define-constant MIDPOINT_CONTRACT 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.data-core-v2)

;; Public function to check if the pool's midpoint values can be updated
(define-public (get-can-update-midpoint)
  (let (
    (pool-data (unwrap! (contract-call? POOL_CONTRACT get-pool) ERR_NO_POOL_DATA))
    (primary-numerator (get midpoint-primary-numerator pool-data))
    (updated-numerator (try! (contract-call? MIDPOINT_CONTRACT get-stx-per-ststx 'SP4SZE494VC2YC5JYG7AYFQ44F5Q4PYV7DVMDPBG.reserve-v1)))
  )
...
(define-public (update-midpoint)
  (let (
    ...
    (begin
      ...
      ;;                   POOL_CONTRACT updated-numerator primary-denominator
