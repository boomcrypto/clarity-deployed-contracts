...
(define-non-fungible-token stonex uint)
...
(define-data-var artist-address principal 'SP3GP4QNB7KFPWER7FKVB259734CN106Q248W093X)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qme8LxwjjKVwx7Z7CYSTD2xVHs6B7a7EHHJkVRjY98HsGP/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stonex next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stonex token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stonex token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stonex token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stonex id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stonex id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stonex id) (err ERR-NOT-FOUND)))
