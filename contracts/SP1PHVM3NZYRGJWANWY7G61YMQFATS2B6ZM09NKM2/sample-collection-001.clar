...
(define-non-fungible-token sample-collection-001 uint)
...
(define-data-var admin (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF 'SP35K3WCA9GCJV2XC7X021MR2D9D2PKF855CVCKB0 ))
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? sample-collection-001 item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? sample-collection-001 item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? sample-collection-001 item-id)))
      (begin
        (try! (nft-burn? sample-collection-001 item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sample-collection-001 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sample-collection-001 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sample-collection-001 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sample-collection-001 id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP35K3WCA9GCJV2XC7X021MR2D9D2PKF855CVCKB0 u500 "Sample Collection" u500000000 u64242 u64386 "ipfs://SAMPLE_HASH/sample-slug/1.json"))
  (ok true)
)
