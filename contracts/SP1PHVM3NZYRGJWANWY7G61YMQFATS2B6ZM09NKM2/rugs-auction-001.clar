...
(define-non-fungible-token rugs-auction-001 uint)
...
(define-data-var admin (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF 'SP1A32MBZDN1B28ZT5DH6MA739VJ0BFH05CJHCM8W ))
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? rugs-auction-001 item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? rugs-auction-001 item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? rugs-auction-001 item-id)))
      (begin
        (try! (nft-burn? rugs-auction-001 item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? rugs-auction-001 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rugs-auction-001 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rugs-auction-001 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rugs-auction-001 id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP1A32MBZDN1B28ZT5DH6MA739VJ0BFH05CJHCM8W u500 "Rugs Auction" u10000000 u65149 u65221 "ipfs://Qmey5cv5aYW72bsjVV1moQCyxYhvcPZJMM38WnKoSwTxPS/rugs-auction/1.json"))
  (ok true)
)
