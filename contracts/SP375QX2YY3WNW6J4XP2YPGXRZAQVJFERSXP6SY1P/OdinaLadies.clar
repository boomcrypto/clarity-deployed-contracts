...
(define-non-fungible-token OdinaLadies uint)
...
(define-data-var base-uri (string-ascii 80) "bafybeib7rw3uyfrjhn7vyusd65kgqxoq4aaj4hzqsbvlgmttf6dsmx7jim/")
(define-data-var mint-price uint u6900000)
;; Percent u100 = 1%
(define-data-var royalty-percent uint u250)
(define-data-var mint-limit uint u15)
(define-data-var max-supply uint u1111)
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? OdinaLadies next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? OdinaLadies next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? OdinaLadies id sender recipient)
        success
          (let
            (
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? OdinaLadies id))
)
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? OdinaLadies id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? OdinaLadies id) ERR_NO_OWNER))
