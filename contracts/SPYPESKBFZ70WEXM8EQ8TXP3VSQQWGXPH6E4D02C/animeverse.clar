...
(define-non-fungible-token animeverse uint)
...
(define-data-var artist-address principal 'SPYPESKBFZ70WEXM8EQ8TXP3VSQQWGXPH6E4D02C)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPpA5QHwUfFEWJxtMwqH1YVGJkm42D8JFfYPQ3vQPAFJK/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? animeverse next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? animeverse token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? animeverse token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? animeverse token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? animeverse id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? animeverse id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? animeverse id) (err ERR-NOT-FOUND)))
