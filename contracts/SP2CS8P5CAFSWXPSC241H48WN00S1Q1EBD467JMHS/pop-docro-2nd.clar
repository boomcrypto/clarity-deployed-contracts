...
(define-non-fungible-token pop-docro-2nd uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNP4KCcwzKpnLk9hSTFQHz3RicagwHFAFxD4srUeTsjgQ/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? pop-docro-2nd next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? pop-docro-2nd token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? pop-docro-2nd token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? pop-docro-2nd token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pop-docro-2nd id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pop-docro-2nd id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pop-docro-2nd id) (err ERR-NOT-FOUND)))
