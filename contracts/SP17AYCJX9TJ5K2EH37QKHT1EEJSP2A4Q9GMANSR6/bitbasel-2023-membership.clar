;; bitbasel-2023-membership
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token bitbasel-2023-membership uint)
...
(define-data-var artist-address principal 'SP17AYCJX9TJ5K2EH37QKHT1EEJSP2A4Q9GMANSR6)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUBLxKnbJFvLcz3LAaZEMProtLgKcV2yinpZ8NeBWBSt7/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? bitbasel-2023-membership next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitbasel-2023-membership token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitbasel-2023-membership token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitbasel-2023-membership token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitbasel-2023-membership id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitbasel-2023-membership id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitbasel-2023-membership id) (err ERR-NOT-FOUND)))
