...
(define-non-fungible-token crypto-ice-cream uint)
...
;; Internal variables
(define-data-var mint-limit uint u2044)
(define-data-var last-id uint u1)
(define-data-var total-price uint u4200000)
(define-data-var artist-address principal 'SP125PTPC4P46K9KH6VJEXEXC5FV950RN3NC7CYPZ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWdoXKa3mA4F6KqUqQ1acmATEowAw73gvPwXR4ajVopgH/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? crypto-ice-cream next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? crypto-ice-cream token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? crypto-ice-cream token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? crypto-ice-cream token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? crypto-ice-cream id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? crypto-ice-cream id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? crypto-ice-cream id) (err ERR-NOT-FOUND)))
