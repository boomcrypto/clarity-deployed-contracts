...
(define-non-fungible-token nostr uint)
...
(define-data-var total-price uint u49000000)
(define-data-var artist-address principal 'SP1G00YGHEYYV1PJTHXY7JD7NAY72AA02ZJBV97ME)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeV9AdUhUYsCGXd3mSgNJVeRxoUgddJ1m9RC4KdvBKKVg/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? nostr next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? nostr token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? nostr token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? nostr token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? nostr id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? nostr id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? nostr id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u161580)
