...
(define-constant TOKEN_URI u"https://quicknode.quicknode-ipfs.com/ipfs/Qmcu8GUubJZsAzg11gjsWKwrtjYTnpzcShKtWinTCSvCG4") ;; utf-8 string with token metadata host
(define-constant TOKEN_NAME "sbtcdao.com")
(define-constant TOKEN_SYMBOL "sbtcdao.com")
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    (asserts! (is-eq tx-sender CONTRACT_OWNER) ERR_OWNER_ONLY)
    (ft-mint? sbtcdao amount recipient)
  )
)
...
(begin
    ...
    ;; Transfer the tokens
    (let ((result (ft-transfer? sbtcdao amount sender recipient)))
      (if (is-ok result)
        (begin
...
(define-public (sbtcdao-rewards (recipients (list 1000 principal) ))
...
(begin
  (try! (ft-mint? sbtcdao u500000000000000 CONTRACT_OWNER)) 
  ;; (try! (ft-mint? sbtcdao u20000000000 'SP31GQBM80BBKWABPHGZPRY6W97QTNJX6Z39NZQHV)) 
  ;; (try! (ft-mint? sbtcdao u100000000000 'ST18VZNQWK7R44BFN75X3RANYRAT4B9ZA8S47KZ24)) 
)