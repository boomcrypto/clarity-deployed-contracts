;; title: usdh-token-v1-subnet
;; author: rozar.btc
;; summary: Intent-based subnet token wrapper for usdh-token-v1 using blaze-intent verifier.
...
(define-read-only (get-name)         (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 get-name))
(define-read-only (get-symbol)       (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 get-symbol))
(define-read-only (get-decimals)     (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 get-decimals))
(define-read-only (get-token-uri)    (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 get-token-uri))
(define-read-only (get-total-supply) (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 get-total-supply))
...
(define-public (deposit (amount uint) (recipient (optional principal)))
  (let ((sender tx-sender)
    ...
    (try! (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 transfer amount sender (as-contract tx-sender) none))
...
(define-public (withdraw (amount uint) (recipient (optional principal)))
  (let ((owner tx-sender)
    ...
    (try! (as-contract (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.usdh-token-v1 transfer amount tx-sender recv none)))
