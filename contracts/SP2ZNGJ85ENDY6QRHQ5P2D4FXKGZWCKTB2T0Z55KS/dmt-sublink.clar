...
;; Vault Metadata
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://metadata.charisma.rocks/api/v1/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.dmt-sublink"))
...
(define-public (deposit (amount uint) (recipient principal))
    (begin
        (try! (contract-call? .dmtoken-subnet deposit amount (some recipient)))
...
(define-public (withdraw (amount uint) (recipient principal))
    (begin
        (try! (contract-call? .dmtoken-subnet withdraw amount (some recipient)))
        ;; Return the expected tuple
        (ok {dx: amount, dy: amount, dk: u0})
    ))