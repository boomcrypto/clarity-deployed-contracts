...
;; Vault Metadata
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma-metadata.vercel.app/api/v1/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.stability-v1"))
...
(define-public (deposit (amount uint) (recipient principal))
    (begin
        (try! (contract-call? .susdh-token-subnet-v1 deposit amount (some recipient)))
...
(define-public (withdraw (amount uint) (recipient principal))
    (begin
        (try! (contract-call? .susdh-token-subnet-v1 withdraw amount (some recipient)))
        ;; Return the expected tuple
        (ok {dx: amount, dy: amount, dk: u0})
    ))
