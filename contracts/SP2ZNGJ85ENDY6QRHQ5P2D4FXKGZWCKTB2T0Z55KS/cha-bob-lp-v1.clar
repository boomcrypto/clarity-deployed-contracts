...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? CHA-BOB amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "CHA-BOB Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance CHA-BOB who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply CHA-BOB)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? CHA-BOB (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? CHA-BOB (get dk delta) sender))
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.cha-bob-lp-v1"))
    ...
    (try! (contract-call? 'SP2VG7S0R4Z8PYNYCAQ04HCBX1MH75VT11VXCWQ6G.built-on-bitcoin-stxcity transfer u199900000 tx-sender CONTRACT none))
)