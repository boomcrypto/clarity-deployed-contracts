...
;; Vault Metadata
(define-data-var token-uri (optional (string-utf8 256)) (some u"data:application/json;base64,eyJuYW1lIjoiVVNEQS1zdWJsaW5rIiwiaW1hZ2UiOiJkYXRhOmltYWdlL3BuZztiYXNlNjQsaVZCT1J3MEtHZ29BQUFBTlNVaEVVZ0FBQUFFQUFBQUJDQVlBQUFBZkZjU0pBQUFBQVhOU1IwSUFyczRjNlFBQUFBMUpSRUZVR0ZkaitQOXErWDhBQ1F3RGtJSjdzdFVBQUFBQVNVVk9SSzVDWUlJPSJ9"))
...
(define-public (deposit (amount uint) (recipient principal))
    (begin
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.usda-token-subnet deposit amount (some recipient)))
        (ok {dx: amount, dy: amount, dk: u0})))

(define-public (withdraw (amount uint) (recipient principal))
    (begin
        (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.usda-token-subnet withdraw amount (some recipient)))
        (ok {dx: amount, dy: amount, dk: u0})))