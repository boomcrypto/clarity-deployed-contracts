;; title: leo-token-subnet
;; author: rozar.btc
;; summary: Intent-based subnet token wrapper for leo using blaze-v1 verifier.
...
(define-read-only (get-name)         (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token get-name))
(define-read-only (get-symbol)       (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token get-symbol))
(define-read-only (get-decimals)     (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token get-decimals))
(define-read-only (get-token-uri)    (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token get-token-uri))
(define-read-only (get-total-supply) (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token get-total-supply))
...
(define-public (withdraw (amount uint) (recipient (optional principal)))
  (let ((owner tx-sender)
    ...
    (try! (as-contract (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer amount tx-sender recv none)))
