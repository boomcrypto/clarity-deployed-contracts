;; title: skullcoin-stxcity-subnet
;; author: rozar.btc
;; summary: Intent-based subnet token wrapper for skullcoin-stxcity using blaze-intent verifier.
...
(define-read-only (get-name)         (contract-call? 'SP3BRXZ9Y7P5YP28PSR8YJT39RT51ZZBSECTCADGR.skullcoin-stxcity get-name))
(define-read-only (get-symbol)       (contract-call? 'SP3BRXZ9Y7P5YP28PSR8YJT39RT51ZZBSECTCADGR.skullcoin-stxcity get-symbol))
(define-read-only (get-decimals)     (contract-call? 'SP3BRXZ9Y7P5YP28PSR8YJT39RT51ZZBSECTCADGR.skullcoin-stxcity get-decimals))
(define-read-only (get-token-uri)    (contract-call? 'SP3BRXZ9Y7P5YP28PSR8YJT39RT51ZZBSECTCADGR.skullcoin-stxcity get-token-uri))
(define-read-only (get-total-supply) (contract-call? 'SP3BRXZ9Y7P5YP28PSR8YJT39RT51ZZBSECTCADGR.skullcoin-stxcity get-total-supply))
...
(define-public (deposit (amount uint) (recipient (optional principal)))
  (let ((sender tx-sender)
    ...
    (try! (contract-call? 'SP3BRXZ9Y7P5YP28PSR8YJT39RT51ZZBSECTCADGR.skullcoin-stxcity transfer amount sender (as-contract tx-sender) none))
...
(define-public (withdraw (amount uint) (recipient (optional principal)))
  (let ((owner tx-sender)
    ...
    (try! (as-contract (contract-call? 'SP3BRXZ9Y7P5YP28PSR8YJT39RT51ZZBSECTCADGR.skullcoin-stxcity transfer amount tx-sender recv none)))
