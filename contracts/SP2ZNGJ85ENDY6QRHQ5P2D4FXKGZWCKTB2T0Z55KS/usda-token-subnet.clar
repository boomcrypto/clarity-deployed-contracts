;; title: usda-token-subnet
;; author: rozar.btc
;; summary: Intent-based subnet token wrapper for usda-token using blaze-intent verifier.
...
(define-read-only (get-name)         (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.usda-token get-name))
(define-read-only (get-symbol)       (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.usda-token get-symbol))
(define-read-only (get-decimals)     (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.usda-token get-decimals))
(define-read-only (get-token-uri)    (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.usda-token get-token-uri))
(define-read-only (get-total-supply) (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.usda-token get-total-supply))
...
(define-public (deposit (amount uint) (recipient (optional principal)))
  (let ((sender tx-sender)
    ...
    (try! (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.usda-token transfer amount sender (as-contract tx-sender) none))
...
(define-public (withdraw (amount uint) (recipient (optional principal)))
  (let ((owner tx-sender)
    ...
    (try! (as-contract (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.usda-token transfer amount tx-sender recv none)))
