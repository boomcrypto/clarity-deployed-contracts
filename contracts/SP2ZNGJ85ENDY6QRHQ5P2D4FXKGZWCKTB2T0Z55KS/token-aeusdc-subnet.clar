;; title: token-aeusdc-subnet
;; author: rozar.btc
;; summary: Intent-based subnet token wrapper for token-aeusdc using blaze-intent verifier.
...
(define-read-only (get-name)         (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc get-name))
(define-read-only (get-symbol)       (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc get-symbol))
(define-read-only (get-decimals)     (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc get-decimals))
(define-read-only (get-token-uri)    (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc get-token-uri))
(define-read-only (get-total-supply) (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc get-total-supply))
...
(define-public (deposit (amount uint) (recipient (optional principal)))
  (let ((sender tx-sender)
    ...
    (try! (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer amount sender (as-contract tx-sender) none))
...
(define-public (withdraw (amount uint) (recipient (optional principal)))
  (let ((owner tx-sender)
    ...
    (try! (as-contract (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer amount tx-sender recv none)))
