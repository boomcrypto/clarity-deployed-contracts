...
(define-non-fungible-token drone-wars uint)
...
(define-data-var artist-address principal 'SPCRNBR9ADQ2AWG8R475BM1G7D6BQSTVJVEMH2XW)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTboz3hhTpGDAb76R8oATffszeMwZFp7Y6wwotNZ1UPB3/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? drone-wars next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? drone-wars token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? drone-wars token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? drone-wars token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? drone-wars id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? drone-wars id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? drone-wars id) (err ERR-NOT-FOUND)))
