...
(define-non-fungible-token retro uint)
...
;; Internal variables
(define-data-var mint-limit uint u1801)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeWAqbwoMcNjTZo9Yv7kgtE2qmcuqbtzcveKprcucf371/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? retro next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? retro token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? retro token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? retro token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? retro id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? retro id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? retro id) (err ERR-NOT-FOUND)))
