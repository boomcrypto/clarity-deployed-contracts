...
(define-non-fungible-token phat-penguins uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPHHGSnx8P2BeTHWjjhNhS7iqhq6UqMnYsyddn11iK2FC/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? phat-penguins next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? phat-penguins token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? phat-penguins token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? phat-penguins token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? phat-penguins id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? phat-penguins id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? phat-penguins id) (err ERR-NOT-FOUND)))
