...
;; --- Define LP Token ---
(define-fungible-token WIF-ROCK)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (try! (ft-transfer? WIF-ROCK amount sender recipient))
    (match memo to-print (print to-print) 0x0000)
    (ok true)
  )
)

(define-read-only (get-name) (ok "WIF-ROCK Liquidity Pool"))
(define-read-only (get-symbol) (ok "WIF-ROCK"))
(define-read-only (get-decimals) (ok u6))

(define-read-only (get-balance (who principal))
  (ok (ft-get-balance WIF-ROCK who))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply WIF-ROCK))
)
...
(define-public (swap-b-to-a (amount uint))
  (let (
    ...
    (try! (as-contract (contract-call? 'SP3WPNAEBYMX06RQNNYTH5PTJ1FRGX5A13ZZMZ01D.dogwifhat-token transfer (get dy delta) CONTRACT sender none)))
...
(define-public (add-liquidity (amount uint))
  (let (
    ...
    (try! (contract-call? 'SP3WPNAEBYMX06RQNNYTH5PTJ1FRGX5A13ZZMZ01D.dogwifhat-token transfer (get dx delta) sender CONTRACT none))
    (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock transfer (get dy delta) sender CONTRACT none))
    (try! (ft-mint? WIF-ROCK (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
  (let (
    ...
    (try! (ft-burn? WIF-ROCK (get dk delta) sender))
    (try! (as-contract (contract-call? 'SP3WPNAEBYMX06RQNNYTH5PTJ1FRGX5A13ZZMZ01D.dogwifhat-token transfer (get dx delta) CONTRACT sender none)))
    (try! (as-contract (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock transfer (get dy delta) CONTRACT sender none)))
...
(define-private (get-reserves)
  {
    a: (unwrap-panic (contract-call? 'SP3WPNAEBYMX06RQNNYTH5PTJ1FRGX5A13ZZMZ01D.dogwifhat-token get-balance CONTRACT)),
...
(begin
  ;; Set token URI
  (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP22KATK6MJF40987KB2KSZQ6E027HQ0CPP73C9Y.wif-rock-v1"))

  ;; Add initial balanced liquidity (handles both token transfers at 1:1)
  (try! (add-liquidity u1150000000000))

  ;; Transfer additional token A to achieve desired ratio
  (try! (contract-call? 'SP3WPNAEBYMX06RQNNYTH5PTJ1FRGX5A13ZZMZ01D.dogwifhat-token transfer u2350000000000 tx-sender CONTRACT none))
)
