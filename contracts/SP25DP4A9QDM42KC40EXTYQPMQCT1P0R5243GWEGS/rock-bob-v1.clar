...
;; Define LP token
(define-fungible-token ROCK-BOB)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (try! (ft-transfer? ROCK-BOB amount sender recipient))
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance ROCK-BOB who))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply ROCK-BOB))
)
...
(define-public (add-liquidity (amount uint))
  (let (
    ...
    (try! (ft-mint? ROCK-BOB (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
  (let (
    ...
    (try! (ft-burn? ROCK-BOB (get dk delta) sender))
    (try! (as-contract (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock transfer (get dx delta) CONTRACT sender none)))
    (try! (as-contract (contract-call? 'SP2VG7S0R4Z8PYNYCAQ04HCBX1MH75VT11VXCWQ6G.built-on-bitcoin-stxcity transfer (get dy delta) CONTRACT sender none)))
...
(begin
  ;; Set token URI
  (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP22KATK6MJF40987KB2KSZQ6E027HQ0CPP73C9Y.rock-bob-v1"))
  ...
  (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock transfer u9990000000000 tx-sender CONTRACT none))
)
