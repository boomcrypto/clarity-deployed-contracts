...
;; Define LP token
(define-fungible-token ROCK-ROO)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (try! (ft-transfer? ROCK-ROO amount sender recipient))
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance ROCK-ROO who))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply ROCK-ROO))
)
...
(define-public (swap-a-to-b (amount uint))
  (let (
    ...
    (try! (as-contract (contract-call? 'SP2C1WREHGM75C7TGFAEJPFKTFTEGZKF6DFT6E2GE.kangaroo transfer (get dy delta) CONTRACT sender none)))
...
(define-public (add-liquidity (amount uint))
  (let (
    ...
    (try! (contract-call? 'SP2C1WREHGM75C7TGFAEJPFKTFTEGZKF6DFT6E2GE.kangaroo transfer (get dy delta) sender CONTRACT none))
    (try! (ft-mint? ROCK-ROO (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
  (let (
    ...
    (try! (ft-burn? ROCK-ROO (get dk delta) sender))
...
(define-private (get-reserves)
  {
    ...
    b: (unwrap-panic (contract-call? 'SP2C1WREHGM75C7TGFAEJPFKTFTEGZKF6DFT6E2GE.kangaroo get-balance CONTRACT))
  }
)
...
(begin
  ;; Set token URI
  (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP22KATK6MJF40987KB2KSZQ6E027HQ0CPP73C9Y.rock-roo-v1"))
  ...
  (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock transfer u11769000000000 tx-sender CONTRACT none))
)
