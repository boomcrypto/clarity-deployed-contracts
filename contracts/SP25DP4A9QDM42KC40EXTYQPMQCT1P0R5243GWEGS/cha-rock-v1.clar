...
;; Define LP token
(define-fungible-token CHA-ROCK)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (try! (ft-transfer? CHA-ROCK amount sender recipient))
    (match memo to-print (print to-print) 0x0000)
    (ok true)
  )
)

(define-read-only (get-name) (ok "CHA-ROCK Liquidity Pool"))
(define-read-only (get-symbol) (ok "CHA-ROCK"))
(define-read-only (get-decimals) (ok u6))
(define-read-only (get-balance (who principal)) (ok (ft-get-balance CHA-ROCK who)))
(define-read-only (get-total-supply) (ok (ft-get-supply CHA-ROCK)))
...
(define-public (add-liquidity (amount uint))
  (let (
    ...
    (try! (ft-mint? CHA-ROCK (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
  (let (
    ...
    (try! (ft-burn? CHA-ROCK (get dk delta) sender))
...
(begin
  ;; Set token URI
  (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP22KATK6MJF40987KB2KSZQ6E027HQ0CPP73C9Y.cha-rock-v1"))
  ...
  (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock transfer u1176999000000 tx-sender CONTRACT none))
)
