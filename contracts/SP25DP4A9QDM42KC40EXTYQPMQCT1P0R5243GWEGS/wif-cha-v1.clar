...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (try! (ft-transfer? WIF-CHA amount sender recipient))
    (match memo to-print (print to-print) 0x0000)
    (ok true)
  )
)

(define-read-only (get-name) (ok "WIF-CHA Liquidity Pool"))
(define-read-only (get-symbol) (ok "WIF-CHA"))
(define-read-only (get-decimals) (ok u6))
(define-read-only (get-balance (who principal)) (ok (ft-get-balance WIF-CHA who)))
(define-read-only (get-total-supply) (ok (ft-get-supply WIF-CHA)))
...
(define-public (add-liquidity (amount uint))
  (let (
    ...
    (try! (ft-mint? WIF-CHA (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
  (let (
    ...
    (try! (ft-burn? WIF-CHA (get dk delta) sender))
...
;; --- Initialization ---
(begin
  (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP22KATK6MJF40987KB2KSZQ6E027HQ0CPP73C9Y.wif-cha-v1"))
  (try! (add-liquidity u1000000))
  (try! (contract-call? 'SP3WPNAEBYMX06RQNNYTH5PTJ1FRGX5A13ZZMZ01D.dogwifhat-token transfer u3499999000000 tx-sender CONTRACT none))
)
