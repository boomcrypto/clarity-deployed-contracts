...
(define-constant err-not-approval (err u101))
...
;; full uri: https://graphigo.prd.galaxy.eco/metadata/stacks/{contract_hash}/{nft_id}.json
(define-constant base-uri "https://graphigo.prd.galaxy.eco/metadata/stacks/")
...
(define-public (test-mint (token-id uint)) 
    (ok (try! (nft-mint? starnfts token-id tx-sender)))
)
...
(define-private (nft-uri (token-id uint)) 
    (concat 
        (concat 
            ...
            (concat
                (concat
                    ;; (default-to "" (get name (unwrap-panic (principal-destruct? (as-contract tx-sender)))))
                    (concat
                        token-name
                        token-symbol                    
                    )
