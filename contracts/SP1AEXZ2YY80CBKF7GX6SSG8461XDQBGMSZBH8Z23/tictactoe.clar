...
(define-non-fungible-token tictactoe uint)
...
;; Internal variables
(define-data-var mint-limit uint u1901)
(define-data-var last-id uint u1)
(define-data-var total-price uint u200000000)
(define-data-var artist-address principal 'SP1AEXZ2YY80CBKF7GX6SSG8461XDQBGMSZBH8Z23)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRveGn4xpM1ytSuBhqviPEKniSDLVjtUZr295BRpV6hED/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? tictactoe next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? tictactoe token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? tictactoe token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tictactoe token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? tictactoe id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? tictactoe id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? tictactoe id) (err ERR-NOT-FOUND)))
