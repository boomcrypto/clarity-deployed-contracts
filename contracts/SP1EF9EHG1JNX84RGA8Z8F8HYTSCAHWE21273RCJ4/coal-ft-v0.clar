...
(define-fungible-token ata-coal)

(define-data-var token-uri (optional (string-utf8 256)) (some u"https://ata-game.space/v0/token-metadata/coal"))

(define-map authorized-to-mint-contracts principal bool)
(map-insert authorized-to-mint-contracts .coal-v0 true)
...
(define-public (mint (amount uint))
  (begin
    ...
    (ft-mint? ata-coal amount tx-sender)
  )
)

(define-public (burn (amount uint))
  (ft-burn? ata-coal amount tx-sender)
)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? ata-coal amount tx-sender recipient))
    (match memo to-print (print to-print) 0x)
    (ok true)
  )
)

(define-read-only (get-name) (ok "ATA Coal"))

(define-read-only (get-symbol) (ok "aCOL"))

(define-read-only (get-decimals) (ok u0))

(define-read-only (get-balance (who principal)) (ok (ft-get-balance ata-coal who)))

(define-read-only (get-total-supply) (ok (ft-get-supply ata-coal)))
