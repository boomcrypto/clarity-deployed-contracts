...
(define-private (spend-upgrade-resources (lvl uint))
  (begin
    ...
    (try! (contract-call? .ata-spend-v0 spend-resource .gravel-ft-v0 (var-get cost-data-0) lvl))
    (try! (contract-call? .ata-spend-v0 spend-resource .stone-ft-v0 (var-get cost-data-1) lvl))
    (try! (contract-call? .ata-spend-v0 spend-resource .wood-ft-v0 (var-get cost-data-2) lvl))
...
;; COLLECT HANDLING
(define-data-var base-production uint u8)
...
(define-public (collect)
  (let (
    (player (unwrap! (contract-call? .sand-store-v0 get-player tx-sender) ERR_NOT_REGISTERED))
    ...
    (try! (contract-call? .sand-ft-v0 mint to-mint))
    (contract-call? .sand-store-v0 save-lma)
  )
)
...
(define-public (register)
  (begin
    ...
    (try! (contract-call? .sand-ft-v0 mint (var-get initial-mint)))
    (ok (try! (contract-call? .sand-store-v0 register-first-factory tx-sender)))
  )
)
...
(define-public (add-factory)
  (let (
    (factories (unwrap! (contract-call? .sand-store-v0 get-factories tx-sender) ERR_NOT_REGISTERED))
