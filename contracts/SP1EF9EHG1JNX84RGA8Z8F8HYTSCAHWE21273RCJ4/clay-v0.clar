...
(define-private (spend-upgrade-resources (lvl uint))
  (begin
    ...
    (try! (contract-call? .ata-spend-v0 spend-resource .sand-ft-v0 (var-get cost-data-0) lvl))
    ...
    (try! (contract-call? .ata-spend-v0 spend-resource .stone-ft-v0 (var-get cost-data-3) lvl))
...
(define-public (collect)
  (let (
    (player (unwrap! (contract-call? .clay-store-v0 get-player tx-sender) ERR_NOT_REGISTERED))
    ...
    (try! (contract-call? .clay-ft-v0 mint to-mint))
    (contract-call? .clay-store-v0 save-lma)
  )
)
...
(define-public (register)
  (begin
    ...
    (try! (contract-call? .clay-ft-v0 mint (var-get initial-mint)))
    (ok (try! (contract-call? .clay-store-v0 register-first-factory tx-sender)))
  )
)
...
(define-public (add-factory)
  (let (
    (factories (unwrap! (contract-call? .clay-store-v0 get-factories tx-sender) ERR_NOT_REGISTERED))
