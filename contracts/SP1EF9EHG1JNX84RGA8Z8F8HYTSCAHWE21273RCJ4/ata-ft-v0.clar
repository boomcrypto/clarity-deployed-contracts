...
(define-fungible-token ata)

(define-data-var token-uri (optional (string-utf8 256)) (some u"https://ata-game.space/v0/token-metadata/ata"))

(define-map authorized-to-mint-contracts
  principal
  bool
)
(map-insert authorized-to-mint-contracts .ata-v0 true)
...
(define-public (mint (amount uint))
  (begin
    ...
    (ft-mint? ata amount tx-sender)
  )
)

(define-public (burn (amount uint))
  (ft-burn? ata amount tx-sender)
)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? ata amount tx-sender recipient))
    (match memo
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance ata who))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply ata))
)
...
(define-public (transfer-from-treasury
    (amount uint)
    (recipient principal)
    (memo (optional (buff 34)))
  )
  (begin
    (try! (contract-call? .ata-admin-v0 is-admin))
    (try! (as-contract (transfer amount .ata-ft-v0 recipient memo)))
    (match memo
      to-print (print to-print)
      0x
    )
