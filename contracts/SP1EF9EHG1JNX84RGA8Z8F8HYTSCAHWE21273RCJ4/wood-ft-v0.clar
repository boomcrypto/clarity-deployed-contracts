...
(define-fungible-token ata-wood)

(define-data-var token-uri (optional (string-utf8 256)) (some u"https://ata-game.space/v0/token-metadata/wood"))

(define-map authorized-to-mint-contracts principal bool)
(map-insert authorized-to-mint-contracts .wood-v0 true)
...
(define-public (mint (amount uint))
  (begin
    ...
    (ft-mint? ata-wood amount tx-sender)
  )
)

(define-public (burn (amount uint))
  (ft-burn? ata-wood amount tx-sender)
)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? ata-wood amount tx-sender recipient))
    (match memo to-print (print to-print) 0x)
    (ok true)
  )
)

(define-read-only (get-name) (ok "ATA Wood"))

(define-read-only (get-symbol) (ok "aWOD"))

(define-read-only (get-decimals) (ok u0))

(define-read-only (get-balance (who principal)) (ok (ft-get-balance ata-wood who)))

(define-read-only (get-total-supply) (ok (ft-get-supply ata-wood)))
