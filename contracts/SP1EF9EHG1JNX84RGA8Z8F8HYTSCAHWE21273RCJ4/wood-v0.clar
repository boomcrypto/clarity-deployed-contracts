...
(define-private (spend-upgrade-resources (lvl uint))
  (begin
    ...
    (try! (contract-call? .ata-spend-v0 spend-resource .coal-ft-v0 (var-get cost-data-2) lvl))
...
;; COLLECT HANDLING
(define-data-var base-production uint u25)
...
(define-public (collect)
  (let (
    (player (unwrap! (contract-call? .wood-store-v0 get-player tx-sender) ERR_NOT_REGISTERED))
    ...
    (try! (contract-call? .wood-ft-v0 mint to-mint))
    (contract-call? .wood-store-v0 save-lma)
  )
)
...
(define-public (register)
  (begin
    ...
    (try! (contract-call? .wood-ft-v0 mint (var-get initial-mint)))
    (ok (try! (contract-call? .wood-store-v0 register-first-factory tx-sender)))
  )
)
...
(define-public (add-factory)
  (let (
    (factories (unwrap! (contract-call? .wood-store-v0 get-factories tx-sender) ERR_NOT_REGISTERED))
