...
;; Define LP token
(define-fungible-token sBTC-WPS)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? sBTC-WPS amount sender recipient))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance sBTC-WPS who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply sBTC-WPS)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP14J806BWEPQAXVA0G6RYZN7GNA126B7JFRRYTEM.world-peace-stacks-stxcity transfer (get dy delta) CONTRACT sender none)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP14J806BWEPQAXVA0G6RYZN7GNA126B7JFRRYTEM.world-peace-stacks-stxcity transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP14J806BWEPQAXVA0G6RYZN7GNA126B7JFRRYTEM.world-peace-stacks-stxcity transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? sBTC-WPS (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? sBTC-WPS (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP14J806BWEPQAXVA0G6RYZN7GNA126B7JFRRYTEM.world-peace-stacks-stxcity get-balance CONTRACT))
    })
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP2KGJEAZRDVK78ZWTRGSDE11A1VMZVEATNQFZ73C.world-peace-sbtc-v1"))
    ;; Add initial balanced liquidity (handles both token transfers at 1:1)
    (try! (add-liquidity u900))
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SP14J806BWEPQAXVA0G6RYZN7GNA126B7JFRRYTEM.world-peace-stacks-stxcity transfer u299999999100 tx-sender CONTRACT none))
)