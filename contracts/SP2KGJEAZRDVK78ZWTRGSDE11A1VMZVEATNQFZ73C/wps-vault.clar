...
;; Vault Metadata
(define-data-var token-uri (optional (string-utf8 256)) (some u"data:application/json;base64,eyJuYW1lIjoiV1BTIiwiaW1hZ2UiOiJkYXRhOmltYWdlL3BuZztiYXNlNjQsaVZCT1J3MEtHZ29BQUFBTlNVaEVVZ0FBQUFFQUFBQUJDQVlBQUFBZkZjU0pBQUFBQVhOU1IwSUFyczRjNlFBQUFBMUpSRUZVR0ZkaitIK1g0VDhBQjVjQzNLekJkeHdBQUFBQVNVVk9SSzVDWUlJPSJ9"))
...
(define-public (deposit (amount uint) (recipient principal))
    (begin
        (try! (contract-call? 'SP2KGJEAZRDVK78ZWTRGSDE11A1VMZVEATNQFZ73C.world-peace-stacks-stxcity-subnet deposit amount (some recipient)))
        (ok {dx: amount, dy: amount, dk: u0})))

(define-public (withdraw (amount uint) (recipient principal))
    (begin
        (try! (contract-call? 'SP2KGJEAZRDVK78ZWTRGSDE11A1VMZVEATNQFZ73C.world-peace-stacks-stxcity-subnet withdraw amount (some recipient)))
        (ok {dx: amount, dy: amount, dk: u0})))