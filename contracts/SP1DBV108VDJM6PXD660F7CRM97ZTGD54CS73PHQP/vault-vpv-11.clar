...
;; Vault Trait 
(impl-trait .vault-trait-vpv-11.vault-trait)
...
(define-public (new-vault-wrapper (collateral-sbtc uint) (loan-bsd uint) (interest uint) (hint (optional uint)) (price-data (optional (buff 8192))) (bsd <bsd-trait>) (sbtc <sbtc-trait>) (oracle <oracle-trait>) (registry <registry-trait>) (stability <stability-trait>) (sorted-vaults <sorted-vaults-trait>))
    (let 
        (
            ...
            (valid-stability (try! (contract-call? .controller-vpv-11 check-approved-contract "stability" (contract-of stability))))
...
(define-public (protocol-transfer-sbtc (to principal) (amount uint) (sbtc <sbtc-trait>))
     (let 
        ...
        (try! (contract-call? .controller-vpv-11 is-protocol-caller contract-caller))
...
(define-public (process-epoch-rates (vaults (list 100 uint)) (hints (list 100 (optional uint))) (registry <registry-trait>) (sorted-vaults <sorted-vaults-trait>))
    (let
        (
            (valid-registry (try! (contract-call? .controller-vpv-11 check-approved-contract "registry" (contract-of registry))))
            (valid-sorted-vaults (try! (contract-call? .controller-vpv-11 check-approved-contract "sorted-vaults" (contract-of sorted-vaults))))
            (is-keeper (unwrap! (contract-call? .controller-vpv-11 is-keeper tx-sender) ERR_NO_KEEPER))
        )
...
(define-private (accrue-vault (vault-id uint) (bsd <bsd-trait>) (registry <registry-trait>))
    (let 
        (
            ...
            (is-keeper (unwrap-panic (contract-call? .controller-vpv-11 is-keeper tx-sender)))
        )
