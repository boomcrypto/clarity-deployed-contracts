...
(use-trait stability-trait .stability-trait-vpv-17.stability-trait)
...
(define-public (propose-vault-protocol-transfer-sbtc 
    ...
    (let 
        (
          ...
          (delay (unwrap! (contract-call? .registry-vpv-17 get-timelock-delay) ERR_INVALID_DELAY))
...
(define-private (execute-redeem-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-17 set-redeem-parameters 
...
(define-private (execute-decay-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-17 set-decay-parameters 
            new-max-hours-decay 
...
(define-private (execute-vault-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-17 set-vault-parameters 
...
(define-private (execute-borrow-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-17 set-borrow-parameters 
...
(define-private (execute-global-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-17 set-global-parameters 
...
(define-private (execute-add-hot-pause-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-17 add-hot-pause-principal add-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-hot-pause-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-17 remove-hot-pause-principal remove-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-add-supported-contract-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-17 add-supported-contract contract-key contract-address qualified-contract)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-supported-contract-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-17 remove-supported-contract contract-key)
            ok-result
            (begin
                (print {
...
(define-private (execute-set-keeper-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-17 set-keeper new-keeper)
            ok-result
            (begin
                (print {
...
(define-private (execute-add-controller-privileged-protocol-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-17 add-privileged-protocol-principal new-protocol-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-controller-privileged-protocol-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-17 remove-privileged-protocol-principal remove-protocol-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-propose-admin-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-17 propose-admin new-admin)
            ok-result
            (begin
                (print {
...
(define-private (execute-set-contracts-allowed-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-17 set-contracts-allowed allowed)
            ok-result
            (begin
                (print {
...
(define-private (execute-keeper-allow-all-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-17 keeper-allow-all allowed)
            ok-result
            (begin
                (print {
