...
(use-trait vault-trait .vault-trait-vpv-12.vault-trait)
...
(define-public (propose-vault-protocol-transfer-sbtc 
    ...
    (let 
        (
          ...
          (delay (unwrap! (contract-call? .registry-vpv-12 get-timelock-delay) ERR_INVALID_DELAY))
...
(define-private (execute-redeem-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-12 set-redeem-parameters 
...
(define-private (execute-decay-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-12 set-decay-parameters 
...
(define-private (execute-vault-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-12 set-vault-parameters 
...
(define-private (execute-borrow-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-12 set-borrow-parameters 
...
(define-private (execute-global-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-12 set-global-parameters 
...
(define-private (execute-add-hot-pause-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-12 add-hot-pause-principal add-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-hot-pause-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-12 remove-hot-pause-principal remove-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-add-supported-contract-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-12 add-supported-contract contract-key contract-address qualified-contract)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-supported-contract-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-12 remove-supported-contract contract-key)
            ok-result
            (begin
                (print {
...
(define-private (execute-set-keeper-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-12 set-keeper new-keeper)
            ok-result
            (begin
                (print {
...
(define-private (execute-add-controller-privileged-protocol-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-12 add-privileged-protocol-principal new-protocol-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-controller-privileged-protocol-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-12 remove-privileged-protocol-principal remove-protocol-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-propose-admin-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-12 propose-admin new-admin)
            ok-result
            (begin
                (print {
...
(define-private (execute-set-contracts-allowed-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-12 set-contracts-allowed allowed)
            ok-result
            (begin
                (print {
...
(define-private (execute-keeper-allow-all-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-12 keeper-allow-all allowed)
            ok-result
            (begin
                (print {
