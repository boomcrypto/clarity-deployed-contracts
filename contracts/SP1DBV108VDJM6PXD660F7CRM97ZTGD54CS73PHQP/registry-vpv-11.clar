...
(impl-trait .registry-trait-vpv-11.registry-trait)
...
(define-public (set-processed-epoch (new-epoch uint))
    (let
        (
            (keeper (unwrap-panic (contract-call? .controller-vpv-11 is-keeper tx-sender)))
        )
...
(define-public (hot-pause)
    (begin
        (try! (contract-call? .controller-vpv-11 is-hot-pause-caller tx-sender))
