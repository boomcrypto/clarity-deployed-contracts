...
;; Redeem Trait 
(impl-trait .redeem-trait-vpv-15.redeem-trait)

;; bsd protocol
(use-trait bsd-trait .bsd-trait-vpv-15.bsd-trait)

;; sip-010-trait
(use-trait sbtc-trait .sip-010-trait-ft-standard-vpv-15.sip-010-trait)

;; oracle
(use-trait oracle-trait .oracle-trait-vpv-15.oracle-trait)

;; registry
(use-trait registry-trait .registry-trait-vpv-15.registry-trait)

;; vault
(use-trait vault-trait .vault-trait-vpv-15.vault-trait)
...
;; redeem bsd
(define-public (redeem-wrapper (bsd-amount uint) (price-data (optional (buff 8192))) (bsd <bsd-trait>) (sbtc <sbtc-trait>) (oracle <oracle-trait>) (registry <registry-trait>) (vault <vault-trait>) (sorted-vaults <sorted-vaults-trait>))
    (let 
        (
            ;; checks
            (valid-principal (try! (contract-call? .controller-vpv-15 verify-principal contract-caller)))
            (valid-vault (try! (contract-call? .controller-vpv-15 check-approved-contract "vault" (contract-of vault))))
            (valid-registry (try! (contract-call? .controller-vpv-15 check-approved-contract "registry" (contract-of registry))))
            (valid-oracle (try! (contract-call? .controller-vpv-15 check-approved-contract "oracle" (contract-of oracle))))
            (valid-bsd (try! (contract-call? .controller-vpv-15 check-approved-contract "bsd" (contract-of bsd))))
            (valid-sbtc (try! (contract-call? .controller-vpv-15 check-approved-contract "sbtc" (contract-of sbtc))))
            (valid-sorted-vaults (try! (contract-call? .controller-vpv-15 check-approved-contract "sorted-vaults" (contract-of sorted-vaults))))
        ...
        ;; transfer bsd to redeem contract
        (try! (contract-call? bsd transfer bsd-amount tx-sender (as-contract tx-sender) none))

        ;; burn the bsd
        (try! (contract-call? bsd burn bsd-amount (as-contract tx-sender)))

        ;; Transfer the sbtc to the user - we need to use protocol-transfer here because the sbtc is coming from the vault contract
        (try! (contract-call? vault protocol-transfer-sbtc tx-sender redeem-to-user sbtc))
    ...
    (match  helper-tuple
        ...
        (let
            (
                ...
                (current-vault-info (unwrap-panic (contract-call? registry get-vault-compounded-info vault-id (get price ok-tuple))))
