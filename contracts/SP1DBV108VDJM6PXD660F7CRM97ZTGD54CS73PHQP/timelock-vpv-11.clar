...
(use-trait stability-trait .stability-trait-vpv-11.stability-trait)
...
(define-public (propose-vault-protocol-transfer-sbtc 
    ...
    (let 
        (
          ...
          (delay (unwrap! (contract-call? .registry-vpv-11 get-timelock-delay) ERR_INVALID_DELAY))
        ...
        (try! (contract-call? .controller-vpv-11 is-admin tx-sender))
...
(define-public (propose-vault-params
                (new-interest-minimum uint)
                (new-interest-maximum uint)
                (new-vault-collateral-ratio-threshold uint)
                (new-vault-recovery-ratio-threshold uint))
    (let ((proposal-id (var-get proposal-counter))
          (delay (unwrap! (contract-call? .registry-vpv-11 get-timelock-delay) ERR_INVALID_DELAY))
          (eta (+ burn-block-height delay))
          (args-buff (unwrap! (to-consensus-buff? { 
...
(define-public (propose-borrow-params
                (new-min-borrow-fee uint)
                (new-max-borrow-fee uint)
                (new-loan-minimum uint))
    (let ((proposal-id (var-get proposal-counter))
          (delay (unwrap! (contract-call? .registry-vpv-11 get-timelock-delay) ERR_INVALID_DELAY))
          (eta (+ burn-block-height delay))
          (args-buff (unwrap! (to-consensus-buff? { 
...
(define-public (propose-redeem-params
                (new-min-redeem-fee uint)
                (new-max-redeem-fee uint)
                (new-alpha uint)
                (new-min-redeem-amount uint)
                (new-max-vaults-to-redeem uint))
    (let ((proposal-id (var-get proposal-counter))
          (delay (unwrap! (contract-call? .registry-vpv-11 get-timelock-delay) ERR_INVALID_DELAY))
          (eta (+ burn-block-height delay))
          (args-buff (unwrap! (to-consensus-buff? { 
...
(define-public (propose-global-params
                (new-global-collateral-ratio-threshold uint)
                (new-global-collateral-cap uint)
                (new-protocol-fee-destination principal)
                (new-min-stability-provider-balance uint)
                (new-epoch-genesis uint)
                (new-oracle-stale-threshold-seconds uint)
                (new-oracle-allowable-price-deviation uint)
...
(define-private (execute-redeem-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-11 set-redeem-parameters 
...
(define-private (execute-decay-update (args-buff (buff 10000)))
    (let
        (
            ...
            (new-delta (get new-delta proposal-args))
            (new-max-decay (get new-max-decay proposal-args))
            (new-max-hours-decay (get new-max-hours-decay proposal-args))
            (new-block-decay-rates (get new-block-decay-rates proposal-args))
            (new-blocks-per-hour (get new-blocks-per-hour proposal-args))
            (new-hours-per-epoch (get new-hours-per-epoch proposal-args))
        )
        (match (contract-call? .registry-vpv-11 set-decay-parameters 
            new-delta new-max-decay new-max-hours-decay 
            new-block-decay-rates new-blocks-per-hour new-hours-per-epoch)
            ok-result
            (begin
                (print {
...
(define-private (execute-vault-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-11 set-vault-parameters 
...
(define-private (execute-borrow-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-11 set-borrow-parameters 
...
(define-private (execute-global-params-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .registry-vpv-11 set-global-parameters 
            new-global-collateral-ratio-threshold new-global-collateral-cap new-protocol-fee-destination 
            new-min-stability-provider-balance new-epoch-genesis new-oracle-stale-threshold-seconds 
...
(define-private (execute-add-hot-pause-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-11 add-hot-pause-principal add-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-hot-pause-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-11 remove-hot-pause-principal remove-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-add-supported-contract-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-11 add-supported-contract contract-key contract-address qualified-contract)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-supported-contract-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-11 remove-supported-contract contract-key)
            ok-result
            (begin
                (print {
...
(define-private (execute-set-keeper-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-11 set-keeper new-keeper)
            ok-result
            (begin
                (print {
...
(define-private (execute-add-controller-privileged-protocol-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-11 add-privileged-protocol-principal new-protocol-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-remove-controller-privileged-protocol-principal-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-11 remove-privileged-protocol-principal remove-protocol-principal)
            ok-result
            (begin
                (print {
...
(define-private (execute-propose-admin-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-11 propose-admin new-admin)
            ok-result
            (begin
                (print {
...
(define-private (execute-set-contracts-allowed-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-11 set-contracts-allowed allowed)
            ok-result
            (begin
                (print {
...
(define-private (execute-keeper-allow-all-update (args-buff (buff 10000)))
    (let
        ...
        (match (contract-call? .controller-vpv-11 keeper-allow-all allowed)
            ok-result
            (begin
                (print {
