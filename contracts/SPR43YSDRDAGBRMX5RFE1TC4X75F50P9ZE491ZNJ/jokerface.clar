...
(define-non-fungible-token jokerface uint)
...
;; Internal variables
(define-data-var mint-limit uint u988)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1000000)
(define-data-var artist-address principal 'SPR43YSDRDAGBRMX5RFE1TC4X75F50P9ZE491ZNJ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXp6yEiZHYHFgK5V8VFK66HsJRA1JDHtKzk4s7WA2RmTN/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? jokerface next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? jokerface token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? jokerface token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? jokerface token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? jokerface id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? jokerface id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? jokerface id) (err ERR-NOT-FOUND)))
