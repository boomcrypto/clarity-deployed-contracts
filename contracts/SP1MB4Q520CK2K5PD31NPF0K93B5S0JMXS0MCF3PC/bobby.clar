...
(define-non-fungible-token bobby uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmXSNb2RpSN7ai5bFUNyjkCkxggh7ELuoUbpgMPVAdQ8J9/bobby/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP2RVCMFS8C2XZYGMQ36HKZM0PKXDWE8GE64QMQQ1 )) 
...
(define-data-var public-target-block-height uint u64264)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bobby id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace bobby
  (ok (nft-get-owner? bobby id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? bobby next-id new-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bobby id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bobby id) ERR-NOT-FOUND))
