...
(define-non-fungible-token eggs uint)
...
;; Internal variables
(define-data-var mint-limit uint u1773)
(define-data-var last-id uint u1)
(define-data-var total-price uint u800000000)
(define-data-var artist-address principal 'SP2XBP39H3CDPN8MHXK7YNE7S69SMTCDAGDP7YNYV)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRBUexyLqCRd39Rfdbm1v3sWjWLkHzjW9atxAFdCkhvn7/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? eggs next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? eggs token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? eggs token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? eggs token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? eggs id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? eggs id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? eggs id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u1061408)
