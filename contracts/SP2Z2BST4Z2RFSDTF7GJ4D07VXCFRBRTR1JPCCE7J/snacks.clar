...
(define-non-fungible-token snacks uint)
...
(define-data-var artist-address principal 'SP2420MQMMWX05HKC5CEAB2R2GR9DSAQX07DY3WBT)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXWmmfwKM9UHpy8cfeMCzahUjEss8tNMLvPNZ4gQdLGNn/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? snacks next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? snacks token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? snacks token-id)))
