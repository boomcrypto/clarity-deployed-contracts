...
(define-non-fungible-token btc-shadyz uint)
...
(define-data-var artist-address principal 'SP3G8VKEGFMNNATXKJR8211YX195BFMPG4RQDNMSN)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeL5iPwXvVnicJMY9s5Uik62iAFVkqUMDg5kuYWEt1NPK/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? btc-shadyz next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? btc-shadyz token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? btc-shadyz token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? btc-shadyz token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? btc-shadyz id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? btc-shadyz id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? btc-shadyz id) (err ERR-NOT-FOUND)))
