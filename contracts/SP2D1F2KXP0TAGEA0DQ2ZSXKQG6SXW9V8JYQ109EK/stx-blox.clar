...
(define-non-fungible-token stx-blox uint)
...
(define-data-var artist-address principal 'SP2D1F2KXP0TAGEA0DQ2ZSXKQG6SXW9V8JYQ109EK)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXHzwe731n4neK3YYxpXwYShsgWVbRCzm2r6rTGpriX9d/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stx-blox next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stx-blox token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stx-blox token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stx-blox token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stx-blox id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stx-blox id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stx-blox id) (err ERR-NOT-FOUND)))
