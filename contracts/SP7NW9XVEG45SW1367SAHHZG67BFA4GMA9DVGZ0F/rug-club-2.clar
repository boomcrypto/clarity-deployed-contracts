...
(define-non-fungible-token rug-club-2 uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmWFWBnXjMSjAvmtsPpoXgPLUiK4ddNpUY8oDdCCN1ALvo/")
...
(define-data-var royalty (list 1000 uint) (list u6300 u2700 ))
(define-data-var artist-address (list 1000 principal) (list 'SP7NW9XVEG45SW1367SAHHZG67BFA4GMA9DVGZ0F 'SP3KAPN7TWS0H7CHMTPJ2DWWX9AGPJ821TMHGMDGX )) 

(define-data-var private-sale-active bool true)
(define-data-var sale-active bool true)
(define-data-var private-target-block-height uint u63649)
(define-data-var public-target-block-height uint u63787)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rug-club-2 id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace rug-club-2
  (ok (nft-get-owner? rug-club-2 id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? rug-club-2 next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? rug-club-2 id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rug-club-2 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rug-club-2 id) ERR-NOT-FOUND))
...
(define-public (pay (id uint) (price uint))
    (if (> (var-get secondary-commission) u0)
        (begin  
                ...
                ;; total of 5% = u500
                  (try! (stx-transfer? (/ (* price u350) u10000) tx-sender 'SP7NW9XVEG45SW1367SAHHZG67BFA4GMA9DVGZ0F))
  (try! (stx-transfer? (/ (* price u150) u10000) tx-sender 'SP3KAPN7TWS0H7CHMTPJ2DWWX9AGPJ821TMHGMDGX))
...
(define-data-var slime-cost-per-mint uint u85000000)
...
(begin
  ...
  (try! (set-private-wl 'SP35EWH2ZV6TX3QR65HEXR22HVKZ16NW9ZCAZW1P3 u5))
  (try! (set-private-wl 'SP2DWEVQD0J6R463AW0BEBMR6X197PG5XV5Q8MMT4 u3))
  (try! (set-private-wl 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF u8))
  (try! (set-private-wl 'SP7NW9XVEG45SW1367SAHHZG67BFA4GMA9DVGZ0F u15))
  (ok true)
)
