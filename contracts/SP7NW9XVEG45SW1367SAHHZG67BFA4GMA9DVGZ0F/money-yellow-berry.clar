...
(define-non-fungible-token money-yellow-berry uint)
...
(define-data-var admin (list 1000 principal) (list 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP26K3JHN9YP2QDST5MFC3GYQY6V4Z523AGHANHMF 'SP7NW9XVEG45SW1367SAHHZG67BFA4GMA9DVGZ0F ))
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? money-yellow-berry item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? money-yellow-berry item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? money-yellow-berry item-id)))
      (begin
        (try! (nft-burn? money-yellow-berry item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? money-yellow-berry token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? money-yellow-berry id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? money-yellow-berry id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? money-yellow-berry id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP7NW9XVEG45SW1367SAHHZG67BFA4GMA9DVGZ0F u500 "money-yellow-berry" u500000 u63884 u64292 "ipfs://QmfVPRcPq8FyD8FRSgCErGeyXJptLUBeVhthFp3FcWUHke/money-yellow-berry/1.json"))
  (ok true)
)
