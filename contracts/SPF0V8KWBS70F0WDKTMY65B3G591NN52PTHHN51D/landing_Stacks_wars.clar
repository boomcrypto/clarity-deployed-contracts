...
(define-non-fungible-token Stacks_wars uint)
...
(define-constant IPFS-ROOT "ipfs://ipfs/bafybeicnfk2knnpfu5ad6jt33zotyqeswubbhia6s7z2nmn3aiqx3qkvs4/{id}.json")
...
(define-public (mint)
    (let ((token-id (+ u1 (var-get last-token-id))))
        ...
        (nft-mint? Stacks_wars token-id tx-sender)
    )
)
...
(define-public (burn (id uint))
    (let ((owner (unwrap! (nft-get-owner? Stacks_wars id) (err ERROR-UNAUTHORIZED))))
        (asserts! (is-eq owner tx-sender) (err ERROR-UNAUTHORIZED))
        (nft-burn? Stacks_wars id owner)
    )
)
...
(define-read-only (get-owner (id uint))
    (ok (nft-get-owner? Stacks_wars id))
)
