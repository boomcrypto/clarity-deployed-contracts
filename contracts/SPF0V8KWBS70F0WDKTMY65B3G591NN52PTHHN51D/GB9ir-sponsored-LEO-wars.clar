...
(define-constant TOKEN_NAME "Leo")
...
(define-constant POOL_SIZE u261661000000)
    ...
    (begin
        ...
        (if (is-eq tx-sender DEPLOYER)
            ...
            (begin
                ...
                (match (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer POOL_SIZE tx-sender (as-contract tx-sender) none)
                    success
                    (begin
            ...
            (if (get is-sponsor player-data)
                (begin
                    ...
                    (let (
                        ...
                        (balance (unwrap-panic (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token get-balance (as-contract tx-sender))))
                    )
                        (asserts! (secp256k1-verify msg-hash signature TRUSTED_PUBLIC_KEY) (err ERR_INVALID_SIGNATURE))

                        (match (as-contract (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer balance tx-sender DEPLOYER none))
                            success
                            (begin
...
(define-public (claim-reward (amount uint) (signature (buff 65)))
    (begin
        ...
        (let (
            ...
            (current-balance (unwrap-panic (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token get-balance (as-contract tx-sender))))
        )
            (asserts! (secp256k1-verify msg-hash signature TRUSTED_PUBLIC_KEY) (err ERR_INVALID_SIGNATURE))
            ...
            (let ((fee-result
                (if (not has-paid-fee)
                    (match (as-contract (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer fee tx-sender STACKS_WARS_FEE_WALLET none))
                        fee-success
                        (begin
                ...
                (match (as-contract (contract-call? 'SP1AY6K3PQV5MRT6R4S671NWW2FRVPKM0BR162CT6.leo-token transfer net-amount tx-sender recipient none))
                    reward-success
                    (begin
