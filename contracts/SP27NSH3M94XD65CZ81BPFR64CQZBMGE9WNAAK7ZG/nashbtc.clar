...
;; Variables
(define-fungible-token NASHBTC MAXSUPPLY)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://gaia.hiro.so/hub/14WXFooSrhe2VUGKbtcfjDaDnXxTcsuFtN/ea4f97b3-cc69-4ffa-970c-18d1c4edf4e2-metadata.json"))
(define-data-var token-name (string-ascii 32) "nashbtc")
(define-data-var token-symbol (string-ascii 32) "NASHBTC")
(define-data-var token-decimals uint u6)

;; Read-Only Functions
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance NASHBTC owner))
)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance NASHBTC from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? NASHBTC amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
(begin
  ...
  (let ((result (ft-mint? NASHBTC MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
