...
(define-read-only (get-round-data (bet-type uint) (round uint))
  {
    summary: (map-get? bet-summary-map { bet-type: bet-type, round: round }),
    records: (map get-bet-record LIST_400)
  }
)
...
(define-read-only (get-round-data-v2 (ll (list 400 (list 3 uint))))
  (let
    (
      (first-ele (unwrap-panic (element-at ll u0)))
    )
    {
      summary: (map-get? bet-summary-map { bet-type: (default-to u0 (element-at first-ele u0)), round: (default-to u0 (element-at first-ele u1)) }),
      records: (map get-bet-record-v2 ll)
    }
  )
)

(define-read-only (get-bet-record-v2 (ll (list 3 uint)))
  (map-get? bet-record-map { bet-type: (default-to u0 (element-at ll u0)), round: (default-to u0 (element-at ll u1)), index: (default-to u0 (element-at ll u2)) })
)
