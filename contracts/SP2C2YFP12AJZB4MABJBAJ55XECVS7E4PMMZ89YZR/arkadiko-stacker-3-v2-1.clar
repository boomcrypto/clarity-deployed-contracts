...
;; mainnet pox contract: SP000000000000000000002Q6VF78.pox-2
...
(define-public (initiate-stacking (pox-addr (tuple (version (buff 1)) (hashbytes (buff 32))))
  ...
  (let (
    ...
    (match (as-contract (contract-call? 'SP000000000000000000002Q6VF78.pox-2 can-stack-stx pox-addr tokens-to-stack start-burn-ht lock-period))
      success (begin
        ...
        (match (as-contract (contract-call? 'SP000000000000000000002Q6VF78.pox-2 stack-stx tokens-to-stack pox-addr start-burn-ht lock-period))
          result (begin
            (print result)
...
(define-public (stack-increase (for-stacker (string-ascii 256)) (additional-tokens-to-stack uint))
  (let (
    ...
    (match (as-contract (contract-call? 'SP000000000000000000002Q6VF78.pox-2 stack-increase additional-tokens-to-stack))
      result (begin
        (print result)
...
(define-public (stack-extend (extend-count uint) (pox-addr { version: (buff 1), hashbytes: (buff 32) }))
  (begin
    ...
    (match (as-contract (contract-call? 'SP000000000000000000002Q6VF78.pox-2 stack-extend extend-count pox-addr))
      result (begin
        (print result)
        (var-set stacking-unlock-burn-height (get unlock-burn-height result))
        (try! (contract-call? .arkadiko-freddie-v1-1 set-stacking-unlock-burn-height (var-get stacker-name) (get unlock-burn-height result)))
