...
;; mainnet pox contract: SP000000000000000000002Q6VF78.pox-3 TODO: update
;; https://github.com/stacks-network/stacks-blockchain/blob/next/src/chainstate/stacks/boot/pox-3.clar
...
(define-public (initiate-stacking (pox-addr (tuple (version (buff 1)) (hashbytes (buff 32))))
  ...
  (let (
    ...
    (match (as-contract (contract-call? 'SP000000000000000000002Q6VF78.pox-3 can-stack-stx pox-addr tokens-to-stack start-burn-ht lock-period))
      success (begin
        (try! (contract-call? .arkadiko-stx-reserve-v1-1 request-stx-to-stack (var-get stacker-name) tokens-to-stack))
        (match (as-contract (contract-call? 'SP000000000000000000002Q6VF78.pox-3 stack-stx tokens-to-stack pox-addr start-burn-ht lock-period))
          result (begin
            (print result)
...
(define-public (stack-extend (extend-count uint) (pox-addr { version: (buff 1), hashbytes: (buff 32) }))
  (begin
    ...
    (match (as-contract (contract-call? 'SP000000000000000000002Q6VF78.pox-3 stack-extend extend-count pox-addr))
      result (begin
        (print result)
        (var-set stacking-unlock-burn-height (get unlock-burn-height result))
        (try! (contract-call? .arkadiko-freddie-v1-1 set-stacking-unlock-burn-height (var-get stacker-name) (get unlock-burn-height result)))
