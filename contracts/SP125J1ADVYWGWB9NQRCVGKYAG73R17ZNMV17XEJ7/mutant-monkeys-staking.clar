...
(define-data-var admin principal 'SP125J1ADVYWGWB9NQRCVGKYAG73R17ZNMV17XEJ7)
(define-data-var blocks-per-token uint u3)
(define-data-var staking-fees (list 1000 uint) (list u2000000 u1000000 u2000000))
...
(define-data-var payout-addresses (list 1000 principal) (list 'SP1FGT2GA12XQVFDCBZBVZJTAHX6GSN4P7M8AEPMC 'SP2597NW8VYYVV4C22WQF3DK0WGQS8TAVDDPXQ5H8 'SP1363QMABWXGSRF7535QG440G6H48WBTB6X7DQFM))
...
(define-data-var token-address principal .slime-token)
...
(define-public (stake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (asserts! (is-none (index-of ids item)) (err ERR-ALREADY-STAKED))
    ...
    (begin 
        (if (> (get-staking-fee) u0)
          (begin
            (try! (contract-call? .mutant-staking stake collection lookup-table item))
...
(define-public (unstake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (asserts! (is-some (index-of ids item)) (err ERR-NOT-STAKED))
    ...
    (begin 
        ...
        (if (> (get-unstaking-fee) u0)
          (begin
            (try! (contract-call? .mutant-staking unstake collection lookup-table item))
...
(define-public (admin-stake (owner principal) (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (begin 
        (print points)
        (try! (contract-call? .mutant-staking admin-stake owner collection lookup-table item))
...
(define-public (admin-unstake (collection <nft-trait>) (lookup-table <lookup-trait>) (item uint))
    ...
    (begin 
        (print points)
            (try! (contract-call? .mutant-staking admin-unstake collection lookup-table item))
