...
(define-non-fungible-token neko-gamma uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYtx3okYXNNTRzpVVBz87DEzzkNsyEWoArbyqiM5DBuTC/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? neko-gamma next-id tx-sender) next-id)
      (unwrap! (nft-transfer? neko-gamma next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? neko-gamma token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? neko-gamma token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? neko-gamma token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? neko-gamma id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? neko-gamma id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? neko-gamma id) (err ERR-NOT-FOUND)))
