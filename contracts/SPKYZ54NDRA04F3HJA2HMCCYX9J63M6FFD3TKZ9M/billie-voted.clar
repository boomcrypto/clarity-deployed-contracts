...
(define-non-fungible-token billie-voted uint)
...
(define-data-var total-price uint u2100000)
(define-data-var artist-address principal 'SPKYZ54NDRA04F3HJA2HMCCYX9J63M6FFD3TKZ9M)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRnnjozVH9nFRt3GchrtKEJeEFgNX3yXzL7pZd9dNgQ14/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? billie-voted next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? billie-voted token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? billie-voted token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? billie-voted token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? billie-voted id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? billie-voted id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? billie-voted id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u74300000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u118600000000)
