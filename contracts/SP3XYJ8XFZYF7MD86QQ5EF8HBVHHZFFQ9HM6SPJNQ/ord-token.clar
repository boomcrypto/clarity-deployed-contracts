;; $ORD (https://lasereye.vip/)
;; Total: 21,000,000, Distribution as below:
;; Airdrop: 750,000 (for current 15 members, each get 50,000)
;; Members: 10,000,000 (first 200 members can claim, each get 50,000)
;; BNS: 10,250,000 (first 51,250 names, each get 200. Only available for .btc, .id(length<=2), .stx(length<=2), .app(length<=2), .stacks(length<=2))
...
(define-constant MAX_CLAIM_BNS_COUNT u51250)
...
(define-read-only (get-balance (user principal))
  (ok (ft-get-balance ORD user))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply ORD))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (if (is-eq tx-sender sender)
    (begin
      (try! (ft-transfer? ORD amount sender recipient))
      (print memo)
      (ok true)
    )
    (err u4)
...
(define-public (burn (count uint))
  (ft-burn? ORD count tx-sender)
)

(define-public (get-token-uri)
  (ok (some u"https://ipfs.io/ipfs/Qmd1eLjcpXw9pmWeVMYPXTG2nGPbcnPH64V4Ji7ubGZhWA"))
)
...
(define-public (member_claim)
  (let
    ...
    (ft-mint? ORD CLAIM_COUNT_PER_MEMBER tx-sender)
  )
)
...
(define-public (bns_claim)
  (let
    ...
    (ft-mint? ORD CLAIM_COUNT_PER_BNS tx-sender)
  )
)
...
(define-private (airdrop (tid uint))
  (match (contract-call? .laser-eyes-v5 get_player_by_id tid) user
    (is-ok (ft-mint? ORD AIRDROP_COUNT_PER_MEMBER user))
    false
  )
)

;; Airdrop to early users
(map airdrop (list u1 u2 u3 u4 u5 u6 u7 u8 u9 u10 u11 u12 u13 u14 u15))
