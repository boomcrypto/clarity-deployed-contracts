...
(define-public (like (tid uint))
  (let
    (
      (sender_tid (unwrap! (contract-call? .laser-eyes-v5 get_id_by_player tx-sender) (err ERR_NO_AUTHORITY)))
      (liked_owner (unwrap! (contract-call? .laser-eyes-v5 get_player_by_id tid) (err ERR_TOKEN_NOT_EXIST)))
    ...
    (try! (contract-call? .laser-eyes-v5 deduct (- LIKE_PRICE liked_award)))
    ...
    (contract-call? .laser-eyes-v5 set_ud tid u8 u5 liked_cnt)
  )
)
