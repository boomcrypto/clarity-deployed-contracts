...
(define-public (accept_bid (namespace (buff 20)) (name (buff 48)) (bidder principal))
  (let
    ...
    (if b_host
      (try! (contract-call? .bridge transfer ad_pos namespace name bidder none))
...
(define-public (buy (namespace (buff 20)) (name (buff 48)) (ad_pos uint))
  (let
    ...
    (try! (contract-call? .bridge transfer ad_pos namespace name tx-sender none))
    (map-delete map_name_list { namespace: namespace, name: name })
      ...
      (let
        ...
        (match extra_address addr
          (match (contract-call? 'SP000000000000000000002Q6VF78.bns resolve-principal host_account)
            resolve_host_info (try! (contract-call? .bridge transfer pos (get namespace resolve_host_info) (get name resolve_host_info) addr none))
            err_value false
          )
          false
        )
...
(define-public (cancel_host (namespace (buff 20)) (name (buff 48)) (pos uint) (extra_address (optional principal)))
  (let
    ...
    (try! (contract-call? .bridge transfer pos namespace name (default-to origin_owner extra_address) (some (get zonefile-hash resolve_rsp))))
    (refund_host_fee pos)
...
(define-private (update_ad (i uint))
  ...
  (match (contract-call? .bridge transfer i ns name o z) k (begin (refund_host_fee i) (map-delete map_ad i) (map-delete map_player2adpos o) (map-delete map_name2adpos { namespace: ns, name: name })) e (map-set map_ad i (merge ai { expired_flag: true }))) (begin (is-ok (contract-call? .bridge transfer i ns name o z)) (map-delete map_ad i) (map-delete map_player2adpos o) 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; misc begin ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; When host & origin_owner has DOMAIN(so domain hosted in .bridge contract can't be sent it back) & exceed grace period, domain will left in .bridge contract. Anyone can transfer it to specified address in such situation.
...
(define-public (discard_domain (info { pos: uint, address: principal }))
  (let
    ...
    (try! (contract-call? .bridge transfer ad_pos (get namespace resolve_info) (get name resolve_info) (get address info) none))
    (ok true)
  )
)
