...
(define-constant MINT_PRICE u200000000)
(define-constant UPDATE_PRICE u5000000)
(define-constant LAUNCH_VOTE_PRICE u5000000)
(define-constant MAX_VOTE_DAYS u30)
...
(define-public (create_club (name (buff 25)) (cid (string-ascii 64)) (bio (buff 80)))
  (let
    (
      ...
      (tid (unwrap! (contract-call? .laser-eyes-v4 get_id_by_player sender) (err ERR_ACCOUNT_NOT_ASSOCIATED)))
      (ctid (+ u1 (var-get m_last_id)))
    ...
    (and (> ctid u1) (try! (contract-call? .laser-eyes-v4 deduct MINT_PRICE))) ;; u1 for recovery
    (var-set m_last_id ctid)
...
(define-public (update_summary (name (buff 25)) (cid (string-ascii 64)) (bio (buff 80)))
  (let
    ...
    (try! (contract-call? .laser-eyes-v4 deduct UPDATE_PRICE))
...
(define-public (transfer (receiver_tid uint))
  (let
    (
      (tid (unwrap! (contract-call? .laser-eyes-v4 get_id_by_player contract-caller) (err ERR_ACCOUNT_NOT_ASSOCIATED)))
      (ctid (unwrap! (map-get? map_admin tid) (err ERR_CLUB_NOT_ADMIN)))
      (summary (unwrap! (map-get? map_club_summary ctid) (err ERR_CLUB_NOT_EXIST)))
      (club_event_index (+ (default-to u0 (map-get? map_club_event_count ctid)) u1))
    )
    (asserts! (is-some (contract-call? .laser-eyes-v4 get_player_by_id receiver_tid)) (err ERR_MEMBER_NOT_EXIST))
...
(define-public (club_invite (tid uint))
  (let
    (
      (sender_tid (unwrap! (contract-call? .laser-eyes-v4 get_id_by_player contract-caller) (err ERR_ACCOUNT_NOT_ASSOCIATED)))
    ...
    (asserts! (is-some (contract-call? .laser-eyes-v4 get_player_by_id tid)) (err ERR_MEMBER_NOT_EXIST))
    (asserts! (is-none (map-get? map_member { tid: tid, ctid: ctid })) (err ERR_MEMBER_ALREADY_IN_CLUB))
...
(define-public (accept_club_invite (ctid uint))
  (match (contract-call? .laser-eyes-v4 get_id_by_player contract-caller) tid
...
(define-public (launch_vote (ctid uint) (days uint) (title (buff 100)) (choices (list 4 (buff 25))))
  (let
    ...
    (asserts! (is-some (map-get? map_member { tid: tid, ctid: ctid })) (err ERR_CLUB_NO_SUCH_MEMBER))
    (asserts! (and (>= days u1) (<= days MAX_VOTE_DAYS)) (err ERR_VOTE_DAYS_INVALID))
    ...
    (or
      ...
      (try! (contract-call? .laser-eyes-v4 deduct LAUNCH_VOTE_PRICE))
    )
...
(define-read-only (get_member_cb (player principal))
  (let
    (
      (tid (default-to u0 (contract-call? .laser-eyes-v4 get_id_by_player player)))
...
(define-read-only (get_club_cb (ctid uint) (player (optional principal)))
  (let
    (
      (tid (if (is-none player) u0 (default-to u0 (contract-call? .laser-eyes-v4 get_id_by_player (unwrap-panic player)))))
