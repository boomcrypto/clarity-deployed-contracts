...
(define-public (create_club (name (buff 25)) (cid (string-ascii 64)) (bio (buff 80)))
  (let
    (
      ...
      (tid (unwrap! (contract-call? .laser-eyes-v5 get_id_by_player sender) (err ERR_ACCOUNT_NOT_ASSOCIATED)))
    ...
    (and (> ctid u1) (try! (contract-call? .laser-eyes-v5 deduct MINT_PRICE))) ;; u1 for recovery
...
(define-public (update_summary (name (buff 25)) (cid (string-ascii 64)) (bio (buff 80)))
  (let
    ...
    (try! (contract-call? .laser-eyes-v5 deduct UPDATE_PRICE))
...
(define-public (transfer (receiver_tid uint))
  (let
    ...
    (asserts! (is-some (contract-call? .laser-eyes-v5 get_player_by_id receiver_tid)) (err ERR_MEMBER_NOT_EXIST))
...
(define-public (club_invite (tid uint))
  (let
    (
      (sender_tid (unwrap! (contract-call? .laser-eyes-v5 get_id_by_player contract-caller) (err ERR_ACCOUNT_NOT_ASSOCIATED)))
    ...
    (asserts! (is-some (contract-call? .laser-eyes-v5 get_player_by_id tid)) (err ERR_MEMBER_NOT_EXIST))
    (asserts! (is-none (map-get? map_member { tid: tid, ctid: ctid })) (err ERR_MEMBER_ALREADY_IN_CLUB))
...
(define-public (accept_club_invite (ctid uint))
  (match (contract-call? .laser-eyes-v5 get_id_by_player contract-caller) tid
...
(define-public (launch_vote (ctid uint) (days uint) (title (buff 100)) (choices (list 4 (buff 25))))
  (let
    ...
    (or
      ...
      (try! (contract-call? .laser-eyes-v5 deduct LAUNCH_VOTE_PRICE))
    )
...
(define-read-only (get_member_cb (player principal))
  (let
    (
      (tid (default-to u0 (contract-call? .laser-eyes-v5 get_id_by_player player)))
...
(define-read-only (get_club_cb (ctid uint) (player (optional principal)))
  (let
    (
      (tid (if (is-none player) u0 (default-to u0 (contract-call? .laser-eyes-v5 get_id_by_player (unwrap-panic player)))))
