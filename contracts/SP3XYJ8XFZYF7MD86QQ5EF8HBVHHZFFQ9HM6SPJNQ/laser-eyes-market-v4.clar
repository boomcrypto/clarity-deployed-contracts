...
(define-public (list_nft (tid uint) (price uint))
  (let
    (
      (sys_owner (unwrap! (unwrap-panic (contract-call? .laser-eyes-v4 get-owner tid)) (err ERR_TOKEN_NOT_EXIST)))
      (caller tx-sender)
    )
    ...
    (stat_list tid price)
    (contract-call? .laser-eyes-v4 transfer_by_market_list tid sys_owner (as-contract tx-sender))
  )
)
...
(define-public (cancel_list (tid uint))
  (let
    ...
    (stat_list tid u0)
    (contract-call? .laser-eyes-v4 transfer_by_market_list tid (as-contract tx-sender) owner)
  )
)
...
(define-public (accept_bid (tid uint) (bidder principal))
  (let
    (
      (owner (unwrap! (contract-call? .laser-eyes-v4 get_player_by_id tid) (err ERR_TOKEN_NOT_EXIST)))
    ...
    (asserts! (is-none (contract-call? .laser-eyes-v4 get_id_by_player bidder)) (err ERR_ACCOUNT_ALREADY_ASSOCIATED))
    ...
    (try! (as-contract (contract-call? .laser-eyes-v4 deduct fee)))
    ;;
    (if b_list
      (try! (contract-call? .laser-eyes-v4 transfer_by_market_trade tid tx-sender bidder))
      (try! (contract-call? .laser-eyes-v4 transfer tid tx-sender bidder))
    )
...
(define-public (buy (tid uint))
  (let
    ...
    (asserts! (is-none (contract-call? .laser-eyes-v4 get_id_by_player caller)) (err ERR_ACCOUNT_ALREADY_ASSOCIATED))
    ...
    (try! (contract-call? .laser-eyes-v4 deduct fee))
    ;;
    (try! (contract-call? .laser-eyes-v4 transfer_by_market_trade tid (get owner list_info) caller))
    ;;
    (map-delete map_list_info tid)
...
(define-public (bid (tid uint) (price uint) (days uint))
  (let
    ...
    (asserts! (is-some (contract-call? .laser-eyes-v4 get_player_by_id tid)) (err ERR_TOKEN_NOT_EXIST))
    (asserts! (is-none (contract-call? .laser-eyes-v4 get_id_by_player tx-sender)) (err ERR_ACCOUNT_ALREADY_ASSOCIATED))
    (asserts! (and (>= price MIN_PRICE) (<= price MAX_PRICE)) (err ERR_PRICE_INVALID))
...
(define-read-only (get_summary (player_opt (optional principal)))
  (match player_opt player
    (let
      (
        (resolve_info (contract-call? .laser-eyes-v4 resolve_player player))
...
(define-read-only (get_token_brief (tid uint))
  (let
    (
      (info (contract-call? .laser-eyes-v4 resolve_tid tid))
      (ud (get ud info))
    )
    {
      bh: block-height,
...
(define-read-only (get_pre_act_info (player principal))
  {
    tid: (contract-call? .laser-eyes-v4 get_id_by_player player),
    deposit: (default-to u0 (map-get? map_bidder_deposit player)),
  }
)
          ...
          (if (> block-height (get expiration bi))
            ...
            (if (> (get up ud) u0) ud
              (if (< (default-to u0 (map-get? map_bidder_deposit (get player bi))) (get price bi))
                ...
                (if (> (get ap ud) u0)
                  ud
                  (if (is-some (contract-call? .laser-eyes-v4 get_id_by_player (get player bi)))
                    (merge ud { ap: i }) 
