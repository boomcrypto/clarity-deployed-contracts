...
(define-read-only (get-name) (contract-call? 'SPRZX4M2HHMJQCYZZHY3QTA8TRCNJVNXG8EPEV4C.doggo get-name))
(define-read-only (get-symbol) (contract-call? 'SPRZX4M2HHMJQCYZZHY3QTA8TRCNJVNXG8EPEV4C.doggo get-symbol))
(define-read-only (get-base-decimals) (contract-call? 'SPRZX4M2HHMJQCYZZHY3QTA8TRCNJVNXG8EPEV4C.doggo get-decimals))
(define-read-only (get-token-uri) (contract-call? 'SPRZX4M2HHMJQCYZZHY3QTA8TRCNJVNXG8EPEV4C.doggo get-token-uri))
(define-read-only (get-balance (who principal)) (if (is-eq who VAULT) (ok (ft-get-balance wrapped-token who)) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SPRZX4M2HHMJQCYZZHY3QTA8TRCNJVNXG8EPEV4C.doggo get-balance who))))))
(define-read-only (get-total-supply) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SPRZX4M2HHMJQCYZZHY3QTA8TRCNJVNXG8EPEV4C.doggo get-total-supply)))))
...
(define-read-only (get-base-token)
  'SPRZX4M2HHMJQCYZZHY3QTA8TRCNJVNXG8EPEV4C.doggo)
  ...
  (begin
    ...
    (if (is-eq sender VAULT)
			(begin
				...
				(as-contract (contract-call? 'SPRZX4M2HHMJQCYZZHY3QTA8TRCNJVNXG8EPEV4C.doggo transfer (fixed-to-decimals amount) tx-sender recipient memo)))
			(begin
				(and (is-eq recipient VAULT) (try! (ft-mint? wrapped-token amount recipient)))
				(contract-call? 'SPRZX4M2HHMJQCYZZHY3QTA8TRCNJVNXG8EPEV4C.doggo transfer (fixed-to-decimals amount) tx-sender (if (is-eq recipient VAULT) (as-contract tx-sender) recipient) memo)))))
