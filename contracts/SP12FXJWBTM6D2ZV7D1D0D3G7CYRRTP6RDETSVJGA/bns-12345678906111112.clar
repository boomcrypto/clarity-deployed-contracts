...
;;         (ok u0);;(ok (get dy (try! (swap-x-for-y token-x-trait token-y-trait factor dx min-dy))))
;;         (ok u1);;(ok (get dx (try! (swap-y-for-x token-y-trait token-x-trait factor dx min-dy))))
;;     )
;; )
...
(define-read-only (get-x-given-y-call (balance-x uint) (balance-y uint) (t uint) (dy uint))
    (if (>= t (var-get switch-threshold))
    (let
        (
            (t-comp (if (<= ONE_8 t) u0 (- ONE_8 t)))
        )        
        (ok (div-down (+ (mul-down t-comp dy) (mul-down t (mul-down dy balance-x))) (+ t-comp (mul-down t (+ balance-y dy)))))
    )  
    (let 
        (          
            (t-comp (if (<= ONE_8 t) u0 (- ONE_8 t)))
        ...
        (if (<= balance-x final-term) (ok u0) (ok (- balance-x final-term)))
    )
    )    
)
(define-read-only (get-y-given-x-call (balance-x uint) (balance-y uint) (t uint) (dx uint))
    (if (>= t (var-get switch-threshold))
    (let
        (
            (t-comp (if (<= ONE_8 t) u0 (- ONE_8 t)))
        )        
        (ok (div-down (+ (mul-down t-comp dx) (mul-down t (mul-down dx balance-y))) (+ t-comp (mul-down t (+ balance-x dx)))))
    )
    (let
        (
            (t-comp (if (<= ONE_8 t) u0 (- ONE_8 t)))
        ...
        (if (<= balance-y final-term) (ok u0) (ok (- balance-y final-term)))
    )  
    )      
)
