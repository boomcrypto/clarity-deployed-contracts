...
(define-non-fungible-token the-littles uint)
...
(define-data-var artist-address principal 'SP148Q6KES9YGFVZ1BJ9HSCVHAB91D3JRTBBF7E33)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmefvxDsmBVFwCt32iA5eAbN4fxUfxNfNtwtENkDKfKGjH/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? the-littles next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? the-littles token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? the-littles token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-littles token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-littles id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-littles id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-littles id) (err ERR-NOT-FOUND)))
