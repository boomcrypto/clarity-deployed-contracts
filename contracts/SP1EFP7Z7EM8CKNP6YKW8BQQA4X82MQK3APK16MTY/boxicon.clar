...
(define-non-fungible-token boxicon uint)
...
(define-data-var artist-address principal 'SP1EFP7Z7EM8CKNP6YKW8BQQA4X82MQK3APK16MTY)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUxqg45M31ZEmhozDXHVaAVmmJxFveH5CnvcQfkXR9Pzz/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? boxicon next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? boxicon token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? boxicon token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? boxicon token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? boxicon id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? boxicon id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? boxicon id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u28364)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u34355000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u31766000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u250000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u6080000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u18300000000)
