...
(define-non-fungible-token abc-dao uint)
...
(define-data-var artist-address principal 'SPB6VSZRH1518CGAK5B9RS0AY60105K18K6S3MD5)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWfZRxevDNB3Q9UaWyVLqJbZPeALbAAkRJqM6Vmfa3DHW/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? abc-dao next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? abc-dao token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? abc-dao token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? abc-dao token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? abc-dao id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? abc-dao id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? abc-dao id) (err ERR-NOT-FOUND)))
