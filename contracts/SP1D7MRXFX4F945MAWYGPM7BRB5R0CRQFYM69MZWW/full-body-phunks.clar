...
(define-non-fungible-token full-body-phunks uint)
...
(define-data-var artist-address principal 'SP1D7MRXFX4F945MAWYGPM7BRB5R0CRQFYM69MZWW)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmP1SENRRpCR6UHaSokNs6sPS6VzCTF8ZSwZMRgiLyGJsZ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? full-body-phunks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? full-body-phunks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? full-body-phunks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? full-body-phunks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? full-body-phunks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? full-body-phunks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? full-body-phunks id) (err ERR-NOT-FOUND)))
