...
(define-non-fungible-token gepard uint)
...
(define-data-var artist-address principal 'SP21FTV3Z1ZYSQQJK9QZXJ1YZNRV0SE3FNW22TAPS)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qma4N6Lt8zxahWEvQv8nDDo4RCEvpGXKgpMCgKCxUdjbrE/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? gepard next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gepard token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gepard token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gepard token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gepard id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gepard id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gepard id) (err ERR-NOT-FOUND)))
