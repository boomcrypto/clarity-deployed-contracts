...
(define-non-fungible-token cipher-punks uint)
...
(define-data-var artist-address principal 'SP1B83AD6MRE6QQ2FB1P0NZT72PXDRBDRT9TQ450Y)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmX2GfAwScmoTWPmeG4c6pVwSmHUwZHLNBmcxozPEFsGBZ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? cipher-punks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? cipher-punks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? cipher-punks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? cipher-punks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? cipher-punks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? cipher-punks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? cipher-punks id) (err ERR-NOT-FOUND)))
