;; alex-anniversary-series
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token alex-anniversary-series uint)
...
(define-data-var artist-address principal 'SP3N7Y3K01Y24G9JC1XXA13RQXXCY721WAVBMMD38)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWRQyaVxUjHGjBUoZqGcNjL37VN99jcFwmoB1wZnpjJEg/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? alex-anniversary-series next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? alex-anniversary-series token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? alex-anniversary-series token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? alex-anniversary-series token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? alex-anniversary-series id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? alex-anniversary-series id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? alex-anniversary-series id) (err ERR-NOT-FOUND)))
