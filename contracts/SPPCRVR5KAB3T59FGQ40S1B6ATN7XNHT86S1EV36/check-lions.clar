...
(define-non-fungible-token check-lions uint)
...
(define-data-var artist-address principal 'SPPCRVR5KAB3T59FGQ40S1B6ATN7XNHT86S1EV36)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQEVkWkFQfXQjds33G3fshptyXZnJubSUbnBCzNukrG96/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? check-lions next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? check-lions token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? check-lions token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? check-lions token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? check-lions id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? check-lions id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? check-lions id) (err ERR-NOT-FOUND)))
