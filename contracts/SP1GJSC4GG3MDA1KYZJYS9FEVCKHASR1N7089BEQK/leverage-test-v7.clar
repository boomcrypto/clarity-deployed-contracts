...
(define-public (on-granite-flash-loan (amount uint) (fee uint) (data (optional (buff 20480))))
  (let (
    ...
    (try! (as-contract (swap-aeusdc-to-sbtc-bitflow amount min-sbtc-out)))
    (try! (leverage-helper pyth-price-feed-data borrow-amount min-sbtc-out user))
    (try! (repay-flash-loan repay-amount user sender))
    SUCCESS
  )
)

(define-private (repay-flash-loan (repay-amount uint) (user principal) (sender principal))
  (begin
    (try! (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer repay-amount user sender none))
    ;;(try! (as-contract (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer repay-amount user sender none)))
    SUCCESS
  )
)
