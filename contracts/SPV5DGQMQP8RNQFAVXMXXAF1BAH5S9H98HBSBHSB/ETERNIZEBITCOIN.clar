...
(define-non-fungible-token ETERNIZEBITCOIN uint)
...
(define-data-var base-uri (string-ascii 80) "bafybeiai7ivx4ylw3mitaffh4w6fo4absrhuw7ftpwq4pg3cnvakza3g24/")
(define-data-var mint-price uint u7000000)
;; Percent u100 = 1%
(define-data-var royalty-percent uint u700)
(define-data-var mint-limit uint u3)
(define-data-var max-supply uint u80)
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? ETERNIZEBITCOIN next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? ETERNIZEBITCOIN next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ETERNIZEBITCOIN id sender recipient)
        success
          (let
            (
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? ETERNIZEBITCOIN id))
)
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? ETERNIZEBITCOIN id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? ETERNIZEBITCOIN id) ERR_NO_OWNER))
