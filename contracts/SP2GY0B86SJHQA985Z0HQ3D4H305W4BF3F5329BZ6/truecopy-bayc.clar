...
(define-non-fungible-token truecopy-bayc uint)
...
(define-data-var artist-address principal 'SP2GY0B86SJHQA985Z0HQ3D4H305W4BF3F5329BZ6)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmabeZPSgQjDm7txVwsLKcVmPGt6GV3hsoMtCHRLzpRHuB/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? truecopy-bayc next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? truecopy-bayc token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? truecopy-bayc token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? truecopy-bayc token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? truecopy-bayc id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? truecopy-bayc id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? truecopy-bayc id) (err ERR-NOT-FOUND)))
