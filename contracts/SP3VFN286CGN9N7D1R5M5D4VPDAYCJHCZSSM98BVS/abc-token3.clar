...
(define-fungible-token abc-token)

;; get the token balance of owner
(define-read-only (get-balance (owner principal))
  (begin
    (ok (ft-get-balance abc-token owner))))

;; returns the total number of tokens
(define-read-only (get-total-supply)
  (ok (ft-get-supply abc-token)))
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (if (is-eq tx-sender sender)
    (begin
      (try! (ft-transfer? abc-token amount sender recipient))
      (print memo)
      (ok true)
    )
    (err u4)))
...
(define-public (get-token-uri)
  (ok (some u"https://sbp-public.s3.amazonaws.com/json/abc.json")))
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    ...
    (ft-mint? abc-token amount recipient)
  )
)

;; Mint this token to a few people when deployed
(ft-mint? abc-token u100000000 'SP3VFN286CGN9N7D1R5M5D4VPDAYCJHCZSSM98BVS)