...
(define-read-only (get-balance (user principal))
  (ok (ft-get-balance Mojo user))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply Mojo))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (if (is-eq tx-sender sender)
    (begin
      (try! (ft-transfer? Mojo amount sender recipient))
      (print memo)
      (ok true)
    )
    (err u4)
...
(define-public (member_claim)
  (let
    ...
    (ft-mint? Mojo CLAIM_COUNT_PER_MEMBER tx-sender)
  )
)
...
(define-public (bns_claim)
  (let
    ...
    (ft-mint? Mojo CLAIM_COUNT_PER_BNS tx-sender)
  )
)

(define-private (s (receiver { to: principal, amount: uint }))
  (is-err (ft-transfer? Mojo (get amount receiver) tx-sender (get to receiver)))
)
...
(define-private (airdrop (tid uint))
  (match (contract-call? 'SP3XYJ8XFZYF7MD86QQ5EF8HBVHHZFFQ9HM6SPJNQ.laser-eyes-v5 get_player_by_id tid) user
    (is-ok (ft-mint? Mojo AIRDROP_COUNT_PER_MEMBER user))
    false
  )
)
...
(ft-mint? Mojo u230000000000000 tx-sender)
