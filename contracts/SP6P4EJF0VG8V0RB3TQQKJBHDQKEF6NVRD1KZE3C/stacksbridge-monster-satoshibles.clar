;; StacksBridge - Monster Satoshibles
...
(define-public (lock (id uint) (address (string-ascii 42)))
    (let (
        (nft-owner (unwrap-panic (unwrap! (contract-call? .monster-satoshibles get-owner id) ERR-NOT-FOUND)))
        )
        (asserts! (not (var-get bridge-closed)) ERR-BRIDGE-CLOSED)
        (asserts! (is-eq tx-sender nft-owner) ERR-NOT-AUTHORIZED)       
        (asserts! (or (< id u3306) (and (> id u666000) (< id u666667))) ERR-INVALID-ID)
        (try! (stx-transfer? (var-get escrow-fee) tx-sender (as-contract tx-sender)))
        (try! (contract-call? .monster-satoshibles transfer id tx-sender (as-contract tx-sender)))
...
(define-public (release (id uint) (address principal))
    (begin
        ...
        (try! (as-contract (contract-call? .monster-satoshibles transfer id tx-sender address)))
