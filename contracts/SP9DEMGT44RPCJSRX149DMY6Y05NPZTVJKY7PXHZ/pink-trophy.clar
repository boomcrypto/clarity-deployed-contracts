...
(define-non-fungible-token pink-trophy uint)
...
(define-data-var base-uri (string-ascii 80) "bafybeidply2twth6y6v5yumb7zh2m36jda36zle2froxvu4dilgbop3z3y/")
(define-data-var mint-price uint u1000000)
;; Percent u100 = 1%
(define-data-var royalty-percent uint u250)
(define-data-var mint-limit uint u1)
(define-data-var max-supply uint u49)
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? pink-trophy next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? pink-trophy next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pink-trophy id sender recipient)
        success
          (let
            (
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? pink-trophy id))
)
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? pink-trophy id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? pink-trophy id) ERR_NO_OWNER))
