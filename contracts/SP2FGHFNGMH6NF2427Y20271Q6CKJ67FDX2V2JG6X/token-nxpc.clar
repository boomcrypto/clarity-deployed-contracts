...
(define-fungible-token token-nxpc)

(define-data-var token-name (string-ascii 32) "MapleStory Universe")
(define-data-var token-symbol (string-ascii 10) "NXPC")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.brotocol.xyz/metadata/token-nxpc.json"))
...
(define-public (transfer
	...
	(begin
		...
		(try! (ft-transfer? token-nxpc amount sender recipient))
		(match memo
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance token-nxpc who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply token-nxpc))
)
...
(define-public (mint
	...
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? token-nxpc amount recipient)
	)
)
...
(define-public (burn
	...
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? token-nxpc amount sender)
	)
)
