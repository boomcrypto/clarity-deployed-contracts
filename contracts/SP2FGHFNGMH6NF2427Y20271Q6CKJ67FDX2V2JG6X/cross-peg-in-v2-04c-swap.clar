...
(define-read-only (validate-cross-swap-order 
  ...
  (let (
    ...
    (asserts! (or (is-eq (get token-in order) routing-token-0) (is-eq (try! (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.cross-router-v2-03 get-approved-wrapped-or-fail (get token-in order))) routing-token-0)) ERR-INVALID-TOKEN)
    (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.cross-router-v2-03 validate-route (get amount-in-fixed order) (get routing-tokens order) (get routing-factors order) (get token-out order) (get min-amount-out-fixed order) { address: (get to order), chain-id: (get dest-chain-id order) })))
...
(define-public (transfer-to-cross-swap
    ...
    (let (
      ...
      ;; @dev TODO peg-out-gas-fee is hard-coded to zero for Runes/BRC20
      (and (or (is-eq (get chain-id pair-tuple) (some u1001)) (is-eq (get chain-id pair-tuple) (some u1002))) 
        (try! (update-peg-out-fee pair-tuple (get peg-out-fee default-fee-tuple) u0)))
      (match (validate-cross-swap-order order token-in-trait routing-traits token-out-trait)
        ok-value
        (begin          
          (as-contract (try! (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.cross-router-v2-03 route (get amount-in-fixed order) routing-traits (get routing-factors order) token-out-trait (get min-amount-out-fixed order) { address: (get to order), chain-id: (get dest-chain-id order) })))          
