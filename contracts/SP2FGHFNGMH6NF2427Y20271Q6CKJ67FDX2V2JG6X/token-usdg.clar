...
(define-fungible-token usdg)

(define-data-var token-name (string-ascii 32) "USDG")
(define-data-var token-symbol (string-ascii 32) "USDG")
...
(define-read-only (get-base-decimals)
  (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt get-decimals))

(define-read-only (get-balance (who principal))
  (ok (/ (* (unwrap-panic (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt get-balance who)) (pow-decimals)) (pow u10 (unwrap-panic (get-base-decimals))))))

(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    (asserts! (is-eq sender tx-sender) ERR-NOT-AUTHORIZED)
    (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt transfer (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender recipient memo)))

(define-public (mint (amount uint) (recipient principal))
  (begin
    (try! (is-dao-or-extension))
    (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt mint (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) recipient)))

(define-public (burn (amount uint) (sender principal))
  (begin
    (try! (is-dao-or-extension))
    (contract-call? 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-susdt burn (/ (* amount (pow u10 (unwrap-panic (get-base-decimals)))) (pow-decimals)) sender)))
;; ------
