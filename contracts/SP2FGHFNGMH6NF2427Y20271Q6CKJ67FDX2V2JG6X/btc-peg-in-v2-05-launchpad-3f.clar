...
(define-public (finalize-peg-in-launchpad
  ...
  (let (
    ...
    (match (validate-tx-launchpad-extra validation-data)
      ok-value
      (let (
        	(bounds (as-contract (try! (contract-call? 'SP1KK89R86W73SJE6RQNQPRDM471008S9JY4FQA62.alex-launchpad-v2-03f register { address: (get user order-details), chain-id: (some u0) } (get dest order-details) (get launch-id order-details) (get amount-net validation-data) 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-abtc)))))
...
(define-private (validate-tx-launchpad-extra (validation-data { order-details: { user: (buff 128), launch-id: uint, dest: (buff 128) }, fee: uint, amount-net: uint }))
  (let (
      (order-details (get order-details validation-data)))
    (try! (contract-call? 'SP1KK89R86W73SJE6RQNQPRDM471008S9JY4FQA62.alex-launchpad-v2-03f validate-register { address: (get user order-details), chain-id: (some u0) } (get dest order-details) (get launch-id order-details) (get amount-net validation-data) 'SP2XD7417HGPRTREMKF748VNEQPDRR0RMANB7X1NK.token-abtc))
    (ok validation-data)))
