...
(define-fungible-token token-avax)

(define-data-var token-name (string-ascii 32) "AVAX")
(define-data-var token-symbol (string-ascii 10) "AVAX")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.brotocol.xyz/metadata/token-avax.json"))
...
(define-public (transfer
	...
	(begin
		...
		(try! (ft-transfer? token-avax amount sender recipient))
		(match memo
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance token-avax who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply token-avax))
)
...
(define-public (mint
	...
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? token-avax amount recipient)
	)
)
...
(define-public (burn
	...
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? token-avax amount sender)
	)
)
