...
(define-fungible-token token-spx6900)

(define-data-var token-name (string-ascii 32) "SPX6900")
(define-data-var token-symbol (string-ascii 10) "SPX6900")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://cdn.brotocol.xyz/metadata/token-spx6900.json"))
...
(define-public (transfer
	...
	(begin
		...
		(try! (ft-transfer? token-spx6900 amount sender recipient))
		(match memo
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance token-spx6900 who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply token-spx6900))
)
...
(define-public (mint
	...
	(begin
		(try! (is-dao-or-extension))
		(ft-mint? token-spx6900 amount recipient)
	)
)
...
(define-public (burn
	...
	(begin
		(try! (is-dao-or-extension))
		(ft-burn? token-spx6900 amount sender)
	)
)
