...
(define-non-fungible-token Punks-Army-Celebrities uint)
...
(define-private (mint_punk (punk-id uint) (new-owner principal))
  (begin
    ...
    (nft-mint? Punks-Army-Celebrities punk-id new-owner)
  )
)
...
(define-public 
  ...
  (begin     
    ...
    (nft-transfer? Punks-Army-Celebrities token-id sender recipient))
)
;; BURN A TOKEN
(define-public 
  (burn-token 
  ...
  (begin     
    (asserts! (is-eq (some tx-sender) (nft-get-owner? Punks-Army-Celebrities token-id) ) (err u403)) 
    (asserts! (is-none (map-get? delegated-nft token-id)) (err u401))    
    (nft-burn? Punks-Army-Celebrities token-id tx-sender))
)

(
  define-read-only (get-owner (punk-id uint))  
  (ok (nft-get-owner? Punks-Army-Celebrities punk-id))
)
...
(define-public (delegate-nft (token-id uint) (sender principal) (recipient principal) )
  (begin
    (asserts! (is-eq tx-sender sender) (err u403))  
    (asserts! (is-eq tx-sender (unwrap-panic (nft-get-owner? Punks-Army-Celebrities token-id)) ) (err u401))  
