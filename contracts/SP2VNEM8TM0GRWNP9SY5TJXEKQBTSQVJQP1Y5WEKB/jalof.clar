(impl-trait 'SP20X3DC5R091J8B6YPQT638J8NR1W83KN6TN5BJY.ft-trait.ft-trait)
  ...
  (define-constant MAX_SUPPLY (* u10000000000 (pow u10 u8)))

  (define-fungible-token jalof MAX_SUPPLY)
  ...
  (define-public
    ...
    (begin
      ...
      (ft-transfer? jalof amt from to)))


  (define-public (mint (amt uint) (to principal))
    (begin
      (try! (check-owner))
      (ft-mint? jalof amt to) ))

  (define-read-only (get-name)                   (ok "jalof"))
  (define-read-only (get-symbol)                 (ok "jalo"))
  (define-read-only (get-decimals)               (ok u8))
  (define-read-only (get-balance (of principal)) (ok (ft-get-balance jalof of)))
  (define-read-only (get-total-supply)           (ok (ft-get-supply jalof)))
