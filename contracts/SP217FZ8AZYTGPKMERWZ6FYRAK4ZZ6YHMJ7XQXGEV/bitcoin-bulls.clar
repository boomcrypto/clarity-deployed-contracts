...
(define-data-var total-spoints-price uint u2100000000)
(define-data-var artist-address principal 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/bafybeidbft7hu7xhtxomvv6m4rhj5sjpaplxa6anjnq53j6yes6e6l46hm/")
...
(define-private (spoint-mint (spc-id uint) (orders (list 2 bool)))
  (let 
    ...
    (unwrap-panic (contract-call? .spoints spend spc-id price))
    (ok id-reached)))
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? Bitcoin-Bulls next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? Bitcoin-Bulls token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? Bitcoin-Bulls token-id) false)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Bitcoin-Bulls id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? Bitcoin-Bulls id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? Bitcoin-Bulls id) (err ERR-NOT-FOUND)))
