    ...
    (let (
        ...
        (if (is-eq amount u0) 
            ...
            (ok (if (< amount to-collect) 
                (begin (unwrap-panic (contract-call? subscriber collect item amount .spoints)) (var-set current-collect u0)) 
                (begin (unwrap-panic (contract-call? subscriber collect item to-collect .spoints)) (var-set current-collect (- amount to-collect)))
            )))))
