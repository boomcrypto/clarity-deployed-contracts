;;Contract that enables the subscription of nfts in collection SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.bitcoin-whales to gain spoints.
...
(define-constant collection-address 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.bitcoin-whales)
;;maps
...
(define-public (subscribe (lookup-table <lookup-trait>) (item uint)) 
    (let (
        ...
        (asserts! (is-eq (unwrap-panic (unwrap-panic (contract-call? 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.bitcoin-whales get-owner item))) tx-sender) (err ERR-NOT-AUTHORIZED))
...
(define-public (collect (spc-id uint) (amount uint) (points <points-trait>))
  (let (
    ...
    (begin      
      (map-set subscription-per-address { address: tx-sender} {time: tenure-height, points-balance: (- to-collect amount), total-multiplier: total-multiplier })
