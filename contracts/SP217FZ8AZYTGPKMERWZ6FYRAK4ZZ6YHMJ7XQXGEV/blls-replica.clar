...
(define-private (subscription-transfer (id uint)) 
    (let (
        (subscriber (unwrap-panic (contract-call? 'SP1N057R0S5BBAQKTW0GF2J7BXKFHY2GQV9MP2BEN.bitcoin-bulls-spoints-subscriber get-item-subscriber id)))
    )
    (asserts! (is-eq tx-sender admin) ERR-NOT-AUTHORIZED)
    (if (not (or (is-none subscriber) (get-id-checked id))) 
        (begin
            (try! (contract-call? .bitcoin-bulls-spoints-subscriber admin-subscribe 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.bull-multipliers id (unwrap-panic subscriber))) 
            (map-set id-checked id true)
            (if (not (get-address-checked (unwrap-panic subscriber)))
                (begin  
                    (try! (contract-call? .bitcoin-bulls-spoints-subscriber allocate-balance (unwrap-panic (contract-call? 'SP1N057R0S5BBAQKTW0GF2J7BXKFHY2GQV9MP2BEN.bitcoin-bulls-spoints-subscriber get-collect (unwrap-panic subscriber))) (unwrap-panic subscriber)))
