...
(define-public (spaghettipunk-club-claim-one-with-referral (ref-code (string-ascii 30)))
  (let (
    ...
    (total-reward (contract-call? .tame-moccasin-moose get-total-reward 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.spaghettipunk-club))
    (spoint-reward (contract-call? .tame-moccasin-moose get-spoint-referral-bonus-reward))
    (mints u1)
    ...
    (if (> spoint-reward u0)
      (begin
        (try! (contract-call? .tame-moccasin-moose handout-referral-reward ref-code 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.spaghettipunk-club mints (some (+ last-id u1))))
        (print { action: "claim-with-referral", code: ref-code, address: some-ref-addr, mints: mints, reward: reward, spoint-reward: spoint })
      )
      (begin
        (try! (contract-call? .tame-moccasin-moose handout-referral-reward ref-code 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.spaghettipunk-club mints none))
...
(define-public (bitcoin-bears-claim-one-with-referral (ref-code (string-ascii 30)))
  (let (
    (some-ref-addr (unwrap! (contract-call? .balanced-harlequin-ocelot get-referral-address ref-code) (err ERR_NOT_VALID_REFERRAL)))
    (total-reward (contract-call? .tame-moccasin-moose get-total-reward 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.bitcoin-bears))
    (mints u1)
    (reward (* mints total-reward))
  )
    (asserts! (is-eq (validate-referral ref-code) (ok true)) (err ERR_NOT_VALID_REFERRAL))
    (try! (contract-call? 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.bitcoin-bears claim))
    (try! (contract-call? .tame-moccasin-moose handout-referral-reward ref-code 'SP1C2K603TGWJGKPT2Z3WWHA0ARM66D352385TTWH.bitcoin-bears mints none))
