...
(define-read-only (get-name) (contract-call? 'SP15ZNQ9FT2YJFNXQQHC16B1VAXHAPQ1H5HA5VQ2T.buildon get-name))
(define-read-only (get-symbol) (contract-call? 'SP15ZNQ9FT2YJFNXQQHC16B1VAXHAPQ1H5HA5VQ2T.buildon get-symbol))
(define-read-only (get-base-decimals) (contract-call? 'SP15ZNQ9FT2YJFNXQQHC16B1VAXHAPQ1H5HA5VQ2T.buildon get-decimals))
(define-read-only (get-token-uri) (contract-call? 'SP15ZNQ9FT2YJFNXQQHC16B1VAXHAPQ1H5HA5VQ2T.buildon get-token-uri))
(define-read-only (get-balance (who principal)) (if (is-eq who VAULT) (ok (ft-get-balance wrapped-token who)) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SP15ZNQ9FT2YJFNXQQHC16B1VAXHAPQ1H5HA5VQ2T.buildon get-balance who))))))
(define-read-only (get-total-supply) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SP15ZNQ9FT2YJFNXQQHC16B1VAXHAPQ1H5HA5VQ2T.buildon get-total-supply)))))
...
(define-read-only (get-base-token)
  'SP15ZNQ9FT2YJFNXQQHC16B1VAXHAPQ1H5HA5VQ2T.buildon)
  ...
  (begin
    ...
    (if (is-eq sender VAULT)
			(begin
				...
				(as-contract (contract-call? 'SP15ZNQ9FT2YJFNXQQHC16B1VAXHAPQ1H5HA5VQ2T.buildon transfer (fixed-to-decimals amount) tx-sender recipient memo)))
			(begin
				(and (is-eq recipient VAULT) (try! (ft-mint? wrapped-token amount recipient)))
				(contract-call? 'SP15ZNQ9FT2YJFNXQQHC16B1VAXHAPQ1H5HA5VQ2T.buildon transfer (fixed-to-decimals amount) tx-sender (if (is-eq recipient VAULT) (as-contract tx-sender) recipient) memo)))))
