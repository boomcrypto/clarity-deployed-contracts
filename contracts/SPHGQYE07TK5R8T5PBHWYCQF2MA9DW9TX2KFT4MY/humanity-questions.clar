...
(define-non-fungible-token humanity-questions uint)
...
;; Internal variables
(define-data-var mint-limit uint u12700)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1000000)
(define-data-var artist-address principal 'SPHGQYE07TK5R8T5PBHWYCQF2MA9DW9TX2KFT4MY)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdexUQKSfAxvnnGEzghLJ7k3VEQYhKvZMRmTk9Fj14xvs/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u12700)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? humanity-questions next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? humanity-questions token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? humanity-questions token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? humanity-questions token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? humanity-questions id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? humanity-questions id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? humanity-questions id) (err ERR-NOT-FOUND)))
