;; technotopia-prologue
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token technotopia-prologue uint)
...
(define-data-var artist-address principal 'SP168K1Q9QYNXJ9814T0F1WDNSP4J4K8NT6D0FX8E)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTJnrPF7FQPtg2BL1j4S8Vjxn5h9i9AuYoEaWaUVYne9y/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? technotopia-prologue next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? technotopia-prologue token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? technotopia-prologue token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? technotopia-prologue token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? technotopia-prologue id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? technotopia-prologue id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? technotopia-prologue id) (err ERR-NOT-FOUND)))
