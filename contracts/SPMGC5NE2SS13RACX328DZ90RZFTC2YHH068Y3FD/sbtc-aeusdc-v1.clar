...
;; Title: sBTC-aeUSDC Liquidity Pool
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token sBTC-aeUSDC)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? sBTC-aeUSDC amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "sBTC-aeUSDC Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance sBTC-aeUSDC who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply sBTC-aeUSDC)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? sBTC-aeUSDC (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? sBTC-aeUSDC (get dk delta) sender))
...
(define-read-only (get-reserves-quote)
    (let (
        (reserves (get-reserves))
        (supply (ft-get-supply sBTC-aeUSDC)))
        {
          dx: (get a reserves),
          dy: (get b reserves),
          dk: supply
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SPMGC5NE2SS13RACX328DZ90RZFTC2YHH068Y3FD.sbtc-aeusdc-v1"))
    ...
    (try! (contract-call? 'SP3Y2ZSH8P7D50B0VBTSX11S7XSG24M1VB9YFQA4K.token-aeusdc transfer u11690000 tx-sender CONTRACT none))
)