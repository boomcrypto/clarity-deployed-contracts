...
(define-non-fungible-token aurora-flight uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSr2GzbctLTf78zYvEUXfjTGRDGwjpTNY8qwRsJAYSjPz/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? aurora-flight next-id tx-sender) next-id)
      (unwrap! (nft-transfer? aurora-flight next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? aurora-flight token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? aurora-flight token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? aurora-flight token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? aurora-flight id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? aurora-flight id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? aurora-flight id) (err ERR-NOT-FOUND)))
