...
(define-non-fungible-token urban-artifacts uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWntxPaX28vjCdhFXi626HYpSg13HAifVRNv1t9pDpKqN/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? urban-artifacts next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? urban-artifacts token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? urban-artifacts token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? urban-artifacts token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? urban-artifacts id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? urban-artifacts id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? urban-artifacts id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u23474)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u23476000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u19460000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u130000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u1216000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u8900000000)
