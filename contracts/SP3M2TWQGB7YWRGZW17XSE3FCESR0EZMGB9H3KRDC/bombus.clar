...
(define-non-fungible-token bombus uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRD4DrTxuKiYmxbBx7d1dg4YZYWivK6KwdSyfF6EGDEKE/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bombus next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bombus token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bombus token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bombus token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bombus id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bombus id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bombus id) (err ERR-NOT-FOUND)))
