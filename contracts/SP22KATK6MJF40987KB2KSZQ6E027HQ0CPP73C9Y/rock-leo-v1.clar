...
;; Define LP token
(define-fungible-token ROCK-LEO)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? ROCK-LEO amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "ROCK-LEO Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance ROCK-LEO who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply ROCK-LEO)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? ROCK-LEO (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? ROCK-LEO (get dk delta) sender))
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP22KATK6MJF40987KB2KSZQ6E027HQ0CPP73C9Y.rock-leo-v1"))
    ...
    (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock transfer u64400000000 tx-sender CONTRACT none))
)