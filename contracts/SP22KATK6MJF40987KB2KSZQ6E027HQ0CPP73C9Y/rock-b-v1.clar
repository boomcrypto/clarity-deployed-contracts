...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? ROCK-B amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "ROCK-B Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance ROCK-B who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply ROCK-B)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.b-faktory transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? ROCK-B (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? ROCK-B (get dk delta) sender))
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP22KATK6MJF40987KB2KSZQ6E027HQ0CPP73C9Y.rock-b-v1"))
    ;; Add initial balanced liquidity (handles both token transfers at 1:1)
    (try! (add-liquidity u200000000000))
    ;; Transfer additional token B to achieve desired ratio
    (try! (contract-call? 'SPV9K21TBFAK4KNRJXF5DFP8N7W46G4V9RCJDC22.b-faktory transfer u800000000000 tx-sender CONTRACT none))
)