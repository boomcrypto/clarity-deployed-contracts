...
;; Title: ROCK-SKULL Liquidity Pool
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token ROCK-SKULL)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? ROCK-SKULL amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "ROCK-SKULL Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance ROCK-SKULL who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply ROCK-SKULL)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? ROCK-SKULL (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? ROCK-SKULL (get dk delta) sender))
...
(define-read-only (get-reserves-quote)
    (let (
        (reserves (get-reserves))
        (supply (ft-get-supply ROCK-SKULL)))
        {
          dx: (get a reserves),
          dy: (get b reserves),
          dk: supply
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP22KATK6MJF40987KB2KSZQ6E027HQ0CPP73C9Y.rock-skull-v1"))
)