...
;; Define LP token
(define-fungible-token ROCK-GUS)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? ROCK-GUS amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "ROCK-GUS Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance ROCK-GUS who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply ROCK-GUS)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SP1JFFSYTSH7VBM54K29ZFS9H4SVB67EA8VT2MYJ9.gus-token transfer (get dy delta) CONTRACT sender none)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SP1JFFSYTSH7VBM54K29ZFS9H4SVB67EA8VT2MYJ9.gus-token transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? ROCK-GUS (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? ROCK-GUS (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SP1JFFSYTSH7VBM54K29ZFS9H4SVB67EA8VT2MYJ9.gus-token get-balance CONTRACT))
    })
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP22KATK6MJF40987KB2KSZQ6E027HQ0CPP73C9Y.rock-gus-v1"))
    ...
    (try! (contract-call? 'SP4M2C88EE8RQZPYTC4PZ88CE16YGP825EYF6KBQ.stacks-rock transfer u50000000000 tx-sender CONTRACT none))
)