...
;; Title: ROCK-DROID Liquidity Pool
;; Version: 1.0.0
;; Description: 
...
;; Define LP token
(define-fungible-token ROCK-DROID)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? ROCK-DROID amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "ROCK-DROID Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance ROCK-DROID who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply ROCK-DROID)))
...
(define-public (swap-a-to-b (amount uint))
    (let (
        ...
        (try! (as-contract (contract-call? 'SPQVE2WZ07ZBQ6AG6W99JGDYCWGS6SEMAX3PKSZJ.droid transfer (get dy delta) CONTRACT sender none)))
...
(define-public (swap-b-to-a (amount uint))
    (let (
        ...
        (try! (contract-call? 'SPQVE2WZ07ZBQ6AG6W99JGDYCWGS6SEMAX3PKSZJ.droid transfer amount sender CONTRACT none))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (contract-call? 'SPQVE2WZ07ZBQ6AG6W99JGDYCWGS6SEMAX3PKSZJ.droid transfer (get dy delta) sender CONTRACT none))
        (try! (ft-mint? ROCK-DROID (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? ROCK-DROID (get dk delta) sender))
...
(define-private (get-reserves)
    { 
      ...
      b: (unwrap-panic (contract-call? 'SPQVE2WZ07ZBQ6AG6W99JGDYCWGS6SEMAX3PKSZJ.droid get-balance CONTRACT))
    })
...
(define-read-only (get-reserves-quote)
    (let (
        (reserves (get-reserves))
        (supply (ft-get-supply ROCK-DROID)))
        {
          dx: (get a reserves),
          dy: (get b reserves),
          dk: supply
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP22KATK6MJF40987KB2KSZQ6E027HQ0CPP73C9Y.rock-droid-v1"))
)