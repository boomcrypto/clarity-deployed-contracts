...
(define-non-fungible-token ppn uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmYmNtAgcxpozpDUqvSFC1wsM7cprBji3z2UgG5YisqpwA/ppn/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP3KCXQ9F762M41TY4YZE398NA9FYZ2ADRQDH4RTS )) 
...
(define-data-var public-target-block-height uint u64385)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u20000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ppn id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace ppn
  (ok (nft-get-owner? ppn id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? ppn next-id new-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ppn id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ppn id) ERR-NOT-FOUND))
