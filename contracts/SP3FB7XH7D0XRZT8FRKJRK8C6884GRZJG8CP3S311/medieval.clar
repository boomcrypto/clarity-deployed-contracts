...
(define-non-fungible-token medieval uint)
...
(define-data-var artist-address principal 'SP3FB7XH7D0XRZT8FRKJRK8C6884GRZJG8CP3S311)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWo3r2PZ9Jzftep8ZhsrGAtMJKcJC526cV2dqavJfbasG/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? medieval next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? medieval token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? medieval token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? medieval token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? medieval id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? medieval id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? medieval id) (err ERR-NOT-FOUND)))
