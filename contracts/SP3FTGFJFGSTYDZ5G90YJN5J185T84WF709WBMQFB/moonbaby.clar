...
(define-non-fungible-token moonbaby uint)
...
;; Internal variables
(define-data-var mint-limit uint u5850)
(define-data-var last-id uint u1)
(define-data-var total-price uint u0)
(define-data-var artist-address principal 'SP3FTGFJFGSTYDZ5G90YJN5J185T84WF709WBMQFB)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWe2qJekiJj33kJTyxMDVousmRHU8MESmQTGdnQojius8/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? moonbaby next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? moonbaby token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? moonbaby token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? moonbaby token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? moonbaby id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? moonbaby id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? moonbaby id) (err ERR-NOT-FOUND)))
