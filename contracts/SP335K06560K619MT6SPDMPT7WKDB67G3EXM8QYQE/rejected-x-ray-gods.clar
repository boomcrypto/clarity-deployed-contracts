...
(define-non-fungible-token rejected-x-ray-gods uint)
...
;; Internal variables
(define-data-var mint-limit uint u535)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRvwsZwwq4Khs1snf2U5MnZUowiTbrG3HEsEHeSyeHeeW/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? rejected-x-ray-gods next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? rejected-x-ray-gods token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? rejected-x-ray-gods token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? rejected-x-ray-gods token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rejected-x-ray-gods id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rejected-x-ray-gods id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rejected-x-ray-gods id) (err ERR-NOT-FOUND)))
