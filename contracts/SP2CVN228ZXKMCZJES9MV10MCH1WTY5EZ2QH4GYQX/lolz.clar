...
(define-non-fungible-token lolz uint)
...
;; Internal variables
(define-data-var mint-limit uint u8939)
(define-data-var last-id uint u1)
(define-data-var total-price uint u300000)
(define-data-var artist-address principal 'SP2CVN228ZXKMCZJES9MV10MCH1WTY5EZ2QH4GYQX)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbQYvaPcMaFMo4j4L2zznx9dNmZTwzL5Pzq5WzNdrK3Sc/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? lolz next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? lolz token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? lolz token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? lolz token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? lolz id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? lolz id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? lolz id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u3589)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u3573000000)
