...
;; Variables
(define-fungible-token HAPPY MAXSUPPLY)
(define-data-var contract-owner principal tx-sender)
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://gaia.hiro.so/hub/1NGN9Z8NxdYAyeZqzTuNcrxsp13U3hWtBn/48e81987-023e-4b46-81b8-839e9f26f68f-metadata.json"))
(define-data-var token-name (string-ascii 32) "happy")
(define-data-var token-symbol (string-ascii 32) "HAPPY")
(define-data-var token-decimals uint u6)

;; Read-Only Functions
(define-read-only (get-balance (owner principal))
  (ok (ft-get-balance HAPPY owner))
)
...
(define-public (transfer (amount uint) (from principal) (to principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (>= (ft-get-balance HAPPY from) amount) (err ERR-NOT-ENOUGH-FUND))
    (asserts! (not (is-eq from to)) (err ERR-INVALID-PARAMS))
    (let ((result (ft-transfer? HAPPY amount from to)))
      (print
        {
          event: "transfer",
          from: from,
...
(begin
  ...
  (let ((result (ft-mint? HAPPY MAXSUPPLY (var-get contract-owner))))
    (print
      {
        event: "mint",
