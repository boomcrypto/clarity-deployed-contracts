...
(define-non-fungible-token phil-crypto uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmciZRd8bApjGLZKTK9LE5f35yjivyWL47ShFuJ6DDBvMz/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? phil-crypto next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? phil-crypto token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? phil-crypto token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? phil-crypto token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? phil-crypto id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? phil-crypto id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? phil-crypto id) (err ERR-NOT-FOUND)))
