...
(define-non-fungible-token phil-crypto-colored uint)
...
(define-data-var artist-address principal 'SP1S7NH168W3GMJXAJAHVVF19N0PVRA5S6M5TAZ89)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYoi97cqX9BnHnok5fyfRR9rTtMi3evx1BzYqvMRu9Loe/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? phil-crypto-colored next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? phil-crypto-colored token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? phil-crypto-colored token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? phil-crypto-colored token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? phil-crypto-colored id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? phil-crypto-colored id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? phil-crypto-colored id) (err ERR-NOT-FOUND)))
