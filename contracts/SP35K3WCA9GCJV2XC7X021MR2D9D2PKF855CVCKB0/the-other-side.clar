...
(define-non-fungible-token the-other-side uint)
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? the-other-side item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? the-other-side item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? the-other-side item-id)))
      (begin
        (try! (nft-burn? the-other-side item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? the-other-side token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? the-other-side id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? the-other-side id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? the-other-side id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP35K3WCA9GCJV2XC7X021MR2D9D2PKF855CVCKB0 u500 "The Other Side" u500000000 u64225 u64369 "ipfs://QmceUWy532nQiozH7qLg5us282au7wU4j5ztNDVo36H9v2/the-other-side/1.json"))
  (ok true)
)
