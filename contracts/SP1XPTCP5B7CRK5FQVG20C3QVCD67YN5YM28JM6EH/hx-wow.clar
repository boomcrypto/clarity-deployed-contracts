...
(define-non-fungible-token hx-wow uint)
...
(define-constant MINT-LIMIT u290)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmcKiGi1jfn9VtYS7ZuAKqE9WsHsDbLp3wMEpdVcVmydSN/hx-wow/")
...
(define-data-var public-target-block-height uint u65570)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u9000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? hx-wow id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace hx-wow
  (ok (nft-get-owner? hx-wow id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? hx-wow next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? hx-wow id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? hx-wow id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? hx-wow id) ERR-NOT-FOUND))
