...
(define-non-fungible-token big-mimi uint)
...
(define-constant MINT-LIMIT u150)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmbcnKCc1YRPrYd4zt3SonBH7M7yNxVUp5jwXHMyiMUv1q/big-mimi/")
...
(define-data-var public-target-block-height uint u65192)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? big-mimi id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace big-mimi
  (ok (nft-get-owner? big-mimi id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? big-mimi next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? big-mimi id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? big-mimi id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? big-mimi id) ERR-NOT-FOUND))
