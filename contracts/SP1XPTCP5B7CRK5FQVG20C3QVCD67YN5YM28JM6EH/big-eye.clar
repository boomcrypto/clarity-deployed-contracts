...
(define-non-fungible-token big-eye uint)
...
(define-constant MINT-LIMIT u415)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmUmpKUsCTVM383hhhJqbWFZmiygBvDAHz98Aoyi4E6Xzz/big-eye/")
...
(define-data-var public-target-block-height uint u65107)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? big-eye id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace big-eye
  (ok (nft-get-owner? big-eye id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? big-eye next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? big-eye id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? big-eye id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? big-eye id) ERR-NOT-FOUND))
