...
(define-non-fungible-token fb- uint)
...
(define-constant MINT-LIMIT u106)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmczgvbbNGP67PmScSSv9nJhARYExLFuCJ8NcFUZJYLmjt/fb-/")
...
(define-data-var public-target-block-height uint u71598)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? fb- id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace fb-
  (ok (nft-get-owner? fb- id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? fb- next-id new-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? fb- id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? fb- id) ERR-NOT-FOUND))
