...
(define-non-fungible-token xingzou uint)
...
(define-constant MINT-LIMIT u133)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmXcuB4AZAbE5aQsM3F5Buojks38grbmFyWhCdMHSj75gP/xingzou/")
...
(define-data-var public-target-block-height uint u65710)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? xingzou id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace xingzou
  (ok (nft-get-owner? xingzou id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? xingzou next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? xingzou id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? xingzou id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? xingzou id) ERR-NOT-FOUND))
