...
(define-non-fungible-token xb uint)
...
(define-constant MINT-LIMIT u211)

;; Define Variables
(define-data-var last-id uint u0)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmSsYuJcU4E6W6a1HCH9oriBimpebVY3ABSoBivZtBtSBk/xb/")
...
(define-data-var artist-address (list 1000 principal) (list 'SP1N8S5TVA56V2QDTEGGQDENTK7TMA8Y30GAJ7KX )) 
...
(define-data-var public-target-block-height uint u65448)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? xb id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace xb
  (ok (nft-get-owner? xb id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? xb next-id new-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? xb id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? xb id) ERR-NOT-FOUND))
