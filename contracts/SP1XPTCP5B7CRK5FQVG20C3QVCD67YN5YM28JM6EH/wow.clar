...
(define-data-var base-uri (string-ascii 100) "ipfs://QmXyXFuFA36H31xSYzFdniyfhRyCWNxKTM8Zg4qMd9hqBX/wow/")
...
(define-data-var public-target-block-height uint u65959)
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace wow
  (ok (nft-get-owner? wow id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? wow next-id new-owner)
        success
        (let
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? wow id) ERR-NOT-FOUND))
