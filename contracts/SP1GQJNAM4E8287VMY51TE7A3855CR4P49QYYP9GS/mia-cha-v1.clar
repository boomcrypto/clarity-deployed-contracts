...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
    (begin
        ...
        (try! (ft-transfer? MIA-CHA amount sender recipient))
        (match memo to-print (print to-print) 0x0000)
        (ok true)))

(define-read-only (get-name)
    (ok "MIA-CHA Liquidity Pool"))
...
(define-read-only (get-balance (who principal))
    (ok (ft-get-balance MIA-CHA who)))

(define-read-only (get-total-supply)
    (ok (ft-get-supply MIA-CHA)))
...
(define-public (add-liquidity (amount uint))
    (let (
        ...
        (try! (ft-mint? MIA-CHA (get dk delta) sender))
...
(define-public (remove-liquidity (amount uint))
    (let (
        (sender tx-sender)
        (delta (get-liquidity-quote amount)))
        (try! (ft-burn? MIA-CHA (get dk delta) sender))
...
(begin
    ;; Set token URI
    (var-set token-uri (some u"https://metadata.charisma.rocks/api/v1/metadata/SP1GQJNAM4E8287VMY51TE7A3855CR4P49QYYP9GS.mia-cha-v1"))
)