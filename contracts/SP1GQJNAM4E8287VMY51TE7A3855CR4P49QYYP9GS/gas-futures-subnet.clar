;; title: gas-futures-subnet
;; author: rozar.btc
;; summary: Intent-based subnet token wrapper for gas-futures using blaze-intent verifier.
...
(define-read-only (get-decimals)     (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.gas-futures get-decimals))
(define-read-only (get-token-uri)    (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.gas-futures get-token-uri))
(define-read-only (get-total-supply) (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.gas-futures get-total-supply))
...
(define-public (deposit (amount uint) (recipient (optional principal)))
  (let ((sender tx-sender)
    ...
    (try! (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.gas-futures transfer amount sender (as-contract tx-sender) none))
...
(define-public (withdraw (amount uint) (recipient (optional principal)))
  (let ((owner tx-sender)
    ...
    (try! (as-contract (contract-call? 'SP39859AD7RQ6NYK00EJ8HN1DWE40C576FBDGHPA0.gas-futures transfer amount tx-sender recv none)))
