;; into-the-light-reflection
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token into-the-light-reflection uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSP1CgvyotsYs4bP9t9VCMXUjezvKqAaCoKpDXFhzzgiD/json/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? into-the-light-reflection next-id tx-sender) next-id)
      (unwrap! (nft-transfer? into-the-light-reflection next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? into-the-light-reflection token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? into-the-light-reflection token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? into-the-light-reflection token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? into-the-light-reflection id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? into-the-light-reflection id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? into-the-light-reflection id) (err ERR-NOT-FOUND)))
