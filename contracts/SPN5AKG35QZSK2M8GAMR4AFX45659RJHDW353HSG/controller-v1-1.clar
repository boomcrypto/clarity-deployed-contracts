...
(define-public (log-reward (reward-usdh uint))
  (let (
    ...
    (total-usdh-supply-staked (unwrap-panic (contract-call? .usdh-token-v1 get-balance .staking-reserve-v1)))
  )
    ...
    (if (> reward-usdh u0)
      (begin
        ...
        (try! (contract-call? .usdh-token-v1 mint-for-protocol reward-usdh .staking-reserve-v1))
      )
      true
    )
