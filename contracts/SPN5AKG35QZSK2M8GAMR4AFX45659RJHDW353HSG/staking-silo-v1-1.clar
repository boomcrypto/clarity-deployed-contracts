...
(impl-trait .staking-silo-trait-v1.staking-silo-trait)
...
(define-data-var current-claim-id uint u1234)
...
(define-public (create-claim (amount uint) (recipient principal))
  (let (
    ...
    (ts (+ (get-current-ts) (contract-call? .staking-state-v1 get-custom-cooldown recipient)))
  )
    (asserts! (is-eq contract-caller .staking-v1-1) ERR_ONLY_STAKING_CONTRACT)
