...
;; Define LP token
(define-fungible-token sBTC-POS)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? sBTC-POS amount sender recipient))
    (match memo
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance sBTC-POS who))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply sBTC-POS))
)
...
(define-public (swap-a-to-b
  ...
  (let (
    ...
    (print {
      ...
      amount-in: amount,
      token-out: 'SPGNH14RQWAT05PVG8CEXCM7BGC5PPR01XVGQXPZ.pos-coin-stxcity,
      amount-out: dy-d,
...
(define-public (swap-b-to-a (amount uint) (min-y-out uint))
  (let (
    ...
    (print {
      type: "sell",
      sender: sender,
      token-in: 'SPGNH14RQWAT05PVG8CEXCM7BGC5PPR01XVGQXPZ.pos-coin-stxcity,
      amount-in: amount,
...
(define-public (add-liquidity (amount uint))
  (let (
    ...
    (try! (ft-mint? sBTC-POS dk-d sender))
    (print {
      type: "add-liquidity",
      sender: sender,
      token-a: 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token,
      token-a-amount: dx-d,
      token-b: 'SPGNH14RQWAT05PVG8CEXCM7BGC5PPR01XVGQXPZ.pos-coin-stxcity,
      token-b-amount: dy-d,
      lp-tokens: dk-d,
      pool-reserves: (get-reserves-quote),
      pool-contract: CONTRACT,
...
(define-public (remove-liquidity (amount uint))
  (let (
    ...
    (try! (ft-burn? sBTC-POS dk-d sender))
    (try! (as-contract (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
...
(begin
  ...
  (try! (contract-call?
    'SPGNH14RQWAT05PVG8CEXCM7BGC5PPR01XVGQXPZ.pos-coin-stxcity
    transfer u112764698281 tx-sender CONTRACT none
  ))
  (print {
