...
(define-public (deposit-stx-for-lp (stx-amount uint))
    (let (
          (amounts (unwrap! (calculate-amounts-for-lp stx-amount) ERR_CALC_AMOUNTS))
...
(define-public (withdraw-lp-tokens)
  (let ((unlock-block (+ (var-get creation-block) LOCK_PERIOD))
        ...
        (bfaktory-depositor-principal (unwrap! (var-get bfaktory-depositor) ERR_NOT_INITIALIZED)))
...
(define-read-only (calculate-amounts-for-lp (stx-amount uint))
  (begin 
        ...
        (match (get-quote-for-lp stx-amount)
          liquidity-quote (ok {
