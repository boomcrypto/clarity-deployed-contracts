      ...
      ;; Title: sBTC-welshcorgicoin lp-token (Single Pool)
      ;; Version: 1.0.0
      ...
      ;; Define LP token
      (define-fungible-token sBTC-welshcorgicoin)
      ...
      (define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
          (begin
              ...
              (try! (ft-transfer? sBTC-welshcorgicoin amount sender recipient))
      ...
      (define-read-only (get-name)
          (ok "sBTC-welshcorgicoin lp-token"))
  
      (define-read-only (get-symbol)
          (ok "sBTC-welshcorgicoin"))
  
      (define-read-only (get-decimals)
          (ok u6))
  
      (define-read-only (get-balance (who principal))
          (ok (ft-get-balance sBTC-welshcorgicoin who)))
  
      (define-read-only (get-total-supply)
          (ok (ft-get-supply sBTC-welshcorgicoin)))
      ...
      (define-public (swap-a-to-b (amount uint) (min-y-out uint))
          (let (
              ...
              (try! (as-contract (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer dy-d CONTRACT sender none)))
              (print {
                  type: "buy",
                  sender: sender,
                  token-in: 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token,
                  amount-in: amount,
                  token-out: 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token,
                  amount-out: dy-d,
      ...
      (define-public (swap-b-to-a (amount uint) (min-y-out uint))
          (let (
              ...
              (print {
                  type: "sell",
                  sender: sender,
                  token-in: 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token,
                  amount-in: amount,
      ...
      (define-public (add-liquidity (amount uint))
          (let (
              ...
              (try! (contract-call? 'SP3NE50GEXFG9SZGTT51P40X2CKYSZ5CC4ZTZ7A2G.welshcorgicoin-token transfer dy-d sender CONTRACT none))
              (try! (ft-mint? sBTC-welshcorgicoin dk-d sender))
              (print {
                  type: "add-liquidity",
                  sender: sender,
      ...
      (define-public (remove-liquidity (amount uint))
          (let (
              ...
              (dy-d (get dy delta))
              (dk-d (get dk delta)))
              (try! (ft-burn? sBTC-welshcorgicoin dk-d sender))
      ...
      (define-read-only (get-liquidity-quote (amount uint))
          (let (
              (k (ft-get-supply sBTC-welshcorgicoin))
              (reserves (get-reserves)))
              {
      ...
      (define-read-only (get-reserves-quote)
          (let (
              (reserves (get-reserves))
              (supply (ft-get-supply sBTC-welshcorgicoin)))
              {
                dx: (get a reserves),
                dy: (get b reserves),
                dk: supply
      ...
      (begin
        ...
        (try! (contract-call?
          ...
          transfer u242593614810 tx-sender CONTRACT none
        ))
        (print {
