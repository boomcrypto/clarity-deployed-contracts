...
;; Define LP token
(define-fungible-token sBTC-stxAI)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? sBTC-stxAI amount sender recipient))
    (match memo
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance sBTC-stxAI who))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply sBTC-stxAI))
)
...
(define-public (swap-a-to-b
  ...
  (let (
    ...
    (print {
      ...
      amount-in: amount,
      token-out: 'SP2BQ0676YV3F7QBJXS1PT7XA975ZG03XEXS9C8TN.stacksai-stxcity,
      amount-out: dy-d,
...
(define-public (swap-b-to-a (amount uint) (min-y-out uint))
  (let (
    ...
    (print {
      type: "sell",
      sender: sender,
      token-in: 'SP2BQ0676YV3F7QBJXS1PT7XA975ZG03XEXS9C8TN.stacksai-stxcity,
      amount-in: amount,
...
(define-public (add-liquidity (amount uint))
  (let (
    ...
    (try! (ft-mint? sBTC-stxAI dk-d sender))
    (print {
      type: "add-liquidity",
      sender: sender,
      token-a: 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token,
      token-a-amount: dx-d,
      token-b: 'SP2BQ0676YV3F7QBJXS1PT7XA975ZG03XEXS9C8TN.stacksai-stxcity,
      token-b-amount: dy-d,
      lp-tokens: dk-d,
      pool-reserves: (get-reserves-quote),
      pool-contract: CONTRACT,
...
(define-public (remove-liquidity (amount uint))
  (let (
    ...
    (try! (ft-burn? sBTC-stxAI dk-d sender))
    (try! (as-contract (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
...
(define-read-only (get-reserves-quote)
  (let (
      (reserves (get-reserves))
      (supply (ft-get-supply sBTC-stxAI))
    )
    {
      dx: (get a reserves),
      dy: (get b reserves),
...
(begin
  ...
  (try! (contract-call?
    'SP2BQ0676YV3F7QBJXS1PT7XA975ZG03XEXS9C8TN.stacksai-stxcity
    transfer u6724920165 tx-sender CONTRACT none
  ))
  (print {
