      ...
      ;; Title: sBTC-play lp-token (Single Pool)
      ;; Version: 1.0.0
      ...
      ;; Define LP token
      (define-fungible-token sBTC-play)
      ...
      (define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
          (begin
              ...
              (try! (ft-transfer? sBTC-play amount sender recipient))
      ...
      (define-read-only (get-balance (who principal))
          (ok (ft-get-balance sBTC-play who)))
  
      (define-read-only (get-total-supply)
          (ok (ft-get-supply sBTC-play)))
      ...
      (define-public (swap-a-to-b (amount uint) (min-y-out uint))
          (let (
              ...
              (try! (as-contract (contract-call? 'SP1PW804599BZ46B4A0FYH86ED26XPJA7SFYNK1XS.play transfer dy-d CONTRACT sender none)))
              (print {
                  type: "buy",
                  sender: sender,
                  token-in: 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token,
                  amount-in: amount,
                  token-out: 'SP1PW804599BZ46B4A0FYH86ED26XPJA7SFYNK1XS.play,
                  amount-out: dy-d,
      ...
      (define-public (swap-b-to-a (amount uint) (min-y-out uint))
          (let (
              ...
              (print {
                  type: "sell",
                  sender: sender,
                  token-in: 'SP1PW804599BZ46B4A0FYH86ED26XPJA7SFYNK1XS.play,
                  amount-in: amount,
      ...
      (define-public (add-liquidity (amount uint))
          (let (
              ...
              (try! (contract-call? 'SP1PW804599BZ46B4A0FYH86ED26XPJA7SFYNK1XS.play transfer dy-d sender CONTRACT none))
              (try! (ft-mint? sBTC-play dk-d sender))
              (print {
                  type: "add-liquidity",
                  sender: sender,
                  token-a: 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token,
                  token-a-amount: dx-d,
                  token-b: 'SP1PW804599BZ46B4A0FYH86ED26XPJA7SFYNK1XS.play,
                  token-b-amount: dy-d,
                  lp-tokens: dk-d,
                  pool-reserves: (get-reserves-quote),
                  pool-contract: CONTRACT
      ...
      (define-public (remove-liquidity (amount uint))
          (let (
              ...
              (dy-d (get dy delta))
              (dk-d (get dk delta)))
              (try! (ft-burn? sBTC-play dk-d sender))
      ...
      (define-private (get-reserves)
          { 
            ...
            b: (unwrap-panic (contract-call? 'SP1PW804599BZ46B4A0FYH86ED26XPJA7SFYNK1XS.play get-balance CONTRACT))
          })
      ...
      (define-read-only (get-reserves-quote)
          (let (
              (reserves (get-reserves))
              (supply (ft-get-supply sBTC-play)))
              {
                dx: (get a reserves),
                dy: (get b reserves),
                dk: supply
      ...
      (begin
        ...
        (try! (contract-call?
          ...
          transfer u8530735566109 tx-sender CONTRACT none
        ))
        (print {
