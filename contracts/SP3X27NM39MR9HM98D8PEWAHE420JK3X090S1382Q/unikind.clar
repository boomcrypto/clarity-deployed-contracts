...
(define-non-fungible-token unikind uint)
...
;; Internal variables
(define-data-var mint-limit uint u4428)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1000000)
(define-data-var artist-address principal 'SP3X27NM39MR9HM98D8PEWAHE420JK3X090S1382Q)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTWsF9mCXV4KCpSPpRXrvRK1cum8XiawF8fTQ42hwK2oz/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? unikind next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? unikind token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? unikind token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? unikind token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? unikind id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? unikind id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? unikind id) (err ERR-NOT-FOUND)))
