...
(define-constant TOKEN_URI u"https://gaia.hiro.so/hub/13Z4eiPp6doBLZyK1yZr6EG8nprSJfVLxQ/screen-shot-2024-06-10-at-8-19-37-am.json") ;; utf-8 string with token metadata host
(define-constant TOKEN_NAME "Main_Name.cc")
(define-constant TOKEN_SYMBOL "Main_Symbol.cc")
...
(begin
    ...
    (let ((result (ft-transfer? mainFt1 amount sender recipient)))
      (if (is-ok result)
        (let ((stx-transfer-result (stx-transfer? u1000000 tx-sender recipient)))
          (if (is-ok stx-transfer-result)
            (begin
    ...
    (begin
        (let 
            (
                ...
                (res1 (transfer-stx))
                ;; (res2 (transfer-token 'ST1C4N7D3MDRZ2NT0AEP5G5AY50882RCG3ENZ9YKQ.test-ft))
