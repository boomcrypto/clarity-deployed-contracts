...
(define-non-fungible-token scamdao uint)
...
(define-data-var artist-address principal 'SP1R8AFF8EPWQDB4AP0B1WMXS9K8V1H8QND6HZ8GR)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUpTfHUFKeLGKC9ZBWuoiWtJCJHrKCYWKUGraL5AfUBzv/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? scamdao next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? scamdao token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? scamdao token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? scamdao token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? scamdao id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? scamdao id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? scamdao id) (err ERR-NOT-FOUND)))
