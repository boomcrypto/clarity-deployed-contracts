...
(define-non-fungible-token zebras uint)
...
(define-data-var artist-address principal 'SP8Q0MG7K00RBZ6NPGA8J1EVMT093QDJNE8W9QNP)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWZhvJ2TqWma3BMSypV747FRNMjJn72S1u4xSuRiLLgud/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? zebras next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? zebras token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? zebras token-id)))
