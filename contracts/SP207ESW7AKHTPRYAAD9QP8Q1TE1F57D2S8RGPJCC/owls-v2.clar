...
(define-non-fungible-token owls uint)
...
(define-data-var artist-address principal 'SP3QZ7WMFGJ9HASJKG64SREM1YYT7BYF3YD2SCTPS)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qme8tq973vsa9ajUZHrFaPBYG1iXDXpc6eVvtbxA1LRFx4/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? owls next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? owls token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? owls token-id)))
