...
(define-data-var artist-address principal 'SPEGDNNSRF6NPBF2YS6X6C8ZCC6FZQARN494NQP3)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmPUq5e6oXaeVearyV5XtKGbmrimocwADBboXMBFaVbuzf/")
...
(define-private (mint-helper (new-owner principal) (next-id uint))
    (match (nft-mint? bulls next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? bulls token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bulls token-id)))
