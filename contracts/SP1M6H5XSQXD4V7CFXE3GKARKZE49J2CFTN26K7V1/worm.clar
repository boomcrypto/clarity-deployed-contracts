...
(define-non-fungible-token worm uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmc4pBKjxM3RNciBqaQjUb9HR4URfa3V7jNKwRnPEF7D3N/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? worm next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? worm token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? worm token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? worm token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? worm id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? worm id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? worm id) (err ERR-NOT-FOUND)))
