...
(define-non-fungible-token ordinal-funky-whale uint)
...
(define-data-var artist-address principal 'SPXE486HHVRNB68D0S60V0N0TJK0XWAEK9YSN61R)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSqaF7dWAizKkXV7rG4NpbRokD6s3TSW9bg6JUH7RcbRg/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinal-funky-whale next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinal-funky-whale token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinal-funky-whale token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinal-funky-whale token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinal-funky-whale id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinal-funky-whale id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinal-funky-whale id) (err ERR-NOT-FOUND)))
