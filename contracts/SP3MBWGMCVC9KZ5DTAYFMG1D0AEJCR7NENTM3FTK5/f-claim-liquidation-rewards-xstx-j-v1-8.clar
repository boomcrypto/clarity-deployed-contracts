...
(define-read-only (check-job)
  (let (
    ...
    (last-id (contract-call? .f-claim-liquidation-rewards-xstx-e-v1-8 get-last-total-reward-ids))
    (next-unlock-block (contract-call? .f-claim-liquidation-rewards-xstx-e-v1-8 get-next-unlock-block))
  )
    ...
    (if (is-eq next-unlock-block u0)
      ...
      (ok (> next-unlock-block block-height))
    )
  )
)
...
(define-public (run-job)
  (begin
    ...
    (try! (contract-call? .lydian-dao-executor-v1-1 execute-proposal .f-claim-liquidation-rewards-xstx-e-v1-8))
    (ok true)
  )
)
