...
(define-read-only (check-job)
  (begin
    (ok (contract-call? .f-claim-liquidation-rewards-xstx-e-v1-1 can-claim-rewards))
  )
)
...
(define-public (run-job)
  (begin
    ...
    (try! (contract-call? .lydian-dao-executor-v1-1 execute-proposal .f-claim-liquidation-rewards-xstx-e-v1-1))
    (ok true)
  )
)
