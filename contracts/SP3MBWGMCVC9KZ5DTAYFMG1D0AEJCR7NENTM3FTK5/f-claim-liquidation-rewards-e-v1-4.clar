...
(define-private (claim-reward-stx (reward-id uint))
  (begin
    ...
    ;; Wrap
    (let (
      (stx-balance (stx-get-balance .lydian-dao))
    )
      (print { stx-balance: stx-balance })
      (if (> stx-balance u0)
    ...
    (let (
      (wstx-balance (unwrap-panic (contract-call? .wrapped-stacks-token get-balance .lydian-dao)))
    )
      (print { wstx-balance: wstx-balance })
      (if (> wstx-balance u0)
