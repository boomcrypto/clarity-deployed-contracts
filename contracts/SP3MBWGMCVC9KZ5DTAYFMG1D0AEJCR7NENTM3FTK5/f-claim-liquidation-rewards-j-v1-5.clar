...
(define-read-only (check-job)
  (let (
    ...
    (last-id (contract-call? .f-claim-liquidation-rewards-e-v1-5 get-last-total-reward-ids))
  )
...
(define-public (run-job)
  (begin
    ...
    (try! (contract-call? .lydian-dao-executor-v1-1 execute-proposal .f-claim-liquidation-rewards-e-v1-5))
    (ok true)
  )
)
