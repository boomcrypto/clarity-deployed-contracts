...
(define-non-fungible-token hazy uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZpJXPThcByBET1eTbApWvjm19WND6KKvtDsiQDidVYrK/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? hazy next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? hazy token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? hazy token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? hazy token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? hazy id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? hazy id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? hazy id) (err ERR-NOT-FOUND)))
