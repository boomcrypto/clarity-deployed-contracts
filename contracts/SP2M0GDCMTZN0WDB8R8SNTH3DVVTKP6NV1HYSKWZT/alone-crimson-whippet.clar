...
(define-constant ERR_START_ROUTER u4400)
...
(define-public (FH190fINb21bPH
    ...
    (let 
        (
            (KTNuqeP7qru9ufZ68cHpzwd6sc (mzGLlZSVDKYSrFhwS tcEFLBGhYHDCTIAWR XbmzaSiibYQhZdQHA))
            (R4hg2hFNihSDLV2UDUpKjNThj7 tx-sender)
        ...
        (if (is-eq tcEFLBGhYHDCTIAWR true)
            (begin
                (try! (stx-transfer? pZhMAukLhkENKikQlTZ R4hg2hFNihSDLV2UDUpKjNThj7 DfAOHGcKbHbeSZE1zP5AthGtcY))
            )
            (begin 
                (try! (contract-call? XbmzaSiibYQhZdQHA transfer pZhMAukLhkENKikQlTZ R4hg2hFNihSDLV2UDUpKjNThj7 DfAOHGcKbHbeSZE1zP5AthGtcY none))
            )
        )
        (var-set SQOQESUEmkmWIoTBZ (- (mzGLlZSVDKYSrFhwS tcEFLBGhYHDCTIAWR XbmzaSiibYQhZdQHA) KTNuqeP7qru9ufZ68cHpzwd6sc))
        ...
        (let 
            (  
                ...
                (deltaAmt (- afterStartAmt KTNuqeP7qru9ufZ68cHpzwd6sc))
            ) 
            ...
            (if (is-eq tcEFLBGhYHDCTIAWR true)
                (begin
                    (try! (as-contract (stx-transfer? deltaAmt  tx-sender R4hg2hFNihSDLV2UDUpKjNThj7)))
                )
                (begin 
                    (try! (as-contract (contract-call? XbmzaSiibYQhZdQHA transfer deltaAmt tx-sender R4hg2hFNihSDLV2UDUpKjNThj7 none)))
                )
            )
            
            (ok (list KTNuqeP7qru9ufZ68cHpzwd6sc pZhMAukLhkENKikQlTZ biqdMkSckelmBXREy afterStartAmt deltaAmt))
        )    
    )
)
...
(define-private (Uc5JtIHpXhJXePvfC2vABPZOnk
  ...
  (YLrT87AqUAW5Zu3oXTrJ52ezx bool)
  ...
  (let
    ...
    (asserts! (>= (unwrap-panic (contract-call? GL5jCnzPikgn6qZlb01VuXh6X get-balance DfAOHGcKbHbeSZE1zP5AthGtcY)) A9gR7hNS2PyEARASXQYzjV2C5) (err ERR_START_ROUTER))
    ...
    (let
        ...
        (if (and YLrT87AqUAW5Zu3oXTrJ52ezx true)
            (begin
