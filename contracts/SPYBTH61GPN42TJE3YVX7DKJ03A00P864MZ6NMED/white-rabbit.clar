...
(define-non-fungible-token white-rabbit uint)
...
(define-data-var artist-address principal 'SPYBTH61GPN42TJE3YVX7DKJ03A00P864MZ6NMED)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVhDkiECUFFLBbaYJRu31Zewsq32xvxu7LuaTLaRe6RQq/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? white-rabbit next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? white-rabbit token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? white-rabbit token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? white-rabbit token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? white-rabbit id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? white-rabbit id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? white-rabbit id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u18682)
