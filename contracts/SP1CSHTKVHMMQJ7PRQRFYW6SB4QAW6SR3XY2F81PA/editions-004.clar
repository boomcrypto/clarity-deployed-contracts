...
(define-non-fungible-token editions-004 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmem2FSnRqWKa357uKnqxMzWcUcafZT75YGJr51TZinvVF/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? editions-004 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? editions-004 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? editions-004 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? editions-004 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? editions-004 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? editions-004 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? editions-004 id) (err ERR-NOT-FOUND)))
...
(define-data-var total-price-mia uint u783)
...
;; Alt Minting Mintpass
(define-data-var total-price-nyc uint u759)
