...
(define-non-fungible-token editions-007 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQc2tfmk8By45Vy4suV54ffx3g77afwM1Y5DUwndw39j3/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? editions-007 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? editions-007 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? editions-007 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? editions-007 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? editions-007 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? editions-007 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? editions-007 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u3858)
...
;; Alt Minting Default
(define-data-var total-price-mia uint u109000000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc uint u183600000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u14000000)
