...
;; (define-data-var artist-address principal 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM)
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get collection-size))
    (begin
...
;; Helper function for fold pattern
...
(define-public (mint-many-with (orders uint) (token <ft-trait>))
  (let
    ...
    ;; Mint NFTs using conditional checks
...
;; Initial token whitelist setup - these will be called when contract deploys
;; Uncomment the tokens you want to whitelist by default

;; STX-based tokens
(admin-add-token 'SP1Q4E6971M7HJ6H8XXDSCAFJ96GEQ02RNN8S55ZZ.doge) ;; Doge token
;; (admin-add-token 'SP2J6ZY48GV1EZ5V2V5RB9MP66SW86PYKKNRV9EJ7)  ;; alex-token
;; (admin-add-token 'SP3K1YRE7Y7M5F6H3Y2D9Z9X8N4K3J2L1M0N9O8P6)  ;; Example token 2

;; Test tokens for development
;; (admin-add-token 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.test-ft)  ;; Local test FT
;; (admin-add-token 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.blob-token)  ;; Blob token

;; Popular DeFi tokens (examples - replace with actual contract addresses)
;; (admin-add-token 'SP2J6ZY48GV1EZ5V2V5RB9MP66SW86PYKKNRV9EJ7.usda-token)  ;; USDA
;; (admin-add-token 'SP2J6ZY48GV1EZ5V2V5RB9MP66SW86PYKKNRV9EJ7.diko-token)  ;; DIKO
;; (admin-add-token 'SP2J6ZY48GV1EZ5V2V5RB9MP66SW86PYKKNRV9EJ7.alex-token)  ;; ALEX