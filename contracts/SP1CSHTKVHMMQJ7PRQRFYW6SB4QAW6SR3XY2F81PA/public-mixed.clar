...
(define-non-fungible-token public-mixed uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVYzCq9TqfGxWbKm8dwgS59mhcsvocNV5AM9ygPrJ2y4W/json/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? public-mixed next-id tx-sender) next-id)
      (unwrap! (nft-transfer? public-mixed next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? public-mixed token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? public-mixed token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? public-mixed token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? public-mixed id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? public-mixed id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? public-mixed id) (err ERR-NOT-FOUND)))
