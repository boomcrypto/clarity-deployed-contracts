...
(define-non-fungible-token public-015 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRPpB7S7f1aDAWZsaDG43GdrB4bbu6DrgWzUsQELY6nCJ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? public-015 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? public-015 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? public-015 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? public-015 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? public-015 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? public-015 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? public-015 id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u1731)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u664000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u981000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u4000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u79000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u1800000000)
