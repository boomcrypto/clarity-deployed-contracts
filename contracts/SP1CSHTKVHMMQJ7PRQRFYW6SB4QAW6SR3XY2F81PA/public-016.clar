...
(define-non-fungible-token public-016 uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQjjt2HcgysEhNKDijmcMzQHUyQ2cfxbsuTL3pB1ix7Y6/metadata/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? public-016 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? public-016 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? public-016 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? public-016 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? public-016 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? public-016 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? public-016 id) (err ERR-NOT-FOUND)))
...
(define-data-var total-price-mia uint u2835000000)
