...
(define-non-fungible-token team-chariot uint)
...
(define-data-var private-target-block-height uint u63902)
(define-data-var public-target-block-height uint u64046)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? team-chariot id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace team-chariot
  (ok (nft-get-owner? team-chariot id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? team-chariot next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? team-chariot id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? team-chariot id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? team-chariot id) ERR-NOT-FOUND))
