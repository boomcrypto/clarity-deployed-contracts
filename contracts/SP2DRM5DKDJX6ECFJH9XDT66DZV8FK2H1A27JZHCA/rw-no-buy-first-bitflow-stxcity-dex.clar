...
(define-constant token-supply u68999997000000) ;; match with the token's supply (6 decimals)
(define-constant BONDING-DEX-ADDRESS (as-contract tx-sender)) ;; one contract per token

;; bonding curve config
(define-constant STX_TARGET_AMOUNT u27000000000)
(define-constant VIRTUAL_STX_VALUE u5400000000) ;; 1/5 of STX_TARGET_AMOUNT
(define-constant COMPLETE_FEE u540000000) ;; 2% of STX_TARGET_AMOUNT
...
(define-constant allow-token 'SP2DRM5DKDJX6ECFJH9XDT66DZV8FK2H1A27JZHCA.rw-no-buy-first-bitflow-stxcity)
...
(define-public (buy (token-trait <sip-010-trait>) (stx-amount uint) ) 
  (begin
    ...
    (let (
      ...
      (try! (stx-transfer? (/ stx-fee u2) tx-sender STX_CITY_SWAP_FEE_WALLET))
      ;; send rewards to creator
      (try! (stx-transfer? (/ stx-fee u2) tx-sender deployer))
...
(define-public (sell (token-trait <sip-010-trait>) (tokens-in uint) ) ;; swap out for virtual trading
  (begin
    ...
    (let (
      ...
      ;; send fee to stxcity
      (try! (as-contract (stx-transfer? (/ stx-fee u2) tx-sender STX_CITY_SWAP_FEE_WALLET)))
      ;; send rewards to creator
      (try! (as-contract (stx-transfer? (/ stx-fee u2) tx-sender deployer)))
