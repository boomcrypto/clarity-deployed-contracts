...
(define-constant allow-token 'SP2DRM5DKDJX6ECFJH9XDT66DZV8FK2H1A27JZHCA.doge-smile-stxcity)
...
(define-public (buy (token-trait <sip-010-trait>) (stx-amount uint) ) 
  (begin
    ...
    (let (
      ...
      (try! (stx-transfer? (/ stx-fee u2) tx-sender (contract-of token-trait)))
...
(define-public (sell (token-trait <sip-010-trait>) (tokens-in uint) ) ;; swap out for virtual trading
  (begin
    ...
    (let (
      ...
      ;; send rewards to creator
      (try! (as-contract (stx-transfer? (/ stx-fee u2) tx-sender (contract-of token-trait))))
