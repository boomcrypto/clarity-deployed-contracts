...
(define-constant STX_TARGET_AMOUNT u17000000000)
(define-constant VIRTUAL_STX_VALUE u3400000000) ;; 1/5 of STX_TARGET_AMOUNT
(define-constant COMPLETE_FEE u340000000) ;; 2% of STX_TARGET_AMOUNT
...
(define-constant allow-token 'SP2DRM5DKDJX6ECFJH9XDT66DZV8FK2H1A27JZHCA.rw-no-buy-first-alex-stxcity)
      ...
      (if (>= new-stx-balance  STX_TARGET_AMOUNT)
        (begin
          (let (
            ...
            (remain-stx-alex-format (* remain-stx ONE_2))
            (remain-tokens-alex-format (* remain-tokens ONE_2))
          )
            ;; burn tokens
            ...
            ;; Send assets to ALEX's self-service contract
