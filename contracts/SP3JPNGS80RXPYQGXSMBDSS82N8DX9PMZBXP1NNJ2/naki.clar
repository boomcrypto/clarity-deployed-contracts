...
(define-non-fungible-token naki uint)
...
(define-data-var artist-address principal 'SP3JPNGS80RXPYQGXSMBDSS82N8DX9PMZBXP1NNJ2)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbPuPx5f1BEiAaWJQL89HVJPeFXD7HSPv7awSSyWCxyak/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? naki next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? naki token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? naki token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? naki token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? naki id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? naki id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? naki id) (err ERR-NOT-FOUND)))
