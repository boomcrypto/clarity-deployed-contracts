...
(define-constant ERR-PRICE-DECODE (err u10015))
...
(define-public (on-granite-flash-loan (amount uint) (fee uint) (data (optional (buff 20480))))
  (let (
    ...
    (if (is-eq dex u3)
      (try! (liqudate-and-swap-inner-usdh (get pyth-price-feed-data cdata) (get batch cdata) (get price-slippage-tolerance cdata)))
...
(define-public (liquidate-with-swap-usdh
  ...
  (begin 
    (asserts! (or (is-owner) (is-operator)) ERR-UNAUTHORIZED)
    (asserts! (> deadline (default-to u0 (get-stacks-block-info? time (- stacks-block-height u1)))) ERR-TIMEOUT)
    (try! (liqudate-and-swap-inner-usdh pyth-price-feed-data batch price-slippage-tolerance))
    SUCCESS
  )
)
    ...
    (let (
      ...
      (let
        (
          ...
          (asset-amount-repaid (- initial-market-balance market-balance-before-swap))
          (collateral-obtained (- collateral-balance initial-collateral-balance ))
          (btc-price (try! (decode-price pyth-price-feed-data)))
...
(define-read-only (compute-mintable-usdh (btc-amount uint) (btc-price uint))
  (let (
    (whitelist-data (contract-call? 'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.minting-auto-v1 get-whitelist SELF 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token))
...
(define-private (mint-usdh (amount-usdh-requested uint) (price-slippage-tolerance uint) (price-feed-bytes (optional (buff 8192))))
  (contract-call? 
    'SPN5AKG35QZSK2M8GAMR4AFX45659RJHDW353HSG.minting-auto-v1 mint 
...
(define-private (decode-price (pyth-price-feed-data (optional (buff 8192)))) 
  (let 
    (
      (decoded-data
        (match pyth-price-feed-data value
          (element-at (try! (contract-call? 'SP3R4F6C1J3JQWWCVZ3S7FRRYPMYG6ZW6RZK31FXY.pyth-oracle-v3 decode-price-feeds value {
            pyth-storage-contract: 'SP3R4F6C1J3JQWWCVZ3S7FRRYPMYG6ZW6RZK31FXY.pyth-storage-v3,
            pyth-decoder-contract: 'SP3R4F6C1J3JQWWCVZ3S7FRRYPMYG6ZW6RZK31FXY.pyth-pnau-decoder-v2,
            wormhole-core-contract: 'SP3R4F6C1J3JQWWCVZ3S7FRRYPMYG6ZW6RZK31FXY.wormhole-core-v3
          })) u0)
    ...
    (asserts! (> (unwrap-panic (get publish-time decoded-data)) u0) ERR-PRICE-DECODE)
    (ok (to-uint (unwrap-panic (get price decoded-data))))
  )
)
