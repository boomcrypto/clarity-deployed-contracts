...
(define-public (send-message (content (string-utf8 256)) (attachment-uri (optional (string-utf8 256))) (thumbnail-uri (optional (string-utf8 256))) (reply-to (optional (string-utf8 256))) (cityName (string-ascii 10)))
  (let 
    (
      (chime-amount (unwrap! (unwrap! (get chime-cost (map-get? cost APP_DEV)) notfound) notfound))
      (mine-amount (unwrap! (unwrap! (get mine-cost (map-get? cost APP_DEV)) notfound) notfound))
    ) 
    (print { content: content, publisher: tx-sender, attachment-uri: attachment-uri, thumbnail-uri: thumbnail-uri, reply-to: reply-to, cityName: cityName})
    (begin 
        (is-ok (mine-citycoin cityName (list (/ mine-amount))))
        (ok (stx-transfer? chime-amount tx-sender APP_DEV))
    )
  )   
)
...
(define-public (like-message (author-principal principal) (liked-txid (string-utf8 256)) (contractId <sip-010-trait>))
  (let 
    (
      (like-amount (unwrap! (unwrap! (get like-cost (map-get? cost APP_DEV)) notfound) notfound))
    ) 
    (print { author-principal: author-principal, liked-txid: liked-txid })
    (ok (transfer-citycoin like-amount author-principal contractId))
  ) 
)
...
(define-public (dislike-message (author-principal principal) (disliked-txid (string-utf8 256)) (cityName (string-ascii 10)))
  (let 
    (
      (dislike-amount (unwrap! (unwrap! (get dislike-cost (map-get? cost APP_DEV)) notfound) notfound))
    ) 
    (print { author-principal: author-principal, disliked-txid: disliked-txid })
    (ok (stack-citycoin cityName dislike-amount u1))
  )   
)

;; Replies with Sentiment
(define-public (favorable-reply-message (content (string-utf8 256)) (attachment-uri (optional (string-utf8 256))) (thumbnail-uri (optional (string-utf8 256))) (reply-to (optional (string-utf8 256))) (author-principal principal) (liked-txid (string-utf8 256)) (cityName (string-ascii 10)) (contractId <sip-010-trait>))
    (begin 
...
(define-public (unfavorable-reply-message (content (string-utf8 256)) (attachment-uri (optional (string-utf8 256))) (thumbnail-uri (optional (string-utf8 256))) (reply-to (optional (string-utf8 256))) (author-principal principal) (disliked-txid (string-utf8 256)) (cityName (string-ascii 10)))
    (begin 
        (is-ok (send-message content attachment-uri thumbnail-uri reply-to cityName))
