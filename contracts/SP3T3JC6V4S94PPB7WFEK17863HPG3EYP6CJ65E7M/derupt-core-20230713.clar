...
(impl-trait 'SP2BZ2YP68CABE7D32HE6308P6C7GMYN2ECJM8A7P.derupt-core-trait.derupt-core-trait)
(use-trait sip-010-trait 'SP3FBR2AGK5H9QBDH3EEN6DF8EK8JY7RX8QJ5SVTE.sip-010-trait-ft-standard.sip-010-trait)

;; Error
(define-constant notfound (err u101))
(define-constant insufficient-balance (err u104))
...
(define-public (send-message 
  ...
  (let 
    (
      ...
      (asserts! (> user-balance (+ chime-amount mine-amount)) insufficient-balance)
...
(define-public (dislike-message (author-principal principal) (disliked-txid (string-utf8 256)) (cityName (string-ascii 10)) (contractId <sip-010-trait>))
  (let
    (
      (dislike-amount (unwrap! (get-dislike-amount) notfound))
      (user-balance (unwrap! (contract-call? contractId get-balance tx-sender) notfound))
    )
      (asserts! (> user-balance dislike-amount) insufficient-balance)
...
(define-public (unfavorable-reply-message 
  ...
  (extras 
    ...
    (begin
      (is-ok (send-message content attachment-uri thumbnail-uri reply-to cityName alt-origin extras))
      (is-ok (dislike-message author-principal disliked-txid cityName contractId))
      (ok true)
    )
)
