...
(define-non-fungible-token splatter uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcdriYFcz4NQEhAyj7mrwPKJ4uYzkexdCa3sLGs6Fq2Hf/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? splatter next-id tx-sender) next-id)
      (unwrap! (nft-transfer? splatter next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? splatter token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? splatter token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? splatter token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? splatter id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? splatter id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? splatter id) (err ERR-NOT-FOUND)))
