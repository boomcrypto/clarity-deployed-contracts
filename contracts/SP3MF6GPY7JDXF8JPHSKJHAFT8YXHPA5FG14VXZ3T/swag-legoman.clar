...
(define-non-fungible-token swag-legoman uint)
...
;; Internal variables
(define-data-var mint-limit uint u501)
(define-data-var last-id uint u1)
(define-data-var total-price uint u10000000)
(define-data-var artist-address principal 'SP3MF6GPY7JDXF8JPHSKJHAFT8YXHPA5FG14VXZ3T)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRkceTLMfz9gNVww1j7SeSVJNUoWER4uayUkeE8mTYg1g/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? swag-legoman next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? swag-legoman token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? swag-legoman token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? swag-legoman token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? swag-legoman id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? swag-legoman id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? swag-legoman id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u18628)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u598700000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u1096500000000)
