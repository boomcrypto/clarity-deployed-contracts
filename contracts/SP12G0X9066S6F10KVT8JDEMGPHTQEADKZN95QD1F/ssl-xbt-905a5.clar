...
(define-read-only (get-name) (contract-call? 'SP12G0X9066S6F10KVT8JDEMGPHTQEADKZN95QD1F.xbt get-name))
(define-read-only (get-symbol) (contract-call? 'SP12G0X9066S6F10KVT8JDEMGPHTQEADKZN95QD1F.xbt get-symbol))
(define-read-only (get-base-decimals) (contract-call? 'SP12G0X9066S6F10KVT8JDEMGPHTQEADKZN95QD1F.xbt get-decimals))
(define-read-only (get-token-uri) (contract-call? 'SP12G0X9066S6F10KVT8JDEMGPHTQEADKZN95QD1F.xbt get-token-uri))
(define-read-only (get-balance (who principal)) (if (is-eq who VAULT) (ok (ft-get-balance wrapped-token who)) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SP12G0X9066S6F10KVT8JDEMGPHTQEADKZN95QD1F.xbt get-balance who))))))
(define-read-only (get-total-supply) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SP12G0X9066S6F10KVT8JDEMGPHTQEADKZN95QD1F.xbt get-total-supply)))))
...
(define-read-only (get-base-token)
  'SP12G0X9066S6F10KVT8JDEMGPHTQEADKZN95QD1F.xbt)
  ...
  (begin
    ...
    (if (is-eq sender VAULT)
			(begin
				...
				(as-contract (contract-call? 'SP12G0X9066S6F10KVT8JDEMGPHTQEADKZN95QD1F.xbt transfer (fixed-to-decimals amount) tx-sender recipient memo)))
			(begin
				(and (is-eq recipient VAULT) (try! (ft-mint? wrapped-token amount recipient)))
				(contract-call? 'SP12G0X9066S6F10KVT8JDEMGPHTQEADKZN95QD1F.xbt transfer (fixed-to-decimals amount) tx-sender (if (is-eq recipient VAULT) (as-contract tx-sender) recipient) memo)))))
