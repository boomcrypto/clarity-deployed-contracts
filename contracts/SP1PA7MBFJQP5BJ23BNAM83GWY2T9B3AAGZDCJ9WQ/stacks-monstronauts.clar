...
(define-non-fungible-token stacks-monstronauts uint)
...
(define-data-var artist-address principal 'SP1PA7MBFJQP5BJ23BNAM83GWY2T9B3AAGZDCJ9WQ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQbzJ97GmPNWocgpnEqqMHda7RLP268DCvm4jQsuFRh9n/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-monstronauts next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-monstronauts token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-monstronauts token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-monstronauts token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-monstronauts id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-monstronauts id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-monstronauts id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-alex uint u4400000000)
