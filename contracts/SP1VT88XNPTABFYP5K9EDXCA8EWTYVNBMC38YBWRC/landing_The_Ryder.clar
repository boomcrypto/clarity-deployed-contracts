...
(define-non-fungible-token The_Ryder uint)
...
(define-constant IPFS-ROOT "ipfs://ipfs/bafybeid3qwigm22kyv6n6dqcskzuwuh3e3zuxaotnrdlbjptzin6vlfnki/{id}.json")
...
(define-public (mint)
    (let ((token-id (+ u1 (var-get last-token-id))))
        ...
        (nft-mint? The_Ryder token-id tx-sender)
    )
)
...
(define-public (burn (id uint))
    (let ((owner (unwrap! (nft-get-owner? The_Ryder id) (err ERROR-UNAUTHORIZED))))
        (asserts! (is-eq owner tx-sender) (err ERROR-UNAUTHORIZED))
        (nft-burn? The_Ryder id owner)
    )
)
