;; ordinal-lady-ape-club
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token ordinal-lady-ape-club uint)
...
;; Internal variables
(define-data-var mint-limit uint u422)
(define-data-var last-id uint u1)
(define-data-var total-price uint u75000000)
(define-data-var artist-address principal 'SP3X1CG6HJQWTHZ85DQVES9WMSGCNYASCZ20VVGPW)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUUUPtmZyqSPYzBYzsAQtyyQk1srPXLX1qz6XUYAezKYd/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ordinal-lady-ape-club next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ordinal-lady-ape-club token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ordinal-lady-ape-club token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ordinal-lady-ape-club token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ordinal-lady-ape-club id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ordinal-lady-ape-club id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ordinal-lady-ape-club id) (err ERR-NOT-FOUND)))
