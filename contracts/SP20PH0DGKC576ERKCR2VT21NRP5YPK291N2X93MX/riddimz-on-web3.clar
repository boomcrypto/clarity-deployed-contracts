...
(define-non-fungible-token riddimz-on-web3 uint)
...
;; Internal variables
(define-data-var mint-limit uint u299)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmS5uX5LDnJ99HKnrL495qZ3urnYEfYjrY87Gi2Uz3MwpQ/json/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u300)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? riddimz-on-web3 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? riddimz-on-web3 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? riddimz-on-web3 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? riddimz-on-web3 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? riddimz-on-web3 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? riddimz-on-web3 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? riddimz-on-web3 id) (err ERR-NOT-FOUND)))
