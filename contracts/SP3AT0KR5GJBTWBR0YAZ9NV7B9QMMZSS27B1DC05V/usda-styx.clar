;; Usda Styx Pool structure to track sBTC liquidity (single pool per contract)
;; Trustless one-way bridge from Bitcoin to Usda 
;; Ultra-fast passage via Clarity's direct Bitcoin state reading to ultra fast thru put Blaze subnet 
...
(define-constant OPERATOR_STYX 'SP3VES970E3ZGHQEZ69R8PY62VP3R0C8CTQ8DAMQW) 
        ...
        (match (get out (unwrap! (get-out-value wtx btc-receiver) ERR_NATIVE_FAILURE))
          out (if (>= (get value out) MIN_SATS)
            (let (
                ...
                (min-usda-out (unwrap! (get amount payload) ERR-ELEMENT-EXPECTED))
