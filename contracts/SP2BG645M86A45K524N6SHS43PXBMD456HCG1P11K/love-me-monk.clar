...
(define-non-fungible-token love-me-monk uint)
...
;; Internal variables
(define-data-var mint-limit uint u63)
(define-data-var last-id uint u1)
(define-data-var total-price uint u27000000)
(define-data-var artist-address principal 'SP2BG645M86A45K524N6SHS43PXBMD456HCG1P11K)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdkxmynqWp846nwo7Udg9yvok8TnAELgxGjGcomFycLm3/json/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? love-me-monk next-id tx-sender) next-id)
      (unwrap! (nft-transfer? love-me-monk next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? love-me-monk token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? love-me-monk token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? love-me-monk token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? love-me-monk id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? love-me-monk id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? love-me-monk id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u46027)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u11000000)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u1600600000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u2120800000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u96000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u771000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u35100000000)
