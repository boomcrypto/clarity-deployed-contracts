;; title: OldBoy
;; version: 1
;; summary: Burn a OldBoy Cartridge to Mint a Stacks OldBoy!
...
;; Defining NFT
(define-non-fungible-token OldBoy uint)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? OldBoy id sender recipient) ;;Transfer NFT from sender to recipient
    success
      (begin
...
(define-public (mint (id uint))
  (begin
    (asserts! (is-eq (unwrap! (unwrap! (contract-call? 'SPZNR5CYBDGGYJC1SCM66RC2FWAQ4CZBXQ1PFV82.metaboys get-owner id) ERR-INVALID-USER) ERR-INVALID-USER) tx-sender) ERR-NOT-AUTHORIZED)
    (match (nft-mint? OldBoy id tx-sender) ;; MINT
      success
        (begin
          (unwrap! (contract-call? 'SPZNR5CYBDGGYJC1SCM66RC2FWAQ4CZBXQ1PFV82.metaboys burn id) ERR-CANT-BURN)
...
(define-private (mint-many-iter (id uint) )
    (begin
      (asserts! (is-eq (unwrap! (unwrap! (contract-call? 'SPZNR5CYBDGGYJC1SCM66RC2FWAQ4CZBXQ1PFV82.metaboys get-owner id) ERR-INVALID-USER) ERR-INVALID-USER) tx-sender) ERR-NOT-AUTHORIZED)
      (match (nft-mint? OldBoy id tx-sender)
        success
            (begin
              (unwrap! (contract-call? 'SPZNR5CYBDGGYJC1SCM66RC2FWAQ4CZBXQ1PFV82.metaboys burn id) ERR-CANT-BURN)
              (var-set last-id (+ u1 (var-get last-id)))
...
(define-public (burn (id uint))
    ...
    (asserts! (is-none (map-get? market id)) ERR-LISTING)
    (match (nft-burn? OldBoy id token-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? OldBoy id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? OldBoy id) ERR-NOT-FOUND))
