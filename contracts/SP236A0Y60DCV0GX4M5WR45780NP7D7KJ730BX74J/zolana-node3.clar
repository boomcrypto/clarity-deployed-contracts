...
(define-constant price_mint_1k_n u50000000000000) ;; $0.000000000906 5(8)k nothing
...
(define-fungible-token zolana-wooo  u5722900000000000 )
...
(define-read-only (get-balance (user principal))
  (begin
    ...
    (ok (ft-get-balance zolana-wooo user))
  )
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply zolana-wooo))
)
...
(define-public (transfer (amount uint) (sender principal) (receiver principal) (memo (optional (buff 34)))) 
    ...
    ;; #[allow(unchecked_data)]
        (try! (ft-transfer? zolana-wooo amount sender receiver))
...
(define-public (mint (amount uint))
  (begin
    ...
    (ft-mint? zolana-wooo amount receiver_address_mint)
  )
)
...
(define-public (burn (amount uint))
  (begin
    (asserts! (is-eq (var-get on_off_switch) true) ERR_ZOLANA_OFFLINE)
    (ft-burn? zolana-wooo amount tx-sender)
  )
)
...
(define-public (zolana_on)
  (begin
    (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.woo-meme-world-champion transfer price_on tx-sender receiver_address_switch none))
...
(define-public (zolana_off)
  (begin
    (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.woo-meme-world-champion transfer price_off tx-sender receiver_address_switch none))
...
(define-public (mint_welsh_1000)
  (begin
    ...
    (try! (ft-mint? zolana-wooo u1000000000 tx-sender ))
...
(define-public (censor_user (user principal))
  (begin
     (try! (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.woo-meme-world-champion transfer price_censor tx-sender receiver_address_switch none))
    ;; #[allow(unchecked_data)]
