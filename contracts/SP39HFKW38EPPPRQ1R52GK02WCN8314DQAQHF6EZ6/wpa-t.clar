...
(define-non-fungible-token wpa uint)
...
(define-data-var ipfs-full-metadata (string-ascii 500) "https://gateway.pinata.cloud/ipfs/QmU6q2RuNgWF6LR4svuKUYdnVqTBbjdjCt4d7AWbL8JJSG")
(define-data-var ipfs-root (string-ascii 120) "https://gateway.pinata.cloud/ipfs/QmQMeMepQVyHzERfKtBwWGMNMU3aAizkSwD6wVkw7znoeD/wolf_pack_{token-id}")
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? wpa next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? wpa token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
