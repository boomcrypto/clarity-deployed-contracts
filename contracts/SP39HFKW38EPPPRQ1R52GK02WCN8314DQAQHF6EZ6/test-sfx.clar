...
(define-data-var commission-per-mint uint u1000000)
...
(define-data-var ipfs-root (string-ascii 120) "https://gateway.pinata.cloud/ipfs/QmQMeMepQVyHzERfKtBwWGMNMU3aAizkSwD6wVkw7znoeD/wolf_pack_")
(define-data-var artist-address principal 'SP2B7QGY4T7NAJKG5YK5WEQFCCEHAJA5AH9MK0Q0)
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? test-sfx-nft next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? test-sfx-nft token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? test-sfx-nft token-id)))
