...
(define-non-fungible-token wolfy-nft uint)
...
(define-data-var ipfs-full-metadata (string-ascii 500) "QmQMeMepQVyHzERfKtBwWGMNMU3aAizkSwD6wVkw7znoeD")
(define-data-var ipfs-root (string-ascii 120) "QmQMeMepQVyHzERfKtBwWGMNMU3aAizkSwD6wVkw7znoeD")
  ...
  (let (
      ...
      (asserts! (< count MINT-LIMIT) (err ERR-ALL-MINTED))
        (match (stx-transfer? (var-get cost-per-mint) tx-sender (as-contract tx-sender))
          success (begin
            (try! (nft-mint? wolfy-nft next-id new-owner))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? wolfy-nft token-id sender recipient)
        success (ok success)
        error (err error))
      (err u500)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? wolfy-nft token-id)))
