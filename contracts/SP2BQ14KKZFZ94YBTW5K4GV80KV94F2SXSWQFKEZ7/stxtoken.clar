...
(define-non-fungible-token stxtoken uint)
...
(define-data-var artist-address principal 'SP1SP0QKWW45DVA3H7KTWDVCQXTH0HM1DS5GCCTNH)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmcd89bjD2AcSoQU9ag5eQ4SjwA3h5YkLuk1PoX5Q8yUJc/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? stxtoken next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stxtoken token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stxtoken token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stxtoken token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stxtoken id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stxtoken id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stxtoken id) (err ERR-NOT-FOUND)))
