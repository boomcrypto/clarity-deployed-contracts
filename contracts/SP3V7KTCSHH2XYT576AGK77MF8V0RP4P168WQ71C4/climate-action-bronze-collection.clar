;; climate-action-bronze-collection
;; contractType: sf-editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token climate-action-bronze-collection uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTaApbBH6q19JjADqyrbrY1yigXU1xnKsccAsvLt2AtLB/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? climate-action-bronze-collection next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
     (begin 
        ...
        (nft-burn? climate-action-bronze-collection token-id tx-sender))
)


(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? climate-action-bronze-collection token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? climate-action-bronze-collection token-id)))
