;; climate-action-silver-collection
;; contractType: sf-editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token climate-action-silver-collection uint)

;; Constants
(define-constant DEPLOYER tx-sender)
(define-constant COMM u0)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWqTt28mpSryn27JeG3FoykzJrQL1DBFUD5AaqKbk9M3t/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? climate-action-silver-collection next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
     (begin 
        ...
        (nft-burn? climate-action-silver-collection token-id tx-sender))
)


(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? climate-action-silver-collection token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? climate-action-silver-collection token-id)))
