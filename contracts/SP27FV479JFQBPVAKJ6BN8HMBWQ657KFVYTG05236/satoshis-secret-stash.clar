;; satoshis-secret-stash
;; contractType: editions

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token satoshis-secret-stash uint)
...
(define-data-var artist-address principal 'SP27FV479JFQBPVAKJ6BN8HMBWQ657KFVYTG05236)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmasZyKf4yB4SuwiC9mVSNuFx688ya8pqvzFobHVUsdoHG/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? satoshis-secret-stash next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? satoshis-secret-stash token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? satoshis-secret-stash token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? satoshis-secret-stash token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? satoshis-secret-stash id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? satoshis-secret-stash id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? satoshis-secret-stash id) (err ERR-NOT-FOUND)))
