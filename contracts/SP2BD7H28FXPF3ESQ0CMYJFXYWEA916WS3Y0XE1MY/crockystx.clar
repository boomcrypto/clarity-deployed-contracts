...
(define-non-fungible-token crockystx uint)
...
(define-data-var artist-address principal 'SP2BD7H28FXPF3ESQ0CMYJFXYWEA916WS3Y0XE1MY)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXgzNH31wQ1w9eMfUkT584yLbqLQUYBJi2sefBoAsVRzN/json/")
...
(define-private (mint-for-many-iter (recipient principal) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? crockystx next-id tx-sender) next-id)
      (unwrap! (nft-transfer? crockystx next-id tx-sender recipient) next-id)
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? crockystx token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? crockystx token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? crockystx token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? crockystx id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? crockystx id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? crockystx id) (err ERR-NOT-FOUND)))
