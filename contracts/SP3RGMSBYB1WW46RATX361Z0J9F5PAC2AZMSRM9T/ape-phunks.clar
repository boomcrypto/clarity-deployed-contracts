...
(define-non-fungible-token ape-phunks uint)
...
(define-data-var artist-address principal 'SP3RGMSBYB1WW46RATX361Z0J9F5PAC2AZMSRM9T)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmd5zj6VhcjcUB9yoRnXVNhtzne8wfBizJaZ6ZcAvSFxDR/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ape-phunks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? ape-phunks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? ape-phunks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ape-phunks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? ape-phunks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? ape-phunks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? ape-phunks id) (err ERR-NOT-FOUND)))
