...
(define-non-fungible-token space-war uint)
...
;; Internal variables
(define-data-var mint-limit uint u242)
(define-data-var last-id uint u1)
(define-data-var total-price uint u4500000)
(define-data-var artist-address principal 'SP3AWV304NKJWC82W7S9HFJ3MK9CP0BE4QE4G91BW)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSoaFiyjmqQJjQTsV1A3HHV3R3F79WGo5PFAcmByzKCkm/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? space-war next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? space-war token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? space-war token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? space-war token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? space-war id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? space-war id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? space-war id) (err ERR-NOT-FOUND)))
