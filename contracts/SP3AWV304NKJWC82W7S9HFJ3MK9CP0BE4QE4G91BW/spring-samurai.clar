...
(define-non-fungible-token spring-samurai uint)
...
;; Internal variables
(define-data-var mint-limit uint u2218)
(define-data-var last-id uint u1)
(define-data-var total-price uint u2000000)
(define-data-var artist-address principal 'SP3AWV304NKJWC82W7S9HFJ3MK9CP0BE4QE4G91BW)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmWmtUY6KdFEF3zEkEQTFsnMRCvqGEMzZqxrcjV69Q65oe/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? spring-samurai next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? spring-samurai token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? spring-samurai token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? spring-samurai token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? spring-samurai id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? spring-samurai id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? spring-samurai id) (err ERR-NOT-FOUND)))
