...
(define-non-fungible-token portrait-punk uint)
...
;; Internal variables
(define-data-var mint-limit uint u1495)
(define-data-var last-id uint u1)
(define-data-var total-price uint u1000000)
(define-data-var artist-address principal 'SP12N7VCDWBA1ZSHBQ0ZV2MKQSM8EBNRGEKQ9VM3Y)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYa8uo8KpkHaozsDHEF1gut5QTivsPvEE7XqDtQ7UtVsK/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? portrait-punk next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? portrait-punk token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? portrait-punk token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? portrait-punk token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? portrait-punk id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? portrait-punk id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? portrait-punk id) (err ERR-NOT-FOUND)))
