...
(define-non-fungible-token wizard uint)
...
(define-data-var artist-address principal 'SP3K95V9YEJ8C7ZSC5QNM4W8E9806R0HX1CJQW4B3)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmXbuCDG4Jf5f6rTQhQMACHYpoXLSNuqgGf2sfuHBMuW8G/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? wizard next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? wizard token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? wizard token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? wizard token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? wizard id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? wizard id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? wizard id) (err ERR-NOT-FOUND)))
