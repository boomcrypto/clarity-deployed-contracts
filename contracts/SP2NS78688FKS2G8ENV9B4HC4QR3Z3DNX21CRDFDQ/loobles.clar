...
(define-non-fungible-token loobles uint)
...
;; Internal variables
(define-data-var mint-limit uint u9900)
(define-data-var last-id uint u1)
(define-data-var total-price uint u7800000)
(define-data-var artist-address principal 'SP2NS78688FKS2G8ENV9B4HC4QR3Z3DNX21CRDFDQ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQaytvwDTZLkUtnxKH8XKzH2WbeWZUwhzyxSAUpjxDnvn/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? loobles next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? loobles token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? loobles token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? loobles token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? loobles id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? loobles id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? loobles id) (err ERR-NOT-FOUND)))
