...
(define-non-fungible-token skeleton-art uint)
...
;; Internal variables
(define-data-var mint-limit uint u270)
(define-data-var last-id uint u1)
(define-data-var total-price uint u10000000)
(define-data-var artist-address principal 'SPTA93QCSKZ45C508AAQ57KQMNQJKG2KXXE9G0M4)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQToBR6BcWfcvg1gP15Kw4UD2Pz9WApDPqXmgETgGBEkK/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? skeleton-art next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? skeleton-art token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? skeleton-art token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? skeleton-art token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? skeleton-art id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? skeleton-art id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? skeleton-art id) (err ERR-NOT-FOUND)))
