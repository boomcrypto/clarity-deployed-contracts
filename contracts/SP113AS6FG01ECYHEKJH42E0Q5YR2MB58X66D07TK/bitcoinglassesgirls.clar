...
(define-non-fungible-token bitcoinglassesgirls uint)
...
(define-data-var total-price uint u2000)
(define-data-var artist-address principal 'SP113AS6FG01ECYHEKJH42E0Q5YR2MB58X66D07TK)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUUcAwB4XA4cQH6ezfvuQSGaXt4A53JfgUtYyqYggUExB/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoinglassesgirls next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoinglassesgirls token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoinglassesgirls token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoinglassesgirls token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoinglassesgirls id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoinglassesgirls id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoinglassesgirls id) (err ERR-NOT-FOUND)))
