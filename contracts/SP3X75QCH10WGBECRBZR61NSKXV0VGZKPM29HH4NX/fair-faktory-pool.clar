...
;; Define LP token
(define-fungible-token sBTC-FAIR)
...
(define-public (transfer
  ...
  (begin
    ...
    (try! (ft-transfer? sBTC-FAIR amount sender recipient))
    (match memo
...
(define-read-only (get-balance (who principal))
  (ok (ft-get-balance sBTC-FAIR who))
)

(define-read-only (get-total-supply)
  (ok (ft-get-supply sBTC-FAIR))
)
...
(define-public (swap-a-to-b
  ...
  (let (
    ...
    (print {
      ...
      amount-in: amount,
      token-out: 'SP253J64EGMH59TV32CQXXTVKH5TQVGN108TA5TND.fair-bonding-curve,
      amount-out: dy-d,
...
(define-public (swap-b-to-a (amount uint) (min-y-out uint))
  (let (
    ...
    (print {
      type: "sell",
      sender: sender,
      token-in: 'SP253J64EGMH59TV32CQXXTVKH5TQVGN108TA5TND.fair-bonding-curve,
      amount-in: amount,
...
(define-public (add-liquidity (amount uint))
  (let (
    ...
    (try! (ft-mint? sBTC-FAIR dk-d sender))
    (print {
      type: "add-liquidity",
      sender: sender,
      token-a: 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token,
      token-a-amount: dx-d,
      token-b: 'SP253J64EGMH59TV32CQXXTVKH5TQVGN108TA5TND.fair-bonding-curve,
      token-b-amount: dy-d,
      lp-tokens: dk-d,
      pool-reserves: (get-reserves-quote),
      pool-contract: CONTRACT,
...
(define-public (remove-liquidity (amount uint))
  (let (
    ...
    (try! (ft-burn? sBTC-FAIR dk-d sender))
    (try! (as-contract (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
...
(define-read-only (get-reserves-quote)
  (let (
      (reserves (get-reserves))
      (supply (ft-get-supply sBTC-FAIR))
    )
    {
      dx: (get a reserves),
      dy: (get b reserves),
...
(begin
  ...
  (try! (add-liquidity u656600))
  ;; Transfer additional token B to achieve desired ratio
  (try! (contract-call?
    'SP253J64EGMH59TV32CQXXTVKH5TQVGN108TA5TND.fair-bonding-curve
    transfer u499999999343400 tx-sender CONTRACT none
  ))
  (print {
