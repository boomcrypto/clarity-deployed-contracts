...
(define-non-fungible-token nothing uint)
...
(define-data-var artist-address principal 'SP2BB6J0HK5D4Y75HHMEMZ2Q1ZS7N0AK1APJNTK4J)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmU4GZFLB9m2qvUYejS9bCUr4Hgg3rkmWZKpzCNoJJCP7V/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? nothing next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? nothing token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? nothing token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? nothing token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? nothing id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? nothing id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? nothing id) (err ERR-NOT-FOUND)))
