...
(define-constant CLIENT 0xb81c35d81b97d8d633402cbac94362361cd8e2e6ec89319d90f02e802cae7245)
(use-trait wrapper-trait 'SP2C32VK7C5MS1SA6ASDB2JV3J751DH8BN09BFV10.cf-traits-v0.wrapper-trait)
(use-trait token-trait 'SP2C32VK7C5MS1SA6ASDB2JV3J751DH8BN09BFV10.sip-010-trait-ft-standard.sip-010-trait)
...
(define-public (execute-transaction (user-id (string-ascii 64))
    ...
    (let
        (
            ...
            (user (unwrap! (contract-call? 'SP2C32VK7C5MS1SA6ASDB2JV3J751DH8BN09BFV10.cf-helpers-state-v0 get-user CLIENT user-id) ERR_INVALID_USER))
            ;; Fetch & check for valid policy
            (policy (unwrap! (contract-call? 'SP2C32VK7C5MS1SA6ASDB2JV3J751DH8BN09BFV10.cf-helpers-state-v0 get-policy CLIENT policy-id) ERR_INVALID_POLICY))
...
(define-public (execute-deposit (policy-id (string-ascii 64))
                                (amount uint)
                                (token <token-trait>)
                                (auth-id (string-ascii 64))
                                (signed-data (list 35 {signer: (buff 33), signature: (buff 65)})))
    (let
        ...
        (asserts! (is-eq (contract-of token) (get token transfer-policy)) ERR_INVALID_TOKEN)
        ...
        ;; Check that the policy type is "Crypto Onramp"
        (asserts! (is-eq (get type policy) "Crypto Onramp") ERR_INVALID_TYPE)

        ;; Check that all signatures are valid & all signers are in the policy
        (try! (verify-transfer-signature policy-id (get signers policy) "Crypto Onramp" amount (contract-of token) (as-contract tx-sender) auth-id signed-data))
...
(define-private (verify-transaction-signature (policy-id (string-ascii 64))
    ...
    (begin
        ...
        (asserts! (is-none (contract-call? 'SP2C32VK7C5MS1SA6ASDB2JV3J751DH8BN09BFV10.cf-helpers-state-v0 get-used-auth-ids CLIENT auth-id)) ERR_AUTH_ID_REPLAY)
        ;; Check that all signatures are valid & all signers are in the policy
        ...
        ;; update auth-id
        (unwrap! (contract-call? 'SP2C32VK7C5MS1SA6ASDB2JV3J751DH8BN09BFV10.cf-helpers-state-v0 set-auth-id CLIENT "users" auth-id) ERR_AUTH_ID_REPLAY)
        (ok true)
    )
)
...
(contract-call? 'SP2C32VK7C5MS1SA6ASDB2JV3J751DH8BN09BFV10.cf-helpers-state-v0 new-client CLIENT 0x5e27eee82dc48c8edd36e0f222b8e49760567429346046c644357e83337246d0 "2ac67676-4ce8-4909-8c3f-ff50c8117601")
