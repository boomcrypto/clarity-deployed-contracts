...
(define-non-fungible-token rare-turtle-club uint)
...
(define-data-var artist-address principal 'SP10G80A1E720FNHBTRGDAPJ7EEDDTAHCT15CHT2D)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbMncAv3rPTcr2oMMm9sddR7wycweFNA1FZCq67ZMNKXW/json/")
(define-data-var mint-paused bool true)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u250)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? rare-turtle-club next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? rare-turtle-club token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? rare-turtle-club token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? rare-turtle-club token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? rare-turtle-club id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? rare-turtle-club id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? rare-turtle-club id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-banana uint u106000000)
