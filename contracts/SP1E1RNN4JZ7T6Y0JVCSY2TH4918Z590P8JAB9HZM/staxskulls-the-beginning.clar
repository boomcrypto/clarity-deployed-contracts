;; staxskulls-the-beginning
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token staxskulls-the-beginning uint)
...
;; Internal variables
(define-data-var mint-limit uint u903)
(define-data-var last-id uint u1)
(define-data-var total-price uint u9750000)
(define-data-var artist-address principal 'SP1E1RNN4JZ7T6Y0JVCSY2TH4918Z590P8JAB9HZM)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcqL6VpB7nXfT4wufi8nZqy42CmZj656b3MaWAYo6T8w1/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? staxskulls-the-beginning next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? staxskulls-the-beginning token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? staxskulls-the-beginning token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? staxskulls-the-beginning token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? staxskulls-the-beginning id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? staxskulls-the-beginning id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? staxskulls-the-beginning id) (err ERR-NOT-FOUND)))
