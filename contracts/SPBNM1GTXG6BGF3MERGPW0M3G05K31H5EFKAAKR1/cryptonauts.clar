...
(define-non-fungible-token cryptonauts uint)
...
(define-data-var artist-address principal 'SPBNM1GTXG6BGF3MERGPW0M3G05K31H5EFKAAKR1)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmTLZXAN6xLC6h4zfe7zDTmTb2ZsjncjGdfvQ9owEptW5F/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? cryptonauts next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? cryptonauts token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? cryptonauts token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? cryptonauts token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? cryptonauts id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? cryptonauts id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? cryptonauts id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u79066)
