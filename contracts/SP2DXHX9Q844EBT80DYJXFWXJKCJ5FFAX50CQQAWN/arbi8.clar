...
(define-public (stx-usda-alex-arkadiko (spend uint) (min-receive uint))
    (begin 
        ...
        (let 
            (
                (balance-before (stx-get-balance tx-sender))
                (swapped (unwrap-panic (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.token-wusda get-balance tx-sender)))
            )
            (try! (contract-call? 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.arkadiko-swap-v2-1 swap-x-for-y 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.usda-token 'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR.wrapped-stx-token (eight-to-six swapped) u0))
            (let 
                (
                    (balance-after (stx-get-balance tx-sender))
                    (recevied (- balance-after balance-before))
