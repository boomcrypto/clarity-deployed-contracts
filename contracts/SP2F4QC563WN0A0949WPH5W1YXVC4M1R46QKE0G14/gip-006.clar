...
(define-public (execute (sender principal) (opt uint))
  (begin
    ...
    (try! (contract-call? .memegoat-community-dao set-extensions 
      (list 
        {extension: .memegoat-stake-pools, enabled: false}
        {extension: .memegoat-community-pools-v1, enabled: true}
      )
    ))

    (try! (contract-call? .memegoat-community-pools-v1 move-pools 
      (list u0 u1 u2 u3 u4 u5 u6 u7 u8 u9 u10 u11 u12 u13 u14 u15 u16 u17 u18)
    ))
    
    (try! (contract-call? .memegoat-community-pools-v1 move-user-records
      u3 
      (list 
