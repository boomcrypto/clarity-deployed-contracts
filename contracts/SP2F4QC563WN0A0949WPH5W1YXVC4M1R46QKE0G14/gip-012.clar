...
(define-public (execute (sender principal) (opt uint))
  (begin
    ...
    (try! (contract-call? .memegoat-community-dao set-extensions 
      (list 
        {extension: .memegoat-dao-operator, enabled: false}
        {extension: .memegoatstx-dao-operator, enabled: true}
      )
    ))
    (try! (contract-call? .memegoatstx-dao-operator set-operators 
      (list 
    ...
    (try! (contract-call? .memegoatstx-dao-operator set-approval-threshold 2))
    (try! (contract-call? .memegoatstx-dao-operator set-proposal-duration u432))
    (ok true)
  )
)
