...
(define-non-fungible-token bpt uint)
...
(define-data-var artist-address principal 'SP30TSNJ07SBXE9J3QVAA2QB6X2BP5GEME2B9ZX33)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYiTRQgD4npnkKYNMYWYzHm45NTvm13hjp4tH4ugh1yhj/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bpt next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bpt token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bpt token-id) false)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bpt id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bpt id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bpt id) (err ERR-NOT-FOUND)))
...
(map-set mint-passes 'SP30TSNJ07SBXE9J3QVAA2QB6X2BP5GEME2B9ZX33 u2)
