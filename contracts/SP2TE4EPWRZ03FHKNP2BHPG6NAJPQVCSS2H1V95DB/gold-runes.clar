...
(define-non-fungible-token gold-runes uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmR88wrwHUoL7LJirEMHU3sL3PjGrigcCRjN6YB4ADh8u9/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? gold-runes next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gold-runes token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gold-runes token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gold-runes token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gold-runes id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gold-runes id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gold-runes id) (err ERR-NOT-FOUND)))
