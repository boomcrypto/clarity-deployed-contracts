...
(define-non-fungible-token DRONES uint)
...
(define-data-var base-uri (string-ascii 80) "https://dronewars.xyz/metadata/DRONES/")
...
(define-public (transfer
  ...
  (let (
    ...
    (nft-transfer? DRONES id sender recipient)
  )
)
...
(define-public (admin-change-wl-stx-price (price uint)) 
  (begin 
    ;; Only the contract owner can start the mint.
    (asserts! (is-admin) ERR_OWNER_ONLY)
    (asserts! (> price u0 ) ERR_OUT_OF_RANGE)
    (var-set WL_PRICE_STX price)
    (print {
        topic: "new WL stx price",
        price: price
    })
    (ok true)
  )
...
(define-private (admin-airdrop (recipient principal)) 
  ...
  (let ((id (+ (var-get last-token-id) u1)) )
    ...
    (try! (nft-mint? DRONES id recipient))
...
(define-private (al-mint (token <sip-010-trait>) (price uint))
  ...
  (let (
    ...
    (asserts! (< (var-get last-token-id) (var-get COLLECTION_LIMIT)) ERR_SOLD_OUT)
    ;; Mint the NFT and send it to the given recipient.
    (try! (nft-mint? DRONES id contract-caller))
    ;; Update the last minted token ID.
    (var-set last-token-id id)
    ;; Print mint information
    (print {
...
(define-public (buy-in-ustx (id uint) (commission <commission-trait>) )
  (let (
    ...
    (try! (nft-transfer? DRONES id owner tx-sender))
    (map-delete MARKET id)
    ;; Increase the wallet balance
...
(admin-add-token 'SP1JBV7TE0490KNVM1VAM19KHZG0CPC9426YCY3ZF.drones-stxcity u7000000000 "AL")
(admin-add-token 'SP1JBV7TE0490KNVM1VAM19KHZG0CPC9426YCY3ZF.drones-stxcity u7500000000 "WL")
(admin-add-token 'SP1JBV7TE0490KNVM1VAM19KHZG0CPC9426YCY3ZF.drones-stxcity u8000000000 "PU")
(admin-add-token 'SP3HNEXSXJK2RYNG5P6YSEE53FREX645JPJJ5FBFA.salt u100000 "AL")
(admin-add-token 'SP3HNEXSXJK2RYNG5P6YSEE53FREX645JPJJ5FBFA.salt u110000 "WL")
(admin-add-token 'SP3HNEXSXJK2RYNG5P6YSEE53FREX645JPJJ5FBFA.salt u130000 "PU")