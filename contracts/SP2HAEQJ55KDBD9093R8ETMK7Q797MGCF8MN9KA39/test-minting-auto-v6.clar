...
(define-public (mint
  ...
  (let (
    ...
    (asserts! (>= amount-usdh-requested (contract-call? .test-minting-state get-min-amount-usdh-requested)) ERR_BELOW_MIN)
...
(define-public (redeem
  ...
  (let (
    ...
    (try! (contract-call? .test-redeeming-reserve-v3 transfer (- amount-asset-required fee-amount) contract-caller redeeming-asset memo))

    (if (> fee-amount u0)
      (try! (contract-call? .test-redeeming-reserve-v3 transfer fee-amount (get fee-address state) redeeming-asset none))
      true
    )
