...
(define-public (re-price (id uint) (ft <fungible-token>) (fees <fees-trait>) (amount uint) (expiry (optional uint)) (ft-sender (optional principal)))
  (let ((swap (unwrap! (map-get? swaps id) ERR_INVALID_ID))
    (ustx (get ustx swap)))
      (asserts! (is-eq (contract-of ft) (get ft swap)) ERR_INVALID_FUNGIBLE_TOKEN)
      (asserts! (is-eq (contract-of fees) (get fees swap)) ERR_INVALID_FEES_TRAIT)
      ...
      (match expiry
            some-expiry (begin 
                        (asserts! (map-set swaps id (merge swap {amount: amount, expired-height: (some (+ burn-block-height some-expiry)), ft-sender: ft-sender})) ERR_NATIVE_FAILURE)
                        (print 
                        {
                            type: "re-price",
                            swap-id: id, 
            ...
            (begin
            (asserts! (map-set swaps id (merge swap {amount: amount, expired-height: none, ft-sender: ft-sender})) ERR_NATIVE_FAILURE)
                        (print 
                        {
                            type: "re-price",
                            swap-id: id, 
