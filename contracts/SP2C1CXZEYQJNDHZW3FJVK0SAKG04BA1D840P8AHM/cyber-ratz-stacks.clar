...
(define-non-fungible-token cyber-ratz-stacks uint)
...
;; Internal variables
(define-data-var mint-limit uint u172)
(define-data-var last-id uint u1)
(define-data-var total-price uint u2000000)
(define-data-var artist-address principal 'SP2C1CXZEYQJNDHZW3FJVK0SAKG04BA1D840P8AHM)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRweFQCrwUreXBCCzACrVcDsqo6RaAHo1z8nzNdiHnkAL/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? cyber-ratz-stacks next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? cyber-ratz-stacks token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? cyber-ratz-stacks token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? cyber-ratz-stacks token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? cyber-ratz-stacks id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? cyber-ratz-stacks id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? cyber-ratz-stacks id) (err ERR-NOT-FOUND)))
