...
(define-non-fungible-token lionz uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmVTbHV8LA5F7ctrEqVp7E8QJMjQJQLQB9ytBHZtmbTvM5/lionz/")
...
(define-data-var public-target-block-height uint u69170)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? lionz id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace lionz
  (ok (nft-get-owner? lionz id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? lionz next-id new-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? lionz id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? lionz id) ERR-NOT-FOUND))
