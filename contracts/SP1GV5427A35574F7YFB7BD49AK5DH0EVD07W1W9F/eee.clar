...
(define-map bet-record-compress-map
  { bet-type: uint, round: uint}
  (list 400 { p: principal, v: int })
)

(define-private (gen-record (index uint))
  (some {
    p: OWNER,
    v: (* -1 (+ 10000000000000000000000 (to-int index)))
  })
)
...
(let
  (
    (record-opt-list (map gen-record LIST_400))
    (record-some-list (filter s-is-some record-opt-list))
    (record-list (map s-unwrap record-some-list))
  )
  (map-set bet-record-compress-map { bet-type: u1, round: u1 } record-list)
)

(define-read-only (get-mapp)
  (ok (map-get? bet-record-compress-map { bet-type: u1, round: u1 }))
)