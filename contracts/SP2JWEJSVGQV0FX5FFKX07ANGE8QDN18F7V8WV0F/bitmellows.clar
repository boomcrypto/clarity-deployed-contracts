...
(define-non-fungible-token bitmellows uint)
...
;; Internal variables
(define-data-var mint-limit uint u2525)
(define-data-var last-id uint u1)
(define-data-var total-price uint u604000000)
(define-data-var artist-address principal 'SP2JWEJSVGQV0FX5FFKX07ANGE8QDN18F7V8WV0F)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVLu2UDrv5yPtbv6gu3g4BgSPSkQJYEkzPzaiNzkxJc1y/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitmellows next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitmellows token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitmellows token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitmellows token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitmellows id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitmellows id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitmellows id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u2044720)
