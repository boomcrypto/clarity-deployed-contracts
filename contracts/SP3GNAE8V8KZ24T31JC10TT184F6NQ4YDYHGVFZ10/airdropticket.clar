...
(define-non-fungible-token airdropticket uint)
...
(define-data-var public-target-block-height uint u0) ;;set these at time of deployment
(define-data-var sale-active bool true)
(define-data-var metadata-frozen bool false)
(define-data-var base-uri (string-ascii 100) "ipfs://QmY7prwodjx2NjjXAnPLcQ2eu64K8H1Bru8o575vTiU2p4/airdropticket/")
...
(define-read-only (get-public-balance (account principal))
  (- u1 (get-minted account))
)

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? airdropticket id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace airdropticket
  (ok (nft-get-owner? airdropticket id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (asserts! (> minted-check u0) ERR-ONE-MINT-PER-WALLET)
      (asserts! (>= block start) ERR-BEFORE-MINT-TIME)
      (match (nft-mint? airdropticket next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? airdropticket id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? airdropticket id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? airdropticket id) ERR-NOT-FOUND))
...
(define-public (admin-mint-and-burn)
    (let (
      ...
      (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
      (asserts! (< (var-get last-id) MINT-LIMIT) ERR-SOLD-OUT)
      (match (nft-mint? airdropticket next-id (as-contract tx-sender))
        success
        (let
        ((current-balance (get-balance (as-contract tx-sender))))
          (begin
            (var-set last-id next-id)
            (try! (nft-burn? airdropticket next-id (as-contract tx-sender)))
            (ok true)))
        error (err (* error u10000)))
    )
)
