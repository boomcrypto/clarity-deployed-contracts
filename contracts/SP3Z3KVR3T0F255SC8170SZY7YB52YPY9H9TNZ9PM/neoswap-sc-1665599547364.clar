...
(begin
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u975097 tx-sender agent-8)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u50006 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u1025103 tx-sender agent-1)))
	)
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SP213KNHB5QD308TEESY1ZMX1BP8EZDPG4JWD0MEA.web4 transfer u2982 tx-sender agent-8)))
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u1025103 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
