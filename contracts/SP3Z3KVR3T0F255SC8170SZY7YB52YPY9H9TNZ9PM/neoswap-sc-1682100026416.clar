...
;; Room: https://neoswap.xyz/rooms/Gg8vZUVpDdBa0QzifCZJ
;; SwapId: 60itDb40W7jczaIO4mW4

;; traders
(define-constant TRADER-1 'SP1Q6N226KFMA496MVWSB0VZC0T0R9FSKTYYYZ403)
(define-constant TRADER-2 'SP2NS78688FKS2G8ENV9B4HC4QR3Z3DNX21CRDFDQ)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u2150000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP2NS78688FKS2G8ENV9B4HC4QR3Z3DNX21CRDFDQ.bottled transfer u2 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (contract-call? 'SP2NS78688FKS2G8ENV9B4HC4QR3Z3DNX21CRDFDQ.bottled transfer u2 tx-sender TRADER-3)))

	(unwrap-panic (as-contract (stx-transfer? u200000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u1000 tx-sender TRADER-2)))
	(unwrap-panic (as-contract (stx-transfer? u1949000 tx-sender TRADER-3)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u2150000 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2NS78688FKS2G8ENV9B4HC4QR3Z3DNX21CRDFDQ.bottled transfer u2 tx-sender TRADER-2)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-3)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.citadels transfer u2020 tx-sender TRADER-3)))
	    )
            true
        )
