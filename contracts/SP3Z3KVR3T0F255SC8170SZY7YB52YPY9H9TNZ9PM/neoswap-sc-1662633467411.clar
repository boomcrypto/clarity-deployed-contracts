...
(define-constant agent-1 'SPZ5DJGRVZHXEEEYYGWEX84KQB8P69GC715ZRNW1)
...
(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u4645883 tx-sender agent-1)))
	)
	(unwrap-panic (begin
		...
		(as-contract (stx-transfer? u2810000 tx-sender agent-2)))
	)
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u42934706 tx-sender agent-5)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u7650000 tx-sender agent-6)))
	)
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u1580000 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
		...
		(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.byzantion-matryoshka-dolls transfer u50 tx-sender agent-5)))
	...
	(unwrap-panic (begin
		...
		(as-contract (stx-transfer? u18570589 tx-sender agent-8)))
	)
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u18570589 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
