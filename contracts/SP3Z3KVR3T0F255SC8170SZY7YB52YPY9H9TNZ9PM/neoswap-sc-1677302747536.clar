...
(define-constant TRADER-4 'SP3WFDD787ES26F96H67T7CNC6D8QZ9X6KM5YBVJK)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
		...
		(unwrap! (contract-call? 'SP3X1CG6HJQWTHZ85DQVES9WMSGCNYASCZ20VVGPW.bitcoin-owls transfer u30 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u6376000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	...
	(if (is-eq tx-sender TRADER-3)
            (begin
		(unwrap! (stx-transfer? u1000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-4)
            (begin
		(unwrap! (contract-call? 'SP1XY24C7AJ8XZ2QQ5BMD43YWJBYAM388G00P354V.bitcoin-army transfer u11 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-5)
            (begin
		(unwrap! (stx-transfer? u8600000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP1XY24C7AJ8XZ2QQ5BMD43YWJBYAM388G00P354V.bitcoin-army transfer u11 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (contract-call? 'SP19WKA4H1ZVCWRCY82SPGJHJ3GXD02D60YQT7Y7H.fox-finds-clothes-and-shares transfer u9 tx-sender TRADER-2)))
	(unwrap-panic (as-contract (contract-call? 'SP3X1CG6HJQWTHZ85DQVES9WMSGCNYASCZ20VVGPW.bitcoin-owls transfer u30 tx-sender TRADER-3)))
	(unwrap-panic (as-contract (contract-call? 'SP3QWNA75CY7QAK7S9XG7T258KSVQE1DW4HGTVRA3.king-katz transfer u193 tx-sender TRADER-5)))

	(unwrap-panic (as-contract (stx-transfer? u7825000 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (stx-transfer? u2152000 tx-sender RECEIVER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
		...
		(unwrap-panic (as-contract (contract-call? 'SP3X1CG6HJQWTHZ85DQVES9WMSGCNYASCZ20VVGPW.bitcoin-owls transfer u30 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u6376000 tx-sender TRADER-2)))
		(unwrap-panic (as-contract (contract-call? 'SP3QWNA75CY7QAK7S9XG7T258KSVQE1DW4HGTVRA3.king-katz transfer u193 tx-sender TRADER-2)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-3)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u1000 tx-sender TRADER-3)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-4)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP1XY24C7AJ8XZ2QQ5BMD43YWJBYAM388G00P354V.bitcoin-army transfer u11 tx-sender TRADER-4)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-5)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u8600000 tx-sender TRADER-5)))
	    )
            true
        )
