...
(define-constant agent-2 'SP2BFJNNYGJW2X7KHVJ8M11A4D0Z8MKN4X9BXGWCT)
...
(begin
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u870770 tx-sender agent-2)))
	)
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u544654 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u11415424 tx-sender agent-3)))
	)
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u11415424 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
