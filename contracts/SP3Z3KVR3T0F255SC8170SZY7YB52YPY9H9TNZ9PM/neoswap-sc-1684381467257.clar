...
;; Room: https://neoswap.xyz/rooms/x9WN44s3BddW7wmssL2V
;; SwapId: jNpCq9PYK68hb8WREhkq

;; traders
(define-constant TRADER-1 'SP2Z7K9Q3GT5CHPNJHYXZ84FYWK5RXYNJ5HCMX8Y)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u780691 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u780691 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u780691 tx-sender TRADER-2)))
