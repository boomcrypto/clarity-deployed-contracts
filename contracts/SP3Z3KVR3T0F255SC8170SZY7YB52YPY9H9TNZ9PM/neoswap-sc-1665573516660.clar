...
(begin
	...
	(unwrap-panic (as-contract (contract-call? 'SP125J1ADVYWGWB9NQRCVGKYAG73R17ZNMV17XEJ7.mutant-monkeys transfer u1705 tx-sender agent-1)))
	(unwrap-panic (begin
		(as-contract (stx-transfer? u3066472 tx-sender agent-7)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u78414623 tx-sender agent-8)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u4082362 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u85563457 tx-sender agent-1)))
	)
	...
	(begin
		...
		(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.citadels transfer u604 tx-sender agent-7)))
	(var-set agent-7-status false))
	true)
	(if (is-eq (var-get agent-8-status) true)
	(begin
		(unwrap-panic (as-contract (contract-call? 'SP125J1ADVYWGWB9NQRCVGKYAG73R17ZNMV17XEJ7.mutant-monkeys transfer u1705 tx-sender agent-8)))
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u85563457 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
		...
		(asserts! (is-ok (contract-call? 'SP125J1ADVYWGWB9NQRCVGKYAG73R17ZNMV17XEJ7.mutant-monkeys transfer u1705 tx-sender (as-contract tx-sender))) cannot-escrow-nft )
