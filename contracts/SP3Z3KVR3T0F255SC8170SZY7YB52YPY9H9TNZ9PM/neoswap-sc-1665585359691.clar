...
(define-constant agent-5 'SP2DFX28F1S3CB46B5XH9M5JQ7N4SMCE7CQY1TNYS)
...
(begin
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u2409015 tx-sender agent-5)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u55000000 tx-sender agent-8)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u2873540 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u60282555 tx-sender agent-1)))
	)
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u60282555 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
