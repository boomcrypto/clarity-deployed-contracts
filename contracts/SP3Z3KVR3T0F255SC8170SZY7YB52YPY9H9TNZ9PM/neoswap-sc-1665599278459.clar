...
(begin
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u1692104 tx-sender agent-8)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u244028 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u5002604 tx-sender agent-1)))
	)
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.stx-jokers transfer u735 tx-sender agent-8)))
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u5002604 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
