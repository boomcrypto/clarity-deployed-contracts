...
(define-constant agent-3 'SPGTN4T2DP2PQHYNYA3H4P7J8S1J6E5DK202F2C8)
...
(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u721692 tx-sender agent-1)))
	)
	(unwrap-panic (as-contract (contract-call? 'SPQZF23W7SEYBFG5JQ496NMY0G7379SRYEDREMSV.Candy transfer u1116 tx-sender agent-2)))
	(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.hback-whales-nft transfer u811 tx-sender agent-2)))
	(unwrap-panic (begin
		(as-contract (stx-transfer? u37735850 tx-sender agent-3)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u1931160 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u40388702 tx-sender agent-2)))
	)
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.hback-whales-nft transfer u811 tx-sender agent-3)))
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u40388702 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
