...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u657 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u479343 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-robot-component-nft transfer u19187 tx-sender TRADER-2)))
	    )
            true
        )
