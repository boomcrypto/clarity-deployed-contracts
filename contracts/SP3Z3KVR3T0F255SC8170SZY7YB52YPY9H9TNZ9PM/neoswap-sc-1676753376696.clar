...
;; traders
(define-constant TRADER-1 'SP1EYHN37P81RMB4GKP6KT5XT8Y8NWWNG8AY35AZ4)
(define-constant TRADER-2 'SP3PK37H2EA79H6PGZJ9V93MM3NZRJ0XC9QB8T4FD)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (contract-call? 'SP3NYR87HRE7PHXJBP6VBAKSWZ98PJKN7WENAVT3Z.btc-pizzas transfer u3 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u199000000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP3NYR87HRE7PHXJBP6VBAKSWZ98PJKN7WENAVT3Z.btc-pizzas transfer u3 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u189050000 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (stx-transfer? u9950000 tx-sender RECEIVER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP3NYR87HRE7PHXJBP6VBAKSWZ98PJKN7WENAVT3Z.btc-pizzas transfer u3 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u199000000 tx-sender TRADER-2)))
	    )
            true
        )
