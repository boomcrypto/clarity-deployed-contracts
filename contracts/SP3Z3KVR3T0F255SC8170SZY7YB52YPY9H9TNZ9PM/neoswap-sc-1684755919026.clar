...
;; Room: https://neoswap.xyz/rooms/LyEqh0e5SqHBmurGoRxf
;; SwapId: kMxuEfkGPxuJDicRbbNb
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u3628037 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP3X1CG6HJQWTHZ85DQVES9WMSGCNYASCZ20VVGPW.ordinal-punks-pirates transfer u58 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP3X1CG6HJQWTHZ85DQVES9WMSGCNYASCZ20VVGPW.ordinal-punks-pirates transfer u58 tx-sender TRADER-1)))
	...
	(unwrap-panic (as-contract (stx-transfer? u3628037 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u3628037 tx-sender TRADER-1)))
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP3X1CG6HJQWTHZ85DQVES9WMSGCNYASCZ20VVGPW.ordinal-punks-pirates transfer u58 tx-sender TRADER-2)))
	    )
            true
        )
