...
(define-constant agent-8 'SP3K22XKPT9WJFCE957J94J6XXVZHP7747YNPDTFD)
...
(begin
	...
	(unwrap-panic (as-contract (contract-call? 'SP2RFR92YDK6JBZQ9FTKYBYSQWEW447N9N6SZKFSV.vaping-ape transfer u229 tx-sender agent-1)))
	...
	(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.rehab-resort transfer u26 tx-sender agent-4)))
	(unwrap-panic (begin
		(as-contract (stx-transfer? u30000000 tx-sender agent-8)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u3194020 tx-sender agent-0)))
	)
...
(begin        
	...
	(unwrap-panic (begin
		...
		(as-contract (stx-transfer? u31597403 tx-sender agent-1)))
	)
	...
	(unwrap-panic (begin
		...
		(as-contract (stx-transfer? u1596617 tx-sender agent-4)))
	)
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SP2RFR92YDK6JBZQ9FTKYBYSQWEW447N9N6SZKFSV.vaping-ape transfer u229 tx-sender agent-8)))
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u31597403 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
		...
		(asserts! (is-ok (stx-transfer? u1596617 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
		...
		(asserts! (is-ok (contract-call? 'SP2RFR92YDK6JBZQ9FTKYBYSQWEW447N9N6SZKFSV.vaping-ape transfer u229 tx-sender (as-contract tx-sender))) cannot-escrow-nft )
