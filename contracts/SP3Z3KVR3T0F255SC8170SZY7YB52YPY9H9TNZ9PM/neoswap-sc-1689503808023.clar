...
;; Room: https://neoswap.xyz/rooms/Ai76xDO5hnHmIgELD7ed
;; SwapId: G4d1tZ07jUEF3XrYYRIZ
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u44800000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP3BRRCHKMPBR60V8ES9J5YF40VXWMABWXK4SEB9G.loop-isle transfer u25 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP3BRRCHKMPBR60V8ES9J5YF40VXWMABWXK4SEB9G.loop-isle transfer u25 tx-sender TRADER-1)))
	...
	(unwrap-panic (as-contract (stx-transfer? u44800000 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u44800000 tx-sender TRADER-1)))
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP3BRRCHKMPBR60V8ES9J5YF40VXWMABWXK4SEB9G.loop-isle transfer u25 tx-sender TRADER-2)))
	    )
            true
        )
