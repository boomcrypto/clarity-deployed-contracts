...
(begin
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u1453133 tx-sender agent-5)))
	)
	(unwrap-panic (begin
		(unwrap-panic (as-contract (contract-call? 'SP1MX3R99FENJJJVKW27DSA6X2M6SDSVJH4Y0HE23.pepepunks transfer u597 tx-sender agent-8)))
		(as-contract (stx-transfer? u642201 tx-sender agent-8)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u185794 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u2281128 tx-sender agent-3)))
	)
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u2281128 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
