...
(begin
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u3007500 tx-sender agent-2)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u1607500 tx-sender agent-3)))
	)
	(unwrap-panic (as-contract (contract-call? 'SPJJYJVZ4H7B34GG8D3SSN70WVWDYSHCC9E9ZV4V.bitcoin-toadz transfer u3511 tx-sender agent-4)))
	...
	(unwrap-panic (as-contract (contract-call? 'SP32AEEF6WW5Y0NMJ1S8SBSZDAY8R5J32NBZFPKKZ.free-punks-v0 transfer u1054 tx-sender agent-5)))
	(unwrap-panic (begin
...
(begin        
	...
	(unwrap-panic (begin
		...
		(as-contract (stx-transfer? u3152500 tx-sender agent-1)))
	)
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u1692500 tx-sender agent-4)))
	)
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u3152500 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
		...
		(asserts! (is-ok (stx-transfer? u1692500 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
