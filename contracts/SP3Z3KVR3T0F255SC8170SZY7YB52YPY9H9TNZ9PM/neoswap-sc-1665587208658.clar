...
(define-constant agent-3 'SP1JF9VSNJBP4YZVC7AJ9CE6CXBD2ZV0W67T0E4T0)
...
(begin
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u7216912 tx-sender agent-3)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u1898617 tx-sender agent-8)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u467464 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u9582993 tx-sender agent-2)))
	)
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SP1XJ6GNTDVF6HR1VHPQDMFZJY87D9W6TGMH3QCP.satoshi-city transfer u3 tx-sender agent-8)))
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u9582993 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
