...
;; traders
(define-constant TRADER-1 'SP28NCDY6V4T7NJBMYGTJ55NHMXMC0GG806JW1ZTB)
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SPJJYJVZ4H7B34GG8D3SSN70WVWDYSHCC9E9ZV4V.bitcoin-toadz transfer u6871 tx-sender TRADER-2)))
	(unwrap-panic (as-contract (contract-call? 'SPYHY9MV6S08YJQVW0R400ADXZBBJ0GM096BMY34.liquidium-early-access-ticket transfer u8 tx-sender TRADER-3)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SPYHY9MV6S08YJQVW0R400ADXZBBJ0GM096BMY34.liquidium-early-access-ticket transfer u8 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
		...
		(unwrap-panic (as-contract (contract-call? 'SPJJYJVZ4H7B34GG8D3SSN70WVWDYSHCC9E9ZV4V.bitcoin-toadz transfer u6871 tx-sender TRADER-3)))
	    )
            true
        )
