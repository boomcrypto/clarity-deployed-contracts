...
;; traders
(define-constant TRADER-1 'SPHK8A7P61C6ASWKYDX1PCDX9YA54DKVJN49EXGJ)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (contract-call? 'SP37PGRC42BRJ33HMC2SN4DT9QN8RHGC6AZ7DRGH9.stacks-checks transfer u159 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP37PGRC42BRJ33HMC2SN4DT9QN8RHGC6AZ7DRGH9.stacks-checks transfer u159 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP37PGRC42BRJ33HMC2SN4DT9QN8RHGC6AZ7DRGH9.stacks-checks transfer u159 tx-sender TRADER-1)))
	    )
            true
        )
