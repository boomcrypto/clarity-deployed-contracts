...
;; traders
(define-constant TRADER-1 'SP247RS63PWW7ZQZ9EYYA9CXKKPWEP71M14W8N294)
...
(define-constant TRADER-4 'SPP28QCMZ51GVN9933VFH9TXKQ3XCT4WE5A07533)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u28367000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
		(unwrap! (contract-call? 'SP2ABNX65BSKVM00ZQZ7K174DFV18CXVGGEMP7Y6X.weed-monsters transfer u60 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-3)
            (begin
		(unwrap! (contract-call? 'SP25VWGTPR19E344S4ENTHQT8651216EPNABRYE51.project-indigo-equipment transfer u281 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-4)
            (begin
		(unwrap! (contract-call? 'SPP28QCMZ51GVN9933VFH9TXKQ3XCT4WE5A07533.warrior-pigeons transfer u18 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
		(unwrap! (contract-call? 'SP1NKN5S805RDB78MMQKD26F00XEFE89X02DPSZW4.rugaritas transfer u27 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (contract-call? 'SP1NKN5S805RDB78MMQKD26F00XEFE89X02DPSZW4.rugaritas transfer u27 tx-sender TRADER-2)))
	(unwrap-panic (as-contract (contract-call? 'SPP28QCMZ51GVN9933VFH9TXKQ3XCT4WE5A07533.warrior-pigeons transfer u18 tx-sender TRADER-2)))
	(unwrap-panic (as-contract (contract-call? 'SP2ABNX65BSKVM00ZQZ7K174DFV18CXVGGEMP7Y6X.weed-monsters transfer u60 tx-sender TRADER-4)))
	(unwrap-panic (as-contract (contract-call? 'SP25VWGTPR19E344S4ENTHQT8651216EPNABRYE51.project-indigo-equipment transfer u281 tx-sender TRADER-4)))

	(unwrap-panic (as-contract (stx-transfer? u7714000 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (stx-transfer? u9000000 tx-sender TRADER-3)))
	(unwrap-panic (as-contract (stx-transfer? u11653000 tx-sender TRADER-4)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u28367000 tx-sender TRADER-2)))
		(unwrap-panic (as-contract (contract-call? 'SP2ABNX65BSKVM00ZQZ7K174DFV18CXVGGEMP7Y6X.weed-monsters transfer u60 tx-sender TRADER-2)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP25VWGTPR19E344S4ENTHQT8651216EPNABRYE51.project-indigo-equipment transfer u281 tx-sender TRADER-3)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SPP28QCMZ51GVN9933VFH9TXKQ3XCT4WE5A07533.warrior-pigeons transfer u18 tx-sender TRADER-4)))
		(unwrap-panic (as-contract (contract-call? 'SP1NKN5S805RDB78MMQKD26F00XEFE89X02DPSZW4.rugaritas transfer u27 tx-sender TRADER-4)))
	    )
            true
        )
