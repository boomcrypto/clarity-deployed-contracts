...
(begin
	(unwrap-panic (begin
		(unwrap-panic (as-contract (contract-call? 'SPNWZ5V2TPWGQGVDR6T7B6RQ4XMGZ4PXTEE0VQ0S.pox-monks transfer u79 tx-sender agent-3)))
		...
		(as-contract (stx-transfer? u3454127 tx-sender agent-3)))
	)
	...
	(unwrap-panic (begin
		(unwrap-panic (as-contract (contract-call? 'SP32AEEF6WW5Y0NMJ1S8SBSZDAY8R5J32NBZFPKKZ.free-punks-v0 transfer u1820 tx-sender agent-8)))
		(as-contract (stx-transfer? u1147599 tx-sender agent-8)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u705588 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(unwrap-panic (as-contract (contract-call? 'SPNWZ5V2TPWGQGVDR6T7B6RQ4XMGZ4PXTEE0VQ0S.pox-monks transfer u79 tx-sender agent-4)))
		(as-contract (stx-transfer? u5307314 tx-sender agent-4)))
	)
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SPXM7A1WWQPS3R504SNGXERRHEKHZEGQE40DJGJY.stockings transfer u251 tx-sender agent-8)))
		...
		(unwrap-panic (as-contract (contract-call? 'SPQZF23W7SEYBFG5JQ496NMY0G7379SRYEDREMSV.Candy transfer u878 tx-sender agent-8)))
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u5307314 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
