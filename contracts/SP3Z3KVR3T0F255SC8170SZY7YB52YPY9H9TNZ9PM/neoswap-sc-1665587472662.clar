...
(define-constant agent-3 'SP1DMPD0JNAVDRCTY17S2MNHX8F6502NB0Z25RVR)
...
(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u624292 tx-sender agent-3)))
	)
	...
	(unwrap-panic (begin
		(unwrap-panic (as-contract (contract-call? 'SP3QWNA75CY7QAK7S9XG7T258KSVQE1DW4HGTVRA3.king-katz transfer u115 tx-sender agent-8)))
		(as-contract (stx-transfer? u30677754 tx-sender agent-8)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u1638892 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u32940938 tx-sender agent-7)))
	)
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.stx-youth transfer u2735 tx-sender agent-8)))
		...
		(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.hback-whales-nft transfer u2162 tx-sender agent-8)))
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u32940938 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
