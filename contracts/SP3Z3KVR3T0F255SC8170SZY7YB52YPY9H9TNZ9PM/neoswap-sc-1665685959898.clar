...
(define-constant agent-2 'SP2846GM0SKSB3XJ57CVVWH4AX61YGXT0N2GKRZ6W)
...
(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u1913827 tx-sender agent-1)))
	)
	(unwrap-panic (begin
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u148146 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SP2EK5VZQKRR1WYQ3F8MH86QDRDEHMC5WY6E3KA34.flavored-pops transfer u5 tx-sender agent-2)))
	(var-set agent-2-status false))
	true)
	(if (is-eq (var-get agent-3-status) true)
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u3061973 tx-sender agent-3)))
	)
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u3061973 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
