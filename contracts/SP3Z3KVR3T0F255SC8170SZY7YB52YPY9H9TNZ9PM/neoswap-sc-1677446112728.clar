...
;; traders
(define-constant TRADER-1 'SP364J7EDJXRE1FPDZDABP9M58HPY4G88BFCP2HD0)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u333000000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.venice-visuals transfer u46 tx-sender TRADER-2)))

	(unwrap-panic (as-contract (stx-transfer? u11944045 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u321055955 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u333000000 tx-sender TRADER-2)))
	    )
            true
        )
