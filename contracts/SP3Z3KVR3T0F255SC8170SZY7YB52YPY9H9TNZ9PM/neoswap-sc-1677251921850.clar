...
(define-constant TRADER-6 'SP30AF1XADY530B2MTQRNP1NFDE22A4X51QZ056KV)
(define-constant TRADER-7 'SP3C5JYPB8YE5H9WC2SM196RVJ0JXN2GHCWXJWSES)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (stx-transfer? u527000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (contract-call? 'SP2W58YQZEAC870M2MKE6QEKS1DPG0RMZZ2BGXSM4.tickets-90stx-mini-raffle-26 transfer u87 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-3)
            (begin
		(unwrap! (stx-transfer? u14714000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
	...
	(if (is-eq tx-sender TRADER-5)
            (begin
		(unwrap! (stx-transfer? u2196000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	...
	(if (is-eq tx-sender TRADER-6)
            (begin
		(unwrap! (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-robot-component-nft transfer u22780 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-7)
            (begin
		(unwrap! (stx-transfer? u2629000 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
	    )
            true
        )
	(if (is-eq tx-sender TRADER-8)
            (begin
		(unwrap! (contract-call? 'SP2ZY7ETKYAN1M7R4HWQ77Q4CVDVH8PVQ41XS0N0S.unforgettable-pepe transfer u78 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
		(unwrap! (contract-call? 'SP1FWE300K4QMTNDPKV9YDX0NVJ99B138KQ3JN9DT.ukrainian-children transfer u13 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP2W58YQZEAC870M2MKE6QEKS1DPG0RMZZ2BGXSM4.tickets-90stx-mini-raffle-26 transfer u87 tx-sender TRADER-1)))
	(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-space-agency transfer u581 tx-sender TRADER-3)))
	(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-space-agency transfer u1511 tx-sender TRADER-3)))
	(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-robot-component-nft transfer u22780 tx-sender TRADER-5)))
	(unwrap-panic (as-contract (contract-call? 'SP2ZY7ETKYAN1M7R4HWQ77Q4CVDVH8PVQ41XS0N0S.unforgettable-pepe transfer u78 tx-sender TRADER-5)))
	(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.rehab-resort transfer u292 tx-sender TRADER-5)))
	(unwrap-panic (as-contract (contract-call? 'SP1FWE300K4QMTNDPKV9YDX0NVJ99B138KQ3JN9DT.ukrainian-children transfer u13 tx-sender TRADER-7)))

	(unwrap-panic (as-contract (stx-transfer? u475000 tx-sender TRADER-2)))
	(unwrap-panic (as-contract (stx-transfer? u3180000 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u11222000 tx-sender TRADER-4)))
	(unwrap-panic (as-contract (stx-transfer? u435000 tx-sender TRADER-6)))
	(unwrap-panic (as-contract (stx-transfer? u4754000 tx-sender TRADER-8)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u527000 tx-sender TRADER-1)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2W58YQZEAC870M2MKE6QEKS1DPG0RMZZ2BGXSM4.tickets-90stx-mini-raffle-26 transfer u87 tx-sender TRADER-2)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-3)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u14714000 tx-sender TRADER-3)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-4)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.rehab-resort transfer u292 tx-sender TRADER-4)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-5)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u2196000 tx-sender TRADER-5)))
		(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-space-agency transfer u1511 tx-sender TRADER-5)))
		(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-space-agency transfer u581 tx-sender TRADER-5)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-6)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP3D6PV2ACBPEKYJTCMH7HEN02KP87QSP8KTEH335.megapont-robot-component-nft transfer u22780 tx-sender TRADER-6)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-7)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u2629000 tx-sender TRADER-7)))
	    )
            true
        )
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-8)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2ZY7ETKYAN1M7R4HWQ77Q4CVDVH8PVQ41XS0N0S.unforgettable-pepe transfer u78 tx-sender TRADER-8)))
		(unwrap-panic (as-contract (contract-call? 'SP1FWE300K4QMTNDPKV9YDX0NVJ99B138KQ3JN9DT.ukrainian-children transfer u13 tx-sender TRADER-8)))
	    )
            true
        )
