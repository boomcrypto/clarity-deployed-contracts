...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u12454 tx-sender RECEIVER-1)))
	(unwrap-panic (as-contract (stx-transfer? u9077546 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2N3BAG4GBF8NHRPH6AY4YYH1SP6NK5TGCY7RDFA.stacks-mfers transfer u2269 tx-sender TRADER-2)))
	    )
            true
        )
