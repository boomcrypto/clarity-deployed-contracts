...
(define-constant agent-3 'SP3WYBTZWV90G1ZHRRYSQXJ0KQVY589ZENYC6BWJ8)
...
(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u609375 tx-sender agent-1)))
	)
	...
	(unwrap-panic (begin
		(as-contract (stx-transfer? u6396000 tx-sender agent-3)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u359250 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
	(unwrap-panic (begin
		(as-contract (stx-transfer? u7364625 tx-sender agent-2)))
	)
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C.steady-lads transfer u3052 tx-sender agent-3)))
...
(begin
	...
	(unwrap-panic (begin
		...
		(asserts! (is-ok (stx-transfer? u7364625 tx-sender (as-contract tx-sender))) cannot-escrow-stx )
