...
;; Room: https://neoswap.xyz/rooms/drBnUWkPHw0z6gNIwhdG
;; SwapId: TnpHUhRrdrcbwnZlpd4h

;; traders
(define-constant TRADER-1 'SPK67NRTAF3VW1ARJDM7Y2Y88AST0QD3HJ0WJ54T)
...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-2)
            (begin
		(unwrap! (stx-transfer? u5580001 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_STX))
...
(define-private (release-escrow) 
	...
	(unwrap-panic (as-contract (stx-transfer? u5580001 tx-sender TRADER-1)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-2)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (stx-transfer? u5580001 tx-sender TRADER-2)))
