...
(begin
	(unwrap-panic (begin
		...
		(as-contract (stx-transfer? u67067600 tx-sender agent-1)))
	)
	(unwrap-panic (begin
		(as-contract (stx-transfer? u6752400 tx-sender agent-2)))
	)
	(unwrap-panic (as-contract (contract-call? 'SPJW1XE278YMCEYMXB8ZFGJMH8ZVAAEDP2S2PJYG.blocks transfer u3 tx-sender agent-3)))
	(unwrap-panic (begin
		(as-contract (stx-transfer? u2080000 tx-sender agent-0)))
	)
...
(begin        
	...
	(begin
		(unwrap-panic (as-contract (contract-call? 'SPNWZ5V2TPWGQGVDR6T7B6RQ4XMGZ4PXTEE0VQ0S.spacewhales transfer u259 tx-sender agent-2)))
