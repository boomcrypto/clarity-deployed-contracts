...
(define-private (deposit-escrow) 
	...
	(if (is-eq tx-sender TRADER-1)
            (begin
		(unwrap! (contract-call? 'SP2W9RM9KBQFWVSX4F532PSHJ35N2PPS230380CRH.zyoolah transfer u4 tx-sender (as-contract tx-sender)) (err ERR_FAILED_TO_ESCROW_NFT))
	    )
            true
        )
...
(define-private (release-escrow) 
    (begin
	(unwrap-panic (as-contract (contract-call? 'SP2W9RM9KBQFWVSX4F532PSHJ35N2PPS230380CRH.zyoolah transfer u4 tx-sender TRADER-2)))
...
(define-private (return-escrow) 
	...
	(if (is-eq (default-to ERR_IS_NOT_TRADER (map-get? TraderState TRADER-1)) TRADER_STATE_CONFIRMED)
            (begin
		(unwrap-panic (as-contract (contract-call? 'SP2W9RM9KBQFWVSX4F532PSHJ35N2PPS230380CRH.zyoolah transfer u4 tx-sender TRADER-1)))
	    )
            true
        )
