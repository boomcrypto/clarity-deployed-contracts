...
(define-non-fungible-token gnomads uint)
...
(define-data-var artist-address principal 'SPC3270HXDARJHK319ZXPXDZ6KGZATJ8YMEPB5EZ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmeCuf6soTSTq8MrpBQRTfaV6KsHmNHuPhoNVXHaPYcx6b/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? gnomads next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? gnomads token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? gnomads token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? gnomads token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? gnomads id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? gnomads id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? gnomads id) (err ERR-NOT-FOUND)))
