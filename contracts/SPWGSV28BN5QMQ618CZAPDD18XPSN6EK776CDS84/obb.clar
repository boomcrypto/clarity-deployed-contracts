...
(define-non-fungible-token obb uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmdPRsAPCfUsuTsNgY9X8MS2QxSQtTdRTSz1mrPLspbXPY/obb/")
...
(define-data-var public-target-block-height uint u65100)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u8000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? obb id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace obb
  (ok (nft-get-owner? obb id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? obb next-id new-owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? obb id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? obb id) ERR-NOT-FOUND))
