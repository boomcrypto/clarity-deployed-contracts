...
(define-non-fungible-token pixelbull-club uint)
...
;; Internal variables
(define-data-var mint-limit uint u2048)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUankMvPfQ2nzmAmT5VsxVNf8nk61JoZdxH83p3v7UTr5/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? pixelbull-club next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? pixelbull-club token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? pixelbull-club token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? pixelbull-club token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pixelbull-club id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pixelbull-club id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pixelbull-club id) (err ERR-NOT-FOUND)))
