...
(define-non-fungible-token stella uint)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stella next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stella token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stella token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stella token-id)))
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stella id) false)))
