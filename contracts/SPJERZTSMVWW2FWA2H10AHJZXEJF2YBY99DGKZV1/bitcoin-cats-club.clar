...
(define-non-fungible-token bitcoin-cats-club uint)
...
(define-data-var artist-address principal 'SPJERZTSMVWW2FWA2H10AHJZXEJF2YBY99DGKZV1)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmZpUhGv4U4q7ADLYX9BLz7tgSAwEdMtixS3vqUTNyBiV8/json/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u26)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-cats-club next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-cats-club token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-cats-club token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-cats-club token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-cats-club id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-cats-club id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-cats-club id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u18672)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u614200000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u1098500000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u102000000)
