...
(define-non-fungible-token y00ts-cordinals uint)
...
(define-data-var artist-address principal 'SP18FS734T9SGNSK68Z5C6KTRQQY9FA2Y3T636WPV)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmf6zMSDq3RdNRyd694bYtEz6bchH7TqVhqWThTBtCqa4S/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? y00ts-cordinals next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? y00ts-cordinals token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? y00ts-cordinals token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? y00ts-cordinals token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? y00ts-cordinals id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? y00ts-cordinals id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? y00ts-cordinals id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u3143857)
