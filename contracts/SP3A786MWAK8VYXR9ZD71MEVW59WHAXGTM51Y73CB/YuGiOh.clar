...
(define-non-fungible-token YuGiOh uint)
...
(define-data-var base-uri (string-ascii 80) "bafybeie2w2t7436kpoavrhwcgdtpbpqdshkq2f76ooalw5ckw4q2j6nryy/")
(define-data-var mint-price uint u50000)
;; Percent u100 = 1%
(define-data-var royalty-percent uint u250)
(define-data-var mint-limit uint u5)
(define-data-var max-supply uint u12692)
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? YuGiOh next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? YuGiOh next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? YuGiOh id sender recipient)
        success
          (let
            (
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? YuGiOh id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? YuGiOh id) ERR_NO_OWNER))
