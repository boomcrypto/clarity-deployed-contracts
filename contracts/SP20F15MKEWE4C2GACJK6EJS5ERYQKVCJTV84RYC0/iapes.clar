...
(define-non-fungible-token iapes uint)
...
(define-data-var artist-address principal 'SP20F15MKEWE4C2GACJK6EJS5ERYQKVCJTV84RYC0)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmUATVasct5BKaUEVNbcLzq9aD2ZVR6x4x69WN5Yd16e5J/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? iapes next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? iapes token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? iapes token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? iapes token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? iapes id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? iapes id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? iapes id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u47819)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u1254500000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u2770300000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u46000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u262000000)
