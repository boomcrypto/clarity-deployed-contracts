...
(define-non-fungible-token iape uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNZS2Gg1QMLmVksTQPNjKuFsRztnCSrPTi3FcyXoKjXKV/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? iape next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? iape token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? iape token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? iape token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? iape id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? iape id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? iape id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u45834)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u1217500000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u2208300000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u233000000)
