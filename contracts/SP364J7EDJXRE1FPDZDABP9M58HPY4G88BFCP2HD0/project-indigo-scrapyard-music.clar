;; project-indigo-scrapyard-music
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token project-indigo-scrapyard-music uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmbaGBMDW2wju8GX3zpaLqXNgRNw6eM3L1k9nhdbtRWrQN/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? project-indigo-scrapyard-music next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? project-indigo-scrapyard-music token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? project-indigo-scrapyard-music token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? project-indigo-scrapyard-music token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? project-indigo-scrapyard-music id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? project-indigo-scrapyard-music id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? project-indigo-scrapyard-music id) (err ERR-NOT-FOUND)))
