...
(define-non-fungible-token let-it-snow-single uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmNZzGUtLUVzA4JQyb5kZaQJHsWTFpc8cAR8NJcE1sWmaa/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? let-it-snow-single next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? let-it-snow-single token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? let-it-snow-single token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? let-it-snow-single token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? let-it-snow-single id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? let-it-snow-single id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? let-it-snow-single id) (err ERR-NOT-FOUND)))
