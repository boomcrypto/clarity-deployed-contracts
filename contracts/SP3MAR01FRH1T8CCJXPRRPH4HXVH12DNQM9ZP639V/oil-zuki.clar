...
(define-non-fungible-token oil-zuki uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://Qmb8H3xjdxMNHJwS8LyKszuMxcYfMXC2nckQet1ps88wth/oil-zuki/")
...
(define-data-var public-target-block-height uint u64857)
(define-data-var admin principal tx-sender)

(define-data-var stx-cost-per-mint uint u30000000)
;; Get mint limit
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? oil-zuki id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace oil-zuki
  (ok (nft-get-owner? oil-zuki id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? oil-zuki next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? oil-zuki id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? oil-zuki id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? oil-zuki id) ERR-NOT-FOUND))
