...
(define-non-fungible-token bitcoin-color uint)
...
(define-data-var artist-address principal 'SP23M03FMHMF5822GPAH88YERAK7KQ63KRP619MBS)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmYyX5jmqV3yxXbRchG5UkvQ2QH6b23dckfmeg1cbzZhKW/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? bitcoin-color next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-color token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-color token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-color token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-color id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-color id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-color id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u2444)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u341500000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u189900000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u107000000)
