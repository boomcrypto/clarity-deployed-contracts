...
(define-non-fungible-token bitcoin-mimic uint)
...
(define-data-var artist-address principal 'SP3MCCJN36WCGTV0Y38Y3EB42DWR3Z8C3ZB4TNAGR)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSY1M32xknJ5XootFRQ2L6BWkdRC41a64WyVAEakK7RKG/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? bitcoin-mimic next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? bitcoin-mimic token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? bitcoin-mimic token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? bitcoin-mimic token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bitcoin-mimic id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-mimic id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bitcoin-mimic id) (err ERR-NOT-FOUND)))
