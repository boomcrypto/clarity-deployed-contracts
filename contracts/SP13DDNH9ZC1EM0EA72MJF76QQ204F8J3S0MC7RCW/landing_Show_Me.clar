...
(define-non-fungible-token Show_Me uint)
...
(define-constant IPFS-ROOT "ipfs://ipfs/bafybeidtmtl236xaimddy2ckhsprghhlf5kfcpmxurlzzvrpzbjimp64fq/{id}.json")
...
(define-public (mint)
    (let ((token-id (+ u1 (var-get last-token-id))))
        ...
        (nft-mint? Show_Me token-id tx-sender)
    )
)
...
(define-public (burn (id uint))
    (let ((owner (unwrap! (nft-get-owner? Show_Me id) (err ERROR-UNAUTHORIZED))))
        (asserts! (is-eq owner tx-sender) (err ERROR-UNAUTHORIZED))
        (nft-burn? Show_Me id owner)
    )
)
