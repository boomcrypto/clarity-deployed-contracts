...
(define-non-fungible-token crypto435 uint)
...
;; Internal variables
(define-data-var mint-limit uint u435)
(define-data-var last-id uint u1)
(define-data-var total-price uint u100000)
(define-data-var artist-address principal 'SP1K7VZ230F1SZ9M0NTVK77QCMJWB5AZ9ZX35RJYP)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmfJFL2XnSz6ZKDxYodkWQ5MNB4vEZ6CuBxAxJd6GRtTTM/")
(define-data-var mint-paused bool false)
(define-data-var premint-enabled bool false)
(define-data-var sale-enabled bool false)
(define-data-var metadata-frozen bool false)
(define-data-var airdrop-called bool false)
(define-data-var mint-cap uint u435)
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? crypto435 next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? crypto435 token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? crypto435 token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? crypto435 token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? crypto435 id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? crypto435 id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? crypto435 id) (err ERR-NOT-FOUND)))
