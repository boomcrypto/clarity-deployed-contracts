...
(define-non-fungible-token alleygators uint)
...
(define-data-var artist-address principal 'SPE2FWK978RCFZ63BHNQ30NJHDMZKAB1RSVZPPPD)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaAKkWbD4osWX382ARXA6hhNnENHo5Ag2cEtMCKHCRCYL/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? alleygators next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? alleygators token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? alleygators token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? alleygators token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? alleygators id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? alleygators id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? alleygators id) (err ERR-NOT-FOUND)))
