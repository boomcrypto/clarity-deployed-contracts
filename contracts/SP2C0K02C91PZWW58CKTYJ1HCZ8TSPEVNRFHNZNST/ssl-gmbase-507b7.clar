...
(define-read-only (get-name) (contract-call? 'SP2C0K02C91PZWW58CKTYJ1HCZ8TSPEVNRFHNZNST.gmbase get-name))
(define-read-only (get-symbol) (contract-call? 'SP2C0K02C91PZWW58CKTYJ1HCZ8TSPEVNRFHNZNST.gmbase get-symbol))
(define-read-only (get-base-decimals) (contract-call? 'SP2C0K02C91PZWW58CKTYJ1HCZ8TSPEVNRFHNZNST.gmbase get-decimals))
(define-read-only (get-token-uri) (contract-call? 'SP2C0K02C91PZWW58CKTYJ1HCZ8TSPEVNRFHNZNST.gmbase get-token-uri))
(define-read-only (get-balance (who principal)) (if (is-eq who VAULT) (ok (ft-get-balance wrapped-token who)) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SP2C0K02C91PZWW58CKTYJ1HCZ8TSPEVNRFHNZNST.gmbase get-balance who))))))
(define-read-only (get-total-supply) (ok (decimals-to-fixed (unwrap-panic (contract-call? 'SP2C0K02C91PZWW58CKTYJ1HCZ8TSPEVNRFHNZNST.gmbase get-total-supply)))))
...
(define-read-only (get-base-token)
  'SP2C0K02C91PZWW58CKTYJ1HCZ8TSPEVNRFHNZNST.gmbase)
  ...
  (begin
    ...
    (if (is-eq sender VAULT)
			(begin
				...
				(as-contract (contract-call? 'SP2C0K02C91PZWW58CKTYJ1HCZ8TSPEVNRFHNZNST.gmbase transfer (fixed-to-decimals amount) tx-sender recipient memo)))
			(begin
				(and (is-eq recipient VAULT) (try! (ft-mint? wrapped-token amount recipient)))
				(contract-call? 'SP2C0K02C91PZWW58CKTYJ1HCZ8TSPEVNRFHNZNST.gmbase transfer (fixed-to-decimals amount) tx-sender (if (is-eq recipient VAULT) (as-contract tx-sender) recipient) memo)))))
