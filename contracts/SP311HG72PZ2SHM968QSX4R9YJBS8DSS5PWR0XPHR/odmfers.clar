...
(define-non-fungible-token odmfers uint)
...
;; Internal variables
(define-data-var mint-limit uint u515)
(define-data-var last-id uint u1)
(define-data-var total-price uint u28000000)
(define-data-var artist-address principal 'SP311HG72PZ2SHM968QSX4R9YJBS8DSS5PWR0XPHR)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmReF87QpowdbkJQdrYC97pkT5UUtEcPHACM1MPp6JXXsp/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? odmfers next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? odmfers token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? odmfers token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? odmfers token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? odmfers id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? odmfers id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? odmfers id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u40119)
