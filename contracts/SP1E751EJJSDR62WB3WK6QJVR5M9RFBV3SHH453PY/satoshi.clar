...
(define-fungible-token satoshi)
(define-constant TOKEN_SYMBOL "SATOSHI")
(define-constant TICK_NAME "satoshi")
...
(define-read-only (get-balance (address principal))
  (ok (ft-get-balance satoshi address))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (if (is-eq tx-sender sender)
    (begin
      (try! (ft-transfer? satoshi amount sender recipient))
      (print {
        type: "transfer",
        tick: TICK_NAME,
        sender: sender,
...
(define-read-only (get_summary (address principal))
  {
    ...
    sip10_balance: (ft-get-balance satoshi address),
  }
)
...
(define-public (wrap (amount uint))
  (let
    ...
    (ft-mint? satoshi (* amount ONE_COIN) sender)
  )
)
...
(define-public (unwrap (amount uint))
  (let
    ...
    (try! (ft-burn? satoshi (* amount ONE_COIN) sender))
    (print {
      type: "unwrap",
      tick: TICK_NAME,
      sender: sender,
