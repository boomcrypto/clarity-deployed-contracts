...
(define-fungible-token YES)
(define-constant TOKEN_SYMBOL "YES")
(define-constant TICK_NAME "YES")
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (if (is-eq tx-sender sender)
    (begin
      (try! (ft-transfer? YES amount sender recipient))
      (print {
        type: "transfer",
        tick: TICK_NAME,
        sender: sender,
...
(define-read-only (get_summary (address principal))
  {
    ...
    sip10_balance: (ft-get-balance YES address),
  }
)
...
(define-public (wrap (amount uint))
  (let
    ...
    (ft-mint? YES (* amount ONE_COIN) sender)
  )
)
...
(define-public (unwrap (amount uint))
  (let
    ...
    (try! (ft-burn? YES (* amount ONE_COIN) sender))
    (print {
      type: "unwrap",
      tick: TICK_NAME,
      sender: sender,
