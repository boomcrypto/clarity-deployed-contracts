...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance testC account))
)

(define-read-only (get-balance-simple (account principal))
  (ft-get-balance testC account)
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? testC amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    (ft-mint? testC amount recipient)
  )
)

(define-public (burn (amount uint))
  (begin
    (ft-burn? testC amount tx-sender)
  )
)
