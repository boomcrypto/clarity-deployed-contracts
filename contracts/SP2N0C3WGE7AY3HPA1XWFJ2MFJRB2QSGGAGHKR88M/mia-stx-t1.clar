...
(define-public (mia-stx (dy uint) (min_dx (optional uint)))
(let
  (
    (sender tx-sender)
  )
  (try! (contract-call?
    'SP1H1733V5MZ3SZ9XRW9FKYGEZT0JDGEB8Y634C7R.miamicoin-token-v2 transfer dy tx-sender (as-contract tx-sender) none)
  )
  (as-contract
  (let
  (
    ...
    (dx1 (if (> dy1 u0) (try! (alex dy1)) u0))
    (dx2 (if (> dy2 u0) (try! (stsw dy2)) u0))
    (dx (+ dx1 dx2))
  )
