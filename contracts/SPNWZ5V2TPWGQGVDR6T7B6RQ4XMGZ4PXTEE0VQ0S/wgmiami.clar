...
(define-non-fungible-token wgmiami uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcZPP4DnecZUALwS8edvY62duezds55UsojzBXHzquhp4/")

(define-private (mint (new-owner principal) (next-id uint))
    (match (nft-mint? wgmiami next-id new-owner)
            success
              (begin
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (if (and
      ...
      (match (nft-transfer? wgmiami token-id sender recipient)
        success (ok success)
        error (err error))
      (err err-invalid-user)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? wgmiami token-id)))
