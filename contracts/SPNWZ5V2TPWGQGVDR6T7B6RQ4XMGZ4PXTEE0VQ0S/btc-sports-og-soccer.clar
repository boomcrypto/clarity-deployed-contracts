;; btc-sports-og-soccer
;; contractType: public

(impl-trait 'SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.nft-trait.nft-trait)

(define-non-fungible-token btc-sports-og-soccer uint)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/bafybeibwfwxyqgxzlpuwqcef5myvr5ux3xowf7wk7q2yrm5jrdigkbdhma/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? btc-sports-og-soccer next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? btc-sports-og-soccer token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? btc-sports-og-soccer token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? btc-sports-og-soccer token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? btc-sports-og-soccer id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? btc-sports-og-soccer id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? btc-sports-og-soccer id) (err ERR-NOT-FOUND)))
...
(define-public (add-mint-passes (addr principal) (passes uint))
  (let (
      (existing-passes (get-passes addr))
    )
    (asserts! (or (is-eq tx-sender (var-get artist-address)) (is-eq tx-sender DEPLOYER)) (err ERR-NOT-AUTHORIZED))
