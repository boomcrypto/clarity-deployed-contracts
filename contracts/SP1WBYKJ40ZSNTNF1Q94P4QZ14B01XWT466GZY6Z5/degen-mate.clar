...
(define-non-fungible-token degen-mate uint)
...
;; Internal variables
(define-data-var mint-limit uint u146)
(define-data-var last-id uint u1)
(define-data-var total-price uint u50000000)
(define-data-var artist-address principal 'SP1WBYKJ40ZSNTNF1Q94P4QZ14B01XWT466GZY6Z5)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmP8X91TGQG4QPwtu6ptGHKMzcfvB57PskmbQj2JEF6K3A/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? degen-mate next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? degen-mate token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? degen-mate token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? degen-mate token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? degen-mate id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? degen-mate id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? degen-mate id) (err ERR-NOT-FOUND)))
