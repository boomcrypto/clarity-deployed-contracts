...
(define-non-fungible-token solmonna uint)
...
(define-data-var artist-address principal 'SPRP9FG81K1N9DW4H9K18ESPSAJ1R8TGP1JW4KVC)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmRjuaMyvPazeRmMZ3PYJmVxbriUgfhKckJSNT6vUqYrvP/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? solmonna next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? solmonna token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? solmonna token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? solmonna token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? solmonna id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? solmonna id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? solmonna id) (err ERR-NOT-FOUND)))
