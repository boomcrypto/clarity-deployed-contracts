...
(define-data-var admins (list 100 principal) (list 'SP1GR33848GSTMFR955Z77DAB835XYE9FZG19Y7NX 'SP2KAF9RF86PVX3NEE27DFV1CQX0T4WGR41X3S45C 'SP1A32MBZDN1B28ZT5DH6MA739VJ0BFH05CJHCM8W))
...
(define-public (list-item (collection <nft-trait>) (collection-id (string-ascii 256)) (item-id uint) (price uint))
  (let (
    ...
    (asserts! (not (is-eq (get name collection-entry) "null")) (err ERR-PROJECT-NOT-WHITELISTED))
...
(define-public (admin-withdraw-bid (collection-id (string-ascii 256)) (item-id uint) (order uint))
  (let (
    (item (get-item-for-sale collection-id item-id))
    (bid (get-item-bid collection-id item-id order))
    (amount (get offer bid))
...
(define-public (admin-withdraw-collection-bid (collection-id (string-ascii 256)) (order uint))
  (let (
    (bid (get-collection-bid collection-id order))
    (amount (get offer bid))
    (sender tx-sender)
    ...
    (match (as-contract (stx-transfer? (get offer bid) (as-contract tx-sender) (get buyer bid)))
      success (begin
        ...
        (print {
          type: "marketplace",
          action: "admin-withdraw-collection-bid",
          order: order,
          data: { collection-id: collection-id, buyer: (get buyer bid) }
        })
        (ok (get offer bid))
      )
      error (err error)
...
(define-public (admin-withdraw-multiple-bid (collection-id (string-ascii 256)) (order uint))
  (let (
    (bid (get-multiple-bid collection-id order))
    (amount (get offer bid))
    (sender tx-sender)
    ...
    (match (as-contract (stx-transfer? (get offer bid) (as-contract tx-sender) (get buyer bid)))
      success (begin
        ...
        (print {
          type: "marketplace",
          action: "admin-withdraw-multiple-bid",
          order: order,
          trait: trait-name,
          data: { collection-id: collection-id, buyer: (get buyer bid) }
        })
...
(define-public (accept-bid (collection <nft-trait>) (collection-id (string-ascii 256)) (item-id uint) (order uint))
  ...
  (let (
    ...
    (asserts! (is-eq (contract-of collection) (unwrap-panic (get address collection-entry))) (err ERR-NOT-AUTHORIZED))
    (asserts! (is-eq (var-get shutoff-valve) false) (err ERR-NOT-AUTHORIZED))
    (try! (as-contract (stx-transfer? commission (as-contract tx-sender) 'SP1GR33848GSTMFR955Z77DAB835XYE9FZG19Y7NX)))
...
(define-public (buy-item (collection <nft-trait>) (collection-id (string-ascii 256)) (item-id uint))
  (let (
    ...
    (try! (stx-transfer? commission sender 'SP1GR33848GSTMFR955Z77DAB835XYE9FZG19Y7NX))
...
(define-public (shutoff (valve bool))
  (begin
    (asserts! (is-some (index-of (var-get admins) tx-sender)) (err ERR-NOT-AUTHORIZED))
    (var-set shutoff-valve valve)
    (ok true)
  )
)
