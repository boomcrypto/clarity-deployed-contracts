...
(impl-trait .googlier-collateral-types-trait-v1.collateral-types-trait)
...
(define-public (add-debt-to-collateral-type (token (string-ascii 12)) (debt uint))
  (begin
    ;; hettich should be calling this method
    (asserts! (is-eq contract-caller .googlier-hettich-v1-1) (err ERR-NOT-AUTHORIZED))
...
(begin
  ...
  (map-set collateral-types
    { name: "XBTC-A" }
    {
      ...
      token: "xBTC",
      token-type: "xBTC-A",
      token-address: 'SP32X8J03KE3FDJAKN2JR9CQZGW8A517AFAYA217.Wrapped-Bitcoin,
  ...
  (map-set collateral-types
    { name: "GGLR-A" }
    {
      ...
      token: "GGLR",
      token-type: "GGLR-A",
      token-address: 'SP32X8J03KE3FDJAKN2JR9CQZGW8A517AFAYA217.googlier-token,
      url: "https://www.googlier.finance",
      total-debt: u0,
      liquidation-ratio: u150,
      collateral-to-debt-ratio: u250, ;; ~50% LTV
      maximum-debt: u500000000000000, ;; 500M
