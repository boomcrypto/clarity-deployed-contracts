;; @contract Vault Rewards - Claim GGLR vault rewards
;; @version 1.1
...
(define-public (remove-collateral (collateral uint) (user principal))
  (let (
    ...
    ;; Only hettich is allowed to call this method
  ...
  (begin
    ...
    (let (
      ...
      (if (>= pending-rewards u1)
        (begin
          ;; Mint GGLR rewards for user
          (try! (contract-call? .googlier-dao mint-token .googlier-token pending-rewards user))
...
;; Hettich can claim rewards when vault gets liquidated
  ...
  (begin
    ...
    (let (
      ...
      (if (>= pending-rewards u1)
        (begin
          ;; Mint GGLR rewards for user
          (try! (contract-call? .googlier-dao mint-token .googlier-token pending-rewards contract-caller))
...
(define-private (get-rewards-per-block)
  (if (is-eq (var-get vault-rewards-shutdown-activated) true)
    u0
    (contract-call? .googlier-gglr-guardian-v1-1 get-vault-rewards-per-block)
  )
)
