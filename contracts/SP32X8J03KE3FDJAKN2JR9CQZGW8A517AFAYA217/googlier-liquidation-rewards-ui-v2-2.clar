...
(define-private (has-claimed-reward (reward-id uint))
  (let (
    (claimed (get claimed (contract-call? .googlier-liquidation-rewards-v1-2 get-reward-claimed reward-id tx-sender)))
  )
    (if claimed
      true
      (let (
        (share-block (get share-block (contract-call? .googlier-liquidation-rewards-v1-2 get-reward-data reward-id)))
      )
        (if (is-eq share-block u0)
          false
          (let (
            (user-shares (unwrap-panic (contract-call? .googlier-liquidation-pool-v1-1 get-shares-at tx-sender share-block)))
          )
...
(define-public (claim-simple-50-gglr-rewards-of (reward-ids (list 50 uint)))
  (begin
...
(define-public (claim-stx-rewards-of (reward-id uint))
  (contract-call? .googlier-liquidation-rewards-v1-2 claim-rewards-of reward-id .xstx-token .googlier-liquidation-pool-v1-1)
)

(define-public (claim-gglr-rewards-of (reward-id uint))
  (contract-call? .googlier-liquidation-rewards-v1-2 claim-rewards-of reward-id .googlier-token .googlier-liquidation-pool-v1-1)
)

(define-public (claim-xbtc-rewards-of (reward-id uint))
  ;; TODO - UPDATE ADDRESS FOR MAINNET
  (contract-call? .googlier-liquidation-rewards-v1-2 claim-rewards-of reward-id .Wrapped-Bitcoin .googlier-liquidation-pool-v1-1)
)
