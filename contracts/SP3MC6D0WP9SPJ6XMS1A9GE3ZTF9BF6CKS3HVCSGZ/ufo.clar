...
(define-non-fungible-token ufo uint)
...
;; Internal variables
(define-data-var mint-limit uint u493)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmcDY1Sv6yEsg4Xjh47DcwNWd4UTYR1LaFNZYC2vD3mhEq/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? ufo next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? ufo token-id sender recipient)))

;; read-only functions
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ufo token-id)))
