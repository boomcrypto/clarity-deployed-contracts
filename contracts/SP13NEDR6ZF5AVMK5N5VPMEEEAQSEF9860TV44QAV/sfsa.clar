...
(define-non-fungible-token sfsa uint)
...
(define-data-var total-price uint u1600)
(define-data-var artist-address principal 'SP13NEDR6ZF5AVMK5N5VPMEEEAQSEF9860TV44QAV)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qmeap1SCwpLGZeTZPkx7mtFYav5FvEaL7hufPgjFNXNBnq/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? sfsa next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? sfsa token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? sfsa token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sfsa token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sfsa id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sfsa id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sfsa id) (err ERR-NOT-FOUND)))
