...
(define-non-fungible-token pacheca-galileoak uint)
...
(define-data-var total-price uint u1200000000)
(define-data-var artist-address principal 'SP3DYDDE2QX2PAQ26462D6SZ8R1D6DNNEHDF1Q6KV)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmdyBpxSDBoy6yCuAomLihqqZzN1zwMB7VKebhR9XCVUSm/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (or (is-eq (var-get mint-limit) u0) (<= next-id (var-get mint-limit)))
    (begin
      (unwrap! (nft-mint? pacheca-galileoak next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? pacheca-galileoak token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? pacheca-galileoak token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? pacheca-galileoak token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pacheca-galileoak id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pacheca-galileoak id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pacheca-galileoak id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u4530377)
