...
(define-non-fungible-token roll-the-dice uint)
...
;; Internal variables
(define-data-var mint-limit uint u216)
...
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmatF2Ymrdf44H4ybAKrgZsH976SWZwRQqxhQfvppeVXYQ/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? roll-the-dice next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? roll-the-dice token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? roll-the-dice token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? roll-the-dice token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? roll-the-dice id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? roll-the-dice id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? roll-the-dice id) (err ERR-NOT-FOUND)))
...
(define-public (admin-airdrop)
  (let
    ...
    (begin
      ...
      (try! (nft-mint? roll-the-dice (+ last-nft-id u0) 'SP9Q5F8EQRZ3N5DE80QW8AVAQ08JGAYZDMAMTG5C))
