...
(define-read-only (is-valid-chain (chain (buff 256)))
    (contract-call? .gov-matic is-valid-chain chain)
)

(define-read-only (is-validator (validator principal))
	(contract-call? .gov-matic is-validator validator)
)

(define-read-only (get-required)
    (contract-call? .gov-matic get-min-confirmation)
)

(define-read-only (get-chain-fee (chain (buff 256)))
    (contract-call? .gov-matic get-chain-fee chain)
)

(define-read-only (get-fee-governance)
    (contract-call? .gov-matic get-fee-governance)
)

(define-read-only (get-tax-receiver)
    (contract-call? .gov-matic get-tax-receiver)
)

(define-read-only (get-tax-rate)
    (contract-call? .gov-matic get-tax-rate)
)
...
;; ================ constructor ================
(var-set name "Orbit Bridge Stacks Matic")
(var-set symbol "oMATIC")
(var-set token-decimals u18)
(var-set governance-id 0xaf9c9bf1d10f96432ef783c6ff34815d5e22e5083554a9940cf5dcbbdce7cfe7)
