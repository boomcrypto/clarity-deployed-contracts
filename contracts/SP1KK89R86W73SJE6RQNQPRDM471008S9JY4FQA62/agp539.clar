...
(define-private (reset-liquidity-iter (pool-id uint) (prior (response bool uint)))
	(let (
			...
			(token-supply (unwrap-panic (contract-call? 'SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-amm-pool-v2-01 get-total-supply-fixed pool-id)))
			(update-flag (is-eq (/ token-supply u1000000) u0))
			(updated-pool-details (if update-flag (merge pool-details { balance-x: u0, balance-y: u0, total-supply: u0, start-block: MAX_UINT }) (merge pool-details { start-block: MAX_UINT }))))
		...
		(print { pool-id: pool-id, pool-details: updated-pool-details, updated: update-flag })
		(ok true)))
