...
(define-private (process-claim-iter (recipient { address: (buff 128), chain-id: (optional uint) }) (details { launch-id: uint, chain-id: (optional uint), token-trait: <ft-trait> }))
	(let (
		(claim-amt (unwrap-panic (map-get? tmp-claim recipient)))) ;; @dev it cannot be none
		(as-contract (unwrap-panic (contract-call? 'SP673Z4BPB4R73359K9HE55F2X91V5BJTN5SXZ5T.cross-router-v2-03 route claim-amt (list (get token-trait details)) (list) (get token-trait details) none { address: (unwrap-panic (get-tickets-dest-or-fail (get launch-id details) recipient)), chain-id: (get chain-id details) })))
		(map-set tmp-claim recipient u0)
		(print { type: "claim", launch-id: (get launch-id details), recipient: recipient, claim: claim-amt })
		details))
