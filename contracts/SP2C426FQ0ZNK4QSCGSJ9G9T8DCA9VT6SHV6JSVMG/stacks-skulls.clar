...
(define-non-fungible-token stacks-skulls uint)
...
(define-data-var total-price uint u160000)
(define-data-var artist-address principal 'SP2C426FQ0ZNK4QSCGSJ9G9T8DCA9VT6SHV6JSVMG)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmSPbMwHnmEut48u1YCLd9oFVH7ZcQJYoGfUNs9xGHBRA4/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? stacks-skulls next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? stacks-skulls token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? stacks-skulls token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? stacks-skulls token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? stacks-skulls id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? stacks-skulls id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? stacks-skulls id) (err ERR-NOT-FOUND)))
