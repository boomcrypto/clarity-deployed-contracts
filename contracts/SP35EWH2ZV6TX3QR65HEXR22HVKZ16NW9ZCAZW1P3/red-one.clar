...
(define-non-fungible-token red-one uint)
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? red-one item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? red-one item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? red-one item-id)))
      (begin
        (try! (nft-burn? red-one item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? red-one token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? red-one id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? red-one id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? red-one id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP35EWH2ZV6TX3QR65HEXR22HVKZ16NW9ZCAZW1P3 u500 "red one" u5000000 u65284 u65514 "ipfs://Qmdww5H3cryTbxpfRGwx4oMGo9ZvRwRWawS24XXJbBDCRd/red-one/1.json"))
  (ok true)
)
