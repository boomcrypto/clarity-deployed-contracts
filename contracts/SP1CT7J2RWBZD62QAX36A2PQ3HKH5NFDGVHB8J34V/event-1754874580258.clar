;; ======================
;; TITLE: 20 Questions with GPSC
;; VERSION: 1.0.0
;; SUMMARY: Ask GPSC the questions you've always wanted to ask. 80% of all sales go to getting the very best orange slices for Flag Football.
;; ======================
...
(define-constant contract-name "event-1754874580258")
(define-constant asset-identifier "event-1754874580258")
(define-constant default-name "20 Questions with GPSC")
(define-constant default-description "Ask GPSC the questions you've always wanted to ask. 80% of all sales go to getting the very best orange slices for Flag Football.")
(define-constant thumbnail-uri "https://9ok0w2gk6hipqasu.public.blob.vercel-storage.com/20%20Questions...-OjVCOTpquoH5iH5Nh0o17QWw71QNtx.jpg")
(define-constant event-type u3)
...
(define-constant token-uri-template (concat api-host "/api/contracts/SP1CT7J2RWBZD62QAX36A2PQ3HKH5NFDGVHB8J34V/event-1754874580258/tokens/"))

;; Creator Fee Schedule Constants
(define-constant creator-info { 
  creator-1: { percent-of-100: u20, name: "GPSC", address: 'SP2RNHHQDTHGHPEVX83291K4AQZVGWEJ7WCQQDA9R }, creator-2: { percent-of-100: u80, name: "Nephew GP", address: 'SP2G3EMMZ7SPJFC2NCT6G30JP5DDNHG6YFY0WVSZV }
})
...
(define-non-fungible-token event-1754874580258 uint)
...
(define-data-var event-details {
  ...
  contact: (string-ascii 128)
    ...
    name: "",
    start-time: u1755558000,
    end-time: u1755565200,
    address-one: "",
    address-two: "",
    city: "",
    state: "",
...
(define-data-var min-reservation-fee uint u10000000)
(define-data-var max-reservations uint u20)
(define-data-var open-reservations uint u0)
(define-data-var is-transferable bool false)
...
(define-data-var min-reservation-fee-sbtc uint u10000)
...
(define-public (burn-reservation (reservation-id uint))
    (begin 
        ...
        (if (map-delete reservation-data reservation-id)
            ...
            (ok (unwrap-panic (nft-burn? event-1754874580258 reservation-id tx-sender)))
            )
            (ok false)
        )
    )
...
(define-public (transfer (reservation-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? event-1754874580258 reservation-id sender recipient)
  )
)
...
(define-read-only (get-owner (reservation-id uint))
  (ok (nft-get-owner? event-1754874580258 reservation-id))
)
...
(define-private (mint (new-owner principal) (reservation-id uint))
  (begin 
      (asserts! (is-none (map-get? reservation-data reservation-id)) err-reservation-id-exists)
      (nft-mint? event-1754874580258 reservation-id new-owner)
  )
)
...
(define-private (create-launch-uri (reservation-id uint))
  (concat "https://deorganized-events-api.vercel.app/api/contracts/SP1CT7J2RWBZD62QAX36A2PQ3HKH5NFDGVHB8J34V/event-1754874580258/reservations/" (int-to-ascii reservation-id))
)
...
(define-private (distribute-fees (total-fees uint))
  (begin
    (if (not (is-eq tx-sender (get address (get creator-1 creator-info)))) (unwrap-panic (stx-transfer? (* (/ total-fees u100) (get percent-of-100 (get creator-1 creator-info))) tx-sender (get address (get creator-1 creator-info)))) true) (if (not (is-eq tx-sender (get address (get creator-2 creator-info)))) (unwrap-panic (stx-transfer? (* (/ total-fees u100) (get percent-of-100 (get creator-2 creator-info))) tx-sender (get address (get creator-2 creator-info)))) true)
  )
)
...
(define-private (distribute-fees-sbtc (total-fees uint))
  (begin
    (if (not (is-eq tx-sender (get address (get creator-1 creator-info)))) (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer (* (/ total-fees u100) (get percent-of-100 (get creator-1 creator-info))) tx-sender (get address (get creator-1 creator-info)) none)) true) (if (not (is-eq tx-sender (get address (get creator-2 creator-info)))) (try! (contract-call? 'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token transfer (* (/ total-fees u100) (get percent-of-100 (get creator-2 creator-info))) tx-sender (get address (get creator-2 creator-info)) none)) true) (ok true)
  )
)
