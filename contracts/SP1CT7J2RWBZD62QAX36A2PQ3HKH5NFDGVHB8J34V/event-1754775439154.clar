;; ======================
;; TITLE: Coding with Trip (Week 3)
;; VERSION: 1.0.0
;; SUMMARY: Episode of Coding with Trip Let's code for a cause! 10 donation slots available, first come first serve.
;; ======================
...
(define-constant contract-name "event-1754775439154")
(define-constant asset-identifier "event-1754775439154")
(define-constant default-name "Coding with Trip (Week 3)")
(define-constant default-description "Episode of Coding with Trip Let's code for a cause! 10 donation slots available, first come first serve.")
(define-constant thumbnail-uri "https://9ok0w2gk6hipqasu.public.blob.vercel-storage.com/_Coding%20with%20Trip...-4ci6eV8VqEdufWW5Z9sQpbuoHFRj5x.jpg")
(define-constant event-type u4)
...
(define-constant token-uri-template (concat api-host "/api/contracts/SP1CT7J2RWBZD62QAX36A2PQ3HKH5NFDGVHB8J34V/event-1754775439154/tokens/"))

;; Creator Fee Schedule Constants
(define-constant creator-info { 
  creator-1: { percent-of-100: u100, name: "tripnmonkey", address: 'SP1CT7J2RWBZD62QAX36A2PQ3HKH5NFDGVHB8J34V }
})
...
(define-non-fungible-token event-1754775439154 uint)
...
(define-data-var max-reservations uint u5)
...
(define-public (burn-reservation (reservation-id uint))
    (begin 
        ...
        (if (map-delete reservation-data reservation-id)
            ...
            (ok (unwrap-panic (nft-burn? event-1754775439154 reservation-id tx-sender)))
            )
            (ok false)
        )
    )
...
(define-public (transfer (reservation-id uint) (sender principal) (recipient principal))
  (begin
    ...
    ;; not sure what to check about the recipient?
    ;; #[allow(unchecked_data)]
    (unwrap-panic (nft-transfer? event-1754775439154 reservation-id sender recipient))
...
(define-read-only (get-owner (reservation-id uint))
  (ok (nft-get-owner? event-1754775439154 reservation-id))
)
...
(define-private (mint (new-owner principal) (reservation-id uint))
  (begin 
      (asserts! (is-none (map-get? reservation-data reservation-id)) err-reservation-id-exists)
      (nft-mint? event-1754775439154 reservation-id new-owner)
  )
)
...
(define-private (create-launch-uri (reservation-id uint))
  (concat "https://deorganized-events-api.vercel.app/api/contracts/SP1CT7J2RWBZD62QAX36A2PQ3HKH5NFDGVHB8J34V/event-1754775439154/reservations/" (int-to-ascii reservation-id))
)
