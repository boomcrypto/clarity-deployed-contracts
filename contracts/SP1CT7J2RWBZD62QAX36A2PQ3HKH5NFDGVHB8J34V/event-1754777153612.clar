...
;; VERSION: 1.0.0
;; SUMMARY: 10 donation slots available, first come first serve.
;; ======================
...
(define-constant contract-name "event-1754777153612")
(define-constant asset-identifier "event-1754777153612")
(define-constant default-name "Coding for a Cause")
(define-constant default-description "10 donation slots available, first come first serve.")
...
(define-constant token-uri-template (concat api-host "/api/contracts/SP1CT7J2RWBZD62QAX36A2PQ3HKH5NFDGVHB8J34V/event-1754777153612/tokens/"))
...
(define-non-fungible-token event-1754777153612 uint)
...
(define-data-var min-reservation-fee uint u1000000)
(define-data-var max-reservations uint u10)
...
(define-data-var min-reservation-fee-sbtc uint u1000)
...
(define-public (burn-reservation (reservation-id uint))
    (begin 
        ...
        (if (map-delete reservation-data reservation-id)
            ...
            (ok (unwrap-panic (nft-burn? event-1754777153612 reservation-id tx-sender)))
            )
            (ok false)
        )
    )
...
(define-public (transfer (reservation-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? event-1754777153612 reservation-id sender recipient)
  )
)
...
(define-read-only (get-owner (reservation-id uint))
  (ok (nft-get-owner? event-1754777153612 reservation-id))
)
...
(define-private (mint (new-owner principal) (reservation-id uint))
  (begin 
      (asserts! (is-none (map-get? reservation-data reservation-id)) err-reservation-id-exists)
      (nft-mint? event-1754777153612 reservation-id new-owner)
  )
)
...
(define-private (create-launch-uri (reservation-id uint))
  (concat "https://deorganized-events-api.vercel.app/api/contracts/SP1CT7J2RWBZD62QAX36A2PQ3HKH5NFDGVHB8J34V/event-1754777153612/reservations/" (int-to-ascii reservation-id))
)
