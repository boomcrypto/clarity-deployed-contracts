...
(define-public (claim-pending-rewards (amountIn uint))
  (let (
    (b1 (try! (swap-wstx-wusd-velar amountIn)))
    (b2 (try! (swap-xusd-susdt-alex b1)))
    (b3 (try! (swap-susdt-wstx-alex b2)))
  )
    (print { b1: b1, b2: b2, b3: b3 })
    (begin
      (asserts! (> b3 amountIn) (err ERR-COOLDOWN))
...
(define-public (get-pending-rewards (amountIn uint))
  (let (
    (b1 (try! (swap-wstx-susdt-alex amountIn)))
    (b2 (try! (swap-susdt-xusd-alex b1)))
    (b3 (try! (swap-wusd-wstx-velar b2)))
  )
    (print { b1: b1, b2: b2, b3: b3 })
    (begin
      (asserts! (> b3 amountIn) (err ERR-COOLDOWN))
...
(define-public (swap-xusd-susdt-alex (dx uint))
...
(define-public (swap-susdt-xusd-alex (dx uint))
...
(define-public (swap-wstx-susdt-alex (dx uint))
...
(define-public (swap-susdt-wstx-alex (dx uint))
