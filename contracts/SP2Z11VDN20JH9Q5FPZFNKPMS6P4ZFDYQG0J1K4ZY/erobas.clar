...
(define-non-fungible-token erobas uint)
...
;; Internal variables
(define-data-var mint-limit uint u142)
(define-data-var last-id uint u1)
(define-data-var total-price uint u20000000)
(define-data-var artist-address principal 'SP2Z11VDN20JH9Q5FPZFNKPMS6P4ZFDYQG0J1K4ZY)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmaGZrvucbiBuZqwkdHC4fJbjxnB2agFyGX6enE7TyxFXY/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? erobas next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? erobas token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? erobas token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? erobas token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? erobas id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? erobas id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? erobas id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-usda uint u25000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u19500000000)
