...
(define-non-fungible-token sl-spot-for-where-is-dr-suss- uint)
...
(define-public (auction-ended (item-id uint))
  (let (
    ...
    (begin
        ...
        (try! (nft-mint? sl-spot-for-where-is-dr-suss- item-id buyer))
...
(define-public (burn (item-id uint))
  (let (
    ...
    (asserts! (not (is-eq (nft-get-owner? sl-spot-for-where-is-dr-suss- item-id) none)) (err ERR-TOKEN-NOT-MINTED))
    (if (is-eq tx-sender (unwrap-panic (nft-get-owner? sl-spot-for-where-is-dr-suss- item-id)))
      (begin
        (try! (nft-burn? sl-spot-for-where-is-dr-suss- item-id tx-sender))
        (ok true)
      )
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? sl-spot-for-where-is-dr-suss- token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? sl-spot-for-where-is-dr-suss- id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? sl-spot-for-where-is-dr-suss- id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? sl-spot-for-where-is-dr-suss- id) (err ERR-NOT-FOUND)))
...
(begin
  (try! (set-item u1 'SP2F2KH0RVX6GF1Y9FWMMSR9RHG0TW3NN72D724NX u1000 " SL spot for Where is Dr. Suss (1/2) " u690000 u64326 u64344 "ipfs://QmUkDyBYJaxuxmcdnxBddBz8ptufWjGHqYoY4dZw2s9q7P/sl-spot-for-where-is-dr-suss-/1.json"))
  (ok true)
)
