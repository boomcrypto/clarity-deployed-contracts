...
(define-non-fungible-token bird-d uint)
...
(define-data-var base-uri (string-ascii 100) "ipfs://QmZTj3VTPXb1uMkqc9YeRpfHoPKvrW89PqmDfd6nzmRNuS/bird-d/")
...
(define-data-var public-target-block-height uint u73133)
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? bird-d id sender recipient)
        success
          (let
...
(define-read-only (get-owner (id uint))
  ;; Make sure to replace bird-d
  (ok (nft-get-owner? bird-d id)))
...
(define-private (mint (new-owner principal))
    (let (
      ...
      (match (nft-mint? bird-d next-id new-owner)
        success
        (let
...
(define-public (burn (id uint) (owner principal))
    ...
    (match (nft-burn? bird-d id owner)
        success
        (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? bird-d id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? bird-d id) ERR-NOT-FOUND))
