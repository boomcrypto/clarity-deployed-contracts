...
(define-data-var PRICE_STX uint u2000000) ;; Price in STX
...
(define-private (wl-mint (token <sip-010-trait>) (price uint))
  ;; Create the new token ID by incrementing the last minted ID.
  (let (
    (id (+ (var-get last-token-id) u1)) 
    )
...
(define-public (wl-mint-many (token <sip-010-trait>) (quantity uint)) 
    (let (
...
(define-private (wl-mint-single (token {token: <sip-010-trait>, price: uint}))
  (wl-mint (get token token) (get price token))
)
