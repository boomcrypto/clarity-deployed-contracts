...
(define-non-fungible-token prismatic-gem uint)
...
(define-data-var artist-address principal 'SP90JFVBK7YYK4529SMPJE0VJBJE9DCT5WZKHB36)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmVX8r5iLWVpErHPwqNfKShmvNjXGCSaexS9Y1w46hubxo/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? prismatic-gem next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? prismatic-gem token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? prismatic-gem token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? prismatic-gem token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? prismatic-gem id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? prismatic-gem id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? prismatic-gem id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u116238)
...
;; Alt Minting Default
(define-data-var total-price-usda uint u44000000)
...
;; Alt Minting Default
(define-data-var total-price-mia2 uint u145854000000)
...
;; Alt Minting Default
(define-data-var total-price-nyc2 uint u104551000000)
...
;; Alt Minting Default
(define-data-var total-price-banana uint u333000000)
...
;; Alt Minting Default
(define-data-var total-price-slime uint u4253000000)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u23700000000)
