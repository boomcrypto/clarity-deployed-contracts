...
(define-non-fungible-token checkapes-origins uint)
...
(define-data-var artist-address principal 'SPKPQHMB5MNBNYVR3ZVAPXVM9Z6F76018T1KT225)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/Qma1rEgMErqRbgCr3CB7EZJzCVz9KVLE7L1zWU5Rf5P5ni/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? checkapes-origins next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? checkapes-origins token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? checkapes-origins token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? checkapes-origins token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? checkapes-origins id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? checkapes-origins id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? checkapes-origins id) (err ERR-NOT-FOUND)))
...
;; Alt Minting Default
(define-data-var total-price-xbtc uint u16616)
...
;; Alt Minting Default
(define-data-var total-price-alex uint u28600000000)
