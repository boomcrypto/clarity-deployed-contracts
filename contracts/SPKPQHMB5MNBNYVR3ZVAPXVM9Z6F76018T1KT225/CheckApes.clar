...
(define-non-fungible-token CheckApes uint)
...
(define-data-var base-uri (string-ascii 80) "bafybeicrikf4ka4ruuipjbmeo7zprzlvtbkqgtuzgfhlm7jcliegu4xbpe/")
(define-data-var mint-price uint u15000000)
;; Percent u100 = 1%
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin 
          ...
          (try! (nft-mint? CheckApes next-id tx-sender))
...
(define-public (airdrop (recipient principal))
    ...
    (
      ...
      (current-balance (get-balance tx-sender))
        ...
        (begin
          (unwrap! (stx-transfer? fee tx-sender LMNFT) ERR_TRANSFER_FAIL)
          (try! (nft-mint? CheckApes next-id recipient))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? CheckApes id sender recipient)
        success
          (let
            (
...
(define-private (is-sender-owner (id uint))
  (let 
    (
      (owner (unwrap! (nft-get-owner? CheckApes id) false))
    )
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      (owner (unwrap! (nft-get-owner? CheckApes id) ERR_NO_OWNER))
