...
(define-non-fungible-token fds uint)
...
(define-data-var total-price uint u100)
(define-data-var artist-address principal 'ST12CA4V7ZF7G4VF8JMR0ACD19PFDD1H8ERPWX2AQ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmQ1bVZcNw1PTQLtrxBzCqGjo13jCnu5gdpZdPAWPfbnf3/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? fds next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? fds token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? fds token-id) false)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? fds id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? fds id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? fds id) (err ERR-NOT-FOUND)))
...
(map-set mint-passes 'STHM2TEZ2JJ7GZW1DFF9C2SSJDRRCHG4JG0NM6EJ u10)
(map-set mint-passes 'ST2MFEAMJEXVYTPV7CRNM4G846CEZTSS7MYYFENSD u4)
(map-set mint-passes 'ST10M6HSR04FAB5NZSRRE268DVWFV9BBDX44EXTGR u4)
