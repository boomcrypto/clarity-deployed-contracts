...
(define-non-fungible-token pointillist-portraits uint)
...
(define-data-var artist-address principal 'SPB2JYEZN7RYPYN7KENJ9DHC7HDJBBYV8YGTGHXJ)
(define-data-var ipfs-root (string-ascii 80) "ipfs://ipfs/QmT3VzALdNyyfeL1rAsfC6zoULJSsGcz9THSztrLZWtk1n/json/")
...
(define-private (mint-many-iter (ignore bool) (next-id uint))
  (if (<= next-id (var-get mint-limit))
    (begin
      (unwrap! (nft-mint? pointillist-portraits next-id tx-sender) next-id)
      (+ next-id u1)    
    )
    next-id))
...
(define-public (burn (token-id uint))
  (begin 
    ...
    (nft-burn? pointillist-portraits token-id tx-sender)))

(define-private (is-owner (token-id uint) (user principal))
    (is-eq user (unwrap! (nft-get-owner? pointillist-portraits token-id) false)))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? pointillist-portraits token-id)))
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? pointillist-portraits id sender recipient)
    success
      (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? pointillist-portraits id) false)))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? pointillist-portraits id) (err ERR-NOT-FOUND)))
