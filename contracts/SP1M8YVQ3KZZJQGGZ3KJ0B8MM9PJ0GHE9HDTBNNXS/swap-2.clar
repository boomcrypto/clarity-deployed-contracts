...
(define-private (collect-fee (token <ft-trait>) (fee uint))
    (if (> fee u0)
        (contract-call? token transfer-fixed fee tx-sender .swap-1 none)
        (ok true)
    )
)
...
(define-public (swap-helper-with-fee (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (factor uint) (dx uint) (min-dy (optional uint)) (swap-fee (optional uint)))
    (let (
        ...
        (try! (collect-fee token-x-trait fee))
        (print {
            dx: dx,
            fee: fee,
...
(define-public (swap-helper-b-with-fee (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (token-z-trait <ft-trait>) (token-w-trait <ft-trait>) (factor-x uint) (factor-y uint) (factor-z uint) (dx uint) (min-dw (optional uint)) (swap-fee (optional uint)))
    (let (
        (fee (mul-up dx (default-to (var-get DEFAULT_FEE) swap-fee)))
        (dx-net-fees (if (<= dx fee) u0 (- dx fee)))
    )   
...
(define-public (swap-helper-c-with-fee (token-x-trait <ft-trait>) (token-y-trait <ft-trait>) (token-z-trait <ft-trait>) (token-w-trait <ft-trait>) (token-v-trait <ft-trait>) (factor-x uint) (factor-y uint) (factor-z uint) (factor-w uint) (dx uint) (min-dv (optional uint)) (swap-fee (optional uint)))
    (let (
        (fee (mul-up dx (default-to (var-get DEFAULT_FEE) swap-fee)))
        (dx-net-fees (if (<= dx fee) u0 (- dx fee)))
    )   
...
(define-private (withdrawal-one (token-and-amount { token: <ft-trait>, amount: uint, recipient: principal }))
    (let (
...
(define-public (withdraw-fees-many (tokens-and-amounts (list 200 {token: <ft-trait>, amount: uint, recipient: principal})))
    (begin
        (try! (assert-contract-owner))
        (fold check-err 
            (map withdrawal-one tokens-and-amounts)
            (ok true)
        )
    )
)
