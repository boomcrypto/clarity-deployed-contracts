...
(define-constant MAX_BLOCKS_PER_ROUND u4) ;;TODO u36 6 hours
(define-constant DRAW_CD_BLOCKS u1)       ;; 0.5 hour
...
(define-read-only (get-bet-summary (bet-type uint) (round uint))
  (let
    (
      ...
      (start-time (if (> block-height start-at) (default-to u0 (get-block-info? time start-at)) u0))
      (end-at (default-to u0 (get end-at bet-summary)))
      (end-time (if (> block-height end-at) (default-to u0 (get-block-info? time end-at)) u0))
    )
    {
      bet-type: bet-type,
      round: round,
