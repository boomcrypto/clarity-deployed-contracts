...
(define-constant MAX_PLAYERS u400)
...
(define-constant ONE_ROUND_DURATION_BLOCKS u24) ;;TODO u36 6 hours
(define-constant DRAW_CD_BLOCKS u3)            ;;TODO u3 ;; 0.5 hour
(define-constant CANCEL_CD_BLOCKS u72)         ;;TODO u48 ;; 12 hours
...
(define-map bet-records-compress-map
  { bet-type: uint, round: uint }
  (list 400 { p: principal, v: int }) ;; (bet-value + shares*10000 + |win-num|*10000000000) * (win-num>0 ? 1 : -1)
)
...
(define-public (bet (bet-type uint) (bet-round uint) (bet-v uint) (bet-s uint) (stx-cost uint))
  (let
    ...
    (asserts! (and (<= v-num MAX_PLAYERS) (<= s-num MAX_PLAYERS)) (err ERR_EXCEED_MAX_PLAYERS_COUNT))
    ...
    ;; if (last-round has draw):
    ;;    if last round has not been compressed, compress it
    ;; else
    ...
    (let
      ...
      (match last-bet-summary-tuple-opt last-bet-summary-tuple
        (if (> (get rand-num last-bet-summary-tuple) u0)
          (match (map-get? bet-records-compress-map { bet-type: bet-type, round: last-round }) com-records
            true  ;; already compressed
            (compress-records bet-type last-round)
          )
          ;;;
          (begin
            ;; try mark end-time
...
(define-private (compress-records (bet-type uint) (round uint))
  (begin
    ...
    (let
      (
        (record-opt-list (map compress-record LIST_400))
...
(define-read-only (get-cur-bet-summary (bet-type uint))
  (let
    (
      ...
      (start-time (default-to u0 (get start-time bet-summary)))
      (real-start-time (if (> start-time u0) start-time (default-to u0 (get-block-info? time start-at))))
    )
    (ok {
      round: round,
      bet-summary: bet-summary,
      start-time: real-start-time
    })
  )
)
...
(define-read-only (get-bet-record2 (bet-type uint) (round uint) (index uint))
