(impl-trait .pure-nft-trait.pure-nft-trait)
...
(define-constant ERR-STX-UNENOUGH u1008)
(define-constant ERR-INVALID-LIST u1009)

(define-non-fungible-token Cyberpunk2140 uint)
...
(define-read-only (get-owner (id uint))
  (ok (nft-get-owner? Cyberpunk2140 id))
)
...
(define-public (set-token-uri (uri (string-ascii 256)))
  (ok (and (not (var-get m-collection-data-frozen)) (is-eq contract-caller CREATOR) (var-set m-token-uri uri)))
)

(define-public (set-collection-attribute (value (string-utf8 666)))
  (ok (and (not (var-get m-collection-data-frozen)) (is-eq contract-caller CREATOR) (var-set m-collection-attribute value)))
)

(define-public (set-collection-icon-data (value (buff 128000)))
  (ok (and (not (var-get m-collection-data-frozen)) (is-eq contract-caller CREATOR) (var-set m-collection-icon-data value)))
)

(define-public (set-tokens (values (list 8 { id: uint, data: (buff 128000), attribute: (string-utf8 666) })))
  (ok (and (not (var-get m-collection-data-frozen)) (is-eq contract-caller CREATOR) (begin (map set-token values) true)))
)
...
;;; mint
(define-data-var m-mint-price uint u99000000)
...
(define-public (set-mint-price (price uint))
  (ok (and (is-eq contract-caller CREATOR) (<= price u100000000) (var-set m-mint-price price)))
)
...
(define-public (mint)
  (let
    ...
    (nft-mint? Cyberpunk2140 rand-id tx-sender)
  )
)
...
(define-data-var team-address principal 'SP3H8FZFQZTYGYBJEMBWH53WN0VBEEBBTGZ8M6Y8C)
(define-data-var royalty-percent uint u300)
(define-map map-market uint {price: uint, commission: principal, royalty: uint})

(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? Cyberpunk2140 id sender recipient)
    success
      (ok 
        (and
...
(define-private (is-sender-owner (id uint))
  (is-eq (nft-get-owner? Cyberpunk2140 id) (some tx-sender))
)
...
(define-public (list-in-ustx (id uint) (price uint) (comm-trait <commission-trait>))
  (let
    ...
    (asserts! (and (>= price u100000) (<= price u10000000000000)) (err ERR-INVALID-VALUE))
...
(define-public (buy-in-ustx (id uint) (comm-trait <commission-trait>))
  (let 
    (
      ...
      (owner (unwrap! (nft-get-owner? Cyberpunk2140 id) (err ERR-NOT-FOUND)))
...
(define-public (set-partner-percent (percent uint))
  (ok (and (is-eq contract-caller (var-get partner-address)) (<= percent u1000) (var-set partner-percent percent)))
)
...
(define-public (set-royalty-percent (percent uint))
  (ok (and (is-eq tx-sender (var-get team-address)) (<= percent u500) (var-set royalty-percent percent)))
)
...
(define-private (loop-award (i uint) (ud { index: uint, step: uint, award: uint }))
  (let
    (
      ...
      (owner (unwrap! (nft-get-owner? Cyberpunk2140 id) ud))
    )
